...
char const* EnvName = "ASCMD"; /* Environment-Variable fuer Default-
                                  Parameter */
...
char const* DivideChars;      /* Trennzeichen fuer Parameter. Inhalt Read Only! */
Boolean     HasAttrs;         /* Opcode hat Attribut */
char const* AttrChars;        /* Zeichen, mit denen Attribut abgetrennt wird */
...
char const* PCSymbol; /* Symbol, womit Programmzaehler erreicht wird. Inhalt Read Only! */
...
Boolean SwitchIsOccupied,              /* TRUE: SWITCH/PAGE/SHIFT ist Prozessorbefehl */
        PageIsOccupied, ShiftIsOccupied;
...
char const* pCommentLeadIn; /* list of comment lead-in sequences */
...
void AppendArg(size_t ReqSize) {
    ...
    if (ArgCnt >= AllocArgCnt) {
        size_t NewArgStrSize
                = sizeof(*ArgStr) * (ArgCnt + 1); /* one more, [0] is unused */
        int z;

        ArgStr = ArgStr ? (tStrComp*)realloc(ArgStr, NewArgStrSize)
                        : (tStrComp*)malloc(NewArgStrSize);
