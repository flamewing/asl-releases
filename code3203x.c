...
typedef struct {
    GenOrder const* pOrder;
    Boolean         Is3;
    ShortInt        Src2Mode, Src1Mode, DestMode;
    Word            Src2Part, Src1Part, DestPart;
} tGenOrderInfo;

static CPUVar CPU32030, CPU32031, CPU32040, CPU32044;
...
static char const *C4XRegs[] = {"DP",  "IR0", "IR1", "BK", "SP", "ST", "DIE",
                                "IIE", "IIF", "RS",  "RE", "RC", NULL},
                  *C3XRegs[] = {"DP", "IR0", "IR1", "BK", "SP", "ST", "IE",
                                "IF", "IOF", "RS",  "RE", "RC", NULL},
                  **CxxRegs;
static char const* ExpRegs[] = {"IVTP", "TVTP", NULL};

static Boolean Is4x(void) {
    return MomCPU >= CPU32040;
}

static Boolean DecodeReg(char const* Asc, Byte* Erg) {
    if ((as_toupper(*Asc) == 'R') && (strlen(Asc) <= 3) && (strlen(Asc) >= 2)) {
        Boolean OK;

        *Erg = ConstLongInt(Asc + 1, &OK, 10);

        /* For C3x, tolerate R8...R27 as aliases for other registers for backward
           compatibility. For C4x, four new registers R8...R11 are mapped at register file
           address 28..31. So for C4x, only allow the defined R0..11 registers: */
    ...
    if ((strlen(Asc) == 3) && (as_toupper(*Asc) == 'A') && (as_toupper(Asc[1]) == 'R')
        && (Asc[2] >= '0') && (Asc[2] <= '7')) {
...
static Boolean DecodeExpReg(char const* Asc, Byte* Erg) {
...
static void DecodeAdr(tStrComp const* pArg, Byte Erl, Boolean ImmFloat) {
    Byte    HReg;
    Integer Disp;
    char*   p;
    int     l;
...
static void JudgePar(GenOrder const* Prim, int Sec, Byte* ErgMode, Byte* ErgCode) {
    if (Sec > 3) {
        *ErgMode = 3;
    } else if (Prim->May3) {
        *ErgMode = 1;
...
static LongWord EvalAdrExpression(
        tStrComp const* pArg, Boolean* OK, tSymbolFlags* pFlags) {
    return EvalStrIntExpressionOffsWithFlags(
            pArg, !!(*pArg->str.p_str == '@'), UInt24, OK, pFlags);
}
...
static Boolean Is4xArDisp(ShortInt ThisAdrMode, LongWord ThisAdrPart) {
    return (ThisAdrMode == ModInd) && ((ThisAdrPart & 0xf800) == 0x0000)
...
static void DecodeGen(Word Index) {
    ...
    if ((ActArgCnt < 2) || (ActArgCnt > 3)
        || ((CurrGenInfo.Is3) && (!CurrGenInfo.pOrder->May3))) {
    ...
    if (CurrGenInfo.Is3) {
        ...
        DecodeAdr(
                pArg[1],
                MModReg | MModInd
                        | ((Is4x() && !CurrGenInfo.pOrder->ImmFloat) ? MModImm : 0),
        ...
        if (AdrMode == ModImm) {
            ...
            /* note that for type 2, bit 21 defines addressing mode of src2 and bit 22
               defines addressing mode of src1, which is the opposite of the type 1
               format! */
    ...
    } else /* !CurrGenInfo.Is3 */
    {
        DecodeAdr(
                pArg[1],
                MModDir + MModInd
                        + ((CurrGenInfo.pOrder->OnlyMem) ? 0 : MModReg + MModImm),
                CurrGenInfo.pOrder->ImmFloat);
        if (AdrMode == ModNone) {
            return;
        }
        ...
        case ModReg:
            ...
            break;
        case ModInd:
            if (((strcmp(OpPart.str.p_str, "TSTB")) && (strcmp(OpPart.str.p_str, "CMPI"))
                 && (strcmp(OpPart.str.p_str, "CMPF")))
                || ((CurrGenInfo.Src2Mode == ModDir) || (CurrGenInfo.Src2Mode == ModImm))
                || ((CurrGenInfo.Src2Mode == ModInd)
                    && ((CurrGenInfo.Src2Part & 0xe000) == 0)
                    && (Lo(CurrGenInfo.Src2Part) != 1))
    ...
    if (ThisPar) {
        ...
        ParIndex = MatchParIndex(
                PrevGenInfo.Is3 ? PrevGenInfo.pOrder->Par3Mask
                                : PrevGenInfo.pOrder->ParMask,
                CurrGenInfo.Is3 ? CurrGenInfo.pOrder->ParIndex3
                                : CurrGenInfo.pOrder->ParIndex);
        ...
        /* in gedrehter Reihenfolge suchen */
            ...
            ParIndex = MatchParIndex(
                    CurrGenInfo.Is3 ? CurrGenInfo.pOrder->Par3Mask
                                    : CurrGenInfo.pOrder->ParMask,
                    PrevGenInfo.Is3 ? PrevGenInfo.pOrder->ParIndex3
                                    : PrevGenInfo.pOrder->ParIndex);
            if (ParIndex < ParOrderCount) {
        ...
        switch (HReg) {
        case 1:
            if ((!strcmp(PrevOp, "LSH3")) || (!strcmp(PrevOp, "ASH3"))
                || (!strcmp(PrevOp, "SUBF3")) || (!strcmp(PrevOp, "SUBI3"))) {
            ...
            if ((PrevGenInfo.Src1Mode == ModInd) && (PrevGenInfo.Src2Mode == ModReg)
        ...
        case 2:
            ...
            if ((!strcmp(PrevOp, OpPart.str.p_str)) && (*OpPart.str.p_str == 'L')) {
                ...
                if (PrevGenInfo.DestPart == CurrGenInfo.DestPart) {
            ...
            break;
        case 3:
            if ((PrevGenInfo.DestPart > 1) || (CurrGenInfo.DestPart < 2)
                || (CurrGenInfo.DestPart > 3)) {
                WrError(ErrNum_InvReg);
                return;
            }
            Sum = 0;
            if (PrevGenInfo.Src2Mode == ModInd) {
                Sum++;
            }
            if (PrevGenInfo.Src1Mode == ModInd) {
                Sum++;
            }
            if (CurrGenInfo.Src2Mode == ModInd) {
                Sum++;
            }
            if (CurrGenInfo.Src1Mode == ModInd) {
                Sum++;
            }
            if (Sum != 2) {
                WrError(ErrNum_InvParAddrMode);
            ...
            if (CurrGenInfo.Src1Mode == ModReg) {
                if (CurrGenInfo.Src2Mode == ModReg) {
                    DAsmCode[0] += ((LongWord)0x00000000)
                                   ...
                                   + (PrevGenInfo.Src1Part & 0xff00)
                                   + Hi(PrevGenInfo.Src2Part);
                } else {
                    DAsmCode[0] += ((LongWord)0x03000000)
                                   + (((LongWord)CurrGenInfo.Src1Part) << 16)
                                   + Hi(CurrGenInfo.Src2Part);
                    if (PrevGenInfo.Src2Mode == ModReg) {
                        DAsmCode[0] += (((LongWord)PrevGenInfo.Src2Part) << 19)
                                       + (PrevGenInfo.Src1Part & 0xff00);
                    } else {
                        DAsmCode[0] += (((LongWord)PrevGenInfo.Src1Part) << 19)
                                       + (PrevGenInfo.Src2Part & 0xff00);
                    }
                }
            } else {
                if (CurrGenInfo.Src2Mode == ModReg) {
    ...
    /* ...sequentiell */
        ...
        if (CurrGenInfo.Is3) {
            DAsmCode[0] = 0x20000000 | T28 | (((LongWord)CurrGenInfo.pOrder->Code3) << 23)
                          | T21_22 | (((LongWord)CurrGenInfo.DestPart) << 16)
...
/* Due to the way it is executed in the instruction pipeline, LDA has some restrictions
   on its operands: only ARx, DP-SP allowed as destination, and source+dest reg cannot be
   same register */
...
static void DecodeLDA(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_ParNotPossible);
    } else if (!DecodeReg(ArgStr[2].str.p_str, &HReg)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else if ((HReg < 8) || (HReg > 20)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else {
        Boolean RegClash;
...
static void DecodeLDIcc_LDFcc(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (AdrMode != ModNone) {
            HReg = AdrPart;
            DecodeAdr(
                    &ArgStr[1], MModReg + MModDir + MModInd + MModImm,
                    InstrCode == 0x40000000);
            if (AdrMode != ModNone) {
...
static void DecodeSing(Word Index) {
    SingOrder const* pOrder = SingOrders + Index;
...
static void DecodeLDP(Word Code) {
    ...
    if (!ChkArgCnt(1, 2))
        ...
        WrError(ErrNum_ParNotPossible);
    } else if ((ArgCnt == 2) && (as_strcasecmp(ArgStr[2].str.p_str, "DP"))) {
        WrError(ErrNum_InvAddrMode);
    } else {
        Boolean      OK;
        tSymbolFlags Flags;
...
static void DecodeLdExp(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_ParNotPossible);
    } else if (!(Swapped ? DecodeReg(ArgStr[1].str.p_str, &Src)
                         : DecodeExpReg(ArgStr[1].str.p_str, &Src))) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if (!(Swapped ? DecodeExpReg(ArgStr[2].str.p_str, &Dest)
                         : DecodeReg(ArgStr[2].str.p_str, &Dest))) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else {
...
static void DecodeRegImm(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_ParNotPossible);
    } else if (!DecodeReg(ArgStr[2].str.p_str, &Dest)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else {
...
static void DecodeLDPK(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        WrError(ErrNum_ParNotPossible);
    } else if (!DecodeReg("DP", &Dest)) {
        WrXError(ErrNum_InvReg, "DP");
    } else {
        DecodeAdr(&ArgStr[1], MModImm, False);
        if (AdrMode == ModImm) {
            DAsmCode[0] = (((LongWord)Code) << 23) | (((LongWord)AdrMode) << 21)
...
static void DecodeRPTB_C4x(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        tSymbolFlags Flags;
        LongInt      AdrLong
                = EvalAdrExpression(&ArgStr[1], &OK, &Flags) - (EProgCounter() + 1);

        if (OK) {
            if (!mSymbolQuestionable(Flags)
                && ((AdrLong > 0x7fffffl) || (AdrLong < -0x800000l))) {
...
static void DecodeBR_BRD_CALL_C3x(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        (void)ChkMinCPU(CPU32040); /* no LAJ on C3x */
    } else if (ThisPar) {
        WrError(ErrNum_ParNotPossible);
    } else {
        Boolean      OK;
...
static void DecodeBR_BRD_CALL_LAJ_C4x(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        tSymbolFlags Flags;
        LongInt      AdrLong
                = EvalAdrExpression(&ArgStr[1], &OK, &Flags) - (EProgCounter() + Dist);
...
static void DecodeBcc(Word Code) {
    LongWord CondCode = Lo(Code) & 0x7f, DFlag = ((LongWord)Hi(Code)) << 21;
    ...
    if (!ChkArgCnt(1, 1))
        ...
        tSymbolFlags Flags;
        LongInt      AdrLong
                = EvalAdrExpression(&ArgStr[1], &OK, &Flags) - (EProgCounter() + Disp);

        if (OK) {
            if (!mSymbolQuestionable(Flags)
                && ((AdrLong > 0x7fffl) || (AdrLong < -0x8000l))) {
                WrError(ErrNum_JmpDistTooBig);
            } else {
...
static void DecodeDBcc(Word Code) {
    LongWord CondCode = Lo(Code), DFlag = ((LongWord)Hi(Code)) << 21;
    LongInt  Disp = DFlag ? 3 : 1;
    Byte     HReg, HReg2;
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_ParNotPossible);
    } else if (!DecodeReg(ArgStr[1].str.p_str, &HReg2)) {
        WrError(ErrNum_InvAddrMode);
    } else if ((HReg2 < 8) || (HReg2 > 15)) {
        WrError(ErrNum_InvAddrMode);
    } else {
        HReg2 -= 8;
...
static void AddFixed(char const* NName, LongWord NCode) {
...
static void AddSing(char const* NName, LongWord NCode, Byte NMask) {
    if (InstrZ >= SingOrderCount) {
        exit(255);
    }
...
static void AddGen(
        char const* NName, CPUVar NMin, Boolean NMay1, Boolean NMay3, Word NCode,
        Word NCode3, Boolean NOnly, Boolean NSwap, Boolean NImm, Boolean NComm,
        Byte NMask1, Byte NMask3, Byte C20, Byte C21, Byte C22, Byte C23, Byte C24,
        Byte C25, Byte C26, Byte C27, Byte C30, Byte C31, Byte C32, Byte C33, Byte C34,
        Byte C35, Byte C36, Byte C37) {
    char     NName3[30];
    unsigned z;

    if (InstrZ >= GenOrderCount) {
        exit(255);
    }

    as_snprintf(NName3, sizeof(NName3), "%s3", NName);

    GenOrders[InstrZ].ParIndex = GenOrders[InstrZ].ParIndex3 = ParOrderCount;
...
static void InitFields(void) {
    ...
    AddInstTable(
            InstTable, "BR", 0x0160,
            Is4x() ? DecodeBR_BRD_CALL_LAJ_C4x : DecodeBR_BRD_CALL_C3x);
    AddInstTable(
            InstTable, "BRD", 0x0361,
            Is4x() ? DecodeBR_BRD_CALL_LAJ_C4x : DecodeBR_BRD_CALL_C3x);
    AddInstTable(
            InstTable, "CALL", 0x0162,
            Is4x() ? DecodeBR_BRD_CALL_LAJ_C4x : DecodeBR_BRD_CALL_C3x);
    AddInstTable(
            InstTable, "LAJ", 0x0363,
            Is4x() ? DecodeBR_BRD_CALL_LAJ_C4x : DecodeBR_BRD_CALL_C3x);
    ...
    ParOrders           = (char const**)malloc(sizeof(char*) * ParOrderCount);
    InstrZ              = 0;
    ...
    InstrZ    = 0;
    /*        Name      MinCPU    May1   May3   Cd    Cd3   OnlyM  Swap   ImmF   Comm PM1
     * PM3     */
    AddGen("ABSF", CPU32030, True, False, 0x00, 0xff, False, False, True, False, 4, 0,
           0xff, 0xff, 0x04, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    AddGen("ABSI", CPU32030, True, False, 0x01, 0xff, False, False, False, False, 8, 0,
           0xff, 0xff, 0xff, 0x05, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("ADDF", CPU32030, False, True, 0x03, 0x01, False, False, True, True, 0, 4,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x06, 0xff, 0xff,
           0xff, 0xff, 0xff);
    AddGen("ADDI", CPU32030, False, True, 0x04, 0x02, False, False, False, True, 0, 8,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x07, 0xff,
           0xff, 0xff, 0xff);
    AddGen("AND", CPU32030, False, True, 0x05, 0x03, False, False, False, True, 0, 8,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x08, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("ASH", CPU32030, False, True, 0x07, 0x05, False, False, False, False, 0, 8,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x09, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("FIX", CPU32030, True, False, 0x0a, 0xff, False, False, True, False, 8, 0,
           0xff, 0xff, 0xff, 0x0a, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    AddGen("FLOAT", CPU32030, True, False, 0x0b, 0xff, False, False, False, False, 4, 0,
           0xff, 0xff, 0x0b, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    AddGen("FRIEEE", CPU32040, False, False, 0x38, 0xff, True, False, True, False, 4, 0,
           0xff, 0xff, 0x19, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("LDF", CPU32030, False, False, 0x0e, 0xff, False, False, True, False, 5, 0,
           0x02, 0xff, 0x0c, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("LDI", CPU32030, False, False, 0x10, 0xff, False, False, False, False, 10, 0,
           0xff, 0x03, 0xff, 0x0d, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("LSH", CPU32030, False, True, 0x13, 0x08, False, False, False, False, 0, 8,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x0e, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("MPYF", CPU32030, False, True, 0x14, 0x09, False, False, True, True, 0, 52,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x0f, 0xff, 0x00,
           0x01, 0xff, 0xff);
    ...
    AddGen("NEGF", CPU32030, True, False, 0x17, 0xff, False, False, True, False, 4, 0,
           0xff, 0xff, 0x11, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    AddGen("NEGI", CPU32030, True, False, 0x18, 0xff, False, False, False, False, 8, 0,
           0xff, 0xff, 0xff, 0x12, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("NOT", CPU32030, True, False, 0x1b, 0xff, False, False, False, False, 8, 0,
           0xff, 0xff, 0xff, 0x13, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    AddGen("OR", CPU32030, False, True, 0x20, 0x0b, False, False, False, True, 0, 8, 0xff,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x14, 0xff, 0xff,
           0xff, 0xff);
    AddGen("RCPF", CPU32040, False, False, 0x3a, 0xff, False, False, True, False, 0, 0,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x14, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("RSQRF", CPU32040, False, False, 0x39, 0xff, False, False, True, False, 0, 0,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x14, 0xff,
           0xff, 0xff, 0xff);
    AddGen("STF", CPU32030, False, False, 0x28, 0xff, True, True, True, False, 4, 0, 0xff,
    ...
    AddGen("STI", CPU32030, False, False, 0x2a, 0xff, True, True, False, False, 8, 0,
           0xff, 0xff, 0xff, 0x01, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("SUBF", CPU32030, False, True, 0x2f, 0x0d, False, False, True, False, 0, 4,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x15, 0xff, 0xff,
           0xff, 0xff, 0xff);
    AddGen("SUBI", CPU32030, False, True, 0x30, 0x0e, False, False, False, False, 0, 8,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x16, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("TOIEEE", CPU32040, False, False, 0x37, 0xff, False, False, True, False, 4, 0,
           0xff, 0xff, 0x18, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
           0xff, 0xff, 0xff);
    ...
    AddGen("XOR", CPU32030, False, True, 0x35, 0x10, False, False, False, True, 0, 8,
           0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x17, 0xff,
           0xff, 0xff, 0xff);
