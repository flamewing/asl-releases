...
/*                                                                           */
/*****************************************************************************/
/* $Id: code3203x.c,v 1.10 2014/11/16 13:15:06 alfred Exp $                       */
/***************************************************************************** 
 * $Log: code3203x.c,v $
...
typedef struct
{
  ...
  Boolean Is3;
  ShortInt Src1Mode, Src2Mode, DestMode;
  Word Src1Part, Src2Part, DestPart;
} tGenOrderInfo;
...
{
#define RegCnt (sizeof(Regs) / sizeof(*Regs))
#define RegStart 0x10
  ...
  *Erg = 0;
  while ((*Erg < RegCnt) && (strcasecmp(Regs[*Erg], Asc)))
    (*Erg)++;
  if (*Erg < RegCnt)
  {
    *Erg += RegStart;
...
static void DecodeGen(Word Index)
{
  ...
  if (CurrGenInfo.Is3)
  {
    ...
    DecodeAdr(ArgStr[1], MModReg + MModInd, CurrGenInfo.pOrder->ImmFloat);
    if (AdrMode == ModNone)
      return;
    if ((AdrMode == ModInd) && ((AdrPart & 0xe000) == 0) && (Lo(AdrPart) != 1))
    {
  ...
  else /* !CurrGenInfo.Is3 */
  {
    ...
    switch (AdrMode)
    {
      case ModReg:
        CurrGenInfo.DestMode = AdrMode;
        CurrGenInfo.DestPart = AdrPart;
        CurrGenInfo.Src2Mode = CurrGenInfo.Src1Mode;
        CurrGenInfo.Src2Part = CurrGenInfo.Src1Part;
        break;
      case ModInd:
        if (((strcmp(OpPart, "TSTB")) && (strcmp(OpPart, "CMPI")) && (strcmp(OpPart, "CMPF")))
        ||  ((CurrGenInfo.Src1Mode == ModDir) || (CurrGenInfo.Src1Mode == ModImm))
        ||  ((CurrGenInfo.Src1Mode == ModInd) && ((CurrGenInfo.Src1Part & 0xe000) == 0) && (Lo(CurrGenInfo.Src1Part) != 1))
  ...
  if (ThisPar)
  {
    ...
    switch (HReg)
    {
      case 1:
        ...
        {
          SwapMode(&PrevGenInfo.Src1Mode, &PrevGenInfo.Src2Mode);
          SwapPart(&PrevGenInfo.Src1Part, &PrevGenInfo.Src2Part);
        }
        ...
        if  ((PrevGenInfo.Src2Mode == ModInd) && (PrevGenInfo.Src1Mode == ModReg) && (PrevGenInfo.pOrder->Commutative))
        {
          SwapMode(&PrevGenInfo.Src1Mode, &PrevGenInfo.Src2Mode);
          SwapPart(&PrevGenInfo.Src1Part, &PrevGenInfo.Src2Part);
        }
        if ((PrevGenInfo.Src2Mode != ModReg) || (PrevGenInfo.Src2Part > 7)
         || (PrevGenInfo.Src1Mode != ModInd) || (CurrGenInfo.Src1Mode != ModInd))
        {
          WrError(1355);
          return;
        }
        ...
        DAsmCode[0] = 0xc0000000 + (((LongWord)HReg2) << 25)
                    ...
                    + (((LongWord)PrevGenInfo.Src2Part) << 19)
                    + (((LongWord)CurrGenInfo.DestPart) << 16)
                    + (CurrGenInfo.Src1Part & 0xff00) + Hi(PrevGenInfo.Src1Part);
        CodeLen = 1;
        NextPar = False;
        break;
      case 2:
        ...
        if ((PrevGenInfo.Src1Mode != ModInd) || (CurrGenInfo.Src1Mode != ModInd))
        {
          WrError(1355);
          return;
        }
        ...
        if (CurrGenInfo.Src2Mode == ModReg)
        {
          if (CurrGenInfo.Src1Mode == ModReg)
          {
            DAsmCode[0] += ((LongWord)0x00000000)
                         + (((LongWord)CurrGenInfo.Src2Part) << 19)
                         + (((LongWord)CurrGenInfo.Src1Part) << 16)
                         + (PrevGenInfo.Src2Part & 0xff00) + Hi(PrevGenInfo.Src1Part);
          }
          else
          {
            ...
            if (PrevGenInfo.Src1Mode == ModReg)
        ...
        else
        {
          ...
          else
          {
            DAsmCode[0] += ((LongWord)0x02000000)
                         + (((LongWord)PrevGenInfo.Src2Part) << 19)
                         + (((LongWord)PrevGenInfo.Src1Part) << 16)
                         + (CurrGenInfo.Src2Part & 0xff00) + Hi(CurrGenInfo.Src1Part);
          }
        }
        break;
    }
  ...
  else
  {
    ...
    if (CurrGenInfo.Is3)
      DAsmCode[0] = 0x20000000 + (((LongWord)CurrGenInfo.pOrder->Code3) << 23)
                  + (((LongWord)CurrGenInfo.DestPart) << 16)
                  + (((LongWord)CurrGenInfo.Src2Mode) << 20) + (EffPart(CurrGenInfo.Src2Mode, CurrGenInfo.Src2Part) << 8)
                  + (((LongWord)CurrGenInfo.Src1Mode) << 21) + EffPart(CurrGenInfo.Src1Mode, CurrGenInfo.Src1Part);
    else
      DAsmCode[0] = 0x00000000 + (((LongWord)CurrGenInfo.pOrder->Code) << 23)
                  + (((LongWord)CurrGenInfo.Src1Mode) << 21) + CurrGenInfo.Src1Part
                  + (((LongWord)CurrGenInfo.DestPart) << 16);
    CodeLen = 1;
    NextPar = True;
  }
...
static void DecodeRPTB(Word Code)
{
  UNUSED(Code);
...
static void DecodeBR_BRD_CALL(Word Code)
{
  if (ArgCnt != 1) WrError(1110);
  else if (ThisPar) WrError(1950);
  else
...
static void DecodeTRAPcc(Word Code)
{
  ...
  {
    Boolean OK;
    Byte HReg = EvalIntExpression(ArgStr[1], UInt4, &OK);
      ...
      if (OK)
      {
        ...
        switch (t.Typ)
        {
          case TempInt:
            ...
            break;
          case TempFloat:
            if (!ExtToSingle(t.Contents.Float, DAsmCode + (CodeLen++)))
              OK = False;
            break;
          case TempString:
          {
...
static void AddCondition(char *NName, Byte NCode)
{
  ...
  AddInstTable(InstTable, InstName, NCode, DecodeBcc);
  sprintf(InstName, "B%sD", NName);
  AddInstTable(InstTable, InstName, 0x100 | NCode, DecodeBcc);
...
static void InitFields(void)
{
  ...
  AddInstTable(InstTable, "BR", 0x60, DecodeBR_BRD_CALL);
  AddInstTable(InstTable, "BRD", 0x61, DecodeBR_BRD_CALL);
  AddInstTable(InstTable, "CALL", 0x62, DecodeBR_BRD_CALL);
  AddInstTable(InstTable, "SINGLE", 0, DecodeSINGLE);
  AddInstTable(InstTable, "EXTENDED", 0, DecodeEXTENDED);
...
/*        Name    May1   May3   Cd    Cd3   OnlyM  Swap   ImmF   Comm   PM1 PM3     */
  AddGen("ABSF" , True , False, 0x00, 0xff, False, False, True , False, 4, 0,
         0xff, 0xff, 0x04, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("ABSI" , True , False, 0x01, 0xff, False, False, False, False, 8, 0,
         0xff, 0xff, 0xff, 0x05, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("ADDC" , False, True , 0x02, 0x00, False, False, False, True,  0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("ADDF" , False, True , 0x03, 0x01, False, False, True , True,  0, 4,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0x06, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("ADDI" , False, True , 0x04, 0x02, False, False, False, True,  0, 8,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0x07, 0xff, 0xff, 0xff, 0xff);
  AddGen("AND"  , False, True , 0x05, 0x03, False, False, False, True,  0, 8,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0x08, 0xff, 0xff, 0xff, 0xff);
  AddGen("ANDN" , False, True , 0x06, 0x04, False, False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("ASH"  , False, True , 0x07, 0x05, False, False, False, False, 0, 8,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0x09, 0xff, 0xff, 0xff, 0xff);
  AddGen("CMPF" , False, True , 0x08, 0x06, False, False, True , False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("CMPI" , False, True , 0x09, 0x07, False, False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("FIX"  , True , False, 0x0a, 0xff, False, False, True , False, 8, 0,
         0xff, 0xff, 0xff, 0x0a, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("FLOAT", True , False, 0x0b, 0xff, False, False, False, False, 4, 0,
         0xff, 0xff, 0x0b, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("LDE"  , False, False, 0x0d, 0xff, False, False, True , False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("LDF"  , False, False, 0x0e, 0xff, False, False, True , False, 5, 0,
         0x02, 0xff, 0x0c, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("LDFI" , False, False, 0x0f, 0xff, True , False, True , False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("LDI"  , False, False, 0x10, 0xff, False, False, False, False, 10, 0,
         0xff, 0x03, 0xff, 0x0d, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("LDII" , False, False, 0x11, 0xff, True , False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("LDM"  , False, False, 0x12, 0xff, False, False, True , False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("LSH"  , False, True , 0x13, 0x08, False, False, False, False, 0, 8,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0x0e, 0xff, 0xff, 0xff, 0xff);
  AddGen("MPYF" , False, True , 0x14, 0x09, False, False, True , True,  0,52,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0x0f, 0xff, 0x00, 0x01, 0xff, 0xff);
  AddGen("MPYI" , False, True , 0x15, 0x0a, False, False, False, True,  0,200,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0x10, 0xff, 0xff, 0x02, 0x03);
  AddGen("NEGB" , True , False, 0x16, 0xff, False, False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("NEGF" , True , False, 0x17, 0xff, False, False, True , False, 4, 0,
         0xff, 0xff, 0x11, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("NEGI" , True , False, 0x18, 0xff, False, False, False, False, 8, 0,
         0xff, 0xff, 0xff, 0x12, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("NORM" , True , False, 0x1a, 0xff, False, False, True , False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("NOT"  , True , False, 0x1b, 0xff, False, False, False, False, 8, 0,
         0xff, 0xff, 0xff, 0x13, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("OR"   , False, True , 0x20, 0x0b, False, False, False, True,  0, 8,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0x14, 0xff, 0xff, 0xff, 0xff);
  AddGen("RND"  , True , False, 0x22, 0xff, False, False, True , False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("STF"  , False, False, 0x28, 0xff, True , True , True , False, 4, 0,
         0xff, 0xff, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("STFI" , False, False, 0x29, 0xff, True , True , True , False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("STI"  , False, False, 0x2a, 0xff, True , True , False, False, 8, 0,
         0xff, 0xff, 0xff, 0x01, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("STII" , False, False, 0x2b, 0xff, True , True , False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("SUBB" , False, True , 0x2d, 0x0c, False, False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("SUBC" , False, False, 0x2e, 0xff, False, False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("SUBF" , False, True , 0x2f, 0x0d, False, False, True , False, 0, 4,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0x15, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("SUBI" , False, True , 0x30, 0x0e, False, False, False, False, 0, 8,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0x16, 0xff, 0xff, 0xff, 0xff);
  AddGen("SUBRB", False, False, 0x31, 0xff, False, False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("SUBRF", False, False, 0x32, 0xff, False, False, True , False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("SUBRI", False, False, 0x33, 0xff, False, False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("TSTB" , False, True , 0x34, 0x0f, False, False, False, False, 0, 0,
         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
  AddGen("XOR"  , False, True , 0x35, 0x10, False, False, False, True,  0, 8,
