...
static Boolean Overlap(
        LargeWord Start1, LargeWord Len1, LargeWord Start2, LargeWord Len2) {
    return ((Start1 == Start2) || ((Start2 > Start1) && (Start1 + Len1 >= Start2))
            || ((Start1 > Start2) && (Start2 + Len2 >= Start1)));
}

static void SetChunk(
        OneChunk* NChunk, LargeWord Start1, LargeWord Len1, LargeWord Start2,
        LargeWord Len2) {
...
static void IncChunk(ChunkList* NChunk) {
    if (NChunk->RealLen + 1 > NChunk->AllocLen) {
        if (NChunk->RealLen == 0) {
            ...
            NChunk->Chunks = (OneChunk*)realloc(
                    NChunk->Chunks, sizeof(OneChunk) * (NChunk->RealLen + 1));
        }
...
Boolean AddChunk(ChunkList* NChunk, LargeWord NewStart, LargeWord NewLen, Boolean Warn) {
    ...
    for (z = 0; z < NChunk->RealLen; z++) {
        if (Overlap(NewStart, NewLen, NChunk->Chunks[z].Start,
                    NChunk->Chunks[z].Length)) {
            Found = True;
            f1    = z;
            break;
        }
    ...
    if (Found) {
        ...
        SetChunk(
                NChunk->Chunks + f1, NewStart, NewLen, NChunk->Chunks[f1].Start,
                NChunk->Chunks[f1].Length);
        if (Warn) {
            if (PartSum != NChunk->Chunks[f1].Length) {
                Result = True;
            }
        }
        ...
        do {
            ...
            for (z = 1; z < NChunk->RealLen; z++) {
                if (z != f1) {
                    if (Overlap(NChunk->Chunks[z].Start, NChunk->Chunks[z].Length,
                                NChunk->Chunks[f1].Start, NChunk->Chunks[f1].Length)) {
                        Found = True;
                        f2    = z;
                        break;
                    }
            ...
            if (Found) {
                SetChunk(
                        NChunk->Chunks + f1, NChunk->Chunks[f1].Start,
                        NChunk->Chunks[f1].Length, NChunk->Chunks[f2].Start,
                        NChunk->Chunks[f2].Length);
...
void DeleteChunk(ChunkList* NChunk, LargeWord DelStart, LargeWord DelLen) {
    ...
    while (z <= NChunk->RealLen) {
        if (Overlap(DelStart, DelLen, NChunk->Chunks[z].Start,
                    NChunk->Chunks[z].Length)) {
            if (NChunk->Chunks[z].Start >= DelStart) {
                if (DelStart + DelLen
                    >= NChunk->Chunks[z].Start + NChunk->Chunks[z].Length) {
                    /* ganz loeschen */
                ...
                NChunk->Chunks[NChunk->RealLen].Length
                        = NChunk->Chunks[z].Start + NChunk->Chunks[z].Length
                          - NChunk->Chunks[NChunk->RealLen].Start;
                NChunk->Chunks[z].Length = DelStart - NChunk->Chunks[z].Start;
            }
        }
        z++;
...
static int CompareChunk(void const* c1, void const* c2) {
    OneChunk const *pChunk1 = (OneChunk const*)c1, *pChunk2 = (OneChunk const*)c2;
