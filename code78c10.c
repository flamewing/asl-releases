...
static decode_reg_res_t decode_r(tStrComp const* p_arg, ShortInt* p_res) {
    static char const Names[] = "VABCDEHL";
    char const*       p;
...
static decode_reg_res_t decode_r1(tStrComp const* p_arg, ShortInt* p_res) {
    if ((pCurrCPUProps->Core == eCore7810) && !as_strcasecmp(p_arg->str.p_str, "EAL")) {
        *p_res = 1;
    } else if (
            (pCurrCPUProps->Core == eCore7810)
            && !as_strcasecmp(p_arg->str.p_str, "EAH")) {
        *p_res = 0;
    } else {
...
static decode_reg_res_t decode_r2(tStrComp const* p_arg, ShortInt* p_res) {
    decode_reg_res_t ret = decode_r(p_arg, p_res);
...
static Boolean DecodeAdrMode(
        char* pAsc, tAdrMode const pModes[], ShortInt* pErg, Boolean* pWasIndirect) {
    int z;
...
static Boolean Decode_rpa2(
        tStrComp const* pArg, Boolean* pWasIndirect, ShortInt* Erg, ShortInt* Disp) {
    static tAdrMode const AdrModes[] = {
            {    "B",  1,  True},
            {   "BC",  1,  True},
            {    "D",  2,  True},
            {   "DE",  2,  True},
...
static Boolean Decode_rpa(tStrComp const* pArg, ShortInt* Erg) {
    ShortInt Dummy;
    Boolean  WasIndirect = False;

    if (!Decode_rpa2(pArg, &WasIndirect, Erg, &Dummy)) {
        return False;
    }
    return (*Erg <= 7);
}

static Boolean Decode_rpa1(tStrComp const* pArg, ShortInt* Erg) {
    ShortInt Dummy;
    Boolean  WasIndirect = False;
...
static Boolean Decode_rpa3(tStrComp const* pArg, ShortInt* Erg, ShortInt* Disp) {
    static tAdrMode const AdrModes[] = {
            {   "D++", 4,  True},
            {  "DE++", 4,  True},
            {"(DE)++", 4, False},
...
#define FlagCnt 3
    static char const Flags[FlagCnt][3] = {"CY", "HC", "Z"};

    for (*Erg = 0; *Erg < FlagCnt; (*Erg)++) {
        if (!as_strcasecmp(Flags[*Erg], Asc)) {
            break;
        }
    }
    *Erg += 2;
...
static sreg_t const* decode_sr_core(tStrComp const* p_arg) {
    int z;
...
static decode_reg_res_t decode_sr_flag(
        tStrComp const* p_arg, ShortInt* p_res, Byte flag) {
    sreg_t const* p_reg = decode_sr_core(p_arg);
...
static Boolean Decode_sr3(tStrComp const* p_arg, ShortInt* p_res) {
    if (!as_strcasecmp(p_arg->str.p_str, "ETM0")) {
        *p_res = 0;
    } else if (!as_strcasecmp(p_arg->str.p_str, "ETM1")) {
        *p_res = 1;
    } else {
...
static Boolean Decode_sr4(tStrComp const* p_arg, ShortInt* p_res) {
    if (!as_strcasecmp(p_arg->str.p_str, "ECNT")) {
        *p_res = 0;
    } else if (!as_strcasecmp(p_arg->str.p_str, "ECPT")) {
        *p_res = 1;
    } else {
        WrStrErrorPos(ErrNum_InvCtrlReg, p_arg);
        return False;
...
static Boolean Decode_irf(tStrComp const* p_arg, ShortInt* p_res) {
...
static Boolean Decode_wa(tStrComp const* pArg, Byte* Erg) {
    Word         Adr;
    Boolean      OK;
    tSymbolFlags Flags;
    ...
    if (!mFirstPassUnknown(Flags) && (Hi(Adr) != WorkArea)) {
...
static void DecodeFixed(Word index) {
    order_t const* p_order = &fixed_orders[index];
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (OK) {
            ...
            if ((res = decode_r(&ArgStr[1], &HReg)) != e_decode_reg_unknown) {
                ...
                if (res == e_decode_reg_ok) {
                    if (pCurrCPUProps->Core != eCore7810) {
...
static void DecodeLDAX_STAX(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if (!Decode_rpa2(&ArgStr[1], &WasIndirect, &HReg, (ShortInt*)BAsmCode + 1)) {
        WrError(ErrNum_InvAddrMode);
    } else {
...
static void DecodeLDEAX_STEAX(Word Code) {
    ...
    if (!ChkArgCnt(1, 1) || !check_core(core_mask_7810))
        ;
    else if (!Decode_rpa3(&ArgStr[1], &HReg, (ShortInt*)BAsmCode + 2)) {
        WrError(ErrNum_InvAddrMode);
    } else {
...
static void DecodeLXI(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if (!Decode_rp2(ArgStr[1].str.p_str, &HReg)) {
        WrError(ErrNum_InvAddrMode);
    } else {
        AdrInt = EvalStrIntExpression(&ArgStr[2], Int16, &OK);
        if (OK) {
...
static void DecodePUSH_POP(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if (!Decode_rp1(ArgStr[1].str.p_str, &HReg)) {
        WrError(ErrNum_InvAddrMode);
    } else {
...
static void DecodeDMOV(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && check_core(core_mask_7810)) {
        ...
        tStrComp const *pArg1 = Swap ? &ArgStr[2] : &ArgStr[1],
                       *pArg2 = Swap ? &ArgStr[1] : &ArgStr[2];

        if (as_strcasecmp(pArg1->str.p_str, "EA")) {
            WrError(ErrNum_InvAddrMode);
...
static void DecodeALUImm(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (OK) {
            ...
            StrCompRefRight(
                    &Arg1, &ArgStr[1], as_strcasecmp(ArgStr[1].str.p_str, ">A") ? 0 : 1);
            if (!as_strcasecmp(ArgStr[1].str.p_str, "A")) {
                ...
                if (res != e_decode_reg_ok)
                    ;
                else if (pCurrCPUProps->Core == eCore7800Low) {
                    WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
                } else {
                    CodeLen     = 3;
                ...
                if (res != e_decode_reg_ok)
                    ...
                    BAsmCode[1]
                            = (HReg & 7) | (Code << 3)
                              | ((pCurrCPUProps->Core == eCore7810) ? ((HReg & 8) << 4)
                                                                    : 0x80);
                    BAsmCode[2] = HVal8;
                }
            } else {
...
static void DecodeALUReg(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (as_strcasecmp(pArg1->str.p_str, "A")) {
            WrStrErrorPos(ErrNum_InvAddrMode, pArg1);
        } else if ((res = decode_r(pArg2, &HReg)) == e_decode_reg_unknown) {
            WrStrErrorPos(ErrNum_InvAddrMode, pArg2);
        } else if (res == e_decode_reg_ok) {
            if (NoSwap && !(Flags & ALUReg_Src)) {
                WrStrErrorPos(ErrNum_InvAddrMode, pArg2);
            } else if (!NoSwap && !(Flags & ALUReg_Dest)) {
                WrStrErrorPos(ErrNum_InvAddrMode, pArg2);
            } else {
                CodeLen     = 2;
                BAsmCode[0] = 0x60;
...
static void DecodeALURegW(Word Code) {
    if (ChkArgCnt(1, 1) && check_core(core_mask_no_low)
...
static void DecodeALUEA(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_InvAddrMode);
    } else if (!Decode_rp3(ArgStr[2].str.p_str, &HReg)) {
        WrError(ErrNum_InvAddrMode);
    } else {
        CodeLen     = 2;
        BAsmCode[0] = 0x74;
...
static void DecodeReg2(Word index) {
    order_t const*   p_order = &reg2_orders[index];
    ShortInt         HReg;
    decode_reg_res_t res;

    if (!ChkArgCnt(1, 1) || !check_core(p_order->core_mask))
        ;
    else if ((res = decode_r2(&ArgStr[1], &HReg)) == e_decode_reg_unknown) {
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
    } else if (res == e_decode_reg_ok) {
...
static void DecodeWork(Word Code) {
    if (ChkArgCnt(1, 1) && Decode_wa(&ArgStr[1], BAsmCode + 1)) {
        CodeLen     = 2;
        BAsmCode[0] = Code;
    }
}
...
static void DecodeEADD_ESUB(Word Code) {
    ...
    if (!ChkArgCnt(2, 2) || !check_core(core_mask_7810))
        ...
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
    } else if ((res = decode_r2(&ArgStr[2], &HReg)) == e_decode_reg_unknown) {
...
static void DecodeJ_JR_JRE(Word Type) {
    ...
    switch (Type) {
    case 1: /* JR */
        if (!mSymbolQuestionable(Flags) && !RangeCheck(AdrInt, SInt6)) {
    ...
    case 2:
        ...
        if (!mSymbolQuestionable(Flags) && !RangeCheck(AdrInt, SInt9)) {
            WrError(ErrNum_JmpDistTooBig);
        } else {
            CodeLen     = 2;
...
static void DecodeCALF(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[1], Int16, &OK, &Flags);
        if (OK) {
            if (!mFirstPassUnknown(Flags) && ((AdrInt >> 11) != 1)) {
...
static void DecodeCALT(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (OK) {
            ...
            if (!mFirstPassUnknown(Flags) && ((AdrInt & AdrMask) != 0x80)) {
                WrError(ErrNum_NotFromThisAddress);
            } else {
                CodeLen     = 1;
...
static void DecodeSKIT_SKNIT(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && check_core(Hi(Code)) && Decode_irf(&ArgStr[1], &HReg)) {
        CodeLen     = 2;
        BAsmCode[0] = 0x48;
        BAsmCode[1] = Lo(Code) + HReg;
    }
...
static void DecodeIN_OUT(Word Code) {
    tStrComp const* p_port_arg;

    switch (ArgCnt) {
    case 2: {
        tStrComp const* p_acc_arg = &ArgStr[(Code & 1) + 1];
...
static void AddFixed(char const* p_name, Word code, unsigned core_mask) {
...
static void AddSReg(char const* p_name, Word code, Byte flags) {
    if (InstrZ >= SRegCnt) {
        exit(255);
    }
...
static void AddIntFlag(char const* p_name, Byte code) {
    if (InstrZ >= IntFlagCnt) {
        exit(255);
    }
...
static void AddALU(
        Byte NCode, Word Flags, char const* NNameI, char const* NNameReg,
        char const* NNameEA) {
    char Name[20];

    AddInstTable(InstTable, NNameI, ((Flags & ALUImm_SR) << 8) | NCode, DecodeALUImm);
    AddInstTable(
            InstTable, NNameReg, ((Flags & (ALUReg_Src | ALUReg_Dest)) << 8) | NCode,
            DecodeALUReg);
...
static void AddAbs(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeAbs);
}

static void AddReg2(char const* p_name, Word code, unsigned core_mask) {
    if (InstrZ >= Reg2OrderCnt) {
        exit(255);
    }
...
static void AddWork(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeWork);
}

static void AddA(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeA);
}

static void AddEA(char const* NName, Word NCode) {
...
static void InitFields(void) {
    ...
    AddInstTable(
            InstTable, "SKIT", (core_mask_no_low << 8) | (Is781x ? 0x40 : 0x00),
            DecodeSKIT_SKNIT);
    AddInstTable(
            InstTable, "SKNIT", (core_mask_all << 8) | (Is781x ? 0x60 : 0x10),
            DecodeSKIT_SKNIT);

    fixed_orders = (order_t*)calloc(sizeof(*fixed_orders), FixedOrderCnt);
    InstrZ       = 0;
    ...
    s_regs = (sreg_t*)calloc(SRegCnt, sizeof(*s_regs));
    InstrZ = 0;
    ...
    int_flags = (intflag_t*)calloc(IntFlagCnt, sizeof(*int_flags));
    InstrZ    = 0;
    if (Is781x) {
        AddIntFlag("NMI", 0);
        AddIntFlag("FT0", 1);
    ...
    AddALU(10, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "ACI", "ADC",
           "DADC");
    AddALU(4, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "ADINC", "ADDNC",
           "DADDNC");
    AddALU(8, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "ADI", "ADD",
           "DADD");
    AddALU(1, IsLow ? ALUImm_SR | ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest,
           "ANI", "ANA", "DAN");
    AddALU(15, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "EQI", "EQA",
           "DEQ");
    AddALU(5, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "GTI", "GTA",
           "DGT");
    AddALU(7, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "LTI", "LTA",
           "DLT");
    AddALU(13, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "NEI", "NEA",
           "DNE");
    AddALU(11, IsLow ? ALUImm_SR : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "OFFI", "OFFA",
           "DOFF");
    AddALU(9, IsLow ? ALUImm_SR : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "ONI", "ONA",
           "DON");
    AddALU(3, IsLow ? ALUImm_SR | ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest,
           "ORI", "ORA", "DOR");
    AddALU(14, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "SBI", "SBB",
           "DSBB");
    AddALU(6, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "SUINB", "SUBNB",
           "DSUBNB");
    AddALU(12, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "SUI", "SUB",
           "DSUB");
    AddALU(2, IsLow ? ALUReg_Src : ALUImm_SR | ALUReg_Src | ALUReg_Dest, "XRI", "XRA",
           "DXR");
    ...
    reg2_orders = (order_t*)calloc(sizeof(*reg2_orders), Reg2OrderCnt);
    InstrZ      = 0;
