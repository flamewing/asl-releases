...
#    define bswap32(x)                                                                \
        ((((x) & 0xffUL) << 24) | (((x) & 0xff00UL) << 8) | (((x) & 0xff0000UL) >> 8) \
         | ((x) >> 24))
#    define bswap64(x)                                                        \
...
INLINE uint16_t read16_as_little_endian(void const* Buffer) {
    uint16_t val;
...
INLINE uint32_t read32_as_little_endian(void const* Buffer) {
    uint32_t val;
    memcpy(&val, Buffer, sizeof(val));
#if HostBigEndian
    val = bswap32(val);
...
INLINE uint64_t read64_as_little_endian(void const* Buffer) {
    uint64_t val;
    memcpy(&val, Buffer, sizeof(val));
#if HostBigEndian
    val = bswap64(val);
...
INLINE uint16_t read16_as_big_endian(void const* Buffer) {
    uint16_t val;
    memcpy(&val, Buffer, sizeof(val));
#if HostBigEndian == 0
    val = bswap16(val);
...
INLINE uint32_t read32_as_big_endian(void const* Buffer) {
    uint32_t val;
    memcpy(&val, Buffer, sizeof(val));
#if HostBigEndian == 0
    val = bswap32(val);
...
INLINE uint64_t read64_as_big_endian(void const* Buffer) {
    uint64_t val;
    memcpy(&val, Buffer, sizeof(val));
#if HostBigEndian == 0
    val = bswap64(val);
...
INLINE uint32_t read32_and_swap_words(unsigned char const* Buffer) {
    uint32_t val;
    memcpy(&val, Buffer, sizeof(val));
    val = rot32(val, 16);
    return val;
...
INLINE void WSwap(void* Field, int Cnt) {
    ...
    register unsigned char const* End = Run + Cnt;
