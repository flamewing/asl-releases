...
static void DecodeAdr(int Start, int Stop, Boolean LongAdr, Byte Mask) {
    ...
    if (*ArgStr[Start].str.p_str == '#') {
        ...
        case eSymbolSize16Bit:
            V16 = EvalStrIntExpressionOffs(
                    &ArgStr[Start], 1 + Offset, (Size == ShortDisp) ? SInt8 : Int16, &OK);
            if ((Size == NoDisp) && (V16 >= -128) && (V16 <= 127)
                && ((Mask & MModImmExt) != 0)) {
                Size = ShortDisp;
            }
            if (OK) {
...
static void DecodeAux(Word Code) {
    if (ChkArgCnt(1, 2)) {
        ...
        DecodeAdr(
                1, ArgCnt, False,
                (*OpPart.str.p_str == 'S' ? 0 : MModImm) | MModDisp8 | MModDisp16
                        | MModAbs);
        switch (AdrMode) {
        case ModDisp8:
...
static void DecodeBrBit(Word Index) {
    ...
    if (ChkArgCnt(3, 4)) {
        ...
        if (AdrMode == ModImm) {
            BAsmCode[1] = AdrVals[0];
            AdrLong = EvalStrIntExpressionWithFlags(&ArgStr[ArgCnt], UInt20, &OK, &Flags)
                      - EProgCounter() - 6;
            if (OK) {
                ...
                case ModDisp8:
                    if ((AdrLong >= -128) && (AdrLong < 127)) {
                        ...
                        CodeLen     = 4;
                            ...
                            !mSymbolQuestionable(Flags)
                            && ((AdrLong < -0x8000l) || (AdrLong > 0x7fffl))) {
                        WrError(ErrNum_JmpDistTooBig);
                    } else {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (AdrLong & 1) {
            WrError(ErrNum_NotAligned);
                ...
                !mSymbolQuestionable(Flags) & ((AdrLong > 0x7fl) || (AdrLong < -0x80l))) {
            WrError(ErrNum_JmpDistTooBig);
        } else {
            BAsmCode[0] = 0x36;
            BAsmCode[1] = AdrLong & 0xff;
...
static void AddLRel(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeLRel);
}

static void AddGen(
        char const* NName, tSymbolSize NSize, Word NCode, Word NExtCode, Byte NShift,
        Byte NMask) {
    if (InstrZ >= GenOrderCnt) {
        exit(255);
    }
...
static void AddAux(char const* NName, Word NCode) {
...
static void AddExt(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeExt);
}

static void AddEmu(char const* NName, Word NCode1, Word NCode2) {
    if (InstrZ >= EmuOrderCnt) {
        exit(255);
    }
...
static void InitFields(void) {
    ...
    AddRel("BHS", 4);
    AddRel("BLO", 5);
    AddRel("LBCC", 0x8000 | 4);
    AddRel("LBCS", 0x8000 | 5);
    AddRel("LBEQ", 0x8000 | 7);
    AddRel("LBGE", 0x8000 | 12);
    AddRel("LBGT", 0x8000 | 14);
    AddRel("LBHI", 0x8000 | 2);
    AddRel("LBLE", 0x8000 | 15);
    AddRel("LBLS", 0x8000 | 3);
    AddRel("LBLT", 0x8000 | 13);
    AddRel("LBMI", 0x8000 | 11);
    AddRel("LBNE", 0x8000 | 6);
    AddRel("LBPL", 0x8000 | 10);
    AddRel("LBRA", 0x8000 | 0);
    AddRel("LBRN", 0x8000 | 1);
    AddRel("LBVC", 0x8000 | 8);
    AddRel("LBVS", 0x8000 | 9);
    AddRel("LBHS", 0x8000 | 4);
    AddRel("LBLO", 0x8000 | 5);
    ...
    InstrZ    = 0;
    AddGen("ADCA", eSymbolSize8Bit, 0x43, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ADCB", eSymbolSize8Bit, 0xc3, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ADCD", eSymbolSize16Bit, 0x83, 0xffff, 0x20,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ADCE", eSymbolSize16Bit, 0x03, 0xffff, 0x20, MModImm | MModDisp16 | MModAbs);
    AddGen("ADDA", eSymbolSize8Bit, 0x41, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ADDB", eSymbolSize8Bit, 0xc1, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ADDD", eSymbolSize16Bit, 0x81, 0xfc, 0x20,
           MModDisp8 | MModImm | MModImmExt | MModDisp16 | MModAbs | MModDispE);
    AddGen("ADDE", eSymbolSize16Bit, 0x01, 0x7c, 0x20,
           MModImm | MModImmExt | MModDisp16 | MModAbs);
    AddGen("ANDA", eSymbolSize8Bit, 0x46, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ANDB", eSymbolSize8Bit, 0xc6, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ANDD", eSymbolSize16Bit, 0x86, 0xffff, 0x20,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    ...
    AddGen("BITA", eSymbolSize8Bit, 0x49, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("BITB", eSymbolSize8Bit, 0xc9, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    ...
    AddGen("CMPA", eSymbolSize8Bit, 0x48, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("CMPB", eSymbolSize8Bit, 0xc8, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    ...
    AddGen("CPD", eSymbolSize16Bit, 0x88, 0xffff, 0x20,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    ...
    AddGen("EORA", eSymbolSize8Bit, 0x44, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("EORB", eSymbolSize8Bit, 0xc4, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("EORD", eSymbolSize16Bit, 0x84, 0xffff, 0x20,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    ...
    AddGen("LDAA", eSymbolSize8Bit, 0x45, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("LDAB", eSymbolSize8Bit, 0xc5, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("LDD", eSymbolSize16Bit, 0x85, 0xffff, 0x20,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    ...
    AddGen("ORAA", eSymbolSize8Bit, 0x47, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ORAB", eSymbolSize8Bit, 0xc7, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("ORD", eSymbolSize16Bit, 0x87, 0xffff, 0x20,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    ...
    AddGen("SBCA", eSymbolSize8Bit, 0x42, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("SBCB", eSymbolSize8Bit, 0xc2, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("SBCD", eSymbolSize16Bit, 0x82, 0xffff, 0x20,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("SBCE", eSymbolSize16Bit, 0x02, 0xffff, 0x20, MModImm | MModDisp16 | MModAbs);
    AddGen("STAA", eSymbolSize8Bit, 0x4a, 0xffff, 0x00,
           MModDisp8 | MModDisp16 | MModAbs | MModDispE);
    AddGen("STAB", eSymbolSize8Bit, 0xca, 0xffff, 0x00,
           MModDisp8 | MModDisp16 | MModAbs | MModDispE);
    AddGen("STD", eSymbolSize16Bit, 0x8a, 0xffff, 0x20,
           MModDisp8 | MModDisp16 | MModAbs | MModDispE);
    AddGen("STE", eSymbolSize16Bit, 0x0a, 0xffff, 0x20, MModDisp16 | MModAbs);
    AddGen("SUBA", eSymbolSize8Bit, 0x40, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("SUBB", eSymbolSize8Bit, 0xc0, 0xffff, 0x00,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    AddGen("SUBD", eSymbolSize16Bit, 0x80, 0xffff, 0x20,
           MModDisp8 | MModImm | MModDisp16 | MModAbs | MModDispE);
    ...
    Regs    = (char const**)malloc(sizeof(char*) * RegCnt);
    Regs[0] = "D";
    Regs[1] = "E";
    Regs[2] = "X";
    Regs[3] = "Y";
