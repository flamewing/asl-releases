...
static void DecodeAdr(tStrComp const* pArg, Byte Erl) {
    static char const Reg8Names[][2]   = {"B", "C", "D", "E", "H", "L", "A"};
    static int const  Reg8Cnt          = sizeof(Reg8Names) / sizeof(*Reg8Names);
    static char const Reg16Names[][3]  = {"BC", "DE", "HL", "\0", "IX", "IY", "SP"};
    static int const  Reg16Cnt         = sizeof(Reg16Names) / sizeof(*Reg16Names);
    static char const IReg16Names[][3] = {"IX", "IY", "SP"};
    static int const  IReg16Cnt        = sizeof(IReg16Names) / sizeof(*IReg16Names);
...
static Boolean ArgPair(char const* Arg1, char const* Arg2) {
    return (((!as_strcasecmp(ArgStr[1].str.p_str, Arg1))
             && (!as_strcasecmp(ArgStr[2].str.p_str, Arg2)))
            || ((!as_strcasecmp(ArgStr[1].str.p_str, Arg2))
                && (!as_strcasecmp(ArgStr[2].str.p_str, Arg1))));
}
...
static void DecodeShift(Word Code) {
    if (ChkArgCnt(1, 1)) {
        DecodeAdr(
                &ArgStr[1],
                (Hi(Code) ? MModReg8 : 0) | MModIndReg | MModIdxReg | MModMem | MModDir);
        switch (AdrType) {
        case ModReg8:
            CodeLen     = 2;
...
static void DecodeALU2(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                &ArgStr[1],
                MModReg8 | MModReg16 | MModIdxReg | MModIndReg | MModDir | MModMem);
        switch (AdrType) {
        case ModReg8:
            DecodeAdr(
                    &ArgStr[2], MModImm
                                        | (((HReg = AdrMode) == AccReg)
                                                   ? MModReg8 | MModIndReg | MModIdxReg
                                                             | MModDir | MModMem
                                                   : 0));
            switch (AdrType) {
            case ModReg8:
        ...
        case ModReg16:
            if ((AdrMode == 2) || ((Code == 0) && (AdrMode >= 4))) {
                HReg = AdrMode;
                DecodeAdr(
                        &ArgStr[2], MModReg16 | MModIndReg | MModIdxReg | MModDir
                                            | MModMem | MModImm);
                switch (AdrType) {
                case ModReg16:
                    CodeLen     = 2;
...
static void DecodeLD(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                &ArgStr[1],
                MModReg8 | MModReg16 | MModIndReg | MModIdxReg | MModDir | MModMem);
        switch (AdrType) {
        case ModReg8:
            HReg = AdrMode;
            DecodeAdr(
                    &ArgStr[2],
                    MModReg8 | MModIndReg | MModIdxReg | MModDir | MModMem | MModImm);
            switch (AdrType) {
            case ModReg8:
                if (HReg == AccReg) {
                    CodeLen     = 1;
        ...
        case ModReg16:
            ...
            DecodeAdr(
                    &ArgStr[2],
                    MModReg16 | MModIndReg | MModIdxReg | MModDir | MModMem | MModImm);
            switch (AdrType) {
            case ModReg16:
                if (HReg == HLReg) {
                    CodeLen     = 1;
...
static void DecodeLDAR(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        Boolean OK;
        Integer AdrInt
                = EvalStrIntExpression(&ArgStr[2], Int16, &OK) - (EProgCounter() + 2);
        if (OK) {
            CodeLen     = 3;
            BAsmCode[0] = 0x17;
...
static void DecodeJR(Word Code) {
    ...
    if (ChkArgCnt(1, 2)) {
        int Cond
                = (ArgCnt == 1) ? DefaultCondition : DecodeCondition(ArgStr[1].str.p_str);
        if (Cond >= ConditionCnt) {
            ...
            tSymbolFlags Flags;
            Integer      AdrInt
                    = EvalStrIntExpressionWithFlags(&ArgStr[ArgCnt], Int16, &OK, &Flags)
                      - (EProgCounter() + 2);
...
static void DecodeRET(Word Code) {
    ...
    if (ChkArgCnt(0, 1)) {
        unsigned Cond
                = (ArgCnt == 0) ? DefaultCondition : DecodeCondition(ArgStr[1].str.p_str);

        if (Cond == DefaultCondition) {
            CodeLen     = 1;
            BAsmCode[0] = 0x1e;
        } else if (Cond >= ConditionCnt) {
            WrStrErrorPos(ErrNum_UndefCond, &ArgStr[1]);
        } else {
            CodeLen     = 2;
            BAsmCode[0] = 0xfe;
...
static void DecodeJRL_CALR(Word Code) {
    if (ChkArgCnt(1, 1)) {
        Boolean OK;
        Integer AdrInt
                = EvalStrIntExpression(&ArgStr[1], Int16, &OK) - (EProgCounter() + 2);
        if (OK) {
            CodeLen     = 3;
            BAsmCode[0] = Code;
            if ((Code == 0x1b) && (AdrInt >= -128) && (AdrInt <= 127)) {
...
static void AddW(char const* Name, Word Code, InstProc Proc) {
    char Str[20];
...
static void AddMove(char const* NName, Byte NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeMove);
}

static void AddShift(char const* NName, Word NCode, Boolean NMay) {
