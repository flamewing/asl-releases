...
static void DecodeJump(Word Index) {
    if (ChkArgCnt(1, 1)) {
        ...
        if (EvalResult.OK) {
            if (AdrWord > (SegLimits[SegCode] - AddCodeSpace)) {
                ...
                for (RegBit = 3, Mask = 0x800; RegBit <= 4; RegBit++, Mask <<= 1) {
                    if (XORVal & Mask) {
                        WAsmCode[CodeLen++] = 0x100a | (RegBit << 7)
...
static void AddAri(char const* NName, Word NCode, Word NDir) {
