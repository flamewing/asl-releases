...
static Boolean DecodeRegLinear(
        tStrComp const* pArg, int Offset, Word* pResult, tEvalResult* pEvalResult) {
...
static Word Lin2PagedRegAddr(tStrComp const* pArg, Word LinAddr, tSymbolFlags Flags) {
...
static Boolean DecodeReg(tStrComp const* pArg, int Offset, Word* pResult) {
...
static Boolean DecodeRegAppendix(tStrComp* pArg, char const* pAppendix, Word* pResult) {
    int     ArgLen = strlen(pArg->str.p_str), AppLen = strlen(pAppendix);
    Boolean Result = False;

    if ((ArgLen > AppLen)
        && !as_strcasecmp(pArg->str.p_str + (ArgLen - AppLen), pAppendix)) {
...
static Boolean DecodeBitSymbol(
        tStrComp const* pArg, Word* pBitSymbol, tEvalResult* pEvalResult) {
    char* pSplit;
...
static Boolean DecodeRegBitPacked(tStrComp const* pArg, Word* pPackedResult) {
...
static void DecodeOneReg(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeReg(&ArgStr[1], 0, &Reg)) {
    ...
    if (!as_strcasecmp(ArgStr[1].str.p_str, "W")) {
        if (*ArgStr[2].str.p_str == '#') {
            ...
            WAsmCode[CodeLen++] = 0x042;
                ...
                !strncmp(ArgStr[2].str.p_str, "/", 1) && DecodeReg(&ArgStr[2], 1, &Reg)) {
            WAsmCode[CodeLen++] = 0x240 | Reg;
        } else if (
                !strncmp(ArgStr[2].str.p_str, "--", 2)
                && DecodeReg(&ArgStr[2], 2, &Reg)) {
        ...
        if (!as_strcasecmp(ArgStr[1].str.p_str, "!OPTION")) {
            ...
            WAsmCode[CodeLen++] = 0x003;
                ...
                !strncmp(ArgStr[1].str.p_str, "!", 1) && DecodeReg(&ArgStr[1], 1, &Reg)) {
            if (ChkRange(Reg, 0, 7)) {
...
static void DecodeJMP_CALL(Word Code) {
    ...
    if (!mFirstPassUnknown(EvalResult.Flags) && IsCall && (Addr & 0x100)) {
        WrStrErrorPos(ErrNum_NotFromThisAddress, &ArgStr[1]);
    } else {
        if (!mFirstPassUnknown(EvalResult.Flags)
            && ((Reg_STATUS & 0xe0) != ((Addr >> 4) & 0xe0))) {
...
static void DecodeBIT(Word Code) {
    ...
    UNUSED(Code);
    if (ChkArgCnt(1, 1) && DecodeBitSymbol(&ArgStr[1], &BitSymbol, &EvalResult)) {
        *ListLine = '=';
...
static void SwitchTo_SX20(void) {
    PFamilyDescr const pDescr = FindFamilyByName("SX20");
#define ASSUMESX20Count (sizeof(ASSUMESX20s) / sizeof(*ASSUMESX20s))
    static ASSUMERec const ASSUMESX20s[] = {
