...
static tSymbolSize CutSizeSuffix(tStrComp const* pArg) {
...
static void DecideSize(
        LongWord Mask, tStrComp const* pArg, tAdrMode Mode8, tAdrMode Mode16,
        tAdrMode Mode24, Byte Part8, Byte Part16, Byte Part24, Boolean IsCode,
        tAdrVals* pAdrVals) {
    ...
    if (OK) {
        ...
        /* this may only happen if SizeTypes was forced to 24 Bit because of code
         * addessing: */

        if ((Size == eSymbolSize24Bit) && (Mode24 == eModNone)
 ...
 * \fn     DecideIndirectSize(LongWord Mask, const tStrComp *pArg,
                              tAdrMode Mode8_8, tAdrMode Mode8_16, tAdrMode Mode16_16,
 tAdrMode Mode16_24, Byte Part8_8, Byte Part8_16, Byte Part16_16, Byte Part16_24, tAdrVals
 *pAdrVals)
...
Boolean DecideIndirectSize(
    ...
    Boolean      OK;
    int          Offset;
    tSymbolSize  AddrSize = eSymbolSizeUnknown, PtrSize = eSymbolSizeUnknown;
    IntType      SizeType;
    Word         Address;
    tSymbolFlags Flags;
    ...
    if (OK) {
        ...
        if (eSymbolSizeUnknown == AddrSize) {
            if ((Address <= 0xff) && (PtrSize == eSymbolSize8Bit)
                && (Mode8_8 != eModNone)) {
                AddrSize = eSymbolSize8Bit;
            } else if (
                    (Address <= 0xff) && (PtrSize == eSymbolSize16Bit)
                    && (Mode8_16 != eModNone)) {
                AddrSize = eSymbolSize8Bit;
            } else {
                AddrSize = eSymbolSize16Bit;
...
static Boolean ChkAdrMode(
        tAdrMode* pAdrMode, LongWord Mask, tErrorNum ErrorNum, tStrComp const* pArg) {
...
static Boolean ChkAdrValsMode(
        tAdrVals* pAdrVals, LongWord Mask, tErrorNum ErrorNum, tStrComp const* pArg) {
...
static Boolean DecodeRegCore(tStrComp const* pArg, tAdrMode* pResult) {
    *pResult = eModNone;
    ...
    if ((!as_strcasecmp(pArg->str.p_str, "S"))
        || ((pCurrCPUProps->Core == eCoreSTM8)
            && !as_strcasecmp(pArg->str.p_str, "SP"))) {
        *pResult = eModS;
        return True;
    }
...
static Boolean DecodeReg(tStrComp const* pArg, tAdrMode* pResult, LongWord Mask) {
    return DecodeRegCore(pArg, pResult) && ChkAdrMode(pResult, Mask, ErrNum_InvReg, pArg);
}
...
static Boolean DecodeAdr(
        tStrComp const* pArg, LongWord Mask, Boolean IsCode, tAdrVals* pAdrVals) {
    Boolean OK;
    int     ArgLen;
    ...
    if (*pArg->str.p_str == '#') {
        Word Value = EvalStrIntExpressionOffs(
                pArg, 1, (OpSize == eSymbolSize16Bit) ? Int16 : Int8, &OK);
        if (OK) {
    ...
    if ((*pArg->str.p_str == '[') && (pArg->str.p_str[ArgLen - 1] == ']')) {
        ...
        Comp.str.p_str[ArgLen - 2] = '\0';
        Comp.Pos.Len--;
        OK = DecideIndirectSize(
                Mask, &Comp, eModIAbs8, eModIAbs16, eModI16Abs16, eModI16Abs24, 0xb, 0xc,
                0xc, 0xb, pAdrVals);
    ...
    if (IsIndirect(pArg->str.p_str)) {
        ...
        if (!p) {
            pAdrVals->Part = 0xf;
            if (!as_strcasecmp(Comp.str.p_str, "X")) {
                pAdrVals->Mode = eModIX;
        ...
        if ((*Left.str.p_str == '[') && (Left.str.p_str[ArgLen - 1] == ']')) {
            ...
            if (YReg) {
                if (DecideIndirectSize(
                            Mask, &Right, eModIYAbs8, eModIYAbs16, eModNone,
                            eModI16YAbs24, 0xe, 0xd, 0x0, 0xa, pAdrVals)) {
                    AddPrefix(0x91);
                }
            } else {
                if (DecideIndirectSize(
                            Mask, &Right, eModIXAbs8, eModIXAbs16, eModI16XAbs16,
                            eModI16YAbs24, 0xe, 0xd, 0xd, 0xa, pAdrVals)) {
            ...
            if (YReg) {
                DecideSize(
                        Mask, &Left, eModIY8, eModIY16, eModIY24, 0xe, 0xd, 0xa, IsCode,
                        pAdrVals);
            } else if (SPReg) {
                DecideSize(
                        Mask, &Left, eModISP8, eModNone, eModNone, 0x1, 0x0, 0x0, IsCode,
                        pAdrVals);
            } else {
                DecideSize(
                        Mask, &Left, eModIX8, eModIX16, eModIX24, 0xe, 0xd, 0xa, IsCode,
                        pAdrVals);
            }
            if ((pAdrVals->Mode != eModNone) && YReg) {
                AddPrefix(0x90);
            }
        }
    ...
    DecideSize(
            Mask, pArg, eModAbs8, eModAbs16, eModAbs24, 0xb, 0xc, 0xb, IsCode, pAdrVals);
...
static LongWord ConstructMask(LongWord TotMask, tSymbolSize OpSize) {
    if (pCurrCPUProps->Core == eCoreST7) {
        ...
        TotMask
                &= ~(MModISP8 | MModI16Abs16 | MModI16XAbs16 | MModXL | MModYL | MModXH
                     | MModYH);
...
static LongWord AssembleBitSymbol(Byte BitPos, Word Address) {
    return (BitPos & 7) | (((LongWord)Address & 0xffff) << 3);
}
...
static Boolean DecodeBitArg2(
    ...
    Addr = EvalStrIntExpression(
            pRegArg, (pCurrCPUProps->Core == eCoreSTM8) ? UInt16 : UInt8, &OK);
    if (!OK) {
        return False;
    }
    ...
    *pResult = 0;
    ...
    if (Start == Stop) {
        ...
        *pResult = EvalStrIntExpressionWithResult(
                &ArgStr[Start], (pCurrCPUProps->Core == eCoreSTM8) ? UInt19 : UInt11,
                &EvalResult);
        if (EvalResult.OK) {
...
static Boolean DecodeBitAdrWithIndir(
        int Start, int Stop, Byte* pBitPos, tAdrVals* pAdrVals) {
    Boolean OK;
    ...
    if (Start == Stop) {
        ...
        *pBitPos = EvalBitPosition(&ArgStr[Stop], &OK);
        ...
        return DecodeAdr(
                &ArgStr[Start],
                (pCurrCPUProps->Core == eCoreSTM8) ? MModAbs16 : (MModAbs8 | MModIAbs8),
                False, pAdrVals);
    } else {
        return False;
    }
...
static void CompleteCode(tAdrVals const* pAdrVals) {
...
static void WriteMOV(tAdrVals* pDestAdrVals, tAdrVals* pSrcAdrVals) {
    if ((pDestAdrVals->Mode == eModAbs16) || (pSrcAdrVals->Mode == eModAbs16)
...
static void DecodeFixed(Word Code) {
    ...
    } else if (Hi(Code) && (pCurrCPUProps->Core != eCoreSTM8)) {
...
static void DecodeLD(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        Mask = ConstructMask(
                MModA | MModX | MModY | MModS | MModXL | MModYL | MModXH | MModYH
                        | MModImm | MModAbs8 | MModAbs16 | MModIX | MModIX8 | MModIX16
                        | MModIY | MModIY8 | MModIY16 | MModISP8 | MModIAbs8 | MModIAbs16
                        | MModI16Abs16 | MModIXAbs8 | MModIXAbs16 | MModI16XAbs16
        ...
        if (DecodeAdr(&ArgStr[1], Mask, False, &DestAdrVals)) {
            ...
            case eModA:
                Mask = ConstructMask(
                        MModImm | MModX | MModY | MModS | MModXL | MModYL | MModXH
                                | MModYH | MModAbs8 | MModAbs16 | MModIX | MModIX8
                                | MModIX16 | MModIY | MModIY8 | MModIY16 | MModISP8
                                | MModIAbs8 | MModIAbs16 | MModI16Abs16 | MModIXAbs8
                                | MModIXAbs16 | MModI16XAbs16 | MModIYAbs8 | MModIYAbs16,
                        eSymbolSize8Bit);
            ...
            case eModX:
                ...
                Mask = ConstructMask(
                        MModA | MModY | MModS | MModImm | MModAbs8 | MModAbs16 | MModIX
                                | MModIX8 | MModIX16 | MModISP8 | MModIAbs8 | MModIAbs16
                                | MModI16Abs16 | MModIXAbs8 | MModIXAbs16 | MModI16XAbs16,
                        OpSize);
                if (DecodeAdr(&ArgStr[2], Mask, False, &SrcAdrVals)) {
                    switch (SrcAdrVals.Mode) {
            ...
            case eModY:
                ...
                Mask = ConstructMask(
                        MModA | MModX | MModS | MModImm | MModAbs8 | MModAbs16 | MModIY
                                | MModIY8 | MModIY16 | MModISP8 | MModIAbs8 | MModIAbs16
                                | MModIYAbs8 | MModIYAbs16,
                        OpSize);
                if (DecodeAdr(&ArgStr[2], Mask, False, &SrcAdrVals)) {
                    switch (SrcAdrVals.Mode) {
            ...
            default: {
                ...
                /* set unknown size to allow X&Y also on STM8 if LD is written instead of
                 * LDW */
                ...
                if (Result) {
                    ...
                    case eModA:
                        Mask = ConstructMask(
                                MModAbs8 | MModAbs16 | MModIX | MModIX8 | MModIX16
                                        | MModIY | MModIY8 | MModIY16 | MModISP8
                                        | MModIAbs8 | MModIAbs16 | MModI16Abs16
                                        | MModIXAbs8 | MModIXAbs16 | MModI16XAbs16
                                        | MModIYAbs8 | MModIYAbs16,
                                eSymbolSize8Bit);
                        if (ChkAdrValsMode(
                                    &DestAdrVals, Mask, ErrNum_InvAddrMode, &ArgStr[1])) {
                            BAsmCode[PrefixCnt]
                                    = (DestAdrVals.Mode == eModISP8)
                                              ? 0x6b
                                              : (0x07 + (DestAdrVals.Part << 4));
                    ...
                    case eModX:
                        ...
                        if (pCurrCPUProps->Core == eCoreST7) {
                            Mask |= MModIX | MModIX8 | MModIX16 | MModIAbs8 | MModIXAbs8
                                    | MModIXAbs16;
                        }
                        if (pCurrCPUProps->Core == eCoreSTM8) {
                            Mask |= MModIY | MModIY8 | MModIY16 | MModIYAbs16 | MModISP8
                                    | MModI16Abs16;
                        }
                        if (ChkAdrValsMode(
                                    &DestAdrVals, Mask, ErrNum_InvAddrMode, &ArgStr[1])) {
                    ...
                    case eModY:
                        Mask = MModAbs8 | MModAbs16 | MModIAbs16;
                        if (pCurrCPUProps->Core == eCoreST7) {
                            Mask |= MModIY | MModIY8 | MModIY16 | MModIAbs8 | MModIYAbs8
                                    | MModIYAbs16;
                        }
                        if (pCurrCPUProps->Core == eCoreSTM8) {
                            Mask |= MModIX | MModIX8 | MModIX16 | MModIXAbs16
                                    | MModI16XAbs16 | MModISP8;
                        }
                        if (ChkAdrValsMode(
                                    &DestAdrVals, Mask, ErrNum_InvAddrMode, &ArgStr[1])) {
                            switch (DestAdrVals.Mode) {
...
static void DecodeLDW(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if (pCurrCPUProps->Core != eCoreSTM8) {
        WrStrErrorPos(ErrNum_InstructionNotSupported, &OpPart);
    } else if (*AttrPart.str.p_str) {
        WrStrErrorPos(ErrNum_UseLessAttr, &AttrPart);
    } else {
        Mask = ConstructMask(
                MModX | MModY | MModS | MModAbs8 | MModAbs16 | MModIX | MModIX8 | MModIX16
                        | MModIY | MModIY8 | MModIY16 | MModISP8 | MModIAbs16
                        | MModI16Abs16 | MModIXAbs16 | MModI16XAbs16 | MModIYAbs16,
                OpSize = eSymbolSize16Bit);
        if (DecodeAdr(&ArgStr[1], Mask, False, &DestAdrVals)) {
            switch (DestAdrVals.Mode) {
            case eModX:
                Mask = ConstructMask(
                        MModY | MModS | MModImm | MModAbs8 | MModAbs16 | MModIX | MModIX8
                                | MModIX16 | MModISP8 | MModIAbs16 | MModI16Abs16
                                | MModIXAbs16 | MModI16XAbs16,
                        eSymbolSize16Bit);
                if (DecodeAdr(&ArgStr[2], Mask, False, &SrcAdrVals)) {
                    switch (SrcAdrVals.Mode) {
                    case eModY:
            ...
            case eModY:
                PrefixCnt = 0;
                Mask      = ConstructMask(
                        MModX | MModS | MModImm | MModAbs8 | MModAbs16 | MModIY | MModIY8
                                | MModIY16 | MModISP8 | MModIAbs16 | MModIYAbs16,
                        eSymbolSize16Bit);
...
static void DecodeLDF(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        case eModA:
            if (DecodeAdr(
                        &ArgStr[2],
                        MModAbs24 | MModIX24 | MModIY24 | MModI16XAbs24 | MModI16YAbs24
                                | MModI16Abs24,
                        False, &AdrVals)) {
                switch (AdrVals.Mode) {
                case eModAbs24:
...
static void DecodeCP(Word IsCPW) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if (IsCPW && (pCurrCPUProps->Core != eCoreSTM8)) {
        WrStrErrorPos(ErrNum_InstructionNotSupported, &OpPart);
    } else if (*AttrPart.str.p_str) {
        WrStrErrorPos(ErrNum_UseLessAttr, &AttrPart);
    } else {
        ...
        if (DecodeReg(&ArgStr[1], &DestMode, (IsCPW ? 0 : MModA) | MModX | MModY)) {
            ...
            case eModA:
                Mask = ConstructMask(
                        MModImm | MModAbs8 | MModAbs16 | MModIX | MModIX8 | MModIX16
                                | MModIY | MModIY8 | MModIY16 | MModISP8 | MModIAbs8
                                | MModIAbs16 | MModI16Abs16 | MModIXAbs8 | MModIXAbs16
                                | MModI16XAbs16 | MModIYAbs8 | MModIYAbs16,
                        eSymbolSize8Bit);
                if (DecodeAdr(&ArgStr[2], Mask, False, &SrcAdrVals)) {
            ...
            case eModX:
                ...
                if (pCurrCPUProps->Core == eCoreST7) {
                    Mask |= MModIAbs8 | MModIXAbs8 | MModIX | MModIX8 | MModIX16
                            | MModIXAbs16;
                }
                if (pCurrCPUProps->Core == eCoreSTM8) {
                    Mask |= MModIY | MModIY8 | MModIY16 | MModISP8 | MModI16Abs16
                            | MModIYAbs16;
                    OpSize = eSymbolSize16Bit;
                }
                if (DecodeAdr(&ArgStr[2], Mask, False, &SrcAdrVals)) {
            ...
            case eModY:
                ...
                Mask      = MModImm | MModAbs8 | MModAbs16 | MModIAbs16;
                if (pCurrCPUProps->Core == eCoreST7) {
                    Mask |= MModIAbs8 | MModIYAbs8 | MModIY | MModIY8 | MModIY16
                            | MModIYAbs16;
                }
                if (pCurrCPUProps->Core == eCoreSTM8) {
...
static void DecodeAri16(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        case eModY:
            OpSize = eSymbolSize16Bit;
            if (DecodeAdr(
                        &ArgStr[2], MModImm | MModAbs16 | MModISP8, False, &SrcAdrVals)) {
                switch (SrcAdrVals.Mode) {
                case eModImm:
                    if (PrefixCnt) {
...
static void DecodeRMW(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if (IsW && (pCurrCPUProps->Core != eCoreSTM8)) {
        WrStrErrorPos(ErrNum_InstructionNotSupported, &OpPart);
    } else if (*AttrPart.str.p_str) {
        WrStrErrorPos(ErrNum_UseLessAttr, &AttrPart);
    } else {
        ...
        Mask = MModX | MModY;
        if (!IsW) {
            Mask |= MModA | MModAbs8 | MModIX | MModIX8 | MModIY | MModIY8;
            if (pCurrCPUProps->Core == eCoreST7) {
                Mask |= MModIAbs8 | MModIXAbs8 | MModIYAbs8;
            }
            if (pCurrCPUProps->Core == eCoreSTM8) {
                Mask |= MModAbs16 | MModIX16 | MModIY16 | MModISP8 | MModIAbs16
                        | MModI16Abs16 | MModIXAbs16 | MModI16XAbs16 | MModIYAbs16;
            }
        }
...
static void DecodeDIV(Word IsW) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrStrErrorPos(ErrNum_UseLessAttr, &AttrPart);
            ...
            DecodeAdr(&ArgStr[1], MModX | (IsW ? 0 : MModY), False, &DestAdrVals)
...
static void DecodeBitOp(Word Code) {
    ...
    if (!ChkArgCnt(1, 2))
        ;
    else if ((Hi(Code == 0x90)) && (pCurrCPUProps->Core != eCoreSTM8)) {
        WrStrErrorPos(ErrNum_InstructionNotSupported, &OpPart);
    } else if (*AttrPart.str.p_str) {
        WrStrErrorPos(ErrNum_UseLessAttr, &AttrPart);
...
static void DecodeBTJF_BTJT(Word Code) {
    ...
    if (!ChkArgCnt(2, 3))
        ...
        AdrInt = EvalStrIntExpressionWithFlags(
                         &ArgStr[3], pCurrCPUProps->AddrIntType, &OK, &Flags)
                 - (EProgCounter() + PrefixCnt + 1 + AdrVals.Cnt + 1);
        if (OK) {
            if (!mSymbolQuestionable(Flags) && ((AdrInt < -128) || (AdrInt > 127))) {
...
static void DecodeJP_CALL(Word Code) {
    if (!ChkArgCnt(1, 1))
        ...
        Mask = ConstructMask(
                MModAbs8 | MModAbs16 | MModIX | MModIX8 | MModIX16 | MModIY | MModIY8
                        | MModIY16 | MModIAbs8 | MModIAbs16 | MModI16Abs16 | MModIXAbs8
                        | MModIXAbs16 | MModI16XAbs16 | MModIYAbs8 | MModIYAbs16,
                eSymbolSizeUnknown);
...
static void DecodeRel(Word Code) {
    if (*AttrPart.str.p_str) {
        ...
        AdrInt = EvalStrIntExpressionWithFlags(
                         &ArgStr[1], pCurrCPUProps->AddrIntType, &OK, &Flags)
                 - (EProgCounter() + 2 + PrefixCnt);
        if (OK) {
            if (!mSymbolQuestionable(Flags) && ((AdrInt < -128) || (AdrInt > 127))) {
...
static void DecodeRxWA(Word Code) {
    ...
    if (*AttrPart.str.p_str) {
        ...
        WrStrErrorPos(ErrNum_InstructionNotSupported, &OpPart);
            ...
            ((ArgCnt == 1) || DecodeReg(&ArgStr[2], &SrcMode, MModA))
            && DecodeAdr(&ArgStr[1], MModX | MModY, False, &DestAdrVals)) {
        BAsmCode[PrefixCnt] = Code;
        CodeLen             = PrefixCnt + 1;
    }
}
...
static void AddAri(char const* NName, Word NCode, Boolean NMay) {
    AddInstTable(InstTable, NName, NCode | (NMay << 8), DecodeAri);
}

static void AddAri16(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeAri16);
}

static void AddRMW(char const* NName, Byte NCode) {
    char WName[10];
...
static void AddRel(char const* NName, Word NCode) {
...
static void InitFields(void) {
    ...
    AddFixed("TRAP", 0x83);
    AddFixed("WFI", 0x8f);
    AddFixed("BREAK", 0x018b);
    AddFixed("WFE", 0x728f);
    AddFixed("RVF", 0x019c);
    AddFixed("RETF", 0x0187);
    AddFixed("CCF", 0x018c);
    ...
    AddAri16("ADDW", 0x090b);
    AddAri16("SUBW", 0x0200);
