...
{\large Alfred Arnold, Stefan Hilse, Stephan Kanthak, Oliver
 Sellke, Vittorio De Tomasi, flamewing}
\vspace{1cm}\par
...
{\large Note: This is a slightly modified version by flamewing}
\end{raggedright}
...
\end{itemize}
You can contact me (flamewing) as follows:
\begin{itemize}
\item{by E-Mail: \tty{flamewing.sonic@gmail.com}}
\end{itemize}
...
\item{Intel MCS-48/41, including Siemens SAB80C382, and the OKI variants}
...
In theory, any platform with a reasonably modern C compiler (must support C11)
should support AS. This includes modern versions of GCC, clang, Windows with
Microsoft's compiler, and AS even compiles with TinyC Compiler.
...
the accompanying tools and auxiliary files readily compiled, so you can
immediately start to use it after unpacking the archive to the desired
destination on your hard drive.
Binary distributions are made for widespread platforms.  A source distribution
in contrast contains the complete set of C sources to generate AS; it is
...
{\bf Directory DOC} & \\
\hline
AS.DOC            & documentation, ASCII format \\
AS.HTML           & documentation, HTML format \\
AS.TEX            & documentation, LaTeX format \\
\hline
...
There is no need for a special installation prior to usage of AS.  It is
sufficient to unpack the archive in a fitting place and to add a few minor
settings.  For example, this is an installation a user used to UNIX-like
operating systems might choose:
...
unpack the archive, keeping the path names stored in the archive (when
...
\item{the directory named in the {\tt AS\_MSGPATH} environment variable,
      or alternatively the directories listed in the {\tt PATH} environment
      variable;}
...
interpret every letter as an individual switch.  For example, if you
write
\begin{verbatim}
 -queit
\end{verbatim}
...
a switch without the optional argument, AS will try to interpret the
...
the name of the key file by a \tty{@} ahead in the \tty{ASCMD} variable,
e.g.
\begin{verbatim}
...
operating systems to find out the amount of available memory resp. stack,
...
the attribute is is the specification of the operand size, for example in
...
queried via the built-in function \tty{SYMTYPE}, e.g.:
\begin{verbatim}
Label:
        .
        .
...
there is an even simpler variant: If one places a single plus or minus
sign as a label, this is converted to special internal symbol names.
Those symbols are ``unpronounceable" and unpredictable, and should not be
relied on in a program; instead, they are referenced via the special names
{\tt - -- ---} respectively {\tt + ++ +++}, which refer to the three last
`minus symbols' and the next three `plus symbols'.  Therefore, the selection
between these two variants depends on whether one wants to forward- or
backward-reference a symbol.
...
things would become too puzzling otherwise (this only a good advice,
...
Hex       & Suffix H      & Prefix \$   & Prefix 0x   & \tty{X$'$..$'$} or \tty{H$'$..$'$} \\
\ii{Ident}& \tty{hexh}    & \tty{\$hex} & \tty{0xhex} & \tty{x$'$hex$'$} \\
          &               &             &             & \tty{h$'$hex$'$} \\
Binary    & Suffix B      & Prefix \%   & Prefix 0b   & \tty{O$'$..$'$} \\
\ii{Ident}& \tty{binb}    & \tty{\%bin} & \tty{0bbin} & \tty{b$'$bin$'$} \\
Octal     & Suffix O or Q & Prefix @    & Prefix 0    & \tty{B$'$..$'$} \\
...
prefixed (e.g. 0F0H instead of F0H).  The Motorola and C syntaxes, which
...
\end{verbatim}
\bb{CAUTION!} The assembler first tries to interpret a constant as an
...
according decimal ASCII value.  The numerical value may alternatively be
...
consisting of multiple constants, which are sometimes called ``multi
character constants'':
\begin{verbatim}
'A'    ==$41
'AB'   ==$4142
...
TAN      & tangent              & $arg \neq (2n+1)*\pi\mathbin{/}2$ & floating point \\
...
as the second passes' new symbol values might again shorten or
...
a drastic effect: the translation table is reinitialized to its initial
...
\hline
\multicolumn{11}{|l|}{$^{7}$ size and availability depend on target processor} \\
\hline
...
compatible to the 6809, but unofficially offers more registers and a
...
with a built-tin banking mechanism.  Furthermore, it features some
...
versions with a strongly shrunk instruction set, for which the 8022
...
instructions become that have to be emulated in software. While, for example,
the 29245 doesn't even have a hardware multiplier, the two representatives in
...
\tty{STDDEF3X.INC}.  The C4x variants are source-code upward
...
larger, and that there are representatives with differently sized
...
option!  In case of the aforementioned AT90S8515, this option is even
...
mapped into the physical address space.  AS initially assumes the reset
...
register specifies which 256 byte page of the address space is addressed
...
both program space and program memory in pages of 2 KBytes.  Page one of the
...
approximation, AS regards program space to be linear and of the size of
...
one-megabyte address space via banking.  Banking is realized with the
...
Processors with a 78K3 core have register banks that consist of
...
The assembler will then insert the alternate register numbers
...
Aside from many new instructions and addressing modes, the most
...
had to fill the gap.  In detail, these are PCB for the program code, DTB
...
If the address is within the window, it will compute the correct t16-bit offset.
...
\begin{verbatim}
        bne     target
\end{verbatim}
with a longer sequence of same functionality, in case the branch target is
...
skip:
\end{verbatim}
This feature however has the side effect that there is no unambiguous
...
\subsection{MACRO}
\ttindex{MACRO}\ttindex{ENDM}\ttindex{ENDR}
\label{SectMacros}
...
Note: AS defines `ENDR' as a synomym of `ENDM', which can be used whenever
`ENDM' is used.
...
\end{verbatim}
Additionally, parameter names surrounded by \\ will be replaced as one.
For example:
\begin{verbatim}
concat  macro   part1,part2
        call    \part1\\part2\
...
listing, after arguments have been expanded.  This can significantly increase
...
in curly braces is shortened so that it only contains the last (and therefore
...
is similar to \tty{IRP}, but it uses a specifiable amount of arguments each
expansion.  \tty{IRPN} needs a number of arguments per iteration as its first
parameter (lets call this \tty{count}), \tty{count} symbol names, and an
(almost) arbitrary number of parameters that are sequentially inserted in
batches of \tty{count} parameters into the block of code.  For example, one
can write
\begin{verbatim}
...
\tty{IRPC} is a variant of \tty{IRP} where the first argument's occurrences
...
processor and it is unfortunately not that easy to fix.  I hope no one
minds...
...
each step...
An alternative to recursion is to use {\tt IRP} or {\tt IRPN} instead of
{\tt REPT}:
\begin{verbatim}
pushlist  macro reg
          irp  reg, ALLARGS
...
one has to use {\tt SHIFT} instead.
...
{\tt MAXNEST} allows to adjust how often a macro may be called recursively
...
The definition of a structure is begun with the statement
...
with \tty{STRUCT}, the current program counter is saved and reset
...
separated with a special character.  By default, this is the underline
...
\item{\tty{DOTS} instructs AS to use the dot as connecting character
      instead of the underline.  It should however be pointed out that
...
structure's name via the same rules - or alternatively with the label name
...
once again getting prepended to this concatenated name.  This may look like
the following:
\begin{verbatim}
TreeRec struct
left    dd         ?
...
like \tty{IF...} or \tty{SWITCH...}.  These statements work at
...
statement \tty{MACEXP\_OVR}: it accepts the same arguments like
...
de-initialization string with \tty{PRTEXIT $<$string$>$}.
...
sub-procedures(functions), AS requires an explicit definition of an
...
The assembler primarily tries to open the file in the directory
containing the source file with the \tty{INCLUDE} statement.  This
...
listing and the console (the latter only if the quiet mode is not used).
...
set is selected (see section \ref{SectPseudoInst}).  This set may be augmented
...
\begin{verbatim}
        RELAXED ON
\end{verbatim}
right at the program's beginning, one may further use any syntax
...
Though the assembler strives to behave like the corresponding "original
...
makes programmers' and tool developers' lives simpler...how does one
...
AS acts like this were one large unified address space, with the following
layout:
\begin{itemize}
...
missing (hopefully no one misses them!).  I will finish this as soon
...
which turns \$ff into the desired value \$ffff.  AS is aware of this
...
as ``don't care''.  However, if one specifies a long-word move or omits the
...
there are many short forms for instructions with common operands.  For
...
{\em src, dest}         & \tty{A, W, X, Y, Z, DPL, DPH} (Register) \\
                        & \tty{T, TL, TH} (Timer, upper/lower half) \\
                        & \tty{(DP), M} (Memory addressed by DPH/DPL) \\
...
instructions have enough space to accommodate seven address bits,
...
intra-segment branches if they refer to targets in this single code
...
macros:
\begin{itemize}
\item{\tty{NOP} is a short form for \tty{MOVE AUX,AUX}}
\item{\tty{HALT} is a short form for {\tt JMP \verb!*!}}
\item{\tty{XML ii} is a short form for \tty{XMIT ii,R12} (only 8X305)}
\item{\tty{XMR ii} is a short form for \tty{XMIT ii,R13} (only 8X305)}
\item{\tty{SEL $<$busobj$>$} is a short form for \tty{XMIT $<$adr$>$,IVL/IVR},
...
to write hexadecimal constants instead of the C syntax.  All right, I
...
organization in 16 bit word as used by the original Atmel assembler.  This
...
\begin{verbatim}
        ...
        OUT     (C),0
\end{verbatim}
Their operation should be clear.  \bb{CAUTION!}  No one can
...
statements.  It is however also possible to regard work registers as
...
:24) to the displacement resp. the address.

The syntax chosen by Toshiba is a bit unfortunate in the respect of
choosing an single quote (\verb!'!) to reference the previous register bank.  The
...
``built-ins'' and are therefore also available without the include file:
\begin{itemize}
...
market this processor at all.  This of course had the result that
...
...Fast forward 20: all of a sudden, people are contacting me
and tell me that Toshiba actually did sell
...
remaining uncertainties.  Nevertheless, errors in this code generator
...
These processors have the ability to map their code ROM page-wise into the
...
believe that they are binary compatible: To get space for the longer
...
{\tt SHIFT RIGHT}, {\it cnt}, {\it Reg} & {\tt SFTR} {\it [cnt,]} {\it Reg} & arithmetic Shift \\
...
If a code file contains multi-byte values, they are stored in little
...
needed by a re-translator to step upward through a tree of sections
...
other hand, this should not be an obstacle for symbol back-translation
...
notation,  either by all ending in \verb!h!, or by prepending a dollar
...
address range (as displayed e.g. by PLIS) starts at zero and is
continuous.
...
\end{verbatim}
It is allowed to specify a single dollar character or `0x' as start
...
\errentry{  40}{FPU possibly cannot read this value ($>$=1E1000)}
               ...
               {The BCD-floating point format used by the 680x0-FPU
                ...
                datasheets, this cannot be fully interpreted. The
...
\errentry{ 150}{localization operator senseless out of a section}
               {warning}
               {A prefixed \@ must be used, so that it is
...
\errentry{ 180}{address is not properly aligned}
               ...
               {An address was used that is not an exact multiple of the
                operand size. Although the CPU datasheet forbids this, the
...
\errentry{ 250}{instruction crosses page boundary}
               {warning}
               {The parts of a machine statement partially lie on
...
\errentry{ 300}{bit number will be truncated}
               {warning}
               {This instruction only operates on byte resp. long word
...
\errentry{1200}{unknown instruction}
               ...
               {An instruction was used that is neither an AS instruction, nor a
                known machine instruction for the current processor type.}
               {none}
...
\errentry{1352}{address must be aligned}
               {error}
               {At this point, only aligned (i.e. a multiple of 2,4,8...) addresses
...
\item{version 1.33:
      ...
      \item{The instructions \tty{TITLE, PRTINIT, PRTEXIT, ERROR},
            ...
            \verb!"! instead of \verb!'! characters.  This is also true
...
\subsubsection{test\_driver.c}

This is used by \em{ctest} to run tests.
