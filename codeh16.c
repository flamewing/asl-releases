...
#define MModeAll                                                                        \
    (MModeReg | MModeIReg | MModePost | MModePre | MModeImm | MModeAbs | MModeIRegScale \
     | MModeIdxScale | MModePCIdxScale | MModePCRel | MModeDoubleIndir)
...
static tFormat DecodeFormat(unsigned FormatMask) {
    static char const Formats[][3] = {"G", "Q", "R", "RQ", ""};
    tFormat           Result;
...
static Boolean SplitOpSize(tStrComp* pArg, tSymbolSize* pSize) {
    static char const Suffixes[][4] = {".B", ":8", ".W", ":16", ".L", ":32"};
...
static char const ScaleSuffixes[][3] = {"*1", "*2", "*4", "*8"};
...
static Boolean DecodeRegCore(char const* pArg, Byte* pResult, Byte* pPrefix) {
    Boolean OK;
    ...
    if ((strlen(pArg) > 1) && (as_toupper(*pArg) == 'R')) {
        *pPrefix = 0x00;
    } else if (
            (strlen(pArg) > 2) && (as_toupper(*pArg) == 'C')
            && (as_toupper(pArg[1]) == 'R')) {
        *pPrefix = PREFIX_CRn;
    } else if (
            (strlen(pArg) > 2) && (as_toupper(*pArg) == 'P')
            && (as_toupper(pArg[1]) == 'R')) {
        *pPrefix = PREFIX_PRn;
    } else {
        return False;
...
static tRegEvalResult DecodeReg(
        tStrComp const* pArg, Byte* pReg, Byte* pPrefix, Boolean AllowPrefix,
        Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static tRegEvalResult DecodeRegWithSize(
        tStrComp const* pArg, Byte* pReg, tSymbolSize* pSize, Byte* pPrefix,
        Boolean MustBeReg) {
    String   Str;
    tStrComp Copy;
...
static Boolean CheckSup(tStrComp const* pArg) {
    if (!SupAllowed) {
...
static Boolean DecodeCReg(tStrComp const* pArg, Byte* pResult) {
    tCReg const* pReg;
...
static Boolean ChkSize(LongInt Value, tSymbolSize OpSize) {
    ...
    case eSymbolSize8Bit:
        return ChkRange(Value, -128, 127);
    case eSymbolSize16Bit:
        return ChkRange(Value, -32768, 32767);
    default:
        return True;
    }
}
...
static Boolean ChkSizePCRelDisplacement(
        LongInt* pAbsAddr, tSymbolSize SymbolSize, LongInt DispAddr) {
    switch (SymbolSize) {
    case eSymbolSize8Bit:
        *pAbsAddr -= DispAddr + 1;
...
static Boolean ChkSizePCRelBranch(
        tStrComp const* pArg, LongInt* pAbsAddr, tSymbolSize SymbolSize,
        tSymbolFlags Flags, LongInt DispAddr) {
    Boolean OK;
...
static Boolean SetRegPrefix(Byte* pDest, Byte Src, tStrComp const* pArg) {
    if (*pDest && (*pDest != Src)) {
...
static LongInt EvalArg(
        tStrComp const* pArg, int Offset, tSymbolSize OpSize, Boolean* pOK) {
    switch ((int)OpSize) {
    case eSymbolSize8Bit:
...
static Boolean ValidIndexRegSize(tSymbolSize Size) {
    return (Size
            == eSymbolSizeUnknown) /* no size given: regarded as default index reg size */
           || (Size == eSymbolSize16Bit) || (Size == eSymbolSize32Bit);
}
...
static Boolean ClassComp(tStrComp* pArg, tAdrComps* pComps) {
    ...
    if (*pArg->str.p_str == '@') {
        ...
        if (!InnerComps.InnerDispPresent && (InnerComps.InnerReg == NOREG)
            && (InnerComps.IndexReg == NOREG) && (InnerComps.BaseReg != NOREG)
            ...
            if ((pComps->IndexReg == NOREG) && ValidIndexRegSize(OpSize)) {
                ...
                if ((Scale == 0) && ValidBaseRegSize(OpSize)) {
                    pComps->BaseReg = Reg;
                        ...
                        (Scale != 0) && (pComps->IndexRegScale == 0)
                        && (pComps->IndexRegSize == eSymbolSize32Bit)) {
                    pComps->BaseReg       = pComps->IndexReg;
                    pComps->IndexReg      = Reg;
                    pComps->IndexRegScale = Scale;
                    pComps->IndexRegSize
                            = (OpSize == eSymbolSizeUnknown) ? DefIndexRegSize : OpSize;
                } else {
                    return False;
                }
            } else {
...
static Boolean DecodeAdr(
        tStrComp const* pArg, unsigned Mask, tAdrVals* pAdrVals, LongInt EAAddr) {
    Byte     Reg, Prefix;
    tStrComp Arg;
    ...
    while (True) {
        if (!as_strncasecmp(Arg.str.p_str, "<CRn>", 5)
            ...
            AppendAdrVals(
                    pAdrVals,
                    (toupper(Arg.str.p_str[1]) == 'C') ? PREFIX_CRn : PREFIX_PRn);
    ...
    if (*Arg.str.p_str == '@') {
        ...
        if (!Comps.InnerDispPresent && (Comps.InnerReg == NOREG)
            && (Comps.IndexReg == NOREG) && NormalReg(Comps.BaseReg)
            && !Comps.BaseRegIncr) {
        ...
        if (!Comps.InnerDispPresent && !Comps.OuterDispPresent
            && (Comps.InnerReg == NOREG) && (Comps.IndexReg == NOREG)
            && NormalReg(Comps.BaseReg) && Comps.BaseRegIncr) {
        ...
        if (!Comps.InnerDispPresent && Comps.OuterDispPresent && (Comps.InnerReg == NOREG)
            && (Comps.IndexReg == NOREG) && (Comps.BaseReg == NOREG)) {
            if (Comps.OuterDispSize == eSymbolSizeUnknown) {
        ...
        if (!Comps.InnerDispPresent && (Comps.InnerReg == NOREG)
            && (Comps.BaseReg == NOREG) && NormalReg(Comps.IndexReg)
        ...
        if (!Comps.InnerDispPresent && (Comps.InnerReg == NOREG)
            && NormalReg(Comps.BaseReg) && NormalReg(Comps.IndexReg)) {
        ...
        if (!Comps.InnerDispPresent && (Comps.InnerReg == NOREG)
            && (Comps.BaseReg == PCREG) && NormalReg(Comps.IndexReg)) {
            ...
            if (Comps.OuterDispPresent) {
                ...
                if (!ChkSizePCRelDisplacement(
                            &Comps.OuterDisp, Comps.OuterDispSize, EAAddr + 3)) {
                    goto Check;
                }
                Exp1Val |= (1 + Comps.OuterDispSize) << 4;
            }
        ...
        if (!Comps.InnerDispPresent && (Comps.InnerReg == NOREG)
            && (Comps.BaseReg == PCREG) && (Comps.IndexReg == NOREG)) {
            Byte Exp1Val = 0x80;
            ...
            if (Comps.OuterDispPresent) {
                ...
                if (!ChkSizePCRelDisplacement(
                            &Comps.OuterDisp, Comps.OuterDispSize, EAAddr + 2)) {
                    goto Check;
                }
                Exp1Val |= (1 + Comps.OuterDispSize) << 4;
            }
            AppendRegPrefix(
                    pAdrVals,
                    Comps.RegPrefix); /* should never occur, no Rn in expression */
            AppendAdrVals(pAdrVals, 0x7d);
            AppendAdrVals(pAdrVals, Exp1Val);
            if (Comps.OuterDispPresent) {
                AppendToAdrValsBySize(pAdrVals, Comps.OuterDisp, Comps.OuterDispSize);
        ...
        if (NormalReg(Comps.InnerReg) && (Comps.BaseReg == NOREG)
            ...
            AppendAdrVals(
                    pAdrVals, Comps.InnerReg | ((Comps.InnerDispSize + 1) << 4)
                                      | ((Comps.OuterDispSize + 1) << 6));
...
Check:
    if ((pAdrVals->Mode != eAdrModeNone) && !((Mask >> pAdrVals->Mode) & 1)) {
        WrStrErrorPos(ErrNum_InvAddrMode, pArg);
        ResetAdrVals(pAdrVals);
        return False;
    }
...
static Boolean GlobalBankReg(tAdrVals const* pAdrVals) {
    return (pAdrVals->Mode == eAdrModeReg) && (pAdrVals->Cnt == 1);
}

/*!------------------------------------------------------------------------
 * \fn     ImmValS8(const tAdrVals *pAdrVals)
 * \brief  check whether parsed address expression is an immediate value representable as
 8 bits, assuming sign extension
 * \param  pAdrVals parsed expression
 * \return True if immediate & value range OK
 * ------------------------------------------------------------------------ */

static Boolean ImmValS8(tAdrVals const* pAdrVals) {
    Byte     Sign;
    unsigned z;

    if ((pAdrVals->Mode != eAdrModeImm) || (pAdrVals->Cnt < 1)) {
        return False;
    }
 ...
 * \brief  check whether parsed address expression is an immediate value representable as
 4 bits, assuming sign extension
 * \param  pAdrVals parsed expression
 * \return True if immediate & value range OK
 * ------------------------------------------------------------------------ */

static Boolean ImmValS4(tAdrVals const* pAdrVals) {
    Byte Sign;
...
static void AppendAdrValsToCode(tAdrVals const* pAdrVals) {
...
static void AppendAdrValPair(tAdrVals* pSrcAdrVals, tAdrVals const* pDestAdrVals) {
...
static LongWord AssembleBitSymbol(Byte BitPos, tSymbolSize OpSize, LongWord Address) {
    ...
    return BitPos | ((Address & 0xfffffful) << AddrShift) | ((LongWord)OpSize << 30);
}
...
static void DissectBit_H16(char* pDest, size_t DestSize, LargeWord Inp) {
    ...
    as_snprintf(
            pDest, DestSize, "$%lx(%c).%u", (unsigned long)Address, Attribute,
            (unsigned)BitPos);
}
...
static void ExpandH16Bit(
    ...
    tSymbolSize OpSize
            = (pStructElem->OpSize < 0) ? eSymbolSize8Bit : pStructElem->OpSize;
    ...
    PushLocHandle(-1);
    EnterIntSymbol(
            pVarName, AssembleBitSymbol(pStructElem->BitPos, OpSize, Address), SegBData,
            False);
    PopLocHandle();
    /* TODO: MakeUseList? */
}
...
static LongInt ExtractDisp(Byte const* pVals, Byte SizeCode) {
    LongInt Result = 0;
...
static void DecodeMOV_ADD_SUB_CMP(Word Code) {
    ...
    tFormat  Format;
    unsigned DestMask;
    Boolean  IsSUB = (Code == 0x04), IsCMP = (Code == 0x08);
    ...
    if (ChkArgCnt(2, 2)
        ...
        && DecodeAdr(
                &ArgStr[2], DestMask, &DestAdrVals,
                EProgCounter() + 1 + SrcAdrVals.Cnt)) {
        ...
        if (!*FormatPart.str.p_str) {
            ...
            /* MOV R0,<ea> -> MOVF <ea> if PC-relative distance size does not change. */
                    ...
                    GlobalBankReg(&SrcAdrVals) && (SrcAdrVals.Vals[0] == 0x40)
                    && PCDistOK(&DestAdrVals, SrcAdrVals.Cnt)) {
                Format = eFormatF;
        ...
        } else if (!(Format = DecodeFormat(
                             (1 << eFormatG) | (1 << eFormatQ) | (1 << eFormatR)
                             | (1 << eFormatRQ)))) {
            WrStrErrorPos(ErrNum_InvFormat, &FormatPart);
            return;
        }
        ...
        case eFormatG:
            ...
            break;
        case eFormatR:
            if (!GlobalBankReg(&SrcAdrVals)) {
                WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
            } else if (!GlobalBankReg(&DestAdrVals)) {
                WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[2]);
            } else {
                BAsmCode[CodeLen++] = 0x20 | Code | OpSize;
                BAsmCode[CodeLen++]
                        = ((SrcAdrVals.Vals[0] & 15) << 4) | (DestAdrVals.Vals[0] & 15);
            }
            break;
        case eFormatRQ:
            if (SrcAdrVals.Mode != eAdrModeImm) {
                WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
            } else if (!ImmValS4(&SrcAdrVals)) {
                WrStrErrorPos(ErrNum_OverRange, &ArgStr[1]);
            } else if (!GlobalBankReg(&DestAdrVals)) {
                WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[2]);
            } else {
                BAsmCode[CodeLen++] = 0x30 | Code | OpSize;
                BAsmCode[CodeLen++] = ((DestAdrVals.Vals[0] & 15) << 4)
                                      | (SrcAdrVals.Vals[SrcAdrVals.Cnt - 1] & 15);
            }
            break;
        case eFormatQ:
            if (IsSUB) {
                WrStrErrorPos(ErrNum_InvAddrMode, &AttrPart);
            } else if (SrcAdrVals.Mode != eAdrModeImm) {
                WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
            } else if (!ImmValS8(&SrcAdrVals)) {
                WrStrErrorPos(ErrNum_OverRange, &ArgStr[1]);
            } else {
            ...
            break;
        case eFormatF:
            if (!GlobalBankReg(&SrcAdrVals) || (SrcAdrVals.Vals[0] != 0x40)) {
                WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
            } else {
...
static void DecodeSignExt(Word Code) {
    ...
    if (!ChkEmptyFormat() || !ChkOpSize(eSymbolSize16Bit, 7) || !ChkArgCnt(2, 2)
        || !DecodeAdr(&ArgStr[1], MModeAll, &SrcAdrVals, EProgCounter() + 1)) {
        return;
    }
    SrcOpSize = OpSize;
...
static void DecodeTwo(Word Code) {
    ...
    if (ChkEmptyFormat()
        ...
        && DecodeAdr(&ArgStr[1], MModeAll, &SrcAdrVals, EProgCounter() + 1)
        && DecodeAdr(
                &ArgStr[2], MModeAll - MModeImm, &DestAdrVals,
                EProgCounter() + 1 + SrcAdrVals.Cnt)) {
...
static void DecodeOne(Word Code) {
    ...
    if (ChkEmptyFormat()
        && ChkOpSize(Only8 ? eSymbolSize8Bit : eSymbolSize16Bit, Only8 ? 1 : 7)
        && ChkArgCnt(1, 1) && DecodeAdr(&ArgStr[1], Mask, &AdrVals, EProgCounter() + 1)) {
        BAsmCode[CodeLen++] = Code | OpSize;
...
static void DecodeMUL_DIV(Word Code) {
    ...
    if (!ChkEmptyFormat() || !ChkOpSize(eSymbolSize16Bit, 3) || !ChkArgCnt(2, 2)
...
static void DecodeEXT(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize16Bit, 7) && ChkArgCnt(1, 1)) {
...
static void DecodeCInstr(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkArgCnt(2, 2) && DecodeCReg(&ArgStr[CIdx], &CReg)
...
static void DecodeBranch(Word Code) {
    ...
    if (!ChkEmptyFormat() || !ChkArgCnt(1, 1)) {
        return;
    }

    Addr = EvalStrIntExpressionOffsWithFlags(
            &ArgStr[1], !!(*ArgStr[1].str.p_str == '@'), UInt24, &OK, &Flags);
    if (!OK) {
        return;
    }
...
static void DecodeFixed(Word Code) {
    if (ChkEmptyFormat() && (!(Code & 0x8000) || CheckSup(&OpPart)) && ChkArgCnt(0, 0)) {
...
static void DecodeSET(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize8Bit, 1) && ChkArgCnt(1, 1)
...
static void DecodeTRAP(Word Code) {
    if (ChkEmptyFormat() && ChkArgCnt(0, 0)) {
...
static void DecodeSCB(Word Code) {
    ...
    if (!ChkEmptyFormat() || !ChkArgCnt(2, 2)) {
        return;
    }
    ...
    Addr = EvalStrIntExpressionOffsWithFlags(
            &ArgStr[2], !!(*ArgStr[1].str.p_str == '@'), UInt24, &OK, &Flags);
    if (!OK) {
        return;
    }
...
static void DecodeJMP_JSR(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkArgCnt(1, 1)
        && DecodeAdr(
                &ArgStr[1], MModeAll - MModeReg - MModeImm, &Arg, EProgCounter() + 1)) {
        BAsmCode[CodeLen++] = Code;
...
static void DecodeSWAP(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize16Bit, 3) && ChkArgCnt(1, 1)
...
static void DecodeXCH(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize32Bit, 4) && ChkArgCnt(2, 2)) {
...
static void DecodeLINK(Word Code) {
    ...
    if (!DecodeReg(&ArgStr[1], &Reg, &Prefix, False, True))
        ...
        WrStrErrorPos(ErrNum_OnlyImmAddr, &ArgStr[2]);
    } else {
        Boolean OK;
        LongInt Val = EvalArg(
                &ArgStr[2], 1, (OpSize == eSymbolSizeUnknown) ? eSymbolSize32Bit : OpSize,
                &OK);
...
static void DecodeRTD(Word Code) {
    if (!ChkEmptyFormat() || !ChkArgCnt(1, 1)) {
        ...
        Boolean OK;
        LongInt Val = EvalArg(
                &ArgStr[1], 1, (OpSize == eSymbolSizeUnknown) ? eSymbolSize32Bit : OpSize,
                &OK);
...
static void DecodeLDM_STM(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize16Bit, 7) && ChkArgCnt(2, ArgCntMax)
        && DecodeRegList(&RegList, 1 + IsLDM, ArgCnt - 1 + IsLDM)
        && DecodeAdr(
                &ArgStr[IsLDM ? 1 : ArgCnt], MModeAll - MModeImm - MModeReg, &AdrVals,
                EProgCounter() + 1)) {
        BAsmCode[CodeLen++] = Code | OpSize;
        AppendAdrValsToCode(&AdrVals);
...
static void DecodeCGBN(Word Code) {
    ...
    if (ChkEmptyFormat() && CheckSup(&OpPart) && ChkOpSize(eSymbolSize8Bit, 1)
        ...
        BAsmCode[CodeLen++] = (AdrVals.Mode == eAdrModeImm) ? AdrVals.Vals[1]
                                                            : (AdrVals.Vals[0] & 0x0f);
        if (RegList) {
            BAsmCode[CodeLen++] = Hi(RegList);
            BAsmCode[CodeLen++] = Lo(RegList);
...
static void DecodeMOVFP(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize16Bit, 6) && ChkArgCnt(2, 2)
        && DecodeAdr(
                &ArgStr[1], MModeAll - MModeReg - MModePre - MModePost - MModeImm,
                &SrcAdrVals, EProgCounter() + 1)
        && DecodeAdr(
                &ArgStr[2], MModeAll - MModeImm, &DestAdrVals,
                EProgCounter() + 1 + SrcAdrVals.Cnt)) {
...
static void DecodeMOVTP(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize16Bit, 6) && ChkArgCnt(2, 2)
        && DecodeAdr(&ArgStr[1], MModeAll, &SrcAdrVals, EProgCounter() + 1)
        && DecodeAdr(
                &ArgStr[2], MModeAll - MModeReg - MModePre - MModePost - MModeImm,
                &DestAdrVals, EProgCounter() + 1 + SrcAdrVals.Cnt)) {
        BAsmCode[CodeLen++] = Code | (OpSize - 1);
        AppendAdrValPair(&SrcAdrVals, &DestAdrVals);
    }
}
...
static void DecodeMOVA(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize32Bit, 4) && ChkArgCnt(2, 2)
        && DecodeAdr(
                &ArgStr[1], MModeAll - MModeReg - MModeImm, &SrcAdrVals,
                EProgCounter() + 1)
        && DecodeAdr(
                &ArgStr[2], MModeAll - MModeImm, &DestAdrVals,
                EProgCounter() + 1 + SrcAdrVals.Cnt)) {
...
static void DecodeString(Word Code) {
    ...
    if (ChkEmptyFormat() && ChkOpSize(eSymbolSize16Bit, 3)
...
static void DecodeBField(Word Code) {
    ...
    if (OpSize != eSymbolSizeUnknown) {
        WrError(ErrNum_InvOpSize);
            ...
            ChkEmptyFormat() && ChkArgCnt(4, 4)
            && DecodeStringReg(&ArgStr[1], MModeReg, &PosReg)
            && DecodeStringReg(&ArgStr[2], MModeReg, &CntReg)
            && DecodeAdr(
                    &ArgStr[3], MModeAll - MModePre - MModePost - MModeImm, &SrcAdrVals,
                    EProgCounter() + 2)
            && DecodeAdr(
                    &ArgStr[4], MModeAll - MModePre - MModePost - MModeImm, &DestAdrVals,
                    EProgCounter() + 2 + SrcAdrVals.Cnt)) {
        BAsmCode[CodeLen++] = Code;
...
static void DecodeBit(Word Code) {
    ...
    if (!ChkEmptyFormat() || !ChkArgCnt(1, 2)) {
        return;
    }
    ...
    if (1 == ArgCnt) {
        ...
        if (DecodeBitArg(&BitArg, 1, 1, OpSize)
            ...
            /* SetOpSize() has set OpSize to 0..2 or confirmed it, no need to check for
             * invalid size */
    ...
    /* bit + <ea> */
        ...
        if (ChkOpSize(eSymbolSize16Bit, 7)
            ...
            if (DecodeAdr(
                        &ArgStr[1], MModeImm | MModeReg, &BitAdrVals,
                        EProgCounter() + 1)) {
...
static void DecodeBIT(Word Code) {
    ...
    if (ActPC == StructSeg) {
        ...
        BitPos = EvalBitPosition(
                &ArgStr[1], &OK,
                (OpSize == eSymbolSizeUnknown) ? eSymbolSize32Bit : OpSize);
        if (!OK) {
            return;
        }
...
static void SwitchTo_H16(void) {
    PFamilyDescr const pDescr = FindFamilyByName("H16");
