...
static void DecodeFixed(Word Index) {
    ...
    if (ChkArgCnt(0, 0) && (ChkExactCPUMask(pOrder->CPUMask, CPUCOP410) >= 0)) {
        if (Hi(pOrder->Code)) {
...
static void DecodeImm(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(pOrder->CPUMask, CPUCOP410) >= 0)) {
        Byte    Val;
        Boolean OK;
...
static void DecodeXAD(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (OK) {
            ...
            if (OK) {
                if ((MomCPU < CPUCOP420) && ((Reg1 != 3) || (Reg2 != 15))) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (OK) {
            ...
            if (OK) {
                if ((Val > 0) && (Val < 9)) {
                    if (ChkExactCPUMask(
                                M_CPUCOP420 | M_CPUCOP440 | M_CPUCOP440, CPUCOP410)
                        >= 0) {
...
static void DecodeJSRP(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (OK) {
            if (((Addr >> 6) != 2) || (Addr == 0xbf)) {
                WrError(ErrNum_NotFromThisAddress);
            } else if ((EProgCounter() >> 7) == 1) {
...
static void AddFixed(char const* NName, Word NCode, Word NMask) {
...
static void AddImm(char const* NName, Word NCode, Word NMask) {
...
static void SwitchTo_COP4(void) {
    ...
    MakeCode   = MakeCode_COP4;
    IsDef      = IsDef_COP4;
    SwitchFrom = SwitchFrom_COP4;
    InitFields();
}
