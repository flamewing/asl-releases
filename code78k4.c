...
static Word const     PSWLAddr = 0xfffe, PSWHAddr = 0xffff;
static ShortInt const WHLReg = 3;
static LongWord const Bit27  = ((LongWord)1) << 27;

static tAdrModeMask const MModImm = (1UL << ModImm), MModReg8 = (1UL << ModReg8),
                          MModReg8_U16 = (1UL << ModReg8_U16),
                          MModReg16 = (1UL << ModReg16), MModReg24 = (1UL << ModReg24),
                          MModMem = (1UL << ModMem), MModShort1 = (1UL << ModShort1),
                          MModShort2 = (1UL << ModShort2), MModSFR = (1UL << ModSFR),
                          MModAbs16 = (1UL << ModAbs16), MModAbs20 = (1UL << ModAbs20),
                          ...
                          MModShortIndir_All
                          = ((1UL << ModShortIndir1_16) | (1UL << ModShortIndir2_16)
                             | (1UL << ModShortIndir1_24) | (1UL << ModShortIndir2_24)),
                          MModSP = (1UL << ModSP), MModSTBC = (1UL << ModSTBC),
                          MModWDM = (1UL << ModWDM);
...
static Boolean SetOpSize(ShortInt NewSize) {
    if (OpSize < 0) {
        ...
        as_snprintf(
                Sizes, sizeof(Sizes), "%d<->%d", (int)((OpSize + 1) * 8),
                (int)((NewSize + 1) * 8));
...
static ShortInt DecodeReg8(char const* pAsc) {
    switch (strlen(pAsc)) {
    case 1: {
        static char const Reg8Names[9] = "XACBEDLH";
        char const*       pPos         = strchr(Reg8Names, as_toupper(*pAsc));
    ...
    case 3: {
        static char const Reg8Names[][4] = {"VPL", "VPH", "UPL", "UPH", "R10", "R11",
                                            "R12", "R13", "R14", "R15", ""};
        int               z;
...
static ShortInt DecodeReg8_U16(char const* pAsc) {
    static char const Reg8Names[5] = "VUTW";
    char const*       pPos;
...
static ShortInt DecodeReg16(char const* pAsc) {
    ShortInt Result = -1;

    switch (strlen(pAsc)) {
    case 2: {
        static char const Reg16Names[][3] = {"AX", "BC", "VP", "UP", "DE", "HL", ""};
        int               z;
    ...
    case 3: {
        if ((toupper(pAsc[0]) == 'R') && (toupper(pAsc[1]) == 'P') && (pAsc[2] >= '0')
            && (pAsc[2] <= '7')) {
            Result = pAsc[2] - '0';
        }
...
static ShortInt DecodeReg24(char const* pAsc) {
    static char const Reg24Names[][4]
            = {"VVP", "UUP", "TDE", "WHL", "RG4", "RG5", "RG6", "RG7", ""};
    int z;
...
static Boolean DecodeRB(char const* pAsc, Byte* pErg) {
    if ((strlen(pAsc) != 3) || (toupper(*pAsc) != 'R') || (toupper(pAsc[1]) != 'B')
        || (pAsc[2] < '0') || (pAsc[2] > '7')) {
        return False;
    } else {
        *pErg = pAsc[2] - '0';
...
static Boolean DecodeAdr(
        tStrComp const* pArg, tAdrModeMask AdrModeMask, tEncodedAddress* pAddress) {
    int      z, ArgLen;
    LongWord Addr;
    ShortInt AddrSize;
    ...
    if (*pArg->str.p_str == '#') {
        ...
        if ((OpSize >= 0) && (OpSize < 3)) {
            static IntType const IntTypes[3] = {Int8, Int16, Int24};
            Boolean              OK;
    ...
    if ((ArgLen >= 2) && (pArg->str.p_str[ArgLen - 1] == ']')) {
        ...
        if (pStart == Arg.str.p_str) {
            ...
            if (!as_strcasecmp(Base.str.p_str, "SP")) {
                ...
                switch (tmp) {
                case -1:
                    pAddress->AdrVals[0] = 0xff;
                    break; /* no register */
                case 0:
                    pAddress->AdrVals[0] = 4;
                    break; /* VVP */
                case 1:
            ...
            *pSep = Save;
            ...
            if (*pSep == '+') {
                ...
                if (tmp == -1)
                    ; /* no reg at all, go on with 16-bit reg below */
            ...
            /* it's a number: put a fake 0 in front so displacement expression evaluates
             * correctly! */
            ...
            *pSep = '0';
            pAddress->AdrVals[1]
                    = EvalStrIntExpressionOffs(&Arg, pSep - Arg.str.p_str, Int8, &OK);
            if (OK) {
                pAddress->AdrMode = ModMem;
    ...
    Addr = EvalStrIntExpressionOffsWithFlags(
            pArg, Offset, (AdrModeMask & MModAbs20) ? UInt20 : UInt24, &OK,
            &pAddress->AdrValSymFlags);
    if (!OK) {
        return False;
    }
    ...
    if (!AddrSize) {
        if (!IsShort1 && !IsShort2 && !IsSFR && !Is16
            && (AdrModeMask & (MModAbs24 | MModAbs20))) {
            AddrSize = 2;
        } else if (
                !IsShort1 && !IsShort2 && !IsSFR && Is16
                && (!(AdrModeMask & MModAbs16))) {
            AddrSize = 2;
...
static Boolean DecodeMem3(char const* pAsc, Byte* pResult) {
    int      l = strlen(pAsc);
    char     Reg[10];
    ShortInt BinReg;
...
static void AppendAdrVals(tEncodedAddress const* pAddress) {
...
static void AppendAdrValsMem(tEncodedAddress const* pAddress, Byte LowNibble) {
    AppendAdrVals(pAddress);
    BAsmCode[CodeLen - pAddress->AdrCnt]
            = (BAsmCode[CodeLen - pAddress->AdrCnt] << 4) | LowNibble;
}
...
static LongWord GetAbsVal(tEncodedAddress const* pAddress) {
    LongWord Result = 0;
...
static Boolean DecodeBitAdr(tStrComp const* pArg, LongWord* pResult) {
    char*   pSplit;
    Boolean OK;
    ...
    if (pSplit) {
        ...
        *pResult = EvalStrIntExpression(&BitArg, UInt3, &OK) << 24;
        if (OK) {
            DecodeAdr(
                    &AddrArg,
                    MModReg8 | MModSFR | MModShort1 | MModShort2 | MModMem | MModAbsAll,
                    &Addr);
            switch (Addr.AdrMode) {
            case ModReg8:
            ...
            case ModSFR:
                if (Addr.AdrVals[0] == (PSWLAddr & 0xff)) {
                    ...
                    *pResult |= Addr.AdrVals[0] | (((LongWord)eBitTypeSAddr2_SFR) << 28)
                                | Bit27;
                }
                break;
            case ModShort1:
                *pResult |= Addr.AdrVals[0] | (((LongWord)eBitTypeSAddr1) << 28);
                break;
            case ModShort2:
                *pResult |= Addr.AdrVals[0] | (((LongWord)eBitTypeSAddr2_SFR) << 28);
                break;
...
static void AppendRel8(tStrComp const* pArg) {
    Boolean      OK;
    LongInt      Dist;
    tSymbolFlags Flags;

    Dist = EvalStrIntExpressionOffsWithFlags(
                   pArg, !!(*pArg->str.p_str == '$'), UInt20, &OK, &Flags)
...
static void DecodeBitOpCore(tStrComp const* pBitArg, Word Code) {
    LongWord BitAddr;
...
static void DecodeMOV(Word ForceOpSize) {
    ...
    AdrModeMask = MModShort1 | MModShort2 | MModAbsAll | MModShortIndir1_24
                  | MModShortIndir2_24 | MModMem;
    if (OpSize != 2) {
    ...
    case ModReg16:
        AdrModeMask
                = MModImm | MModReg16 | MModShort1 | MModShort2 | MModSFR | MModAbsAll;
    ...
    case ModReg24:
        ...
        case ModMem:
            if ((Src.AdrVal == 0x16)
                && ((Src.AdrVals[0] == 1) || (Src.AdrVals[0] == 3))) {
                WrError(ErrNum_InvAddrMode);
            } else {
                BAsmCode[CodeLen++] = Src.AdrVal;
    ...
    case ModShort1:
        ...
        AdrModeMask = 0;
        if (OpSize != 2) {
            AdrModeMask |= MModImm | MModShort2 | MModShort1;
        }
        if ((OpSize == -1) || (OpSize == 0)) {
...
static void DecodeXCH(Word ForceOpSize) {
    ...
    AdrModeMask = MModShort1 | MModShort2 | MModSFR | MModAbsAll | MModShortIndir_All
                  | MModMem;
    if ((OpSize == -1) || (OpSize == 0)) {
        AdrModeMask |= MModReg8;
    }
    ...
    case ModShort2:
        ...
        case ModShort1:
            ...
            BAsmCode[CodeLen++] = 0x2a;
            BAsmCode[CodeLen++]
                    = (OpSize << 7) | ((Src.AdrMode == ModShort1) ? 0x14 : 0x04);
            BAsmCode[CodeLen++] = Src.AdrVals[0];
            BAsmCode[CodeLen++] = Dest.AdrVals[0];
            break;
        case ModNone:
    ...
    case ModShort1:
        ...
        case ModShort1:
            ...
            BAsmCode[CodeLen++] = 0x2a;
            BAsmCode[CodeLen++]
                    = (OpSize << 7) | ((Src.AdrMode == ModShort1) ? 0x34 : 0x24);
            BAsmCode[CodeLen++] = Src.AdrVals[0];
            BAsmCode[CodeLen++] = Dest.AdrVals[0];
            break;
        case ModNone:
...
static void DecodeDIVUW(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeAdr(&ArgStr[1], MModReg8, &Src)) {
        if (Src.AdrVal >= 8) {
            BAsmCode[CodeLen++] = 0x3c;
        }
        BAsmCode[CodeLen++] = 0x05;
...
static void DecodeMULW_DIVUX(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeAdr(&ArgStr[1], MModReg16, &Src)) {
        BAsmCode[CodeLen++] = 0x05;
...
static void DecodeSACW(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1], MModMem, &Addr)) {
        if ((Addr.AdrVal != 0x16) || (Addr.AdrVals[0] != 0x00)) {
            WrError(ErrNum_InvAddrMode);
        } else if (DecodeAdr(&ArgStr[2], MModMem, &Addr)) {
            if ((Addr.AdrVal != 0x16) || (Addr.AdrVals[0] != 0x01)) {
                WrError(ErrNum_InvAddrMode);
            } else {
                BAsmCode[CodeLen++] = 0x09;
...
static void DecodeINCDEC(Word Code) {
    ...
    AdrModeMask = 0;
    if ((OpSize == -1) || (OpSize == 0)) {
        AdrModeMask |= MModReg8 | MModShort1 | MModShort2;
    }
    if ((OpSize == -1) || (OpSize == 1)) {
        AdrModeMask |= MModReg16 | MModShort1 | MModShort2;
    }
    if ((OpSize == -1) || (OpSize == 2)) {
        AdrModeMask |= MModReg24 | MModSP;
    }
...
static void DecodeShift(Word Code) {
    ...
    if ((Code & 0x8000) && ((OpSize == -1) || (OpSize == 1))) {
        AdrModeMask |= MModReg16;
    }
    DecodeAdr(&ArgStr[1], AdrModeMask, &Addr);
    switch (Addr.AdrMode) {
...
static void DecodeROR4_ROL4(Word Code) {
    if (!ChkArgCnt(1, 1))
        ;
    else if (!DecodeMem3(ArgStr[1].str.p_str, BAsmCode + 1)) {
        WrError(ErrNum_InvAddrMode);
    } else {
        BAsmCode[CodeLen++] = 0x05;
        BAsmCode[CodeLen] |= Code;
...
static void DecodePUSH_POP(Word Code) {
    ...
    Code = (Code >> 4) & 0xff;
    IsU = (Code == 0x37) || (Code == 0x36), IsPUSH = (Code == 0x35);
    ...
    for (z = 1; z <= ArgCnt; z++) {
        ...
        if (SumAddress.AdrMode == ModNone) {
            ...
            if (Address.AdrMode == ModReg16) {
                ...
                (Address.AdrMode != SumAddress.AdrMode)
                || (SumAddress.AdrMode != ModReg16)) {
            WrError(ErrNum_InvAddrMode);
            return;
        } else {
            SumAddress.AdrVals[0] |= 1 << Address.AdrVal;
...
static void DecodeCALL_BR(Word IsCALL) {
    ...
    if (ChkArgCnt(1, 1) && StripIndirect(&ArgStr[1])) {
        ...
        case ModAbs20: {
            ...
            if (!Addr.ForceRel) {
                ...
                if (!mFirstPassUnknown(Addr.AdrValSymFlags) && !Dist16OK) {
                    WrError(ErrNum_JmpDistTooBig);
                } else {
                ...
                if (!mFirstPassUnknown(Addr.AdrValSymFlags) && !Dist8OK) {
                    WrError(ErrNum_JmpDistTooBig);
                } else {
...
static void DecodeCALLF(Word Code) {
    ...
    if (ArgCnt != 1) {
        ...
        Addr = EvalStrIntExpressionOffsWithFlags(
                &ArgStr[1], !!(*ArgStr[1].str.p_str == '!'), UInt12, &OK, &Flags);
        if (mFirstPassUnknown(Flags)) {
            Addr |= 0x800;
        }
...
static void DecodeCALLT(Word Code) {
    ...
    if (ArgCnt != 1) {
        WrError(ErrNum_InvAddrMode);
    } else if (!StripIndirect(&ArgStr[1])) {
        WrError(ErrNum_InvAddrMode);
    } else {
        Boolean      OK;
        Word         Addr;
        ...
        Addr = EvalStrIntExpressionOffsWithFlags(
                &ArgStr[1], !!(*ArgStr[1].str.p_str == '!'), UInt7, &OK, &Flags);
        if (mFirstPassUnknown(Flags)) {
            Addr = (Addr | 0x40) & 0xfe;
        }
        if (OK && (Addr & 1)) {
...
static void DecodeBRKCS(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if (!DecodeRB(ArgStr[1].str.p_str, BAsmCode + 1)) {
...
static void DecodeRETCS(Word Code) {
    if (ArgCnt != 1) {
        ...
        Addr = EvalStrIntExpressionOffs(
                &ArgStr[1], !!(*ArgStr[1].str.p_str == '!'), UInt16, &OK);
        if (OK) {
            PutCode(Code);
            BAsmCode[CodeLen++] = Lo(Addr);
...
static void DecodeSEL(Word Code) {
    ...
    if (!DecodeRB(ArgStr[1].str.p_str, &Bank)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else {
        BAsmCode[CodeLen++] = 0x05;
...
static void DecodeCHK(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeAdr(&ArgStr[1], MModSFR, &Address)) {
        PutCode(Code);
...
static void DecodeMOVTBLW(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        Addr = EvalStrIntExpressionOffs(
                &ArgStr[1], !!(*ArgStr[1].str.p_str == '!'), UInt8, &OK);
        if (OK) {
...
static void DecodeStringAcc(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[2], MModReg8, &Addr)) {
        if (Addr.AdrVal != AccReg8()) {
            WrError(ErrNum_InvAddrMode);
        } else if (DecodeAdr(&ArgStr[1], MModMem, &Addr)) {
            if (Addr.AdrVal != 0x16) {
                WrError(ErrNum_InvAddrMode);
            } else if ((Addr.AdrVals[0] & 0xfd) != 0) {
                WrError(ErrNum_InvAddrMode);
            } else {
...
static void AddInstr(char const* pInstr, InstProc Proc, Word Code, unsigned SizeMask) {
    char Instr[20];
...
static void InternSymbol_78K4(char* pAsc, TempResult* pErg) {
    if ((!as_strcasecmp(pAsc, "PSWL")) || (!as_strcasecmp(pAsc, "PSW"))) {
