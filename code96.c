...
static void ChkSFR(Word Adr, tStrComp const* pArg) {
    if ((Adr >= SFRStart) && (Adr <= SFRStop)) {
        WrStrErrorPos(ErrNum_IOAddrNotAllowed, pArg);
    }
}

static void Chk296(Word Adr, tStrComp const* pArg) {
...
static void ChkAlign(Byte Adr, tStrComp const* pArg) {
    if (((OpSize == 0) && (Adr & 1)) || ((OpSize == 1) && (Adr & 3))) {
        WrStrErrorPos(ErrNum_AddrNotAligned, pArg);
    }
}

static void ChkAdr(Byte Mask, tStrComp const* pArg) {
...
static void DecodeAdr(tStrComp const* pArg, Byte Mask, Boolean AddrWide) {
    LongInt      AdrInt;
    LongWord     AdrWord;
    Word         BReg;
    Boolean      OK;
    ...
    if (p) {
        ...
        if (!p2 || (p2 > Mid.str.p_str + ArgLen - 1)
            || (p2 < Mid.str.p_str + ArgLen - 2)) {
            WrStrErrorPos(ErrNum_InvAddrMode, pArg);
        } else {
            ...
            if (OK) {
                if (!ChkWork(&BReg)) {
                    WrStrErrorPos(ErrNum_OverRange, &Mid);
                } else {
                    Reg = Lo(BReg);
                    ChkSFR(Reg, &Mid);
                    if (Reg & 1) {
                        WrStrErrorPos(ErrNum_AddrMustBeEven, &Mid);
                    } else if (
                            (strlen(Left.str.p_str) == 0)
                            && !strcmp(Right.str.p_str, "+")) {
                        AdrType    = ModPost;
                        AdrMode    = 2;
                        AdrCnt     = 1;
                        AdrVals[0] = Reg + 1;
                    } else if (strlen(Right.str.p_str) != 0) {
                        WrStrErrorPos(ErrNum_InvAddrMode, pArg);
                        ...
                        AdrInt = EvalStrIntExpressionOffsWithFlags(
                                &Left, Offset, AddrWide ? Int24 : Int16, &OK, &Flags);
                        if (OK) {
                            if ((AdrInt == 0) && (Mask & MModInd) && !ForceSize) {
                                ...
                                if (ForceSize == eForceShort) {
                                    if ((AdrInt > 127) && !mSymbolQuestionable(Flags)) {
                                        WrStrErrorPos(ErrNum_OverRange, &Left);
                                    } else if (
                                            (AdrInt < -128)
        ...
        if (OK) {
            if (AdrWord & OMask) {
                ...
                if ((!(BReg & 0xffff0000)) && ChkWork(&BReg) && !ForceSize) {
                    ...
                    if (ForceSize == eForceShort) {
                        if (!IsShort) {
                            WrStrErrorPos(ErrNum_UnderRange, pArg);
                        } else {
                            AdrType    = ModIdx;
                            AdrMode    = 3;
                            AdrCnt     = 2;
...
static void DecodeALU3(Word Code) {
    ...
    if (ChkArgCnt(2, 3)) {
        ...
        BAsmCode[Start++]
                = 0x40 + (Ord(ArgCnt == 2) << 5) + ((1 - OpSize) << 4) + (Lo(Code) << 2);
...
static void DecodeCMPL(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU80196)) {
        OpSize = 2;
        DecodeAdr(&ArgStr[1], MModDir, False);
        if (AdrType != ModNone) {
...
static void DecodeIDLPD(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU80196)) {
        OpSize = 0;
...
static void DecodeShift(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (AdrType != ModNone) {
            BAsmCode[0]
                    = 0x08 + Lo(Code) + (Ord(OpSize == 0) << 4) + (Ord(OpSize == 2) << 2);
            BAsmCode[2] = AdrVals[0];
            OpSize      = 0;
            DecodeAdr(&ArgStr[2], MModDir | MModImm, False);
            if (AdrType != ModNone) {
                if ((AdrType == ModImm) && (AdrVals[0] > 15)) {
                    WrStrErrorPos(ErrNum_OverRange, &ArgStr[2]);
                } else if ((AdrType == ModDir) && (AdrVals[0] < 16)) {
...
static void DecodeELD_EST(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU80196N)) {
...
static void DecodeMac(Word Code) {
    if (ChkArgCnt(1, 2) && ChkMinCPU(CPU80296)) {
        OpSize      = 1;
...
static void DecodeMVAC_MSAC(Word Code) {
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU80296)) {
        OpSize = 2;
        DecodeAdr(&ArgStr[1], MModDir, False);
        if (AdrType == ModDir) {
            BAsmCode[0] = 0x0d;
...
static void DecodeRpt(Word Code) {
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU80296)) {
        OpSize = 1;
...
static void DecodeRel(Word Code) {
    if (ChkArgCnt(1, 1)) {
        Boolean      OK;
        tSymbolFlags Flags;
        LongInt AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[1], MemInt, &OK, &Flags)
                         - (EProgCounter() + 2);
...
static void DecodeTIJMP(Word Code) {
    ...
    if (ChkArgCnt(3, 3) && ChkMinCPU(CPU80196)) {
        OpSize = 1;
        DecodeAdr(&ArgStr[1], MModDir, False);
        if (AdrType != ModNone) {
...
static void DecodeDJNZ_DJNZW(Word Size) {
    if (ChkArgCnt(2, 2) && (!Size || ChkMinCPU(CPU80196))) {
        OpSize = Size;
        DecodeAdr(&ArgStr[1], MModDir, False);
        if (AdrType != ModNone) {
            Boolean      OK;
    ...
    if (ChkArgCnt(3, 3)) {
        ...
        if (OK) {
            ...
            if (AdrType != ModNone) {
                ...
                BAsmCode[1] = AdrVals[0];
                AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[3], MemInt, &OK, &Flags)
                         - (EProgCounter() + 3);
                if (OK) {
                    if (!mSymbolQuestionable(Flags) && !IsByteBranch(AdrInt)) {
...
static void DecodeECALL(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU80196N)) {
        Boolean OK;
        LongInt AdrInt
                = EvalStrIntExpression(&ArgStr[1], MemInt, &OK) - (EProgCounter() + 4);
        if (OK) {
            BAsmCode[0] = 0xf1;
            BAsmCode[1] = AdrInt & 0xff;
...
static void AddSize(char const* NName, Word NCode, InstProc Proc, Word SizeMask) {
    int  l;
    char SizeName[20];
...
static void AddFixed(char const* NName, Byte NCode, CPUVar NMin, CPUVar NMax) {
    if ((MomCPU >= NMin) && (MomCPU <= NMax)) {
        AddInstTable(InstTable, NName, NCode, DecodeFixed);
    }
}

static void AddALU3(char const* NName, Word NCode) {
...
static void AddMac(char const* NName, Word NCode, Boolean NRel) {
    AddInstTable(InstTable, NName, NCode | (NRel ? 0x100 : 0), DecodeMac);
}

static void AddRpt(char const* NName, Byte NCode) {
...
static void SwitchTo_96(void) {
    ...
    if (MomCPU >= CPU80196) {
        ...
        ASSUMERecCnt
                = (MomCPU >= CPU80296) ? (sizeof(ASSUME96s) / sizeof(*ASSUME96s)) : 1;
    }
