...
static Boolean DecodeTable(char const* pArg, Word* pResult, tCodeTable const* pTable) {
...
static tCodeTable const RegCodes[] = {
        { "R0", 0},
        { "R1", 1},
        { "R2", 2},
        { "R3", 3},
...
static Boolean DecodeReg(tStrComp const* pArg, Word* pResult, Byte Mask) {
...
static tCodeTable const DRegCodes[] = {
        {"DR0", 0},
        { NULL, 0}
};
#define DecodeDRegCore(pArg, pResult) (DecodeTable((pArg), (pResult), DRegCodes))

static Boolean DecodeDReg(tStrComp const* pArg) {
    Word    DummyReg;
...
static tCodeTable const SkipCodes[] = {
        {   "",  0},
        {"SKP",  1},
        {  "M",  2},
        { "PZ",  3},
...
static Boolean DecodeSkip(tStrComp const* pArg, Word* pResult) {
...
static tCodeTable const AddrRegCodes[] = {
        {"(X0)", eAddrRegX0},
        {"(X1)", eAddrRegX1},
        {"(IC)", eAddrRegIC},
        {  NULL,          0}
...
static tCodeTable const EECodes[] = {
        {  "", 0},
        {"RE", 1},
        {"SE", 2},
        {"CE", 3},
...
static Boolean DecodeEE(tStrComp const* pArg, Word* pResult) {
...
static tCodeTable const AllBRCodes[] = {
        {"CSBR", 0},
        {"SSBR", 1},
        {"TSR0", 2},
        {"TSR1", 3},
...
#define DecodeAllBRCore(pArg, pResult, IsWrite) \
    (DecodeTable((pArg), (pResult), AllBRCodes + IsWrite))

static tCodeTable const BRCodes[] = {
        {"CSBR", 0},
        {"SSBR", 1},
        {"TSR0", 2},
        {"TSR1", 3},
...
static Boolean DecodeBR(tStrComp const* pArg, Word* pResult) {
...
static tCodeTable const SRegCodes[] = {
        {"SBRB", 0},
        { "ICB", 1},
        { "NPP", 2},
        {  NULL, 0}
...
static Boolean DecodeSOrAllBReg(
        tStrComp const* pArg, Word* pResult, unsigned IsS, Boolean IsWrite) {
    Boolean Result = IsS ? DecodeSRegCore(pArg->str.p_str, pResult)
                         : DecodeAllBRCore(pArg->str.p_str, pResult, IsWrite);
...
static Boolean DecodeHReg(tStrComp const* pArg, Word* pResult, Boolean IsWrite) {
    Boolean Result;
...
static Word ChkPage(LongWord Addr, Word Base, tStrComp const* pArg) {
...
static Boolean DecodeMem(tStrComp* pArg, Word* pResult) {
    ...
    } else if (TotIndirect && DecodeRegCore(Arg.str.p_str, &R)) /* plain (ic) (x0) (x1)
    ...
    plaindisp:
        ...
        Addr = EvalStrIntExpressionOffsWithFlags(
                &Arg, ArgOffset, (MomCPU == CPUMN1613) ? UInt18 : UInt16, &OK, &Flags);
        if (!OK) {
            return False;
        }
...
static void DecodeFixed(Word Index) {
    tFixedOrder const* pOrder = FixedOrders + Index;
...
static void DecodeRegImm(Word Code) {
        ...
        && DecodeReg(&ArgStr[1], &Reg, 0x7f)) /* do not allow IC as register */
    {
        ...
        WAsmCode[0]
                = EvalStrIntExpressionWithResult(&ArgStr[2], Int8, &EvalResult) & 0xff;
        if (EvalResult.OK) {
            if (Code & 0x1000) {
...
static void DecodeEAReg(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeReg(&ArgStr[1], &R, 0x3f)
...
static void DecodeEA(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeMem(&ArgStr[1], &EA)) {
        WAsmCode[0] = Code | EA;
        CodeLen     = 1;
    }
}
...
static void DecodeImm4(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && DecodeReg(&ArgStr[1], &R, 0x7f)
        && ((ArgCnt < 3) || DecodeSkip(&ArgStr[3], &Skip))) {
        Word    Num;
        Boolean OK;
...
static void DecodeLD_STD(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && ChkMinCPU(CPUMN1613)
...
static void DecodeR0RISkip(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && ChkMinCPU(CPUMN1613) && DecodeReg(&ArgStr[1], &R, 0x01)
...
static void DecodeDR0RISkip(Word Code) {
    ...
    Code &= ~0x10;
    if (ChkArgCnt(2, 3 + OptCarry) && ChkMinCPU(CPUMN1613) && DecodeDReg(&ArgStr[1])
        && DecodeIReg(&ArgStr[2], &Ri, 0xc0, 0x40)
...
static void DecodeDAA_DAS(Word Code) {
    ...
    if (ChkArgCnt(2, 4) && ChkMinCPU(CPUMN1613) && DecodeReg(&ArgStr[1], &R, 0x01)
        && DecodeIReg(&ArgStr[2], &Ri, 0xc0, 0x40) && ChkCarry(3, &HasCarryArg, &CarryVal)
        && ((ArgCnt < 3 + HasCarryArg) || DecodeSkip(&ArgStr[3 + HasCarryArg], &Skip))) {
        WAsmCode[0] = Code | (Skip << 4) | (Ri & 3) | (CarryVal << 3);
        CodeLen     = 1;
    }
...
static void DecodeNEG(Word Code) {
    ...
    if (ChkArgCnt(1, 3) && ChkMinCPU(CPUMN1613)
        && DecodeReg(&ArgStr[1], &R, 0x7f) /* do not allow IC as register */
...
static void DecodeRDR_WTR(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPUMN1613)
...
static void DecodeBD_BALD(Word Code) {
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPUMN1613)) {
        Boolean  OK;
...
static void DecodeBL_BALL(Word Code) {
    if (!ChkArgCnt(1, 1))
        ...
        ;
    else if (!IsIndirect(ArgStr[1].str.p_str)) {
...
static void DecodeFIX(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && ChkMinCPU(CPUMN1613) && DecodeReg(&ArgStr[1], &R, 0x01)
        && DecodeDReg(&ArgStr[2]) && ((ArgCnt < 3) || DecodeSkip(&ArgStr[3], &Skip))) {
...
static void DecodeFLT(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && ChkMinCPU(CPUMN1613) && DecodeDReg(&ArgStr[1])
...
static void DecodeSRBT(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPUMN1613) && DecodeReg(&ArgStr[1], &R, 0x01)
...
static void DecodeDEBP(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPUMN1613) && DecodeReg(&ArgStr[2], &R, 0x01)
...
static void DecodeBLK(Word Code) {
    ...
    if (ChkArgCnt(3, 3) && ChkMinCPU(CPUMN1613) && DecodeIReg(&ArgStr[1], &Ri, 0xff, 0x41)
        && DecodeIReg(&ArgStr[2], &Ri, 0xff, 0x40) && DecodeReg(&ArgStr[3], &Ri, 0x01)) {
        WAsmCode[0] = Code;
        CodeLen     = 1;
    }
}
...
static void DecodeCPYH_SETH(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPUMN1613) && DecodeReg(&ArgStr[1], &R, 0xff)
...
static void DecodeCLR(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeReg(&ArgStr[1], &R, 0x7f)) /* do not allow IC */
    {
        /* == EOR R,R */
...
static void DecodeSKIP(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeReg(&ArgStr[1], &R, 0x7f) /* do not allow IC */
...
static tFixedOrder* AddFixed(
        tFixedOrder* pOrder, char const* pName, Word Code, CPUVar MinCPU) {
