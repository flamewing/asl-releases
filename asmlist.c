...
void MakeList(char const* pSrcLine) {
    String  h2, Tmp;
  ...
  fprintf(stderr, "[%s] WasIF %u WasMACRO %u DoLst %u\n", OpPart.Str, WasIF, WasMACRO, DoLst);
    ...
    if ((!ListToNull) && (ThisDoLst) && ((ListMask & 1) != 0) && (!IFListMask())) {
        ...
        as_sdprcatf(
                &list_buf, "%8.*" PRIx64 " %c ", ListRadixBase, ListPC,
                Retracted ? 'R' : ':');
        ...
        if (*ListLine) {
            as_sdprcatf(
                    &list_buf, "%s %s%s", ListLine,
                    Blanks(LISTLINESPACE - strlen(ListLine)), pSrcLine);
        ...
        /* Code ausgeben */
            ...
            do {
                /* If not the first code line, prepend blanks to fill up space below line
                 * number: */

                if (!First) {
                    as_sdprintf(
                            &list_buf, "%*s%8.*" PRIx64 " %c ",
                            (ListMask & ListMask_LineNums) ? 9 : 3, "", ListRadixBase,
                            ListPC, Retracted ? 'R' : ':');
                }
                ...
                do {
                    ...
                    if ((Index < EffLen) && !DontPrint) {
                        ...
                        as_sdprcatf(
                                &list_buf, "%0*.*" PRIx64 " ", SystemListLen,
                                ListRadixBase, ThisWord);
                    } else {
...
void asmlist_init(void) {
    ...
    SysString(
            Dummy, sizeof(Dummy), 0xff, ListRadixBase, 0, False, HexStartCharacter,
            SplitByteCharacter);
    SystemListLen8 = strlen(Dummy);
    SysString(
            Dummy, sizeof(Dummy), 0xffffu, ListRadixBase, 0, False, HexStartCharacter,
            SplitByteCharacter);
    SystemListLen16 = strlen(Dummy);
    SysString(
            Dummy, sizeof(Dummy), 0xfffffffful, ListRadixBase, 0, False,
