...
static LongInt PCRelAdr(void) {
    if (PrevDelayed) {
        ...
        return EProgCounter() + 4;
    }
}

static void ChkDelayed(void) {
    if (PrevDelayed) {
        WrError(ErrNum_Pipeline);
    }
}
...
static Boolean DecodeCtrlReg(char* Asc, Word* Erg) {
    ...
    *Erg = 0xff;
    if (!as_strcasecmp(Asc, "SR")) {
        *Erg = 0;
    } else if (!as_strcasecmp(Asc, "GBR")) {
        *Erg = 1;
    } else if (!as_strcasecmp(Asc, "VBR")) {
        *Erg = 2;
        ...
        *Erg   = 4;
        MinCPU = CPU7700;
            ...
            (strlen(Asc) == 7) && (as_toupper(*Asc) == 'R')
            && (!as_strcasecmp(Asc + 2, "_BANK")) && (Asc[1] >= '0') && (Asc[1] <= '7')) {
        *Erg   = Asc[1] - '0' + 8;
        MinCPU = CPU7700;
    }
...
static void DecodeAdr(tStrComp const* pArg, Word Mask, Boolean Signed) {
    Byte         p;
    Word         HReg;
    char*        pos;
            ...
            do {
                ...
                if (!as_strcasecmp(Arg.str.p_str, "PC")) {
                    ...
                    case eIsReg:
                        ...
                        break;
                    case eIsNoReg:
                        DispAcc += EvalStrIntExpressionWithFlags(
                                &Arg, Int32, &OK, &Flags);
    ...
    if (*pArg->str.p_str == '#') {
        ...
        if (OK) {
            ...
            DOpSize = (OpSize == eSymbolSize8Bit)
                              ? eSymbolSize8Bit
                              : (Critical ? eSymbolSize16Bit : eSymbolSize8Bit);
            ...
            if (DOpSize == 0) {
                ...
                while ((Lauf) && (!Found)) {
                    ...
                    if ((!Critical) && (!Lauf->IsForward)
                        ...
                        if (((Lauf->Is32 == NIs32) && (DispAcc == Lauf->Value))
                            || ((Lauf->Is32) && (!NIs32)
                                && (DispAcc == (Lauf->Value >> 16)))) {
                            Found = True;
                        } else if (
                                (Lauf->Is32) && (!NIs32)
                                && (DispAcc == (Lauf->Value & 0xffff))) {
                            Found = True;
                            p     = 2;
                        }
                    }
                ...
                if (!Found) {
                    ...
                    do {
                        ...
                        as_snprintf(
                                LComp.str.p_str, STRINGSIZE, "%s%s",
                                LiteralName(Lauf, Name, sizeof(Name)), AdrStr);
                ...
                if (OK) {
                    ...
                    if (DispAcc < 0) {
                        WrXError(ErrNum_UnderRange, "Disp<0");
                        OK = False;
                    } else if ((DispAcc > 255) && !mSymbolQuestionable(Flags)) {
...
static void DecodeMOV(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        DecodeAdr(
                &ArgStr[2],
                MModReg | MModIReg | MModPreDec | MModIndReg | MModR0Base | MModGBRBase,
                True);
        switch (AdrMode) {
        case ModReg:
        ...
        DecodeAdr(
                &ArgStr[1],
                MModImm | MModPCRel | MModIReg | MModPostInc | MModIndReg | MModR0Base
                        | MModGBRBase,
                True);
        switch (AdrMode) {
        case ModIReg:
        ...
        case ModPCRel:
            if (OpSize == eSymbolSize8Bit) {
                ...
                SetCode(0x9000 + (((Word)OpSize - 1) << 14) + (((Word)HReg) << 8)
                        + AdrPart);
            }
            break;
        case ModImm:
...
static void DecodeLDS_STS(Word IsLDS) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (!DecodeSReg(pArg1->str.p_str, &HReg)) {
...
static void DecodeOneReg(Word Index) {
    OneRegOrder const* pOrder = OneRegOrders + Index;
...
static void DecodeTwoReg(Word Index) {
    TwoRegOrder const* pOrder = TwoRegOrders + Index;

    if (!ChkArgCnt(2, 2))
        ;
    else if (*AttrPart.str.p_str && (OpSize != pOrder->DefSize)) {
        WrError(ErrNum_UseLessAttr);
    } else if (ChkMinCPU(pOrder->MinCPU)) {
        DecodeAdr(&ArgStr[1], MModReg, False);
        if (AdrMode != ModNone) {
...
static void DecodeMulReg(Word Index) {
    FixedMinOrder const* pOrder = MulRegOrders + Index;
...
static void DecodeBW(Word Index) {
    FixedOrder const* pOrder = BWOrders + Index;
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if ((OpSize != eSymbolSize8Bit) && (OpSize != eSymbolSize16Bit)) {
        WrError(ErrNum_InvOpSize);
    } else {
        DecodeAdr(&ArgStr[1], MModReg, False);
        if (AdrMode != ModNone) {
            ...
            if (CompLiterals) {
                ...
                while (EqLauf) {
                    if ((EqLauf->Is32) && (EqLauf->PassNo >= 0)
                        ...
                        StrCompMkTemp(&TmpComp, Name, sizeof(Name));
                        EnterIntSymbol(
                                &TmpComp, EProgCounter() + CodeLen, SegCode, False);
                        EqLauf->PassNo = -1;
                    }
                    EqLauf = EqLauf->Next;
                }
...
static void AddFixed(char const* NName, Word NCode, Boolean NPriv, CPUVar NMin) {
...
static void AddOneReg(
        char const* NName, Word NCode, CPUVar NMin, Boolean NPriv, Boolean NDel) {
...
static void AddTwoReg(
        char const* NName, Word NCode, Boolean NPriv, CPUVar NMin, ShortInt NDef) {
...
static void AddMulReg(char const* NName, Word NCode, CPUVar NMin) {
    if (InstrZ >= MulRegOrderCount) {
        exit(255);
    }
...
static void AddBW(char const* NName, Word NCode) {
    if (InstrZ >= BWOrderCount) {
        exit(255);
    }
...
static void AddSReg(char const* NName, Word NCode, CPUVar NMin, Boolean NDSP) {
    if (InstrZ >= SRegCnt) {
        exit(255);
    }
...
static void InitFields(void) {
    ...
    OneRegOrders = (OneRegOrder*)malloc(sizeof(OneRegOrder) * OneRegOrderCount);
    InstrZ       = 0;
    ...
    TwoRegOrders = (TwoRegOrder*)malloc(sizeof(TwoRegOrder) * TwoRegOrderCount);
    InstrZ       = 0;
    ...
    RegDefs = (TRegDef*)malloc(sizeof(TRegDef) * SRegCnt);
    InstrZ  = 0;
