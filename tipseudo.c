...
typedef void (*tcallback)(Boolean*, int*, LongInt, tSymbolFlags);
...
static Boolean Is99(char const* pStr, Integer* pNum) {
    int l = strlen(pStr);

    if ((l >= 3) && as_isdigit(pStr[l - 2]) && as_isdigit(pStr[l - 1])) {
...
Boolean DecodeTIPseudo(void) {
    ...
    if (!as_strncasecmp(OpPart.str.p_str, "Q", 1) && Is99(OpPart.str.p_str, &Num)) {
        pseudo_qxx(Num);
        return True;
    }
    ...
    if (!as_strncasecmp(OpPart.str.p_str, "LQ", 2) && Is99(OpPart.str.p_str, &Num)) {
        pseudo_lqxx(Num);
        return True;
    }
...
Boolean IsTIDef(void) {
    static char const* defs[] = {"BSS",   "STRING", "RSTRING", "BYTE",   "WORD",   "LONG",
                                 "FLOAT", "DOUBLE", "EFLOAT",  "BFLOAT", "TFLOAT", NULL};
    char const**       cp     = defs;
...
static void DecodeWORD_TI34x(Word Code) {
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        OK = True;
        forallargs(pArg, True) if (OK) DAsmCode[CodeLen++]
                = EvalStrIntExpression(pArg, Int32, &OK);
        if (!OK) {
            CodeLen = 0;
        }
    }
                ...
                for (z2 = 0; z2 < t.Contents.str.len; z2++) {
                    ...
                    DAsmCode[CodeLen - 1]
                            |= (((LongWord)CharTransTable
                                         [((usint)t.Contents.str.p_str[z2]) & 0xff]))
                               << (8 * (3 - (z2 & 3)));
                }
                break;
            }
