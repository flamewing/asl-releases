...
static void DecodeAdr(tStrComp const* pArg, Byte Erl) {
    static char const Reg16Names[][3] = {"WA", "BC", "DE", "HL"};
    static int const  Reg16Cnt        = sizeof(Reg16Names) / sizeof(*Reg16Names);
    static char const AdrRegs[][3]    = {"HL", "DE", "C", "PC", "A"};
...
static void DecodeLD(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        case ModReg16:
            ...
            case ModMem:
                if (AdrMode > 5) {
                    WrError(ErrNum_InvAddrMode); /* (-HL), (HL+) */
                } else {
                    CodeLen     = 2 + AdrCnt;
        ...
        case ModMem:
            ...
            case ModReg16:
                if ((HMode < 2) || (HMode > 4)) {
                    WrError(ErrNum_InvAddrMode); /* (HL),(DE),(HL+d) */
                } else {
                    CodeLen     = 2 + HCnt;
                    BAsmCode[0] = 0xf0 | HMode;
                    memcpy(BAsmCode + 1, &HVal, HCnt);
            ...
            case ModImm:
                if ((HMode == 1) || (HMode == 5)) {
                    WrError(ErrNum_InvAddrMode); /* (HL+C),(PC+A) */
                } else if (HMode == 3)           /* (HL) */
                {
                    CodeLen     = 2;
                    BAsmCode[0] = 0x2d;
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModMem:
            ...
            if (AdrType != ModNone) {
                ...
                if ((AdrMode >= 6) && ((BAsmCode[0] & 0x0f) == 6)) {
                    WrError(ErrNum_Unpredictable);
                }
            }
            break;
...
static void DecodeCLR(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        case ModAbs:
            ...
            break;
        case ModMem:
            if ((AdrMode == 5) || (AdrMode == 1)) {
                WrError(ErrNum_InvAddrMode); /* (PC+A, HL+C) */
    ...
    if (!ChkArgCnt(1, 1))
        ...
        case ModMem:
            if (HReg < 8) {
                ...
                BAsmCode[0] = 0xe0 | HReg;
                BAsmCode[1]
                        = ((Code & 0x18) >> 1) | ((Code & 0x80) >> 3) | 0x80 | AdrMode;
            }
            break;
        }
    }
...
static void DecodeALU(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (Code != 5) {
            WrError(ErrNum_InvAddrMode); /* XOR */
        } else if (!SplitBit(&ArgStr[2], &HReg)) {
            WrError(ErrNum_InvBitPos);
        } else if (HReg >= 8) {
            WrError(ErrNum_InvAddrMode);
        } else {
        ...
        if (Condition >= ConditionCnt) {
            ...
            if (OK) {
                if (((AdrInt < -16) || (AdrInt > 15)) && !mSymbolQuestionable(Flags)) {
                    ...
                    CodeLen = 1;
                    BAsmCode[0]
                            = ((Conditions[Condition].Code - 2) << 5) | (AdrInt & 0x1f);
                }
            }
        }
    }
        ...
        if (Condition >= ConditionCnt) {
            ...
            if (OK) {
                if (((AdrInt < -128) || (AdrInt > 127)) && !mSymbolQuestionable(Flags)) {
                    ...
                    CodeLen     = 2;
                    BAsmCode[0] = (Condition == -1) ? 0xfb
                                                    : 0xd0 | Conditions[Condition].Code;
                    BAsmCode[1] = AdrInt & 0xff;
                }
            }
        }
