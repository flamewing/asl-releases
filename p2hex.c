...
static char const* HexSuffix = ".hex";
#define MaxLineLen     254
#define AVRLEN_DEFAULT 3
#define DefaultCFormat "dSEl"

typedef void (*ProcessProc)(char const* FileName, LongWord Offset);
...
static void Filename2CName(char* pDest, char const* pSrc) {
    char        Trans;
    char const* pPos;
...
static void PrCData(
        FILE* pTargFile, char Ident, char const* pName, char const* pCTargName,
        char const* pCBlockName, LongWord Value) {
    char const* pFormat;
...
static void ProcessFile(char const* FileName, LongWord Offset) {
    FILE*    SrcFile;
    Word     TestID;
    ...
    Boolean  CDataLower = !!strchr(CFormat, 'd'), CDataUpper = !!strchr(CFormat, 'D');
    Byte     Buffer[MaxLineLen];
    Word*    WBuffer = (Word*)Buffer;
    LongWord ErgStart, ErgStop = 0xfffffffful, IntOffset = 0, MaxAdr;
    LongInt  NextPos;
    LongWord ValidSegs;
        ...
        if (InpHeader == FileHeaderStartAdr) {
            ...
            if (doit) {
                ...
                case eHexFormatIntel16:
                    ...
                    IntOffset /= Gran;
                    errno = 0;
                    fprintf(TargFile, ":02000002%04X%02X\n", LoWord(HSeg),
                            Lo(0x100 - ChkSum));
                    ChkIO(TargName);
                    if (MaxIntel < 1) {
                        MaxIntel = 1;
                ...
                case eHexFormatC:
                    ...
                    PrCData(TargFile, 'e', "end", CTargName, CBlockName,
                            ErgStart + ErgLen - 1);
                ...
                while (ErgLen > 0) {
                    ...
                    if ((ActFormat == eHexFormatIntel32)
                        && ((ErgStart & 0xffff) + (TransLen / Gran) >= 0x10000)) {
                    ...
                    case eHexFormatMotoS:
                        errno = 0;
                        fprintf(TargFile, "S%c%02X", '1' + MotRecType,
                                Lo(TransLen + 3 + MotRecType));
                        ChkIO(TargName);
                        ChkSum = TransLen + 3 + MotRecType;
                        if (MotRecType >= 2) {
                            errno = 0;
                            fprintf(TargFile, "%02X", Lo(ErgStart >> 24));
                            ChkIO(TargName);
                            ChkSum += ((ErgStart >> 24) & 0xff);
                        }
                        if (MotRecType >= 1) {
                            errno = 0;
                            fprintf(TargFile, "%02X", Lo(ErgStart >> 16));
                            ChkIO(TargName);
                            ChkSum += ((ErgStart >> 16) & 0xff);
                        }
                        errno = 0;
                        fprintf(TargFile, "%04X", LoWord(ErgStart));
                        ChkIO(TargName);
                        ChkSum += Hi(ErgStart) + Lo(ErgStart);
                        break;
                    case eHexFormatMOS:
                        errno = 0;
                        fprintf(TargFile, ";%02X%04X", Lo(TransLen), LoWord(ErgStart));
                        ChkIO(TargName);
                    ...
                    case eHexFormatIntel32: {
                        ...
                        WrErgStart
                                = (ErgStart - IntOffset) * ((MultiMode < 2) ? Gran : 1);
                        errno = 0;
                        fprintf(TargFile, ":%02X%04X00", Lo(WrTransLen),
                                LoWord(WrErgStart));
                        ChkIO(TargName);
                    ...
                    case eHexFormatTiDSK:
                        errno = 0;
                        fprintf(TargFile, "9%04X", LoWord(/*Gran**/ ErgStart));
                        ChkIO(TargName);
                        ChkSum = 0;
                        break;
                    case eHexFormatAtmel:
                    ...
                    case eHexFormatTiDSK:
                        ...
                        for (z = 0; z < (TransLen / 2); z++) {
                            errno = 0;
                            if (((ErgStart + z >= StartAdr[SegData])
                                 && (ErgStart + z <= StopAdr[SegData]))
                                || (InpSegment == SegData)) {
                    ...
                    case eHexFormatMico8:
                        if (TransLen >= 4) {
                            fprintf(TargFile, "%01X%02X%02X", Buffer[1] & 0x0fu,
                                    Buffer[2] % 0xffu, Buffer[3] & 0xffu);
                            SumLen += 4;
                        }
                        break;
                    case eHexFormatC:
                    ...
                    default:
                        for (z = 0; z < (LongInt)TransLen; z++) {
                            if ((MultiMode < 2) || (z % Gran == MultiMode - 2)) {
                                errno = 0;
                                fprintf(TargFile, "%02X", Lo(Buffer[z]));
                                ChkIO(TargName);
                                ChkSum += Buffer[z];
                                SumLen++;
                            }
                ...
                case eHexFormatMotoS:
                    if (SepMoto) {
                        errno = 0;
                        fprintf(TargFile, "S%c%02X", '9' - MotRecType,
                                Lo(3 + MotRecType));
                        ChkIO(TargName);
                        for (z = 1; z <= 2 + MotRecType; z++) {
                            errno = 0;
                            fprintf(TargFile, "%02X", 0U);
                            ChkIO(TargName);
                        }
                        errno = 0;
    ...
    if (!SumLen) {
        errno = 0;
        fputs(getmessage(Num_WarnEmptyFile), stderr);
        ChkIO(OutName);
    }
...
static void ProcessGroup(char const* GroupName_O, ProcessProc Processor) {
    ...
    if (!DirScan(GroupName, Callback)) {
        fprintf(stderr, "%s%s%s\n", getmessage(Num_ErrMsgNullMaskA), GroupName,
                getmessage(Num_ErrMsgNullMaskB));
    }
}

static void MeasureFile(char const* FileName, LongWord Offset) {
    FILE*    f;
...
static CMDResult CMD_AdrRange(Boolean Negate, char const* Arg) {
    if (Negate) {
        DefStartStopAdr(1 << SegCode);
        return CMDOK;
    } else {
        return CMD_Range(
                &StartAdr[SegCode], &StopAdr[SegCode], &StartAuto, &StopAuto, Arg);
    }
}

static CMDResult CMD_RelAdr(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_AdrRelocate(Boolean Negate, char const* Arg) {
    Boolean ok;
    UNUSED(Arg);
...
static CMDResult CMD_Rec5(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_SepMoto(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_IntelMode(Boolean Negate, char const* Arg) {
    int     Mode;
    Boolean ok;
...
static CMDResult CMD_MultiMode(Boolean Negate, char const* Arg) {
    int     Mode;
    Boolean ok;
...
static CMDResult CMD_DestFormat(Boolean Negate, char const* pArg) {
#define NameCnt (sizeof(Names) / sizeof(*Names))

    static char const* Names[] = {"DEFAULT", "MOTO", "INTEL", "INTEL16", "INTEL32", "MOS",
                                  "TEK",     "DSK",  "ATMEL", "MICO8",   "C"};
...
static CMDResult CMD_ForceSegment(Boolean Negate, char const* Arg) {
...
static CMDResult CMD_DataAdrRange(Boolean Negate, char const* Arg) {
    ...
    if (Negate) {
        ...
        CMDResult Ret = CMD_Range(
                &StartAdr[SegData], &StopAdr[SegData], &StartDataAuto, &StopDataAuto,
                Arg);
...
static CMDResult CMD_EntryAdr(Boolean Negate, char const* Arg) {
    Boolean ok;
...
static CMDResult CMD_LineLen(Boolean Negate, char const* Arg) {
    Boolean ok;
...
static CMDResult CMD_MinMoto(Boolean Negate, char const* Arg) {
    Boolean ok;
...
static CMDResult CMD_AutoErase(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_AVRLen(Boolean Negate, char const* Arg) {
    Word    Temp;
    Boolean ok;
...
static CMDResult CMD_CFormat(Boolean Negate, char const* pArg) {
    if (Negate) {
...
int main(int argc, char** argv) {
    ...
    *TargName       = '\0';
    ...
    strcpy(CFormat, DefaultCFormat);
    ProcessCMD(
            argc, argv, P2HEXParams, P2HEXParamCnt, ParUnprocessed, "P2HEXCMD",
            ParamError);
    ...
    if (StartAuto || StopAuto) {
        ...
        if (!QuietMode) {
            printf("%s: 0x%08lX-", getmessage(Num_InfoMessDeducedRange),
                   LoDWord(StartAdr[ChkSegment]));
    ...
    if ((FormatOccured & eMotoOccured) && (!SepMoto)) {
        errno = 0;
        fprintf(TargFile, "S%c%02X", '9' - MaxMoto, Lo(3 + MaxMoto));
        ChkIO(TargName);
        ChkSum = 3 + MaxMoto;
        if (!EntryAdrPresent) {
            EntryAdr = 0;
        ...
        if (MaxMoto >= 2) {
            errno = 0;
            fprintf(TargFile, "%02X", Lo(EntryAdr >> 24));
            ChkIO(TargName);
            ChkSum += (EntryAdr >> 24) & 0xff;
        }
        if (MaxMoto >= 1) {
            errno = 0;
            fprintf(TargFile, "%02X", Lo(EntryAdr >> 16));
            ChkIO(TargName);
            ChkSum += (EntryAdr >> 16) & 0xff;
        }
        errno = 0;
        fprintf(TargFile, "%04X", LoWord(EntryAdr & 0xffff));
        ChkIO(TargName);
    ...
    if (FormatOccured & eIntelOccured) {
        ...
        if (EntryAdrPresent) {
            ...
            case 2:
                errno = 0;
                fprintf(TargFile, ":04000005");
                ChkIO(TargName);
                ChkSum = 4 + 5;
                errno  = 0;
                fprintf(TargFile, "%08lX", LoDWord(EntryAdr));
                ChkIO(TargName);
                ChkSum += ((EntryAdr >> 24) & 0xff) + ((EntryAdr >> 16) & 0xff)
                          + ((EntryAdr >> 8) & 0xff) + (EntryAdr & 0xff);
                goto WrChkSum;
            ...
            case 1:
                ...
                Ofs   = EntryAdr & 0x000f;
                errno = 0;
                fprintf(TargFile, ":04000003%04X%04X", LoWord(Seg), LoWord(Ofs));
                ChkIO(TargName);
    ...
    if (DestFormat == eHexFormatC) {
        ...
        errno = 0;
        fprintf(TargFile, "typedef struct\n"
                          "{\n");
        ...
        for (ThisCBlock = 0; ThisCBlock < NumCBlocks; ThisCBlock++) {
            ...
            for (pFormat = CFormat; *pFormat; pFormat++) {
                ...
                switch (toupper(*pFormat)) {
                case 'D':
                    fprintf(TargFile, "%s%s_data", CTargName, CBlockName);
                    break;
                case 'S':
                    fprintf(TargFile, "%s%s_start", CTargName, CBlockName);
                    break;
                case 'L':
                    fprintf(TargFile, "%s%s_len", CTargName, CBlockName);
                    break;
                case 'E':
                    fprintf(TargFile, "%s%s_end", CTargName, CBlockName);
                    break;
                default:
                    break;
                }
