...
static void DecodeAdr(tStrComp const* pArg, Word Mask) {
    static tNotation const Notations[] = {
            {   "A",    ModA},
            {   "M",    ModM},
            {"(DP)",    ModM},
            {   "W",    ModW},
    ...
    AdrVal = EvalStrIntExpressionOffs(
            pArg, !!(0 [pArg->str.p_str] == '#'), OpSizeType, &OK);
    if (OK) {
        AdrMode = ModImm;
...
static void DecodeLD(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                &ArgStr[1], MModA | MModDPL | MModDPH | MModW | MModX | MModY | MModZ
                                    | MModPP | MModT);
        switch (AdrMode) {
        case ModA:
            DecodeAdr(
                    &ArgStr[2], MModImm | MModW | MModX | MModY | MModZ | MModM | MModDPL
                                        | MModTL | MModTH);
            switch (AdrMode) {
            case ModW:
            case ModX:
...
static void DecodeINCDEC(Word IsDEC) {
    if (ChkArgCnt(1, 1)) {
        DecodeAdr(
                &ArgStr[1], MModA | MModW | MModX | MModY | MModZ | MModDPL | MModM
...
static void CodeFixed(Word Code) {
    if (ChkArgCnt(0, 0) && (ChkExactCPUMask(Hi(Code), CPU5840) >= 0)) {
        BAsmCode[CodeLen++] = Lo(Code);
    }
}

static void CodeTHB(Word Code) {
    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(Hi(Code), CPU5840) >= 0)) {
        Boolean OK     = True;
...
static void CodeLTI(Word Code) {
    if (ChkArgCnt(0, 1) && (ChkExactCPUMask(Hi(Code), CPU5840) >= 0)) {
        Boolean OK     = True;
...
static void CodeJC(Word Code) {
    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(Hi(Code), CPU5840) >= 0)) {
        ...
        if (EvalResult.OK
            && ChkSamePage(EProgCounter() + 1, Address, 6, EvalResult.Flags)) {
