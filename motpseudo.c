...
static Boolean CutRep(
        tStrComp* pDest, tStrComp const* pSrc, LongInt* pErg, tSymbolFlags* pFlags) {
    tStrComp Src;
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        ...
        forallargs(pArg, OK) {
            ...
            if (!strcmp(Arg.str.p_str, "?")) {
                ...
                case TempInt:
                ToInt:
                    if (!mFirstPassUnknownOrQuestionable(t.Flags)
                        && !RangeCheck(t.Contents.Int, Int8)) {
        ...
        forallargs(pArg, OK) {
            ...
            if (!strcmp(Arg.str.p_str, "?")) {
                ...
                ToInt:
                    ...
                    if (!mSymbolQuestionable(Res.Flags)
                        && !RangeCheck(Res.Contents.Int, Int16)) {
...
static void EnterQWord(LargeWord q) {
    if (ListGran() == 1) {
        ...
        BAsmCode[CodeLen] = BAsmCode[CodeLen + 1] = BAsmCode[CodeLen + 2]
...
#else
        /* TempResult is LargeInt, so sign-extend */
        WAsmCode[(CodeLen >> 1)] = WAsmCode[(CodeLen >> 1) + 1]
                = (q & 0x80000000ul) ? 0xffff : 0x00;
#endif
...
void DecodeMotoDC(tSymbolSize OpSize, Boolean Turn) {
    ...
    Boolean   PadBeforeStart
            = Odd(EProgCounter()) && DoPadding && (OpSize != eSymbolSize8Bit);
            ...
            ToInt:
                if (!EnterInt) {
                    if (ConvertFloat && EnterFloat) {
                        ...
                        !mFirstPassUnknownOrQuestionable(t.Flags)
                        && !RangeCheck(t.Contents.Int, IntTypeEnum)) {
                    WrError(ErrNum_OverRange);
                    OK = False;
    ...
    if (Memo("DS")) {
        ...
        if (ChkArgCnt(1, 1)) {
            ...
            if (ValOK && !mFirstPassUnknown(Flags)) {
                Boolean OddSize
                        = (eSymbolSize8Bit == OpSize) || (eSymbolSize24Bit == OpSize);
                ...
                /* value of 0 means aligning the PC.  Doesn't make sense for bytes and 24
                 * bit values */
...
static Boolean DecodeMoto16AttrSizeCore(
        char SizeSpec, tSymbolSize* pResult, Boolean Allow24) {
    switch (as_toupper(SizeSpec)) {
    case 'B':
        *pResult = eSymbolSize8Bit;
        break;
    case 'W':
        *pResult = eSymbolSize16Bit;
        break;
    case 'L':
        *pResult = eSymbolSize32Bit;
        break;
    case 'Q':
        *pResult = eSymbolSize64Bit;
        break;
    case 'S':
        *pResult = eSymbolSizeFloat32Bit;
        break;
    case 'D':
        *pResult = eSymbolSizeFloat64Bit;
        break;
    case 'X':
        *pResult = eSymbolSizeFloat96Bit;
        break;
    case 'C':
        *pResult = eSymbolSizeFloat16Bit;
        break;
    case 'P':
        *pResult = Allow24 ? eSymbolSize24Bit : eSymbolSizeFloatDec96Bit;
        break;
    case '\0':
        break;
    default:
...
Boolean DecodeMoto16AttrSizeStr(
