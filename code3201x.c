...
static void DecodeAdr(tStrComp const* pArg, int Aux, Boolean Must1) {
    Byte  h;
    char* p;
    ...
    if ((!strcmp(pArg->str.p_str, "*")) || (!strcmp(pArg->str.p_str, "*-"))
        || (!strcmp(pArg->str.p_str, "*+"))) {
        AdrMode = 0x88;
        if (strlen(Arg) == 2) {
        ...
        if ((strlen(pArg->str.p_str) > 3)
            && (!as_strncasecmp(pArg->str.p_str, "DAT", 3))) {
            AdrOK = True;
...
static void DecodeAdrShift(Word Index) {
    ...
    if (ChkArgCnt(1, 3)) {
        ...
        if (AdrOK) {
            ...
            if (OK) {
                if ((pOrder->AllowShifts & (1 << AdrWord)) == 0) {
...
static void DecodeImm(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (OK) {
            ...
            if (AdrLong < pOrder->Min) {
                WrError(ErrNum_UnderRange);
            } else if (AdrLong > pOrder->Max) {
...
static void AddAdr(char const* NName, Word NCode, Word NMust1) {
    AddInstTable(InstTable, NName, NCode | NMust1, DecodeAdrInst);
}

static void AddAdrShift(char const* NName, Word NCode, Word NAllow) {
...
static void InternSymbol_3201X(char* pArg, TempResult* pResult) {
    if ((strlen(pArg) == 3) && (as_toupper(pArg[0]) == 'P')
        && (as_toupper(pArg[1]) == 'A') && ((pArg[2] >= '0') && (pArg[2] <= '7'))) {
