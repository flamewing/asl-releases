...
static int CheckForcePrefix(char const* pArg, Boolean* pForce) {
    if (*pArg == '>') {
        *pForce = True;
        return 1;
    }
...
static Boolean DecodeSpecReg(char* Asc, Byte* Result) {
    if (!as_strcasecmp(Asc, "SP")) {
        ...
        *Result = 0x81;
    } else if (!as_strcasecmp(Asc, "ESP")) {
        *Result = 0x83;
    } else if (!as_strcasecmp(Asc, "PBP")) {
        *Result = 0x05;
    } else if (!as_strcasecmp(Asc, "CBP")) {
        *Result = 0x07;
    } else if (!as_strcasecmp(Asc, "PSW")) {
        *Result = 0x89;
    } else if (!as_strcasecmp(Asc, "IMC")) {
        *Result = 0x0b;
    } else if (!as_strcasecmp(Asc, "CC")) {
        *Result = 0x0e;
    } else {
        return False;
    }
...
static Boolean DecodeRegAdr(tStrComp const* pArg, Byte* pResult) {
    tSymbolSize Size;
...
static void DecodeAdr(
        tStrComp const* pArg, Byte PrefInd, tImmAllow MayImm, Boolean MayReg) {
#define FreeReg 0xff
#define SPReg   0xfe
#define PCReg   0xfd
    ...
    if (IsIndirect(pArg->str.p_str)) {
        ...
        do {
            ...
            if ((EPos == Arg.str.p_str + ArgLen - 2)
                && ((Arg.str.p_str[ArgLen - 1] == '1')
                    || (Arg.str.p_str[ArgLen - 1] == '2')
                    || (Arg.str.p_str[ArgLen - 1] == '4')
                    || (Arg.str.p_str[ArgLen - 1] == '8'))
                && ((RegEvalResult = DecodeReg(&TmpComp, &Reg, &Size, False))
                    != eIsNoReg)) {
            ...
            /* I.4.d. Sonderregister */
                    ...
                    (!as_strcasecmp(Arg.str.p_str, "PC"))
                    || (!as_strcasecmp(Arg.str.p_str, "SP"))) {
            ...
            /* I.4.e. Displacement */
                ...
                DispPart = EvalStrIntExpressionOffsWithFlags(
                        &Arg, CheckForcePrefix(Arg.str.p_str, &ForcePrefix), Int32, &OK,
                        &Flags);
                if (!OK) {
                    return;
                }
        ...
        if ((BaseReg == FreeReg) && (IndReg == FreeReg)) {
            AdrMode = 0x20; /* 0x60 should be equivalent: adding 0 as RW0 or RD0 is
                               irrelvant */
    ...
    /* III. Spezialregister */
        ...
        case eImmAsAbs: {
            ...
            LongInt DispAcc     = EvalStrIntExpressionOffs(
                    pArg, CheckForcePrefix(pArg->str.p_str, &ForcePrefix), Int32, &AdrOK);
...
static Boolean DecodeCondition(char const* pAsc, Word* pCondition) {
    int z;
...
static char DecideGA(void) {
    if (((IsShort()) && (Is2Absolute())) || ((Is2Short()) && (IsAbsolute()))) {
        return 'A';
    } else {
        return 'G';
    }
...
static void DecodeRMW(Word Index) {
    RMWOrder const* pOrder = RMWOrders + Index;
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (AdrOK) {
            if (OpSize == -1) {
                WrError(ErrNum_UndefOpSizes);
            } else if (!(pOrder->Mask & (1 << OpSize))) {
                WrError(ErrNum_InvOpSize);
            } else {
                WAsmCode[0] = (((Word)OpSize + 1) << 14) + (((Word)pOrder->Code) << 8)
                              + AdrMode;
        ...
        if (AdrOK) {
            ...
            if (AdrOK) {
                if (OpSize == -1) {
                    ...
                    if (Format == ' ') {
                        if (((IsReg()) && (Is2Short())) || ((Is2Reg()) && (IsShort()))) {
                            Format = 'S';
                        } else if (
                                (IsImmediate()) && (OpSize > 0)
                                && ((ImmValue > 127) || (ImmValue < -128))) {
                            Format = 'I';
                        } else {
                            Format = DecideGA();
                        }
                    ...
                    case 'S':
                        if ((IsShort()) && (Is2Reg())) {
                            ...
                            WAsmCode[0] = 0x0000 + (((Word)OpSize + 1) << 14)
                                          + (AdrMode & 15) + ((AdrMode & 0xf0) << 5)
                                          + ((AdrMode2 & 1) << 12)
                                          + ((AdrMode2 & 14) << 4) + ((Code & 1) << 4)
                                          + ((Code & 2) << 10);
                            memcpy(WAsmCode + 1, AdrVals, AdrCnt);
                            CodeLen = 2 + AdrCnt;
                            ...
                            WAsmCode[0] = 0x0100 + (((Word)OpSize + 1) << 14)
                                          + (AdrMode2 & 15) + ((AdrMode2 & 0xf0) << 5)
                                          + ((AdrMode & 1) << 12) + ((AdrMode & 14) << 4)
                                          + ((Code & 1) << 4) + ((Code & 2) << 11);
                            memcpy(WAsmCode + 1, AdrVals2, AdrCnt2);
                            CodeLen = 2 + AdrCnt2;
                        } else {
                        ...
                        break;
                    case 'I':
                        if ((!IsImmediate()) || (OpSize == 0)) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            WAsmCode[0]
                                    = AdrMode2 + (((Word)OpSize - 1) << 11) + (Code << 8);
                            memcpy(WAsmCode + 1, AdrVals2, AdrCnt2);
                ...
                if (OpSize == -1) {
                    ...
                    case 'I':
                        if ((!IsImmediate()) || (OpSize == 0)) {
                            ...
                            WAsmCode[0] = 0x400 + AdrMode2 + (((Word)OpSize - 1) << 11)
                                          + (Code << 8);
                            memcpy(WAsmCode + 1, AdrVals2, AdrCnt2);
                            memcpy(WAsmCode + 1 + (AdrCnt2 >> 1), AdrVals, AdrCnt);
                            CodeLen = 2 + AdrCnt + AdrCnt2;
        ...
        if (AdrOK) {
            if (OpSize == -1) {
                ...
                if (AdrOK) {
                    ...
                    OpSize = OpSize2;
                    if (Format == ' ') {
                        if ((Is2Reg()) && (IsImmediate()) && (ImmValue > 0)
                            && (ImmValue < (1 << (OpSize + 3)))) {
                            Format = 'S';
                        } else {
                            Format = DecideGA();
                    ...
                    case 'G':
                        WAsmCode[0] = 0x700 + (((Word)OpSize + 1) << 14);
                        if ((IsImmediate()) && (ImmValue >= LowLim8)
                            && (ImmValue < 127)) {
                            AdrMode = ImmValue & 0xff;
                            AdrCnt  = 0;
                        } else {
                    ...
                    case 'A':
                        ...
                        break;
                    case 'S':
                        if ((Is2Reg()) && (IsImmediate()) && (ImmVal() >= 0)
                            && (ImmVal() < (1 << (3 + OpSize)))) {
                            if (OpSize == 2) {
                                if (ImmVal() >= 16) {
                                    AdrVals[0] -= 16;
            ...
            if (OpSize == -1) {
                ...
                if (AdrOK) {
                    ...
                    case 'G':
                        ...
                        WAsmCode[0] += AdrMode;
                        memcpy(WAsmCode + 1, AdrVals, AdrCnt);
                        WAsmCode[1 + (AdrCnt >> 1)] = 0xb400 + ((Code & 3) << 8)
                                                      + ((Code & 4) << 9) + AdrMode2;
                        memcpy(WAsmCode + 2 + (AdrCnt >> 1), AdrVals2, AdrCnt2);
                        CodeLen = 4 + AdrCnt + AdrCnt2;
                        break;
        ...
        if (!DecodeReg(pArg1, &Reg, &Size, True))
            ...
            if (OK) {
                ...
                if (Num2 > 15) {
                    ...
                    switch (OpSize) {
                    case 0:
                        Num1 = EvalStrIntExpression(&ArgStr[3], UInt3, &OK) & 7;
                        break;
                    case 1:
                        Num1 = EvalStrIntExpression(&ArgStr[3], Int4, &OK) & 15;
                        break;
                    case 2:
                        Num1 = EvalStrIntExpression(&ArgStr[3], Int5, &OK) & 31;
                        break;
                    default:
                        abort();
                    }
            ...
            if ((OpSize != 1) && (OpSize != 2)) {
                ...
                if (AdrOK) {
                    ...
                    case 'G':
                        ...
                        memcpy(WAsmCode + 1, AdrVals2, AdrCnt2);
                        WAsmCode[1 + (AdrCnt2 >> 1)] = 0xa600 + AdrMode + (IsSigned << 8);
                        memcpy(WAsmCode + 2 + (AdrCnt2 >> 1), AdrVals, AdrCnt);
                        CodeLen = 4 + AdrCnt + AdrCnt2;
                        break;
                    case 'A':
                        if ((IsAbsolute()) && (Is2Short())) {
                            ...
                            memcpy(WAsmCode + 1, AdrVals2, AdrCnt2);
                            WAsmCode[1 + (AdrCnt2 >> 1)]
                                    = 0x4000 + (AdrVals[0] & 0x1fff) + (IsSigned << 13);
                            CodeLen = 4 + AdrCnt2;
                        } else if ((Is2Absolute()) && (IsShort())) {
                            ConvertShort();
                            ...
                            memcpy(WAsmCode + 1, AdrVals, AdrCnt);
                            WAsmCode[1 + (AdrCnt >> 1)]
                                    = 0x4000 + (AdrVals2[0] & 0x1fff) + (IsSigned << 13);
                            CodeLen = 4 + AdrCnt;
                        } else {
                            WrError(ErrNum_InvAddrMode);
                        }
        ...
        if (AdrOK) {
            ...
            if (AdrOK) {
                if (OpSize == -1) {
                    ...
                    WAsmCode[0] += ((Word)OpSize + 1) << 14;
                    WAsmCode[1 + (Cnt >> 1)]
                            = 0x8000 + AdrMode + (Code << 8) + (((Word)Reg) << 11);
                    memcpy(WAsmCode + 2 + (Cnt >> 1), AdrVals, AdrCnt);
                    CodeLen = 4 + AdrCnt + Cnt;
                }
            }
...
static void DecodeCALR_JR(Word Code) {
    if (!ChkArgCnt(1, 1))
        ...
        AdrInt = EvalStrIntExpressionOffs(
                         &ArgStr[1], CheckForcePrefix(ArgStr[1].str.p_str, &ForcePrefix),
                         Int32, &OK)
                 - EProgCounter();
...
static void DecodeJRC(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (!DecodeCondition(ArgStr[1].str.p_str, &Condition)) {
            ...
            AdrInt = EvalStrIntExpressionOffs(
                             &ArgStr[2],
                             CheckForcePrefix(ArgStr[2].str.p_str, &ForcePrefix), Int32,
                             &OK)
                     - EProgCounter();
    ...
    if (!ChkArgCnt(3, 3))
        ...
        if (OK) {
            ...
            AdrLong = EvalStrIntExpressionOffs(
                    &ArgStr[2], CheckForcePrefix(ArgStr[2].str.p_str, &AdrLongPrefix),
                    Int24, &OK);
            if (OK) {
                LongInt AdrInt;
                ...
                AdrInt = EvalStrIntExpressionOffs(
                                 &ArgStr[3],
                                 CheckForcePrefix(ArgStr[3].str.p_str, &AdrIntPrefix),
                                 Int32, &OK)
                         - EProgCounter();
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (AdrOK) {
            if ((OpSize != 1) && (OpSize != 2)) {
                ...
                AdrInt = EvalStrIntExpressionOffs(
                                 &ArgStr[2],
                                 CheckForcePrefix(ArgStr[2].str.p_str, &ForcePrefix),
                                 Int32, &OK)
                         - (EProgCounter() + 4 + AdrCnt + 2 * Ord(PrefUsed[0]));
...
static void DecodeDJNZC(Word Code) {
    ...
    if (ChkArgCnt(3, 3)) {
        ...
        if (!DecodeCondition(ArgStr[2].str.p_str, &Condition)) {
            WrStrErrorPos(ErrNum_UndefCond, &ArgStr[2]);
        } else {
            Condition %= 16;
            DecodeAdr(&ArgStr[1], 0, eImmNo, True);
            if (AdrOK) {
                if ((OpSize != 1) && (OpSize != 2)) {
                    ...
                    AdrInt = EvalStrIntExpressionOffs(
                                     &ArgStr[3],
                                     CheckForcePrefix(ArgStr[3].str.p_str, &ForcePrefix),
                                     Int32, &OK)
                             - EProgCounter();
                    if ((OK) && (AddRelPrefix(1, 13, &AdrInt, ForcePrefix))) {
                        if (Odd(AdrInt)) {
...
static void DecodeLINK_RETD(Word Code) {
    if (!ChkArgCnt(1, 1))
        ...
        AdrInt = EvalStrIntExpressionOffsWithFlags(
                &ArgStr[1], CheckForcePrefix(ArgStr[1].str.p_str, &ForcePrefix), Int32,
                &OK, &Flags);
        if (mFirstPassUnknown(Flags)) {
            AdrInt &= 0x1fe;
        }
...
static void AddRMW(char const* NName, Byte NCode, Byte NMask) {
    if (InstrZ >= RMWOrderCount) {
        exit(255);
    }
...
static void AddGAEq(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeGAEq);
}

static void AddGAHalf(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeGAHalf);
}

static void AddGAFirst(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeGAFirst);
}

static void AddGASecond(char const* NName, Word NCode) {
...
static void InitFields(void) {
    ...
    Conditions           = (char const**)malloc(sizeof(char*) * ConditionCount);
    InstrZ               = 0;
    Conditions[InstrZ++] = "C";
