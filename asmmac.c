...
static Boolean ExpandDefines_NErl(char inp) {
    return (((inp >= '0') && (inp <= '9')) || ((inp >= 'A') && (inp <= 'Z'))
            || ((inp >= 'a') && (inp <= 'z')));
}
        ...
        do {
            ...
            do {
                ...
                while ((z2 >= 0) && (p2 <= p - FromLen)) {
                    ...
                    if (z2 >= 0) {
                        p2 += Lauf->Compiled[(unsigned int)t_toupper(
                                Line[p2 + FromLen - 1])];
                    }
                }
                if (z2 == -1) {
                    if (((p2 == 0) || (!ExpandDefines_NErl(Line[p2 - 1])))
                        && ((p2 + FromLen == p)
                            || (!ExpandDefines_NErl(Line[p2 + FromLen])))) {
                        if (Diff != 0) {
                            memmove(Line + p2 + ToLen, Line + p2 + FromLen,
                                    strlen(Line) - p2 - FromLen + 1);
                        }
...
static Boolean MacroAdder(PTree* PDest, PTree Neu, void* pData) {
    ...
    if ((*Node)->Defined) {
        if (Protest) {
            WrXError(ErrNum_DoubleMacro, Neu->Name);
        } else {
...
static void PrintMacroList_PNode(PTree Tree, void* pData) {
    ...
    if (Node->Contents->LstMacroExpMod.Count) {
        strmaxcat(h, "{", STRINGSIZE);
        DumpLstMacroExpMod(
                &Node->Contents->LstMacroExpMod, h + strlen(h), sizeof(h) - strlen(h));
        strmaxcat(h, "}", STRINGSIZE);
    }
...
void PrintMacroList(void) {
    ...
    WrLstLine("");
    as_snprintf(
            Context.OneS, sizeof(Context.OneS), "%7lu%s", (unsigned long)Context.Sum,
