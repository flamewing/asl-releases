...
static Boolean DecodeAdr(tStrComp const* pArg, int MinArgCnt, int aux, Boolean Must1) {
    tAdrMode const* pAdrMode = AdrModes;
    Byte            h;
    Boolean         AdrOK = False;
...
static void DecodeCNFD(Word Code) {
    ...
    if (ChkArgCnt(0, 0) && ChkExcludeCPU(CPU32026)) {
        CodeLen     = 1;
        WAsmCode[0] = 0xce04;
    }
}
...
static void DecodeCONF(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkExactCPU(CPU32026)) {
...
static void DecodeAdrInst(Word Index) {
    AdrOrder const* pOrder = AdrOrders + Index;
...
static void Decode2ndAdr(Word Index) {
    AdrOrder const* pOrder = Adr2ndAdrOrders + Index;
    Boolean         OK;
...
static void DecodeShiftAdr(Word Index) {
    AdrShiftOrder const* pOrder = AdrShiftOrders + Index;
...
static void DecodeImm(Word Index) {
    ImmOrder const* pOrder = ImmOrders + Index;
        ...
        if (EvalResult.OK) {
            ...
            if (pOrder->Mask == 0xffff) {
                if (ChkRange(AdrLong, -32768, 65535)) {
                    ...
                    EvalResult.OK = True;
                    if (ArgCnt == 2) {
                        AdrWord = EvalStrIntExpressionWithResult(
                                &ArgStr[2], Int4, &EvalResult);
...
static void AddAdr(char const* NName, Word NCode, Boolean NMust1) {
...
static void Add2ndAdr(char const* NName, Word NCode, Boolean NMust1) {
    if (InstrZ >= Adr2ndAdrOrderCnt) {
        exit(255);
    }
...
static void AddShiftAdr(char const* NName, Word NCode, Word nallow) {
    if (InstrZ >= AdrShiftOrderCnt) {
        exit(255);
    }
...
static void AddImm(
        char const* NName, Word NCode, Integer NMin, Integer NMax, Word NMask) {
...
static void AddAdrMode(char const* NName, Word NMode) {
...
static void InitFields(void) {
    ...
    AddFixed("SFR", 0xce19);
    AddFixed("ZAC", 0xca00);
    AddFixed("APAC", 0xce15);
    AddFixed("PAC", 0xce14);
    AddFixed("SPAC", 0xce16);
    AddFixed("BACC", 0xce25);
    AddFixed("CALA", 0xce24);
    AddFixed("RET", 0xce26);
    AddFixed("RFSM", 0xce36);
    AddFixed("RTXM", 0xce20);
    AddFixed("RXF", 0xce0c);
    AddFixed("SFSM", 0xce37);
    AddFixed("STXM", 0xce21);
    AddFixed("SXF", 0xce0d);
    AddFixed("DINT", 0xce01);
    AddFixed("EINT", 0xce00);
    AddFixed("IDLE", 0xce1f);
    AddFixed("NOP", 0x5500);
    AddFixed("POP", 0xce1d);
    AddFixed("PUSH", 0xce1c);
    AddFixed("RC", 0xce30);
    AddFixed("RHM", 0xce38);
    AddFixed("ROVM", 0xce02);
    AddFixed("RSXM", 0xce06);
    AddFixed("RTC", 0xce32);
    AddFixed("SC", 0xce31);
    AddFixed("SHM", 0xce39);
    AddFixed("SOVM", 0xce03);
    AddFixed("SSXM", 0xce07);
    AddFixed("STC", 0xce33);
    ...
    InstrZ    = 0;
    AddAdr("ADDC", 0x4300, False);
    AddAdr("ADDH", 0x4800, False);
    AddAdr("ADDS", 0x4900, False);
    AddAdr("ADDT", 0x4a00, False);
    AddAdr("AND", 0x4e00, False);
    AddAdr("LACT", 0x4200, False);
    AddAdr("OR", 0x4d00, False);
    AddAdr("SUBB", 0x4f00, False);
    AddAdr("SUBC", 0x4700, False);
    AddAdr("SUBH", 0x4400, False);
    AddAdr("SUBS", 0x4500, False);
    AddAdr("SUBT", 0x4600, False);
    AddAdr("XOR", 0x4c00, False);
    AddAdr("ZALH", 0x4000, False);
    AddAdr("ZALR", 0x7b00, False);
    AddAdr("ZALS", 0x4100, False);
    AddAdr("LDP", 0x5200, False);
    AddAdr("MAR", 0x5500, False);
    AddAdr("LPH", 0x5300, False);
    AddAdr("LT", 0x3c00, False);
    AddAdr("LTA", 0x3d00, False);
    AddAdr("LTD", 0x3f00, False);
    AddAdr("LTP", 0x3e00, False);
    AddAdr("LTS", 0x5b00, False);
    AddAdr("MPY", 0x3800, False);
    AddAdr("MPYA", 0x3a00, False);
    AddAdr("MPYS", 0x3b00, False);
    AddAdr("MPYU", 0xcf00, False);
    AddAdr("SPH", 0x7d00, False);
    AddAdr("SPL", 0x7c00, False);
    AddAdr("SQRA", 0x3900, False);
    AddAdr("SQRS", 0x5a00, False);
    AddAdr("DMOV", 0x5600, False);
    AddAdr("TBLR", 0x5800, False);
    AddAdr("TBLW", 0x5900, False);
    AddAdr("BITT", 0x5700, False);
    AddAdr("LST", 0x5000, False);
    AddAdr("LST1", 0x5100, False);
    AddAdr("POPD", 0x7a00, False);
    AddAdr("PSHD", 0x5400, False);
    AddAdr("RPT", 0x4b00, False);
    AddAdr("SST", 0x7800, True);
    AddAdr("SST1", 0x7900, True);

    Adr2ndAdrOrders = (AdrOrder*)malloc(sizeof(AdrOrder) * Adr2ndAdrOrderCnt);
    InstrZ          = 0;
    Add2ndAdr("BLKD", 0xfd00, False);
    Add2ndAdr("BLKP", 0xfc00, False);
    Add2ndAdr("MAC", 0x5d00, False);
    Add2ndAdr("MACD", 0x5c00, False);

    AdrShiftOrders = (AdrShiftOrder*)malloc(sizeof(AdrShiftOrder) * AdrShiftOrderCnt);
    InstrZ         = 0;
    AddShiftAdr("ADD", 0x0000, 0xf);
    AddShiftAdr("LAC", 0x2000, 0xf);
    AddShiftAdr("SACH", 0x6800, 0x7);
    AddShiftAdr("SACL", 0x6000, 0x7);
    AddShiftAdr("SUB", 0x1000, 0xf);
    AddShiftAdr("BIT", 0x9000, 0xf);
