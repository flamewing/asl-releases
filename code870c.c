...
static Boolean DecodeRegDisp(
        tStrComp* pArg, Byte* pRegFlag, LongInt* pDispAcc, Boolean* pFirstFlag) {
    static char const AdrRegs[][3] = {"DE", "HL", "IX", "IY", "SP", "C"};
    static int const  AdrRegCnt    = sizeof(AdrRegs) / sizeof(*AdrRegs);
...
static void DecodeAdr(tStrComp const* pArg, Byte Erl, Boolean IsDest) {
    static char const Reg16Names[][3] = {"WA", "BC", "DE", "HL", "IX", "IY", "SP"};
    static int const  Reg16Cnt        = sizeof(Reg16Names) / sizeof(*Reg16Names);
...
static Boolean DecodeSPDisp(tStrComp const* pArg, Byte* pDisp, Boolean* pDispNeg) {
    Boolean      OK;
    LongInt      DispAcc;
    tSymbolFlags Flags;
    ...
    *pDispNeg = False;
    ...
    return True; /* return True even if disp is out of range, addressing mode was properly
                    detected */
}
...
static void DecodeLD(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (OK) {
            ...
            (!as_strcasecmp(ArgStr[1].str.p_str, "SP"))
            && (DecodeSPDisp(&ArgStr[2], BAsmCode + 1, &NegFlag))) {
        BAsmCode[0] = NegFlag ? 0x3f : 0x37;
        CodeLen     = 2;
    } else if (!as_strcasecmp(ArgStr[1].str.p_str, "CF")) {
        if (!SplitBit(&ArgStr[2], &Bit)) {
        ...
        if (OK) {
            ...
            case ModMem:
                if (AdrMode == 0xe3) /* HL */
                {
                    ...
                    BAsmCode[1] = AdrInt & 0xff;
                    BAsmCode[2] = AdrInt >> 8;
                } else if (AdrMode != 0xe0) {
                    WrError(ErrNum_InvAddrMode); /* (nn) */
                } else {
                    CodeLen     = 3;
                    BAsmCode[0] = 0x08;
                    BAsmCode[1] = AdrInt & 0xff;
...
static void DecodeXCH(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                ...
                FALSE); /* set IsDest FALSE for mirrored MemOp */
        switch (AdrType) {
        case ModReg8:
            HReg = AdrMode;
...
static void DecodeALU(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (Code != 5) {
            WrError(ErrNum_InvAddrMode); /* XOR only */
        } else if (!SplitBit(&ArgStr[2], &Bit)) {
            WrError(ErrNum_InvBitPos);
        } else if (Bit >= 8) {
            WrError(ErrNum_InvAddrMode); /* only fixed bit # */
        } else {
            DecodeAdr(&ArgStr[2], MModReg8 | MModMem, False);
            switch (AdrType) {
            case ModReg8:
        ...
        DecodeAdr(
                &ArgStr[1], MModReg8 | MModReg16 | MModMem,
                FALSE); /* (+SP) allowed as dest mem op instead of (SP-) */
        switch (AdrType) {
        case ModReg8:
            HReg = AdrMode;
            DecodeAdr(&ArgStr[2], MModReg8 | MModMem | MModImm, FALSE);
...
static void DecodeDIV(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(&ArgStr[1], MModReg16, False);
        if (AdrType == ModReg16) {
            if ((AdrMode == 1) || (AdrMode > 3)) {
                WrError(ErrNum_InvAddrMode); /* WA DE HL */
            } else {
                Byte HReg = AdrMode;
                DecodeAdr(&ArgStr[2], MModReg8, False);
                if (AdrType == ModReg8) {
                    if (AdrMode != 2) {
                        WrError(ErrNum_InvAddrMode); /* C */
                    } else {
                        CodeLen     = 2;
                        BAsmCode[0] = 0xe8 | HReg;
                        BAsmCode[1] = 0xf3;
...
static void DecodeNEG(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if (as_strcasecmp(ArgStr[1].str.p_str, "CS")) {
        WrError(ErrNum_InvAddrMode);
    } else {
...
static void DecodeTEST_CPL_SET_CLR(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        case ModMem:
            if ((Bit < 8) && (AdrMode == 0xe0)
                && (Lo(Code) != 0xe0)) /* no short addr. for CPL */
            {
                CodeLen     = 2;
...
static void DecodeJRS(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if ((Condition >= ConditionCnt) || (Condition < ConditionCnt - 2)) {
            WrStrErrorPos(ErrNum_UndefCond, &ArgStr[1]); /* only T/F allowed */
        } else {
            AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[2], Int16, &OK, &Flags)
                     - (EProgCounter() + 2);
            if (OK) {
                if (((AdrInt < -16) || (AdrInt > 15)) && !mSymbolQuestionable(Flags)) {
                    WrError(ErrNum_JmpDistTooBig);
                } else {
                    CodeLen     = 1;
                    BAsmCode[0] = 0x80 | ((Conditions[Condition].Code - 0xde) << 5)
                                  | (AdrInt & 0x1f);
                }
            }
        }
        ...
        if (Condition >= ConditionCnt) {
            ...
            if (OK) {
                if (((AdrInt < -128) || (AdrInt > 127)) && !mSymbolQuestionable(Flags)) {
                    ...
                    BAsmCode[CodeLen - 2]
                            = (Condition == -1) ? 0xfc : Lo(Conditions[Condition].Code);
    ...
    if (ChkArgCnt(1, 2)) {
        ...
        if (CondIndex >= ConditionCnt) {
            ...
            case ModImm: {
                ...
                if ((Dist >= -16) && (Dist < 15)
                    && (CondIndex >= ConditionCnt - 2)) /* JRS T/F */
                {
                    CodeLen     = 1;
...
static void AddCond(char const* NName, Word NCode) {
...
static void InitFields(void) {
    ...
    AddReg16("SHLCA", 0xf0e8);
    AddReg16("SHRCA", 0xf1e8);
