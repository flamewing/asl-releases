...
static char *TableName, *BiblioName, *ContentsName, *IndexName,
...
void PrFontDiff(int OldFlags, int NewFlags) {
    ...
    for (z = FontStandard + 1, Mask = 2; z < FontCnt; z++, Mask = Mask << 1) {
        if ((OldFlags ^ NewFlags) & Mask) {
            as_snprintf(
                    erg, sizeof(erg), "<%s%s>", (NewFlags & Mask) ? "" : "/",
                    FontNames[z]);
            DoAddNormal(erg, "");
        }
    }
...
static void DumpTable(void) {
    int TextCnt, RowCnt, rowz, rowz2, rowz3, colz, colptr, ml, l, diff, sumlen, firsttext,
            indent;
    char* ColTag;
            ...
            for (rowz = 0; rowz < RowCnt; rowz++) {
                if ((!pThisTable->LineFlags[rowz]) && (!pThisTable->MultiFlags[rowz])) {
                    l = (!pThisTable->Lines[rowz][colptr])
                                ? 0
                                : strlen(pThisTable->Lines[rowz][colptr]);
                    if (ml < l) {
                        ml = l;
                    }
                }
    ...
    while (rowz < RowCnt) {
        ...
        if (rowz < RowCnt) {
            ...
            if ((rowz2 <= RowCnt - 3) && (pThisTable->LineFlags[rowz2 + 1])
                && (pThisTable->LineFlags[rowz2 + 2])) {
                ColTag = "TH";
            } else {
                ColTag = "TD";
            }
            ...
            colptr = 0;
            for (colz = 0;
                 colz < ((pThisTable->MultiFlags[rowz]) ? firsttext + 1
                                                        : pThisTable->ColumnCount);
                 colz++) {
                if (pThisTable->ColTypes[colz] != ColBar) {
                    /* start a column */
...
static void StartFile(char* Name) {
    ...
    fputs("<HEAD>\n", outfile);
    fprintf(outfile,
            "<META NAME=\"Author\" CONTENT=\"automatically generated by tex2html from "
            "%s\">\n",
            pInFileName);
