...
static char* getobj(char const* pSrc) {
...
int MayRecurse(char const* pFileName) {
...
static void ParseFile(
        char const* pFileName, char const* pParentFileName, StringRecPtr* pFileList) {
    FILE*  pFile;
    int    l;
...
int main(int argc, char** argv) {
    ...
    for (z = 1; z < argc; z++) {
        ...
        /* object files may be in paths different than root - strip path to deduce
         * associated src file */
        ...
        if (ArgsAreObj) {
            ...
            *SrcFileName = '\0';
            strncat(SrcFileName, pRun, sizeof(SrcFileName) - 1);
            if (pObjExtension) {
        ...
        if (!StringListEmpty(FileList)) {
            fprintf(pDestFile, "%s: %s", ArgsAreObj ? argv[z] : getobj(argv[z]),
                    pSrcFileName);
            while (True) {
