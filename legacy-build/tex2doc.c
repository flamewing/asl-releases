...
static char *TableName, *BiblioName, *ContentsName,
...
static char* EnvNames[EnvCount]
        = {"___NONE___",   "document",        "itemize",         "enumerate",
           "description",  "table",           "tabular",         "raggedleft",
           "raggedright",  "center",          "verbatim",        "quote",
           "tabbing",      "thebibliography", "___MARGINPAR___", "___CAPTION___",
           "___HEADING___"};
...
static tNLSCharacterTab const* pCharacterTab;

/*--------------------------------------------------------------------------*/

static void SetSrcDir(char const* pSrcFile) {
    char const* pSep;
...
static char const CHR_ae[3] = HYPHEN_CHR_ae, CHR_oe[3] = HYPHEN_CHR_oe,
                  CHR_ue[3] = HYPHEN_CHR_ue, CHR_AE[3] = HYPHEN_CHR_AE,
                  CHR_OE[3] = HYPHEN_CHR_OE, CHR_UE[3] = HYPHEN_CHR_UE,
...
static int visible_strlen(char const* pStr) {
    int res = 0;
...
static int visible_strnlen(char const* pStr, int MaxLen) {
    int res = 0;
...
static void outs(char const* pStr) {
    while (*pStr) {
        outc(*pStr++);
    }
}
...
static void PutLine(Boolean DoBlock) {
    ...
    if ((Alignment != AlignNone) || (!DoBlock)) {
        ...
        SkipFirst
                = ((CurrEnv == EnvItemize) || (CurrEnv == EnvEnumerate)
                   || (CurrEnv == EnvDescription) || (CurrEnv == EnvBiblio));
...
static void AddLine(char const* Part, char* Sep) {
            ...
            *OutLineBuffer = '\0';
            ...
            if (*search == ' ') {
                for (lastalpha = search + 1; *lastalpha != '\0'; lastalpha++) {
                    if ((as_tolower(*lastalpha) < 'a')
                        || (as_tolower(*lastalpha) > 'z')) {
                        break;
                    }
                }
                if (lastalpha - search > 3) {
                    ...
                    *lastalpha = save;
                    hlen       = -1;
                    for (z = 0; z < hypcnt; z++) {
                        if (visible_strnlen(OutLineBuffer, search - OutLineBuffer)
                                    + hyppos[z] + 1
                            < mlen) {
                            hlen = hyppos[z];
                        }
                    ...
                    if (hlen > 0) {
                        memmove(search + hlen + 2, search + hlen + 1,
                                strlen(search + hlen + 1) + 1);
                        search[hlen + 1] = '-';
                        search += hlen + 2;
                    }
                    if (hypcnt > 0) {
...
static void AddSideMargin(char const* Part, char* Sep) {
    if (strlen(Sep) > 1) {
        Sep[1] = '\0';
    }
    if (*Sep != '\0') {
...
static void NextTableColumn(void) {
    ...
    if ((pThisTable->MultiFlags[CurrRow]) || (CurrCol >= pThisTable->TColumnCount)) {
...
static void AddTableEntry(char const* Part, char* Sep) {
...
static void DumpTable(void) {
        ...
        if (pThisTable->ColTypes[colz] == ColBar) {
            ...
            for (rowz = 0; rowz < RowCnt; rowz++) {
                if ((!pThisTable->LineFlags[rowz]) && (!pThisTable->MultiFlags[rowz])) {
                    l = (!pThisTable->Lines[rowz][colptr])
                                ? 0
                                : visible_strlen(pThisTable->Lines[rowz][colptr]);
                    if (ml < l) {
                        ml = l;
                    }
                }
    ...
    for (colz = sumlen = 0; colz < pThisTable->ColumnCount;
         sumlen += pThisTable->ColLens[colz++])
        ;
    ...
    for (rowz = 0; rowz < RowCnt; rowz++) {
        ...
        if (pThisTable->MultiFlags[rowz]) {
            ...
            for (colz = 0; colz < pThisTable->ColumnCount; colz++) {
                if (!colz) {
                    DoPrnt(pThisTable->Lines[rowz][colz], pThisTable->ColTypes[firsttext],
                           l);
            ...
            for (colz = colptr = 0; colz < pThisTable->ColumnCount; colz++) {
                ...
                } else if (pThisTable->ColTypes[colz] == ColBar) {
                    outc('|');
                } else {
                    DoPrnt(pThisTable->Lines[rowz][colptr], pThisTable->ColTypes[colz],
                           pThisTable->ColLens[colz]);
...
static void DoAddNormal(char const* Part, char* Sep) {
    switch (CurrEnv) {
    case EnvMarginPar:
...
static void TeXFlushLine(Word Index) {
    ...
    if (CurrEnv == EnvTabular) {
        for (CurrCol++; CurrCol < pThisTable->TColumnCount;
             pThisTable->Lines[CurrRow][CurrCol++] = as_strdup(""))
            ;
        CurrRow++;
        if (CurrRow == MAXROWS) {
...
static void TeXEndEnv(Word Index) {
    ...
    if (CurrEnv != NEnv) {
        ...
        as_snprintf(
                Str, sizeof(Str), "begin (%s) and end (%s) of environment do not match",
...
static void TeXMultiColumn(Word Index) {
    ...
    if (CurrEnv != EnvTabular) {
        error("\\multicolumn outside of a table");
    }
    if (CurrCol != 0) {
...
static void TeXDoPot(void) {
    ...
    if (*Token == '2') {
        char        Buf[3];
        char const* pRepl = CharTab_GetNULTermString(pCharacterTab, eCH_e2, Buf);
