...
static int ImmPtr(char const* pArg) {
    return !!(*pArg == '#');
}
...
static void DecodeAri(Word Index) {
    tAriOrder const* pOrder = AriOrders + Index;

    if (ChkArgCnt(2, 2) && (ChkExactCPUMask(pOrder->CPUMask, CPU5054) >= 0)) {
...
static void DecodeADJUST(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && (ChkExactCPUMask(M_5054 | M_5055, CPU5054) >= 0)) {
        if (DecodeAdr(&ArgStr[2], MModAP)) {
            Boolean OK;
...
static void DecodeAP(Word Index) {
    tAPOrder const* pOrder = APOrders + Index;

    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(pOrder->CPUMask, CPU5054) >= 0)) {
...
static void DecodeFixed(Word Index) {
    ...
    if (ChkArgCnt(0, 0) && (ChkExactCPUMask(pOrder->CPUMask, CPU5054) >= 0)) {
...
static void DecodeAPAX(Word Index) {
    tAPAXOrder const* pOrder = APAXOrders + Index;
...
static void DecodeCALL(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(M_6051 | M_6052, CPU5054) >= 0)) {
        Boolean OK;

        WAsmCode[0] = EvalStrIntExpression(
                &ArgStr[1], (MomCPU == CPU6052) ? UInt11 : UInt10, &OK);
        if (OK) {
...
static void DecodeMSA(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(M_6051, CPU5054) >= 0)) {
        Boolean OK;
...
static void DecodeRel(Word Index) {
    ...
    if (ChkArgCnt(1, 1)
        ...
        tSymbolFlags Flags;
        Integer      Distance
                = EvalStrIntExpressionWithFlags(&ArgStr[1], CodeIntType, &OK, &Flags)
                  - (EProgCounter() + 1);
        Integer MinDist = AllowMinus ? -32 : 0, MaxDist = AllowPlus ? 31 : -1;
...
static void DecodeCtrl(Word Index) {
    tCtrlOrder const* pOrder = CtrlOrders + Index;
    ...
    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(pOrder->CPUMask, CPU5054) >= 0)) {
        ...
        case ModImm:
            ...
            break;
        case ModAP:
            if (!pOrder->AllowAP && (AdrVal & 0x100)) {
                WrError(ErrNum_InvAddrMode);
            } else {
                WAsmCode[0] = pOrder->APCode | AdrVal;
                CodeLen     = 1;
...
static void DecodeDSP(Word Index) {
    tDSPOrder const* pOrder = DSPOrders + Index;
    ...
    if (ChkArgCnt(2, 2) && (ChkExactCPUMask(pOrder->CPUMask, CPU5054) >= 0)
        ...
        Digit = EvalStrIntExpressionOffs(
                &ArgStr[1], ImmPtr(ArgStr[1].str.p_str), UInt4, &OK);
        if (OK) {
...
static void DecodeINTENDSAB(Word Code) {
    if (ChkArgCnt(0, 0) && (ChkExactCPUMask(M_5054 | M_5055 | M_6051, CPU5054) >= 0)) {
...
static void DecodeONOFF(Word Index) {
    tONOFFOrder const* pOrder = ONOFFOrders + Index;
    Boolean            IsON;
...
static void DecodeImm(Word Index) {
    tImmOrder const* pOrder = ImmOrders + Index;

    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(pOrder->CPUMask, CPU5054) >= 0)) {
        Boolean OK;
        Word    Freq = EvalStrIntExpressionOffs(
                &ArgStr[1], ImmPtr(ArgStr[1].str.p_str), UInt4, &OK);
...
static void DecodeINP(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && (ChkExactCPUMask(M_5056, CPU5054) >= 0)) {
        Boolean OK;
...
static void DecodeIN(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && (ChkExactCPUMask(M_6052, CPU5054) >= 0)) {
        Boolean OK;
        Word    Port = EvalStrIntExpression(&ArgStr[1], UInt4, &OK);
...
static void DecodeOUT(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && (ChkExactCPUMask(M_5056 | M_6052, CPU5054) >= 0)) {
        Boolean OK;
        Word    Port = EvalStrIntExpression(
                &ArgStr[2], (MomCPU == CPU6052) ? UInt5 : UInt4, &OK);
        ...
        if (OK) {
            ...
            case ModAP:
                WAsmCode[0]
                        = ((MomCPU == CPU6052) ? 0x0800 : 0x3600) | AdrVal | (Port << 4);
                CodeLen = 1;
                break;
            case ModImm:
...
static void DecodeMem(Word Index) {
    tMemOrder const* pOrder = MemOrders + Index;

    if (ChkArgCnt(0, 3) && (ChkExactCPUMask(pOrder->CPUMask, CPU5054) >= 0)) {
        WAsmCode[0] = pOrder->Code;
        if (ArgCnt < 1) {
            CodeLen = 1;
        } else {
...
static void AddAri(char const* pName, Word ACCCode, Word ImmCode, Word CPUMask) {
...
static void AddAP(char const* pName, Word Code, Word CPUMask) {
    if (InstrZ >= APOrderCnt) {
        exit(255);
    }
...
static void AddImm(char const* pName, Word Code, Word CPUMask) {
...
static void AddAPAX(char const* pName, Word APCode, Word AXCode, Word CPUMask) {
    if (InstrZ >= APAXOrderCnt) {
        exit(255);
    }
...
static void AddFixed(char const* pName, Word Code, Word CPUMask) {
...
static void AddRel(char const* pName, Word Code, Word PlusCPUMask, Word MinusCPUMask) {
...
static void AddCtrl(
        char const* pName, Word APCode, Word ImmCode, Boolean AllowAP, Word CPUMask) {
    if (InstrZ >= CtrlOrderCnt) {
        exit(255);
    }
...
static void AddDSP(char const* pName, Word Code, Word CPUMask) {
    if (InstrZ >= DSPOrderCnt) {
        exit(255);
    }
...
static void AddONOFFInstr(char const* pName, Word Code, Word Shift, Word CPUMask) {
    if (InstrZ >= ONOFFOrderCnt) {
        exit(255);
    }
...
static void AddMem(char const* pName, Word Code, Word CPUMask) {
...
static void InitFields(void) {
    ...
    if (CheckCPU(M_6052)) {
        ...
        AddRel("BCS", (MomCPU == CPU5056) ? 0x0620 : 0x0600,
               M_5054 | M_5055 | M_5056 | M_6051, M_6051);
        AddRel("BLT",
               (MomCPU == CPU6051) ? 0x06e0 : (MomCPU == CPU5056 ? 0x0620 : 0x0600),
               M_5054 | M_5055 | M_5056 | M_6051, M_6051);
        AddRel("BCC", (MomCPU == CPU5056) ? 0x06a0 : 0x0680,
               M_5054 | M_5055 | M_5056 | M_6051, M_6051);
        AddRel("BGE",
               (MomCPU == CPU6051) ? 0x0660 : (MomCPU == CPU5056 ? 0x06a0 : 0x0680),
               M_5054 | M_5055 | M_5056 | M_6051, M_6051);
    ...
    AddFixed(
            "HALT", (MomCPU == CPU6052) ? 0x0e10 : 0x0400,
            M_5054 | M_5055 | M_5056 | M_6051 | M_6052);
