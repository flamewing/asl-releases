...
typedef struct {
    ...
    void (*TimeString)(
            Word Hour, Word Minute, Word Second, Word Sec100, char* Dest,
            size_t DestSize);
#if (defined OS2_NLS)
    DateFormat  DateFmt; /* Datumsreihenfolge */
    char const* DateSep; /* Trennzeichen zwischen Datumskomponenten */
    TimeFormat  TimeFmt; /* 12/24-Stundenanzeige */
    char const* TimeSep; /* Trennzeichen zwischen Zeitkomponenten */
#elif defined LOCALE_NLS
    char const* DateFmtStr;
    char const* TimeFmtStr;
#endif
    char const* Currency;     /* Waehrungsname */
    CurrFormat  CurrFmt;      /* Anzeigeformat Waehrung */
    Byte        CurrDecimals; /* Nachkommastellen Waehrungsbetraege */
    char const* ThouSep;      /* Trennzeichen fuer Tausenderbloecke */
    char const* DecSep;       /* Trennzeichen fuer Nachkommastellen */
    char const* DataSep;      /* ??? */
    Boolean     Initialized;
} NLS_CountryInfo;
...
static char const* CodepageNames[eCodepageCnt] = {
        "ascii", "iso8859-1", "iso8859-15", "koi8-r", "437",
        "850",   "866",       "1251",       "1252",   "utf-8",
};
...
static void DOS_OS2_DateString(
        Word Year, Word Month, Word Day, char* Dest, size_t DestSize) {
    switch (NLSInfo.DateFmt) {
    case DateFormatMTY:
        as_snprintf(
                Dest, DestSize, "%d%s%d%s%d", Month, NLSInfo.DateSep, Day,
                NLSInfo.DateSep, Year);
        break;
    case DateFormatTMY:
        as_snprintf(
                Dest, DestSize, "%d%s%d%s%d", Day, NLSInfo.DateSep, Month,
                NLSInfo.DateSep, Year);
        break;
    case DateFormatYMT:
        as_snprintf(
                Dest, DestSize, "%d%s%d%s%d", Year, NLSInfo.DateSep, Month,
                NLSInfo.DateSep, Day);
        break;
    }
}

static void DOS_OS2_TimeString(
    ...
    as_snprintf(
            Dest, DestSize, "%d%s%02d%s%02d", Hour, NLSInfo.TimeSep, Minute,
            NLSInfo.TimeSep, Second);
    if (Sec100 < 100) {
...
static void QueryInfo(void) {
    ...
    case 437:
        NLSInfo.Codepage = eCodepage437;
        break;
    case 850:
        NLSInfo.Codepage = eCodepage850;
        break;
    case 866:
        NLSInfo.Codepage = eCodepage866;
        break;
    case 1251:
        NLSInfo.Codepage = eCodepage1251;
        break;
    case 1252:
        NLSInfo.Codepage = eCodepage1252;
        break;
    case 20866:
...
static void Default_DateString(
        Word Year, Word Month, Word Day, char* Dest, size_t DestSize) {
    as_snprintf(Dest, DestSize, "%u/%u/%u", Month, Day, Year);
}

static void Default_TimeString(
        Word Hour, Word Minute, Word Second, Word Sec100, char* Dest, size_t DestSize) {
    (void)Sec100;
...
static void QueryInfo(void) {
    ...
    if (lc->p_cs_precedes) {
        ...
        NLSInfo.CurrFmt
                = lc->p_sep_by_space ? CurrFormatPostBlank : CurrFormatPostNoBlank;
    }
        ...
        if (pCodepage) {
            ...
            if (!as_strcasecmp(pCodepage, "ISO-8859-1")) {
                ...
                NLSInfo.Codepage = eCodepageISO8859_15;
                    ...
                    !as_strcasecmp(pCodepage, "UTF-8")
                    || !as_strcasecmp(pCodepage, "UTF8")) {
    ...
    if (argc) {
        ...
        while (z < *argc) {
            if ((z < *argc - 1) && !as_strcasecmp(argv[z], "-codepage")) {
                for (OverrideCodepage = (tCodepage)0; OverrideCodepage < eCodepageCnt;
                     OverrideCodepage++) {
...
void NLS_CurrTimeString(Boolean Use100, char* Dest, size_t DestSize) {
    ...
    trec = localtime(&timep);
    NLS_TimeString(
            trec->tm_hour, trec->tm_min, trec->tm_sec, Use100 ? 0 : 100, Dest, DestSize);
}
...
void NLS_CurrencyString(double inp, char* erg, size_t DestSize) {
    ...
    default:
        *p = '\0';
        as_snprintf(
                erg, DestSize, "%s%s%s", s, NLSInfo.Currency, p + strlen(NLSInfo.DecSep));
    }
}
...
void NLS_LowString(char* pStr) {
    ...
    while (*pStr) {
        ...
        Unicode = (Unicode < 256) ? (unsigned int)(LowCaseTable[Unicode] & 0xff)
                                  : Unicode;
        if (eCodepageUTF8 == NLSInfo.Codepage) {
            UnicodeToUTF8(&pStr, Unicode);
        } else {
...
int NLS_StrCmp(char const* s1, char const* s2) {
    while (CollateTable[((unsigned int)*s1) & 0xff]
           == CollateTable[((unsigned int)*s2) & 0xff]) {
        if ((!*s1) && (!*s2)) {
            return 0;
        }
        s1++;
    ...
    return ((int)CollateTable[((unsigned int)*s1) & 0xff]
            - CollateTable[((unsigned int)*s2) & 0xff]);
}
