...
static ShortInt DecodeReg16(char* pAsc) {
    ...
    if (l == 2) {
        static char const Reg16Names[4][3] = {"AX", "BC", "DE", "HL"};
        int               z;
...
static ShortInt DecodeAdr(tStrComp const* pArg, Word Mask, tAdrResult* pResult) {
    ...
    if ((ArgLen >= 2) && (Arg.str.p_str[ArgLen - 1] == ']')) {
        ...
        if (pStart == Arg.str.p_str) {
            static char const Modes[][5] = {"DE+",  "HL+",  "DE-",  "HL-",  "DE",  "HL",
    ...
    LongOp = EvalStrIntExpressionOffsWithFlags(
            &Arg, ForceLong, UInt20, &OK, &pResult->ValSymFlags);
    if (OK) {
        if (!mFirstPassUnknown(pResult->ValSymFlags)) {
            LongWord CompBank = pResult->AltBank ? Reg_P6 : Reg_PM6;
            if (CompBank != (LongOp >> 16)) {
...
static Boolean ChkAcc(tStrComp const* pArg) {
    tAdrResult Result;
...
static Boolean ChkMem1(tAdrResult const* pResult) {
...
static void DecodeMOV(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1],
                MModReg8 | MModShort | MModSFR | MModMem | MModAbs | MModPSW | MModSTBC,
                &DestResult)) {
        case ModReg8: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2],
                    MModImm | MModReg8
                            | (DestResult.Val == AccReg8 ? (MModShort | MModSFR | MModAbs
                                                            | MModMem | MModPSW)
                                                         : 0),
                    &SrcResult)) {
            case ModImm:
...
static void DecodeXCH(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1], MModReg8 | MModShort | MModSFR | MModMem, &DestResult)) {
        case ModReg8: {
            tAdrResult SrcResult;
            ...
            switch (DecodeAdr(
                    &ArgStr[2],
                    MModReg8
                            | ((DestResult.Val == AccReg8)
                                       ? (MModShort | MModSFR | MModMem)
                                       : 0),
                    &SrcResult)) {
            case ModReg8:
                if (DestResult.Val == AccReg8) {
...
static void DecodeMOVW(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1], MModReg16 | MModSP | MModShort | MModSFR | MModMem,
                &DestResult)) {
        case ModReg16: {
            tAdrResult SrcResult;
            ...
            switch (DecodeAdr(
                    &ArgStr[2],
                    MModReg16 | MModImm
                            | ((DestResult.Val == AccReg16)
                                       ? (MModSP | MModShort | MModSFR | MModMem)
                                       : 0),
                    &SrcResult)) {
            case ModReg16:
                *pCode++ = 0x24;
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg8: {
            ...
            switch (DecodeAdr(
                    ...
                    MModReg8
                            | ((DestResult.Val == AccReg8)
                                       ? (MModImm | MModShort | MModSFR | MModMem)
                                       : 0),
                    &SrcResult)) {
            case ModReg8:
                *pCode++ = 0x88 | Index;
...
static void DecodeALU16(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (ChkAcc(&ArgStr[1])) {
            ...
            switch (DecodeAdr(
                    &ArgStr[2], MModImm | MModReg16 | MModShort | MModSFR, &Result)) {
            case ModImm:
                *pCode++ = 0x2c | Index;
                *pCode++ = *Result.Vals;
                *pCode++ = 1 [Result.Vals];
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        case ModAbs:
            if (Rel) {
                ...
                Addr -= EProgCounter() + 2;
                if (!mSymbolQuestionable(Result.ValSymFlags)
                    && ((Addr < -128) || (Addr > 127))) {
...
static void DecodeBrBit(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && DecodeBitAdr(&ArgStr[1], &Bit)) {
        tSymbolFlags Flags;
...
static void SwitchTo_78K2(void) {
    static ASSUMERec const ASSUME78K2s[] = {
    ...
    MakeCode   = MakeCode_78K2;
    IsDef      = IsDef_78K2;
    SwitchFrom = SwitchFrom_78K2;
    InitFields();
}
