...
static char const WrapFlagName[] = "WRAPMODE";
...
static void DissectBit_AVR(char* pDest, size_t DestSize, LargeWord Inp) {
    ...
    as_snprintf(
            pDest, DestSize, "0x%0*x(%c).%d", (BitSpec & BitFlag_IO) ? 2 : 3,
            (unsigned)((BitSpec >> 3) & 0xffff),
            (BitSpec & BitFlag_Data)
                    ? SegShorts[SegData]
...
static Boolean DecodeRegCore(char const* pArg, Word* pResult) {
    ...
    *pResult = ConstLongInt(pArg + 1, &OK, 10);
    return (OK && ((*pResult >= 16) || (pCurrCPUProps->Core != eCoreMinTiny))
            && (*pResult < 32));
}
...
static Boolean DecodeReg(tStrComp const* pArg, Word* pResult) {
    tRegDescr      RegDescr;
...
static Boolean DecodeMem(char* Asc, Word* Erg) {
    if (as_strcasecmp(Asc, "X") == 0) {
        *Erg = 0x1c;
    } else if (as_strcasecmp(Asc, "X+") == 0) {
        *Erg = 0x1d;
    } else if (as_strcasecmp(Asc, "-X") == 0) {
        *Erg = 0x1e;
    } else if (as_strcasecmp(Asc, "Y") == 0) {
        *Erg = 0x08;
    } else if (as_strcasecmp(Asc, "Y+") == 0) {
        *Erg = 0x19;
    } else if (as_strcasecmp(Asc, "-Y") == 0) {
        *Erg = 0x1a;
    } else if (as_strcasecmp(Asc, "Z") == 0) {
        *Erg = 0x00;
    } else if (as_strcasecmp(Asc, "Z+") == 0) {
        *Erg = 0x11;
    } else if (as_strcasecmp(Asc, "-Z") == 0) {
...
static Boolean DecodeBitArg2(
        tStrComp const* pRegArg, tStrComp const* pBitArg, LongWord* pResult) {
    tEvalResult EvalResult;
    LongWord    Addr;
    ...
    *pResult = EvalStrIntExpressionWithResult(pBitArg, UInt3, &EvalResult);
    ...
    if (EvalResult.AddrSpaceMask & (1 << SegIO)) {
        if (!mFirstPassUnknown(EvalResult.Flags)
            && !ChkRange(Addr, 0, IOAreaStdSize - 1)) {
            return False;
        }
        *pResult |= BitFlag_IO | (Addr & 0x3f) << 3;
        ...
        if (!mFirstPassUnknown(EvalResult.Flags)
            && !ChkRange(Addr, 0, SegLimits[SegData])) {
            return False;
        }
        *pResult |= ((EvalResult.AddrSpaceMask & (1 << SegData)) ? BitFlag_Data : 0)
                    | (Addr & 0x1ff) << 3;
        return True;
    }
}
...
static LongWord const AllRegMask = 0xfffffffful, UpperHalfRegMask = 0xffff0000ul,
                      Reg16_23Mask = 0x00ff0000ul, EvenRegMask = 0x55555555ul,
...
static void DecodeReg1(Word Index) {
    FixedOrder const* pOrder = Reg1Orders + Index;
    Word              Reg;

    if (ChkArgCnt(1, 1) && ChkCoreMask(pOrder->CoreMask)
...
static void DecodeReg2(Word Index) {
    FixedOrder const* pOrder = Reg2Orders + Index;
    Word              Reg1, Reg2;

    if (ChkArgCnt(2, 2) && ChkCoreMask(pOrder->CoreMask)
        && DecodeArgReg(1, &Reg1, AllRegMask) && DecodeArgReg(2, &Reg2, AllRegMask)) {
...
static void DecodeImm(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeArgReg(1, &Reg, UpperHalfRegMask)) {
        ...
        if (OK) {
            AppendCode(
                    Code | ((Const & 0xf0) << 4) | (Const & 0x0f) | ((Reg & 0x0f) << 4));
        }
    }
}
...
static void DecodeLDST(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (!DecodeArgReg(RegI, &Reg, AllRegMask))
            ;
        else if (!DecodeMem(ArgStr[MemI].str.p_str, &Mem)) {
            WrError(ErrNum_InvAddrMode);
        } else if ((pCurrCPUProps->Core == eCore90S1200) && (Mem != 0)) {
            WrError(ErrNum_AddrMustBeAligned);
        } else {
            AppendCode(0x8000 | Index | (Reg << 4) | (Mem & 0x0f) | ((Mem & 0x10) << 8));
            if (((Mem >= 0x1d) && (Mem <= 0x1e) && (Reg >= 26)
                 && (Reg <= 27)) /* X+/-X with X */
                || ((Mem >= 0x19) && (Mem <= 0x1a) && (Reg >= 28)
                    && (Reg <= 29)) /* Y+/-Y with Y */
                || ((Mem >= 0x11) && (Mem <= 0x12) && (Reg >= 30)
                    && (Reg <= 31))) { /* Z+/-Z with Z */
    ...
    if (ChkArgCnt(2, 2) && ChkMinCore(eCoreClassic)) {
        ...
        if (!OK) {
            ...
            *ArgStr[MemI].str.p_str = RegChar;
            if (OK) {
                AppendCode(
                        0x8000 | Index | (Reg << 4) | (Disp & 7) | ((Disp & 0x18) << 7)
                        | ((Disp & 0x20) << 8));
            }
        }
    }
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (DecodeArgReg(RegI, &Reg, AllRegMask)) {
            ...
            if (EvalResult.OK) {
                ...
                AppendCode(
                        0xb000 | Index | (Reg << 4) | (Mem & 0x0f) | ((Mem & 0xf0) << 5));
            }
        }
    }
}
...
static void DecodeLDSSTS(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkCoreMask(MinCoreMask(eCoreClassic) | (1 << eCoreMinTiny))) {
        RegI = Index ? 2 : 1; /* STS */
        MemI = 3 - RegI;
        if (DecodeArgReg(RegI, &Reg, AllRegMask)) {
            tEvalResult EvalResult;
            Word        Address
                    = EvalStrIntExpressionWithResult(&ArgStr[MemI], UInt16, &EvalResult);
            if (EvalResult.OK) {
                ChkSpace(SegData, EvalResult.AddrSpaceMask);
...
static void DecodeCBR(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && DecodeArgReg(1, &Reg, UpperHalfRegMask)) {
        ...
        if (OK) {
            AppendCode(
                    0x7000 | ((Mask & 0xf0) << 4) | (Mask & 0x0f) | ((Reg & 0x0f) << 4));
        }
    }
}
...
static void DecodePBit(Word Code) {
    ...
    if (DecodeBitArg(1, ArgCnt, &BitSpec)) {
        Word Bit = BitSpec & 7, Adr = (BitSpec >> 3) & 0xffff;

        if (BitSpec & BitFlag_Data) {
...
static void DecodeJMPCALL(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCore(eCoreTiny16K)) {
        AdrInt = GetWordCodeAddress(&ArgStr[1], &EvalResult);
        if (EvalResult.OK) {
            AppendCode(
                    0x940c | Index | ((AdrInt & 0x3e0000) >> 13)
                    | ((AdrInt & 0x10000) >> 16));
...
static void DecodeMULS(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCore(eCoreMega)
...
static void DecodeMegaMUL(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCore(eCoreMega) && DecodeArgReg(1, &Reg1, Reg16_23Mask)
...
static void DecodeMOVW(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCore(eCoreTiny) && DecodeArgReg(1, &Reg1, EvenRegMask)
...
static void DecodeLPM(Word Index) {
    ...
    if (!ArgCnt) {
        ...
        if (!ChkMinCore(eCoreTiny))
            ...
            ;
        else if (!DecodeMem(ArgStr[2].str.p_str, &Adr)) {
            WrError(ErrNum_InvAddrMode);
        } else if ((Adr != 0x00) && (Adr != 0x11)) {
            WrError(ErrNum_InvAddrMode);
        } else {
            if (((Reg == 30) || (Reg == 31)) && (Adr == 0x11)) {
                WrError(ErrNum_Unpredictable);
            }
...
static void AddFixed(char const* NName, Word NMin, Word NCode) {
...
static void AddReg1(char const* NName, Word NMin, Word NCode) {
    if (InstrZ >= Reg1OrderCnt) {
        exit(255);
    }
...
static void AddReg2(char const* NName, Word NMin, Word NCode) {
...
static void AddReg3(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeReg3);
}

static void AddImm(char const* NName, Word NCode) {
...
static void AddPBit(char const* NName, Word NCode) {
...
static void InitFields(void) {
    ...
    AddFixed("RET", MinCoreMask(eCoreMinTiny), 0x9508);
    AddFixed("RETI", MinCoreMask(eCoreMinTiny), 0x9518);
    AddFixed("SEC", MinCoreMask(eCoreMinTiny), 0x9408);
    AddFixed("CLC", MinCoreMask(eCoreMinTiny), 0x9488);
    AddFixed("SEN", MinCoreMask(eCoreMinTiny), 0x9428);
    AddFixed("CLN", MinCoreMask(eCoreMinTiny), 0x94a8);
    AddFixed("SEZ", MinCoreMask(eCoreMinTiny), 0x9418);
    AddFixed("CLZ", MinCoreMask(eCoreMinTiny), 0x9498);
    AddFixed("SEI", MinCoreMask(eCoreMinTiny), 0x9478);
    AddFixed("CLI", MinCoreMask(eCoreMinTiny), 0x94f8);
    AddFixed("SES", MinCoreMask(eCoreMinTiny), 0x9448);
    AddFixed("CLS", MinCoreMask(eCoreMinTiny), 0x94c8);
    AddFixed("SEV", MinCoreMask(eCoreMinTiny), 0x9438);
    AddFixed("CLV", MinCoreMask(eCoreMinTiny), 0x94b8);
    AddFixed("SET", MinCoreMask(eCoreMinTiny), 0x9468);
    AddFixed("CLT", MinCoreMask(eCoreMinTiny), 0x94e8);
    AddFixed("SEH", MinCoreMask(eCoreMinTiny), 0x9458);
    AddFixed("CLH", MinCoreMask(eCoreMinTiny), 0x94d8);
    AddFixed("NOP", MinCoreMask(eCoreMinTiny), 0x0000);
    AddFixed("SLEEP", MinCoreMask(eCoreMinTiny), 0x9588);
    AddFixed("WDR", MinCoreMask(eCoreMinTiny), 0x95a8);
    AddFixed("EIJMP", MinCoreMask(eCoreMega), 0x9419);
    AddFixed("EICALL", MinCoreMask(eCoreMega), 0x9519);
    AddFixed("SPM", MinCoreMask(eCoreTiny), 0x95e8);
    AddFixed("BREAK", MinCoreMask(eCoreTiny) | (1 << eCoreMinTiny), 0x9598);

    Reg1Orders = (FixedOrder*)malloc(sizeof(*Reg1Orders) * Reg1OrderCnt);
    InstrZ     = 0;
    AddReg1("COM", MinCoreMask(eCoreMinTiny), 0x9400);
    AddReg1("NEG", MinCoreMask(eCoreMinTiny), 0x9401);
    AddReg1("INC", MinCoreMask(eCoreMinTiny), 0x9403);
    AddReg1("DEC", MinCoreMask(eCoreMinTiny), 0x940a);
    AddReg1("PUSH", MinCoreMask(eCoreClassic) | (1 << eCoreMinTiny), 0x920f);
    AddReg1("POP", MinCoreMask(eCoreClassic) | (1 << eCoreMinTiny), 0x900f);
    AddReg1("LSR", MinCoreMask(eCoreMinTiny), 0x9406);
    AddReg1("ROR", MinCoreMask(eCoreMinTiny), 0x9407);
    AddReg1("ASR", MinCoreMask(eCoreMinTiny), 0x9405);
    AddReg1("SWAP", MinCoreMask(eCoreMinTiny), 0x9402);

    Reg2Orders = (FixedOrder*)malloc(sizeof(*Reg2Orders) * Reg2OrderCnt);
    InstrZ     = 0;
    AddReg2("ADD", MinCoreMask(eCoreMinTiny), 0x0c00);
    AddReg2("ADC", MinCoreMask(eCoreMinTiny), 0x1c00);
    AddReg2("SUB", MinCoreMask(eCoreMinTiny), 0x1800);
    AddReg2("SBC", MinCoreMask(eCoreMinTiny), 0x0800);
    AddReg2("AND", MinCoreMask(eCoreMinTiny), 0x2000);
    AddReg2("OR", MinCoreMask(eCoreMinTiny), 0x2800);
    AddReg2("EOR", MinCoreMask(eCoreMinTiny), 0x2400);
    AddReg2("CPSE", MinCoreMask(eCoreMinTiny), 0x1000);
    AddReg2("CP", MinCoreMask(eCoreMinTiny), 0x1400);
    AddReg2("CPC", MinCoreMask(eCoreMinTiny), 0x0400);
    AddReg2("MOV", MinCoreMask(eCoreMinTiny), 0x2c00);
    AddReg2("MUL", MinCoreMask(eCoreMega), 0x9c00);
...
static Boolean IsDef_AVR(void) {
    return (Memo("PORT") || Memo("REG") || Memo("SFR") || Memo("BIT"));
}
...
static void SwitchTo_AVR(void* pUser) {
    ...
    Grans[SegCode] = ListGrans[SegCode] = 1 << CodeSegSize;
    SegInits[SegCode]                   = 0;
    Grans[SegData]                      = 1;
    ListGrans[SegData]                  = 1;
    SegInits[SegData]                   = 32;
    ...
    SegLimits[SegData] = (pCurrCPUProps->RegistersMapped ? RegBankSize : 0)
                         + pCurrCPUProps->IOAreaSize + pCurrCPUProps->RAMSize - 1;
...
static tCPUProps const CPUProps[] = {
        ...
        {     "AT90S2333",  0x03f, 0x0080, 0x007f,        IOAreaStdSize,  True,
         eCoreClassic                                                                       }, /* == ATtiny22 */
        ...
        {    "AT43USB355",  0x2ff, 0x0400, 0x0000, 0x2000 - RegBankSize,  True,
         eCoreClassic                                                                       }, /* allow USB registers @ 0x1fxx */
