...
static Boolean DecodeReg(char const* pAsc, Byte* pRes) {
    Boolean Result;

    Result
            = ((strlen(pAsc) == 2) && (as_toupper(pAsc[0]) == 'R') && (pAsc[1] >= '0')
               && (pAsc[1] <= '3'));
    if (Result) {
        *pRes = pAsc[1] - '0';
    }
...
static Boolean DecodeCondition(char const* pAsc, Byte* pRes) {
    Boolean Result = TRUE;
...
static void DecodeRegAbs(Word Index) {
    ...
    if (!ChkArgCnt(2, 4))
        ;
    else if (!DecodeReg(ArgStr[1].str.p_str, &DReg)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else {
        ...
        if (OK) {
            ...
            if (ArgCnt == 2) {
                ...
                if (!DecodeReg(ArgStr[3].str.p_str, &IReg)) {
...
static void DecodeRegRel(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        BAsmCode[0] = Index | Reg;
        IndFlag     = *ArgStr[2].str.p_str == '*';
        Dist = EvalStrIntExpressionOffsWithFlags(&ArgStr[2], IndFlag, UInt13, &OK, &Flags)
               - (EProgCounter() + 2);
        if (OK) {
            if (((Dist < -64) || (Dist > 63)) && !mSymbolQuestionable(Flags)) {
...
static void DecodeBrAbs(Word Index) {
    ...
    if (!ChkArgCnt(1, 2))
        ;
    else if ((ArgCnt == 2) && (!DecodeReg(ArgStr[2].str.p_str, &Reg))) {
...
static void AddOneReg(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeOneReg);
}

static void AddImm(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeImm);
}

static void AddRegImm(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeRegImm);
}

static void AddRegAbs(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeRegAbs);
}

static void AddRegRel(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeRegRel);
}

static void AddCondAbs(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeCondAbs);
}

static void AddCondRel(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeCondRel);
}

static void AddRegAbs2(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeRegAbs2);
}

static void AddBrAbs(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeBrAbs);
}

static void AddCond(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeCond);
}

static void AddZero(char const* pName, Word Code) {
...
static void SwitchTo_2650(void) {
    ...
    MakeCode = MakeCode_2650;
    IsDef    = IsDef_2650;

    SwitchFrom = SwitchFrom_2650;
    InitFields();
}
