...
static void DumpDebugInfo_MAP(void) {
    ...
    while (Run) {
        ...
        errno = 0;
        as_snprintf(Tmp, sizeof(Tmp), "%" PRId32, Run->Contents.LineNum);
...
static void DumpDebugInfo_Atmel(void) {
    static char const* OBJString = "AVR Object File";
    PLineInfoList      Run;
    ...
    FNamePos = 0;
    RecPos   = 0;
    if (!Write4(OBJFile, &FNamePos)) {
        ChkIO(ErrNum_FileWriteError);
    }
    if (!Write4(OBJFile, &RecPos)) {
        ChkIO(ErrNum_FileWriteError);
    }
    TByte = 9;
    if (fwrite(&TByte, 1, 1, OBJFile) != 1) {
        ChkIO(ErrNum_FileWriteError);
    }
    NameCnt = GetFileCount() - 1;
    if (fwrite(&NameCnt, 1, 1, OBJFile) != 1) {
        ChkIO(ErrNum_FileWriteError);
    }
    if ((int)fwrite(OBJString, 1, strlen(OBJString) + 1, OBJFile)
        != (int)strlen(OBJString) + 1) {
        ChkIO(ErrNum_FileWriteError);
    }
    ...
    for (Run = LineInfoRoot; Run; Run = Run->Next) {
        if (Run->Contents.Space == SegCode) {
            ...
            DSwapBigEndianToHost(&LTurn, 4);
            if (fwrite(((Byte*)&LTurn) + 1, 1, 3, OBJFile) != 3) {
                ChkIO(ErrNum_FileWriteError);
            }
            WTurn = Run->Contents.Code;
            WSwapBigEndianToHost(&WTurn, 2);
            if (fwrite(&WTurn, 1, 2, OBJFile) != 2) {
                ChkIO(ErrNum_FileWriteError);
            }
            TNum = Run->Contents.FileName - 1;
            if (fwrite(&TNum, 1, 1, OBJFile) != 1) {
                ChkIO(ErrNum_FileWriteError);
            }
    ...
    for (z = 1; z <= NameCnt; z++) {
        ...
        if ((int)fwrite(FName, 1, strlen(FName) + 1, OBJFile) != (int)strlen(FName) + 1) {
            ChkIO(ErrNum_FileWriteError);
        }
    }
    TByte = 0;
    ...
    rewind(OBJFile);
    DSwapBigEndianToHost(&FNamePos, 4);
    if (fwrite(&FNamePos, 1, 4, OBJFile) != 4) {
        ChkIO(ErrNum_FileWriteError);
    }
    DSwapBigEndianToHost(&RecPos, 4);
    if (fwrite(&RecPos, 1, 4, OBJFile) != 4) {
        ChkIO(ErrNum_FileWriteError);
    }
...
static void DumpDebugInfo_NOICE(void) {
    ...
    for (ActFile = 0; ActFile < GetFileCount(); ActFile++) {
        ...
        while (Run) {
            if ((Run->Contents.Space == SegCode) && (Run->Contents.FileName == ActFile)) {
                if (!HadLines) {
                    ...
                    as_snprintf(Tmp1, sizeof(Tmp1), "%" PRIX64, Start);
                    errno = 0;
                ...
                errno = 0;
                as_snprintf(Tmp1, sizeof(Tmp1), "%" PRId32, Run->Contents.LineNum);
                as_snprintf(
                        Tmp2, sizeof(Tmp2), "%" PRIX64,
                        (LargeInt)Run->Contents.Address - Start);
        ...
        if (HadLines) {
            as_snprintf(Tmp1, sizeof(Tmp1), "%" PRIX64, End);
            errno = 0;
            fprintf(MAPFile, "ENDFILE 0x%s\n", Tmp1);
            ChkIO(ErrNum_FileWriteError);
        }
    }
