...
void DestroyStructElem(PStructElem pStructElem) {
    if (pStructElem->pElemName) {
        free(pStructElem->pElemName);
    }
    if (pStructElem->pRefElemName) {
        free(pStructElem->pRefElemName);
    }
    free(pStructElem);
}
...
PStructElem CreateStructElem(tStrComp const* pElemName) {
    String      ExtName;
    PStructElem pNeu;
...
Boolean AddStructElem(PStructRec pStructRec, PStructElem pElement) {
    ...
    for (pPrev = NULL, pRun = pStructRec->Elems; pRun; pPrev = pRun, pRun = pRun->Next) {
        Duplicate = CaseSensitive ? !strcmp(pElement->pElemName, pRun->pElemName)
                                  : !as_strcasecmp(pElement->pElemName, pRun->pElemName);
        if (Duplicate) {
...
void SetStructElemSize(PStructRec pStructRec, char const* pElemName, tSymbolSize Size) {
    PStructElem pRun;
    int         Match;

    for (pRun = pStructRec->Elems; pRun; pRun = pRun->Next) {
        Match = CaseSensitive ? strcmp(pElemName, pRun->pElemName)
                              : as_strcasecmp(pElemName, pRun->pElemName);
        if (!Match) {
            pRun->OpSize = Size;
            break;
        }
...
void ResolveStructReferences(PStructRec pStructRec) {
    ...
    /* iterate over list until all symbols resolved, or we failed to resolve at least one
     * per pass */
        ...
        for (pRun = pStructRec->Elems; pRun; pRun = pRun->Next) {
            if (pRun->pRefElemName) {
                ...
                for (pRef = pStructRec->Elems; pRef; pRef = pRef->Next) {
                    if (!strcmp(pRun->pRefElemName, pRef->pElemName)
                        && !pRef->pRefElemName) {
...
void BuildStructName(char* pResult, unsigned ResultLen, char const* pName) {
    ...
    for (ZStruct = StructStack; ZStruct; ZStruct = ZStruct->Next) {
        if (ZStruct->StructRec->DoExt && ZStruct->Name[0]) {
            as_snprintf(
                    tmp2, sizeof(tmp2), "%s%c", ZStruct->pBaseName,
                    ZStruct->StructRec->ExtChar);
            strmaxprep(pResult, tmp2, ResultLen);
        }
    }
}

void AddStructSymbol(char const* pName, LargeWord Value) {
    PStructStack ZStruct;
...

    {
        ...
        as_snprintf(
                tmp, sizeof(tmp), "%s%c%s", pInnermostNamedStruct->Name,
                pInnermostNamedStruct->StructRec->ExtChar, pName);
...
Boolean FoundStruct(PStructRec* Erg, char const* pName) {
    PSaveSection Lauf;
    String       Part;
    ...
    for (Elem = Node->StructRec->Elems; Elem; Elem = Elem->Next) {
        ...
        if (Elem->BitPos >= 0) {
            if (Elem->BitWidthM1 >= 0) {
                as_snprintf(
                        NumStr, sizeof(NumStr), ".%d-%d", Elem->BitPos,
                        Elem->BitPos + Elem->BitWidthM1);
            } else {
...
void PrintStructList(void) {
    ...
    as_snprintf(
            s, sizeof(s), "%" PRILongInt "%s", Context.Sum,
            getmessage(
                    (Context.Sum == 1) ? Num_ListStructSumMsg : Num_ListStructSumsMsg));
    WrLstLine(s);
}
...
static void ExpandStruct_One(
        PStructRec StructRec, char* pVarPrefix, char* pStructPrefix, LargeWord Base) {
    ...
    if ((RemVarLen > 1) && (RemStructLen > 1)) {
        for (StructElem = StructRec->Elems; StructElem; StructElem = StructElem->Next) {
            ...
            if (StructElem->IsStruct) {
                ...
                strmaxcpy(
                        pStructPrefix + StructLen + 1, StructElem->pElemName,
                        RemStructLen);
                Found = FoundStruct(&pSubStruct, pStructPrefix);
                if (Found) {
                    ExpandStruct_One(
                            pSubStruct, pVarPrefix, pStructPrefix,
                            Base + StructElem->Offset);
                }
            }
        }
    }
...
void ExpandStruct(PStructRec StructRec) {
    ...
    for (z = 1; z <= ArgCnt; z++) {
        if (IsIndirectGen(ArgStr[z].str.p_str, "[]")) {
            ...
            Dimensions[DimensionCnt++]
                    = EvalStrIntExpressionWithResult(&Arg, UInt32, &EvalResult);
            if (!EvalResult.OK) {
                return;
            }
    ...
    if (!DimensionCnt) {
        ...
        for (Dim = 0; Dim < DimensionCnt; Dim++) {
            ...
            as_snprcatf(
                    CompVarName, sizeof(CompVarName), "_%llu", (LargeWord)Indices[Dim]);
        }
        while (Indices[0] < Dimensions[0]) {
            ...
            ExpandStruct_One(
                    StructRec, CompVarName, CompStructName, EProgCounter() + CodeLen);
            CodeLen += StructRec->TotLen;
            ...
            /* Dim now holds the most major (leftmost) index that changed.  Build up new
               element name, starting from that: */

            CompVarName[CompVarNameLens[Dim]] = '\0';
            for (; Dim < DimensionCnt; Dim++) {
                as_snprcatf(
                        CompVarName, sizeof(CompVarName), "_%llu",
                        (LargeWord)Indices[Dim]);
