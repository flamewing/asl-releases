...
static void DecodeAdr(tStrComp const* pArg, Boolean MayImm) {
    Boolean OK;
...
static void DecodeRel(Word Code) {
    if (ChkArgCnt(1, 1)) {
        ...
        Integer AdrInt = EvalStrIntExpressionWithResult(&ArgStr[1], Int16, &EvalResult)
                         - (EProgCounter() + 1);
...
static void DecodeMVI(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (EvalResult.OK) {
            ...
            BAsmCode[2] = EvalStrIntExpressionOffsWithResult(
                    &ArgStr[2], 1, Int8, &EvalResult);
            if (EvalResult.OK) {
                BAsmCode[0] = 0xb0;
                CodeLen     = 3;
            }
    ...
    if (ChkArgCnt(3, 3)) {
        ...
        if (EvalResult.OK) {
            ...
            if (EvalResult.OK) {
                ...
                ChkSpace(SegData, EvalResult.AddrSpaceMask);
                AdrInt = EvalStrIntExpressionWithResult(&ArgStr[3], Int16, &EvalResult)
                         - (EProgCounter() + 3);
                if (EvalResult.OK) {
                    if (!mSymbolQuestionable(EvalResult.Flags)
...
static void AddFixed(char const* NName, LongInt NCode) {
...
static void AddRel(char const* NName, LongInt NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeRel);
}

static void AddALU(char const* NName, LongInt NCode) {
