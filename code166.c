...
#define DPPCount 4
static char const RegNames[6][5] = {"DPP0", "DPP1", "DPP2", "DPP3", "CP", "SP"};
...
static Boolean IsRegCore(char const* pArg, tRegInt* pValue, tSymbolSize* pSize) {
    int     l = strlen(pArg);
    Boolean OK;
...
static void DissectReg_166(
        char* pDest, size_t DestSize, tRegInt Value, tSymbolSize InpSize) {
    switch (InpSize) {
    case eSymbolSize8Bit:
        as_snprintf(
                pDest, DestSize, "R%c%u", Value & 1 ? 'H' : 'L', (unsigned)(Value >> 1));
        break;
...
static Boolean ChkRegSize(tSymbolSize ReqSize, tSymbolSize ActSize) {
    return (ReqSize == eSymbolSizeUnknown) || (ReqSize == ActSize)
           || ((ReqSize == eSymbolSize8Bit) && (ActSize == eSymbolSize16Bit));
}

static tRegEvalResult IsReg(
        tStrComp const* pArg, Byte* pValue, tSymbolSize* pSize, tSymbolSize ReqSize,
        Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static tRegEvalResult IsRegM1(
        tStrComp const* pArg, Byte* pValue, tSymbolSize ReqSize, Boolean MustBeReg) {
    if (*pArg->str.p_str) {
        int            l;
...
#define CPAdr  0xfe10
    ...
    if (Mask & M_InCode) {
        if ((HiWord(EProgCounter()) == HiWord(DispAcc)) && (Mask & MModAbs)) {
            ...
            ((Mask & MModMReg) != 0) && (DispAcc >= SFRStart()) && (DispAcc <= SFREnd())
            && (!(DispAcc & 1))) {
...
static int SplitForceSize(char const* pArg, tForceSize* pForceSize) {
    switch (*pArg) {
    case '>':
        *pForceSize = eForceLong;
        return 1;
    case '<':
        *pForceSize = eForceShort;
        return 1;
    default:
        return 0;
    }
...
static ShortInt DecodeAdr(tStrComp const* pArg, Word Mask, tAdrResult* pResult) {
    ...
    if (*pArg->str.p_str == '#') {
        ...
        case eSymbolSize8Bit:
            pResult->Vals[0] = EvalStrIntExpressionOffsWithFlags(
                    pArg, 1 + Offs, Int8, &OK, &pResult->SymFlags);
            pResult->Vals[1] = 0;
            break;
        case eSymbolSize16Bit:
            HDisp = EvalStrIntExpressionOffsWithFlags(
                    pArg, 1 + Offs, Int16, &OK, &pResult->SymFlags);
    ...
    /* Register ? */
            ...
            (RegEvalResult = IsReg(pArg, &pResult->Mode, NULL, OpSize, False))
            != eIsNoReg) {
    ...
    /* indirekt ? */
            ...
            (*pArg->str.p_str == '[')
            && (pArg->str.p_str[strlen(pArg->str.p_str) - 1] == ']')) {
        tStrComp Arg;
        int      ArgLen;
        ...
        if ((ArgLen > 2) && (*Arg.str.p_str == '-')
            && ((RegEvalResult = IsRegP1(&Arg, &pResult->Mode, eSymbolSize16Bit, False))
                != eIsNoReg)) {
        ...
        /* Postinkrement ? */
                ...
                (ArgLen > 2) && (Arg.str.p_str[ArgLen - 1] == '+')
                && ((RegEvalResult
                     = IsRegM1(&Arg, &pResult->Mode, eSymbolSize16Bit, False))
                    != eIsNoReg)) {
            if (eRegAbort == RegEvalResult) {
                return pResult->Type;
            }
        ...
        /* indiziert ? */
            ...
            do {
                ...
                if ((RegEvalResult = IsReg(&Arg, &HReg, NULL, eSymbolSize16Bit, False))
                    != eIsNoReg) {
                    if (RegEvalResult == eRegAbort) {
                        return pResult->Type;
                    }
                    ...
                    HDisp = EvalStrIntExpressionOffs(
                            &Arg, !!(*Arg.str.p_str == '#'), Int32, &OK);
                    if (OK) {
        ...
        DispAcc = EvalStrIntExpressionOffsWithFlags(
                pArg, Offset, MemInt, &OK, &pResult->SymFlags);
        if (OK) {
            DecideAbsolute(DispAcc, Mask, pResult);
        }
    }
...
static Boolean DecodeBitAddr(
        tStrComp const* pArg, Word* Adr, Byte* Bit, Boolean MayBeOut) {
    char*   p;
    Word    LAdr;
    Byte    Reg;
    Boolean OK;
...
static Word WordVal(tAdrResult const* pResult) {
    return pResult->Vals[0] + (((Word)pResult->Vals[1]) << 8);
}

static Boolean DecodePref(tStrComp const* pArg, Byte* Erg) {
    Boolean      OK;
    tSymbolFlags Flags;
...
static void DecodeMOV(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1],
                MModReg | MModMReg | MModIReg | MModPreDec | MModPostInc | MModIndex
                        | MModAbs | M_Dest,
                &DestResult)) {
        case ModReg: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2],
                    MModReg | MModImm | MModIReg | MModPostInc | MModIndex | MModAbs,
                    &SrcResult)) {
            case ModReg:
                CodeLen     = 2;
            ...
            case ModImm: {
                ...
                if (SrcResult.ForceSize == eForceShort) {
                    if (!IsShort && !mSymbolQuestionable(SrcResult.SymFlags)) {
        ...
        case ModMReg: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2],
                    MModImm | MModMReg | ((DPPAssumes[3] == 3) ? MModIReg : 0) | MModAbs,
                    &SrcResult)) {
            case ModImm:
                CodeLen     = 4;
                BAsmCode[0] = 0xe6 + Code;
        ...
        case ModIReg: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2], MModReg | MModIReg | MModPostInc | MModAbs, &SrcResult)) {
            case ModReg:
                CodeLen     = 2;
...
static void DecodeMOVBS_MOVBZ(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1], MModReg | MModMReg | MModAbs | M_Dest, &DestResult)) {
        case ModReg: {
            tAdrResult SrcResult;
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModMReg:
            ...
            if (DecodeAdr(&ArgStr[2], MModAbs | MModImm, &Result) != ModNone) {
                CodeLen     = 4;
                BAsmCode[0] = 0xc6 + (Ord(Result.Type == ModAbs) << 4);
...
static void DecodeALU2(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2], MModReg | MModIReg | MModPostInc | MModAbs | MModImm,
                    &SrcResult)) {
            case ModReg:
                CodeLen     = 2;
                BAsmCode[0] = Code;
        ...
        if (DecodeAdr(&ArgStr[1], MModReg | M_Dest, &Result) == ModReg) {
            ...
            case ModImm: {
                ...
                if (Result.ForceSize == eForceShort) {
                    if (!IsShort && !mSymbolQuestionable(Result.SymFlags)) {
                        WrStrErrorPos(ErrNum_OverRange, &ArgStr[2]);
                    } else {
                        CodeLen     = 2;
                        BAsmCode[0] = Code;
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg: {
            ...
            case ModReg:
                ...
                break;
            case ModImm:
                if ((WordVal(&SrcResult) > 15)
                    && !mSymbolQuestionable(SrcResult.SymFlags)) {
                    WrStrErrorPos(ErrNum_OverRange, &ArgStr[2]);
                } else {
...
static void DecodeBit2(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeBitAddr(&ArgStr[1], &BAdr1, &BOfs1, False)
...
static void DecodeBCLR_BSET(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeBitAddr(&ArgStr[1], &BAdr, &BOfs, False)) {
        CodeLen     = 2;
...
static void DecodeJMP(Word Code) {
    ...
    if (ChkArgCnt(1, 2)) {
        ...
        if (Cond >= ConditionCount) {
            WrStrErrorPos(ErrNum_UndefCond, &ArgStr[1]);
        } else {
            tAdrResult Result;

            switch (DecodeAdr(
                    &ArgStr[ArgCnt], MModAbs | MModLAbs | MModIReg | M_InCode, &Result)) {
            case ModLAbs:
                if (Cond != TrueCond) {
                    WrStrErrorPos(ErrNum_UndefCond, &ArgStr[1]);
                } else {
            ...
            case ModAbs: {
                ...
                Boolean IsShort
                        = (AdrDist <= 254) && (AdrDist >= -256) && ((AdrDist & 1) == 0);
    ...
    if (ChkArgCnt(1, 2)) {
        ...
        if (Cond >= ConditionCount) {
            ...
            case ModAbs: {
                ...
                Boolean IsShort
                        = (AdrLong <= 254) && (AdrLong >= -256) && ((AdrLong & 1) == 0);
...
static void DecodeJMPR(Word Code) {
    ...
    if (ChkArgCnt(1, 2)) {
        ...
        if (Cond >= ConditionCount) {
            ...
            tSymbolFlags Flags;
            LongInt      AdrLong
                    = EvalStrIntExpressionWithFlags(&ArgStr[ArgCnt], MemInt, &OK, &Flags)
                      - (EProgCounter() + 2);
...
static void DecodeBJmp(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeBitAddr(&ArgStr[1], &BAdr, &BOfs, False)) {
        Boolean      OK;
        tSymbolFlags Flags;
        LongInt AdrLong = EvalStrIntExpressionWithFlags(&ArgStr[2], MemInt, &OK, &Flags)
                          - (EProgCounter() + 4);
        if (OK) {
            if (AdrLong & 1) {
                WrError(ErrNum_DistIsOdd);
...
static void DecodeATOMIC(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU80C167) && DecodePref(&ArgStr[1], &HReg)) {
        CodeLen     = 2;
        BAsmCode[0] = 0xd1;
        BAsmCode[1] = HReg << 4;
    }
...
static void DecodeEXTP_EXTPR(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU80C167) && DecodePref(&ArgStr[2], &HReg)) {
        tAdrResult Result;
...
static void DecodeBIT(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeBitAddr(&ArgStr[1], &Adr, &Bit, True)) {
        PushLocHandle(-1);
...
static void AddBInstTable(char const* NName, Word NCode, InstProc Proc) {
    char BName[30];
...
static void AddFixed(char const* NName, CPUVar NMin, Word NCode1, Word NCode2) {
