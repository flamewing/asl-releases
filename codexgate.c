...
static void DissectReg_XGATE(
        char* pDest, size_t DestSize, tRegInt Reg, tSymbolSize Size) {
    switch (Size) {
...
static tRegEvalResult DecodeReg(tStrComp const* pArg, Word* pReg, Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static void DecodeBranch(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        Dist = EvalStrIntExpressionWithFlags(&ArgStr[1], UInt16, &OK, &Flags)
               - (EProgCounter() + 2);
        if (OK) {
            if (!mSymbolQuestionable(Flags) && (Dist & 1)) {
                WrError(ErrNum_NotAligned);
            } else if (!mSymbolQuestionable(Flags) && ((Dist < -512) || (Dist > 510))) {
                WrError(ErrNum_NotAligned);
            } else {
...
static void DecodeBRA(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (OK) {
            if (!mSymbolQuestionable(Flags) && (Dist & 1)) {
                WrError(ErrNum_NotAligned);
            } else if (!mSymbolQuestionable(Flags) && ((Dist < -1024) || (Dist > 1022))) {
                WrError(ErrNum_NotAligned);
            } else {
...
static void DecodeReg3(Word Index) {
    ...
    if (ChkArgCnt(3, 3) && DecodeArgReg(1, &DReg) && DecodeArgReg(2, &SReg1)
...
static void DecodeCPC(Word Index) {
    ...
    if (ChkArgCnt(2, 3) && DecodeArgReg(1, &DReg) && DecodeArgReg(2, &SReg)) {
...
static void DecodeMem(Word Code) {
    ...
    } else if (!IsIndirect(ArgStr[2].str.p_str)) {
        WrError(ErrNum_InvAddrMode);
    } else {
        ...
        if (OK) {
            ...
            if (*pIndexComp->str.p_str == '#') {
                ...
                if (OK) {
                    ...
                    ((l = strlen(pIndexComp->str.p_str)) > 1)
                    && (pIndexComp->str.p_str[l - 1] == '+')) {
                Code |= 0x2000;
