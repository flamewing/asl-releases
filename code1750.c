...
static Boolean DecodeReg(char const* pAsc, Word* pResult) {
    if (toupper(*pAsc) != 'R') {
        return False;
    }
    pAsc++;
...
static Boolean DecodeBaseReg(char const* pAsc, Word* pResult) {
...
static Boolean DecodeCondition(char const* pAsc, Word* pResult) {
    static tCondition const Conditions[] = {
            { "LT", 0x1}, /* 0001 */
            { "LZ", 0x1}, /* 0001 */
            { "EQ", 0x2}, /* 0010 */
            { "EZ", 0x2}, /* 0010 */
...
static void DecodeR(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeArgReg(1, &Ra) && DecodeArgReg(2, &Rb)) {
...
static void DecodeMem(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && DecodeArgReg(1, &Ra) && DecodeAdr(2, ArgCnt)) {
...
static void DecodeBX(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrXErrorPos(ErrNum_InvAddrMode, "!R0", &ArgStr[2].Pos);
    } else {
...
static void DecodeJump(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && DecodeArgCondition(1, &Cond) && DecodeAdr(2, ArgCnt)) {
...
static void DecodeXIO(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && DecodeArgReg(1, &Ra) && DecodeArgXIOCmd(2, &Cmd)) {
        if (ArgCnt == 3) {
            Word Ri;
            ...
            if (!DecodeArgReg(3, &Ri))
                ...
                WrXErrorPos(ErrNum_InvAddrMode, "!R0", &ArgStr[3].Pos);
            } else {
...
static void DecodeFLOAT(Word Extended) {
    ...
    printf("Sign %u SignedExponent %d Mantissa 0x%02x%02x%02x%02x%02x%02x%02x\n", Sign, SignedExponent,
        ...
        if (Sign) {
            ...
            if ((Field[1] == 0x08) && !Field[2] && !Field[3] && !Field[4] && !Field[5]
                && !Field[6] && !Field[7] && (SignedExponent > -127)) {
                ShiftMantLeft(Field);
                SignedExponent--;
            }
        ...
        Word0 = ((Word)(Field[1] & 0x1f) << 11) | ((Word)Field[2] << 3)
                | ((Field[3] >> 5) & 0x07);
        Word1 = ((Word)(Field[3] & 0x1f) << 11) | ((Word)(Field[4] & 0xe0) << 3);
        if (Extended) {
            Word2 = ((Word)(Field[4] & 0x1f) << 11) | ((Word)Field[5] << 3)
                    | ((Field[6] >> 5) & 0x07);
        } else {
            Word2 = 0;
        }
