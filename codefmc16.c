...
static char const BankNames[4][4] = {"PCB", "DTB", "ADB", "SPB"};
...
static Boolean DecodeAdr(tStrComp const* pArg, int Mask) {
    Integer           AdrVal;
    LongWord          ImmVal;
    Boolean           OK;
    unsigned          Index;
    static char const SpecNames[7][4] = {"DTB", "ADB", "SSB", "USB", "DPR", "\a", "PCB"};
    ...
    if (as_toupper(*pArg->str.p_str) == 'R') {
        ...
        case 'W':
            if ((pArg->str.p_str[3] == '\0') && (pArg->str.p_str[2] >= '0')
                && (pArg->str.p_str[2] <= '7')) {
    ...
    if ((*pArg->str.p_str == '(') && (as_toupper(pArg->str.p_str[1]) == 'R')
        && (as_toupper(pArg->str.p_str[2]) == 'L') && (pArg->str.p_str[3] >= '0')
        && (pArg->str.p_str[3] <= '3') && (pArg->str.p_str[4] == ')')
        && (pArg->str.p_str[5] == '\0')) {
    ...
    /* 4. immediate: */
        ...
        if (OpSize == -1) {
            ...
            ImmVal = EvalStrIntExpressionOffs(
                    pArg, 1, (OpSize == 2) ? Int32 : ((OpSize == 1) ? Int16 : Int8), &OK);
            if (OK) {
                AdrMode           = ModImm;
    ...
    if (*pArg->str.p_str == '@') {
        ...
        /* PC-relativ: */
            ...
            AdrPart = 0x1e;
            if ((*RegComp.str.p_str == '+') || (*RegComp.str.p_str == '-')
                || (as_isspace(*RegComp.str.p_str))) {
        ...
        /* base register, 32 bit: */
                ...
                (as_toupper(*Arg.str.p_str) == 'R')
                && (as_toupper(Arg.str.p_str[1]) == 'L') && (Arg.str.p_str[2] >= '0')
                && (Arg.str.p_str[2] <= '3')) {
        ...
        /* base register, 16 bit: */
                ...
                (as_toupper(*Arg.str.p_str) == 'R')
                && (as_toupper(Arg.str.p_str[1]) == 'W') && (Arg.str.p_str[2] >= '0')
                && (Arg.str.p_str[2] <= '7')) {
            tStrComp IComp;
            ...
            case '+':
                ...
                {
                    if (AdrPart > 3) {
                        WrError(ErrNum_InvReg); /* only allowed for RW0..RW3   */
                    } else {
                        AdrPart += 0x0c;
                        AdrMode = ModMem;
                    }
                ...
                } else /* numeric index               */
                {
                    AdrVal = /* max length depends on base  */
                            EvalStrIntExpression(
                                    &IComp, (AdrPart > 3) ? SInt8 : SInt16, &OK);
                    if (OK) { /* optimize length             */
                        AdrVals[0] = AdrVal & 0xff;
                        AdrCnt     = 1;
                        AdrMode    = ModMem;
                        AdrPart |= 0x10;
    ...
    if (OK) {
        ...
        if (((ImmVal >> 8) == 0) && (Mask & MModIO)) {
            AdrMode = ModIO;
                ...
                (Lo(ImmVal >> 8) == Reg_DPR) && ((ImmVal & 0xffff0000) == CurrBank)
                && (Mask & MModDir)) {
            AdrMode = ModDir;
        } else {
            AdrVals[AdrCnt++] = (ImmVal >> 8) & 0xff;
...
static void DecodeCarry8(Word Index) {
    if (ChkArgCnt(1, 2) && DecodeAdr(&ArgStr[1], MModAcc)) {
        if (ArgCnt == 1) {
...
static void DecodeCarry16(Word Index) {
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1], MModAcc)
...
static void DecodeAcc(Word Code) {
    if (ChkArgCnt(1, 1) && DecodeAdr(&ArgStr[1], MModAcc)) {
        BAsmCode[0] = Code;
        CodeLen     = 1;
    }
}
...
static void DecodeShift(Word Code) {
    if (ChkArgCnt(Hi(Code) ? 1 : 2, 2) && DecodeAdr(&ArgStr[1], MModAcc)) {
        if (ArgCnt == 1) {
            BAsmCode[0] = Lo(Code);
            CodeLen     = 1;
        } else {
...
static void DecodeBBcc(Word Index) {
    ...
    if ((ChkArgCnt(2, 2)) && (SplitBit(&ArgStr[1], &BitPos))) {
        HLen = 0;
        ...
        if (AdrMode != ModNone) {
            ...
            if (HLen > 1) {
                ...
                Addr = EvalStrIntExpressionWithFlags(&ArgStr[2], UInt24, &OK, &Flags)
                       - (EProgCounter() + HLen + 1);
                if (OK) {
    ...
    if (ChkArgCnt(3, 3)) {
        ...
        if (DecodeAdr(&ArgStr[1], MModMem | MModReg | MModAcc)) {
            ...
            if ((OK) && (DecodeAdr(&ArgStr[2], MModImm))) {
                ...
                memcpy(BAsmCode + HCnt, AdrVals, AdrCnt);
                HCnt += AdrCnt;
                Addr = EvalStrIntExpressionWithFlags(&ArgStr[3], UInt24, &OK, &Flags)
                       - (EProgCounter() + HCnt + 1);
                if (OK) {
                    if (!mSymbolQuestionable(Flags) && ((Addr > 127) || (Addr < -128))) {
                        WrError(ErrNum_JmpDistTooBig);
                    } else {
...
static void DecodeBit(Word Index) {
    ...
    if ((ChkArgCnt(1, 1)) && (SplitBit(&ArgStr[1], &BitPos))) {
        DecodeAdr(&ArgStr[1], MModMem | MModDir | MModIO);
        if (AdrMode != ModNone) {
...
static void DecodeDBNZ(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (DecodeAdr(&ArgStr[1], MModReg | MModMem)) {
            ...
            Addr = EvalStrIntExpressionWithFlags(&ArgStr[2], UInt16, &OK, &Flags)
                   - (EProgCounter() + 3 + AdrCnt);
            if (OK) {
                if (!mSymbolQuestionable(Flags) && ((Addr < -128) || (Addr > 127))) {
                    WrError(ErrNum_JmpDistTooBig);
                } else {
...
static void DecodeSeg(Word Code) {
    if (ChkArgCnt(1, 1) && DecodeAdr(&ArgStr[1], MModSeg)) {
        BAsmCode[0] = 0x6e;
...
static void DecodeString(Word Code) {
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1], MModSeg)) {
...
static void DecodeMOV(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
            ...
            ((!as_strcasecmp(ArgStr[1].str.p_str, "@AL"))
             || (!as_strcasecmp(ArgStr[1].str.p_str, "@A")))
            && ((!as_strcasecmp(ArgStr[2].str.p_str, "AH"))
                || (!as_strcasecmp(ArgStr[2].str.p_str, "T")))) {
        BAsmCode[0] = 0x6f;
        BAsmCode[1] = 0x15;
        CodeLen     = 2;
    } else {
        SetOpSize(0);
        DecodeAdr(
                &ArgStr[1], MModRP | MModILM | MModAcc | MModDir | MModRDisp | MModIO
                                    | MModSpec | MModReg | MModMem);
        switch (AdrMode) {
        case ModRP: {
            if (DecodeAdr(&ArgStr[2], MModImm)) {
                BAsmCode[0] = 0x0a;
        ...
        case ModAcc: {
            DecodeAdr(
                    &ArgStr[2], MModImm | MModIAcc | MModRDisp | MModIO | MModMem
                                        | MModReg | MModDir | MModSpec);
            switch (AdrMode) {
            case ModImm:
                BAsmCode[0] = 0x42;
                BAsmCode[1] = AdrVals[0];
...
static void DecodeMOVB(Word Index) {
    if (ChkArgCnt(2, 2)) {
        ...
        if ((Index > 0) && SplitBit(&ArgStr[Index], BAsmCode + 1)
...
static void DecodeMOVW(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        DecodeAdr(
                &ArgStr[1],
                MModAcc | MModRDisp | MModSP | MModDir | MModIO | MModReg | MModMem);
        switch (AdrMode) {
        case ModAcc:
            DecodeAdr(
                    &ArgStr[2], MModImm | MModIAcc | MModRDisp | MModSP | MModIO | MModMem
                                        | MModReg | MModDir);
            switch (AdrMode) {
            case ModImm:
                BAsmCode[0] = 0x4a;
...
static void DecodeMOVX(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1], MModAcc)) {
        ...
        DecodeAdr(
                &ArgStr[2],
                MModImm | MModIAcc | MModRDisp | MModDir | MModIO | MModReg | MModMem);
        switch (AdrMode) {
        case ModImm:
            BAsmCode[0] = 0x43;
            CopyVals(1);
    ...
    if (!ChkArgCnt(1, ArgCntMax))
        ...
        for (z = 1; z <= ArgCnt; z++) {
            if ((p = strchr(ArgStr[z].str.p_str, '-')) != NULL) {
                ...
                if (!DecodeAdr(&From, MModReg)) {
                    break;
                }
                HReg = AdrPart;
                if (!DecodeAdr(&To, MModReg)) {
                    break;
                }
                if (AdrPart >= HReg) {
...
static void DecodeSBBS(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && SplitBit(&ArgStr[1], BAsmCode + 1)
        ...
        if (AdrPart != ABSMODE) {
            ...
            Adr = EvalStrIntExpressionWithFlags(&ArgStr[2], UInt24, &OK, &Flags)
                  - (EProgCounter() + 5);
            if (OK) {
                if (!mSymbolQuestionable(Flags) && ((Adr < -128) || (Adr > 127))) {
                    WrError(ErrNum_JmpDistTooBig);
                } else {
                    BAsmCode[0] = 0x6c;
                    BAsmCode[1] += 0xf8;
...
static void DecodeWBit(Word Index) {
    if (ChkArgCnt(1, 1) && SplitBit(&ArgStr[1], BAsmCode + 1)
...
static void AddALU8(char const* NName, Byte NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeALU8);
}

static void AddLog8(char const* NName, Byte NCode) {
...
static void AddShift(char const* NName, Word NCode, Word NMay) {
    AddInstTable(InstTable, NName, NCode | (NMay << 8), DecodeShift);
}

static void AddBranch(char const* NName, Byte NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeBranch);
}

static void AddIncDec(char const* NName, Byte NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeIncDec);
}

static void AddMulDiv(char const* NName, Byte NCode, Word NAccCode) {
    MulDivOrders[InstrZ].Code    = NCode;
    MulDivOrders[InstrZ].AccCode = NAccCode;
    AddInstTable(InstTable, NName, InstrZ++, DecodeMulDiv);
}

static void AddSeg(char const* NName, Byte NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeSeg);
}

static void AddString(char const* NName, Byte NCode) {
...
static void InitFields(void) {
    ...
    AddShift("ASR", 0x2e, FALSE);
    AddShift("ASRL", 0x1e, FALSE);
    AddShift("ASRW", 0x0e, TRUE);
    AddShift("LSLW", 0x0c, TRUE);
    AddShift("LSRW", 0x0f, TRUE);
    AddShift("LSL", 0x2c, FALSE);
    AddShift("LSR", 0x2f, FALSE);
    AddShift("LSLL", 0x1c, FALSE);
    AddShift("LSRL", 0x1f, FALSE);
    AddShift("SHLW", 0x0c, TRUE);
    ...
    InstrZ       = 0;
    AddMulDiv("MULU", 0x00, 0x0027);
    AddMulDiv("MULUW", 0x20, 0x002f);
    AddMulDiv("MUL", 0x40, 0x786f);
    AddMulDiv("MULW", 0x60, 0x796f);
    AddMulDiv("DIVU", 0x80, 0x0026);
    AddMulDiv("DIVUW", 0xa0, 0xffff);
    AddMulDiv("DIV", 0xc0, 0x7a6f);
    AddMulDiv("DIVW", 0xe0, 0xffff);
    ...
    AddString("MOVS", 0x00);
    AddString("MOVSI", 0x00);
    AddString("MOVSD", 0x10);
    AddString("MOVSW", 0x20);
    AddString("MOVSWI", 0x20);
    AddString("MOVSWD", 0x30);
