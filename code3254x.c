...
static char const ShortConds[4][4] = {"EQ", "LT", "GT", "NEQ"};
...
#define IndirCnt 16
    static char const Patterns[IndirCnt][9]
            = /* leading asterisk is omitted since constant */
            {"ARx",    "ARx-",    "ARx+",     "+ARx",   "ARx-0B", "ARx-0",
             "ARx+0",  "ARx+0B",  "ARx-%",    "ARx-0%", "ARx+%",  "ARx+0%",
             "ARx(n)", "+ARx(n)", "+ARx(n)%", "(n)"};
    Boolean OK;
    ...
    if ((strlen(pArg->str.p_str) == 3) && (!as_strncasecmp(pArg->str.p_str, "AR", 2))
    ...
    if (*pArg->str.p_str == '*') {
        ...
        for (z = 0; z < IndirCnt; z++) {
            char const* pPattern = Patterns[z];
...
static Boolean DecodeCondition(
        int StartIndex, Word* Result, int* errindex, Boolean* ErrUnknown) {
    int  z, z2;
...
static void DecodeFixed(Word Index) {
    ...
    } else if ((LastRep) && (!POrder->IsRepeatable)) {
...
static void DecodeAcc(Word Index) {
    FixedOrder const* POrder = AccOrders + Index;
...
static void DecodeAcc2(Word Index) {
    FixedOrder const* POrder = Acc2Orders + Index;
    Boolean           OK;
...
static void DecodeMem(Word Index) {
    FixedOrder const* POrder = MemOrders + Index;
...
static void DecodeXY(Word Index) {
    FixedOrder const* POrder = XYOrders + Index;
    Word              TmpX, TmpY;
...
static void DecodeADDSUB(Word Index) {
    ...
    if (ChkArgCnt(2, 4)) {
        ...
        case ModMem: /* ADD mem[, TS | SHIFT | Ymem], src[, dst] */
        {
            ...
            /* no shift? this is the case for two operands or three operands and the
             * second is an accumulator */
            ...
            else if (Shift == 16) /* optimization for 16 shifts */
            {
                if (ThisPar) {
                    ...
                    WAsmCode[0] |= (0x3c00 + (Index << 10)) | (*AdrVals << 9)
                                   | (DestAcc << 8);
                    CodeLen = 1 + HCnt;
                }
            }

            else if ((DestAcc == *AdrVals) && (Shift == 0)) /* shortform without shift and
                                                               with one accu only */
            {
                if (ThisPar) {
                    AdrMode    = ModMem;
                    AdrVals[0] = WAsmCode[0];
                    if (MakeXY(AdrVals, True)) {
                        /* prev. operation must be STH src,0,Xmem */
                        if ((LastOpCode & 0xfe0f) != 0x9a00) {
                            WrError(ErrNum_ParNotPossible);
                        } else {
                            RetractWords(1);
                            WAsmCode[0] = 0xc000 | (Index << 10) | (DestAcc << 8)
                                          | ((LastOpCode & 0x0100) << 1)
                                          | ((LastOpCode & 0x00f0) >> 4)
                                          | (*AdrVals << 4);
                            CodeLen = 1;
                ...
                if ((Shift >= 0) && (DestAcc == SrcAcc)
                    && (MakeXY(WAsmCode, False))) /* X-Addr and positive shift */
                {
        ...
        *WAsmCode = (*AdrVals) << 8;
        ...
        /* syntax 1/2/4 have memory operand in front */
            ...
            if (ArgCnt == 2) {
                if (ThisPar) {
                    if (MakeXY(AdrVals, True)) {
                        if ((LastOpCode & 0xfe0f)
                            == 0x9400) /* previous op LD Xmem, src */
                        {
                            if ((LastOpCode & 0x0100) == (*WAsmCode & 0x0100)) {
                                WrError(ErrNum_ParNotPossible);
                            } else {
                                RetractWords(1);
                        ...
                        {
                            RetractWords(1);
                            *WAsmCode |= 0xd000 | ((LastOpCode & 0x0100) << 1)
        ...
        *WAsmCode = Index | (0 [AdrVals] << 8);
        if (DecodeAdr(&ArgStr[1], MModMem)) {
            ...
            *WAsmCode |= *AdrVals;
            ...
            WAsmCode[1 + AdrCnt]
                    = EvalStrIntExpressionWithResult(&ArgStr[2], UInt16, &EvalResult);
            if (EvalResult.OK) {
        ...
        if (MakeXY(WAsmCode, TRUE)) {
            ...
            if (DecodeAdr(&ArgStr[2], MModMem)) {
                if (MakeXY(AdrVals, TRUE)) {
                    ...
                    0 [WAsmCode] |= *AdrVals;
                    WAsmCode[1] = EvalStrIntExpressionWithResult(
                            &ArgStr[3], UInt16, &EvalResult);
                    if (EvalResult.OK) {
            ...
            if (ArgCnt == 2) {
                if (ThisPar) {
                    if (MakeXY(AdrVals, True)) {
                        ...
                        {
                            RetractWords(1);
                            *WAsmCode |= 0xd400 | ((LastOpCode & 0x0100) << 1)
                                         | ((LastOpCode & 0x00f0) >> 4) | (*AdrVals << 4);
                            CodeLen = 1;
                        } else {
                            WrError(ErrNum_ParNotPossible);
...
static void DecodeMac(Word Index) {
    ...
    if (!ChkArgCnt(1, ArgCntMax))
        ...
        if ((ArgCnt == 2) && (as_strcasecmp(ArgStr[2].str.p_str, "B"))) {
            WrError(ErrNum_InvAddrMode);
        } else if (DecodeAdr(&ArgStr[1], MModMem)) {
...
static void DecodeMACSU(Word Index) {
    ...
    if (!ChkArgCnt(3, 3))
        ...
        *WAsmCode = 0xa600 | ((*AdrVals) << 8);
        if ((DecodeAdr(&ArgStr[1], MModMem)) && (MakeXY(AdrVals, TRUE))) {
            *WAsmCode |= ((*AdrVals) << 4);
            if ((DecodeAdr(&ArgStr[2], MModMem)) && (MakeXY(AdrVals, TRUE))) {
                *WAsmCode |= *AdrVals;
                CodeLen = 1;
            }
        }
        ...
        case ModAcc: /* Variant 4 */
            if (ChkArgCnt(1, 3)) {
                ...
                *WAsmCode = 0xf080 | Acc | (Index << 5);
                ...
                if (OK) {
                    if (((ArgCnt == 2) && (!IsAcc(ArgStr[2].str.p_str)))
                        || (ArgCnt == 3)) {
            ...
            if (ChkArgCnt(2, 4)) {
                ...
                if (DecodeAdr(&ArgStr[ArgCnt], MModAcc)) {
                    *WAsmCode = Acc = *AdrVals << 8;
                    ...
                    if (OK) {
                        if (((ArgCnt == 3) && (!IsAcc(ArgStr[2].str.p_str)))
                            || (ArgCnt == 4)) {
                            tSymbolFlags Flags;

                            Shift = EvalStrIntExpressionWithFlags(
                                    &ArgStr[2], UInt5, &OK, &Flags);
...
static void DecodeBANZ(Word Index) {
    if (!ChkArgCnt(2, 2))
        ...
        WAsmCode[1 + CodeLen]
                = EvalStrIntExpressionWithResult(&ArgStr[1], UInt16, &EvalResult);
        if (EvalResult.OK) {
            ChkSpace(SegCode, EvalResult.AddrSpaceMask);
            ...
            case ModMem:
                if (Shift == 0xff) /* TS ? */
                {
                    ...
                    if (ThisPar) {
                        ...
                        /* prev. operation must be STH src,0,Xmem */
                        else if ((LastOpCode & 0xfe0f) != 0x9a00) {
                            WrError(ErrNum_ParNotPossible);
                        } else {
                            RetractWords(1);
...
static void DecodeSACCD(Word Index) {
    ...
    if (!ChkArgCnt(3, 3))
        ...
        *WAsmCode = 0x9e00 | ((*AdrVals) << 8);
        if ((DecodeAdr(&ArgStr[2], MModMem)) && (MakeXY(AdrVals, True))) {
            *WAsmCode |= ((*AdrVals) << 4);
            if (!DecodeCondition(3, WAsmCode + 1, &index, &OK)) {
                WrStrErrorPos(ErrNum_UndefCond, &ArgStr[index]);
            } else if ((WAsmCode[1] & 0xf0) != 0x40) {
                WrStrErrorPos(ErrNum_UndefCond, &ArgStr[index]);
            } else {
...
static void DecodeStoreCC(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        *WAsmCode = Index | ((*AdrVals) << 4);
        if (!DecodeCondition(2, WAsmCode + 1, &index, &OK)) {
            WrStrErrorPos(ErrNum_UndefCond, &ArgStr[index]);
        } else if ((WAsmCode[1] & 0xf0) != 0x40) {
            WrStrErrorPos(ErrNum_UndefCond, &ArgStr[index]);
        } else {
...
static void DecodeMVdmadmmr(Word Index) {
    if (!ChkArgCnt(2, 2))
        ...
        tStrComp const *pArg1 = (Index & 0x0100) ? &ArgStr[2] : &ArgStr[1],
...
static Boolean GetReg(tStrComp const* pArg, Word* Res) {
    tEvalResult EvalResult;
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (DecodeAdr(pArg2, MModMem)) {
            ...
            *WAsmCode = Index | *AdrVals;
            ...
            WAsmCode[1 + AdrCnt]
                    = EvalStrIntExpressionWithResult(pArg1, UInt16, &EvalResult);
            if (EvalResult.OK) {
...
static void AddFixed(char const* Name, Word Code, Boolean IsRepeatable) {
...
static void AddAcc(char const* Name, Word Code, Boolean IsRepeatable) {
...
static void AddAcc2(char const* Name, Word Code, Boolean IsRepeatable) {
...
static void AddMem(char const* Name, Word Code, Boolean IsRepeatable) {
...
static void AddXY(char const* Name, Word Code, Boolean IsRepeatable) {
...
static void AddMemAcc(char const* Name, Word Code, Boolean IsRepeatable) {
...
static void AddMemConst(
        char const* Name, Word Code, Boolean IsRepeatable, Boolean Swap,
        IntType ConstType) {
...
static void AddMac(char const* Name, Word Code, Boolean IsRepeatable) {
...
static void AddCondition(char const* NName, Word NClass, Word NCode, Word NMask) {
...
static void InitFields(void) {
    ...
    Acc2Orders = (FixedOrder*)malloc(sizeof(FixedOrder) * Acc2OrderCnt);
    InstrZ     = 0;
    ...
    XYOrders = (FixedOrder*)malloc(sizeof(FixedOrder) * XYOrderCnt);
    InstrZ   = 0;
    ...
    MemAccOrders = (FixedOrder*)malloc(sizeof(FixedOrder) * MemAccOrderCnt);
    InstrZ       = 0;
    ...
    MacOrders = (FixedOrder*)malloc(sizeof(FixedOrder) * MacOrderCnt);
    InstrZ    = 0;
    AddMac("MACA", 0x3508, TRUE);
    AddMac("MACAR", 0x3709, TRUE);
    AddMac("MASA", 0x330a, TRUE);

    MemConstOrders = (MemConstOrder*)malloc(sizeof(MemConstOrder) * MemConstOrderCnt);
    InstrZ         = 0;
