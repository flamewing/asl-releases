...
\title{Makroassembler AS V1.41\\
       Benutzeranleitung}
\author{Alfred Arnold}
\date{Stand Februar 1997}
\maketitle
\begin{titlepage}
\
\end{titlepage}
...
gibt (die einzien Leute, die keine Fehler machen, liegen auf dem
...
\item{per Post: \begin{description}
                ...
                \end{description}}
\item{per E-Mail: \begin{description}
                  \item{direkt:\tt a.arnold@kfa-juelich.de \rm}
                  \item{"uber den Computerclub:\tt ccac@rog.rwth-aachen.de \rm}
                  \end{description}}
\end{itemize}
Sofern Sie die letztere E-Mail-Adresse benutzen, f"ugen Sie der Nachricht
bitte einen Hinweis bei, da"s diese an mich (Alfred Arnold/Turbo-Pascal-Gruppe)
gerichtet ist.
\par
...
Das von mir ausgegebene Paket beinhaltet die in Tabelle
\ref{TabPackageList1} und \ref{TabPackageList2} aufgef"uhrten Dateien;
...
\caption{Dateien im Lieferumfang - Teil 1\label{TabPackageList1}}
\end{table*}
\begin{table*}[ht]
\begin{center}\begin{tabular}{|l|l|}
\hline
...
\hline
\end{tabular}\end{center}
\caption{Dateien im Lieferumfang - Teil 2\label{TabPackageList2}}
\end{table*}
...
\item{Texas Instruments TMS320C8x (die kostenlose CD ist vergriffen
      und die Preise f"ur den angek"undigten Nachfolger sind mir zu
...
eines Makroassemblers f"ur die 68000er-Familie entstanden.  Auf besonderen
...
vom 68000 zum 8051 sind die anderen fast nebenbei abgefallen... Die
Versionsnummer ist also nicht nur Marketing, sondern sie dr"uckt wirklich
aus, da"s dies hier keine 1.0er Version mit Kinderkrankheiten ist (was
nichts am Gewicht des in Teil 1.1 gesagten "andert).
\par
...
sich in Kapitel \ref{ChapCodeFormat}.
\par
...
--- mit einer Breite von 32 Bit f"ur Integerwerte, 80 Bit f"ur
Gleitkommawerte
und 255 Zeichen f"ur Strings gespeichert.  F"ur die Intel-Mikrokontroller
besteht die M"oglichkeit, durch Segmentbildung die Symbole bestimmten Klassen
zuzuordnen.  Dem Assembler kann man auf diese Weise die --- begrenzte ---
M"oglichkeit geben, Zugriffe in falsche Adre"sr"aume zu erkennen.
\par
...
\par
Zur offensichtlich unvermeidlichen Frage, ob es denn einmal eine
Windows-Version geben wird: Ich bin der Ansicht, da"s man ein System,
wenn es offensichtlicher Bockmist ist, nicht auch noch durch Programme
unterst"utzen sollte!  F"ur Leute, die nicht unter DOS arbeiten wollen
oder akute Speicherprobleme haben, gibt es sowohl eine DOS-DPMI- als auch
eine OS/2-Version.  Eine C-Version, die auf einer Reihe von UNIX-Systemen 
(inkl. Linux) "ubersetzbar ist, ist in Arbeit, ist aber noch
nicht 100\% ig ausgereift.
\ifelektor
Die DOS-DPMI-Version ist in diesem Paket aus Platzgr"unden nicht enthalten;
Sie kann bei mir gegen R"uckumschlag und Diskette angefordert werden.
\else
Da aber nicht jeder diese Versionen braucht, gebe ich sie in getrennten 
Paketen aus.
\fi
...
Da AS ein reines DOS-Program ist, l"auft er prinzipiell auf allen 
IBM-kompatiblen PCs, angefangen vom PC/XT mit vierkommawenig Megaherz 
bis hin zum Pentium.  Wie bei vielen anderen Programmen aber auch, 
steigt der Lustgewinn mit der Hardware-Ausstattung.  So d"urfte ein 
XT-Benutzer ohne Festplatte erhebliche Probleme haben, die  "uber 500 
kByte gro"se Overlay-Datei von AS auf einer Diskette unterzubringen...eine
Festplatte sollte der PC also schon haben, allein um vern"unftige 
Ladezeiten zu erreichen.  Im Hauptspeicherbedarf ist AS recht gen"ugsam: 
Das Programm selber belegt knapp 300 kByte Hauptspeicher, AS sollte 
also ab einer Hauptspeichergr"o"se von 512 kByte ausf"uhrbar sein.

%%---------------------------------------------------------------------------

\section{Installation}

Eine besondere Installation ist f"ur die Nutzung von AS nicht erforderlich,
es gen"ugt, alle EXE- sowie OVR-Dateien in ein Verzeichnis kopieren, das 
in der \tty{PATH}-Variable auftaucht.  Ob man ein bestehendes Verzeichnis 
nutzt oder ein neues anlegt, ist dabei egal.  Die Dokumentation, 
Beispielprogramme sowie Includefiles k"onnen v"ollig nach belieben 
verteilt werden...als Beispiel hier eine Installation, die ein UNIX-Anh"anger 
vornehmen w"urde:
\par
...
und Dateiangaben sind in der Kommandozeile anzugeben.  Um den
Speicherbedarf nicht ins Unerme"sliche zu steigern, wurden die
verschiedenen Codegeneratormodule in eine Overlaydatei verlegt, deren
Existenz der Assembler sofort nach Programmstart pr"uft.  Findet er
diese nicht, so ist der Programmlauf auch schon an dieser Stelle
zu Ende...die Datei AS.OVR sollte sich immer im gleichen Verzeichnis wie
das EXE-File befinden.
\par
Die Verwendung von Overlays ergibt naturgem"a"s etwas Overhead.  AS versucht
...
f"uhren, so k"onnen Sie die Verwendung von EMS bzw. XMS unterbinden,
indem Sie einer Environment-Variablen \tty{USEXMS} bzw. \tty{USEEMS} den
...
\par
Da AS alle Ein-und Ausgaben "uber DOS abwickelt (und daher auch auf nicht
ganz so kompatiblen PC's laufen sollte) und eine rudiment"are
Bildschirmsteuerung ben"otigt, gibt er w"ahrend der Assemblierung
ANSI-Steuersequenzen aus.  Falls Sie in den Ausgaben von AS also seltsame
Zeichen sehen sollten, fehlt offensichtlich in Ihrer CONFIG.SYS die
Einbindung des Ansi-Treibers (\tty{device=ansi.sys}).
die weitere Funktion von AS wird dadurch aber nicht beeinflu"st.
Alternativ k"onnen Sie aber auch die Ausgabe von ANSI-Sequenzen durch
das Setzen der Environment-Variablen \tty{USEANSI} auf \tty{n} ganz
unterdr"ucken.
\par
...
geschrieben h"atte, w"urde AS die Buchstaben q, u, e, i und t als
einzelne Schalter auffassen.  Mehrbuchstabige Schalter unterscheiden
sich weiterhin von einbuchstabigen dadurch, da"s AS bei Ihnen beliebige
Gro"s-und Kleinschreibungen akzeptiert, w"ahrend einbuchstabige
Schalter je nach Gro"s- oder Kleinschreibung unterschiedliche Bedeutung
haben.
\par
...
\item{\tty{p}: SHARED-Variablen werden in einem Format abgelegt, das die
      Einbindung in den \tty{CONST}-Block eines Pascal-Programmes erlaubt.
      Die Endung der Datei ist \tty{INC}.}
...
\item{\tty{x}: So vorhanden, wird bei Fehlern noch eine erweiterte Meldung
      ...
      tragen k"onnen, steht im Anhang mit der Liste aller
      Fehlermeldungen.}
\item{\tty{n}: Wird diese Option angegeben, so werden Fehlermeldungen nicht nur
      mit ihrem Klartext, sondern auch mit ihren im Anhang genannten
      internen Nummern ausgegeben.  Diese Option ist prim"ar f"ur Shells
      und Entwicklungsumgebungen gedacht, denen mit diesen Nummern die
      Identifizierung von Fehlern erleichtert werden soll.}
...
entsprechenden dezimalen ASCII-Wert.  So kann man mit \tt\verb!\0! \rm
...
\caption{vordefinierte Funktionen in AS - Teil 2
         (String-Funktionen)\label{TabFuncs2}}
\end{table*}
...
Typkonvertierung statt.  Umgekehrt mu"s allerdings die INT-Funktion
...
eingebaute Funktion.
\par
F"ur die korrekte Umwandlung von Klein-zu Gro"sbuchstaben ist
eine DOS-Version $\geq$ 3.30 erforderlich.
...
Typs im CPU-Befehl wird sich AS also wie beim ,,Original'' verhalten,
...
Nicht f"ur alle Prozessoren sind alle Pseudobefehle definiert.  In
Klammern ist jeweils vermerkt, f"ur welche Prozessortypen dieser
Befehl erlaubt ist.
...
\subsection{SET und EQU (alle Prozessoren)}
...
\par
Mit \tty{EQU/SET} lassen sich sowohl Gleitkomma-als auch Integerkonstanten
definieren, z.B.
\begin{verbatim}
IntZwei   EQU   2
FloatZwei EQU   2.0
...
(\tty{CODE, DATA, IDATA, XDATA, YDATA, BITDATA, IO}) oder \tty{MOMSEGMENT}
f"ur das aktuell gesetzte Segment angegeben werden, um das Symbol einem
bestimmten Adre"sraum zuordnen.

%%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

\subsection{SFR und SFRB (diverse , SFRB nur MCS-51)}
...
\subsection{XSFR und YSFR (DSP56000)}
...
jedoch andererseits dazu, den normalen \tt SFR\rm -Befehl in die beiden
...
\subsection{LABEL (alle Prozessoren)}
\ttindex{LABEL}
...
\subsection{BIT (MCS-(2)51, XA, 80C166, 75K0)}
\ttindex{BIT}
...
\subsection{PORT (8080/8085/8086, XA, Z80, 320xx, TLCS-47, AVR)}
\ttindex{PORT}
...
\subsection{CHARSET (alle Prozessoren)}
\ttindex{CHARSET}
...
\subsection{ENUM (alle Prozessoren)}
\ttindex{ENUM}
...
\subsection{PUSH und POPV (alle Prozessoren)}
...
\subsection{ORG (alle Prozessoren)}
...
und vom Prozessortyp abh"angig (Tabellen \ref{TabORG1} bis \ref{TabORG3}).
...
TLCS-9000 & 16M       & ------    & ------    & ------    & ------    & ------  & ------ \\
          & Init 0    &           &           &           &           &         &        \\
\hline
\multicolumn{8}{|l|}{*) Da der 8051 kein RAM jenseits 80h hat, mu"s der Initialwert f"ur den 8051} \\
\multicolumn{8}{|l|}{   als Zielprozessor auf jeden Fall mit \tty{ORG} angepa"st werden!!} \\
\hline
\multicolumn{8}{|l|}{+) Da der Z180 weiterhin logisch nur 64K ansprechen kann, ist der } \\
\multicolumn{8}{|l|}{   ganze Adre"sraum nur mittels \tty{PHASE}-Anweisungen erreichbar!} \\
\hline
\end{tabular}\end{center}
...
\subsection{CPU (alle Prozessoren)}
\ttindex{CPU}
...
68030 dar, dem 2 Befehle fehlen: \tty{CALLM} und \tty{RTM}.  Die beiden
...
%%-----------
\begin{quote}
q) 87C750 $\rightarrow$ 8051, 8052, 80C320 und 80515 $\rightarrow$ 80C251
\end{quote}
...
drei mittleren Prozessoren nimmt AS selber "uberhaupt keine Unterscheidung
vor, sondern verwaltet den Unterschied lediglich in der Variablen
\tty{MOMCPU} (s.u.), die man mit \tty{IF}-Befehlen abfragen kann. Der
...
das gr"o"sere interne RAM anzusprechen.
%%-----------
\begin{quote}
s) 8080 und 8085
\end{quote}
...
%%-----------
\begin{quote}
v) AT90S1300 $\rightarrow$ AT90S2312 $\rightarrow$ AT90S8414
\end{quote}
...
%%-----------
\begin{quote}
x) 80C166 $\longrightarrow$ 80C167
\end{quote}
Der 80C167 hat anstelle 256 KByte max. 16 MByte Adre"sraum, au"serdem
kennt er einige zus"atzliche Befehle f"ur erweiterte Adressierungsmodi
...
%%-----------
\begin{quote}
al) 320C25 $\rightarrow$ 320C26 $\rightarrow$ 320C28
\end{quote}
...
\subsection{SUPMODE, FPU, PMMU (680x0, FPU auch 80x86, SUPMODE
            auch TLCS-900, SH7000, 29K, XA und PowerPC)}
...
\item{\tty{PMMU}: erlaubt bzw. sperrt die Befehle der Speicherverwaltungseinheit
      68851 bzw. der im 68030 eingebauten MMU.  \bf ACHTUNG! \rm Die 68030-MMU
      erlaubt nur eine rel. kleine Untermenge der 68851-Befehle.  Der
      Assembler nimmt hier keine Pr"ufung vor!  Die Statusvariable hei"st
      \tty{PMMUAVAIL}.}
\end{itemize}
...
\subsection{PADDING (680x0, XA, H8, SH7000)}
\ttindex{PADDING}
...
\subsection{MAXMODE (TLCS-900, H8)}
\ttindex{MAXMODE}
...
\subsection{EXTMODE und LWORDMODE (Z380)}
...
\subsection{BIGENDIAN (MCS-51/251, PowerPC)}
\ttindex{BIGENDIAN}
...
\subsection{SEGMENT (alle Prozessoren)}
\ttindex{SEGMENT}
...
verschiedene Befehle zur Ansprache ben"otigen.  Um auch diese verwalten
zu k"onnen (obwohl kein Code in ihnen erzeugt werden kann), stellt der
Assembler mehrere Programmz"ahler zur Verf"ugung, zwischen denen mit dem
...
\par
Um es noch einmal klarzustellen: Alle Segmente au"ser Code dienen
lediglich der Adre"sverwaltung, d.h. man kann keinen Code oder
Konstanten darin definieren.  Zul"assig sind nur Befehle, die
Speicher reservieren (\tty{DS, DB ?...}).
\par
...
\subsection{PHASE und DEPHASE (alle Prozessoren)}
...
\subsection{SAVE und RESTORE (alle Prozessoren)}
...
Seite existiert ein Seitenregister (bezeichnet als \tty{DPP0...DPP3}),
welche der physikalischen 16/1024 Seiten dort eingeblendet wird.  AS versucht
...
\subsection{DC[.size] (680x0, H8, SH7000, DSP56000, XA)}
\ttindex{DC}
...
\subsection{DS[.size] (680x0, H8, SH7x00, DSP56000, XA)}
\ttindex{DS}
...
\subsection{DB,DW,DD,DQ \& DT (Intel, Zilog, Toshiba, NEC, TMS370, Siemens, AMD,
                               M16(C), COP8}
...
\par
\tty{DUP} ist die einzige Stelle, an der man mit einer anderen Grenze des
...
\subsection{DS (Intel, Zilog, Toshiba, NEC, TMS370, Siemens, AMD, 
            M16(C), COP8)}
\ttindex{DS}
...
\subsection{BYT oder FCB (6502,68xx)}
...
\subsection{BYTE (ST62xx, 320C2x, 320C5x, MSP)}
\ttindex{BYTE}
...
\subsection{ADR oder FDB (6502,68xx)}
...
\subsection{WORD (ST62xx, 320C2x, 320C3x, 320C5x, MSP)}
\ttindex{WORD}
...
\subsection{SINGLE und EXTENDED (320C3x)}
...
\subsection{FLOAT und DOUBLE (320C2x, 320C5x)}
...
\subsection{EFLOAT, BFLOAT, TFLOAT (320C2x/320C5x)}
...
\subsection{Qxx und LQxx (320C2x, 320C5x)}
...
\subsection{DATA (PIC, 320xx, AVR, MELPS-4500)}
\ttindex{DATA}
...
\subsection{ASCII und ASCIZ (ST62xx)}
...
\subsection{STRING und RSTRING (320C2x, 320C5x)}
\ttindex{STRING}\ttindex{RSTRING}

Diese Befehle funktionieren analog zu {\tt DATA}, jedoch werden
...
\subsection{DFS oder RMB (6502,68xx)}
...
\subsection{RES (PIC, MELPS-4500, 3201x, 320C2x, 320C5x, AVR)}
\ttindex{RES}
...
\subsection{BSS (320C2x, 320C3x, 320C5x, MSP)}
\ttindex{BSS}
...
\subsection{ALIGN (alle Prozessoren)}
\ttindex{ALIGN}
...
\section{Makrobefehle (alle Prozessoren)}
...
\item{\tty{GLOBAL[:Sektionsname]} : legt fest, da"s neben diesem
      ...
      Namen \tty{C\_B\_A} erzeugt.  W"urde dagegen \tty{B} als Zielsektion
...
\section{bedingte Assemblierung (alle Prozessoren)}
...
\tty{CASE} ist ein Spezialfall von \tty{IF} und f"ur den Fall gedacht, da"s ein
...
\section{Listing-Steuerung (alle Prozessoren)}
...
spezifizierten Zeile nicht mitgerechntet werden.  Der Minimalwert f"ur
...
(0=\tty{OFF}, 1=\tty{ON}, 2=\tty{NOSKIPPED}, 3={PURECODE}) auslesen.
...
\section{lokale Symbole (alle Prozessoren)}
\label{ChapLocSyms}
...
erreichen.  Damit sind wir auch schon beim passenden Thema:
...
Als Spezialwert f"ur \tt $<$Sektion$>$ \rm ist \tt PARENT \rm  zugelassen,
dieser Wert bedeutet, das Symbol genau eine Ebene hinaufzuexportieren.
...
Rechenzeit.  Ein 1800 Zeilen langes 8086-Progamm z.B. wurde nach der
...
\subsection{SHARED (alle Prozessoren)}
...
\subsection{INCLUDE (alle Prozessoren)}
...
\subsection{BINCLUDE (alle Prozessoren)}
\ttindex{BINCLUDE}
...
\subsection{MESSAGE, WARNING, ERROR und FATAL (alle Prozessoren)}
...
\end{itemize}
Allen vier Befehlen ist das Format gemeinsam, in dem die Fehlermeldung
...
\subsection{READ (alle Prozessoren)}
\ttindex{READ}
...
\subsection{RELAXED (alle Prozessoren)}
\label{SectRELAXED}
\ttindex{RELAXED}
...
\subsection{END (alle Prozessoren)}
\ttindex{END}
...
einmal nicht in einer Hexzahl darstellen, und dann habe ich die Buchstaben
...
\par
Die Unterst"utzung ist momentan noch nicht vollst"andig: es fehlen
sowohl ein Satz an Pseudobefehlen zur Datenablage sowie die etwas
ungew"ohnlicheren, in \cite{Mot601} genannten RS6000-Befehle (die aber
hoffentlich keiner vermi"st...).  Das wird aber nachgeholt, sobald
Informationen verf"ugbar sind!
...
die nach M"oglichkeit nie pa"st.
\par
...
weil AS f"ur die jetzt als fehlerhft erkannten Befehle keinen Code mehr
...
"ubrigens auch f"ur die 740er favorisierte Intel/IEEE-Schreibweise.
Ich halte erstere auch f"ur die bessere Schreibweise, und die Entwickler
des 65816 werden dies vermutlich "ahnlich gesehen haben.  Ein f"ur die
...
\tt SEL\rm -Befehle sollte daher nicht notwendig sein (obwohl sie m"oglich
...
Dem Assembler liegt eine Datei STDDEF51.INC bei, in der alle Bits und SFRs
der Prozessoren 8051, 8052 und 80515 verzeichnet sind.  Je nach Einstellung
des Prozessortyps mit dem \tty{CPU}-Befehl wird dabei die korrekte Untermenge
eingebunden, die richtige Reihenfolge f"ur den Anfang eines Programmes ist
daher
\begin{verbatim}
	CPU     <Prozessortyp>
	INCLUDE stddef51.inc   ,
...
etwas abzuhelfen, ist in der Include-Datei das Makro \tt USING \rm definiert,
...
KByte gro"ses Datensegment mit nicht initialisierten Daten.  Zwischen diesen
beiden Segmenten kann mit dem \tty{SEGMENT}-Befehl hin-und hergeschaltet werden. 
...
aus dem aktuellen Datensegment.  Dis bedeutet aber, da"s diese Adressen
...
wird verworfen, was an einem kleinen \tty{R} im Listing zu erkennen
ist.
...
\begin{verbatim}
        ld      wa',wa
\end{verbatim}
erkennt AS z.B. weder das Komma zur Parametertrennung, noch wandelt er den
zweiten Parameter im Gro"sbuchstaben um.  Dieses Problem kann man aber umgehen,
...
aussehen wird...Bei Codezugriffen arbeitet die Pagingeinheit leider nicht,
...
msb     bit     r5.15
        ...
        bclr    msb+1
\end{verbatim}
F"ur den 80C167 ist der SFR-Bereich verdoppelt worden; da"s ein Bit im
...
in dem steht, wie man mit diesem Befehl einigerma"sen unfallfrei Konstanten
...
zuzugreifen (Ablage im Code ist notwendig, weil AS davon ausgeht, da"s
ein Datensegment aus RAM besteht, das in einem Standalone-System nach
dem Einschalten keinen definierten Inhalt hat und verweigert in
Segmenten au"ser Code deshalb die Ablage von Daten).  Ohne dieses Feature
w"urde AS nun jeden Zugriff auf die abgelegten Daten mit einer Warnung
(,,Symbol aus falschem Segment'') quittieren.  Im einzelnen erzeugen
folgende Pseudobefehle untypisierte Labels:
\begin{quote}\tt
...
\begin{itemize}
 ...
 \item{indirekte Zugriffe der Form Adr.@L (0FC0H $<=$ Adr $<=$ 0FFFH)}
...
\cleardoublepage
\chapter{Dateiformat}\label{ChapCodeFormat}
...
gilt sinngem"a"s das gleiche wie in 1.1. !
...
\begin{itemize}
\item{\tt f $<$Header[,Header...]$>$\rm: gibt eine Liste von Recordtypen
      an, die kopiert werden sollen.  Alle anderen Records werden
      nicht kopiert.  Ohne diese Angabe werden alle Records kopiert.
      Die in der Liste angegebenen entsprechen dem Typfeld in der
      Recordstruktur, wie es in Punkt 4 beschrieben wurden. Die
      einzelnen Typnummern in der Liste werden durch Kommas getrennt.}
\end{itemize}
...
so erscheinen die verschiedenen Hexformat auch gemischt in der Zieldatei
...
\item[60]  {Distanz 0 nicht bei Kurzsprung erlaubt (NOP erzeugt)
            ...
            \item[Ursache]{:\\Ein kurzer Sprung mit der Distanz 0 ist bei
                 680x0-Prozessoren nicht erlaubt, da dieser Sonderwert f"ur
...
\item[240] {NUL-Zeichen in Strings, Ergebnis undefiniert
	    ...
	    \item[Ursache]{:\\Eine String-Konstante enth"alt ein 
            ...
            w"are fr C an dieser Stelle zu Ende...}
	    \item[Argument]{:\\keines}
            \end{description}}
...
\item[A:]{Ja und Nein.  Die Symboltabelle f"ur Ihr Programm ist nur
	  ...
	  einmal mit dem \tty{-A}-Kommandozeilenschalter.}
\vspace{0.3cm}
...
etwas zum bisherigen Gelingen dieses Projektes beigetragen hat.  Der 
...
hat hat sich sehr engagiert, um eine praxisnahe L"osung im ZAM zu 
finden.
\par
...
\par
Der TMS320C2x/5x-Codegenerator sowie die Datei STDDEF2x.INC stammen von
Thomas Sailer, ETH Z"urich.  Erstaunlich, an einem Wochenende hat er
es geschafft, durch meinen Code durchzusteigen und den neuen Generator
zu implementieren.  Entweder waren das reichliche Nachtschichten
...
\item{Version 1.37:
      ...
      \item{vordefinierte Symbole d"urfen nun auch mit einem frei w"ahlbaren
            Wert belegt werden (p5).}
      \end{itemize}}
\item{Version 1.38:
      \begin{itemize}
...
\item{Version 1.41:
      ...
      \item{es wird beim 680x0 jetzt zwischen dem vollen und eingeschr"ankten 
            Befehlssatz unterschieden; eine manuelle Umschaltung ist mit dem 
...
daá"sich ohne Festplatte schon bei Turbo-C mit 4 Disketten jonglieren
...
machte die Sache nicht attraktiver.  Auáerdem kannte ich
...
\item{Die Strukturierung oberhalb der Funktionsebene hat
      ...
      Spracherweiterungen wie in CNU-C hier auch nicht richtig
      weiter!}
...
\end{itemize}
All das f"uhrte dazu, daá ich Turbo-Pascal zur Implementierung von
AS benutzte, und bis heute ist das so geblieben.  Die Zeiten haben
sich aber ge„ndert: Mit der Einstellung der Turbo/Borland-Pascal-Linie
...
Programmen taugt, die aus 90% Oberfl"ache bestehen, fr ein
...
Steuerung der einzelnen Durchl"aufe, das Einlesen der Quelldateien, das
Schreiben der Code-Datei sowie den Makroprozessor.  Dieser Programmteil ist
...
Konstanten und gemeinsam benutzten Variablen.  Wichtig f"ur Erweiterungen
sind die hier definierten Listen mit den vorhandenen Prozessoren und ihren
Namen.  Die Nummer, die f"ur einen Prozessortyp unter dem Symbol \tt MOMCPU
\rm abfragbar ist, leitet AS automatisch aus dem Namen ab, deshalb findet
sich hierf"ur keine Liste.
...
hergeschaltet wird.  Von diesem Modul wird im sp"ateren Verlauf noch einmal
die Rede sein.
...
zum anderen die Pendants f"ur die 8-Bitter von Motorola oder Rockwell.
...
Dieses Modul implemetiert den Mechanismus der Kommdozeilenparameter.  Es
...
\item{Soll der Prozessorname vollst"andig in der Variablen \tty{MOMCPU}
      ...
      werden von AS bei der Eingabe in Gro"sbuchtaben umgewandelt
...
\par
Sollte das Codesegment eine andere Granularit"at als 1 Byte/Adresse
