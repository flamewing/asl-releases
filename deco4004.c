...
static tOpcodeList const OpcodeList[256] = {
...
static tOpcodeList const DummyOpcode = {eUnknown, 0, 0, NULL};
...
static char const* MakeSymbolic(
        Word Address, int AddrLen, char const* pSymbolPrefix, char* pBuffer,
        int BufferSize) {
    char const* pResult;
...
static void Disassemble_4004(
    ...
    case eFullAddr:
        ...
        pOp            = MakeSymbolic(
                OpAddr, 2, strcmp(pOpcode->Memo, "jun") ? "lab_" : "sub_", NumBuf,
                sizeof(NumBuf));
    ...
    case eJumpCond:
        ...
        as_snprintf(
                pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\t%s,%s", pOpcode->Memo,
                NumBuf2, pOp);
        break;
    case eISZ:
        if (!RetrieveData(Address + 1, Data, 1)) {
        ...
        pOp            = MakeSymbolic(OpAddr, 2, "lab_", NumBuf, sizeof(NumBuf));
        as_snprintf(
                pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\tr%u,%s", pOpcode->Memo,
                Opcode & 15U, pOp);
        break;
    case eFIM:
        if (!RetrieveData(Address + 1, Data, 1)) {
        ...
        as_snprintf(
                pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\tr%up,%s", pOpcode->Memo,
                (Opcode & 15U) >> 1, NumBuf);
        break;
    case eOneReg:
        pInfo->CodeLen = 1;
        as_snprintf(
                pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\tr%u", pOpcode->Memo,
                Opcode & 15U);
        break;
    case eOneRReg:
        pInfo->CodeLen = 1;
        as_snprintf(
                pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\tr%up", pOpcode->Memo,
                (Opcode & 15U) >> 1);
        break;
    case eImm4:
        pInfo->CodeLen = 1;
        ...
        as_snprintf(
                pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\t%s", pOpcode->Memo, NumBuf);
        break;
    default:
        if (DataSize < 0) {
            DataSize = 1;
