...
static int to16(Word* pOut, t64 const* pIn) {
    int res = 0;
...
static void from16(t64* pOut, Word const* pIn) {
    pOut->low  = pIn[1];
...
static int divmnu(Word q[], Word r[], Word const u[], Word const v[], int m, int n) {
    unsigned const b = 65536;      /* Number base (16 bits). */
...
void add64(t64* pRes, t64 const* pA, t64 const* pB) {
...
void sub64(t64* pRes, t64 const* pA, t64 const* pB) {
    pRes->low  = pA->low - pB->low;
    pRes->high = pA->high - pB->high - !!(pRes->low > pA->low);
}
...
void mul64(t64* pRes, t64 const* pA, t64 const* pB) {
    Word u[4], v[4], w[8];
    int  m = to16(u, pA), n = to16(v, pB);
...
void div64(t64* pRes, t64 const* pA, t64 const* pB) {
    Word u[4], v[4], q[4];
    int  m = to16(u, pA), n = to16(v, pB);
