...
static char const CondNames[][3] = {"CC", "GE", "NE", "PL", "NN", "EC", "LC", "GT", "CS",
                                    "LT", "EQ", "MI", "NR", "ES", "LS", "LE", "HS", "LO"};

static Byte const MacTable[4][4] = {
        {0,    2, 5,    4},
        {2, 0xff, 6,    7},
        {5,    6, 1,    3},
        {4,    7, 3, 0xff}
...
static Byte const Mac4Table[4][4] = {
        { 0, 13, 10,  4},
        { 5,  1, 14, 11},
        { 2,  6,  8, 15},
        {12,  3,  7,  9}
...
static Byte const Mac2Table[4] = {1, 3, 2, 0};
...
#define MModNoExt                                                               \
    (MModIReg | MModPreDec | MModPostDec | MModPostInc | MModIndex | MModModDec \
     | MModModInc)
...
static tStrComp LeftComp, MidComp, RightComp, Left1Comp, Left2Comp, Right1Comp,
        Right2Comp;
...
static void SplitArg(tStrComp const* pOrig, tStrComp* pLDest, tStrComp* pRDest) {
    char* p;
...
static unsigned CutSize(tStrComp const* pArg, Byte* ShortMode) {
...
static Boolean DecodeALUReg(
        char* Asc, LongInt* Erg, Boolean MayX, Boolean MayY, Boolean MayAcc) {
...
#define RegCount (sizeof(RegNames) / sizeof(*RegNames))
    static char const RegNames[][4] = {"A10", "B10", "X", "Y", "A", "B", "AB", "BA"};
    Word              z;
...
#define RegCount (sizeof(RegNames) / sizeof(*RegNames))
    static char const RegNames[][3] = {"B", "A", "X", "Y", "X0", "Y0", "X1", "Y1"};
    Word              z;
...
#define RegCount (sizeof(RegNames) / sizeof(*RegNames))
    static char const RegNames[][3] = {"A0", "B0", "X0", "Y0", "X1", "Y1"};
    Word              z;
...
#define RegCount (sizeof(RegNames) / sizeof(*RegNames))
    static char const RegNames[][3] = {"A1", "B1", "X0", "Y0", "X1", "Y1"};
    Word              z;
...
#define RegCount (sizeof(RegNames) / sizeof(*RegNames))
    /** vvvv ab 56300 ? */
    static char const RegNames[][4] = {"SZ", "SR", "OMR", "SP", "SSH", "SSL", "LA", "LC"};
    Word              z;
...
static Boolean DecodeAddReg(char* Asc, LongInt* Erg) {
    if ((strlen(Asc) == 2) && (as_toupper(*Asc) == 'M') && (Asc[1] >= '0')
        && (Asc[1] <= '7')) {
        *Erg = Asc[1] - '0';
        return True;
    }
...
static void DecodeAdr(tStrComp const* pArg, Word Erl, Byte ErlSeg, tAdrResult* pResult) {
    static char const ModMasks[ModModInc + 1][8] = {
            "", "", "(Rx)", "-(Rx)", "(Rx)-", "(Rx)+", "(Rx+Nx)", "(Rx)-Nx", "(Rx)+Nx"};
    static Byte const ModCodes[ModModInc + 1] = {0, 0, 4, 7, 2, 3, 5, 0, 1};
#define Seg56KCount (sizeof(Seg56KNames) / sizeof(*Seg56KNames))
    static char const Seg56KNames[] = {'P', 'X', 'Y', 'L'};
    static Byte       Seg56KVals[]  = {SegCode, SegXData, SegYData, SegLData};
    int               z, l, ArgLen;
    ...
    for (SegIndex = 0; SegIndex < Seg56KCount; SegIndex++) {
        if ((as_toupper(*pArg->str.p_str) == Seg56KNames[SegIndex])
            && (pArg->str.p_str[1] == ':')) {
    ...
    if (IsIndirect(Arg.str.p_str)) {
        ...
        if (pp) {
            save = *pp;
            *pp  = '\0';
            if ((DecodeGeneralReg(IArg.str.p_str, &pResult->Mode))
                && (pResult->Mode >= 16) && (pResult->Mode <= 23)) {
                *pp = save;
                pResult->Mode -= 16;
                pResult->Val = EvalStrIntExpressionOffsWithResult(
                        &IArg, pp - IArg.str.p_str, Int24, &EvalResult);
                if (EvalResult.OK) {
...
static Boolean DecodeOpPair(
        tStrComp const* pLeft, tStrComp const* pRight, Byte WorkSeg, LongInt* Dir,
        LongInt* Reg1, LongInt* Reg2, tAdrResult* pResult) {
    Boolean Result = False;

    if (DecodeALUReg(
                pLeft->str.p_str, Reg1, WorkSeg == SegXData, WorkSeg == SegYData, True)) {
        if (DecodeALUReg(
                    pRight->str.p_str, Reg2, WorkSeg == SegXData, WorkSeg == SegYData,
                    True)) {
            *Dir          = 2;
            pResult->Type = ModNone;
            pResult->Mode = 0;
...
static Boolean DecodeTFR(tStrComp const* pArg, LongInt* Erg) {
    LongInt Part1, Part2;

    SplitArg(pArg, &LeftComp, &RightComp);
    if (!DecodeALUReg(RightComp.str.p_str, &Part2, False, False, True)) {
        return False;
    }
    if (!DecodeReg(LeftComp.str.p_str, &Part1)) {
        return False;
    }
    if ((Part1 < 4) || ((Part1 > 7) && (Part1 < 14)) || (Part1 > 15)) {
        return False;
    }
    if (Part1 > 13) {
        if (((Part1 ^ Part2) & 1) == 0) {
...
static Boolean DecodeRR(tStrComp const* pArg, LongInt* Erg) {
    LongInt Part1, Part2;

    SplitArg(pArg, &LeftComp, &RightComp);
    if (!DecodeGeneralReg(RightComp.str.p_str, &Part2)) {
        return False;
    }
    if ((Part2 < 16) || (Part2 > 23)) {
        return False;
    }
    if (!DecodeGeneralReg(LeftComp.str.p_str, &Part1)) {
        return False;
    }
    if ((Part1 < 16) || (Part1 > 23)) {
        return False;
    }
...
static Boolean DecodeMOVE_1(int Start) {
    ...
    if (*RightComp.str.p_str == '\0') {
        ...
        DecodeAdr(
                &LeftComp, MModPostDec | MModPostInc | MModModDec | MModModInc, 0,
                &AdrResult);
            ...
            if (AdrResult.Seg != SegLData) {
                ...
                if (AdrResult.Type == ModDisp) {
                    if ((AdrResult.Val < 63) && (AdrResult.Val > -64) && (RegErg <= 15)) {
                        ...
                        !AdrResult.ForceImmLong && (AdrResult.Type == ModImm)
                        && RangeCheck(AdrResult.Val, UInt8)) {
                    Result = True;
                    DAsmCode[0]
                            = 0x200000 + (RegErg << 16) + ((AdrResult.Val & 0xff) << 8);
                    CodeLen = 1;
                } else if (
                        (AdrResult.Type == ModAbs) && (AdrResult.Val <= 63)
                        && (AdrResult.Val >= 0) && (AdrResult.ShortMode != 2)) {
                    Result      = True;
    ...
    if (DecodeReg(LeftComp.str.p_str, &RegErg)) {
        ...
        if (RightAdrResult.Seg != SegLData) {
            ...
            if (RightAdrResult.Type == ModDisp) {
                if ((RightAdrResult.Val < 63) && (RightAdrResult.Val > -64)
                    && (RegErg <= 15)) {
                    ...
                    (RightAdrResult.Type == ModAbs) && (RightAdrResult.Val <= 63)
                    && (RightAdrResult.Val >= 0) && (RightAdrResult.ShortMode != 2)) {
                Result      = True;
    ...
    if (DecodeLReg(RightComp.str.p_str, &RegErg)) {
        ...
        if ((LeftAdrResult.Type == ModAbs) && (LeftAdrResult.Val <= 63)
            && (LeftAdrResult.Val >= 0) && (LeftAdrResult.ShortMode != 2)) {
            Result      = True;
    ...
    if (DecodeLReg(LeftComp.str.p_str, &RegErg)) {
        ...
        MixErg = ((RegErg & 4) << 17) + ((RegErg & 3) << 16);
        if ((RightAdrResult.Type == ModAbs) && (RightAdrResult.Val <= 63)
            && (RightAdrResult.Val >= 0) && (RightAdrResult.ShortMode != 2)) {
            Result      = True;
            DAsmCode[0] = 0x400000 + MixErg + (RightAdrResult.Val << 8);
            CodeLen     = 1;
...
static Boolean DecodeMOVE_2(int Start) {
    ...
    if (!as_strcasecmp(Left2Comp.str.p_str, "X0")) {
        if (!DecodeALUReg(Right2Comp.str.p_str, &RegErg, False, False, True)) {
            WrError(ErrNum_InvAddrMode);
        } else if (strcmp(Left1Comp.str.p_str, Right2Comp.str.p_str)) {
            WrError(ErrNum_InvAddrMode);
        } else {
    ...
    if (!as_strcasecmp(Left1Comp.str.p_str, "Y0")) {
        if (!DecodeALUReg(Right1Comp.str.p_str, &RegErg, False, False, True)) {
            WrError(ErrNum_InvAddrMode);
        } else if (strcmp(Left2Comp.str.p_str, Right1Comp.str.p_str)) {
            WrError(ErrNum_InvAddrMode);
        } else {
    ...
    if ((DecodeOpPair(
                &Left1Comp, &Right1Comp, SegXData, &Dir1, &Reg1L, &Reg1R, &AdrResult1))
        && (DecodeOpPair(
                &Left2Comp, &Right2Comp, SegYData, &Dir2, &Reg2L, &Reg2R, &AdrResult2))) {
        if ((Reg1R == -1) && (Reg2R == -1)) {
            if ((AdrResult1.Mode >> 3 < 1) || (AdrResult1.Mode >> 3 > 4)
                || (AdrResult2.Mode >> 3 < 1) || (AdrResult2.Mode >> 3 > 4)) {
                WrError(ErrNum_InvAddrMode);
            } else if (((AdrResult1.Mode ^ AdrResult2.Mode) & 4) == 0) {
                WrError(ErrNum_InvRegPair);
            } else {
                DAsmCode[0] = 0x800000 + (Dir2 << 22) + (Dir1 << 15) + (Reg1L << 18)
            ...
            if ((Reg2L < 2) || (Reg2R > 1)) {
                ...
                DAsmCode[0] = 0x100000 + (Reg1L << 18) + ((Reg2L - 2) << 17)
                              + (Reg2R << 16) + (Dir1 << 15) + (AdrResult1.Mode << 8);
                DAsmCode[1] = AdrResult1.Val;
                CodeLen     = 1 + AdrResult1.Cnt;
                Result      = True;
            }
            ...
            if ((Reg1L < 2) || (Reg1R > 1)) {
                ...
                DAsmCode[0] = 0x104000 + (Reg2L << 16) + ((Reg1L - 2) << 19)
                              + (Reg1R << 18) + (Dir2 << 15) + (AdrResult2.Mode << 8);
                DAsmCode[1] = AdrResult2.Val;
                CodeLen     = 1 + AdrResult2.Cnt;
                Result      = True;
            }
            ...
            for (int z = 1; OK && (z <= ArgCnt); z++) {
                ...
                case TempInt:
                    if (mFirstPassUnknown(t.Flags)) {
                        t.Contents.Int &= 0xffffff;
                    }
                    if (!(OK = RangeCheck(t.Contents.Int, Int24))) {
...
static tStrComp const* pErrComp;
...
static void SetXError(tErrorNum Code, tStrComp const* pNewErrComp) {
    ErrCode  = Code;
    pErrComp = pNewErrComp;
}

static void PrError(void) {
    if (pErrComp) {
        WrStrErrorPos(ErrCode, pErrComp);
    } else if (ErrCode != 0) {
        WrError(ErrCode);
    }
}
...
static void DecodePar(Word Index) {
    ParOrder const* pOrder = ParOrders + Index;
    Boolean         OK, DontAdd;
    tSymbolFlags    Flags;
    ...
    if (DecodeMOVE(2)) {
        ...
        switch (pOrder->Typ) {
        case ParAB:
            if (!DecodeALUReg(ArgStr[1].str.p_str, &Reg1, False, False, True)) {
                SetXError(ErrNum_InvReg, &ArgStr[1]);
            } else {
                h = Reg1 << 3;
            }
            break;
        case ParFixAB:
            if (as_strcasecmp(ArgStr[1].str.p_str, "A,B")) {
                SetError(ErrNum_InvRegPair);
            } else {
                h = 0;
            }
            break;
        case ParABShl1:
            if (!strchr(ArgStr[1].str.p_str, ',')) {
                ...
                SetError(ErrNum_ParNotPossible);
            } else if (MomCPU < CPU56300) {
                SetError(ErrNum_InstructionNotSupported);
            } else {
                ...
                if (!DecodeALUReg(RightComp.str.p_str, &Reg1, False, False, True)) {
                    SetXError(ErrNum_InvReg, &RightComp);
                } else if (!DecodeALUReg(MidComp.str.p_str, &Reg2, False, False, True)) {
                    SetXError(ErrNum_InvReg, &MidComp);
        ...
        case ParXYAB:
            SplitArg(&ArgStr[1], &LeftComp, &RightComp);
            if (!DecodeALUReg(RightComp.str.p_str, &Reg2, False, False, True)) {
                SetXError(ErrNum_InvReg, &RightComp);
            } else if (!DecodeLReg(LeftComp.str.p_str, &Reg1)) {
                SetXError(ErrNum_InvReg, &LeftComp);
            } else if ((Reg1 < 2) || (Reg1 > 3)) {
                SetXError(ErrNum_InvReg, &LeftComp);
            } else {
                h = (Reg2 << 3) + ((Reg1 - 2) << 4);
            }
        ...
        case ParABXYnAB:
            ...
            if (!DecodeALUReg(RightComp.str.p_str, &Reg2, False, False, True)) {
                ...
                if (!DecodeXYABReg(LeftComp.str.p_str, &Reg1)) {
                    SetXError(ErrNum_InvReg, &LeftComp);
                } else if ((Reg1 ^ Reg2) == 1) {
                    SetError(ErrNum_InvRegPair);
                } else if ((Memo("CMPM")) && ((Reg1 & 6) == 2)) {
                    SetXError(ErrNum_InvReg, &LeftComp);
                } else {
                    if (Reg1 < 2) {
                        Reg1 = Ord(!Memo("CMPM"));
        ...
        case ParXYnAB:
            ...
            if (!DecodeALUReg(RightComp.str.p_str, &Reg2, False, False, True)) {
                ...
                if (!DecodeReg(LeftComp.str.p_str, &Reg1)) {
                    SetXError(ErrNum_InvReg, &LeftComp);
                } else if ((Reg1 < 4) || (Reg1 > 7)) {
                    SetXError(ErrNum_InvReg, &LeftComp);
                } else {
        ...
        case ParMul:
            ...
            if (!DecodeALUReg(RightComp.str.p_str, &Reg3, False, False, True)) {
                SetXError(ErrNum_InvReg, &RightComp);
            } else if (!DecodeReg(LeftRegComp.str.p_str, &Reg1)) {
                SetXError(ErrNum_InvReg, &LeftRegComp);
            } else if ((Reg1 < 4) || (Reg1 > 7)) {
                SetXError(ErrNum_InvReg, &LeftRegComp);
                ...
                if (!ChkArgCnt(1, 1))
                    ...
                    AddVal = EvalStrIntExpressionOffsWithFlags(
                            &MidComp, 1, UInt24, &OK, &Flags);
                    if (mFirstPassUnknown(Flags)) {
                        AddVal = 1;
                    }
                    if ((!(SingleBit(AddVal, &LAddVal))) || (LAddVal > 22)) {
            ...
            else if (!DecodeReg(MidComp.str.p_str, &Reg2)) {
                SetXError(ErrNum_InvReg, &MidComp);
            } else if ((Reg2 < 4) || (Reg2 > 7)) {
                SetXError(ErrNum_InvReg, &MidComp);
            } else if (MacTable[Reg1 - 4][Reg2 - 4] == 0xff) {
                SetError(ErrNum_InvRegPair);
            } else {
...
static void DecodeDIV(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        SplitArg(&ArgStr[1], &LeftComp, &RightComp);
        if ((*LeftComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            WrError(ErrNum_CannotSplitArg);
        } else if (!DecodeALUReg(RightComp.str.p_str, &Reg2, False, False, True)) {
            WrError(ErrNum_InvAddrMode);
        } else if (!DecodeReg(LeftComp.str.p_str, &Reg1)) {
            WrError(ErrNum_InvAddrMode);
        } else if ((Reg1 < 4) || (Reg1 > 7)) {
            WrError(ErrNum_InvAddrMode);
        } else {
...
static void DecodeImmMac(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if ((*MidComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            WrError(ErrNum_CannotSplitArg);
        } else if (!DecodeALUReg(RightComp.str.p_str, &Reg1, False, False, True)) {
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else if (!DecodeXYABReg(MidComp.str.p_str, &Reg2)) {
            WrStrErrorPos(ErrNum_InvReg, &MidComp);
        } else if ((Reg2 < 4) || (Reg2 > 7)) {
            WrStrErrorPos(ErrNum_InvReg, &MidComp);
        } else if (*LeftArg.str.p_str != '#') {
...
static void DecodeDMAC(Word Code) {
    if (!ChkArgCnt(1, 1))
        ...
        if ((*MidComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            ...
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else if (!DecodeXYAB1Reg(MidComp.str.p_str, &Reg2)) {
            WrStrErrorPos(ErrNum_InvReg, &MidComp);
        } else if (Reg2 < 4) {
            WrStrErrorPos(ErrNum_InvReg, &MidComp);
        } else if (!DecodeXYAB1Reg(LeftReg.str.p_str, &Reg3)) {
            WrStrErrorPos(ErrNum_InvReg, &LeftReg);
        } else if (Reg3 < 4) {
            WrStrErrorPos(ErrNum_InvReg, &LeftReg);
        } else {
...
static void DecodeINC_DEC(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        ;
    else if (!DecodeALUReg(ArgStr[1].str.p_str, &Reg1, False, False, True)) {
...
static void DecodeANDI_ORI(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if ((*LeftComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            WrError(ErrNum_CannotSplitArg);
        } else if (!DecodeControlReg(RightComp.str.p_str, &Reg1)) {
            WrStrErrorPos(ErrNum_InvAddrMode, &RightComp);
        } else if (*LeftComp.str.p_str != '#') {
            WrError(ErrNum_OnlyImmAddr);
        } else {
...
static void DecodeNORMF(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (*RightComp.str.p_str == '\0') {
            ...
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else if (!DecodeXYAB1Reg(LeftComp.str.p_str, &Reg1)) {
            WrStrErrorPos(ErrNum_InvReg, &LeftComp);
        } else {
            CodeLen     = 1;
        ...
        if ((*LeftComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            ...
            if (OK) {
                if ((h < 0) || (h > 23)) {
                    ...
                    DecodeAdr(
                            &RightComp, MModNoImm, MSegXData + MSegYData,
                            &RightAdrResult);
                    ...
                    if ((RightAdrResult.Type == ModAbs) && (RightAdrResult.Val <= 63)
                        ...
                        CodeLen = 1;
                        DAsmCode[0]
                                = 0x0a0000 + h + (RightAdrResult.Val << 8) + Reg3 + Reg2;
                    } else if (
                            (RightAdrResult.Type == ModAbs)
                            && (RightAdrResult.Val >= MemLimit - 0x3f)
                            && (RightAdrResult.Val <= MemLimit)
                            && (RightAdrResult.ShortMode != 2)) {
                        CodeLen     = 1;
                        DAsmCode[0] = 0x0a8000 + h + ((RightAdrResult.Val & 0x3f) << 8)
                                      + Reg3 + Reg2;
                    } else if (
                            (RightAdrResult.Type == ModAbs) && (MomCPU >= CPU56300)
                            && (RightAdrResult.Val >= MemLimit - 0x7f)
                            && (RightAdrResult.Val <= MemLimit - 0x40)
                            && (RightAdrResult.ShortMode != 2)) {
                        Reg2        = ((Code & 1) << 5) + (((LongInt)Code >> 1) << 14);
                        CodeLen     = 1;
                        DAsmCode[0] = 0x010000 + h + ((RightAdrResult.Val & 0x3f) << 8)
                                      + Reg3 + Reg2;
                    } else if (RightAdrResult.Type != ModNone) {
                        CodeLen = 1 + RightAdrResult.Cnt;
                        DAsmCode[0]
                                = 0x0a4000 + h + (RightAdrResult.Mode << 8) + Reg3 + Reg2;
                        DAsmCode[1] = RightAdrResult.Val;
                    }
                }
            }
...
static void DecodeEXTRACT_EXTRACTU(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU56300)) {
        SplitArg(&ArgStr[1], &LeftComp, &MidComp);
        SplitArg(&MidComp, &MidComp, &RightComp);
        if ((*MidComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            WrError(ErrNum_CannotSplitArg);
...
static void DecodeINSERT(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU56300)) {
        ...
        if ((*MidComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            ...
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else if (!DecodeXYAB0Reg(MidComp.str.p_str, &Reg2)) {
            WrStrErrorPos(ErrNum_InvReg, &MidComp);
        } else if (*LeftComp.str.p_str == '#') {
            DAsmCode[1] = EvalStrIntExpressionOffs(&LeftComp, 1, Int24, &OK);
            if (OK) {
...
static void DecodeMERGE(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU56300)) {
        ...
        if (*RightComp.str.p_str == '\0') {
            ...
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else if (!DecodeXYAB1Reg(LeftComp.str.p_str, &Reg2)) {
            WrStrErrorPos(ErrNum_InvReg, &LeftComp);
        } else {
...
static void DecodeCLB(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU56300)) {
        ...
        if (*RightComp.str.p_str == '\0') {
            WrError(ErrNum_CannotSplitArg);
        } else if (!DecodeALUReg(LeftComp.str.p_str, &Reg1, False, False, True)) {
            WrStrErrorPos(ErrNum_InvReg, &LeftComp);
        } else if (!DecodeALUReg(RightComp.str.p_str, &Reg2, False, False, True)) {
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else {
...
static void DecodeCMPU(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU56300)) {
        ...
        if (*RightComp.str.p_str == '\0') {
            ...
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else if (!DecodeXYABReg(LeftComp.str.p_str, &Reg2)) {
            WrStrErrorPos(ErrNum_InvReg, &LeftComp);
        } else if ((Reg1 ^ Reg2) == 1) {
            WrError(ErrNum_InvRegPair);
        } else if ((Reg2 & 6) == 2) {
...
static void DecodeMOVEC(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (*RightComp.str.p_str == '\0') {
        ...
        } else if (!DecodeCtrlReg(RightComp.str.p_str, &Reg1)) {
            WrStrErrorPos(ErrNum_InvCtrlReg, &RightComp);
        } else {
            if (DecodeGeneralReg(LeftComp.str.p_str, &Reg2)) {
                ...
                if ((LeftAdrResult.Type == ModAbs) && (LeftAdrResult.Val <= 63)
                    && (LeftAdrResult.ShortMode != 2)) {
                    DAsmCode[0] = 0x058020 + (LeftAdrResult.Val << 8) + Reg3 + Reg1;
                    CodeLen     = 1;
                } else if (
                        !LeftAdrResult.ForceImmLong && (LeftAdrResult.Type == ModImm)
                        && (LeftAdrResult.Val <= 255)) {
...
static void DecodeMOVEM(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if ((*LeftComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            ...
            if ((RightAdrResult.Type == ModAbs) && (RightAdrResult.Val >= 0)
                && (RightAdrResult.Val <= 63) && (RightAdrResult.ShortMode != 2)) {
                CodeLen     = 1;
            ...
            if ((LeftAdrResult.Type == ModAbs) && (LeftAdrResult.Val >= 0)
                && (LeftAdrResult.Val <= 63) && (LeftAdrResult.ShortMode != 2)) {
                CodeLen     = 1;
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if ((*LeftComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            ...
            if (LeftAdrResult.Type != ModNone) {
                if ((LeftAdrResult.Val <= MemLimit)
                    && (LeftAdrResult.Val >= MemLimit - 0x3f)) {
                    CodeLen     = 1;
                    DAsmCode[0] = 0x084000 + (Ord(LeftAdrResult.Seg == SegYData) << 16)
                                  + (LeftAdrResult.Val & 0x3f) + (Reg2 << 8);
                } else if (
                        (MomCPU >= CPU56300) && (LeftAdrResult.Val <= MemLimit - 0x40)
                        && (LeftAdrResult.Val >= MemLimit - 0x7f)) {
                    CodeLen     = 1;
                    DAsmCode[0] = 0x044000 + (Ord(LeftAdrResult.Seg == SegYData) << 5)
                                  + (Ord(LeftAdrResult.Seg == SegXData) << 7)
                                  + (LeftAdrResult.Val & 0x1f)
                                  + ((LeftAdrResult.Val & 0x20) << 1) + (Reg2 << 8);
                } else {
                    WrError(ErrNum_UnderRange);
                }
            }
            ...
            DecodeAdr(
                    &LeftComp, MModAll, MSegXData + MSegYData + MSegCode, &LeftAdrResult);
            if ((LeftAdrResult.Type == ModAbs) && (LeftAdrResult.Seg != SegCode)
                && (LeftAdrResult.Val >= MemLimit - 0x3f)
                && (LeftAdrResult.Val <= MemLimit)) {
                LongInt    HVal = LeftAdrResult.Val & 0x3f, HSeg = LeftAdrResult.Seg;
                tAdrResult RightAdrResult;

                DecodeAdr(
                        &RightComp, MModNoImm, MSegXData + MSegYData + MSegCode,
                        &RightAdrResult);
                if (RightAdrResult.Type != ModNone) {
                    ...
                    (LeftAdrResult.Type == ModAbs) && (MomCPU >= CPU56300)
                    && (LeftAdrResult.Seg != SegCode)
                    && (LeftAdrResult.Val >= MemLimit - 0x7f)
                    && (LeftAdrResult.Val <= MemLimit - 0x40)
                    && (LeftAdrResult.ShortMode != 2)) {
                LongInt    HVal = LeftAdrResult.Val & 0x3f, HSeg = LeftAdrResult.Seg;
                tAdrResult RightAdrResult;
                ...
                DecodeAdr(&RightComp, MModAbs, MSegXData + MSegYData, &RightAdrResult);
                if (RightAdrResult.Type != ModNone) {
                    if ((RightAdrResult.Val >= MemLimit - 0x3f)
                        && (RightAdrResult.Val <= MemLimit)) {
                        if (HSeg == SegCode) {
                            CodeLen     = 1 + HCnt;
                            DAsmCode[1] = HVal;
                            DAsmCode[0] = 0x08c040 + (RightAdrResult.Val & 0x3f)
                                          + (HMode << 8)
                                          + (Ord(RightAdrResult.Seg == SegYData) << 16);
                        } else {
                            CodeLen     = 1 + HCnt;
                            DAsmCode[1] = HVal;
                            DAsmCode[0] = 0x08c080 + (((Word)RightAdrResult.Val) & 0x3f)
                                          + (HMode << 8)
                                          + (Ord(RightAdrResult.Seg == SegYData) << 16)
                                          + (Ord(HSeg == SegYData) << 6);
                        }
                        ...
                        if (HSeg == SegCode) {
                            CodeLen     = 1 + HCnt;
                            DAsmCode[1] = HVal;
                            DAsmCode[0] = 0x00c000 + (RightAdrResult.Val & 0x3f)
                                          + (HMode << 8)
                                          + (Ord(RightAdrResult.Seg == SegYData) << 6);
                        } else {
                            CodeLen     = 1 + HCnt;
                            DAsmCode[1] = HVal;
                            DAsmCode[0] = 0x078000 + (((Word)RightAdrResult.Val) & 0x3f)
                                          + (HMode << 8)
...
static void DecodeTcc(Word Condition) {
    ...
    if (!ChkArgCnt(1, 2))
        ...
        } else if (!DecodeRR(&ArgStr[2], &Reg2)) {
            WrError(ErrNum_InvAddrMode);
        } else {
            CodeLen     = 1;
        ...
        ;
    else if (!DecodeRR(&ArgStr[1], &Reg1)) {
        WrError(ErrNum_InvAddrMode);
    } else {
            ...
            if (OK) {
                if ((AddVal < 0) || (AddVal > 23)) {
                    ...
                    if ((AdrResult.Type == ModAbs)
                        && (mFirstPassUnknown(AdrResult.AbsSymFlags))) {
                        AdrResult.Val &= 0x3f;
                    }
                    if ((AdrResult.Type == ModAbs) && (AdrResult.Val <= 63)
                        && (AdrResult.Val >= 0) && (AdrResult.ShortMode != 2)) {
                        CodeLen     = 1;
                        DAsmCode[0] = 0x0c8080 + AddVal + (AdrResult.Val << 8) + Reg3 + h
                                      + h2;
                    } else if (
                            (AdrResult.Type == ModAbs)
                            && (AdrResult.Val >= MemLimit - 0x3f)
                            && (AdrResult.Val <= MemLimit)) {
                        CodeLen     = 1;
                        DAsmCode[0] = 0x0cc000 + AddVal + ((AdrResult.Val & 0x3f) << 8)
                                      + Reg3 + h + h2;
                    } else if (
                            (AdrResult.Type == ModAbs)
                        ...
                        CodeLen     = 1;
                        DAsmCode[0] = 0x048000 + AddVal + ((AdrResult.Val & 0x3f) << 8)
                                      + Reg3 + h + (h2 >> 9);
                    } else if (AdrResult.Type == ModAbs) {
                        WrError(ErrNum_InvAddrMode);
                    } else if (AdrResult.Type != ModNone) {
                        CodeLen     = 1;
                        DAsmCode[0] = 0x0c8000 + AddVal + (AdrResult.Mode << 8) + Reg3 + h
                                      + h2;
                    }
                }
            }
...
static void DecodeBRA_BSR(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU56300) && DecodeReg(ArgStr[1].str.p_str, &Reg1)) {
        if ((Reg1 < 16) || (Reg1 > 23)) {
            WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
        } else {
            Reg1 -= 16;
        ...
        Dist = EvalStrIntExpressionOffsWithFlags(&ArgStr[1], Offset, AdrInt, &OK, &Flags)
               - EProgCounter();
        if (Size == 0) {
        ...
        switch (Size) {
        case 1:
            if (!mSymbolQuestionable(Flags) && ((Dist < -256) || (Dist > 255))) {
                WrError(ErrNum_JmpDistTooBig);
            } else {
                Dist &= 0x1ff;
                DAsmCode[0]
                        = 0x050800 + (Code << 4) + ((Dist & 0x1e0) << 1) + (Dist & 0x1f);
                CodeLen = 1;
            }
            break;
        case 2:
...
static void DecodeLUA_LEA(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if ((*LeftComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            WrError(ErrNum_CannotSplitArg);
        } else if (!DecodeReg(RightComp.str.p_str, &Reg1)) {
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else if (Reg1 > 31) {
            WrStrErrorPos(ErrNum_InvReg, &RightComp);
        } else {
            ...
            DecodeAdr(
                    &LeftComp,
                    MModModInc | MModModDec | MModPostInc | MModPostDec | MModDisp,
                    MSegXData, &AdrResult);
            if (AdrResult.Type == ModDisp) {
...
static void DecodePLOCKR_PUNLOCKR(Word Code) {
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU56300)) {
        ...
        DAsmCode[1] = (EvalStrIntExpression(&ArgStr[1], AdrInt, &OK) - EProgCounter())
                      & 0xffffff;
        if (OK) {
            DAsmCode[0] = Code;
            CodeLen     = 2;
...
static void DecodeBitJmp(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        SplitArg(&ArgStr[1], &LeftComp, &MidComp);
        SplitArg(&MidComp, &MidComp, &RightComp);
        if ((*LeftComp.str.p_str == '\0') || (*MidComp.str.p_str == '\0')
            || (*RightComp.str.p_str == '\0')) {
            WrError(ErrNum_CannotSplitArg);
        } else if (*LeftComp.str.p_str != '#') {
            WrError(ErrNum_OnlyImmAddr);
        } else {
                ...
                if (OK) {
                    if ((h < 0) || (h > 23)) {
                        ...
                        if (DecodeGeneralReg(MidComp.str.p_str, &Reg1)) {
                            ...
                            DecodeAdr(
                                    &MidComp, MModNoImm, MSegXData + MSegYData,
                                    &AdrResult);
                            Reg3 = Ord(AdrResult.Seg == SegYData) << 6;
                            if (AdrResult.Type == ModAbs) {
                                if ((AdrResult.Val >= 0) && (AdrResult.Val <= 63)) {
                                    CodeLen     = 2;
                                    DAsmCode[0] = 0x0a0080 + h + Reg2 + Reg3
                                                  + (AdrResult.Val << 8);
                                } else if (
                                        (AdrResult.Val >= MemLimit - 0x3f)
                                        && (AdrResult.Val <= MemLimit)) {
                                    CodeLen     = 2;
                                    DAsmCode[0] = 0x0a8080 + h + Reg2 + Reg3
                                ...
                                CodeLen     = 2;
                                DAsmCode[0] = 0x0a4080 + h + Reg2 + Reg3
                                              + (AdrResult.Mode << 8);
                            }
                        }
                    }
...
static void DecodeDO_DOR(Word Code) {
    if (ChkArgCnt(1, 1)) {
        ...
        if ((*LeftComp.str.p_str == '\0') || (*RightComp.str.p_str == '\0')) {
            ...
            DAsmCode[1]
                    = EvalStrIntExpressionWithResult(&RightComp, AdrInt, &EvalResult) - 1;
            if (EvalResult.OK) {
                ...
                if (!as_strcasecmp(LeftComp.str.p_str, "FOREVER")) {
                    ...
                    if (Reg1 == 0x3c) {
                        WrXError(ErrNum_InvReg, LeftComp.str.p_str); /* kein SSH!! */
                    } else {
                        CodeLen     = 2;
                        DAsmCode[0] = 0x06c000 + (Reg1 << 8) + (Code << 4);
                    }
                } else if (*LeftComp.str.p_str == '#') {
                    Reg1 = EvalStrIntExpressionOffsWithResult(
                            &LeftComp, 1, UInt12, &EvalResult);
                    if (EvalResult.OK) {
                        CodeLen     = 2;
                    ...
                    if (AdrResult.Type == ModAbs) {
                        if ((AdrResult.Val < 0) || (AdrResult.Val > 63)) {
                            ...
                            CodeLen     = 2;
                            DAsmCode[0] = 0x060000 + (AdrResult.Val << 8)
                                          + (Ord(AdrResult.Seg == SegYData) << 6)
                                          + (Code << 4);
                        }
                    } else {
                        CodeLen     = 2;
                        DAsmCode[0] = 0x064000 + (AdrResult.Mode << 8)
                                      + (Ord(AdrResult.Seg == SegYData) << 6)
                                      + (Code << 4);
                    }
                }
...
static void DecodeBRKcc(Word Condition) {
    if (ChkArgCnt(0, 0) && ChkMinCPU(CPU56300)) {
...
static void DecodeREP(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if (AdrResult.Type == ModImm) {
            if ((AdrResult.Val < 0) || (AdrResult.Val > 0xfff)) {
                WrError(ErrNum_OverRange);
            } else {
                CodeLen = 1;
                DAsmCode[0]
                        = 0x0600a0 + (AdrResult.Val >> 8) + ((AdrResult.Val & 0xff) << 8);
            }
        } else if (AdrResult.Type == ModAbs) {
            if ((AdrResult.Val < 0) || (AdrResult.Val > 63)) {
                WrError(ErrNum_OverRange);
                ...
                CodeLen     = 1;
                DAsmCode[0] = 0x060020 + (AdrResult.Val << 8)
                              + (Ord(AdrResult.Seg == SegYData) << 6);
            }
        } else {
            CodeLen     = 1 + AdrResult.Cnt;
            DAsmCode[1] = AdrResult.Val;
            DAsmCode[0] = 0x064020 + (AdrResult.Mode << 8)
                          + (Ord(AdrResult.Seg == SegYData) << 6);
        }
    }
}
...
static void AddFixed(char const* Name, LongWord Code, CPUVar NMin) {
...
static void AddPar(char const* Name, ParTyp Typ, LongWord Code) {
    if (InstrZ >= ParOrderCnt) {
        exit(255);
    }
...
static void AddMix(char const* pName, Word Code, InstProc Proc, unsigned Mask) {
    char TmpName[30];
...
static void AddCondition(char const* pName, InstProc Proc) {
    unsigned z;
    char     TmpName[30];
    Word     Code;
