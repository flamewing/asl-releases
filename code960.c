...
static SpecReg const SpecRegs[SpecRegCnt] = {
        { "FP", 31},
        {"PFP",  0},
        { "SP",  1},
        {"RIP",  2}
...
static Boolean DecodeIRegCore(char const* pArg, LongWord* pResult) {
    int      z;
    LongWord Offs;
...
static Boolean DecodeFPRegCore(char const* pArg, LongWord* pResult) {
...
static void DissectReg_960(
    ...
    case eSymbolSize32Bit: {
        ...
        as_snprintf(
                pDest, DestSize, "%c%u", Value & 16 ? 'G' : 'R', (unsigned)(Value & 15));
        break;
    }
...
static tRegEvalResult DecodeIReg(
        tStrComp const* pArg, LongWord* pResult, Boolean MustBeReg) {
...
static tRegEvalResult DecodeIOrFPReg(
        tStrComp const* pArg, LongWord* pResult, tSymbolSize* pSize, Boolean MustBeReg) {
    if (DecodeIRegCore(pArg->str.p_str, pResult)) {
        *pResult &= ~REG_MARK;
...
static Boolean DecodeAdr(
        tStrComp const* pArg, Byte Mask, OpType Type, LongWord* Erg, LongWord* Mode) {
    Double      FVal;
    tEvalResult EvalResult;
...
static int DecodeMem(tStrComp const* pArg, LongWord* Erg, LongWord* Ext) {
    LongInt  DispAcc;
    ...
    do {
        ...
        if (ArgLen == 0) {
            ...
            case ']':
                if (Index != NOREG) {
                    return AddrError(ErrNum_InvAddrMode);
                }
                ...
                if (p < Arg.str.p_str) {
                    return AddrError(ErrNum_BrackErr);
                }
    ...
    if (Base == IPREG) {
        ...
        *Erg = DispAcc;
        ...
        Mode = (Ord(DispAcc != 0) << 3) + 4 + (Ord(Index != NOREG) << 1)
               + Ord(Base != NOREG);
        if ((Mode & 9) == 0) {
            Mode += 8;
        }
...
static void DecodeReg(Word Index) {
    ...
    if (!ChkArgCntExt(ActArgCnt, NumArgs, NumArgs))
        ;
    else if (((Op->DestType >= SingleOp) || (Op->Src1Type >= SingleOp)) && (!FPUAvail)) {
        WrStrErrorPos(ErrNum_UnknownInstruction, &OpPart);
    } else if (
            ((Op->DestType == NoneOp)
             || (DecodeAdr(
                     pDestArg, MModReg | (Op->DestType >= SingleOp ? MModFReg : 0),
                     Op->DestType, &DReg, &DMode)))
            && (DecodeAdr(
                    &ArgStr[1],
                    MModReg | (Op->Src1Type >= SingleOp ? MModFReg : 0)
                            | (Op->Imm1 ? MModImm : 0),
                    Op->Src1Type, &S1Reg, &S1Mode))
            && ((Op->Src2Type == NoneOp)
                || (DecodeAdr(
                        &ArgStr[2],
                        MModReg | (Op->Src2Type >= SingleOp ? MModFReg : 0)
                                | (Op->Imm2 ? MModImm : 0),
                        Op->Src2Type, &S2Reg, &S2Mode)))) {
...
static void DecodeCobr(Word Index) {
    ...
    if (!ChkArgCnt(NumArgs, NumArgs))
        ;
            ...
            (DecodeAdr(
                    &ArgStr[1], MModReg | (Op->HasSrc ? MModImm : 0), IntOp, &S1Reg,
                    &S1Mode))
            && ((!Op->HasSrc)
                || (DecodeAdr(&ArgStr[2], MModReg, IntOp, &S2Reg, &S2Mode)))) {
        OK     = True;
        Flags  = eSymbolFlag_None;
        AdrInt = (Op->HasSrc)
                         ? EvalStrIntExpressionWithFlags(&ArgStr[3], UInt32, &OK, &Flags)
                                   - EProgCounter()
                         : 0;
...
static void DecodeCtrl(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[1], UInt32, &OK, &Flags)
                 - EProgCounter();
        if (mFirstPassUnknown(Flags)) {
            AdrInt &= (~3);
        }
...
static void DecodeMemO(Word Index) {
    ...
    if (!ChkArgCnt(NumArgs, NumArgs))
        ...
        ;
    else if (Reg & OpMasks[Op->Type]) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[Op->RegPos]);
...
static void AddReg(
        char const* NName, LongWord NCode, OpType NSrc1, OpType NSrc2, OpType NDest,
...
static void AddCobr(char const* NName, LongWord NCode, Boolean NHas) {
    if (InstrZ >= CobrOrderCnt) {
        exit(255);
    }
...
static void AddCtrl(char const* NName, LongWord NCode) {
...
static void AddMem(char const* NName, LongWord NCode, OpType NType, int NPos) {
...
static void InitFields(void) {
    ...
    CobrOrders = (CobrOrder*)malloc(sizeof(CobrOrder) * CobrOrderCnt);
    InstrZ     = 0;
    AddCobr("BBC", 0x30, True);
    AddCobr("BBS", 0x37, True);
    AddCobr("CMPIBE", 0x3a, True);
    AddCobr("CMPOBE", 0x32, True);
    AddCobr("CMPIBNE", 0x3d, True);
    AddCobr("CMPOBNE", 0x35, True);
    AddCobr("CMPIBL", 0x3c, True);
    AddCobr("CMPOBL", 0x34, True);
    AddCobr("CMPIBLE", 0x3e, True);
    AddCobr("CMPOBLE", 0x36, True);
    AddCobr("CMPIBG", 0x39, True);
    AddCobr("CMPOBG", 0x31, True);
    AddCobr("CMPIBGE", 0x3b, True);
    AddCobr("CMPOBGE", 0x33, True);
    AddCobr("CMPIBO", 0x3f, True);
    AddCobr("CMPIBNO", 0x38, True);
    AddCobr("TESTE", 0x22, False);
    AddCobr("TESTNE", 0x25, False);
    AddCobr("TESTL", 0x24, False);
    AddCobr("TESTLE", 0x26, False);
    AddCobr("TESTG", 0x21, False);
    AddCobr("TESTGE", 0x23, False);
    AddCobr("TESTO", 0x27, False);
    AddCobr("TESTNO", 0x27, False);

    CtrlOrders = (FixedOrder*)malloc(sizeof(FixedOrder) * CtrlOrderCnt);
    InstrZ     = 0;
    AddCtrl("B", 0x08);
    AddCtrl("CALL", 0x09);
    AddCtrl("BAL", 0x0b);
