...
static Boolean IsWRegCore(char const* pArg, Word* pResult) {
    Boolean retValue;
...
static tRegEvalResult IsWReg(tStrComp const* pArg, Word* pResult, Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static void DecodeAdr(tStrComp const* pArg, Byte Mask, int Segment) {
    tEvalResult EvalResult;
    char*       p;
    int         ArgLen;
...
static void AddIOop(Char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeIOop);
}

static void AddCondition(char const* NName, Word NCode) {
    if (InstrZ >= CondCnt) {
        exit(255);
    }
...
static void InitFields(void) {
    ...
    AddFixed("EI", 0x8030);
    AddFixed("DI", 0x8010);
    AddFixed("RETIE", 0x80f0);
    AddFixed("RETID", 0x80d0);
