...
static tRegEvalResult DecodeRegCore(char const* pArg, tSymbolSize* pSize, Byte* pResult) {
    int len;
    ...
    len = strlen(pArg);
    if ((len >= 2) && (as_toupper(*pArg) == 'R') && (pArg[1] >= '0')
        && (pArg[1] <= '7')) {
        *pResult = pArg[1] - '0';
        if (len == 2) {
...
static tRegEvalResult DecodeReg(
        tStrComp const* pArg, tSymbolSize* pSize, Byte* pResult, Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static Boolean ChkAdr(Word Mask, tStrComp const* pComp) {
...
static Boolean DecodeAdrIndirect(tStrComp* pArg, Word Mask) {
    ...
    if (pArg->str.p_str[ArgLen - 1] == '+') {
        ...
        if (!DecodeReg(pArg, &NSize, &AdrPart, True))
            ;
        else if (NSize != eSymbolSize16Bit) {
        ...
        do {
            ...
            case eIsReg:
                if ((NSize != eSymbolSize16Bit) || (AdrPart != 0xff) || NegFlag) {
                    WrStrErrorPos(ErrNum_InvAddrMode, &ThisComp);
                    ErrFlag = True;
                } else {
                    AdrPart = Reg;
                }
            ...
            default: {
                ...
                DispPart = EvalStrIntExpressionWithFlags(
                        &ThisComp, Int32, &ErrFlag, &Flags);
                ErrFlag = !ErrFlag;
                if (!ErrFlag) {
        ...
        *p = '\0';
        ...
        if (EvalResult.OK) {
            ...
            case eRegAbort:
                return False;
            case eIsReg:
                if ((Size == eSymbolSize8Bit) && (BPos > 7)) {
            ...
            default:
                if (BPos > 7) {
                    ...
                    if (EvalResult.AddrSpaceMask & (1 << SegIO)) {
                        ...
                        ChkSpace(SegData, EvalResult.AddrSpaceMask);
                        if (mFirstPassUnknown(EvalResult.Flags)) {
                            AdrLong = (AdrLong & 0x00ff003f) | 0x20;
                        }
...
static Boolean ChkLongEvenDist(
        LongInt* pDist, tStrComp const* pComp, tSymbolFlags Flags) {
...
static void DecodeBIT(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        case 0:
            as_snprintf(
                    ListLine, STRINGSIZE, "=R%u.%u", (unsigned)((BAdr >> 4) & 15),
                    (unsigned)(BAdr & 15));
            break;
        case 1:
            as_snprintf(
                    ListLine, STRINGSIZE, "=%x:%x.%u", (unsigned)((BAdr >> 16) & 255),
                    (unsigned)((BAdr & 0x1f8) >> 3), (unsigned)(BAdr & 7));
            break;
        default:
            as_snprintf(
                    ListLine, STRINGSIZE, "=S:%x.%u",
                    (unsigned)(((BAdr >> 3) & 0x3f) + 0x400), (unsigned)(BAdr & 7));
            break;
        }
    }
}
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        ...
        if (OK) {
            ...
            } else if (Code < 0x2000) /* POP(U): obere Register zuerst */
            {
                ...
                if ((OpSize == eSymbolSize16Bit) && (Code == 0x1027) && (Mask & 0x80)) {
        ...
        case ModReg:
            if (OpSize >= eSymbolSize32Bit) {
                ...
                case ModMem:
                    ...
                    CodeLen += AdrCnt;
                    if ((MemPart == 3) && ((HReg >> (1 - OpSize)) == AdrPart)) {
                        WrError(ErrNum_Unpredictable);
                    }
                    break;
                case ModImm:
        ...
        case ModMem:
            ...
            case ModReg:
                if (OpSize == eSymbolSize32Bit) {
                    ...
                    CodeLen += HCnt;
                    if ((HMem == 3) && ((AdrPart >> (1 - OpSize)) == HReg)) {
                        WrError(ErrNum_Unpredictable);
                    }
                }
                break;
        ...
        case ModMem:
            ...
            case ModReg:
                if ((OpSize != eSymbolSize8Bit) && (OpSize != eSymbolSize16Bit)) {
                    ...
                    memcpy(BAsmCode + CodeLen, HVals, HCnt);
                    CodeLen += HCnt;
                    if ((HPart == 3) && ((AdrPart >> (1 - OpSize)) == HReg)) {
                        WrError(ErrNum_Unpredictable);
                    }
                }
                break;
...
static void DecodeMOVC(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (!*AttrPart.str.p_str && !as_strcasecmp(ArgStr[1].str.p_str, "A")) {
            OpSize = eSymbolSize8Bit;
        }
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (AdrMode == ModMem) {
            ...
            case 1:
                ...
                DecodeAdr(&ArgStr[2], MModMem);
                if (AdrMode == ModMem) {
                    if ((OpSize != eSymbolSize16Bit) && (OpSize != eSymbolSize8Bit)) {
                        WrError(ErrNum_InvOpSize);
                    } else {
                        switch (MemPart) {
                        case 1:
...
static void DecodeDIVU(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(&ArgStr[1], MModReg);
        if (AdrMode == ModReg) {
            if ((OpSize == eSymbolSize8Bit) && (AdrPart & 1)) {
...
static void DecodeANLORL(Word Index) {
    if (!ChkArgCnt(2, 2))
        ...
        if (Result) {
            ...
            BAsmCode[CodeLen++] = 0x08;
            BAsmCode[CodeLen++]
                    = 0x40 | (Index << 5) | (Ord(Invert) << 4) | (Hi(AdrLong) & 3);
            BAsmCode[CodeLen++] = Lo(AdrLong);
        }
    }
}
                ...
                case 6:
                    if (OpSize == eSymbolSizeUnknown) {
                        WrError(ErrNum_UndefOpSizes);
                            ...
                            (OpSize != eSymbolSize8Bit) && (OpSize != eSymbolSize16Bit)) {
                        WrError(ErrNum_InvOpSize);
                    } else {
                        BAsmCode[CodeLen]     = 0xe2 + (OpSize << 3);
...
static void MakeCode_XA(void) {
    ...
    if ((ActPC == SegCode) && (!IsRealDef())
        && ((*LabPart.str.p_str != '\0')
            || ((ArgCnt == 1) && (!strcmp(ArgStr[1].str.p_str, "$"))))) {
        ForceAlign();
        if (*LabPart.str.p_str != '\0') {
            EnterIntSymbol(
                    &LabPart, EProgCounter() + CodeLen, (as_addrspace_t)ActPC, False);
        }
    }
...
static void AddJBit(char const* NName, Word NCode) {
    if (InstrZ >= JBitOrderCnt) {
        exit(255);
    }
...
static void AddStack(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeStack);
}

static void AddReg(char const* NName, Byte NMask, Byte NCode) {
...
static void SetInv(char const* Name1, char const* Name2, InvOrder* Orders) {
...
static void InitFields(void) {
    ...
    JBitOrders = (InvOrder*)malloc(sizeof(InvOrder) * JBitOrderCnt);
    InstrZ     = 0;
    AddJBit("JB", 0x80);
    AddJBit("JBC", 0xc0);
    AddJBit("JNB", 0xa0);
    ...
    RelOrders = (InvOrder*)malloc(sizeof(InvOrder) * RelOrderCount);
    InstrZ    = 0;
    AddRel("BCC", 0xf0);
    AddRel("BCS", 0xf1);
    AddRel("BNE", 0xf2);
