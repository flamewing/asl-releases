...
char const* Blanks(int cnt) {
    static char const* BlkStr = "                                                        "
                                "                                                   ";
...
char* SysStringCore(
        char* pDest, char* pDestCurr, LargeWord Num, int System, int Stellen,
        char StartCharacter) {
    LargeWord Digit;
...
int SysString(
        char* pDest, size_t DestSize, LargeWord Num, int System, int Stellen,
    ...
    *pDestCurr = '\0';
    if (SplitCharacter) {
        ...
        if (!SystemByteLen[System]) {
            ...
            SystemByteLen[System] = SysString(
                    Dummy, sizeof(Dummy), 0xff, System, 0, False, StartCharacter, False);
        }

        do {
            Part      = Num % 256;
            Num       = Num / 256;
            pDestNext = SysStringCore(
                    pDest, pDestCurr, Part, System, Num ? SystemByteLen[System] : Stellen,
                    StartCharacter);
...
static void ResetFormatContext(tFormatContext* pContext) {
    ...
    pContext->CurrArg  = 0;
    pContext->IntSize  = 0;
    pContext->InFormat = pContext->LeadZero = pContext->ForceLeadZero = pContext->Signed
            = pContext->LeftAlign = pContext->AddPlus = pContext->ForceUpper = False;
}
...
#else
static size_t FloatConvert(
        char* pDest, size_t DestSize, double Src, int Digits,
        Boolean TruncateTrailingZeros, char FormatType) {
    UNUSED(DestSize);
    UNUSED(TruncateTrailingZeros);
    Boolean const needUpperCaseFormat = HexStartCharacter != 'a';
    switch (FormatType) {
    case 'e':
...
static size_t append(
        dest_format_context_t* p_dest_ctx, char const* p_src, size_t cnt,
        tFormatContext* pFormatContext) {
...
static int vsprcatf_core(
        dest_format_context_t* p_dest_ctx, char const* pFormat, va_list ap) {
    char const*    pFormatStart = pFormat;
    int            Result       = 0;
    ...
    for (; *pFormat; pFormat++) {
        if (FormatContext.InFormat) {
            ...
            case '8':
            case '9': {
                if (!FormatContext.CurrArg
                    && !FormatContext.ArgState[FormatContext.CurrArg]
                    && (*pFormat == '0')) {
                    FormatContext.LeadZero = True;
                }
                FormatContext.Arg[FormatContext.CurrArg]
                        = (FormatContext.Arg[FormatContext.CurrArg] * 10)
                          + (*pFormat - '0');
            ...
            IntCommon: {
                ...
                Cnt = (pStr - Str)
                      + SysString(
                              pStr, sizeof(Str) - (pStr - Str), IntArg,
            ...
            case 'g': {
                ...
                Cnt = FloatConvert(
                        Str, sizeof(Str), va_arg(ap, double), FormatContext.Arg[1], False,
                        *pFormat);
                if (Cnt > sizeof(Str)) {
                    Cnt = sizeof(Str);
                }
            ...
            case 's': {
                char const* pStr = va_arg(ap, char*);
...
int as_vsdprcatf(as_dynstr_t* p_dest, char const* pFormat, va_list ap) {
...
int as_vsdprintf(as_dynstr_t* p_dest, char const* pFormat, va_list ap) {
...
int as_sdprcatf(as_dynstr_t* p_dest, char const* pFormat, ...) {
    va_list ap;
    int     ret;
...
int as_sdprintf(as_dynstr_t* p_dest, char const* pFormat, ...) {
    va_list ap;
    int     ret;
...
int as_vsnprcatf(char* pDest, size_t DestSize, char const* pFormat, va_list ap) {
    dest_format_context_t ctx;
...
int as_vsnprintf(char* pDest, size_t DestSize, char const* pFormat, va_list ap) {
    if (DestSize > 0) {
        *pDest = '\0';
    }
...
int as_snprintf(char* pDest, size_t DestSize, char const* pFormat, ...) {
    va_list ap;
    int     Result;
...
int as_snprcatf(char* pDest, size_t DestSize, char const* pFormat, ...) {
    va_list ap;
    int     Result;
...
int as_strcasecmp(char const* src1, char const* src2) {
    if (!src1) {
        src1 = "";
    }
    if (!src2) {
...
int as_strncasecmp(char const* src1, char const* src2, size_t len) {
    if (!src1) {
        src1 = "";
    }
    if (!src2) {
...
char* strrmultchr(char const* haystack, char const* needles) {
    char const* pPos;
...
size_t strmaxcpy(char* dest, char const* src, size_t Max) {
...
size_t strmaxcat(char* Dest, char const* Src, size_t MaxLen) {
...
void strprep(char* Dest, char const* Src) {
...
void strmaxprep(char* p_dest, char const* p_src, size_t max_len) {
    size_t src_len = strlen(p_src), dest_len = strlen(p_dest);
...
void strmaxprep2(char* p_dest, char const* p_src, size_t max_len) {
    size_t src_len = strlen(p_src), dest_len = strlen(p_dest);
...
void strins(char* Dest, char const* Src, int Pos) {
    memmove(Dest + Pos + strlen(Src), Dest + Pos, strlen(Dest) + 1 - Pos);
    memmove(Dest + Pos, Src, strlen(Src));
}

void strmaxins(char* Dest, char const* Src, int Pos, size_t MaxLen) {
    size_t RLen;
...
int strlencmp(char const* pStr1, unsigned Str1Len, char const* pStr2, unsigned Str2Len) {
    char const *p1, *p2, *p1End, *p2End;
    int         Diff;

    for (p1 = pStr1, p1End = p1 + Str1Len, p2 = pStr2, p2End = p2 + Str2Len;
...
unsigned fstrlenprint(FILE* pFile, char const* pStr, unsigned StrLen) {
    unsigned    Result = 0;
    char const *pRun, *pEnd;
...
size_t as_strnlen(char const* pStr, size_t MaxLen) {
    size_t Res = 0;
...
int strreplace(
    ...
    while (True) {
        ...
        /* See how much of the remainder behind 'To' still fits into buffer after
           replacement, and move accordingly: */
...
LargeInt ConstLongInt(char const* inp, Boolean* pErr, LongInt Base) {
    static char const Prefixes[4]
            = {'$', '@', '%', '\0'}; /* die moeglichen Zahlensysteme */
    static char const    Postfixes[4] = {'H', 'O', '\0', '\0'};
    static LongInt const Bases[3]     = {16, 8, 2}; /* die dazugehoerigen Basen */
    LargeInt             erg, val;
    ...
    if ((InpLen >= 2) && (*inp == '0') && (as_toupper(inp[1]) == 'X')) {
        inp += 2;
        InpLen -= 2;
        Base = 16;
    }
...
int CopyNoBlanks(char* pDest, char const* pSrc, size_t MaxLen) {
    char const* pSrcRun;
    char*       pDestRun = pDest;
    size_t      Cnt      = 0;
    Byte        Flags    = 0;
...
int strqcmp(char const* s1, char const* s2) {
    int erg = (*s1) - (*s2);
...
char* strmov(char* pDest, char const* pSrc) {
...
char* strcpy(char* pDest, char const* pSrc) {
    ...
    if (Overlap) {
        fprintf(stderr,
                "overlapping strcpy() called from address %p, resolve this address with "
                "addr2line and report to author\n",
...
int strmemcpy(char* pDest, size_t DestSize, char const* pSrc, size_t SrcLen) {
