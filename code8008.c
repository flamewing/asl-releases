...
static char const RegNames[] = "ABCDEHLM";
...
static char const Conditions[][4] = {"NC", "NZ", "P", "PO", "C", "Z", "M", "PE"};

static Boolean DecodeCondition_Z80(char const* pAsc, Byte* pResult) {
    for (*pResult = 0; *pResult < sizeof(Conditions) / sizeof(*Conditions);
         (*pResult)++) {
...
static tAdrMode DecodeAdr_Z80(tStrComp const* pArg, Word Mask, tAdrVals* pVals) {
    Boolean OK;
...
static Boolean CheckAcc_Z80(tStrComp const* pArg) {
    tAdrVals AdrVals;
...
static void DecodeJmp(Word Index) {
    if (ChkArgCnt(1, 1) && ChkZ80Syntax(eSyntax808x)) {
...
static void DecodeJP(Word Code) {
    ...
    /* one argument, new 8008 insn set & not exclusively Z80: -> jump on positive or
     * parity even */
...
static void DecodeMOV(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        ;
    else if (!DecodeReg(ArgStr[1].str.p_str, eSyntax808x, &DReg)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if (!DecodeReg(ArgStr[2].str.p_str, eSyntax808x, &SReg)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else if ((DReg == 7) && (SReg == 7)) {
        WrError(ErrNum_InvRegPair); /* MOV M,M not allowed - asame opcode as HLT */
    } else {
...
static void DecodeLD(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkZ80Syntax(eSyntaxZ80)) {
        ...
        case eModReg8:
            switch (DecodeAdr_Z80(
                    &ArgStr[2], MModReg8 | MModReg16 | MModIHL | MModImm, &SrcAdrVals)) {
            case eModReg8:
...
static void DecodeADD(Word Code) {
    ...
    case 1: /* 8008 (new) style - 8 bit register src only, dst is implicitly ACC */
        if (MomCPU == CPU8008) {
            ...
            if (!DecodeReg(ArgStr[1].str.p_str, eSyntax808x, &Reg)) {
...
static void DecodeSingleReg(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        ;
    else if (!DecodeReg(ArgStr[1].str.p_str, eSyntax808x, &Reg)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if (NoAM && ((Reg == 0) || (Reg == 7))) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else {
...
static char const FlagNames[] = "CZSP";

static void AddFixed(char const* NName, Byte NCode, Word SyntaxMask) {
    AddInstTable(InstTable, NName, (SyntaxMask << 8) | NCode, DecodeFixed);
}

static void AddFixeds(char const* NName, Byte NCode, int Shift, Byte RegMask) {
    char Memo[10], *p;
    int  Reg;
...
static void AddImm(char const* NName, Byte NCode, Word SyntaxMask) {
    AddInstTable(InstTable, NName, (SyntaxMask << 8) | NCode, DecodeImm);
}

static void AddImms(char const* NName, Byte NCode, int Pos) {
    char Memo[10], *p;
    int  z;
...
static void AddJmp(char const* NName, Byte NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeJmp);
}

static void AddJmps(char const* NName, Byte NCode, int Pos) {
    char Memo[10], *p;
    int  z;
