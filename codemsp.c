...
static IntType const OpSizeIntTypes[eOpSizeCnt] = {Int8, Int16, Int20};
...
static void ResetAdr(tAdrParts* pAdrParts) {
    ...
    pAdrParts->Part   = 0;
    pAdrParts->Cnt    = 0;
    pAdrParts->WasImm = pAdrParts->WasAbs = False;
}
...
static Boolean DecodeReg(tStrComp const* pArg, Word* pResult, Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static void FillAdrPartsImm(tAdrParts* pAdrParts, LongWord Value, Boolean ForceLong) {
    ...
    if (!ForceLong) {
        ...
        if ((Value == 0xffffffff) || ((OpSize == eOpSizeB) && (Value == 0xff))
...
static Boolean DecodeAdr(
        tStrComp const* pArg, tExtMode ExtMode, Byte Mask, Boolean MayImm,
        tAdrParts* pAdrParts) {
    ...
    if (*pArg->str.p_str == '#') {
        if (!MayImm) {
            ...
            AdrWord = EvalStrIntExpressionOffs(
                    pArg, 1 + ForceLong, OpSizeIntTypes[OpSize], &OK);
            if (OK) {
...
static Word GetMult(tStrComp const* pArg, Boolean* pOK) {
    Word Result = 0x0000;
...
static void AppendAdrVals(tAdrParts const* pParts) {
    Word i;
...
static void ConstructTwoOp(
        Word Code, tAdrParts const* pSrcParts, tAdrParts const* pDestParts) {
...
static void DecodeEmulOneToTwo(Word Code) {
    ...
    if (!DecodeAdr(
                &ArgStr[1], eExtModeNo,
                MModeAd | ((SrcSpec == 0xaa) ? MModeIRegAutoInc : 0), False,
                &DestParts)) {
        return;
    }
    ...
    if (SrcSpec == 0xaa) {
        ...
        if (SrcParts.Mode == eModeIRegAutoInc) {
            static Byte const MemLen[3] = {1, 2, 4};
...
static void DecodePOP(Word Code) {
    ...
    PCDist = 2;
    if (ChkArgCnt(1, 1) && DecodeAdr(&ArgStr[1], eExtModeNo, MModeAd, True, &DstParts)) {
        if (Odd(EProgCounter())) {
            WrError(ErrNum_AddrNotAligned);
        }
        ResetAdr(&SrcParts);
...
static void DecodePOPX(Word Code) {
    ...
    PCDist = 4;
    if (ChkArgCnt(1, 1) && DecodeAdr(&ArgStr[1], eExtModeYes, MModeAd, True, &DstParts)) {
        if (Odd(EProgCounter())) {
            WrError(ErrNum_AddrNotAligned);
        }
        ResetAdr(&SrcParts);
...
static void DecodeOneOp(Word Index) {
    OneOpOrder const* pOrder = OneOpOrders + Index;

    if (!ChkArgCnt(1, 1))
        ;
    else if (OpSize > eOpSizeW) {
        WrError(ErrNum_InvOpSize);
    } else if ((OpSize == eOpSizeB) && (!pOrder->MayByte)) {
        WrError(ErrNum_InvOpSize);
    } else {
        tAdrParts AdrParts;
...
static void DecodeOneOpX(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if (DecodeAdr(&ArgStr[1], eExtModeYes, 15, True, &AdrParts)) {
            ...
            CodeLen += 2;
            WAsmCode[CodeLen >> 1]
                    = pOrder->Code | ActBW | (AdrParts.Mode << 4) | AdrParts.Part;
            CodeLen += 2;
            AppendAdrVals(&AdrParts);
        }
    }
...
static void DecodeCLRA(Word Code) {
    if (ChkArgCnt(1, 1) && (DecodeReg(&ArgStr[1], &WAsmCode[0], True) == eIsReg)) {
        WAsmCode[0] |= Code;
        CodeLen = 2;
    }
}
...
static void DecodeRxM(Word Code) {
    if (!ChkArgCnt(2, 2))
        ;
    else if (OpSize == eOpSizeB) {
        WrError(ErrNum_InvOpSize);
    } else if (DecodeReg(&ArgStr[2], &WAsmCode[0], True) != eIsReg)
        ;
    else if (ArgStr[1].str.p_str[0] != '#') {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if (AdrParts.WasImm) {
            ...
            CodeLen     = 4;
                ...
                (AdrParts.Mode == eModeRegDisp)
                && (((AdrParts.Val & 0xfffff) > 0x7fff)
                    && ((AdrParts.Val & 0xfffff) < 0xf8000))) {
            WrError(ErrNum_OverRange);
        } else {
            WAsmCode[CodeLen >> 1] = 0x1340 | (AdrParts.Mode << 4) | (AdrParts.Part);
            CodeLen += 2;
            AppendAdrVals(&AdrParts);
        }
    }
...
static void DecodeJmp(Word Code) {
    ...
    {
        AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[1], UInt16, &OK, &Flags)
                 - (EProgCounter() + 2);
        if (OK) {
            if (Odd(AdrInt)) {
    ...
    *pArgPart1++ = '\0';
    ...
    if (pAttrPart) {
        AttrPart.Pos.Len = strmemcpy(
                AttrPart.str.p_str, STRINGSIZE, pAttrPart + 1, strlen(pAttrPart + 1));
        *pAttrPart = '\0';
    } else {
...
#define AddFixed(NName, NCode) AddInstTable(InstTable, NName, NCode, DecodeFixed)

static void AddTwoOp(char const* NName, Word NCode) {
...
static void AddEmulOneToTwo(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeEmulOneToTwo);
}

static void AddEmulOneToTwoX(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeEmulOneToTwoX);
}

static void AddOneOp(char const* NName, Boolean NMay, Boolean AllowX, Word NCode) {
    if (InstrZ >= OneOpCount) {
        exit(255);
    }
...
#define AddJmp(NName, NCode) AddInstTable(InstTable, NName, NCode, DecodeJmp)
...
static void InitFields(void) {
    ...
    InstrZ      = 0;
    AddOneOp("RRC", True, True, 0x1000);
    AddOneOp("RRA", True, True, 0x1100);
    AddOneOp("PUSH", True, True, 0x1200);
    AddOneOp("SWPB", False, True, 0x1080);
    AddOneOp("CALL", False, False, 0x1280);
    AddOneOp("SXT", False, True, 0x1180);
