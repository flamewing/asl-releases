...
static void ParseCPUArgs(tStrComp const* pArgs, tCPUArg const* pCPUArgs) {
    tCPUArg const* pCPUArg;
    char *         pNext, *pSep;
    ...
    do {
        ...
        if (!pSep) {
            WrStrErrorPos(ErrNum_ArgValueMissing, &Args);
        } else {
            StrCompSplitRef(&NameComp, &ValueComp, &Args, pSep);
            KillPrefBlanksStrCompRef(&NameComp);
            KillPostBlanksStrComp(&NameComp);
            KillPrefBlanksStrCompRef(&ValueComp);
            KillPostBlanksStrComp(&ValueComp);
            ...
            if (OK) {
                ...
                if (!pCPUArg->pName) {
                    WrStrErrorPos(ErrNum_UnknownArg, &NameComp);
...
static void SetCPUCore(tCPUDef const* pCPUDef, tStrComp const* pCPUArgs) {
    LargeInt    HCPU;
    int         Digit, Base;
    char const* pRun;

    tStrComp          TmpComp;
    static char const Default_CommentLeadIn[] = {';', '\0', '\0'};
    String            TmpCompStr;
    ...
    strmaxcpy(TmpCompStr, MomCPUName, sizeof(TmpCompStr));
    EnterIntSymbol(&TmpComp, HCPU, SegNone, True);
    strmaxcpy(TmpCompStr, MomCPUIdentName, sizeof(TmpCompStr));
    EnterStringSymbol(&TmpComp, MomCPUIdent, True);
    ...
    SetIsOccupiedFnc = SaveIsOccupiedFnc = RestoreIsOccupiedFnc = NULL;
    DecodeAttrPart                                              = NULL;
    SwitchIsOccupied = PageIsOccupied = ShiftIsOccupied = False;
    ChkPC                                               = DefChkPC;
    ASSUMERecCnt                                        = 0;
    pASSUMERecs                                         = NULL;
...
void SetCPUByType(CPUVar NewCPU, tStrComp const* pCPUArgs) {
...
Boolean SetCPUByName(tStrComp const* pName) {
    tCPUDef const* pCPUDef;
...
static void IntLine(
        char* pDest, size_t DestSize, LargeWord Inp, tIntConstMode ThisConstMode) {
...
static void CodeSECTION(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && ExpandStrSymbol(ExpName, sizeof(ExpName), &ArgStr[1])) {
        if (!ChkSymbName(ExpName)) {
            WrStrErrorPos(ErrNum_InvSymName, &ArgStr[1]);
        } else if (
                (PassNo == 1)
                && (GetSectionHandle(ExpName, False, MomSectionHandle) != -2)) {
            WrError(ErrNum_DoubleSection);
        } else {
...
static void CodeENDSECTION(Word Index) {
    ...
    if (!ChkArgCnt(0, 1))
        ;
    else if (!SectionStack) {
        WrError(ErrNum_NotInSection);
    } else if ((ArgCnt == 0) || (ExpandStrSymbol(ExpName, sizeof(ExpName), &ArgStr[1]))) {
        if ((ArgCnt == 1)
            && (GetSectionHandle(ExpName, False, SectionStack->Handle)
                != MomSectionHandle)) {
            WrStrErrorPos(ErrNum_WrongEndSect, &ArgStr[1]);
        } else {
            Tmp          = SectionStack;
            ...
            if (ArgCnt == 0) {
                as_snprintf(
                        ListLine, STRINGSIZE, "[%s]", GetSectionName(MomSectionHandle));
            }
...
static void CodeORG_Core(tStrComp const* pArg) {
    LargeWord    HVal;
    Boolean      ValOK;
    tSymbolFlags Flags;
...
static void CodeSETEQU(Word MayChange) {
    tStrComp const* pName    = *LabPart.str.p_str ? &LabPart : &ArgStr[1];
    ...
    if ((ArgCnt == ValIndex) && !strcmp(pName->str.p_str, PCSymbol)) {
        ...
        if (!mFirstPassUnknown(t.Flags)) {
            ...
            if (DestSeg >= SegCount) {
                WrStrErrorPos(ErrNum_UnknownSegment, &ArgStr[ValIndex + 1]);
            } else {
                SetListLineVal(&t);
                PushLocHandle(-1);
                switch (t.Typ) {
                ...
                case TempFloat:
                    ...
                    break;
                case TempString:
                    EnterNonZStringSymbolWithFlags(
                            pName, &t.Contents.str, MayChange, t.Flags);
                    break;
                case TempReg:
                    EnterRegSymbol(
                            pName, &t.Contents.RegDescr, t.DataSize, MayChange, False);
                    break;
                default:
                    break;
                }
...
static void CodeREGCore(tStrComp const* pNameArg, tStrComp const* pValueArg) {
    TempResult t;
    ...
    forallargs(pArg, True) {
        ...
        if (t.Relocs == NULL) {
            AddExport(pArg->str.p_str, t.Contents.Int, 0);
                ...
                (t.Relocs->Next != NULL) || (strcmp(t.Relocs->Ref, RelName_SegStart))) {
...
static void CodePHASE(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if (ActPC == StructSeg) {
        WrError(ErrNum_PhaseDisallowed);
    } else {
    ...
    if (!ChkArgCnt(0, 3))
        ...
        case TempInt:
            ...
            if (ChkRange(t.Contents.Int, 0, 255)) {
                if (ChkArgCnt(2, 3)) {
                    ...
                    if ((t.Typ == TempString)
                        && (t.Flags & eSymbolFlag_StringSingleQuoted)) {
                        TempResultToInt(&t);
                    }
                    switch (t.Typ) {
        ...
        case TempString:
            if (ChkArgCnt(1, 1)) /* Tabelle von Datei lesen */
            {
                ...
                if (fread(tfield, sizeof(char), 256, f) != 256) {
...
static void CodePRSET(Word Index) {
    ...
    for (z = 0; z < 16; z++) {
        ...
        for (z2 = 0; z2 < 16; z2++) {
            printf("%c",
                   CharTransTable[z * 16 + z2] > ' ' ? CharTransTable[z * 16 + z2] : '.');
        }
        putchar('\n');
    }
}
...
static void CodeCODEPAGE(Word Index) {
    ...
    if (!ChkArgCnt(1, 2))
        ;
    else if (!ChkSymbName(ArgStr[1].str.p_str)) {
        WrStrErrorPos(ErrNum_InvSymName, &ArgStr[1]);
    } else {
        if (!CaseSensitive) {
        ...
        if (!Source) {
            WrStrErrorPos(ErrNum_UnknownCodepage, &ArgStr[2]);
        } else {
...
static void CodeRESTORE(Word Index) {
    ...
    if (!ChkArgCnt(0, 0))
        ;
    else if (!FirstSaveState) {
        WrError(ErrNum_NoSaveFrame);
    } else {
        tStrComp TmpComp;
        String   TmpCompStr;
        ...
        EnterIntSymbol(&TmpComp, ListOn = Old->SaveListOn, SegNone, True);
...
static void CodeMACEXP(Word Index) {
  ...
  if (Index & 0x10)
  {
    ...
    if (!ChkArgCnt((Index & 0x0f) ? 0 : 1, ArgCntMax))
        ...
        if (OK) {
            if (!ChkLstMacroExpMod(&LstMacroExpMod)) {
                ...
                SetLstMacroExp(
                        ApplyLstMacroExpMod(eLstMacroExpAll, &LstMacroExpModDefault));
            }
        }
    }
}

static Boolean DecodeSegment(tStrComp const* pArg, Integer StartSeg, Integer* pResult) {
    Integer SegZ;
    Word    Mask;
...
static void CodeSEGMENT(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && DecodeSegment(&ArgStr[1], SegCode, &NewSegment)) {
        SetNSeg(NewSegment);
    }
}
...
static void CodeREAD(Word Index) {
    ...
    if (ChkArgCnt(1, 2)) {
        if (ArgCnt == 2) {
            EvalStrStringExpression(&ArgStr[1], &OK, Exp.str.p_str);
        } else {
        ...
        if (OK) {
            if (mFirstPassUnknown(Flags)) {
                ...
                if (1 == ArgCnt) {
                    DontPrint = !!CodeLen;
                    BookKeeping();
                } else if (CodeLen > (LongInt)MaxCodeLen) {
...
static void CodeASSUME(Word Index) {
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        ...
        while ((z1 <= ArgCnt) && (OK)) {
            ...
            while ((z2 < ASSUMERecCnt)
                   && (strcmp(pASSUMERecs[z2].Name, RegPart.str.p_str))) {
                z2++;
            }
            OK = (z2 < ASSUMERecCnt);
            if (!OK) {
                ...
                if (!as_strcasecmp(ValPart.str.p_str, "NOTHING")) {
                    if (pASSUMERecs[z2].NothingVal == -1) {
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        for (z = 1; z <= ArgCnt; z++) {
            ...
            if (p) {
                ...
                EnumCurrentValue
                        = EvalStrIntExpressionWithFlags(&SymPart, Int32, &OK, &Flags);
                if (!OK) {
                    return;
                }
                if (mFirstPassUnknown(Flags)) {
            ...
            EnterIntSymbol(
                    &ArgStr[z], EnumCurrentValue, (as_addrspace_t)EnumSegment, False);
            if (z == 1) {
...
static void CodeLISTING(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if (!OK) {
            WrStrErrorPos(ErrNum_OnlyOnOff, &ArgStr[1]);
        } else {
            tStrComp TmpComp;
            String   TmpCompStr;
...
void INCLUDE_SearchCore(tStrComp* pDest, tStrComp const* pArg, Boolean SearchPath) {
    ...
    if (SearchPath) {
        ...
        if (FSearch(FoundFileName, sizeof(FoundFileName), pDest->str.p_str, CurrFileName,
                    SearchPath ? IncludeList : "")) {
...
static void CodeBINCLUDE(Word Index) {
    ...
    if (!ChkArgCnt(1, 3))
        ...
        if (OK) {
            ...
            if (F == NULL) {
                ChkXIO(ErrNum_OpeningFile, FNameArg.str.p_str);
            }
            errno = 0;
            FSize = FileSize(F);
            ChkIO(ErrNum_FileReadError);
            if (Len == -1) {
                if ((Len = FSize - Ofs) < 0) {
                    fclose(F);
                    WrError(ErrNum_ShortRead);
                    return;
                }
            }
            if (!ChkPC(EProgCounter() + Len - 1)) {
...
static void CodeSTRUCT(Word IsUnion) {
    ...
    /* If named and embedded into another struct, add as element to innermost named parent
       struct. Add up all offsets of unnamed structs in between. */
    ...
    if (StructStack && (*LabPart.str.p_str)) {
        ...
        for (pRun = StructStack; pRun && pRun != pInnermostNamedStruct;
             pRun = pRun->Next) {
...
static void CodeENDSTRUCT(Word IsUnion) {
    ...
    if (!ChkArgCnt(0, 1))
        ...
        if (OK) {
            ...
            for (pInnermostNamedStruct = StructStack; pInnermostNamedStruct;
            ...
            if (ArgCnt == 0) {
                if (OStruct->Name[0]) {
                    ...
                    as_snprintf(
                            tmp2, sizeof(tmp2), "%s%clen", OStruct->Name,
                            OStruct->StructRec->ExtChar);
...
static void CodeEXTERN(Word Index) {
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        ...
        while ((OK) && (i <= ArgCnt)) {
            ...
            if (Type >= SegCount) {
                WrXError(ErrNum_UnknownSegment, Split + 1);
            } else {
...
static void CodePPSyms(PForwardSymbol* Orig, PForwardSymbol* Alt1, PForwardSymbol* Alt2) {
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        forallargs(pArg, True) {
            ...
            if (Lauf) {
                WrStrErrorPos(ErrNum_ContForward, pArg);
            } else {
...
Boolean CheckONOFFArg(tStrComp const* pArg, Boolean* pResult) {
...
static PseudoOrder const Pseudos[] = {
        {     "ALIGN",      CodeALIGN,    0},
        {      "ASEG",    CodeSEGTYPE,    0},
        {    "ASSUME",     CodeASSUME,    0},
...
Boolean CodeGlobalPseudo(void) {
    ...
    case 'E':
        ...
        break;
    case 'P':
        if ((!PageIsOccupied && Memo("PAGE")) || (PageIsOccupied && Memo("PAGESIZE"))) {
            CodePAGE(0);
            return True;
        }
        break;
    ...
    if (SectionStack) {
        if (Memo("FORWARD")) {
            if (PassNo <= MaxSymPass) {
                CodePPSyms(
                        &(SectionStack->LocSyms), &(SectionStack->GlobSyms),
        ...
        if (Memo("PUBLIC")) {
            CodePPSyms(
                    &(SectionStack->GlobSyms), &(SectionStack->LocSyms),
                    &(SectionStack->ExportSyms));
            return True;
        }
        if (Memo("GLOBAL")) {
            CodePPSyms(
                    &(SectionStack->ExportSyms), &(SectionStack->LocSyms),
