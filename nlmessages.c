...
static char const* IdentString = "AS Message Catalog - not readable\n\032\004";

static char const* EOpenMsg = "cannot open msg file";
static char const* ERdMsg   = "cannot read from msg file";
static char const* EIndMsg  = "string table index error";

static TMsgCat DefaultCatalog = {NULL, NULL, 0};
...
char* catgetmessage(PMsgCat Catalog, int Num) {
    if ((Num >= 0) && (Num < Catalog->MsgCount)) {
        ...
        as_snprintf(
                umess, STRINGSIZE, "catgetmessage: message number %d does not exist",
                Num);
        return umess;
    }
}
...
FILE* myopen(char const* name, LongInt MsgId1, LongInt MsgId2) {
    FILE*   tmpfile;
    String  line;
    LongInt RId1, RId2;
    Boolean EForm = False;
    ...
    if (EForm) {
        fclose(tmpfile);
        fprintf(stderr,
                "message catalog handling: warning: %s has invalid format or is out of "
                "date\n",
                name);
        return NULL;
    } else {
...
void opencatalog(
        PMsgCat Catalog, char const* File, char const* Path, LongInt MsgId1,
        LongInt MsgId2) {
    FILE*       MsgFile;
    char        str[2048], *ptr;
    char const* pSep;
        ...
        if (!MsgFile) {
            ...
            if (ptr) {
                ...
                if (!ptr) {
                    ...
                    /* absolutely last resort: if we were found via PATH (no slashes in
                       argv[0]/Path), look up this path and replace bin/ with lib/ for
                       'companion path': */

                    if (!MsgFile && !strrchr(Path, PATHSEP)
                        && !FSearch(Dest, sizeof(Dest), Path, NULL, ptr)) {
                        char* pSep2;
    ...
    if ((int)fread(Catalog->StrPosis + 1, 4, Catalog->MsgCount - 1, MsgFile) + 1
        != Catalog->MsgCount) {
        error(ERdMsg);
    }
...
void nlmessages_init(char const* File, char* ProgPath, LongInt MsgId1, LongInt MsgId2) {
