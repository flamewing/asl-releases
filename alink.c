...
static void ReadSymbols(char const* pSrcName, int Index) {
    FILE*    f;
    String   SrcName;
    ...
    DelSuffix(SrcName);
    AddSuffix(SrcName, STRINGSIZE, getmessage(Num_Suffix));
    if (Verbose >= 2) {
                ...
                for (z = 0; z < PNew->RelocInfo->ExportCount; z++) {
                    if (GetExport(PNew->RelocInfo->ExportEntries[z].Name, &Dummy)) {
                        fprintf(stderr, "%s: %s '%s'\n", SrcName,
                                getmessage(Num_DoubleDefSymbol),
...
static void ProcessFile(char const* pSrcName, int Index) {
    FILE*       f;
    String      SrcName;
    PPart       PartRun;
    Byte        Header, CPU, Gran, Segment;
        ...
        /* records with relocation: basically the same, plus the real work...
           the appended relocation info will be skipped in the next loop run. */
            ...
            for (z = 0; z < PartRun->RelocInfo->RelocCount; z++) {
                ...
                if (Found) {
                    if (Verbose >= 2) {
                        printf("%s 0x%" PRIx64 "...", getmessage(Num_InfoMsgReading),
                               (unsigned long long)PReloc->Addr);
                    }
                    RelocVal  = GetValue(PReloc->Type, PReloc->Addr - PartRun->CodeStart);
                    NRelocVal = (PReloc->Type & RelocFlagSUB) ? RelocVal - Value
                                                              : RelocVal + Value;
...
static CMDResult CMD_Verbose(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
int main(int argc, char** argv) {
    ...
    nlmessages_init("alink.msg", *argv, MsgId1, MsgId2);
    ioerrs_init(*argv);
    cmdarg_init(*argv);
    toolutils_init(*argv);
    ...
    ProcessCMD(
            argc, argv, ALINKParams, ALINKParamCnt, ParUnprocessed, "ALINKCMD",
            ParamError);
    ...
    for (PartRun = PartList; PartRun; PartRun = PartRun->Next) {
        if (PartRun->MustReloc) {
            ...
            if (Verbose >= 2) {
                printf("%s 0x%" PRIx64 "\n", getmessage(Num_InfoMsgLocating),
                       PartRun->CodeStart);
            }
