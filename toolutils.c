...
char const* OutName = "STDOUT"; /* Pseudoname Output */
...
void WrCopyRight(char const* Msg) {
...
void AddSuffix(char* pName, unsigned NameSize, char const* Suff) {
    char *p, *z, *Part;
...
void FormatError(char const* Name, char const* Detail) {
    fprintf(stderr, "%s%s%s (%s)\n", catgetmessage(&MsgCat, Num_FormatErr1aMsg), Name,
            catgetmessage(&MsgCat, Num_FormatErr1bMsg), Detail);
    fprintf(stderr, "%s\n", catgetmessage(&MsgCat, Num_FormatErr2Msg));
    exit(3);
}

void ChkIO(char const* Name) {
    int io;
    ...
    fprintf(stderr, "%s%s%s\n", catgetmessage(&MsgCat, Num_IOErrAHeaderMsg), Name,
...
void ReadRecordHeader(
        Byte* Header, Byte* CPU, Byte* Segment, Byte* Gran, char const* Name, FILE* f) {
#ifdef _WIN32
...
void WriteRecordHeader(
    ...
    if ((*Header == FileHeaderEnd) || (*Header == FileHeaderStartAdr)) {
        ...
        if ((*Segment != SegCode) || (*Gran != Granularity(*CPU, *Segment))
            || (*CPU >= 0x80)) {
...
void SkipRecord(Byte Header, char const* Name, FILE* f) {
    int      Length;
...
PRelocInfo ReadRelocInfo(FILE* f) {
    ...
    if (PInfo != NULL) {
        ...
        if ((Read4(f, &PInfo->RelocCount)) && (Read4(f, &PInfo->ExportCount))
            && (Read4(f, &StringLen))) {
            /* allocate memory */

            PInfo->RelocEntries
                    = (PRelocEntry)malloc(sizeof(TRelocEntry) * PInfo->RelocCount);
            if ((PInfo->RelocCount == 0) || (PInfo->RelocEntries != NULL)) {
                PInfo->ExportEntries
                        = (PExportEntry)malloc(sizeof(TExportEntry) * PInfo->ExportCount);
                if ((PInfo->ExportCount == 0) || (PInfo->ExportEntries != NULL)) {
                    ...
                    if ((StringLen == 0) || (PInfo->Strings != NULL)) {
                        ...
                        for (z = 0, PEntry = PInfo->RelocEntries; z < PInfo->RelocCount;
                             z++, PEntry++) {
                        ...
                        for (z = 0, PExp = PInfo->ExportEntries; z < PInfo->ExportCount;
                             z++, PExp++) {
                            if (!Read4(f, &StringPos)) {
                                break;
                            }
...
CMDResult CMD_FilterList(Boolean Negate, char const* Arg) {
    Byte    FTemp;
    Boolean err;
    char*   p;
    int     Search;
...
extern CMDResult CMD_Range(
        LongWord* pStartAddr, LongWord* pStopAddr, Boolean* pStartAuto,
        Boolean* pStopAuto, char const* Arg) {
    char const* p;
    String      StartStr;
    Boolean     ok;
...
CMDResult CMD_QuietMode(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
void EraseFile(char const* FileName, LongWord Offset) {
    UNUSED(Offset);
...
void toolutils_init(char const* ProgPath) {
...
Boolean AddressWildcard(char const* addr) {
