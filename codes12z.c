...
static tSymbolSize const RegSizes[16] = {
...
static Boolean DecodeRegStr(char const* pArg, Byte* pRes) {
    if ((strlen(pArg) == 2) && (toupper(*pArg) == 'D')
        && ((pArg[1] >= '0') && (pArg[1] <= '7'))) {
        static Byte const RegCodes[8] = {4, 5, 0, 1, 2, 3, 6, 7};
...
static Boolean DecodeAdrRegStr(char const* pArg, Byte* pRes) {
    static char const Regs[4][3] = {"X", "Y", "S", "PC"};
    unsigned          z;
...
static Boolean ShortImm(
        LongInt Value, ShortInt OpSize, Byte* pShortValue, Byte* pShiftLSB) {
...
static void CopyIndirect(tStrComp* pDest, tStrComp const* pSrc) {
    pDest->Pos.Len = strmemcpy(
            pDest->str.p_str, STRINGSIZE, pSrc->str.p_str + 1,
            strlen(pSrc->str.p_str) - 2);
...
static Boolean DecodeAdr(int ArgIndex, unsigned ModeMask, tAdrVals* pVals) {
    ...
    if (*ArgStr[ArgIndex].str.p_str == '#') {
        ...
        if ((ModeMask & MModeMemReg)
            && (ShortImm(Value, OpSize, &pVals->Arg, &pVals->ShiftLSB))) {
            pVals->Mode = AdrModeMemReg;
            pVals->Arg |= 0x70;
        } else {
    ...
    if (IsIndirect(ArgStr[ArgIndex].str.p_str)) {
        IndirMode = eIndirModePar;
            ...
            (l >= 2) && (ArgStr[ArgIndex].str.p_str[0] == '[')
            && (ArgStr[ArgIndex].str.p_str[l - 1] == ']')) {
    ...
    if (IndirMode) {
        ...
        while (True) {
            ...
            if (DecodeRegStr(RunComp.str.p_str, &DataReg)) {
                ...
                AdrRegPresent = True;
                    ...
                    IsIncDec(*RunComp.str.p_str, &IncChar)
                    && DecodeAdrRegStr(RunComp.str.p_str + 1, &AdrIncReg)) {
                if (IncMode) {
                    WrStrErrorPos(ErrNum_InvAddrMode, &RunComp);
                    goto done;
                }
        ...
        if ((IndirMode == eIndirModePar) && IncMode && !DispAcc && !AdrRegPresent
            && !DataRegPresent) {
            switch (AdrIncReg) {
            case 0:
            case 1:
        ...
        /* (disp,XYSP) */
                ...
                (IndirMode == eIndirModePar) && AdrRegPresent && !DataRegPresent
                && !IncMode) {
            if ((AdrReg == 3) && (HasDisp)) {
        ...
        /* [Dn,XY] */
                ...
                (IndirMode == eIndirModeSquare) && AdrRegPresent && DataRegPresent
                && !IncMode && !DispAcc) {
            if (AdrReg >= 2) {
                goto error;
            } else {
...
static Boolean IsImmediate(tAdrVals const* pVals, ShortInt OpSize, Byte* pImmVal) {
    switch (pVals->Mode) {
    case AdrModeImm:
...
static Boolean IsReg(tAdrVals const* pVals, Byte* pReg) {
    switch (pVals->Mode) {
    case AdrModeReg:
        *pReg = pVals->Arg;
        return True;
...
static LongWord AssembleBitfieldSymbol(
        Byte BitPos, Byte Width, ShortInt OpSize, Word Address) {
    LongWord CodeOpSize = (OpSize == eSymbolSize24Bit) ? 3 : OpSize;
    int      AddrShift  = (OpSize == eSymbolSize24Bit) ? 5 : (3 + OpSize);

    return BitPos | (((LongWord)Address & 0xfff) << AddrShift) | (CodeOpSize << 20)
...
static Boolean DecodeBitfieldArg2(
        LongWord* pResult, tStrComp const* pRegArg, tStrComp* pBitArg, ShortInt OpSize) {
    Boolean  OK;
    LongWord Addr;
    Word     BitSpec;
...
static Boolean DecodeBitfieldArg(
        LongWord* pResult, int Start, int Stop, ShortInt OpSize) {
    *pResult = 0;
...
static Boolean DissectBitSymbol(
        LongWord BitSymbol, Word* pAddress, Byte* pBitPos, Byte* pWidth,
        tSymbolSize* pOpSize) {
...
static void DissectBit_S12Z(char* pDest, size_t DestSize, LargeWord Inp) {
    ...
    if (BitWidth > 1) {
        as_snprintf(
                pDest, DestSize, "$%x(%c).%u:%u", (unsigned)Address, Attribute,
                (unsigned)BitWidth, (unsigned)BitPos);
    } else {
        as_snprintf(
                pDest, DestSize, "$%x(%c).%u", (unsigned)Address, Attribute,
                (unsigned)BitPos);
    }
}
...
static void ExpandS12ZBit(
    ...
    if (pInnermostNamedStruct) {
        ...
        PushLocHandle(-1);
        EnterIntSymbol(
                pVarName, AssembleBitSymbol(pStructElem->BitPos, OpSize, Address),
                SegBData, False);
        PopLocHandle();
        /* TODO: MakeUseList? */
    }
...
static void ExpandS12ZBitfield(
    ...
    if (pInnermostNamedStruct) {
        ...
        if (!ChkRange(Address, 0, 0xfff)
            || !ChkRange(pStructElem->BitPos, 0, (8 << OpSize) - 1)
            || !ChkRange(
                    pStructElem->BitPos + pStructElem->BitWidthM1, 0,
                    (8 << OpSize) - 1)) {
            return;
        }
        ...
        EnterIntSymbol(
                ...
                AssembleBitfieldSymbol(
                        pStructElem->BitPos, pStructElem->BitWidthM1 + 1, OpSize,
                        Address),
                SegBData, False);
        PopLocHandle();
        /* TODO: MakeUseList? */
...
static Boolean DecodeBranchCore(int ArgIndex) {
    ...
    ShortDist = EvalStrIntExpressionWithFlags(&ArgStr[ArgIndex], UInt24, &OK, &Flags)
    ...
    if (OpSize == eSymbolSizeUnknown) {
        OpSize = ((ShortDist <= 63) && (ShortDist >= -64)) ? eSymbolSizeFloat32Bit
                                                           : eSymbolSize32Bit;
    }
    switch (OpSize) {
    case eSymbolSize32Bit:
...
static void DecodeReg(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeRegArg(1, &Reg, 0xff) && SetOpSize(RegSizes[Reg])) {
        PutCode(Code | Reg);
    }
}
...
static void DecodeTwoReg(Word Code) {
    ...
    if (*AttrPart.str.p_str) {
        WrError(ErrNum_UseLessAttr);
            ...
            ChkArgCnt(2, 2) && DecodeRegArg(2, &DestReg, 0xff)
...
static void DecodeRegMemImm(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeRegArg(1, &Reg, 0xff) && SetOpSize(RegSizes[Reg])
...
static void DecodeSUB(Word Code) {
    ...
    if (ArgCnt == 3) {
        if (DecodeRegArg(1, &Reg, 1 << 6) && DecodeAdrRegArg(2, &Reg, 3)
            && DecodeAdrRegArg(3, &Reg, 1 << (1 - Reg))) {
            BAsmCode[CodeLen++] = 0xfe - Reg;
        }
    } else if (
            ChkArgCnt(2, 3) && DecodeRegArg(1, &Reg, 0xff) && SetOpSize(RegSizes[Reg])
            && DecodeAdr(2, MModeImm | MModeMemReg, &AdrVals)) {
        if (AdrVals.Mode == AdrModeImm) {
            PutCode(Code | Reg);
        } else {
...
static void DecodeImm8(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && SetOpSize(eSymbolSize8Bit)
        && DecodeAdr(1, MModeImm, &AdrVals)) {
        PutCode(Code);
        AppendAdrVals(&AdrVals);
    }
}
...
static void DecodeShift(Word Code) {
    if (ChkArgCnt(2, 3)) {
        ...
        Boolean     IsASL = (Code == 0xc0), IsASR = (Code == 0x80);
        ...
        /* REG-REG-OPR1/2/3 only allowed with ASL: convert to (REG-)OPR1/2/3-OPR1/2/3 for
           other instructions, unless count is immediate: */

        if (!IsASL && DestIsReg && (OpAdrVals.Mode == AdrModeReg) && !ImmediateCnt
            && (CntAdrVals.Mode == AdrModeMemReg)) {
        ...
        if (DestIsReg && (OpAdrVals.Mode == AdrModeReg)
            && (CntAdrVals.Mode == AdrModeReg)) {
        ...
        /* REG-IMM with n=1..2 */
                ...
                DestIsReg && (OpAdrVals.Mode == AdrModeReg) && ImmediateCnt
                && (ImmCnt >= 1) && (ImmCnt <= 2)) {
            BAsmCode[CodeLen++] = (IsASR ? 0x00 : 0x10) | DestReg;
            BAsmCode[CodeLen++]
                    = Code | (IsASR ? 0x10 : 0x00) | ((ImmCnt - 1) << 3) | OpAdrVals.Arg;
        }
        ...
        /* REG-IMM with arbitrary n */
            ...
            BAsmCode[CodeLen++] = 0x10 | DestReg;
            BAsmCode[CodeLen++]
                    = Code | (IsASL ? 0x00 : 0x10) | ((ImmCnt & 1) << 3) | OpAdrVals.Arg;
        ...
        /* REG-OPR1/2/3 - ASL only */
                ...
                IsASL && DestIsReg && (OpAdrVals.Mode == AdrModeReg)
                && (CntAdrVals.Mode == AdrModeMemReg)) {
            BAsmCode[CodeLen++] = 0x10 | DestReg;
        ...
        /* (REG-)OPR1/2/3-OPR1/2/3 */
                ...
                DestIsReg && (OpAdrVals.Mode == AdrModeMemReg)
                && (CntAdrVals.Mode == AdrModeMemReg)) {
            BAsmCode[CodeLen++] = 0x10 | DestReg;
        ...
        /* (src-)OPR/1/2/3-IMM (n=1..2) */
                ...
                (OpAdrVals.Mode == AdrModeMemReg) && ImmediateCnt && (ImmCnt >= 1)
                && (ImmCnt <= 2)) {
...
static void DecodeBitField(Word Code) {
    ...
    if (2 == ArgCnt) {
        ...
        if (DecodeRegArg(RegArg, &Reg, 0xff)
            ...
            && SetOpSize(ThisOpSize) && SizeCode2(OpSize, &SizeCode)) {
        ...
        case AdrModeReg:
            ...
            case AdrModeReg:
                ...
                if (16 == ParamReg) {
                    BAsmCode[CodeLen++]
                            = Code | 0x20 | (SrcAdrVals.Arg << 2) | Hi(ParamImm);
            ...
            case AdrModeMemReg:
                if (OpSize == eSymbolSizeUnknown) {
                    ...
                    if (16 == ParamReg) {
                        BAsmCode[CodeLen++]
                                = Code | 0x70 | (SizeCode << 2) | Hi(ParamImm);
                        BAsmCode[CodeLen++] = Lo(ParamImm);
                    } else {
        ...
        case AdrModeMemReg:
            ...
            case AdrModeReg:
                if (OpSize == eSymbolSizeUnknown) {
                    ...
                    if (16 == ParamReg) {
                        BAsmCode[CodeLen++]
                                = Code | 0x60 | (SizeCode << 2) | Hi(ParamImm);
                        BAsmCode[CodeLen++] = Lo(ParamImm);
                    } else {
...
static void DecodeINC_DEC(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && DecodeAdr(1, MModeReg | MModeMemReg, &AdrVals)) {
        ...
        case AdrModeMemReg: {
            ...
            if (OpSize == eSymbolSizeUnknown) {
                WrError(ErrNum_UndefOpSizes);
            } else if (!SizeCode2(OpSize, &SizeCode) || (OpSize == eSymbolSize24Bit)) {
                WrError(ErrNum_UndefOpSizes);
            } else {
...
static void DecodeDIV_MOD(Word Code) {
    ...
    case AdrModeReg:
        ...
        case AdrModeReg:
            ...
            BAsmCode[CodeLen++]
                    = LoCode | (DividentAdrVals.Arg << 3) | DivisorAdrVals.Arg;
            break;
        case AdrModeImm:
            if (!SizeCode2(OpSize, &DivisorSizeCode) || (OpSize == eSymbolSize24Bit)) {
                WrError(ErrNum_UndefOpSizes);
            } else {
                PutCode(EffCode | DestReg);
                BAsmCode[CodeLen++]
                        = LoCode | 0x44 | (DividentAdrVals.Arg << 3) | DivisorSizeCode;
        ...
        case AdrModeMemReg:
            if (!SizeCode2(OpSize, &DivisorSizeCode) || (OpSize == eSymbolSize24Bit)) {
                ...
                PutCode(EffCode | DestReg);
                BAsmCode[CodeLen++]
                        = LoCode | 0x40 | (DividentAdrVals.Arg << 3) | DivisorSizeCode;
        ...
        break;
    case AdrModeMemReg:
        /* divident==register is filtered out before, so divident size cannot be set from
         * register */
        if (!SizeCode2(OpSize, &DividentSizeCode) || (OpSize == eSymbolSize24Bit)) {
            WrError(ErrNum_UndefOpSizes);
        } else {
            OpSize = OpSize2;
            ...
            case AdrModeMemReg:
                if ((OpSize == eSymbolSizeUnknown)
                    && IsReg(&DivisorAdrVals, &DivisorReg)) {
                    SetOpSize(RegSizes[DivisorReg]);
                }
                if (!SizeCode2(OpSize, &DivisorSizeCode)
                    || (OpSize == eSymbolSize24Bit)) {
                    WrError(ErrNum_UndefOpSizes);
                } else {
                    PutCode(EffCode | DestReg);
                    BAsmCode[CodeLen++] = LoCode | 0x42 | (DividentSizeCode << 4)
                                          | (DivisorSizeCode << 2);
...
static void DecodeEXG_TFR(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeGenRegArg(1, &SrcReg) && DecodeGenRegArg(2, &DestReg)) {
        if ((OpSizeByteLen(RegSizes[SrcReg]) >= OpSizeByteLen(RegSizes[DestReg]))
            && Hi(Code)) {
        ...
        case AdrModeReg:
            ...
            case AdrModeMemReg: {
                ...
                if ((OpSize == eSymbolSize8Bit)
                    && IsImmediate(
                            &SrcAdrVals, OpSize, &ImmVal)) /* same instr length for byte,
        ...
        case AdrModeAReg:
            ...
            case AdrModeImm:
                ...
                if ((DestAdrVals.Arg < 2) && (SrcAdrVals.Vals[0] < 0x04)) {
                    BAsmCode[CodeLen++]
                            = 0xca | DestAdrVals.Arg | (SrcAdrVals.Vals[0] << 4);
...
static void DecodeLEA(Word Code) {
    ...
    if (*AttrPart.str.p_str) {
        ...
        switch (DestAdrVals.Mode) {
        case AdrModeReg:
            if (DestAdrVals.Arg < 6) {
                WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
            } else if (DecodeAdr(2, MModeMemReg, &SrcAdrVals)) {
                if (IsReg(&SrcAdrVals, &Reg)) {
                    WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[2]);
                } else {
            ...
            if (DestAdrVals.Arg > 2) {
                ...
                if (IsReg(&SrcAdrVals, &Reg)) {
                    ...
                    if (((SrcAdrVals.Arg & 0xce) == 0xc0) /* ...,(XYS,i9) */
                        && ((SrcAdrVals.Arg & 0x01)
                            == ((SrcAdrVals.Vals[0] >> 7) & 1)) /* i9 is i8 */
                        && (DestAdrVals.Arg
                            == ((SrcAdrVals.Arg >> 4) & 3))) /* destreg==srcreg */
                    {
...
static void DecodeMOV(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(2, MModeMemReg, &DestAdrVals)) {
...
static void DecodePSH_PUL(Word Code) {
    ...
    int               z;
    Byte              Reg;
    static Word const RegMasks[8]
            = {0x0002, 0x0001, 0x2000, 0x1000, 0x0008, 0x004, 0x0800, 0x0400};
...
static void DecodeTBcc(Word Code) {
    ...
    if (!ChkArgCnt(2, 2)
        || !DecodeAdr(1, MModeReg | MModeAReg | MModeMemReg | MModeImm, &AdrVals)) {
        return;
    }
...
static void AddFixed(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeFixed);
}

static void AddBranch(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeBranch);
}

static void AddReg(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeReg);
}

static void AddRegMemImm(char const* pName, Word Code) {
    AddInstTable(InstTable, pName, Code, DecodeRegMemImm);
}

static void AddCondition(char const* pName, Word Code, InstProc Proc) {
    char InstrName[20];

    as_snprintf(InstrName, sizeof(InstrName), "%s%s", pName, "NE");
    AddInstTable(InstTable, InstrName, Code | (0 << 4), Proc);
    as_snprintf(InstrName, sizeof(InstrName), "%s%s", pName, "EQ");
    AddInstTable(InstTable, InstrName, Code | (1 << 4), Proc);
    as_snprintf(InstrName, sizeof(InstrName), "%s%s", pName, "PL");
    AddInstTable(InstTable, InstrName, Code | (2 << 4), Proc);
    as_snprintf(InstrName, sizeof(InstrName), "%s%s", pName, "MI");
    AddInstTable(InstTable, InstrName, Code | (3 << 4), Proc);
    as_snprintf(InstrName, sizeof(InstrName), "%s%s", pName, "GT");
    AddInstTable(InstTable, InstrName, Code | (4 << 4), Proc);
    as_snprintf(InstrName, sizeof(InstrName), "%s%s", pName, "LE");
    AddInstTable(InstTable, InstrName, Code | (5 << 4), Proc);
}
...
static Boolean DecodeAttrPart_S12Z(void) {
    ...
    for (z = 0; z < 2; z++) {
        ...
        if (!DecodeMoto16AttrSize(
                    AttrPart.str.p_str[z], z ? &OpSize2 : &AttrPartOpSize, True)) {
            return False;
        }
    }
    return True;
...
static void SwitchTo_S12Z(void) {
    PFamilyDescr const pDescr = FindFamilyByName("S12Z");
    TurnWords                 = False;
