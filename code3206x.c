...
static char const UnitNames[UnitCnt][3]
...
static Boolean DecodeMem(tStrComp const* pArg, LongWord* Erg, LongWord Scale) {
...
static Boolean DecodeAdr(
        tStrComp const* pArg, Byte Mask, Boolean Signed, LongWord* AdrVal) {
    Boolean OK;
    ...
    if (DecodeReg(pArg->str.p_str, AdrVal, &OK, False)) {
        ...
        *AdrVal = Signed ? EvalStrIntExpression(pArg, SInt5, &OK) & 0x1f
...
static Boolean DecideUnit(LongWord Reg, char const* Units) {
    Integer z;
    TUnit   TestUnit;
                ...
                if (OK) {
                    ...
                    case TempString: {
                        ...
                        for (z2 = 0; z2 < t.Contents.str.len; z2++) {
                            ...
                            DAsmCode[cnt - 1]
                                    += (((LongWord)CharTransTable
                                                 [((usint)t.Contents.str.p_str[z2])
                                                  & 0xff]))
                                       << (8 * (3 - (z2 & 3)));
                        }
                        break;
                    ...
                    case TempFloat:
                        if (!FloatRangeCheck(t.Contents.Float, Float32)) {
                            ...
                            Double_2_ieee4(
                                    t.Contents.Float, (Byte*)(DAsmCode + cnt),
                                    HostBigEndian);
                            cnt++;
                        }
                        break;
...
static Boolean CodeD(LongWord OpCode, LongWord Dest, LongWord Src1, LongWord Src2) {
    ThisInst = 0x40 + (OpCode << 7) + (UnitFlag << 1) + (Dest << 23) + (Src2 << 18)
               + (Src1 << 13);
    return True;
}
...
static void DecodeIDLE(Word Index) {
    ...
    if (!ChkArgCnt(0, 0))
        ;
    else if ((ThisCross) || (ThisUnit != NoUnit)) {
        WrError(ErrNum_UndefAttr);
    } else {
        ThisInst = 0x0001e000;
        erg      = True;
    ...
    if (ChkArgCnt(3, 3)) {
        if ((DecodeAdr(&ArgStr[3], MModReg, POrder->DSign, &DReg))
            ...
            if (DecodeAdr(&ArgStr[2], MModReg, POrder->SSign2, &S2Reg)) {
                AddSrc(S2Reg);
                DecodeAdr(
                        &ArgStr[1], (POrder->MayImm ? MModImm : 0) + MModReg,
                        POrder->SSign1, &S1Reg);
                switch (AdrMode) {
                case ModReg:
                    if ((ThisCross) && (!IsCross(S2Reg)) && (!IsCross(S1Reg))) {
                        WrError(ErrNum_InvAddrMode);
                    } else if ((IsCross(S2Reg)) && (IsCross(S1Reg))) {
                        WrError(ErrNum_InvAddrMode);
                    } else {
                        if (IsCross(S1Reg)) {
                            SwapReg(&S1Reg, &S2Reg);
                    ...
                    break;
                case ModImm:
                    erg = Memo("MPY") ? CodeM(POrder->Code - 1, DReg, S1Reg, S2Reg)
...
static void DecodeMemO(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (DecodeAdr(pArg2, MModReg, False, &DReg)) {
            ...
            if (OK) {
                OK = (S1Reg & 8) ? ChkUnit(0x1e, D1, D2)
                                 : ChkUnit((S1Reg >> 18) & 31, D1, D2);
            }
            if (OK) {
                ThisInst
                        = S1Reg + (DReg << 23) + (POrder->Code << 4) + ((DReg & 16) >> 3);
                erg = True;
            }
        }
    }
...
static void DecodeSTP(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && ChkUnit(0x10, S1, S2)) {
        if (DecodeAdr(&ArgStr[1], MModReg, False, &S2Reg)) {
            if ((ThisCross) || (S2Reg < 16)) {
                WrError(ErrNum_InvAddrMode);
            } else {
                AddSrc(S2Reg);
...
static void DecodeABS(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && DecodeReg(ArgStr[2].str.p_str, &DReg, &DPFlag, True)
        && ChkUnit(DReg, L1, L2)
        && DecodeReg(ArgStr[1].str.p_str, &S1Reg, &S1Flag, True)) {
        if (DPFlag != S1Flag) {
            WrError(ErrNum_InvAddrMode);
        } else if ((ThisCross) && ((S1Reg >> 4) == UnitFlag)) {
            WrError(ErrNum_InvAddrMode);
        } else {
            SetCross(S1Reg);
            if (DPFlag) {
    ...
    if (ChkArgCnt(3, 3)) {
        ...
        case ModLReg: /* ADD ?,?,long */
            ...
            case ModReg: /* ADD int,?,long */
                ...
                case ModReg: /* ADD int,int,long */
                    if (ChkUnit(DReg, L1, L2)) {
                        if ((ThisCross) && (!IsCross(S1Reg)) && (!IsCross(S2Reg))) {
                            WrError(ErrNum_InvAddrMode);
                        } else if ((IsCross(S1Reg)) && (IsCross(S2Reg))) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            AddSrc(S2Reg);
                            if (IsCross(S1Reg)) {
                ...
                case ModLReg: /* ADD int,long,long */
                    if (ChkUnit(DReg, L1, L2)) {
                        if (IsCross(S2Reg)) {
                            WrError(ErrNum_InvAddrMode);
                        } else if ((ThisCross) && (!IsCross(S1Reg))) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            AddLSrc(S2Reg);
                            SetCross(S1Reg);
            ...
            case ModLReg: /* ADD long,?,long */
                ...
                case ModReg: /* ADD long,int,long */
                    if (ChkUnit(DReg, L1, L2)) {
                        if (IsCross(S1Reg)) {
                            WrError(ErrNum_InvAddrMode);
                        } else if ((ThisCross) && (!IsCross(S2Reg))) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            AddSrc(S2Reg);
                            SetCross(S2Reg);
            ...
            case ModImm: /* ADD imm,?,long */
                if (DecodeAdr(
                            &ArgStr[2], MModLReg, True, &S2Reg)) { /* ADD imm,long,long */
                    if (ChkUnit(DReg, L1, L2)) {
                        if (IsCross(S2Reg)) {
                            WrError(ErrNum_InvAddrMode);
                        } else if (ThisCross) {
        ...
        case ModReg: /* ADD ?,?,int */
            ...
            case ModReg: /* ADD int,?,int */
                ...
                case ModReg: /* ADD int,int,int */
                    AddSrc(S2Reg);
                    if (((DReg ^ S1Reg) > 15) && ((DReg ^ S2Reg) > 15)) {
                        WrError(ErrNum_InvAddrMode);
                    } else if (
                            (ThisCross) && ((DReg ^ S1Reg) < 16)
                            && ((DReg ^ S2Reg) < 15)) {
                        WrError(ErrNum_InvAddrMode);
                    } else {
                        if ((S1Reg ^ DReg) > 15) {
                        ...
                        if (OK) {
                            switch (ThisUnit) {
                            case L1:
                            case L2: /* ADD.Lx int,int,int */
                                erg = CodeL(0x03, DReg, S1Reg, S2Reg);
                                break;
                            case S1:
                            case S2: /* ADD.Sx int,int,int */
                                erg = CodeS(0x07, DReg, S1Reg, S2Reg);
                                break;
                            case D1:
                            case D2: /* ADD.Dx int,int,int */
            ...
            if (ChkUnit(DReg, L1, L2)) {
                ...
                case ModReg: /* ADDU int,?,long */
                    ...
                    case ModReg: /* ADDU int,int,long */
                        if ((IsCross(S1Reg)) && (IsCross(S2Reg))) {
                            WrError(ErrNum_InvAddrMode);
                        } else if (
                                (ThisCross)
                                && (((S1Reg ^ DReg) < 16) && ((S2Reg ^ DReg) < 16))) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            if ((S1Reg ^ DReg) > 15) {
                                SwapReg(&S1Reg, &S2Reg);
                    ...
                    case ModLReg: /* ADDU int,long,long */
                        if (IsCross(S2Reg)) {
                            WrError(ErrNum_InvAddrMode);
                        } else if ((ThisCross) && ((S1Reg ^ DReg) < 16)) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            AddLSrc(S2Reg);
                            SetCross(S1Reg);
    ...
    if (ChkArgCnt(3, 3)) {
        ...
        case ModReg:
            ...
            case ModReg:
                ...
                switch (AdrMode) {
                case ModReg:
                    if ((ThisCross) && ((S1Reg ^ DReg) < 16) && ((S2Reg ^ DReg) < 16)) {
                        WrError(ErrNum_InvAddrMode);
                    } else if (((S1Reg ^ DReg) > 15) && ((S2Reg ^ DReg) > 15)) {
                        WrError(ErrNum_InvAddrMode);
                    } else {
                        AddSrc(S2Reg);
                        ThisCross = ((S1Reg ^ DReg) > 15) || ((S2Reg ^ DReg) > 15);
                        if ((S1Reg ^ DReg) > 15) {
                            OK = DecideUnit(DReg, "L");
                        } else if ((S2Reg ^ DReg) > 15) {
                            OK = DecideUnit(DReg, "LS");
                        } else {
                            OK = DecideUnit(DReg, "LSD");
                        }
                        if (OK) {
                            switch (ThisUnit) {
                            case L1:
                            case L2:
                                if ((S1Reg ^ DReg) > 15) {
                                    erg = CodeL(0x17, DReg, S1Reg, S2Reg);
                                } else {
                                    erg = CodeL(0x07, DReg, S1Reg, S2Reg);
                                }
                                break;
                            case S1:
                            case S2:
                ...
                case ModImm:
                    if (ChkUnit(DReg, D1, D2)) {
                        if ((ThisCross) || ((S1Reg ^ DReg) > 15)) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            erg = CodeD(0x13, DReg, S2Reg, S1Reg);
                        }
                    }
                    break;
                }
            ...
            if (ChkUnit(DReg, L1, L2)) {
                ...
                case ModImm:
                    if (DecodeAdr(&ArgStr[2], MModLReg, True, &S2Reg)) {
                        if ((ThisCross) || (/*NOT*/ IsCross(S2Reg))) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            AddLSrc(S2Reg);
                ...
                case ModReg:
                    ...
                    if (DecodeAdr(&ArgStr[2], MModReg, True, &S2Reg)) {
                        if ((ThisCross) && (!IsCross(S1Reg)) && (!IsCross(S2Reg))) {
                            ...
                            /* what did I do here? */
                            erg = IsCross(S1Reg) ? CodeL(0x37, DReg, S1Reg, S2Reg)
...
static void DecodeSUBU(Word Index) {
    ...
    if (ChkArgCnt(3, 3) && DecodeAdr(&ArgStr[3], MModLReg, False, &DReg)
        && ChkUnit(DReg, L1, L2)) {
        AddLDest(DReg);
        if (DecodeAdr(&ArgStr[1], MModReg, False, &S1Reg)) {
            ...
            if (DecodeAdr(&ArgStr[2], MModReg, False, &S2Reg)) {
                if ((ThisCross) && (!IsCross(S1Reg)) && (!IsCross(S2Reg))) {
                    ...
                    AddSrc(S2Reg);
                    ThisCross = IsCross(S1Reg) || IsCross(S2Reg);
                    erg       = IsCross(S1Reg) ? CodeL(0x3f, DReg, S1Reg, S2Reg)
...
static void DecodeSUBC(Word Index) {
    ...
    if (ChkArgCnt(3, 3) && DecodeAdr(&ArgStr[3], MModReg, False, &DReg)
        && ChkUnit(DReg, L1, L2)) {
        AddLDest(DReg);
        if (DecodeAdr(&ArgStr[1], MModReg, False, &S1Reg)) {
            if (DecodeAdr(&ArgStr[2], MModReg, False, &S2Reg)) {
                if ((ThisCross) && (!IsCross(S2Reg))) {
                    WrError(ErrNum_InvAddrMode);
...
static void DecodeLinAdd(Word Index) {
    ...
    if (!ChkArgCnt(3, 3))
        ...
        if ((DecodeAdr(&ArgStr[3], MModReg, True, &DReg)) && (ChkUnit(DReg, D1, D2))) {
            AddDest(DReg);
...
static void DecodeADDK(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[2], MModReg, False, &DReg)
        && ChkUnit(DReg, S1, S2)) {
        AddDest(DReg);
...
static void DecodeADD2_SUB2(Word Index) {
    ...
    Index = (Index << 5) + 1;
    if (ChkArgCnt(3, 3) && DecodeAdr(&ArgStr[3], MModReg, True, &DReg)
        && ChkUnit(DReg, S1, S2)) {
        AddDest(DReg);
...
static void DecodeMV(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModLReg: /* MV ?,long */
            AddLDest(DReg);
            if (DecodeAdr(&ArgStr[1], MModLReg, True, &SReg)) { /* MV long,long */
                if (ChkUnit(DReg, L1, L2)) {
                    if (IsCross(SReg)) {
                        WrError(ErrNum_InvAddrMode);
                    } else if (ThisCross) {
    ...
    if (ChkArgCnt(3, 3)) {
        if (DecodeAdr(&ArgStr[3], MModReg, True, &DReg)) {
            ...
            if ((OK) && (DecideUnit(DReg, "LS"))) {
                if ((!WithImm) && (IsCross(S1Reg)) && (IsCross(S2Reg))) {
                    WrError(ErrNum_InvAddrMode);
                } else if (
                        (ThisCross) && (!IsCross(S2Reg))
                        && ((WithImm) || (!IsCross(S1Reg)))) {
                    WrError(ErrNum_InvAddrMode);
                } else {
...
static void DecodeCmp(Word Index) {
    CmpOrder const* pOrder = CmpOrders + Index;
    LongWord        DReg, S1Reg, S2Reg;
    ...
    if (ChkArgCnt(3, 3) && DecodeAdr(&ArgStr[3], MModReg, False, &DReg)
        ...
        case ModReg:
            ...
            case ModReg:
                if ((IsCross(S1Reg)) && (IsCross(S2Reg))) {
                    WrError(ErrNum_InvAddrMode);
                } else if ((ThisCross) && (!IsCross(S1Reg)) && (!IsCross(S2Reg))) {
                    WrError(ErrNum_InvAddrMode);
                } else {
                    AddSrc(S2Reg);
                    if (IsCross(S1Reg)) {
...
static void DecodeSADD(Word Code) {
    ...
    if (ChkArgCnt(3, 3) && DecodeAdr(&ArgStr[3], MModReg + MModLReg, True, &DReg)
        && ChkUnit(DReg, L1, L2)) {
        switch (AdrMode) {
        case ModReg:
            AddDest(DReg);
...
static void DecodeSAT(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[2], MModReg, True, &DReg)
        && ChkUnit(DReg, L1, L2)) {
        AddDest(DReg);
...
static void DecodeMVC(Word Code) {
    ...
    if ((ThisUnit != NoUnit) && (ThisUnit != S2)) {
        WrError(ErrNum_InvAddrMode);
        ...
        if (z > 0) {
            if (DecodeAdr(&ArgStr[z], MModReg, False, &S1Reg)) {
                if ((ThisCross) && ((z == 2) || (IsCross(S1Reg)))) {
                    WrError(ErrNum_InvAddrMode);
                } else {
                    if (z == 1) {
                        AddSrc(S1Reg);
...
static void DecodeB(Word Code) {
    ...
    if (ArgCnt != 1) {
        ...
        WrError(ErrNum_InvAddrMode);
    } else if ((ThisUnit != NoUnit) && (ThisUnit != S1) && (ThisUnit != S2)) {
        WrError(ErrNum_InvAddrMode);
    } else {
        OK      = True;
        S2Reg   = 0;
    ...
    for (z1 = 0; z1 < ParCnt; z1++) {
        for (z2 = z1 + 1; z2 < ParCnt; z2++) {
            if ((ParRecs[z1].OpCode >> 28) == (ParRecs[z2].OpCode >> 28)) {
                ...
                if ((z & 1) == 1) {
                    WrXError(ErrNum_UnitMultipleUsed, "Addr. A");
                }
                if ((z & 2) == 2) {
                    WrXError(ErrNum_UnitMultipleUsed, "Addr. B");
                }

                /* Hauptspeicherpfade */

                if ((z & 4) == 4) {
                    WrXError(ErrNum_UnitMultipleUsed, "LdSt. A");
                }
                if ((z & 8) == 8) {
                    WrXError(ErrNum_UnitMultipleUsed, "LdSt. B");
                }
...
static void AddLinAdd(char const* NName, LongInt NCode) {
    if (InstrZ >= LinAddCnt) {
        exit(255);
    }
...
static void AddCmp(char const* NName, LongInt NCode) {
    if (InstrZ >= CmpCnt) {
        exit(255);
    }
...
static void AddMem(char const* NName, LongInt NCode, LongInt NScale) {
    if (InstrZ >= MemCnt) {
        exit(255);
    }
...
static void AddMul(
        char const* NName, LongInt NCode, Boolean NDSign, Boolean NSSign1,
        Boolean NSSign2, Boolean NMay) {
    if (InstrZ >= MulCnt) {
        exit(255);
    }
...
static void AddCtrl(char const* NName, LongInt NCode, Boolean NWr, Boolean NRd) {
    if (InstrZ >= CtrlCnt) {
        exit(255);
    }
...
static void InitFields(void) {
    ...
    LinAddOrders = (FixedOrder*)malloc(sizeof(FixedOrder) * LinAddCnt);
    InstrZ       = 0;
    AddLinAdd("ADDAB", 0x30);
    AddLinAdd("ADDAH", 0x34);
    AddLinAdd("ADDAW", 0x38);
    AddLinAdd("SUBAB", 0x31);
    AddLinAdd("SUBAH", 0x35);
    AddLinAdd("SUBAW", 0x39);

    CmpOrders = (CmpOrder*)malloc(sizeof(CmpOrder) * CmpCnt);
    InstrZ    = 0;
    AddCmp("CMPEQ", 0x50);
    AddCmp("CMPGT", 0x44);
    AddCmp("CMPGTU", 0x4c);
    ...
    MemOrders = (MemOrder*)malloc(sizeof(MemOrder) * MemCnt);
    InstrZ    = 0;
    AddMem("LDB", 2, 1);
    AddMem("LDH", 4, 2);
    AddMem("LDW", 6, 4);
    ...
    MulOrders = (MulOrder*)malloc(sizeof(MulOrder) * MulCnt);
    InstrZ    = 0;
    ...
    CtrlRegs = (CtrlReg*)malloc(sizeof(CtrlReg) * CtrlCnt);
    InstrZ   = 0;
