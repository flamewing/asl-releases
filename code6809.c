...
static char const FlagChars[] = "CVZNIHFE";
...
static unsigned ChkZero(char const* s, Byte* Erg) {
    if (*s == '>') {
        *Erg = 1;
        return 1;
    } else if (*s == '<') {
...
static Boolean IsZeroOrEmpty(tStrComp const* pArg) {
    Boolean OK;
    LongInt Value;
...
static void DecodeAdr(int ArgStartIdx, int ArgEndIdx, unsigned OpcodeLen) {
    ...
    if ((*pStartArg->str.p_str == '[')
        && (pStartArg->str.p_str[strlen(pStartArg->str.p_str) - 1] == ']')) {
    ...
    if ((AdrArgCnt >= 1) && (AdrArgCnt <= 2) && (strlen(pEndArg->str.p_str) == 2)
        && (*pEndArg->str.p_str == '-') && (CodeReg(pEndArg->str.p_str + 1, &EReg))) {
        if ((AdrArgCnt == 2) && !IsZeroOrEmpty(pStartArg)) {
    ...
    if ((AdrArgCnt >= 1) && (AdrArgCnt <= 2) && (strlen(pEndArg->str.p_str) == 3)
        && (!strncmp(pEndArg->str.p_str, "--", 2))
        && (CodeReg(pEndArg->str.p_str + 2, &EReg))) {
        if ((AdrArgCnt == 2) && !IsZeroOrEmpty(pStartArg)) {
            WrError(ErrNum_InvAddrMode);
        } else {
            AdrCnt     = 1;
    ...
    if ((AdrArgCnt >= 1) && (AdrArgCnt <= 2)
        && (!as_strcasecmp(pEndArg->str.p_str, "--W"))) {
        if ((AdrArgCnt == 2) && !IsZeroOrEmpty(pStartArg)) {
            WrError(ErrNum_InvAddrMode);
    ...
    if ((AdrArgCnt <= 2) && (AdrArgCnt >= 1) && (CodeReg(pEndArg->str.p_str, &EReg))) {
        ...
        if (ZeroMode > 1) {
            ...
            AdrInt = EvalStrIntExpressionOffsWithFlags(
                    pStartArg, Offset, Int8, &OK, &Flags);
        ...
        /* 5-Bit-Displacement */
                ...
                (ZeroMode == 3)
                || ((ZeroMode == 0) && (!IndFlag) && (AdrInt >= -16) && (AdrInt <= 15))) {
            if ((AdrInt < -16) || (AdrInt > 15)) {
    ...
    if ((AdrArgCnt <= 2) && (AdrArgCnt >= 1) && (MomCPU >= CPU6309)
        && (!as_strcasecmp(pEndArg->str.p_str, "W"))) {
    ...
    if ((AdrArgCnt == 2)
        && ((!as_strcasecmp(pEndArg->str.p_str, "PCR"))
            || (!as_strcasecmp(pEndArg->str.p_str, "PC")))) {
        ...
        if (OK) {
            if (ZeroMode == 3) {
                WrError(ErrNum_InvAddrMode);
                    ...
                    (ZeroMode == 2)
                    || ((ZeroMode == 0) && (Hi(AdrInt) == DPRValue) && (!IndFlag))) {
                if (IndFlag) {
                    WrError(ErrNum_NoIndir);
                } else if (Hi(AdrInt) != DPRValue) {
                    WrError(ErrNum_NoShortAddr);
                } else {
                    AdrCnt     = 1;
                    AdrMode    = ModDir;
...
#define RegCnt (sizeof(RegNames) / sizeof(*RegNames))
    static char const RegNames[][4]
            = {"D", "X", "Y",   "U",   "S",  "SP", "PC", "W", "V",
               "A", "B", "CCR", "DPR", "CC", "DP", "Z",  "E", "F"};
    static Byte const RegVals[RegCnt]
    ...
    for (z = 0; z < RegCnt; z++) {
        if (!strcmp(Asc, RegNames[z])) {
            if (((RegVals[z] & 6) == 6)
                && !ChkMinCPUExt(CPU6309, ErrNum_AddrModeNotSupported))
                ;
            else {
                *Erg = RegVals[z];
                return True;
...
static void DecodeRel(Word Index) {
    ...
    RelOrder const* pOrder   = RelOrders + (Index & 0x7fff);
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (OK) {
            ...
            if (!mSymbolQuestionable(Flags) && !LongFlag
...
static void DecodeALU(Word Index) {
    ALUOrder const* pOrder = ALUOrders + Index;
    ...
    if (ChkArgCnt(1, 2) && ChkMinCPU(pOrder->MinCPU)) {
        ...
        if (AdrMode != ModNone) {
            if ((!pOrder->MayImm) && (AdrMode == ModImm)) {
                WrError(ErrNum_InvAddrMode);
            } else {
                if (Hi(pOrder->Code)) {
...
static void DecodeLDQ(Word Index) {
    ...
    if (ChkArgCnt(1, 2) && ChkMinCPU(CPU6309)) {
...
static void DecodeRMW(Word Index) {
    BaseOrder const* pOrder = RMWOrders + Index;
...
static void DecodeFlag(Word Index) {
    FlagOrder const* pOrder = FlagOrders + Index;
    Boolean          OK;
    char const*      p;
    int              z2, z3;
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        ...
        for (z2 = 1; z2 <= ArgCnt; z2++) {
            if (OK) {
                p = (strlen(ArgStr[z2].str.p_str) == 1)
                            ? strchr(FlagChars, as_toupper(*ArgStr[z2].str.p_str))
                            : NULL;
                if (p) {
                    z3 = p - FlagChars;
                    if (pOrder->Inv) {
...
static void DecodeImm(Word Index) {
    BaseOrder const* pOrder = ImmOrders + Index;
...
static void DecodeBit(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU6309) && SplitBit(&ArgStr[1], &z2)
        && SplitBit(&ArgStr[2], &z3)) {
        if (!CodeCPUReg(ArgStr[1].str.p_str, BAsmCode + 2)) {
            WrError(ErrNum_InvRegName);
        } else if ((BAsmCode[2] < 8) || (BAsmCode[2] > 11)) {
            WrError(ErrNum_InvRegName);
        } else {
            DecodeAdr(2, 2, 3);
            if (AdrMode != ModDir) {
        ...
        if ((Inc1 != 0) || (Inc2 != 0)) {
            if (Memo("EXG")) {
                WrError(ErrNum_InvAddrMode);
                    ...
                    !CodeCPUReg(ArgStr[1].str.p_str, BAsmCode + 3) || (BAsmCode[3] > 4)) {
                WrStrErrorPos(ErrNum_InvRegName, &ArgStr[1]);
            } else if (
                    !CodeCPUReg(ArgStr[2].str.p_str, BAsmCode + 2) || (BAsmCode[2] > 4)) {
            ...
            WrError(ErrNum_InvAddrMode);
        } else if (!CodeCPUReg(ArgStr[1].str.p_str, BAsmCode + 2)) {
            WrError(ErrNum_InvRegName);
        } else if (!CodeCPUReg(ArgStr[2].str.p_str, BAsmCode + 1)) {
            WrError(ErrNum_InvRegName);
        } else if (
                (BAsmCode[1] != 13)
                && (BAsmCode[2] != 13) /* Z Register compatible to all other registers */
                && (((BAsmCode[1] ^ BAsmCode[2]) & 0x08) != 0)) {
...
static void DecodeALU2(Word Code) {
    if (!ChkArgCnt(2, 2))
        ;
    else if (!CodeCPUReg(ArgStr[1].str.p_str, &BAsmCode[3])) {
        WrError(ErrNum_InvRegName);
    } else if (!CodeCPUReg(ArgStr[2].str.p_str, &BAsmCode[2])) {
        WrError(ErrNum_InvRegName);
    } else if (
            (BAsmCode[2] != 13)
            && (BAsmCode[3] != 13) /* Z Register compatible to all other registers */
            && (((BAsmCode[2] ^ BAsmCode[3]) & 0x08) != 0)) {
        WrError(ErrNum_ConfOpSizes);
    } else {
        CodeLen     = 3;
        BAsmCode[0] = 0x10;
...
static void DecodeLEA(Word Index) {
    BaseOrder const* pOrder = LEAOrders + Index;
...
static void DecodeStack(Word Index) {
    BaseOrder const* pOrder = StackOrders + Index;
    ...
    *StackRegNames[StackRegCnt - 2]
            = OpPart.str.p_str[strlen(OpPart.str.p_str) - 1] ^ ('S' ^ 'U');
    for (z2 = 1; z2 <= ArgCnt; z2++) {
        if (OK) {
...
static void AddFixed(char const* NName, Word NCode, CPUVar NCPU) {
...
static void AddRel(char const* NName, Word NCode8, Word NCode16) {
    char LongName[30];
...
static void AddALU(char const* NName, Word NCode, Byte NSize, Boolean NImm, CPUVar NCPU) {
...
static void AddALU2(char const* NName) {
    char RName[30];
...
static void AddRMW(char const* NName, Word NCode, CPUVar NCPU) {
    if (InstrZ >= RMWOrderCnt) {
        exit(255);
    }
...
static void AddFlag(char const* NName, Word NCode, Boolean NInv, CPUVar NCPU) {
    if (InstrZ >= FlagOrderCnt) {
        exit(255);
    }
...
static void AddLEA(char const* NName, Word NCode, CPUVar NCPU) {
    if (InstrZ >= LEAOrderCnt) {
        exit(255);
    }
...
static void AddImm(char const* NName, Word NCode, CPUVar NCPU) {
...
static void AddStack(char const* NName, Word NCode, CPUVar NCPU) {
    if (InstrZ >= StackOrderCnt) {
        exit(255);
    }
...
#define ASSUME09Count (sizeof(ASSUME09s) / sizeof(*ASSUME09s))
    static ASSUMERec const ASSUME09s[] = {
