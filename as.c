...
static PInputTag GenerateProcessor(void) {
    ...
    PInp->UsesNumArgs = PInp->UsesAllArgs = False;
...
static POutputTag GenerateOUTProcessor(SimpProc Processor, tErrorNum OpenErrMsg) {
    ...
    POut->UsesNumArgs = POut->UsesAllArgs = False;
    *POut->GName                          = '\0';
...
static Boolean MacroStart(void) {
    return ((Memo("MACRO")) || (Memo("IRP")) || (Memo("IRPN")) || (Memo("IRPC"))
            || (Memo("REPT")) || (Memo("WHILE")));
}
...
typedef void (*tMacroArgCallback)(Boolean CtrlArg, tStrComp const* pArg, void* pUser);
    ...
    if (FirstOutputTag->NestLevel == -1) {
        if (IfAsm) {
            ...
            if ((FirstOutputTag->DoGlobCopy) && (SectionStack)) {
                ...
                GMacro->ParamDefVals
                        = DuplicateStringList(FirstOutputTag->Mac->ParamDefVals);
...
static Boolean ReadMacro_SearchArg(char const* pTest, char const* pComp, Boolean* pErg) {
    if (!as_strcasecmp(pTest, pComp)) {
        *pErg = True;
        return True;
    } else if (
            (strlen(pTest) > 2) && (!as_strncasecmp(pTest, "NO", 2))
            && (!as_strcasecmp(pTest + 2, pComp))) {
        *pErg = False;
        return True;
    } else {
        return False;
...
static Boolean ReadMacro_SearchSect(
        char* Test_O, char const* Comp, Boolean* Erg, LongInt* Section) {
    char*  p;
    String Test, Sect;
    ...
    if ((strlen(Test) > 2) && (!as_strncasecmp(Test, "NO", 2))
        && (!as_strcasecmp(Test + 2, Comp))) {
        *Erg = False;
        return True;
...
static void ExpandPList(String PList, char const* pArg, Boolean CtrlArg) {
    if (!*PList) {
...
static void ProcessMACROArgs(Boolean CtrlArg, tStrComp const* pArg, void* pUser) {
    ...
    if (CtrlArg) {
        ...
        if (ReadMacro_SearchArg(
            ...
            if (!AddLstMacroExpMod(
                        &pContext->LstMacroExpMod, DoMacExp, eLstMacroExpAll)) {
            ...
            if (!AddLstMacroExpMod(
                        &pContext->LstMacroExpMod, DoMacExp, eLstMacroExpMacro)) {
                WrStrErrorPos(ErrNum_TooManyMacExpMod, pArg);
            }
            ExpandPList(pContext->PList, pArg->str.p_str, CtrlArg);
        } else if (ReadMacro_SearchArg(pArg->str.p_str, "EXPREST", &DoMacExp)) {
            if (!AddLstMacroExpMod(
                        &pContext->LstMacroExpMod, DoMacExp, eLstMacroExpRest)) {
                WrStrErrorPos(ErrNum_TooManyMacExpMod, pArg);
            }
            ExpandPList(pContext->PList, pArg->str.p_str, CtrlArg);
        } else if (ReadMacro_SearchArg(
        ...
        AddStringListLast(
                &(pContext->pOutputTag->ParamDefVals), pDefault ? pDefault : "");
        pContext->ParamCount++;
    }
}
...
static void ReadMacro(void) {
    ...
    *Context.PList         = '\0';
    ...
    OneMacro->FirstLine = OneMacro->ParamNames = OneMacro->ParamDefVals = NULL;
...
static Boolean MACRO_GetPos(PInputTag PInp, char* dest, size_t DestSize) {
    as_snprintf(
            dest, DestSize, "%s(%lu) ", PInp->SpecName.str.p_str,
            (unsigned long)(PInp->LineZ - 1));
    return False;
}
...
static void ExpandMacro(PMacroRec OneMacro) {
    ...
    if ((NestMax > 0) && (OneMacro->UseCounter > NestMax)) {
        ...
        for (z1 = 1; z1 <= ArgCnt; z1++) {
            ...
            if (p) {
                ...
                *p++ = '\0';
                ...
                for (pParamName = OneMacro->ParamNames, pArg = Tag->Params; pParamName;
                     pParamName = pParamName->Next, pArg = pArg->Next) {
            ...
            /* empty positional parameters mean using defaults - insert non-empty args
               here: */
        ...
        for (pParamName = OneMacro->ParamNames, pArg = Tag->Params,
            pDefault                = OneMacro->ParamDefVals;
             pParamName; pParamName = pParamName->Next, pArg = pArg->Next,
            pDefault = pDefault->Next) {
            if (!pArg->Content) {
...
static void IRP_Cleanup(PInputTag PInp) {
    ...
    if (PInp->Processor == IRP_Processor) {
        for (Lauf = PInp->Params; Lauf->Next; Lauf = Lauf->Next) { /* empty */
        }
...
static void IRP_OutProcessor(void) {
    ...
    if (FirstOutputTag->NestLevel > -1) {
        ...
        int          ParIter
                = FirstOutputTag->Tag->ParIter == 0 ? 1 : FirstOutputTag->Tag->ParIter;
...
static void ProcessIRPArgs(Boolean CtrlArg, tStrComp const* pArg, void* pUser) {
...
static void ProcessIRPNArgs(Boolean CtrlArg, tStrComp const* pArg, void* pUser) {
    ...
    if (CtrlArg) {
        ...
        if (0 == pContext->ArgCnt) {
            ...
            pContext->ParamCnt
                    = EvalStrIntExpressionWithFlags(pArg, Int32, &ValOK, &SymbolFlags);
...
static void ProcessIRPCArgs(Boolean CtrlArg, tStrComp const* pArg, void* pUser) {
...
static void ProcessREPTArgs(Boolean CtrlArg, tStrComp const* pArg, void* pUser) {
...
static void WHILE_OutProcessor(void) {
    ...
    if (FirstOutputTag->NestLevel == -1) {
        ...
        FirstOutputTag    = FirstOutputTag->Next;
        Tmp->Tag->IsEmpty = !Tmp->Tag->Lines;
        Erg               = EvalStrIntExpressionWithFlags(
                &Tmp->Tag->SpecName, Int32, &OK, &SymbolFlags);
        if (mFirstPassUnknown(SymbolFlags)) {
...
static void ProcessWHILEArgs(Boolean CtrlArg, tStrComp const* pArg, void* pUser) {
...
static void INCLUDE_Cleanup(PInputTag PInp) {
    ...
    if ((*LstName != '\0') && !QuietMode) {
        ...
        as_snprintf(
                Tmp, sizeof(Tmp), "%s(%lu)", NamePart(CurrFileName),
...
static Boolean INCLUDE_GetPos(PInputTag PInp, char* dest, size_t DestSize) {
    ...
    as_snprintf(
            dest, DestSize, GNUErrors ? "%s:%lu" : "%s(%lu) ",
            NamePart(PInp->SpecName.str.p_str), (unsigned long)PInp->LineZ);
    return !GNUErrors;
}
...
static void ExpandINCLUDE_Core(tStrComp const* pArg, Boolean SearchPath) {
    tStrComp  FNameArg;
    String    FNameArgStr;
    PInputTag Tag;
...
static void ReallocStr(tAllocStr* pStr, unsigned NewAllocLen) {
    if (NewAllocLen > pStr->AllocLen) {
        ...
        pNewStr     = pStr->AllocLen ? (char*)realloc(pStr->pStr, NewAllocLen)
...
void WriteCode(void) {
    ...
    if ((ActPC != StructSeg) && (!ChkPC(EProgCounter() + CodeLen - 1))
        && (CodeLen != 0)) {
...
static void Produce_Code(void) {
    ...
    case 'I':
        ...
        Found = True;
        if (Memo("IRP")) {
            ResetLastLabel = !ExpandIRP();
        } else if (Memo("IRPN")) {
            ResetLastLabel = !ExpandIRPN();
        } else if (Memo("IRPC")) {
            ResetLastLabel = !ExpandIRPC();
        } else {
            Found = False;
        }
        break;
    ...
    if (!Found) {
        ...
        case 'S':
            ...
            Found = True;
            if (Memo(ShiftIsOccupied ? "SHFT" : "SHIFT")) {
                ExpandSHIFT();
            } else {
                Found = False;
            }
    ...
    /* Makroaufruf ? */
        ...
        if (IfAsm) {
            ...
            if ((MacroNestLevel > 1) && (MacroNestLevel < 100)) {
                as_snprintf(
                        ListLine, STRINGSIZE, "%*s(MACRO-%u)", (int)MacroNestLevel - 1,
                        "", MacroNestLevel);
            } else {
        ...
        if (IfAsm) {
            ...
            if (IsStruct) {
                ...
                strmaxcpy(
                        ListLine, OneStruct->IsUnion ? "(UNION)" : "(STRUCT)",
                        STRINGSIZE);
            } else {
            ...
            if (MacProOutput
                && ((*OpPart.str.p_str != '\0') || (*LabPart.str.p_str != '\0')
                    || (*CommPart.str.p_str != '\0'))) {
                errno = 0;
...
static void adjust_copy_comp(tStrComp* p_comp, char const* p_src, size_t newsz) {
...
static void SplitLine(void) {
    ...
    if ((pRun < pEnd) && *pRun && !as_isspace(*pRun)) {
        ...
        if ((LabPart.Pos.Len > 0)
            && (LabPart.str.p_str[LabPart.Pos.Len - 1] == ':')) { /* needed? */
    ...
    while (True) {
        ...
        if (strchr(DivideChars, *pRun)) {
            ...
            OpPart.Pos.Len
                    = strmemcpy(OpPart.str.p_str, OpPart.str.capacity, pRun, pPos - pRun);
            ...
            if ((*LabPart.str.p_str == '\0') && OpPart.Pos.Len
                && (OpPart.str.p_str[OpPart.Pos.Len - 1] == ':')) {
    ...
    again:
        ...
        if (pAttrPos) {
            AttrSplit = (*pAttrPos);
            AttrPart.Pos.StartCol
                    = OpPart.Pos.StartCol + (pAttrPos + 1 - OpPart.str.p_str);
            AttrPart.Pos.Len = strmemcpy(
                    AttrPart.str.p_str, STRINGSIZE, pAttrPos + 1, strlen(pAttrPos + 1));
            *pAttrPos      = '\0';
            OpPart.Pos.Len = pAttrPos - OpPart.str.p_str;

            /* The dot-prefixed OpPart may itself contain an attribute (.instr.attr).  So
               reiterate splitting off attribute, but only once ;-) */
    ...
    if (*ArgPart.str.p_str) {
        char const *pDivPos, *pActDiv, *pActDivPos;
      ...
      if (!pDivPos)
        pDivPos = pEnd;
            ...
            ArgStr[ArgCnt].Pos.StartCol
                    = ArgPart.Pos.StartCol + (pRun - ArgPart.str.p_str);
    ...
    *OneLine.p_str = *CurrFileName = '\0';
    ...
    while (!InputEnd() && !ENDOccured) {
        ...
        *ListLine = '\0';
        ...
        if (!QuietMode) {
            NxtTime = GTime();
            if (((!ListToStdout) || ((ListMask & 1) == 0))
                && (DTime(ListTime, NxtTime) > 50)) {
                String Num;
...
static char const* TWrite_Plur(int n) {
...
static void TWrite(long DTime, char* dest, size_t DestSize) {
    ...
    *dest = '\0';
    ...
    if (h > 0) {
        as_snprcatf(
                dest, DestSize, "%d%s%s, ", h, getmessage(Num_ListHourName),
                TWrite_Plur(h));
    }
    ...
    if (h > 0) {
        as_snprcatf(
                dest, DestSize, "%d%s%s, ", h, getmessage(Num_ListMinuName),
                TWrite_Plur(h));
    }
    ...
    DTime %= 100;
    as_snprcatf(
            dest, DestSize, "%d.%02d%s%s", h, (int)DTime, getmessage(Num_ListSecoName),
            TWrite_Plur(h));
}
...
static void AssembleFile_InitPass(void) {
    ...
    FirstIfSave    = NULL;
    FirstSaveState = NULL;
    StructStack = pInnermostNamedStruct = NULL;
    ...
    ResetMacroDefines();
    ResetStructDefines();
    strmaxcpy(TmpCompStr, FlagTrueName, sizeof(TmpCompStr));
    EnterIntSymbol(&TmpComp, 1, SegNone, True);
    strmaxcpy(TmpCompStr, FlagFalseName, sizeof(TmpCompStr));
    EnterIntSymbol(&TmpComp, 0, SegNone, True);
    strmaxcpy(TmpCompStr, PiName, sizeof(TmpCompStr));
    EnterFloatSymbol(&TmpComp, 4.0 * atan(1.0), True);
    strmaxcpy(TmpCompStr, VerName, sizeof(TmpCompStr));
    EnterIntSymbol(&TmpComp, VerNo, SegNone, True);
    as_snprintf(ArchVal, sizeof(ArchVal), "%s-%s", ARCHPRNAME, ARCHSYSNAME);
    strmaxcpy(TmpCompStr, ArchName, sizeof(TmpCompStr));
    ...
    EnterIntSymbol(&TmpComp, 0, SegNone, True);
#endif
    strmaxcpy(TmpCompStr, CaseSensName, sizeof(TmpCompStr));
    EnterIntSymbol(&TmpComp, Ord(CaseSensitive), SegNone, True);
    if (PassNo == 0) {
    ...
    strmaxcpy(TmpCompStr, NestMaxName, sizeof(TmpCompStr));
...
static void AssembleFile_WrSummary(char const* pStr) {
    if (!QuietMode) {
...
static void AssembleFile(char* Name) {
    ...
    as_snprintf(
            s, sizeof(s), "%7" PRILongInt "%s", LineSum,
            getmessage((LineSum == 1) ? Num_InfoMessAssLine : Num_InfoMessAssLines));
    AssembleFile_WrSummary(s);

    if (LineSum != MacLineSum) {
        as_snprintf(
                s, sizeof(s), "%7" PRILongInt "%s", MacLineSum,
                getmessage(
                        (MacLineSum == 1) ? Num_InfoMessMacAssLine
                                          : Num_InfoMessMacAssLines));
        AssembleFile_WrSummary(s);
    }

    as_snprintf(
            s, sizeof(s), "%7d%s", (int)PassNo,
    ...
    as_snprintf(
            s, sizeof(s), "%7u%s%s", (unsigned)ErrorCount, getmessage(Num_InfoMessErrCnt),
            (ErrorCount == 1) ? "" : getmessage(Num_InfoMessErrPCnt));
    AssembleFile_WrSummary(s);

    as_snprintf(
            s, sizeof(s), "%7u%s%s", (unsigned)WarnCount, getmessage(Num_InfoMessWarnCnt),
...
#ifdef PROFILE_MEMO
    {
        ...
        as_snprintf(
                s, sizeof(s), "%4lu.%02lu%s", Sum / 100, Sum % 100, " Oppart Compares");
        if (!QuietMode) {
            WrConsoleLine(s, True);
        }
        if (ListMode == 2) {
...
static void AssembleGroup(char const* pFileMask) {
    String FileMask;
...
static CMDResult CMD_SharePascal(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_ShareC(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_ShareAssembler(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_DebugMode(Boolean Negate, char const* pArg) {
    String Arg;
...
static CMDResult CMD_ListConsole(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_ListRadix(Boolean Negate, char const* Arg) {
    Boolean   OK;
...
static CMDResult CMD_ListFile(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_SuppWarns(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_UseList(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_CrossList(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_SectionList(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_BalanceTree(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_MakeDebug(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_MacProOutput(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_MacroOutput(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_MakeIncludeList(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_CodeOutput(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_MsgIfRepass(Boolean Negate, char const* Arg) {
    Boolean OK;
    UNUSED(Arg);
...
static CMDResult CMD_Relaxed(Boolean Negate, char const* pArg) {
    UNUSED(pArg);
...
static CMDResult CMD_SupAllowed(Boolean Negate, char const* pArg) {
    UNUSED(pArg);
...
static CMDResult CMD_ExtendErrors(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_NumericErrors(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_HexLowerCase(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_SplitByte(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_QuietMode(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_CompMode(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_ThrowErrors(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_CaseSensitive(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_GNUErrors(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_IncludeList(Boolean Negate, char const* Arg) {
    char*  p;
    String Copy, part;
...
static CMDResult CMD_ListMask(Boolean Negate, char const* Arg) {
    Word    erg;
    Boolean OK;
...
static CMDResult CMD_DefSymbol(Boolean Negate, char const* Arg) {
...
static CMDResult CMD_ErrorPath(Boolean Negate, char const* Arg) {
    if (Negate) {
        return CMDErr;
    } else if (Arg[0] == '\0') {
        ErrorPath[0] = '\0';
...
static CMDResult CMD_HardRanges(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static CMDResult CMD_OutFile(Boolean Negate, char const* Arg) {
    if (Arg[0] == '\0') {
        if (Negate) {
            ClearOutList();
            return CMDOK;
...
static CMDResult CMD_ShareOutFile(Boolean Negate, char const* Arg) {
    if (Arg[0] == '\0') {
        if (Negate) {
            ClearShareOutList();
            return CMDOK;
...
static CMDResult CMD_ListOutFile(Boolean Negate, char const* Arg) {
    if (Arg[0] == '\0') {
        if (Negate) {
            ClearListOutList();
            return CMDOK;
...
static CMDResult CMD_CPUAlias(Boolean Negate, char const* Arg) {
    char const* p;
    String      s1, s2;
...
static CMDResult CMD_SetCPU(Boolean Negate, char const* Arg) {
    if (Negate) {
        *DefCPU = '\0';
        return CMDOK;
    } else {
...
static CMDResult CMD_NoICEMask(Boolean Negate, char const* Arg) {
    Word    erg;
    Boolean OK;
...
static CMDResult CMD_MaxErrors(Boolean Negate, char const* Arg) {
    if (Negate) {
        MaxErrors = 0;
        return CMDOK;
    } else if (Arg[0] == '\0') {
...
static CMDResult CMD_MaxIncludeLevel(Boolean Negate, char const* pArg) {
    if (Negate) {
...
static CMDResult CMD_TreatWarningsAsErrors(Boolean Negate, char const* Arg) {
    UNUSED(Arg);
...
static void WrHead(void) {
    if (!QuietMode) {
        ...
        NxtLine();
        WrConsoleLine(InfoNewCopyright, True);
        NxtLine();
