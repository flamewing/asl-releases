...
enum {
    ...
    eCore99105  = 1 << 5,
    eCore99110  = 1 << 6,
    eCoreAll = eCore990_10 | eCore990_12 | eCore9900 | eCore9940 | eCore9995 | eCore99105
               | eCore99110,
...
static tOrder *         SingOrders, *ImmOrders, *FixedOrders, *RegOrders, *Type11Orders,
        *Type11aOrders, *Type12Orders, *Type15Orders, *Type16Orders, *Type17Orders,
        *Type20Orders;
...
static Boolean DecodeReg(tStrComp const* pArg, Word* Erg) {
    Boolean OK;
...
static Boolean DecodeAdr(tStrComp const* pArg) {
    Boolean IncFlag;
    Boolean OK;
    char*   p;
...
static Boolean EvalDist(tStrComp const* pArg, Word* pResult, Word InstrLen) {
    Boolean      OK;
    Integer      AdrInt;
    tSymbolFlags Flags;

    AdrInt = EvalStrIntExpressionOffsWithFlags(
                     pArg, !!(*pArg->str.p_str == '@'), UInt16, &OK, &Flags)
             - (EProgCounter() + InstrLen);
...
static Boolean DecodeCond(tStrComp const* pArg, Word* pResult) {
    char const *pConds[]
...
static Boolean CheckNotMode3(tStrComp const* pArg) {
...
static void DecodeTwo(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1])) {
        WAsmCode[0] = AdrPart;
        WAsmCode[1] = AdrVal;
        HPart       = AdrCnt;
...
static void DecodeType11(Word Index) {
    tOrder const* pOrder = &Type11Orders[Index];
    Word          AdrCnt1;

    if (ChkArgCnt(2, (pCurrCPUProps->CoreFlags & eCore990_12) ? 3 : 2)
        && CheckCore(pOrder->Flags & eCoreAll) && DecodeAdr(&ArgStr[1])) {
        Word    Count;
        Boolean OK;
...
static void DecodeType11a(Word Index) {
    tOrder const* pOrder = &Type11aOrders[Index];
    Word          AdrCnt1;

    if (ChkArgCnt(2, 2) && CheckCore(pOrder->Flags & eCoreAll) && DecodeAdr(&ArgStr[1])) {
        WAsmCode[0] = pOrder->Code;
        WAsmCode[1] = AdrPart;
        WAsmCode[2] = AdrVal;
        AdrCnt1     = AdrCnt;
...
static void DecodeType12(Word Index) {
    tOrder const* pOrder = &Type12Orders[Index];
    Word          AdrCnt1, Count, Ckpt;
    Boolean       OK;

    if (!ChkArgCnt(2, 4) || !CheckCore(pOrder->Flags & eCoreAll)
        || !DecodeAdr(&ArgStr[1])) {
        return;
    }
    WAsmCode[0] = pOrder->Code;
...
static void DecodeType15(Word Index) {
    tOrder const* pOrder = &Type15Orders[Index];
    Word          BitField;
...
static void DecodeType16(Word Index) {
    tOrder const* pOrder = &Type16Orders[Index];
    Word          BitField;

    if (ChkArgCnt(3, 3) && CheckCore(pOrder->Flags & eCoreAll) && DecodeAdr(&ArgStr[1])
...
static void DecodeMOVA(Word Code) {
    if (ChkArgCnt(2, 2) && CheckCore(eCore990_12) && DecodeAdr(&ArgStr[1])) {
        Word AdrCnt1 = AdrCnt;
...
static void DecodeType20(Word Index) {
    tOrder const* pOrder = &Type20Orders[Index];
    Word          Condition;

    if (ChkArgCnt(3, 3) && CheckCore(pOrder->Flags & eCoreAll)
        && DecodeCond(&ArgStr[1], &Condition) && DecodeAdr(&ArgStr[2])) {
        Word AdrCnt1 = AdrCnt;
...
static void DecodeType17(Word Index) {
    tOrder const* pOrder = &Type17Orders[Index];
    Word          Reg;

    if (ChkArgCnt(2, 3) && DecodeReg(&ArgStr[ArgCnt], &Reg)) {
        Word    Delta, Dist;
        Boolean OK;
...
static void DecodeEP(Word Code) {
    if (ChkArgCnt(4, 4) && CheckCore(eCore990_12) && DecodeAdr(&ArgStr[1])) {
...
static void DecodeShift(Word Code) {
    if (ChkArgCnt(2, 2) && DecodeReg(&ArgStr[1], WAsmCode + 0)) {
        Word HPart;
...
static void DecodeSLAM_SRAM(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && CheckCore(eCore990_12 | eCore99105 | eCore99110)
        && DecodeReg(&ArgStr[2], &HPart) && DecodeAdr(&ArgStr[1])) {
        WAsmCode[0] = Code;
...
static void DecodeImm(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && CheckCore(pOrder->Flags & eCoreAll)
...
static void DecodeLIIM(Word Code) {
    if (ChkArgCnt(1, 1) && CheckCore(eCore9940)) {
        Boolean OK;
...
static void DecodeRegOrder(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && CheckCore(pOrder->Flags & eCoreAll)
        && DecodeReg(&ArgStr[1], WAsmCode + 0)) {
        WAsmCode[0] |= pOrder->Code;
        CodeLen = 2;
    }
...
static void DecodeLMF(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && CheckCore(eCore990_10 | eCore990_12 | eCore99105 | eCore99110)
        && DecodeReg(&ArgStr[1], WAsmCode + 0)) {
        Boolean OK;
...
static void DecodeMPYS_DIVS(Word Code) {
    if (ChkArgCnt(1, 1) && CheckCore(eCore990_12 | eCore9995 | eCore99105 | eCore99110)
        && DecodeAdr(&ArgStr[1])) {
        WAsmCode[0] = Code + AdrPart;
        WAsmCode[1] = AdrVal;
        CodeLen     = (1 + AdrCnt) << 1;
...
static void DecodeBit(Word Code) {
    if (ChkArgCnt(2, 2) && CheckCore(eCore990_12 | eCore99105 | eCore99110)
        && DecodeAdr(&ArgStr[1]) && CheckNotMode3(&ArgStr[1])) {
        Boolean OK;
...
static void DecodeJmp(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && EvalDist(&ArgStr[1], &Dist, 2)) {
...
static void DecodeSing(Word Index) {
    tOrder const* pOrder = &SingOrders[Index];

    if (ChkArgCnt(1, 1) && CheckCore(pOrder->Flags & eCoreAll) && DecodeAdr(&ArgStr[1])) {
...
static void DecodeFixed(Word Index) {
    ...
    if (ChkArgCnt(0, 0) && CheckCore(pOrder->Flags & eCoreAll)) {
        WAsmCode[0] = pOrder->Code;
        CodeLen     = 2;
        if (pOrder->Flags & eCoreFlagSupMode) {
            CheckSupMode();
...
static void DecodeCKPT(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && CheckCore(eCore990_12)) {
        Word NewDefCkpt;

        if (!as_strcasecmp(ArgStr[1].str.p_str, "NOTHING")) {
            DefCkpt = CKPT_NOTHING;
        } else if (DecodeReg(&ArgStr[1], &NewDefCkpt)) {
            DefCkpt = NewDefCkpt;
        }
    }
}
...
static void AddTwo(char const* NName16, char const* NName8, Word NCode) {
    AddInstTable(InstTable, NName16, (NCode << 13), DecodeTwo);
    AddInstTable(InstTable, NName8, (NCode << 13) + 0x1000, DecodeTwo);
}

static void AddOne(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeOne);
}

static void AddSing(char const* NName, Word NCode, Byte Flags) {
...
static void AddSBit(char const* NName, Word NCode) {
...
static void AddImm(char const* NName, Word NCode, Word Flags) {
...
static void AddReg(char const* NName, Word NCode, Word Flags) {
...
static void AddFixed(char const* NName, Word NCode, Word Flags) {
...
static void AddType11(char const* NName, Word NCode, Word Flags) {
...
static void AddType11a(char const* NName, Word NCode, Word Flags) {
...
static void AddType12(char const* NName, Word NCode, Word Flags) {
...
static void AddType15(char const* NName, Word NCode, Word Flags) {
...
static void AddType16(char const* NName, Word NCode, Word Flags) {
...
static void AddType17(char const* NName, Word NCode, Word Flags) {
...
static void AddType20(char const* NName, Word NCode, Word Flags) {
...
static void InitFields(void) {
    ...
    SingOrders = (tOrder*)malloc(sizeof(*SingOrders) * SingOrderCnt);
    InstrZ     = 0;
    ...
    ImmOrders = (tOrder*)malloc(sizeof(*ImmOrders) * ImmOrderCnt);
    InstrZ    = 0;
    ...
    Type11Orders = (tOrder*)malloc(sizeof(*Type11Orders) * Type11OrderCnt);
    InstrZ       = 0;
    ...
    Type11aOrders = (tOrder*)malloc(sizeof(*Type11aOrders) * Type11aOrderCnt);
    InstrZ        = 0;
    ...
    RegOrders = (tOrder*)malloc(sizeof(*RegOrders) * RegOrderCnt);
    InstrZ    = 0;
    ...
    FixedOrders = (tOrder*)malloc(sizeof(*FixedOrders) * FixedOrderCnt);
    InstrZ      = 0;
    ...
    Type12Orders = (tOrder*)malloc(sizeof(*Type12Orders) * Type12OrderCnt);
    InstrZ       = 0;
    ...
    Type15Orders = (tOrder*)malloc(sizeof(*Type15Orders) * Type15OrderCnt);
    InstrZ       = 0;
    AddType15("IOF", 0x0e00, eCore990_12);

    Type16Orders = (tOrder*)malloc(sizeof(*Type16Orders) * Type16OrderCnt);
    InstrZ       = 0;
    AddType16("INSF", 0x0c10, eCore990_12);
    AddType16("XV", 0x0c30, eCore990_12);
    AddType16("XF", 0x0c20, eCore990_12);

    Type17Orders = (tOrder*)malloc(sizeof(*Type17Orders) * Type17OrderCnt);
    InstrZ       = 0;
    ...
    Type20Orders = (tOrder*)malloc(sizeof(*Type20Orders) * Type20OrderCnt);
    InstrZ       = 0;
