...
#define Mask_AllShort                                                                \
    (MModReg | MModIReg | MModDisp16 | MModImm | MModAbs16 | MModAbs32 | MModPCRel16 \
     | MModPCRel32 | MModPop | MModPush | MModPCChain | MModAbsChain)
...
static PChainRec DecodeChain(tStrComp* pArg) {
    ...
    do {
        ...
        /* Register, mit Skalierungsfaktor ? */
                ...
                (RegEvalResult = DecodeReg(&SReg, Rec->Regs + Rec->RegCnt, False))
                != eIsNoReg) {
            if (RegEvalResult == eRegAbort) {
                ErrFlag = True;
            } else if (Rec->RegCnt >= 5) {
                SetError(ErrNum_InvAddrMode);
            } else {
                tSymbolFlags Flags;
...
static Boolean DecodeAdr(tStrComp const* pArg, int Index, Word Mask) {
    ...
    if (*pArg->str.p_str == '@') {
        ...
        if ((!as_strcasecmp(IArg.str.p_str, "-R15"))
            || (!as_strcasecmp(IArg.str.p_str, "-SP"))) {
        ...
        /* einfaches Register/PC mit Displacement ? */
                ...
                (!RootChain->Next) && (RootChain->RegCnt == 1)
                && (RootChain->Scales[0] == 0)) {
            ...
            if (RootChain->DSize == DispSizeNone) {
                if ((RootChain->DispAcc == 0) && (RootChain->Regs[0] < 16))
                    ...
                    RootChain->DSize
                            = (IsD16(RootChain->DispAcc)) ? DispSize16 : DispSize32;
                }
            }
            ...
            while ((RootChain) && (!Error)) {
                ...
                if ((RunChain->RegCnt != 0) || (RunChain->HasDisp)) {
                    ...
                    if (RunChain->RegCnt != 0) {
                        AdrVals[Index][LastChain] = (RunChain->Regs[0] == 16)
                                                            ? 0x0600
                                                            : RunChain->Regs[0] << 10;
                    ...
                    if (RunChain->HasDisp) {
                        ...
                        if (RunChain->DSize == DispSizeNone) {
                            ...
                            if (IsD4(RunChain->DispAcc)) {
                                DSize = ((RunChain->DispAcc & 3) == 0) ? DispSize4
                                                                       : DispSize16;
                            } else if (
                                    (RunChain->DispAcc >= -32 - MinReserve)
                                    && (RunChain->DispAcc <= 28 + MaxReserve)) {
                                DSize = DispSize4Eps;
                            } else if (IsD16(RunChain->DispAcc)) {
                                DSize = DispSize16;
                            } else if (
                                    (RunChain->DispAcc >= -32768 - MinReserve)
                            ...
                            if (ChkRange(RunChain->DispAcc, -32, 28)) {
                                if (RunChain->DispAcc & 3) {
                                    ...
                                    AdrVals[Index][LastChain]
                                            += (RunChain->DispAcc >> 2) & 0x000f;
                                    RunChain->HasDisp = False;
                                }
                            }
                            break;

                            /* Fall 2: passt nicht mehr in naechstkleineres Displacement,
                               aber wir koennen hier schon einen Teil ablegen, um im
                               naechsten Iterations- schritt ein kuerzeres Displacement zu
                               bekommen */
                        ...
                        case DispSize16:
                            if (ChkRange(RunChain->DispAcc, -32768, 32767)) {
                                ...
                                AdrVals[Index][LastChain + 1]
                                        = RunChain->DispAcc & 0xffff;
                                AdrCnt1[Index] += 2;
                                RunChain->HasDisp = False;
                            } else {
                                Error = True;
...
static Boolean DecodeRegList(tStrComp const* pArg, Word* pResult, Boolean Turn) {
    String   ArgStr;
    ...
    do {
        ...
        if (!p3) {
            ...
            *pResult |= RMask(r1, Turn);
            ...
            if (!DecodeReg(&From, &r1, True) || !DecodeReg(&To, &r2, True)) {
                return False;
            }
            if (r1 <= r2) {
                for (z = r1; z <= r2; z++) {
...
static Boolean DecodeCondition(char const* Asc, Word* Erg) {
    int z;
...
static Boolean DecodeStringCondition(char const* Asc, Word* pErg) {
    Boolean OK = TRUE;
...
static Boolean CheckFormat(char const* FSet) {
    char const* p;
...
static void DecodeACB_SCB(Word IsSCB) {
    if (ChkArgCnt(4, 4) && CheckFormat("GEQR")) {
            ...
            if (GetOpSize(&ArgStr[4], 4)) {
                if (GetOpSize(&ArgStr[1], 1)) {
                    if (GetOpSize(&ArgStr[3], 2)) {
                        ...
                        if ((OpSize[3] == -1) && (OpSize[2] == -1)) {
                            OpSize[3] = 2;
                        }
                        if ((OpSize[3] == -1) && (OpSize[2] != -1)) {
                            OpSize[3] = OpSize[2];
                        } else if ((OpSize[3] != -1) && (OpSize[2] == -1)) {
                            OpSize[2] = OpSize[3];
                        }
                        if (OpSize[1] == -1) {
                            OpSize[1] = OpSize[2];
                        }
                        if (OpSize[3] != OpSize[2]) {
                            ...
                            AdrLong = EvalStrIntExpressionWithFlags(
                                    &ArgStr[4], Int32, &OK, &Flags);
                            if (OK) {
                                if (DecodeAdr(&ArgStr[1], 1, Mask_Source)) {
                                    if (DecodeAdr(&ArgStr[3], 2, Mask_Source)) {
                                        ...
                                        switch (FormatCode) {
                                        case 1:
                                            if (DecideBranchLength(
                                                        &AdrLong, Flags, 4)) /* ??? */
                                            {
                                                WAsmCode[0] = 0xd000 + (OpSize[1] << 8)
                                                              + AdrMode[1];
                                                memcpy(WAsmCode + 1, AdrVals[1],
                                                       AdrCnt1[1]);
                                                WAsmCode[1 + AdrCnt2[1]]
                                                        = 0xf000 + (IsSCB << 11)
                                                          + (OpSize[2] << 8) + AdrMode[2];
                                                memcpy(WAsmCode + 2 + AdrCnt2[1],
                                                       AdrVals[2], AdrCnt1[2]);
                                                WAsmCode[2 + AdrCnt2[1] + AdrCnt2[2]]
                                                        = (HReg << 10) + (OpSize[4] << 8);
                                                ...
                                                if (AdrType[1] != ModImm) {
                                                    ...
                                                    if (ChkRange(HVal, -128, 127)) {
                                                        ...
                                                        WAsmCode[1] = 0xf000
                                                                      ...
                                                                      + (OpSize[2] << 8)
                                                                      + AdrMode[2];
                                                        memcpy(WAsmCode + 2, AdrVals[2],
                                                               AdrCnt1[2]);
                                                        WAsmCode[2 + AdrCnt2[2]]
                                                                = (HReg << 10)
                                                                  + (OpSize[4] << 8);
                                                        CodeLen = 6 + AdrCnt1[2];
                                                    }
                                        ...
                                        case 3:
                                                        ...
                                                        &AdrLong, Flags, 4)) /* ??? */
                                            {
                                                if (AdrType[1] != ModImm) {
                                                    ...
                                                    WrError(ErrNum_Only1);
                                                } else if (AdrType[2] != ModImm) {
                                                    WrError(ErrNum_InvAddrMode);
                                                } else {
                                                    HVal = ImmVal(2);
                                                    if (ChkRange(
...
static void DecodeMOV(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GELSZQI")) {
        if ((GetOpSize(&ArgStr[1], 1)) && (GetOpSize(&ArgStr[2], 2))) {
            ...
            if ((DecodeAdr(&ArgStr[1], 1, Mask_Source))
                ...
                if (FormatCode == 0) {
                    if (AdrType[1] == ModImm) {
                        ...
                        if (HVal == 0) {
                            ...
                            (AdrType[1] == ModReg) && (OpSize[1] == 2) && (IsShort(2))) {
                        FormatCode = 4;
                    } else if (
                            (AdrType[2] == ModReg) && (OpSize[2] == 2) && (IsShort(1))) {
                        FormatCode = 3;
                    } else {
                        FormatCode = 1;
                    }
                ...
                case 2:
                    ...
                    break;
                case 3:
                    if ((!IsShort(1)) || (AdrType[2] != ModReg)) {
                        WrError(ErrNum_InvAddrMode);
                    } else if (OpSize[2] != 2) {
                        WrError(ErrNum_InvOpSize);
                    } else {
                        WAsmCode[0] = 0x0040 + ((AdrMode[2] & 15) << 10)
                                      + (OpSize[1] << 8) + AdrMode[1];
                        memcpy(WAsmCode + 1, AdrVals[1], AdrCnt1[1]);
                        CodeLen = 2 + AdrCnt1[1];
                    }
                    break;
                case 4:
                    if ((!IsShort(2)) || (AdrType[1] != ModReg)) {
                        WrError(ErrNum_InvAddrMode);
                    } else if (OpSize[1] != 2) {
                        WrError(ErrNum_InvOpSize);
                    } else {
                        WAsmCode[0] = 0x0080 + ((AdrMode[1] & 15) << 10)
                                      + (OpSize[2] << 8) + AdrMode[2];
                        memcpy(WAsmCode + 1, AdrVals[2], AdrCnt1[2]);
                        CodeLen = 2 + AdrCnt1[2];
                    }
                ...
                case 6:
                    if ((AdrType[1] != ModImm) || (!IsShort(2))) {
                        ...
                        if (ChkRange(HVal, 1, 8)) {
                            WAsmCode[0] = 0x6000 + ((HVal & 7) << 10) + (OpSize[2] << 8)
                                          + AdrMode[2];
                            memcpy(WAsmCode + 1, AdrVals[2], AdrCnt1[2]);
                            CodeLen = 2 + AdrCnt1[2];
                        }
...
static void DecodeOne(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && CheckFormat("G")) {
        if (GetOpSize(&ArgStr[1], 1)) {
            if ((OpSize[1] == -1) && (pOrder->OpMask != 0)) {
                OpSize[1] = DefSize(pOrder->OpMask);
            }
            if ((OpSize[1] != -1) && (((1 << OpSize[1]) & pOrder->OpMask) == 0)) {
                WrError(ErrNum_InvOpSize);
            } else {
...
static void DecodeADD_SUB(Word IsSUB) {
    if (ChkArgCnt(2, 2) && CheckFormat("GELQI")) {
        if ((GetOpSize(&ArgStr[2], 2)) && (GetOpSize(&ArgStr[1], 1))) {
            ...
            if ((DecodeAdr(&ArgStr[1], 1, Mask_Source))
                ...
                if (FormatCode == 0) {
                    if (AdrType[1] == ModImm) {
                        ...
                        if (IsShort(2)) {
                            ...
                            IsShort(1) && (AdrType[2] == ModReg) && (OpSize[1] == 2)
                            && (OpSize[2] == 2)) {
                        FormatCode = 3;
                    } else {
                        FormatCode = 1;
                ...
                case 3:
                    if ((!IsShort(1)) || (AdrType[2] != ModReg)) {
                        WrError(ErrNum_InvAddrMode);
                    } else if ((OpSize[1] != 2) || (OpSize[2] != 2)) {
                        WrError(ErrNum_InvOpSize);
                    } else {
                        WAsmCode[0] = 0x8100 + (IsSUB << 6) + ((AdrMode[2] & 15) << 10)
                                      + AdrMode[1];
                        memcpy(WAsmCode + 1, AdrVals[1], AdrCnt1[1]);
                        CodeLen = 2 + AdrCnt1[1];
                ...
                case 4:
                    if ((AdrType[1] != ModImm) || (!IsShort(2))) {
                        ...
                        LongInt HVal = ImmVal(1);
                        if (ChkRange(HVal, 1, 8)) {
                            WAsmCode[0] = 0x4040 + (IsSUB << 13) + ((HVal & 7) << 10)
                                          + (OpSize[2] << 8) + AdrMode[2];
                            memcpy(WAsmCode + 1, AdrVals[2], AdrCnt1[2]);
                            CodeLen = 2 + AdrCnt1[2];
                        }
...
static void DecodeCMP(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GELZQI") && GetOpSize(&ArgStr[1], 1)
        ...
        if ((DecodeAdr(&ArgStr[1], 1, Mask_Source))
            ...
            case 3:
                if ((!IsShort(1)) || (AdrType[2] != ModReg)) {
                    ...
                    WAsmCode[0]
                            = ((AdrMode[2] & 15) << 10) + (OpSize[1] << 8) + AdrMode[1];
                    memcpy(WAsmCode + 1, AdrVals[1], AdrCnt1[1]);
                    CodeLen = 2 + AdrCnt1[1];
                }
                break;
            ...
            case 5:
                if ((AdrType[1] != ModImm) || (!IsShort(2))) {
                    ...
                    HVal = ImmVal(1);
                    if (ChkRange(HVal, 1, 8)) {
                        WAsmCode[0] = 0x4000 + (OpSize[2] << 8) + AdrMode[2]
                                      + ((HVal & 7) << 10);
                        memcpy(WAsmCode + 1, AdrVals[2], AdrCnt1[2]);
                        CodeLen = 2 + AdrCnt1[2];
                    }
...
static void DecodeGE2(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GE") && GetOpSize(&ArgStr[2], 2)
        ...
        if (((pOrder->SMask1 & (1 << OpSize[1])) == 0)
            || ((pOrder->SMask2 & (1 << OpSize[2])) == 0)) {
            WrError(ErrNum_InvOpSize);
        } else if (
                (DecodeAdr(&ArgStr[1], 1, pOrder->Mask1))
...
static void DecodeLog(Word Index) {
    if (ChkArgCnt(2, 2) && CheckFormat("GERI") && GetOpSize(&ArgStr[1], 1)
        && GetOpSize(&ArgStr[2], 2)) {
        if (OpSize[2] == -1) {
            OpSize[2] = 2;
        }
        ...
        if ((DecodeAdr(&ArgStr[1], 1, Mask_Source))
            ...
            if (FormatCode == 0) {
                if (AdrType[1] == ModImm) {
                    ...
                    if ((HVal >= 0) && (HVal <= 255)) {
                        ...
                        (AdrType[1] == ModReg) && (AdrType[2] == ModReg)
                        && (OpSize[1] == 2) && (OpSize[2] == 2)) {
                    FormatCode = 3;
                } else {
                    FormatCode = 1;
                }
            ...
            case 2:
                ...
                break;
            case 3:
                if ((AdrType[1] != ModReg) || (AdrType[2] != ModReg)) {
                    WrError(ErrNum_InvAddrMode);
                } else if ((OpSize[1] != 2) || (OpSize[2] != 2)) {
                    WrError(ErrNum_InvOpSize);
                } else {
...
static void DecodeMul(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat(Unsigned ? "GE" : "GER")
        && GetOpSize(&ArgStr[1], 1) && GetOpSize(&ArgStr[2], 2)) {
        if (OpSize[2] == -1) {
            OpSize[2] = 2;
        }
        if (OpSize[1] == -1) {
            ...
            if (FormatCode == 0) {
                if (AdrType[1] == ModImm) {
                    ...
                    if ((HVal >= -128 + (Unsigned << 7))
                        ...
                        (!Unsigned) && (AdrType[1] == ModReg) && (OpSize[1] == 2)
...
static void DecodeGetPut(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("G") && GetOpSize(&ArgStr[1], 1)
        && GetOpSize(&ArgStr[2], 2)) {
        if (OpSize[AdrWord] == -1) {
            OpSize[AdrWord] = Code & 3;
        }
        if (OpSize[3 - AdrWord] == -1) {
            OpSize[3 - AdrWord] = 2;
        }
        if ((OpSize[AdrWord] != (Code & 3)) || (OpSize[3 - AdrWord] != 2)) {
            WrError(ErrNum_InvOpSize);
        } else if (
                (DecodeAdr(&ArgStr[1], 1, Mask)) && (DecodeAdr(&ArgStr[2], 2, Mask2))) {
            Make_G(Code & 0xff00);
            WAsmCode[0] += 0x0400;
        }
    }
...
static void DecodeMOVA(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GR") && GetOpSize(&ArgStr[2], 2)) {
        if (OpSize[2] == -1) {
            OpSize[2] = 2;
        }
        OpSize[1] = 0;
        if (OpSize[2] != 2) {
            WrError(ErrNum_InvOpSize);
                ...
                (DecodeAdr(&ArgStr[1], 1, Mask_PureMem))
...
static void DecodeQINS_QDEL(Word IsQINS) {
    if (ChkArgCnt(2, 2) && CheckFormat("G") && (IsQINS || GetOpSize(&ArgStr[2], 2))) {
        if (OpSize[2] == -1) {
            OpSize[2] = 2;
        }
        OpSize[1] = 0;
...
static void DecodeSHL_SHA(Word IsSHA) {
    if (ChkArgCnt(2, 2) && CheckFormat("GEQ") && GetOpSize(&ArgStr[1], 1)
        && GetOpSize(&ArgStr[2], 2)) {
        if (OpSize[1] == -1) {
            OpSize[1] = 0;
        }
        ...
        if (OpSize[1] != 0) {
            WrError(ErrNum_InvOpSize);
                ...
                (DecodeAdr(&ArgStr[1], 1, Mask_Source))
                && (DecodeAdr(&ArgStr[2], 2, Mask_PureDest))) {
            if (FormatCode == 0) {
                if (AdrType[1] == ModImm) {
                    LongInt HVal = ImmVal(1);
                    if ((IsShort(2)) && (abs(HVal) >= 1) && (abs(HVal) <= 8)
                        && ((IsSHA == 0) || (HVal < 0))) {
                        FormatCode = 3;
                    } else if ((HVal >= -128) && (HVal <= 127)) {
                        FormatCode = 2;
                ...
                if ((AdrType[1] != ModImm) || (!IsShort(2))) {
                    ...
                    if (ChkRange(HVal, -8, (1 - IsSHA) << 3)) {
                        if (HVal == 0) {
                            ...
                            WAsmCode[0] = 0x4080 + (HVal << 10) + (IsSHA << 6)
                                          + (OpSize[2] << 8) + AdrMode[2];
                            memcpy(WAsmCode + 1, AdrVals[2], AdrCnt1[2]);
                            CodeLen = 2 + AdrCnt1[2];
                        }
...
static void DecodeSHXL_SHXR(Word Code) {
    if (ChkArgCnt(1, 1) && CheckFormat("G") && GetOpSize(&ArgStr[1], 1)) {
        if (OpSize[1] == -1) {
            OpSize[1] = 2;
        }
        if (OpSize[1] != 2) {
...
static void DecodeCHK(Word Code) {
    ...
    if (OptionCnt > 1) {
        WrError(ErrNum_WrongOptCnt);
    } else if (
            ChkArgCnt(3, 3) && CheckFormat("G") && GetOpSize(&ArgStr[1], 2)
            && GetOpSize(&ArgStr[2], 1) && GetOpSize(&ArgStr[3], 3)) {
        Boolean OptOK;
        Word    IsSigned;

        if ((OptionCnt == 0) || (!as_strcasecmp(Options[0], "N"))) {
            OptOK    = True;
            IsSigned = 0;
        ...
        if ((OpSize[1] != OpSize[2]) || (OpSize[2] != OpSize[3])) {
            WrError(ErrNum_ConfOpSizes);
        } else if (!OptOK) {
            WrXError(ErrNum_UndefCond, Options[0]);
        } else if (
                (DecodeAdr(&ArgStr[1], 2, Mask_MemGen - MModPop - MModPush))
...
static void DecodeCSI(Word Code) {
    ...
    if (ChkArgCnt(3, 3) && CheckFormat("G") && GetOpSize(&ArgStr[1], 3)
        && GetOpSize(&ArgStr[2], 1) && GetOpSize(&ArgStr[3], 2)) {
        if (OpSize[3] == -1) {
            OpSize[3] = 2;
        }
        if (OpSize[2] == -1) {
...
static void DecodeDIVX_MULX(Word Code) {
    if (ChkArgCnt(3, 3) && CheckFormat("G") && GetOpSize(&ArgStr[1], 1)
        && GetOpSize(&ArgStr[2], 2) && GetOpSize(&ArgStr[3], 3)) {
        if (OpSize[3] == -1) {
            OpSize[3] = 2;
        }
        if (OpSize[2] == -1) {
        ...
        if ((OpSize[1] != 2) || (OpSize[2] != 2) || (OpSize[3] != 2)) {
            WrError(ErrNum_InvOpSize);
        } else if (
                (DecodeAdr(&ArgStr[1], 1, Mask_Source))
                && (DecodeAdr(&ArgStr[2], 2, Mask_PureDest))
...
static void DecodeBit(Word Index) {
    BitOrder const* pOrder = BitOrders + Index;

    if (ChkArgCnt(2, 2) && CheckFormat(pOrder->Code2 ? "GEQ" : "GE")
        && GetOpSize(&ArgStr[1], 1) && GetOpSize(&ArgStr[2], 2)) {
        if (OpSize[1] == -1) {
            OpSize[1] = 2;
        }
        if ((DecodeAdr(&ArgStr[1], 1, Mask_Source))
            ...
            if (((AdrType[2] != ModReg) || (pOrder->MustByte)) && (OpSize[2] != 0)) {
                WrError(ErrNum_InvOpSize);
            } else {
                if (FormatCode == 0) {
                    if (AdrType[1] == ModImm) {
                        ...
                        if ((HVal >= 0) && (HVal <= 7) && (IsShort(2))
                            && (pOrder->Code2 != 0) && (OpSize[2] == 0)) {
                            FormatCode = 3;
                        } else if ((HVal >= -128) && (HVal < 127)) {
                            FormatCode = 2;
                        } else {
...
static void DecodeBField(Word Code) {
    if (ChkArgCnt(4, 4) && CheckBFieldFormat() && GetOpSize(&ArgStr[1], 1)
        && GetOpSize(&ArgStr[2], 2) && GetOpSize(&ArgStr[3], 3)
        ...
        if ((DecodeAdr(&ArgStr[1], 1, MModReg | MModImm))
            ...
            switch (FormatCode) {
            case 1:
                if ((AdrType[1] != ModReg) || (AdrType[3] != ModReg)) {
                    WrError(ErrNum_InvAddrMode);
                } else if ((OpSize[1] != 2) || (OpSize[3] != 2) || (OpSize[4] != 2)) {
                    WrError(ErrNum_InvOpSize);
                } else {
                    ...
                    WAsmCode[2 + AdrCnt2[2] + AdrCnt2[4]]
                            = ((AdrMode[3] & 15) << 10) + (AdrMode[1] & 15);
                    CodeLen = 6 + AdrCnt1[2] + AdrCnt1[4];
                }
                break;
            case 2:
                if ((AdrType[1] != ModImm) || (AdrType[3] != ModReg)) {
                    WrError(ErrNum_InvAddrMode);
                } else if ((OpSize[3] != 2) || (OpSize[4] != 2)) {
                    WrError(ErrNum_InvOpSize);
                } else {
                    WAsmCode[0] = 0xd000 + (OpSize[2] << 8) + AdrMode[2];
                    memcpy(WAsmCode + 1, AdrVals[2], AdrCnt1[2]);
                ...
                break;
            case 3:
                if ((AdrType[1] != ModReg) || (AdrType[2] != ModImm)
                    || (AdrType[3] != ModImm)) {
                    WrError(ErrNum_InvAddrMode);
                } else if ((OpSize[1] != 2) || (OpSize[4] != 2)) {
                    WrError(ErrNum_InvOpSize);
                } else {
                    ...
                    if (ChkRange(Offset, -128, 127)) {
                        ...
                        if (ChkRange(Width, 1, 32)) {
                            ...
                            WAsmCode[2 + AdrCnt2[4]]
                                    = ((Width & 31) << 10) + (AdrMode[1] & 15);
                            CodeLen = 6 + AdrCnt1[4];
                        }
                    }
                }
                break;
            case 4:
                if ((AdrType[1] != ModImm) || (AdrType[2] != ModImm)
                    || (AdrType[3] != ModImm)) {
                    WrError(ErrNum_InvAddrMode);
                } else if (OpSize[4] != 2) {
                    WrError(ErrNum_InvOpSize);
...
static void DecodeBFEXT_BFEXTU(Word IsEXTU) {
    if (ChkArgCnt(4, 4) && CheckFormat("GE") && GetOpSize(&ArgStr[1], 1)
        && GetOpSize(&ArgStr[2], 2) && GetOpSize(&ArgStr[3], 3)
        && GetOpSize(&ArgStr[4], 4)) {
        if (OpSize[1] == -1) {
            OpSize[1] = 2;
        ...
        if ((DecodeAdr(&ArgStr[4], 4, MModReg))
            ...
            if (DecodeAdr(
                        &ArgStr[1], 1, (AdrType[2] == ModReg) ? Mask_Source : MModImm)) {
                if (FormatCode == 0) {
                    FormatCode = (AdrType[2] == ModReg) ? 1 : 2;
                }
                switch (FormatCode) {
                case 1:
                    if ((OpSize[2] != 2) || (OpSize[3] != 2) || (OpSize[4] != 2)) {
                        WrError(ErrNum_InvOpSize);
                    } else {
                        WAsmCode[0] = 0xd000 + (OpSize[1] << 8) + AdrMode[1];
                        memcpy(WAsmCode + 1, AdrVals[1], AdrCnt1[1]);
                        ...
                        WAsmCode[2 + AdrCnt2[1] + AdrCnt2[3]]
                                = ((AdrMode[2] & 15) << 10) + (AdrMode[4] & 15);
                    ...
                    if ((AdrType[1] != ModImm) || (AdrType[2] != ModImm)) {
                        ...
                        if (ChkRange(Offset, -128, 127)) {
                            ...
                            if (ChkRange(Width, 1, 32)) {
                                ...
                                WAsmCode[2 + AdrCnt2[3]]
                                        = ((Width & 31) << 10) + (AdrMode[4] & 15);
                                CodeLen = 6 + AdrCnt1[3];
                            }
                        }
                    }
...
static void DecodeBSCH(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_WrongOptCnt);
    } else if ((strcmp(Options[0], "0")) && (strcmp(Options[0], "1"))) {
        WrXError(ErrNum_UndefCond, Options[0]);
    } else if (
            (CheckFormat("G")) && (GetOpSize(&ArgStr[1], 1))
            && (GetOpSize(&ArgStr[2], 2))) {
        if (OpSize[1] == -1) {
            OpSize[1] = 2;
        }
        ...
        if (OpSize[1] != 2) {
            ...
            memcpy(WAsmCode + 1, AdrVals[1], AdrCnt1[1]);
            WAsmCode[1 + AdrCnt2[1]] = Code + ((Options[0][0] - '0') << 10)
                                       + (OpSize[2] << 8) + AdrMode[2];
            memcpy(WAsmCode + 2 + AdrCnt2[1], AdrVals[2], AdrCnt1[2]);
            CodeLen = 4 + AdrCnt1[1] + AdrCnt1[2];
        }
...
static void DecodeBSR_BRA(Word IsBSR) {
    if (ChkArgCnt(1, 1) && CheckFormat("GD") && GetOpSize(&ArgStr[1], 1)) {
        Boolean      OK;
        tSymbolFlags Flags;
...
static void DecodeTRAP(Word Code) {
    ...
    if (!ChkArgCnt(0, 0))
        ...
        WrError(ErrNum_WrongOptCnt);
    } else if (!DecodeCondition(Options[0], &Condition)) {
        WrXError(ErrNum_UndefCond, Options[0]);
    } else {
        CodeLen     = 2;
...
static void DecodeENTER_EXITD(Word IsEXITD) {
    if (ChkArgCnt(2, 2)) {
        ...
        if ((CheckFormat("GE")) && (GetOpSize(pSizeArg, 1)) && (GetOpSize(pRegList, 2))) {
            Word RegList;
            ...
            if (OpSize[2] != 2) {
                WrError(ErrNum_InvOpSize);
                    ...
                    (DecodeAdr(pSizeArg, 1, MModReg | MModImm))
                    && (DecodeRegList(pRegList, &RegList, IsEXITD))) {
                if ((RegList & 0xc000) != 0) {
                    WrXError(ErrNum_InvRegList, "SP/FP");
                } else {
                    if (FormatCode == 0) {
                        if (AdrType[1] == ModImm) {
                            LongInt HVal = ImmVal(1);
                    ...
                    case 1:
                        WAsmCode[0] = 0x02f7;
                        WAsmCode[1] = 0x8c00 + (IsEXITD << 12) + (OpSize[1] << 8)
                                      + AdrMode[1];
...
static void DecodeSMOV_SSCH(Word Code) {
    ...
    if (ChkArgCnt(0, 0)) {
        ...
        if (!OK) {
            WrXError(ErrNum_UndefCond, Options[z]);
        } else {
...
static void DecodeLDM_STM(Word Code) {
    if (ChkArgCnt(2, 2) && CheckFormat("G")) {
        ...
        if ((GetOpSize(pRegList, 1)) && (GetOpSize(pMemArg, 2))) {
            if (OpSize[1] == -1) {
                OpSize[1] = 2;
            }
            if (OpSize[2] == -1) {
            ...
            if ((OpSize[1] != 2) || (OpSize[2] != 2)) {
                WrError(ErrNum_InvOpSize);
            } else if (
                    (DecodeAdr(pMemArg, 2, Mask))
...
static void DecodeJRNG(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && CheckFormat("GE") && GetOpSize(&ArgStr[1], 1)) {
        if (OpSize[1] == -1) {
            OpSize[1] = 1;
        }
        if (OpSize[1] != 1) {
...
static void AddFixedLong(char const* NName, Word NCode1, Word NCode2) {
    if (InstrZ >= FixedLongOrderCount) {
        exit(255);
    }
...
static void AddOne(char const* NName, Byte NOpMask, Word NMask, Word NCode) {
    if (InstrZ >= OneOrderCount) {
        exit(255);
    }
...
static void AddGE2(
        char const* NName, Word NMask1, Word NMask2, Byte NSMask1, Byte NSMask2,
        Word NCode, Boolean NSigned) {
    if (InstrZ >= GE2OrderCount) {
        exit(255);
    }
...
static void AddBit(char const* NName, Boolean NMust, Word NCode1, Word NCode2) {
    if (InstrZ >= BitOrderCount) {
        exit(255);
    }
...
static void AddGetPut(char const* NName, Byte NSize, Word NCode, Boolean NTurn) {
    AddInstTable(InstTable, NName, NCode | NSize | (NTurn << 7), DecodeGetPut);
}

static void Addcc(char const* BName) {
...
static void InitFields(void) {
    ...
    FixedLongOrders = (BitOrder*)malloc(sizeof(BitOrder) * FixedLongOrderCount);
    InstrZ          = 0;
    ...
    AddOne("LDCTX", 0x00,
           MModIReg | MModDisp16 | MModDisp32 | MModAbs16 | MModAbs32 | MModPCRel16
                   | MModPCRel32,
           0x8600);
    ...
    InstrZ     = 0;
    Conditions = (char const**)malloc(ConditionCount * sizeof(char*));
    Addcc("BXS");
    Addcc("BXC");
    Addcc("BEQ");
    Addcc("BNE");
