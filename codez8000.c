...
#define MModNoImm \
    (MModReg | MModIReg | MModDirect | MModIndexed | MModBaseAddress | MModBaseIndexed)
...
static Boolean DecodeRegCore(char const* pArg, Word* pValue, tSymbolSize* pSize) {
...
static void DissectReg_Z8000(
        char* pDest, size_t DestSize, tRegInt Value, tSymbolSize InpSize) {
    switch (InpSize) {
    case eSymbolSize8Bit:
        as_snprintf(
                pDest, DestSize, "R%c%u", "HL"[(Value >> 3) & 1], (unsigned)(Value & 7));
        break;
...
typedef Boolean (*tChkRegSize)(tStrComp const* pArg, tSymbolSize ActSize);

static Boolean ChkRegSize_Idx(tStrComp const* pArg, tSymbolSize ActSize) {
...
static Boolean ChkRegSize_8To32(tStrComp const* pArg, tSymbolSize ActSize) {
...
static Boolean ChkRegSize_IOAddr(tStrComp const* pArg, tSymbolSize ActSize) {
    if (ActSize != eSymbolSize16Bit) {
...
static Boolean ChkRegSize_MemAddr(tStrComp const* pArg, tSymbolSize ActSize) {
    if (Segmented()) {
        if (ActSize != eSymbolSize32Bit) {
...
static tRegEvalResult DecodeReg(
        tStrComp const* pArg, Word* pValue, tSymbolSize* pSize, tChkRegSize ChkRegSize,
        Boolean MustBeReg) {
    ...
    if ((RegEvalResult == eIsReg) && ChkRegSize
...
static LongWord DecodeAddrPartNum(
        tStrComp const* pArg, LongInt Disp, tEvalResult* pEvalResult, Boolean IsDirect,
        Boolean IsIO, Boolean* pForceShort, Boolean* pIsDirect) {
    tStrComp CopyComp;
    String   Copy;
    ...
    *pForceShort = False;
    ...
    if (!IsIO && Segmented()) {
        ...
        if ((Len >= 2) && (CopyComp.str.p_str[0] == '|')
            && (CopyComp.str.p_str[Len - 1] == '|')) {
        ...
        if (!strncmp(CopyComp.str.p_str, "<<", 2)) {
            ...
            if (pSplitPos) {
                ...
                SegNum = EvalStrIntExpressionOffsWithResult(
                        &SegArg, 2, UInt7, pEvalResult);
                if (!pEvalResult->OK) {
                    return 0;
                }
                HasSeg   = True;
    ...
    /* If we know it's direct, right away limit to the correct address range.  Otherwise,
       it might be immediate up to 32 bits: */
    ...
    if (IsDirect) {
        Result |= (SegNum << 16);
        if (pEvalResult->OK && !mFirstPassUnknownOrQuestionable(pEvalResult->Flags)
            && *pForceShort && (Result & 0x00ff00ul)) {
...
static void FillAbsAddr(
        tAdrVals* pAdrVals, LongWord Address, Boolean IsIO, Boolean ForceShort) {
...
static Boolean DecodeAddrPart(
        tStrComp const* pArg, LongInt Disp, tAdrVals* pAdrVals, Boolean IsDirect,
        Boolean IsIO, Boolean* pIsDirect) {
    LongWord    Address;
    tEvalResult EvalResult;
    Boolean     ForceLong;

    Address = DecodeAddrPartNum(
            pArg, Disp, &EvalResult, IsDirect, IsIO, &ForceLong, pIsDirect);
    if (EvalResult.OK) {
...
static int ShortQualifier(char const* pArg, int NextNonBlankPos, int SplitPos) {
    ...
    return ((FirstNonBlankPos < NextNonBlankPos) && (pArg[FirstNonBlankPos] == '|')
            && (pArg[NextNonBlankPos] == '|'))
                   ? SplitPos
                   : -1;
}

static tAdrMode DecodeAdr(tStrComp const* pArg, unsigned ModeMask, tAdrVals* pAdrVals) {
    tSymbolSize ArgSize;
    ...
    if (*pArg->str.p_str == '#') {
        ...
        case eSymbolSize8Bit:
            ...
            if (OK) {
                pAdrVals->Vals[pAdrVals->Cnt++]
                        = (Result & 0xff) | ((Result & 0xff) << 8);
            }
            break;
        case eSymbolSize16Bit:
    ...
    ChkRegSizeForIOIndir
            = (AMDSyntax && !(ModeMask & MModReg) && (ModeMask & MModIReg) && IsIO)
                      ? ChkRegSize_Addr
                      : NULL;
    ...
    if (*pArg->str.p_str == '@') {
        ...
        if (eIsReg
            == DecodeReg(&RegComp, &pAdrVals->Val, &ArgSize, ChkRegSize_Addr, True)) {
            if (!pAdrVals->Val) {
                WrStrErrorPos(ErrNum_InvAddrMode, &RegComp);
            } else {
                pAdrVals->Mode = eModIReg;
            }
        }
    ...
    if (AMDSyntax && ((ArgLen = strlen(pArg->str.p_str)) > 1)
    ...
    if (SplitPos > 0) {
        ...
        case eIsReg: /* [R]Rx(...) */
            if (!pAdrVals->Val) {
                WrStrErrorPos(ErrNum_InvAddrMode, &OutArg);
            } else if (*InArg.str.p_str == '#') {
                Boolean OK;

                pAdrVals->Vals[pAdrVals->Cnt++]
                        = EvalStrIntExpressionOffs(&InArg, 1, Int16, &OK);
                if (OK) {
                    pAdrVals->Mode = eModBaseAddress;
                }
            } else if (
                    DecodeReg(
                            &InArg, &pAdrVals->Vals[pAdrVals->Cnt], &ArgSize,
                            ChkRegSize_Idx, True)
                    == eIsReg) {
        ...
        case eIsNoReg: /* abs(...) */
        {
            ...
            case eIsNoReg: /* abs/imm(delta) -> direct/imm*/
            {
                ...
                if (OK
                    && DecodeAddrPart(
                            &OutArg, Disp, pAdrVals, !AMDSyntax || !(ModeMask & MModImm),
                            IsIO, &IsDirect)) {
    ...
    if (DecodeAddrPart(
                pArg, 0, pAdrVals, !AMDSyntax || !(ModeMask & MModImm), IsIO,
                &IsDirect)) {
    DirectOrImmediate:
        if (IsDirect) {
            pAdrVals->Val  = 0;
...
static LongWord AssembleBitSymbol(
        Byte BitPos, tSymbolSize OpSize, LongWord Address, Boolean ForceShort) {
    LongWord CodeOpSize = (OpSize == eSymbolSize16Bit) ? 1 : 0;
    int      AddrShift  = CodeOpSize + 3;

    return BitPos | ((Address & 0x7fffff) << AddrShift) | (CodeOpSize << 28)
           | ((!!ForceShort) << 29);
}
...
static Byte EvalBitPosition(tStrComp const* pBitArg, Boolean* pOK, tSymbolSize OpSize) {
    IntType Type;
    ...
    common:
        return EvalStrIntExpressionOffs(
                pBitArg, !!(*pBitArg->str.p_str == '#'), Type, pOK);
    }
}
...
static Boolean DecodeBitArg2(
        LongWord* pResult, tStrComp const* pMemArg, tStrComp const* pBitArg,
        tSymbolSize OpSize) {
    tEvalResult EvalResult;
    LongWord    Addr;
    Byte        BitPos;
...
static Boolean DissectBitSymbol(
        LongWord BitSymbol, LongWord* pAddress, Byte* pBitPos, tSymbolSize* pOpSize,
        Boolean* pForceShort) {
...
static void DissectBit_Z8000(char* pDest, size_t DestSize, LargeWord Inp) {
    ...
    as_snprintf(
            pDest, DestSize, "%s%.*u%s%s(%c).%u", ForceShort ? "|" : "", ListRadixBase,
            (unsigned)Address, GetIntConstIntelSuffix((unsigned)ListRadixBase),
            ForceShort ? "|" : "", Attribute, (unsigned)BitPos);
}
...
static void ExpandZ8000Bit(
    ...
    if (pInnermostNamedStruct) {
        ...
        tSymbolSize OpSize = (pStructElem->OpSize == eSymbolSizeUnknown)
                                     ? eSymbolSize8Bit
                                     : pStructElem->OpSize;
        ...
        PushLocHandle(-1);
        EnterIntSymbol(
                pVarName, AssembleBitSymbol(pStructElem->BitPos, OpSize, Address, False),
                SegBData, False);
        PopLocHandle();
        /* TODO: MakeUseList? */
    }
...
static void AppendAdrVals(tAdrVals const* pVals) {
...
static Boolean DecodeCondition(tStrComp const* pArg, Word* pCondition) {
    int z;
...
static Word DecodeImm1_16(tStrComp const* pArg, tEvalResult* pEvalResult) {
    Word Result;

    Result = EvalStrIntExpressionOffsWithResult(
            pArg, !!(*pArg->str.p_str == '#'), UInt5, pEvalResult);
    if (pEvalResult->OK) {
...
static void DecodeFixed(Word Index) {
    ...
    if (ChkArgCnt(0, 0) && CheckSup(pOrder->Privileged)) {
        CodeLen     = 2;
...
static void DecodeLD(Word Code) {
    ...
    if (!SetOpSizeFromCode(Code) || !ChkArgCnt(2, 2)) {
        return;
    }
    ...
    case eModReg: {
        ...
        case eModReg:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x9400 : (0xa000 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            break;
        case eModIReg:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x1400 : (0x2000 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            break;
        case eModDirect:
        case eModIndexed:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x5400 : (0x6000 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        case eModBaseAddress:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x3500 : (0x3000 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        case eModBaseIndexed:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x7500 : (0x7000 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        case eModImm:
    ...
    case eModIReg: {
        ...
        case eModReg:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x1d00 : (0x2e00 | (OpSize << 8)))
    ...
    case eModIndexed: {
        ...
        case eModReg:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x5d00 : (0x6e00 | (OpSize << 8)))
                    | (DestAdrVals.Val << 4) | SrcAdrVals.Val);
    ...
    case eModBaseAddress: {
        ...
        if (DecodeAdr(&ArgStr[2], MModReg, &SrcAdrVals) == eModReg) {
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x3700 : (0x3200 | (OpSize << 8)))
                    | (DestAdrVals.Val << 4) | SrcAdrVals.Val);
            AppendAdrVals(&DestAdrVals);
        }
        break;
    ...
    case eModBaseIndexed: {
        ...
        if (DecodeAdr(&ArgStr[2], MModReg, &SrcAdrVals) == eModReg) {
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x7700 : (0x7200 | (OpSize << 8)))
                    | (DestAdrVals.Val << 4) | SrcAdrVals.Val);
            AppendAdrVals(&DestAdrVals);
        }
        break;
...
static void DecodeLDA(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1], MModReg, &DestAdrVals)) {
        switch (DecodeAdr(
                &ArgStr[2], MModDirect | MModIndexed | MModBaseAddress | MModBaseIndexed,
                &SrcAdrVals)) {
        case eModDirect:
        case eModIndexed:
...
static void DecodeLDAR(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1], MModReg, &DestAdrVals)) {
        tEvalResult EvalResult;
        LongWord    Addr
                = EvalStrIntExpressionWithResult(&ArgStr[2], MemIntType, &EvalResult);
        if (EvalResult.OK) {
            Word Delta;

            if (!mFirstPassUnknownOrQuestionable(EvalResult.Flags)
                && (GetSegment(Addr) != GetSegment(EProgCounter()))) {
...
static void DecodeLDCTL(Word Code) {
    ...
    UNUSED(Code);
    if (SetOpSizeFromCode(Code) && ChkArgCnt(2, 2) && CheckSup(True)) {
        ...
        case eModReg: {
            ...
            if (DecodeAdr(&ArgStr[2], MModCtl, &SrcAdrVals)) {
                AppendCode(
                        (OpSize ? 0x7d00 : 0x8c00) | (DestAdrVals.Val << 4)
                        | SrcAdrVals.Val);
            }
            break;
        }
        case eModCtl: {
            ...
            if (DecodeAdr(&ArgStr[2], MModReg, &SrcAdrVals)) {
                AppendCode(
                        (OpSize ? 0x7d08 : 0x8c08) | (SrcAdrVals.Val << 4)
                        | DestAdrVals.Val);
            }
            break;
        }
...
static void DecodeLDR(Word Code) {
    ...
    if (SetOpSizeFromCode(Code) && ChkArgCnt(2, 2)) {
        switch (DecodeReg(
                &ArgStr[RegArgIndex], &Reg, &RegSize, ChkRegSize_8To32, False)) {
        case eIsNoReg:
            RegArgIndex = 2;
            if (DecodeReg(&ArgStr[RegArgIndex], &Reg, &RegSize, ChkRegSize_8To32, True)
                != eIsReg) {
                return;
            }
            Code |= 2;
        ...
        case eIsReg: {
            ...
            Addr = EvalStrIntExpressionWithResult(
                    &ArgStr[3 - RegArgIndex], MemIntType, &EvalResult);
            if (EvalResult.OK) {
                Word Delta;
                ...
                Delta = (Addr & 0xffff) - ((EProgCounter() + 4) & 0xffff);
                AppendCode(
                        ((Lo(Code) | ((OpSize == eSymbolSize32Bit) ? 0x5 : OpSize)) << 8)
                        | Reg);
                AppendCode(Delta);
            }
            break;
...
static void DecodeADD(Word Code) {
    ...
    if (DecodeAdr(&ArgStr[1], MModReg, &DestAdrVals) == eModReg) {
        ...
        switch (DecodeAdr(
                &ArgStr[2], MModReg | MModImm | MModIReg | MModDirect | MModIndexed,
                &SrcAdrVals)) {
        case eModReg:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x9600 : (0x8000 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            break;
        case eModIReg:
        case eModImm:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x1600 : (0x0000 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        case eModDirect:
        case eModIndexed:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x5600 : (0x4000 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        default:
...
static void DecodeSUB(Word Code) {
    ...
    if (DecodeAdr(&ArgStr[1], MModReg, &DestAdrVals) == eModReg) {
        ...
        case eModReg:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x9200 : (0x8200 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            break;
        case eModIReg:
        case eModImm:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x1200 : (0x0200 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        case eModDirect:
        case eModIndexed:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x5200 : (0x4200 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        default:
...
static void DecodeCP(Word Code) {
    ...
    switch (DecodeAdr(
            &ArgStr[1], MModReg | MModIReg | MModDirect | MModIndexed, &DestAdrVals)) {
    case eModReg: {
        tAdrVals SrcAdrVals;
        ...
        case eModReg:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x9000 : (0x8a00 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            break;
        case eModIReg:
        case eModImm:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x1000 : (0x0a00 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        case eModDirect:
        case eModIndexed:
            AppendCode(
                    ((OpSize == eSymbolSize32Bit) ? 0x5000 : (0x4a00 | (OpSize << 8)))
                    | (SrcAdrVals.Val << 4) | DestAdrVals.Val);
            AppendAdrVals(&SrcAdrVals);
            break;
        default:
...
static void DecodeADC_SBC(Word Code) {
    ...
    if ((DecodeAdr(&ArgStr[1], MModReg, &DestAdrVals) == eModReg)
        ...
        if (OpSize > eSymbolSize16Bit) {
            ...
            AppendCode(
                    0x8000 | (((Code & 0xfe) | OpSize) << 8) | (SrcAdrVals.Val << 4)
                    | DestAdrVals.Val);
        }
    }
}
...
static void DecodeAND_OR_XOR(Word Code) {
    ...
    if (DecodeAdr(&ArgStr[1], MModReg, &DestAdrVals) == eModReg) {
        ...
        if (OpSize > eSymbolSize16Bit) {
            ...
            case eModImm:
                AppendCode(
                        0x0000 | (((Code & 0xfe) | OpSize) << 8) | (SrcAdrVals.Val << 4)
                        | DestAdrVals.Val);
                AppendAdrVals(&SrcAdrVals);
                break;
            case eModDirect:
            case eModIndexed:
                AppendCode(
                        0x4000 | (((Code & 0xfe) | OpSize) << 8) | (SrcAdrVals.Val << 4)
                        | DestAdrVals.Val);
                AppendAdrVals(&SrcAdrVals);
                break;
            default:
...
static void DecodeINC_DEC(Word Code) {
    ...
    if (SetOpSizeFromCode(Code) && ChkArgCnt(1, 2)
        && DecodeAdr(
                &ArgStr[1], MModReg | MModIReg | MModDirect | MModIndexed, &AdrVals)) {
        if (OpSize == eSymbolSizeUnknown) {
        ...
        if (OpSize > eSymbolSize16Bit) {
            ...
            if (EvalResult.OK) {
                ...
                case eModReg:
                    AppendCode(
                            0x8000 | ((Code | OpSize) << 8) | (AdrVals.Val << 4)
                            | ImmVal);
                    break;
                case eModIReg:
                    AppendCode(
                            0x0000 | ((Code | OpSize) << 8) | (AdrVals.Val << 4)
                            | ImmVal);
                    break;
                case eModDirect:
                case eModIndexed:
                    AppendCode(
                            0x4000 | ((Code | OpSize) << 8) | (AdrVals.Val << 4)
                            | ImmVal);
...
static void DecodeBitOp(Word Code) {
    ...
    case 2:
        if (DecodeAdr(
                    &ArgStr[1], MModReg | MModIReg | MModDirect | MModIndexed,
                    &DestAdrVals)) {
            DestOpSize = (OpSize == eSymbolSizeUnknown) ? eSymbolSize16Bit : OpSize;

            switch (DestOpSize) {
            case eSymbolSize8Bit:
                ImmOpSize = eSymbolSize3Bit;
                break;
            case eSymbolSize16Bit:
                ImmOpSize = eSymbolSize4Bit;
                break;
            default:
                WrStrErrorPos(ErrNum_InvOpSize, &ArgStr[1]);
                return;
            ...
            case eModImm:
                ...
                case eModReg:
                    AppendCode(
                            0x8000 | (((Code & 0xfe) | DestOpSize) << 8)
                            | (DestAdrVals.Val << 4) | SrcAdrVals.Val);
                    break;
                case eModIReg:
                    AppendCode(
                            0x0000 | (((Code & 0xfe) | DestOpSize) << 8)
                            | (DestAdrVals.Val << 4) | SrcAdrVals.Val);
                    break;
                case eModDirect:
                case eModIndexed:
...
static void DecodeCALL_JP(Word Code) {
    ...
    if (ChkArgCnt(1, 2 - IsCALL)) {
        ...
        switch (DecodeAdr(
                &ArgStr[ArgCnt], MModIReg | MModDirect | MModIndexed, &AdrVals)) {
        case eModIReg:
...
static void DecodeBranch(Word Code, Word MaxDist, Boolean Inverse, tStrComp const* pArg) {
    tEvalResult EvalResult;
    ...
    if (EvalResult.OK) {
        ...
        if ((Delta & 1) && !mFirstPassUnknownOrQuestionable(EvalResult.Flags)) {
            WrStrErrorPos(ErrNum_AddrMustBeEven, pArg);
        } else if (
                (Delta > MaxDist) && (Delta < (0xfffe - MaxDist))
...
static void DecodeJR(Word Code) {
    ...
    if (ChkArgCnt(1, 2) && DecodeCondition(ArgCnt == 2 ? &ArgStr[1] : NULL, &Condition)) {
    ...
    if (SetOpSizeFromCode(Code) && ChkArgCnt(2, 2)
        ...
        if (OpSize > eSymbolSize16Bit) {
            ...
            if (EvalResult.OK) {
                ...
                Delta = (Addr & 0xffff) - ((EProgCounter() + 2) & 0xffff);
                if ((Delta & 1) && !mFirstPassUnknownOrQuestionable(EvalResult.Flags)) {
                    WrStrErrorPos(ErrNum_AddrMustBeEven, &ArgStr[2]);
                } else if (
                        (Delta > 0) && (Delta < 0xff02)
                        && !mFirstPassUnknownOrQuestionable(EvalResult.Flags)) {
                    WrStrErrorPos(ErrNum_JmpDistTooBig, &ArgStr[2]);
                } else {
                    Delta = (0x10000lu - Delta) >> 1;
                    AppendCode(
                            0xf000 | (AdrVals.Val << 8) | (OpSize << 7) | (Delta & 0x7f));
                }
            }
        }
    }
...
static void DecodeCLR_COM_NEG_TSET(Word Code) {
    ...
    if (SetOpSizeFromCode(Code) && ChkArgCnt(1, 1)
        && DecodeAdr(
                &ArgStr[1], MModReg | MModIReg | MModDirect | MModIndexed, &AdrVals)) {
        if (OpSize == eSymbolSizeUnknown) {
            OpSize = eSymbolSize16Bit;
...
static void DecodeTEST(Word Code) {
    ...
    if (SetOpSizeFromCode(Code) && ChkArgCnt(1, 1)
        ...
        if (OpSize > eSymbolSize32Bit) {
            ...
            Code = (Lo(Code) | ((OpSize == eSymbolSize32Bit) ? 0x10 : OpSize)) << 8
                   | ((OpSize == eSymbolSize32Bit) ? 8 : 4);
            switch (AdrVals.Mode) {
            case eModReg:
                AppendCode(0x8000 | Code | (AdrVals.Val << 4));
                break;
...
static void DecodeEX(Word Code) {
    ...
    if (SetOpSizeFromCode(Code) && ChkArgCnt(2, 2)
        ...
        && DecodeAdr(
                &ArgStr[2],
                MModReg
                        | ((DestAdrVals.Mode == eModReg)
                                   ? (MModIReg | MModDirect | MModIndexed)
                                   : 0),
                &SrcAdrVals)) {
        if (OpSize == eSymbolSizeUnknown) {
            OpSize = eSymbolSize16Bit;
...
static void DecodeMULT_DIV(Word Code) {
    if (SetOpSizeFromCode(Code) && ChkArgCnt(2, 2)) {
        ...
        if (DecodeAdr(&ArgStr[1], MModReg, &DestAdrVals) == eModReg) {
            ...
            case eModReg:
                AppendCode(
                        0x8000 | ((Code | (OpSize & 1)) << 8) | (SrcAdrVals.Val << 4)
                        | DestAdrVals.Val);
                break;
            case eModImm:
            case eModIReg:
                AppendCode(
                        0x0000 | ((Code | (OpSize & 1)) << 8) | (SrcAdrVals.Val << 4)
                        | DestAdrVals.Val);
                AppendAdrVals(&SrcAdrVals);
                break;
            case eModDirect:
            case eModIndexed:
                AppendCode(
                        0x4000 | ((Code | (OpSize & 1)) << 8) | (SrcAdrVals.Val << 4)
                        | DestAdrVals.Val);
                AppendAdrVals(&SrcAdrVals);
                break;
            default:
...
static void DecodeFLG(Word Code) {
    if (ChkArgCnt(1, ArgCntMax)) {
        int               z;
        Word              Num;
        static char const FlagNames[] = "PSZCV";
        ...
        for (z = 1; z <= ArgCnt; z++) {
            if (!as_strcasecmp(ArgStr[z].str.p_str, "P/V")) {
                ...
                char const* pPos = strchr(FlagNames, as_toupper(*ArgStr[z].str.p_str));
...
static void DecodeDI_EI(Word Code) {
    if (ChkArgCnt(1, ArgCntMax) && CheckSup(True)) {
        int  z;
        Word Num;
...
static void DecodeIN_SIN_OUT_SOUT(Word Code) {
    tAdrVals        RegAdrVals;
    Word            IsOUT = Code & 2, IsSpecial = Code & 1;
    tStrComp const *pRegArg = IsOUT ? &ArgStr[2] : &ArgStr[1],
    ...
    if (SetOpSizeFromCode(Code) && ChkArgCnt(2, 2) && CheckSup(True)
        ...
        switch (DecodeAdr(
                pIOArg, MModIO | MModDirect | (IsSpecial ? 0 : MModIReg), &IOAdrVals)) {
        case eModIReg:
            AppendCode(
                    0x3c00 | (IsOUT << 8) | (OpSize << 8) | (IOAdrVals.Val << 4)
                    | RegAdrVals.Val);
            break;
        case eModDirect:
...
static void DecodeCPRep(Word Code) {
    ...
    if (!SetOpSizeFromCode(Code) || !ChkArgCnt(3, 4)
    ...
    if ((DecodeAdr(&ArgStr[1], 1 << DestMode, &DestAdrVals) == DestMode)
        ...
        if (DecodeAdr(&ArgStr[3], MModReg, &CntAdrVals) == eModReg) {
            if (SaveOpSize == eSymbolSize32Bit) {
                ...
                int Index = ChkRegOverlap(
                        (unsigned)DestAdrVals.Val,
                        ((DestMode == eModIReg) ? AddrRegSize() : SaveOpSize),
                        (unsigned)SrcAdrVals.Val, (int)AddrRegSize(),
                        (unsigned)CntAdrVals.Val, (int)eSymbolSize16Bit, 0,
                        eSymbolSizeUnknown);
...
static void DecodeINOUTRep(Word Code) {
    ...
    if (!SetOpSizeFromCode(Code) || !CheckSup(True) || !ChkArgCnt(3, 3)) {
        return;
    }
    ...
    if (DecodeAdr(&ArgStr[1], MModIReg | (IsOUT ? MModIO : 0), &DestAdrVals)
        ...
        tAdrVals    CntAdrVals;
        tSymbolSize SaveOpSize
                = (OpSize == eSymbolSizeUnknown) ? eSymbolSize16Bit : OpSize;

        OpSize = eSymbolSize16Bit;
        if (DecodeAdr(&ArgStr[3], MModReg, &CntAdrVals)) {
            AppendCode(
                    0x3a00 | (SaveOpSize << 8) | (SrcAdrVals.Val << 4)
                    | ((Code >> 4) & 15));
...
static void DecodeLDRep(Word Code) {
    ...
    if (!SetOpSizeFromCode(Code) || !ChkArgCnt(3, 3)) {
        return;
    }
    ...
    if (DecodeAdr(&ArgStr[1], MModIReg, &DestAdrVals)
        ...
        if (DecodeAdr(&ArgStr[3], MModReg, &CntAdrVals)) {
            AppendCode(
                    0xba00 | (SaveOpSize << 8) | (SrcAdrVals.Val << 4)
                    | ((Code >> 4) & 15));
            AppendCode((CntAdrVals.Val << 8) | (DestAdrVals.Val << 4) | (Code & 15));
        }
    }
...
static void DecodeTRRep(Word Code) {
    ...
    OpSize = eSymbolSize16Bit;
    if (ChkArgCnt(3, 3) && DecodeAdr(&ArgStr[1], MModIReg, &DestAdrVals)
        && DecodeAdr(&ArgStr[2], MModIReg, &SrcAdrVals)
        ...
        Index = ChkRegOverlap(
                (unsigned)DestAdrVals.Val, AddrRegSize(), (unsigned)SrcAdrVals.Val,
                (int)AddrRegSize(), (unsigned)CntAdrVals.Val, (int)eSymbolSize16Bit, 0,
                eSymbolSizeUnknown);
        if (Index >= 0) {
            WrStrErrorPos(ErrNum_OverlapReg, &ArgStr[Index + 1]);
        }
...
static void DecodeLDK(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1], MModReg, &DestAdrVals)) {
        ...
        Value = EvalStrIntExpressionOffs(
                &ArgStr[2], !!(*ArgStr[2].str.p_str == '#'), UInt4, &OK);
        if (OK) {
...
static void DecodeLDM(Word Code) {
    ...
    switch (DecodeAdr(
            &ArgStr[1], MModReg | MModIReg | MModDirect | MModIndexed, &RegAdrVals)) {
    case eModReg:
        RegArgIndex = 1;
    ...
    common:
        if (OpSize != eSymbolSize16Bit) {
            ...
            if (RegAdrVals.Val + Count >= 16) {
                WrStrErrorPos(ErrNum_RegNumWraparound, &ArgStr[3]);
            }
...
static void DecodeLDPS(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && CheckSup(True)) {
...
static void DecodeMREQ(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && CheckSup(True) && DecodeAdr(&ArgStr[1], MModReg, &AdrVals)) {
        AppendCode(Code | (AdrVals.Val << 4));
    }
}
...
static void DecodePUSH_POP(Word Code) {
    ...
    if (SetOpSizeFromCode(Code) && ChkArgCnt(2, 2)
        ...
        if (DecodeAdr(
                    &ArgStr[OpArg],
                    MModReg | MModIReg | MModDirect | MModIndexed | (IsPOP ? 0 : MModImm),
                    &OpAdrVals)) {
            int Index;
            ...
            case eModReg:
                Index = ChkRegOverlap(
                        (unsigned)OpAdrVals.Val, OpSize, (unsigned)RegAdrVals.Val,
                        (int)AddrRegSize(), 0, eSymbolSizeUnknown);
                break;
            case eModIReg:
            case eModIndexed:
                Index = ChkRegOverlap(
                        (unsigned)OpAdrVals.Val, AddrRegSize(), (unsigned)RegAdrVals.Val,
                        (int)AddrRegSize(), 0, eSymbolSizeUnknown);
                break;
            default:
                Index = -1;
...
static void DecodeRotate(Word Code) {
    ...
    if (ChkArgCnt(1, 2) && SetOpSizeFromCode(Code)
        ...
        if (ArgCnt >= 2) {
            Count = EvalStrIntExpressionOffsWithResult(
                    &ArgStr[2], !!(*ArgStr[2].str.p_str == '#'), UInt2, &EvalResult);
        ...
        if (ChkRange(Count, 1, 2)) {
            AppendCode(
                    0xb200 | (OpSize << 8) | (RegAdrVals.Val << 4) | Lo(Code)
                    | ((Count - 1) << 1));
        }
    }
}
...
static void DecodeRotateDigit(Word Code) {
    ...
    SetOpSize(eSymbolSize8Bit, &OpPart);
    if (ChkArgCnt(2, 2) && DecodeAdr(&ArgStr[1], MModReg, &LinkAdrVals)
...
static void DecodeSC(Word Code) {
    if (ChkArgCnt(1, 1)) {
        Boolean OK;
        Word    Arg = EvalStrIntExpressionOffs(
...
static void DecodeSD(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && SetOpSizeFromCode(Code)
        ...
        if (DecodeAdr(&ArgStr[2], MModReg, &SrcAdrVals)) {
            AppendCode(
                    0xb200 | (((SaveOpSize == eSymbolSize32Bit) ? 1 : SaveOpSize) << 8)
                    | (DestAdrVals.Val << 4) | Lo(Code)
                    | (SaveOpSize == eSymbolSize32Bit ? 4 : 0));
...
static void DecodeShift(Word Code) {
    ...
    if (ChkArgCnt(1, 2) && SetOpSizeFromCode(Code)
        ...
        if (ArgCnt >= 2) {
            ...
            Count = EvalStrIntExpressionOffsWithResult(
                    &ArgStr[2], !!(*ArgStr[2].str.p_str == '#'),
                    (OpSize == eSymbolSize32Bit)   ? UInt6
                    : (OpSize == eSymbolSize16Bit) ? UInt5
                                                   : UInt4,
                    &EvalResult);
            if (mFirstPassUnknownOrQuestionable(EvalResult.Flags)) {
                Count = 1;
        ...
        AppendCode(
                0xb200 | (((OpSize == eSymbolSize32Bit) ? 1 : OpSize) << 8)
                | (DestAdrVals.Val << 4) | Lo(Code)
                | (OpSize == eSymbolSize32Bit ? 4 : 0));
        AppendCode(Count);
    }
}
...
static void AddFixed(char const* NName, Word Code, Boolean Privileged) {
...
static void AddCtl(char const* pName, Word Code, tCtlFlags Flags, tSymbolSize Size) {
    if (InstrZ >= CtlRegCnt) {
        exit(255);
    }
...
static void AddCondition(char const* pName, Word Code) {
...
static void InitFields(void) {
    ...
    AddSizeInstTable(
            "LDR", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x30, DecodeLDR);
    AddSizeInstTable("LDCTL", 1 << eSymbolSize8Bit, 0x00, DecodeLDCTL);
    AddSizeInstTable(
            "ADD", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0, DecodeADD);
    AddSizeInstTable(
            "SUB", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0, DecodeSUB);
    ...
    AddSizeInstTable(
            "TEST", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x0c, DecodeTEST);
    AddSizeInstTable("EX", 1 << eSymbolSize8Bit, 0x2c, DecodeEX);
    AddSizeInstTable(
            "EXTS", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x31, DecodeEXTS);
    ...
    AddSizeInstTable(
            "SDA", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x0b, DecodeSD);
    AddSizeInstTable(
            "SDL", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x03, DecodeSD);
    AddSizeInstTable(
            "SLA", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x09, DecodeShift);
    AddSizeInstTable(
            "SLL", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x01, DecodeShift);
    AddSizeInstTable(
            "SRA", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x89, DecodeShift);
    AddSizeInstTable(
            "SRL", (1 << eSymbolSize8Bit) | (1 << eSymbolSize32Bit), 0x81, DecodeShift);
...
static Boolean IsDef_Z8000(void) {
    return Memo("REG") || Memo("PORT") || Memo("DEFBIT") || Memo("DEFBITB");
}
...
static Operator const PotMonadicOperator = {
        "^", 1, False, 8, {TempInt | (TempInt << 4), 0, 0, 0},
            PotOp
};
...
void codez8000_init(void) {
    ...
    static tCPUArg const Z8000Args[] = {
    ...
    for (pRun = CPUProps; pRun->pName; pRun++) {
        (void)AddCPUUserWithArgs(
                pRun->pName, SwitchTo_Z8000, (void*)pRun, NULL, Z8000Args);
    }
}
