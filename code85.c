...
static Byte const AccReg = 7;

static Boolean DecodeReg8(char const* Asc, tZ80Syntax Syntax, Byte* Erg) {
    static char const RegNames[] = "BCDEHLMA";
    char const*       p;
...
static Byte const BCReg = 0;
static Byte const DEReg = 1;
static Byte const HLReg = 2;
static Byte const SPReg = 3;

static Boolean DecodeReg16(char* pAsc, tZ80Syntax Syntax, Byte* pResult) {
    static char const RegNames[8][3] = {"B", "D", "H", "SP", "BC", "DE", "HL", "SP"};

    for (*pResult = (Syntax & eSyntax808x) ? 0 : 4;
         *pResult < ((Syntax & eSyntaxZ80) ? 8 : 4); (*pResult)++) {
...
static char const Conditions[][4]
        = {"NZ", "Z", "NC", "C", "PO", "PE", "P", "M", "NX5", "X5"};

static Boolean DecodeCondition(char const* pAsc, Byte* pResult, Boolean WithX5) {
...
static void DecodeFixed(Word Code) {
    if (ChkArgCnt(0, 0) && ChkZ80Syntax((tZ80Syntax)Hi(Code))) {
        CodeLen     = 1;
        BAsmCode[0] = Lo(Code);
    }
}
...
static void DecodeOp16(Word Code) {
    if (ChkArgCnt(1, 1) && ChkZ80Syntax((tZ80Syntax)Hi(Code))) {
...
static void DecodeMVI(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkZ80Syntax(eSyntax808x)) {
...
static void DecodeRST(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        AdrByte = EvalStrIntExpressionWithFlags(
                &ArgStr[1], (CurrZ80Syntax & eSyntaxZ80) ? UInt6 : UInt3, &OK, &Flags);
        if (mFirstPassUnknown(Flags)) {
            AdrByte = 0;
        }
        if (OK) {
            tZ80Syntax Syntax = (CurrZ80Syntax == eSyntaxBoth)
                                        ? ((AdrByte < 8) ? eSyntax808x : eSyntaxZ80)
                                        : CurrZ80Syntax;
...
static void DecodeDSUB(Word Index) {
    ...
    if (ChkArgCnt(0, 1) && ChkMinCPU(CPU8085U) && ChkZ80Syntax(eSyntax808x)) {
        ...
        if ((ArgCnt == 1)
            && (!DecodeReg16(ArgStr[1].str.p_str, CurrZ80Syntax, &Reg)
                || (Reg != BCReg))) {
            WrStrErrorPos(ErrNum_InvRegName, &ArgStr[1]);
        } else {
            CodeLen     = 1;
...
static void DecodeLD(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkZ80Syntax(eSyntaxZ80)) {
...
static void DecodeADD(Word Code) {
    ...
    case 3: /* Z80 style of undoc 8085 : DE <- (HL,SP) + m */
    {
        ...
        if (!DecodeReg16(ArgStr[1].str.p_str, CurrZ80Syntax, &Reg) || (Reg != DEReg)) {
            WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
        } else if (!DecodeReg16(ArgStr[2].str.p_str, CurrZ80Syntax, &Reg) || (Reg < 2)) {
...
static void DecodeADC(Word Code) {
    ...
    if (!ChkArgCnt(
                (CurrZ80Syntax & eSyntax808x) ? 1 : 2,
                (CurrZ80Syntax & eSyntaxZ80) ? 2 : 1)) {
        return;
    }
...
static void DecodeSUB(Word Code) {
    ...
    /* dest operand is always A and also optional for Z80 mode, so min arg cnt is always
     * 1! */
    ...
    if (DecodeReg8(
                ArgStr[ArgCnt].str.p_str, CurrZ80Syntax,
                &Reg)) /* 808x style incl. M, Z80 style excl. (HL) */
    {
    ...
    DecodeAdr_Z80(
            &ArgStr[ArgCnt],
            MModImm | MModIReg16 | ((AdrMode == ModReg16) ? MModReg16 : 0));
    switch (AdrMode) {
    case ModReg16:
        if (AdrVals[0] != BCReg) {
            WrError(ErrNum_InvAddrMode);
...
static void DecodeALU8_Z80(Word Code) {
    if (!ChkZ80Syntax(eSyntaxZ80) || !ChkArgCnt(1, 2)) {
        return;
    }
...
static void DecodeINCDEC(Word Code) {
    if (ChkZ80Syntax(eSyntaxZ80) && ChkArgCnt(1, 1)) {
...
static void DecodeCP(Word Code) {
    ...
    /* 1 argument -> must be compare anyway in pure Z80 syntax mode, otherwise assume 808x
       call-on-positive */
    ...
    DecodeAdr_Z80(
            &ArgStr[ArgCnt],
            MModImm | ((CurrZ80Syntax & eSyntaxZ80) ? (MModIReg16 | MModReg8) : 0));
    switch (AdrMode) {
    case ModReg8:
...
static void DecodeJP(Word Code) {
    ...
    DecodeAdr_Z80(
            &ArgStr[ArgCnt],
            MModImm | (((ArgCnt == 1) && (CurrZ80Syntax & eSyntaxZ80)) ? MModIReg16 : 0));
    switch (AdrMode) {
    case ModIReg16:
        if (AdrVals[0] != HLReg) {
            WrError(ErrNum_InvAddrMode);
...
static void DecodeINOUT(Word Code) {
    ...
    BAsmCode[1] = EvalStrIntExpressionWithResult(
            &ArgStr[((Code == 0xdb) && (ArgCnt == 2)) ? 2 : 1], UInt8, &EvalResult);
    if (EvalResult.OK) {
...
static void DecodeSRA(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        ;
    else if (!DecodeReg16(ArgStr[1].str.p_str, CurrZ80Syntax, &Reg) || (Reg != HLReg)) {
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
    } else {
...
static void DecodeRLC(Word Code) {
    if (!ChkArgCnt(
                (CurrZ80Syntax & eSyntax808x) ? 0 : 1,
                (CurrZ80Syntax & eSyntaxZ80) ? 1 : 0)) {
        return;
    }
...
static void AddFixed(char const* NName, CPUVar NMinCPU, Byte NCode, Word SyntaxMask) {
    if (MomCPU >= NMinCPU) {
        AddInstTable(InstTable, NName, (SyntaxMask << 8) | NCode, DecodeFixed);
    }
}

static void AddOp16(char const* NName, CPUVar NMinCPU, Byte NCode, Word SyntaxMask) {
    if (MomCPU >= NMinCPU) {
        AddInstTable(InstTable, NName, (SyntaxMask << 8) | NCode, DecodeOp16);
    }
}

static void AddOp8(char const* NName, CPUVar NMinCPU, Word NCode, Word SyntaxMask) {
    if (MomCPU >= NMinCPU) {
        AddInstTable(InstTable, NName, (SyntaxMask << 8) | NCode, DecodeOp8);
    }
}

static void AddALU(char const* NName, Byte NCode, Word SyntaxMask) {
...
static void InitFields(void) {
    ...
    AddFixed(
            "RDEL", CPU8085U, 0x18,
            eSyntax808x | eSyntaxZ80); /* TODO: find LD equivalent */
