...
static void CheckOverlap(tCodeChunk const* pChunk1, tCodeChunk const* pChunk2) {
    LargeWord OverlapStart, OverlapEnd;

    OverlapStart = max(pChunk1->Start, pChunk2->Start);
    OverlapEnd   = min(
            pChunk1->Start + pChunk1->Length - 1, pChunk2->Start + pChunk2->Length - 1);
    if (OverlapStart <= OverlapEnd) {
        fprintf(stderr, "code chunk overlap\n");
    }
}

static int SetCodeChunkListAllocLen(
        tCodeChunkList* pCodeChunkList, unsigned NewAllocLen) {
    int Result;
    ...
    if (!pCodeChunkList->AllocLen) {
        ...
        tCodeChunk* pNewChunks = (tCodeChunk*)realloc(
                pCodeChunkList->Chunks, NewAllocLen * sizeof(tCodeChunk));
...
void InitCodeChunkList(tCodeChunkList* pCodeChunkList) {
    pCodeChunkList->RealLen = pCodeChunkList->AllocLen = 0;
    pCodeChunkList->Chunks                             = NULL;
}

int MoveCodeChunkToList(
    ...
    memmove(pCodeChunkList->Chunks + Index + 1, pCodeChunkList->Chunks + Index,
    ...
    if (WarnOverlap) {
        if (Index > 0) {
            CheckOverlap(
                    pCodeChunkList->Chunks + Index - 1, pCodeChunkList->Chunks + Index);
        }
        if (Index < pCodeChunkList->RealLen - 1U) {
            CheckOverlap(
                    pCodeChunkList->Chunks + Index, pCodeChunkList->Chunks + Index + 1);
        }
    }
...
Boolean RetrieveCodeFromChunkList(
        tCodeChunkList const* pCodeChunkList, LargeWord Start, Byte* pData,
        unsigned Count) {
    tCodeChunk const* pChunk;
    LargeWord         OverlapStart, OverlapEnd;
    Boolean           Found;

    while (Count > 0) {
        Found = False;
        for (pChunk = pCodeChunkList->Chunks;
             pChunk < pCodeChunkList->Chunks + pCodeChunkList->RealLen; pChunk++) {
...
LargeWord GetCodeChunksStored(tCodeChunkList const* pCodeChunkList) {
    LargeWord         Sum = 0;
    tCodeChunk const* pChunk;
