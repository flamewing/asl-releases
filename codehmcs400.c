...
static void DecodeAdr(tStrComp const* pArg, Word Mask) {
    ...
    if ((l >= 3) && (l <= 4) && (as_toupper(pArg->str.p_str[0]) == 'M')
        && (as_toupper(pArg->str.p_str[1]) == 'R') && (isdigit(pArg->str.p_str[2]))
    ...
    if (EvalResult.OK) {
        if (!mFirstPassUnknown(EvalResult.Flags) && (AdrVal > SegLimits[SegData])) {
...
static void DecodeLD(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                &ArgStr[2], MModA | MModB | MModX | MModY | MModW | MModM | MModMInc
                                    | MModMDec | MModDir);
        switch (AdrMode) {
        case ModA:
            DecodeAdr(
                    &ArgStr[1], MModB | MModY | MModM | MModSPX | MModSPY | MModImm
                                        | MModDir | MModMR);
            switch (AdrMode) {
            case ModB:
...
static void DecodeXCH(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                &ArgStr[2], MModA | MModB | MModX | MModY | MModSPX | MModSPY | MModM
                                    | MModDir | MModMR);
        switch (AdrMode) {
        case ModA:
...
static void CodeImm(Word Code) {
    if (ChkArgCnt(1, 1)) {
        Boolean OK;
        Word    ImmVal
                = EvalStrIntExpression(&ArgStr[1], (Code & 0x8000) ? UInt2 : Int4, &OK);
...
static void CodeBR(Word Code) {
    if (ChkArgCnt(1, 1)) {
        ...
        DirVal = EvalStrIntExpressionWithResult(&ArgStr[1], CodeIntType, &EvalResult);
        if (EvalResult.OK
            && ChkSamePage(EProgCounter() + 1, DirVal, 8, EvalResult.Flags)) {
            ChkSpace(SegCode, EvalResult.AddrSpaceMask);
...
static void AddX(char const* pOpPart, Word Code, InstProc Proc) {
    char OpPart[30];
...
static void AddXY(char const* pOpPart, Word Code, InstProc Proc) {
    char OpPart[30];
