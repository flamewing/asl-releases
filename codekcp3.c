...
static Boolean IsWRegCore(char const* pArg, LongWord* pResult) {
    Boolean OK;
...
static tRegEvalResult IsWReg(tStrComp const* pArg, LongWord* pResult, Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static tRegEvalResult IsIWReg(tStrComp const* pArg, LongWord* pResult) {
    char     Tmp[10];
    tStrComp TmpComp;
...
static Boolean IsCond(int OtherArgCnt, LongWord* pErg) {
    static char const Conds[4][3] = {"Z", "NZ", "C", "NC"};
...
static void DecodeConstant(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        as_tempres_ini(&t);
        as_tempres_set_int(
                &t, EvalStrIntExpressionWithFlags(&ArgStr[2], Int32, &OK, &t.Flags));
...
static void DecodeOneReg(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && IsWReg(&ArgStr[1], &Reg, True)) {
...
static void DecodeALU(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && IsWReg(&ArgStr[1], &DReg, True)) {
...
static void DecodeJmp(Word Index) {
    ...
    if (ChkArgCnt(1, 2) && IsCond(1, &Cond)) {
        tEvalResult EvalResult;
        LongWord    Addr
                = EvalStrIntExpressionWithResult(&ArgStr[ArgCnt], UInt10, &EvalResult);
...
static void DecodeRet(Word Index) {
    ...
    if (ChkArgCnt(0, 1) && IsCond(0, &Cond)) {
...
static void DecodeMem(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && IsWReg(&ArgStr[1], &Reg, True)) {
...
static void AddReg(char const* NName, LongWord NCode) {
...
static void AddALU(char const* NName, LongWord NCode) {
