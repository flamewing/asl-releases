...
static Boolean DecodeReg(tStrComp const* pArg, Word* Erg, ShortInt* ErgLen) {
    Boolean OK;
    Word    Acc;
    LongInt Adr;
    char*   z;
    ...
    *ErgLen = -1;
    ...
    if ((Len == 4) && (as_strncasecmp(pArg->str.p_str + 1, "IV", 2) == 0)
        && (pArg->str.p_str[3] >= '0') && (pArg->str.p_str[3] <= '7')) {
...
static Boolean GetLen(tStrComp const* pArg, Word* Erg, tSymbolFlags* pFlags) {
    Boolean OK;
...
static void DecodeXML_XMR(Word Code) {
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU8x305)) {
        Boolean OK;
...
static void DecodeXMIT(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && DecodeReg(&ArgStr[2], &SrcReg, &SrcLen)) {
        if (SrcReg < 16) {
            if (ChkArgCnt(2, 2)) {
...
static void DecodeAri(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && DecodeReg(&ArgStr[ArgCnt], &DestReg, &DestLen)) {
            ...
            } else /* 3 Operanden --> Quelle ist I/O */
            {
                ...
                if (GetLen(&ArgStr[2], &Rot, &Flags)) {
                    if (DecodeReg(&ArgStr[1], &SrcReg, &SrcLen)) {
                        ...
                        } else if ((SrcLen != -1) && (SrcLen != Rot)) {
                            WrError(ErrNum_ConfOpSizes);
                        } else {
                            WAsmCode[0]
                                    = (Code << 13) | (SrcReg << 8) | (Rot << 5) | DestReg;
                            CodeLen = 1;
                        }
                    }
                }
        ...
        } else /* Ziel I/O */
        {
            ...
            if (OK) {
                if (DecodeReg(&ArgStr[1], &SrcReg, &SrcLen)) {
                    ...
                    if ((DestReg >= 16) && (SrcLen != -1) && (SrcLen != Rot)) {
                        WrError(ErrNum_ConfOpSizes);
                    } else {
...
static void DecodeNZT(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && DecodeReg(&ArgStr[1], &SrcReg, &SrcLen)) {
        if (SrcReg < 16) {
            if (ChkArgCnt(2, 2)) {
            ...
            if (OK) {
                ...
                if ((SrcLen != -1) && (Rot != SrcLen)) {
                    ...
                    Adr = EvalStrIntExpressionWithFlags(
                            &ArgStr[ArgCnt], UInt13, &OK, &Flags);
        ...
        if (OK) {
            ...
            if (OK) {
                if (GetLen(&ArgStr[3], &Len, &Flags)) {
                    PushLocHandle(-1);
                    EnterIntSymbol(
                            &LabPart, Code | (Adr << 16) | (Ofs << 8) | (Len & 7),
                            SegNone, False);
                    PopLocHandle();
                }
            }
...
static void AddAri(char const* NName, Word NCode) {
