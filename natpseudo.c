    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (mFirstPassUnknown(Flags)) {
            ...
            if (SetMaxCodeLen(Size << Shift)) {
                ...
                Word Value
                        = EvalStrIntExpression(&ArgStr[2], Shift ? Int16 : Int8, &ValOK);
...
Boolean DecodeNatPseudo(void) {
    ...
    if (!InstTable) {
        ...
        AddInstTable(
                InstTable, "ADDR", eIntPseudoFlag_BigEndian | eIntPseudoFlag_AllowInt,
                DecodeIntelDB);
        AddInstTable(
                InstTable, "ADDRW", eIntPseudoFlag_BigEndian | eIntPseudoFlag_AllowInt,
                DecodeIntelDW);
