...
@@1:    ADD    BX, [PrefixSeg]
        ...
        ADC    DX, 0                    
        TEST   WORD PTR ES:[OvrHeader.CodeSize],1 ;****** take roundup into account
        JZ     NOROUND
        ADD    AX,1
        ADC    DX,0
NOROUND:
...
;  Swap the code from XMS to the heap
        ...
        MOV    AH, 11
        LEA    SI, XmsMove     ; not important if BX destroyed here
        CALL   [XmsDriver]
        OR     AX, AX
        JZ     @@1
        DEC    AX
...
@@1:    mov     ax,dx
        MOV    Word Ptr [XmsMove.BlkSize], DX
        XOR    AX, AX
        MOV    Word Ptr [XmsMove.BlkSize+2], AX
        MOV    [XmsMove.SrcHandle], AX
        MOV    Word Ptr [XmsMove.SrcOffset], AX
        ...
        MOV    AH, 11
        LEA    SI, XmsMove
        push    bx              ;****** preserve MSW of XMS address
        CALL   [XmsDriver]
        pop     bx              ;******
