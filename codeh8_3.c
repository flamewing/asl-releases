        ...
        if ((as_toupper(*pArg) == 'R') && IsNum(pArg[1], pResult)) {
            if (as_toupper(pArg[2]) == 'L') {
                ...
                *pSize = eSymbolSize8Bit;
                ...
                (as_toupper(*pArg) == 'E') && (as_toupper(pArg[1]) == 'R')
                && IsNum(pArg[2], pResult)) {
...
static void DissectReg_H8_3(
        char* pDest, size_t DestSize, tRegInt Value, tSymbolSize InpSize) {
    switch (InpSize) {
    case eSymbolSize8Bit:
        as_snprintf(
                pDest, DestSize, "R%u%c", (unsigned)(Value & 7), "HL"[(Value >> 3) & 1]);
        break;
    case eSymbolSize16Bit:
        if (Value == (REG_SP | REG_MARK)) {
            ...
            as_snprintf(
                    pDest, DestSize, "%c%u", "RE"[(Value >> 3) & 1],
                    (unsigned)(Value & 7));
        }
        break;
...
static tRegEvalResult DecodeReg(
        tStrComp const* pArg, Byte* pResult, unsigned SizeMask, tSymbolSize* pSize,
        Boolean MustBeReg) {
    tRegDescr      RegDescr;
    ...
    if ((RegEvalResult == eIsReg) && !((SizeMask >> *pSize) & 1)) {
...
static Byte DecodeBaseReg(tStrComp const* pArg, Byte* Erg, Boolean MustBeReg) {
    tSymbolSize HSize;
    Word        Mask;

    switch (DecodeReg(
            pArg, Erg, (1 << eSymbolSize16Bit) | (1 << eSymbolSize32Bit), &HSize,
            MustBeReg)) {
    case eIsNoReg:
        return 0;
    case eRegAbort:
    ...
    Mask = (HSize == eSymbolSize16Bit) ? (M_CPUH8_300L | M_CPU6413308 | M_CPUH8_300)
                                       : (M_CPU6413309 | M_CPUH8_300H);
...
static Boolean Is16(LongInt Address) {
    return (CPU16) ? (True)
                   : (((Address >= 0) && (Address <= 0x7fff))
                      || ((Address >= 0xff8000) && (Address <= 0xffffff)));
}
...
static void DecideAbsolute(tStrComp const* pArg, Word Mask) {
    LongInt Addr;
    Boolean OK;
    ...
    if (*pArg->str.p_str == '@') {
        ...
        if (IsIndirect(Arg.str.p_str)) {
            ...
            if (AdrPart == 0xff) {
                ...
                if (MomSize == eSymbolSizeUnknown) {
                    MomSize = ((DispAcc >= -32768) && (DispAcc <= 32767))
                                      ? eSymbolSize16Bit
                                      : eSymbolSize24Bit;
                }
                switch (MomSize) {
...
chk:
    if (((AdrMode == ModReg) && (OpSize == eSymbolSize32Bit))
        ...
        || (AdrMode == ModAbs24) || (AdrMode == ModInd24)) {
...
static void DissectBit_H8_3(char* pDest, size_t DestSize, LargeWord Inp) {
    ...
    as_snprintf(
            pDest, DestSize, "#%u,$%s%x", BitPos,
            (HexStartCharacter == 'A') ? (CPU16 ? "FF" : "FFFF")
                                       : (CPU16 ? "ff" : "ffff"),
            (unsigned)Address);
}
...
static void DecodeMOV(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg:
            ...
            DecodeAdr(
                    &ArgStr[1],
                    MModReg | MModIReg | MModPostInc | MModInd | MModImm
                            | ((OpSize == eSymbolSize8Bit) ? MModAbs
                                                           : (MModAbs16 | MModAbs24)));
            switch (AdrMode) {
            case ModReg: {
...
static void DecodeMOVTPE_MOVFPE(Word CodeTPE) {
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU6413308)) {
        ...
        if (AdrMode != ModNone) {
            if (OpSize == eSymbolSize8Bit) {
                WrError(ErrNum_InvOpSize);
                    ...
                    || ChkCPU32(ErrNum_AddrModeNotSupported)) {
                if (OpSize == eSymbolSize32Bit) {
                    WAsmCode[0] = 0x0100;
                }
                CodeLen                      = 2 * OpSize;
...
static void DecodeLDC_STC(Word CodeIsSTC) {
    if (ChkArgCnt(2, 2)) {
        ...
        if (as_strcasecmp(pRegArg->str.p_str, "CCR")) {
            WrError(ErrNum_InvAddrMode);
        } else {
            SetOpSize(eSymbolSize8Bit);
            DecodeAdr(
                    pMemArg,
                    MModReg | MModIReg | MModInd | MModAbs16 | MModAbs24
                            | (CodeIsSTC ? MModPreDec : (MModImm | MModPostInc)));
            switch (AdrMode) {
            case ModReg:
                CodeLen     = 2;
...
static void DecodeADD_SUB(Word IsSUB) {
    if (ChkArgCnt(2, 2)) {
        ...
        if (AdrMode != ModNone) {
            ...
            if (AdrMode != ModNone) {
                if (((OpSize > eSymbolSize16Bit)
                     || ((OpSize == eSymbolSize16Bit) && (AdrMode == ModImm)))
                    && !ChkCPU32(ErrNum_AddrModeNotSupported))
                    ;
                else {
                    switch (AdrMode) {
                    case ModImm:
                        ...
                        case eSymbolSize8Bit:
                            if (IsSUB) {
                                ...
                                CodeLen = 2;
                                WAsmCode[0]
                                        = 0x8000 + (((Word)HReg) << 8) + Lo(AdrVals[0]);
                            }
                            break;
                        case eSymbolSize16Bit:
                            CodeLen     = 4;
                    ...
                    case ModReg:
                        ...
                        case eSymbolSize32Bit:
                            CodeLen     = 2;
                            WAsmCode[0] = 0x0a00 + (IsSUB << 12) + 0x80 + (AdrPart << 4)
                                          + HReg;
                            break;
                        default:
                            break;
        ...
        if (AdrMode != ModNone) {
            if ((OpSize == eSymbolSizeUnknown) || ChkCPU32(ErrNum_AddrModeNotSupported)) {
                ...
                case ModImm:
                    ...
                    case eSymbolSize8Bit:
                        CodeLen     = 2;
                        WAsmCode[0] = 0xc000 + (Code << 12) + (((Word)HReg) << 8)
                                      + Lo(AdrVals[0]);
                        break;
                    case eSymbolSize16Bit:
                        CodeLen     = 4;
...
static void DecodeADDS_SUBS(Word IsSUBS) {
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(&ArgStr[2], MModReg);
        if (AdrMode != ModNone) {
            if (((CPU16) && (OpSize != eSymbolSize16Bit))
                || ((!CPU16) && (OpSize != eSymbolSize32Bit))) {
                WrError(ErrNum_InvOpSize);
            } else {
                Byte HReg = AdrPart;
                DecodeAdr(&ArgStr[1], MModImm);
                if (AdrMode != ModNone) {
                    ...
                    if ((AdrLong != 1) && (AdrLong != 2) && (AdrLong != 4)) {
...
static void DecodeEXTS_EXTU(Word IsEXTS) {
    if (ChkArgCnt(1, 1) && ChkCPU32(ErrNum_InstructionNotSupported)) {
        DecodeAdr(&ArgStr[1], MModReg);
        if (AdrMode != ModNone) {
            if ((OpSize != eSymbolSize16Bit) && (OpSize != 2)) {
                WrError(ErrNum_InvOpSize);
            } else {
                CodeLen = 2;
                switch (OpSize) {
...
static void DecodeCond(Word Code) {
    if (!ChkArgCnt(1, 1))
        ;
            ...
            (OpSize != eSymbolSizeUnknown) && (OpSize != eSymbolSizeFloat32Bit)
            && (OpSize != eSymbolSize32Bit)) {
        WrError(ErrNum_InvOpSize);
    } else {
        Boolean      OK;
        tSymbolFlags Flags;
        LongInt AdrLong = EvalStrIntExpressionWithFlags(&ArgStr[1], Int24, &OK, &Flags)
                          - (EProgCounter() + 2);
...
static void DecodeTRAPA(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU6413309)) {
        Boolean OK;

        WAsmCode[0] = EvalStrIntExpressionOffs(
                              &ArgStr[1], !!(*ArgStr[1].str.p_str == '#'), UInt2, &OK)
                      << 4;
        if (OK) {
            WAsmCode[0] += 0x5700;
            CodeLen = 2;
...
static void AddLogic(char const* NName, char const* NNameBit, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeLogic);
    AddInstTable(InstTable, NNameBit, NCode, DecodeLogicBit);
}

static void AddMul(char const* NName, Word NCode) {
