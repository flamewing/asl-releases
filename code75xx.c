...
static void DecodeJCP(Word Index) {
    if (ChkArgCnt(1, 1)) {
        tEvalResult EvalResult;
        Word        Address
                = EvalStrIntExpressionWithResult(&ArgStr[1], CodeIntType, &EvalResult);

        if (EvalResult.OK
            && ChkSamePage(Address, EProgCounter() + 1, 6, EvalResult.Flags)) {
...
static void AddImm4(char const* NewName, Word NewCode) {
    AddInstTable(InstTable, NewName, NewCode, DecodeImm4);
}

static void AddImm2(char const* NewName, Word NewCode) {
    AddInstTable(InstTable, NewName, NewCode, DecodeImm2);
}

static void AddImm5(char const* NewName, Word NewCode) {
    AddInstTable(InstTable, NewName, NewCode, DecodeImm5);
}

static void AddImm8(char const* NewName, Word NewCode) {
    AddInstTable(InstTable, NewName, NewCode, DecodeImm8);
}

static void AddImm3(char const* NewName, Word NewCode) {
    AddInstTable(InstTable, NewName, NewCode, DecodeImm3);
}

static void AddPR(char const* NewName, Word NewCode) {
    AddInstTable(InstTable, NewName, NewCode, DecodePR);
}

static void AddDataMem(char const* NewName, Word NewCode) {
    AddInstTable(InstTable, NewName, NewCode, DecodeDataMem);
}

static void AddAbs(char const* NewName, Word NewCode) {
...
static void SwitchTo_75xx(void) {
    ...
    PCSymbol    = "PC";
    HeaderID    = FindFamilyByName("75xx")->Id;
    NOPCode     = 0x00;
    DivideChars = ",";
    HasAttrs    = False;
    ...
    MakeCode   = MakeCode_75xx;
    IsDef      = IsDef_75xx;
    SwitchFrom = SwitchFrom_75xx;
    InitFields();
}
