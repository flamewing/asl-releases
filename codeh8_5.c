...
#define MModAll                                                                        \
    (MModReg | MModIReg | MModDisp8 | MModDisp16 | MModPredec | MModPostInc | MModAbs8 \
     | MModAbs16 | MModImm)
...
static Boolean DecodeRegCore(char const* pArg, Byte* pResult) {
    if (!as_strcasecmp(pArg, "SP")) {
        ...
        *pResult = REG_FP | REG_MARK;
            ...
            (strlen(pArg) == 2) && (as_toupper(*pArg) == 'R') && (pArg[1] >= '0')
            && (pArg[1] <= '7')) {
        *pResult = pArg[1] - '0';
    } else {
        return False;
...
static tRegEvalResult DecodeReg(tStrComp const* pArg, Byte* pResult, Boolean MustBeReg) {
    tRegDescr      RegDescr;
    ...
    RegEvalResult = EvalStrRegExpressionAsOperand(
            pArg, &RegDescr, &EvalResult, eSymbolSize16Bit, MustBeReg);
    ...
    *pResult = 0;
    do {
        ...
        if (p2) {
            ...
            if (DecodeReg(&Left, &Reg1, True) != eIsReg) {
                return False;
            }
            if (DecodeReg(&Right, &Reg2, True) != eIsReg) {
                return False;
            }
            if (Reg1 > Reg2) {
                Reg2 += 8;
            ...
            if (DecodeReg(&Arg, &Reg1, True) != eIsReg) {
                return False;
            }
            *pResult |= (1 << Reg1);
        }
...
static void DecideAbsolute(LongInt Value, tSymbolSize Size, Boolean Unknown, Word Mask) {
    ...
    if (Size == eSymbolSizeUnknown) {
        if (((Value >> 8) == Reg_BR) && (Mask & MModAbs8)) {
...
static void DecodeAdr(tStrComp* pArg, Word Mask) {
    ...
    if (*pArg->str.p_str == '@') {
        ...
        do {
            ...
            case eIsNoReg:
                ...
                DispAcc += EvalStrIntExpressionOffsWithFlags(
                        &Arg, !!(*Arg.str.p_str == '#'), Int32, &OK, &Flags);
        ...
        if (OK) {
            if (RegPart == -1) {
                DecideAbsolute(DispAcc, DispSize, Unknown, Mask);
            } else if (DispAcc == 0) {
                switch (DispSize) {
                case eSymbolSizeUnknown:
                    AdrMode = ModIReg;
...
static Boolean AdaptImmSize(tStrComp const* pArg) {
    LongInt ImmV        = ImmVal();
    Boolean ImmValShort = CompMode ? ((ImmV >= 0) && (ImmV <= 255))
                                   : ((ImmV >= -128) && (ImmV <= 127));
...
static LongWord CodeOpSize(tSymbolSize Size) {
    return (Size == eSymbolSizeUnknown) ? 0 : ((Size == eSymbolSize16Bit) ? 3 : 2);
}

static tSymbolSize DecodeOpSize(Byte SizeCode) {
    return (SizeCode & 2) ? ((SizeCode & 1) ? eSymbolSize16Bit : eSymbolSize8Bit)
                          : eSymbolSizeUnknown;
}
...
static LongWord EvalBitPosition(tStrComp const* pArg, tSymbolSize Size, Boolean* pOK) {
    return EvalStrIntExpressionOffs(
            pArg, !!(*pArg->str.p_str == '#'), (Size == eSymbolSize16Bit) ? UInt4 : UInt3,
            pOK);
}
...
static LongWord AssembleBitSymbol(
        Byte BitPos, Word Address, tSymbolSize OpSize, tSymbolSize AddrSize) {
    return (CodeOpSize(OpSize) << 30) | (CodeOpSize(AddrSize) << 28)
           | (Address << ((OpSize == eSymbolSize8Bit) ? 3 : 4)) | (BitPos << 0);
}
...
static Boolean DecodeBitArg2(
    ...
    BitPos = EvalBitPosition(
            pBitArg, (OpSize == eSymbolSizeUnknown) ? eSymbolSize16Bit : OpSize, &OK);
    if (!OK) {
        return False;
    }
...
static void ExpandBit_H8_5(
    ...
    if (pInnermostNamedStruct) {
        ...
        EnterIntSymbol(
                pVarName,
                AssembleBitSymbol(
                        pStructElem->BitPos, Address, OpSize, eSymbolSizeUnknown),
                SegBData, False);
        PopLocHandle();
        /* TODO: MakeUseList? */
    }
...
static Boolean FormatToBranchSize(char const* pFormat, tSymbolSize* pOpSize) {
    if (!strcmp(pFormat, " "))
        ;

    else if (!strcmp(pFormat, "16")) /* treat like .L */
    {
        if (*pOpSize == eSymbolSizeUnknown) {
            *pOpSize = eSymbolSize32Bit;
...
static void DecodeFixed(Word Code) {
    if (!ChkArgCnt(0, 0))
        ;
    else if (OpSize != eSymbolSizeUnknown) {
...
static void DecodeMOV(Word Dummy) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if ((OpSize != eSymbolSize8Bit) && (OpSize != eSymbolSize16Bit)) {
            ...
            if (AdrMode != ModNone) {
                ...
                if (AdrMode != ModNone) {
                    if (FormatCode == 0) {
                        if ((AdrMode == ModImm)
                            && ((ImmSize == OpSize) || (ImmSize == eSymbolSizeUnknown))
                            && (Adr2Mode == ModReg)) {
                            FormatCode = 2 + OpSize;
                        } else if ((AdrMode == ModReg) && (Adr2Byte == 0xe6)) {
                            FormatCode = 4;
                        } else if ((Adr2Mode == ModReg) && (AdrByte == 0xe6)) {
                            FormatCode = 4;
                        } else if ((AdrMode == ModReg) && (Adr2Mode == ModAbs8)) {
                            FormatCode = 6;
                        } else if ((AdrMode == ModAbs8) && (Adr2Mode == ModReg)) {
                            FormatCode = 5;
                        } else {
                            FormatCode = 1;
                        }
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        if ((AdrMode != ModImm) || (Adr2Mode != ModReg)) {
                            WrError(ErrNum_InvAddrMode);
                        } else if (OpSize != eSymbolSize8Bit) {
                            WrError(ErrNum_InvOpSize);
                        } else {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        if ((AdrMode != ModAbs8) || (Adr2Mode != ModReg)) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                        ...
                        break;
                    case 6:
                        if ((Adr2Mode != ModAbs8) || (AdrMode != ModReg)) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
...
static void DecodeLDC_STC(Word IsSTC_16) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (!DecodeCReg(ArgStr[CRegIdx].str.p_str, &HReg)) {
...
static void DecodeLDM(Word Dummy) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_InvFormat);
    } else if (!DecodeRegList(&ArgStr[2], BAsmCode + 1)) {
...
static void DecodeSTM(Word Dummy) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_InvFormat);
    } else if (!DecodeRegList(&ArgStr[1], BAsmCode + 1)) {
        WrError(ErrNum_InvRegList);
    } else {
...
static void DecodeADD_SUB(Word IsSUB_16) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GQ")) {
        if (OpSize == eSymbolSizeUnknown) {
            SetOpSize(eSymbolSize16Bit);
        }
        if ((OpSize != eSymbolSize8Bit) && (OpSize != eSymbolSize16Bit)) {
            ...
            if (AdrMode != ModNone) {
                ...
                if (AdrMode != ModNone) {
                    AdrLong = ImmVal();
                    if (FormatCode == 0) {
                        if ((AdrMode == ModImm) && (abs(AdrLong) >= 1)
                            && (abs(AdrLong) <= 2) && !IsSUB_16) {
                            FormatCode = 2;
                        } else {
                            FormatCode = 1;
                        }
...
static void DecodeCMP(Word Dummy) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GEI")) {
        if (OpSize == eSymbolSizeUnknown) {
            SetOpSize((FormatCode == 2) ? eSymbolSize8Bit : eSymbolSize16Bit);
        }
...
static void DecodeRegEA(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("G")) {
        if (OpSize == eSymbolSizeUnknown) {
            SetOpSize(pOrder->DefSize);
        }
        if (!((1 << OpSize) & pOrder->SizeMask)) {
            WrError(ErrNum_InvOpSize);
...
static void DecodeTwoReg(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_InvFormat);
            ...
            DecodeReg(&ArgStr[1], &HReg, True) && DecodeReg(&ArgStr[2], &AdrByte, True)) {
        if (OpSize == eSymbolSizeUnknown) {
            SetOpSize(pOrder->DefSize);
        }
        if (!((1 << OpSize) & pOrder->SizeMask)) {
...
static void DecodeLog(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if (!DecodeCReg(ArgStr[2].str.p_str, &HReg)) {
        WrStrErrorPos(ErrNum_InvCtrlReg, &ArgStr[2]);
    } else {
        ...
        if (AdrMode != ModNone) {
            ...
            if ((OpSize != 0) && (OpSize != 1)) {
                ...
                case eIsReg:
                    ...
                    break;
                case eIsNoReg:
                    BitPos = EvalStrIntExpressionOffs(
                            &ArgStr[1], !!(*ArgStr[1].str.p_str == '#'),
                            (OpSize == eSymbolSize8Bit) ? UInt3 : UInt4, &OK);
                    if (OK) {
                        BitPos |= 0x80;
                    }
                    break;
...
static void DecodeJMP_JSR(Word IsJSR_8) {
    if (ChkArgCnt(1, 1) && CheckFormat("G")) {
        ...
        case ModIReg:
            BAsmCode[0] = 0x11;
            BAsmCode[1] = 0xd0 | IsJSR_8 | (AdrByte & 7);
            CodeLen     = 2;
            break;
        case ModDisp8:
        case ModDisp16:
            BAsmCode[0] = 0x11;
            BAsmCode[1] = AdrByte | IsJSR_8;
            memcpy(BAsmCode + 2, AdrVals, AdrCnt);
            CodeLen = 2 + AdrCnt;
            break;
        case ModAbs16:
            BAsmCode[0] = 0x10 | IsJSR_8;
            memcpy(BAsmCode + 1, AdrVals, AdrCnt);
            CodeLen = 1 + AdrCnt;
            break;
        }
...
static void DecodePJMP_PJSR(Word IsPJMP) {
    if (!ChkArgCnt(1, 1))
        ...
        case eIsReg:
            BAsmCode[0] = 0x11;
            BAsmCode[1] = 0xc0 | ((1 - IsPJMP) << 3) | HReg;
            CodeLen     = 2;
            break;
        case eIsNoReg: {
            Boolean OK;
...
static void DecodePRTD_RTD(Word IsPRTD) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        HSize = eSymbolSizeUnknown;
        SplitDisp(&Arg, &HSize);
        if (HSize != eSymbolSizeUnknown) {
            SetOpSize(HSize);
        }
...
static void AddOne(char const* NName, Word NCode, Byte NMask, tSymbolSize NDef) {
...
static void AddOneReg(char const* NName, Word NCode, Byte NMask, tSymbolSize NDef) {
    if (InstrZ >= OneRegOrderCount) {
        exit(255);
    }
...
static void AddRegEA(char const* NName, Word NCode, Byte NMask, tSymbolSize NDef) {
    if (InstrZ >= RegEAOrderCount) {
        exit(255);
    }
...
static void AddTwoReg(char const* NName, Word NCode, Byte NMask, tSymbolSize NDef) {
    if (InstrZ >= TwoRegOrderCount) {
        exit(255);
    }
...
static void AddLog(char const* NName, Word NCode) {
...
static void InitFields(void) {
    ...
    AddOne("CLR", 0x13, 3, eSymbolSize16Bit);
    AddOne("NEG", 0x14, 3, eSymbolSize16Bit);
    AddOne("NOT", 0x15, 3, eSymbolSize16Bit);
    AddOne("ROTL", 0x1c, 3, eSymbolSize16Bit);
    AddOne("ROTR", 0x1d, 3, eSymbolSize16Bit);
    AddOne("ROTXL", 0x1e, 3, eSymbolSize16Bit);
    AddOne("ROTXR", 0x1f, 3, eSymbolSize16Bit);
    AddOne("SHAL", 0x18, 3, eSymbolSize16Bit);
    AddOne("SHAR", 0x19, 3, eSymbolSize16Bit);
    AddOne("SHLL", 0x1a, 3, eSymbolSize16Bit);
    AddOne("SHLR", 0x1b, 3, eSymbolSize16Bit);
    AddOne("TAS", 0x17, 1, eSymbolSize8Bit);
    ...
    AddRegEA("ADDS", 0x28, 3, eSymbolSize16Bit);
    AddRegEA("ADDX", 0xa0, 3, eSymbolSize16Bit);
    AddRegEA("AND", 0x50, 3, eSymbolSize16Bit);
    AddRegEA("DIVXU", 0xb8, 3, eSymbolSize16Bit);
    AddRegEA("MULXU", 0xa8, 3, eSymbolSize16Bit);
    AddRegEA("OR", 0x40, 3, eSymbolSize16Bit);
    AddRegEA("SUBS", 0x38, 3, eSymbolSize16Bit);
    AddRegEA("SUBX", 0xb0, 3, eSymbolSize16Bit);
