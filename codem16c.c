...
static char const Flags[] = "CDZSBOIU";
...
static Boolean DecodeRegCore(char const* pArg, Byte* pResult, tSymbolSize* pSize) {
    ...
    switch (strlen(pArg)) {
    case 3:
        if ((as_toupper(*pArg) == 'R') && (pArg[1] >= '0') && (pArg[1] <= '1')
        ...
        break;
    case 2:
        if ((as_toupper(*pArg) == 'R') && (pArg[1] >= '0') && (pArg[1] <= '3')) {
            *pResult = pArg[1] - '0';
            *pSize   = eSymbolSize16Bit;
            return True;
        }
        if ((as_toupper(*pArg) == 'A') && (pArg[1] >= '0') && (pArg[1] <= '1')) {
            *pResult = pArg[1] - '0' + 4;
            *pSize   = eSymbolSize16Bit;
            return True;
        }
...
static void DissectReg_M16C(
    ...
    case eSymbolSize16Bit:
        ...
        default:
            as_snprintf(
                    pDest, DestSize, "%c%u", (Value & 4) ? 'A' : 'R',
                    (unsigned)(Value & 3));
        }
        break;
...
static tRegEvalResult DecodeReg(
        tStrComp const* pArg, Byte* pResult, Boolean MustBeReg, tSymbolSize* pSize) {
    tRegDescr      RegDescr;
    ...
    RegEvalResult = EvalStrRegExpressionAsOperand(
            pArg, &RegDescr, &EvalResult, *pSize, MustBeReg);
...
static ShortInt DecodeAdr(tStrComp const* pArg, Word Mask, tAdrResult* pResult) {
    ...
    if ((p) && (pArg->str.p_str[ArgLen - 1] == ']')) {
        ...
        if (!as_strcasecmp(RegPart.str.p_str, "SP")) {
            ...
            case eSymbolSize16Bit:
                ...
                case 4:
                case 5: {
                    DispAcc = EvalStrIntExpression(
                            &DispPart, (Mask & MModDisp20) ? Int20 : Int16, &OK);
                    if (OK) {
                        if ((DispAcc == 0) && (Mask & MModGen)) {
                            pResult->Type = ModGen;
                            pResult->Mode = (pResult->Mode & 1) + 6;
                        } else if (
                                (DispAcc >= 0) && (DispAcc <= 255) && (Mask & MModGen)) {
                            pResult->Type    = ModGen;
                            pResult->Vals[0] = DispAcc & 0xff;
                            pResult->Cnt     = 1;
                            pResult->Mode    = (pResult->Mode & 1) + 8;
                        } else if (
                                (DispAcc >= -32768) && (DispAcc <= 65535)
                                && (Mask & MModGen)) {
                            pResult->Type    = ModGen;
                            pResult->Vals[0] = DispAcc & 0xff;
                            pResult->Vals[1] = (DispAcc >> 8) & 0xff;
    ...
    if (*pArg->str.p_str == '#') {
        ...
        case eSymbolSizeUnknown:
            ...
            break;
        case eSymbolSize8Bit:
            pResult->Vals[0] = EvalStrIntExpressionOffsWithFlags(
                    pArg, 1, Int8, &OK, &pResult->ImmSymFlags);
            if (OK) {
                pResult->Type = ModImm;
                pResult->Cnt  = 1;
            }
            break;
        case eSymbolSize16Bit:
            DispAcc = EvalStrIntExpressionOffsWithFlags(
                    pArg, 1, Int16, &OK, &pResult->ImmSymFlags);
            if (OK) {
                pResult->Type    = ModImm;
                pResult->Vals[0] = DispAcc & 0xff;
                pResult->Vals[1] = (DispAcc >> 8) & 0xff;
...
static Boolean DecodeCReg(char* Asc, Byte* Erg) {
    if (!as_strcasecmp(Asc, "INTBL")) {
        *Erg = 1;
    } else if (!as_strcasecmp(Asc, "INTBH")) {
        *Erg = 2;
    } else if (!as_strcasecmp(Asc, "FLG")) {
        *Erg = 3;
    } else if (!as_strcasecmp(Asc, "ISP")) {
        *Erg = 4;
    } else if (!as_strcasecmp(Asc, "SP")) {
        *Erg = 5;
    } else if (!as_strcasecmp(Asc, "SB")) {
        *Erg = 6;
    } else if (!as_strcasecmp(Asc, "FB")) {
        *Erg = 7;
    } else {
...
static void DecodeDisp(
        tStrComp* pArg, IntType Type1, IntType Type2, LongInt* DispAcc, Boolean* OK) {
    if (ArgCnt == 2) {
...
static Boolean DecodeBitAdr(Boolean MayShort, tAdrResult* pResult) {
    ...
    if (ArgCnt == 2) {
        DispAcc = EvalStrIntExpression(
                &ArgStr[1], UInt16, &OK); /* RMS 02: The displacement can be 16 bits */
        if (!OK) {
            return False;
        }
    } else {
    ...
    if (!Pos1) {
        ...
        if (OK && (DispAcc < 0x10000)) /* RMS 09: This is optional, it detects rollover of
                                          the bit address. */
        {
            pResult->Mode    = 15;
            pResult->Vals[0] = DispAcc & 0xff;
        ...
        WrStrErrorPos(
                ErrNum_InvBitPos,
                &ArgStr[ArgCnt]); /* RMS 08: Notify user there's a problem with address */
        return False;
    }
    ...
    if (DecodeReg(&RegPart, &pResult->Mode, True, &RegSize) == eIsReg) {
        ...
        case REG_SB:
            ...
            WrError(ErrNum_InvBitPos); /* RMS 08: Notify user there's a problem with the
                                          offset */
            return False;
        case REG_FB:
        ...
        case 5:
            pResult->Mode &= 1;
            DecodeDisp(
                    &DispPart, UInt16, UInt16, &DispAcc,
                    &OK); /* RMS 03: The offset is a full 16 bits */
            if (OK) {
                if (DispAcc == 0) {
                    pResult->Mode += 6;
                } else if ((DispAcc > 0) && (DispAcc < 256)) {
...
static Integer ImmVal(tAdrResult const* pResult) {
...
static void CodeGen(
        Byte GenCode, Byte Imm1Code, Byte Imm2Code, tAdrResult const* pSrcAdrResult,
        tAdrResult const* pDestAdrResult) {
    if (pSrcAdrResult->Type == ModImm) {
        ...
        memcpy(BAsmCode + 2 + pDestAdrResult->Cnt, pSrcAdrResult->Vals,
               pSrcAdrResult->Cnt);
    } else {
        ...
        memcpy(BAsmCode + 2 + pSrcAdrResult->Cnt, pDestAdrResult->Vals,
               pDestAdrResult->Cnt);
    }
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GSQZ")) {
        ...
        if ((DecodeAdr(&ArgStr[2], MModGen | MModSPRel, &DestAdrResult) != ModNone)
            ...
            if (OpSize == eSymbolSizeUnknown) {
                ...
                if (FormatCode == 0) {
                    if ((DestAdrResult.Type == ModSPRel)
                        || (SrcAdrResult.Type == ModSPRel)) {
                        FormatCode = 1;
                    } else if (
                            (OpSize == 0) && (SrcAdrResult.Type == ModImm)
                            && (IsShort(DestAdrResult.Mode, &SMode))) {
                        FormatCode = (ImmVal(&SrcAdrResult) == 0) ? 4 : 2;
                    } else if (
                            (SrcAdrResult.Type == ModImm) && (ImmVal(&SrcAdrResult) >= -8)
                            && (ImmVal(&SrcAdrResult) <= 7)) {
                        FormatCode = 3;
                    } else if (
                            (SrcAdrResult.Type == ModImm)
                            && ((DestAdrResult.Mode & 14) == 4)) {
                        FormatCode = 2;
                    } else if (
                            (OpSize == 0) && (SrcAdrResult.Type == ModGen)
                            && (IsShort(SrcAdrResult.Mode, &SMode))
                            && ((DestAdrResult.Mode & 14) == 4)
                            && ((SrcAdrResult.Mode >= 2)
                                || (Odd(SrcAdrResult.Mode ^ DestAdrResult.Mode)))) {
                        FormatCode = 2;
                    } else if (
                            (OpSize == 0) && (SrcAdrResult.Type == ModGen)
                            && (SrcAdrResult.Mode <= 1)
                            && (IsShort(DestAdrResult.Mode, &SMode))
                            && ((DestAdrResult.Mode >= 2)
                                || (Odd(SrcAdrResult.Mode ^ DestAdrResult.Mode)))) {
                        FormatCode = 2;
                    } else if (
                            (OpSize == 0) && (DestAdrResult.Mode <= 1)
                            && (SrcAdrResult.Type == ModGen)
                            && (IsShort(SrcAdrResult.Mode, &SMode))
                            && ((SrcAdrResult.Mode >= 2)
                                || (Odd(SrcAdrResult.Mode ^ DestAdrResult.Mode)))) {
                        FormatCode = 2;
                ...
                case 1:
                    if (SrcAdrResult.Type == ModSPRel) {
                        ...
                        memcpy(BAsmCode + 2 + DestAdrResult.Cnt, SrcAdrResult.Vals,
                               SrcAdrResult.Cnt);
                        ...
                        memcpy(BAsmCode + 2 + SrcAdrResult.Cnt, DestAdrResult.Vals,
                               DestAdrResult.Cnt);
                ...
                case 2:
                    if (SrcAdrResult.Type == ModImm) {
                        if (DestAdrResult.Type != ModGen) {
                            WrError(ErrNum_InvAddrMode);
                        } else if ((DestAdrResult.Mode & 14) == 4) {
                            BAsmCode[0] = 0xe2 - (OpSize << 6)
                                          + ((DestAdrResult.Mode & 1) << 3);
                            ...
                            if (OpSize != 0) {
                                ...
                                memcpy(BAsmCode + 1, SrcAdrResult.Vals, SrcAdrResult.Cnt);
                                memcpy(BAsmCode + 1 + SrcAdrResult.Cnt,
                                       DestAdrResult.Vals, DestAdrResult.Cnt);
                        ...
                        if (DestAdrResult.Type != ModGen) {
                            ...
                            if ((SrcAdrResult.Mode <= 1)
                                ...
                                BAsmCode[0] = 0x30 + ((DestAdrResult.Mode & 1) << 2)
                                              + (SMode & 3);
                                memcpy(BAsmCode + 1, SrcAdrResult.Vals, SrcAdrResult.Cnt);
                                CodeLen = 1 + SrcAdrResult.Cnt;
                            }
                            ...
                            if ((SrcAdrResult.Mode <= 1)
                                ...
                                BAsmCode[0] = 0x08 + ((DestAdrResult.Mode & 1) << 2)
                                              + (SMode & 3);
                                memcpy(BAsmCode + 1, SrcAdrResult.Vals, SrcAdrResult.Cnt);
                                CodeLen = 1 + SrcAdrResult.Cnt;
                            }
                        } else if (
                                ((SrcAdrResult.Mode & 14) != 0)
                                || (!IsShort(DestAdrResult.Mode, &SMode))) {
                            WrError(ErrNum_InvAddrMode);
                        } else if (
                                (DestAdrResult.Mode <= 1)
                                && (!Odd(SrcAdrResult.Mode ^ DestAdrResult.Mode))) {
                            WrError(ErrNum_InvAddrMode);
                        } else {
                            if (SMode == 3) {
                            ...
                            BAsmCode[0]
                                    = 0x00 + ((SrcAdrResult.Mode & 1) << 2) + (SMode & 3);
...
static void DecodeLDC_STC(Word IsSTC) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("G")) {
        tStrComp const *pRegArg = IsSTC ? &ArgStr[1] : &ArgStr[2],
        ...
        if (!as_strcasecmp(pRegArg->str.p_str, "PC")) {
            if (!IsSTC) {
                ...
                if ((AdrResult.Type == ModGen) && (AdrResult.Mode < 6)) {
                    WrError(ErrNum_InvAddrMode);
                } else {
                    BAsmCode[0] = 0x7c;
            ...
            SetOpSize(1, NULL);
            if (DecodeAdr(pMemArg, MModGen | (IsSTC ? 0 : MModImm), &AdrResult)
                == ModImm) {
                BAsmCode[0] = 0xeb;
                BAsmCode[1] = CReg << 4;
                memcpy(BAsmCode + 2, AdrResult.Vals, AdrResult.Cnt);
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("G")) {
        ...
        if (DecodeAdr(pArg1, MModGen, &AdrResult1) != ModNone) {
            if (OpSize == eSymbolSizeUnknown) {
                ...
                if (DecodeAdr(pArg2, MModAbs20 | MModDisp20 | MModIReg32, &AdrResult2)
                    != ModNone) {
                    BAsmCode[0] = 0x74 + OpSize;
                    ...
                    memcpy(BAsmCode + 2 + AdrResult1.Cnt, AdrResult2.Vals,
                           AdrResult2.Cnt);
...
static void DecodeDir(Word Code) {
    ...
    if (OpSize > 0) {
        WrError(ErrNum_InvOpSize);
    } else if (ChkArgCnt(2, 2) && CheckFormat("G")) {
        OK   = True;
        Num1 = 0;
        if (!as_strcasecmp(ArgStr[2].str.p_str, "R0L"))
            ;
        else if (!as_strcasecmp(ArgStr[1].str.p_str, "R0L")) {
            Num1 = 1;
        } else {
            OK = False;
        }
        if (!OK) {
            ...
            if (DecodeAdr(&ArgStr[Num1 + 1], MModGen, &AdrResult) != ModNone) {
                if (((AdrResult.Mode & 14) == 4)
                    || ((AdrResult.Mode == 0) && (Num1 == 1))) {
                    WrError(ErrNum_InvAddrMode);
                } else {
                    BAsmCode[0] = 0x7c;
...
static void DecodePUSH_POP(Word IsPOP) {
    if (ChkArgCnt(1, 1) && CheckFormat("GS")) {
        tAdrResult AdrResult;

        if (DecodeAdr(&ArgStr[1], MModGen | (IsPOP ? 0 : MModImm), &AdrResult)
            != ModNone) {
            if (OpSize == eSymbolSizeUnknown) {
                WrError(ErrNum_UndefOpSizes);
            } else if (OpSize > 1) {
...
static void DecodeSTZ_STNZ(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("G")) {
        ...
        if (DecodeAdr(&ArgStr[2], MModGen, &DestAdrResult) != ModNone) {
            if (!IsShort(DestAdrResult.Mode, &SMode)) {
                WrError(ErrNum_InvAddrMode);
            } else {
                tAdrResult SrcAdrResult;
...
static void DecodeSTZX(Word Code) {
    ...
    if (ChkArgCnt(3, 3) && CheckFormat("G")) {
        ...
        if (DecodeAdr(&ArgStr[3], MModGen, &AdrResult3) != ModNone) {
            if (!IsShort(AdrResult3.Mode, &SMode)) {
                WrError(ErrNum_InvAddrMode);
            } else {
                tAdrResult AdrResult1;
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if ((DecodeAdr(&ArgStr[2], MModGen, &DestAdrResult) != ModNone)
            ...
            if (OpSize == eSymbolSizeUnknown) {
                ...
                if (FormatCode == 0) {
                    if (SrcAdrResult.Type == ModImm) {
                        if ((ImmVal(&SrcAdrResult) >= -8)
                            && (ImmVal(&SrcAdrResult) <= 7)) {
                            FormatCode = 2;
                        } else if (
                                (IsShort(DestAdrResult.Mode, &SMode)) && (OpSize == 0)) {
                            FormatCode = 3;
                        } else {
                            FormatCode = 1;
                        }
                    } else {
                        if ((OpSize == 0) && (IsShort(SrcAdrResult.Mode, &SMode))
                            && (DestAdrResult.Mode <= 1)
                            && ((SrcAdrResult.Mode > 1)
                                || (Odd(SrcAdrResult.Mode ^ DestAdrResult.Mode)))) {
                            FormatCode = 3;
                ...
                case 3:
                    if (OpSize != 0) {
                        ...
                        BAsmCode[0] = 0x20 + ((DestAdrResult.Mode & 1) << 2)
                                      + (SMode & 3); /* RMS 05: Just like #04 */
                        memcpy(BAsmCode + 1, SrcAdrResult.Vals, SrcAdrResult.Cnt);
                        CodeLen = 1 + SrcAdrResult.Cnt;
                    }
                    break;
...
static void DecodeCMP(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GQS")) {
        tAdrResult DestAdrResult, SrcAdrResult;
            ...
            if (OpSize == eSymbolSizeUnknown) {
                ...
                case 3:
                    if (OpSize != 0) {
                        ...
                        BAsmCode[0]
                                = 0x38 + ((DestAdrResult.Mode & 1) << 2)
                                  + (SMode & 3); /* RMS 04: destination reg is bit 2! */
                        memcpy(BAsmCode + 1, SrcAdrResult.Vals, SrcAdrResult.Cnt);
                        CodeLen = 1 + SrcAdrResult.Cnt;
                    }
                    break;
    ...
    if (ChkArgCnt(2, 2) && CheckFormat("GQS")) {
        ...
        if ((DecodeAdr(&ArgStr[2], MModGen, &DestAdrResult) != ModNone)
            ...
            if (OpSize == eSymbolSizeUnknown) {
                ...
                if (FormatCode == 0) {
                    if (SrcAdrResult.Type == ModImm) {
                        if ((ImmVal(&SrcAdrResult) >= -7)
                            && (ImmVal(&SrcAdrResult) <= 8)) {
                            FormatCode = 2;
                        } else if (
                                (IsShort(DestAdrResult.Mode, &SMode)) && (OpSize == 0)) {
                            FormatCode = 3;
                ...
                case 3:
                    if (OpSize != 0) {
                        ...
                        BAsmCode[0] = 0x28 + ((DestAdrResult.Mode & 1) << 2)
                                      + (SMode & 3); /* RMS 06: just like RMS 04 */
                        memcpy(BAsmCode + 1, SrcAdrResult.Vals, SrcAdrResult.Cnt);
                        CodeLen = 1 + SrcAdrResult.Cnt;
                    }
                    break;
        ...
        if ((DecodeAdr(&ArgStr[2], MModGen, &DestAdrResult) != ModNone)
            ...
            if (OpSize == eSymbolSizeUnknown) {
                ...
                WrError(ErrNum_InvOpSize);
                    ...
                    (*OpPart.str.p_str == 'M')
                    && ((DestAdrResult.Mode == 3) || (DestAdrResult.Mode == 5)
                        || (DestAdrResult.Mode - OpSize == 1))) {
                WrError(ErrNum_InvAddrMode);
            } else {
                CodeGen(pOrder->Code1, pOrder->Code2, pOrder->Code3, &SrcAdrResult,
                        &DestAdrResult);
            }
        }
    }
    ...
    if (ChkArgCnt(1, 1) && CheckFormat("G")) {
        ...
        if (DecodeAdr(&ArgStr[1], MModGen, &AdrResult) != ModNone) {
            if (OpSize == eSymbolSizeUnknown) {
                ...
                CodeLen     = 1;
            } else if (!IsShort(AdrResult.Mode, &SMode)) {
                WrError(ErrNum_InvAddrMode);
            } else if (OpSize != 0) {
                WrError(ErrNum_InvOpSize);
            } else {
...
static void DecodeEXTS(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && CheckFormat("G")) {
        ...
        if (DecodeAdr(&ArgStr[1], MModGen, &AdrResult) != ModNone) {
            ...
            if (OpSize == 0) {
                if ((AdrResult.Mode == 1)
                    || ((AdrResult.Mode >= 3) && (AdrResult.Mode <= 5))) {
                    WrError(ErrNum_InvAddrMode);
                } else {
                    BAsmCode[0] = 0x7c;
        ...
        && CheckFormat("GS")) /* RMS 01: The format codes are G and S, not G and Q */
    {
        ...
        if ((DecodeAdr(&ArgStr[2], MModGen, &DestAdrResult) != ModNone)
            ...
            if (OpSize == eSymbolSizeUnknown) {
                ...
                switch (FormatCode) {
                case 1:
                    CodeGen(0x90 + (IsOR << 3), 0x76, 0x20 + (IsOR << 4), &SrcAdrResult,
                            &DestAdrResult);
                    break;
                case 2:
                    if (OpSize != 0) {
                        ...
                        BAsmCode[0] = 0x10 + (IsOR << 3) + ((DestAdrResult.Mode & 1) << 2)
                                      + (SMode & 3);
                        memcpy(BAsmCode + 1, SrcAdrResult.Vals, SrcAdrResult.Cnt);
                        CodeLen = 1 + SrcAdrResult.Cnt;
                    }
        ...
        if (DecodeAdr(&ArgStr[2], MModGen, &DestAdrResult) != ModNone) {
            if (OpSize == eSymbolSizeUnknown) {
                ...
                if (DecodeAdr(&ArgStr[1], MModGen | MModImm, &SrcAdrResult) == ModGen) {
                    if (SrcAdrResult.Mode != 3) {
                        WrError(ErrNum_InvAddrMode);
                    } else if (DestAdrResult.Mode + 2 * OpSize2 == 3) {
                        WrError(ErrNum_InvAddrMode);
                    } else {
        ...
        if (DecodeAdr(&ArgStr[2], MModGen | MModReg32, &DestAdrResult) != ModNone) {
            if (OpSize == eSymbolSizeUnknown) {
                WrError(ErrNum_UndefOpSizes);
                    ...
                    (OpSize > 2) || ((OpSize == 2) && (DestAdrResult.Type == ModGen))) {
                WrError(ErrNum_InvOpSize);
            } else {
                tAdrResult SrcAdrResult;
                ...
                if (DecodeAdr(&ArgStr[1], MModImm | MModGen, &SrcAdrResult) == ModGen) {
                    ...
                    if (Num1 == 0) {
                        ...
                        if (OpSize2 == 2) {
                            BAsmCode[0] = 0xeb;
                            BAsmCode[1] = 0x80 | (DestAdrResult.Mode << 4) | (IsSHA << 5)
                                          | (Num1 & 15);
                        } else {
...
static void DecodeFCLR_FSET(Word Code) {
    if (!ChkArgCnt(1, 1))
        ;
    else if (strlen(ArgStr[1].str.p_str) != 1) {
        WrError(ErrNum_InvAddrMode);
    } else {
        char const* p = strchr(Flags, as_toupper(*ArgStr[1].str.p_str));
        if (!p) {
...
static void DecodeJMP(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        */
        ...
        if (OpSize == eSymbolSizeUnknown) {   /* RMS 13: The other part of the */
        ...
        */
        ...
        switch (OpSize) {
        case 4:
            if (((Diff < 1) || (Diff > 9)) && !mSymbolQuestionable(Flags)) {
                WrError(ErrNum_JmpDistTooBig);
            } else {
                if (Diff
                    == 1) { /* RMS 13: To avoid infinite loop phase errors... Turn a */
                    BAsmCode[0] = 0x04; /* JMP (or JMP.S) to following instruction into a
                                           NOP */
                } else {
            ...
            break;
        case 0:
            if (((Diff < -127) || (Diff > 128)) && !mSymbolQuestionable(Flags)) {
                WrError(ErrNum_JmpDistTooBig);
            } else {
                BAsmCode[0] = 0xfe;
            ...
            break;
        case 1:
            if (((Diff < -32767) || (Diff > 32768)) && !mSymbolQuestionable(Flags)) {
                WrError(ErrNum_JmpDistTooBig);
            } else {
                BAsmCode[0] = 0xf4;
                Diff--;
...
static void DecodeJ(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        Boolean      OK;
        tSymbolFlags Flags;
        LongInt AdrLong = EvalStrIntExpressionWithFlags(&ArgStr[1], UInt20, &OK, &Flags)
                          - (EProgCounter() + Num1);

        if (OK) {
            if (!mSymbolQuestionable(Flags) && ((AdrLong > 127) || (AdrLong < -128))) {
                WrError(ErrNum_JmpDistTooBig);
            } else if (Code >= 8) {
                BAsmCode[0] = 0x7d;
...
static void AddString(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeString);
}

static void AddGen1(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeGen1);
}

static void AddGen2(char const* NName, Byte NCode1, Byte NCode2, Byte NCode3) {
    if (InstrZ >= Gen2OrderCnt) {
        exit(255);
    }
...
static void AddDiv(char const* NName, Byte NCode1, Byte NCode2, Byte NCode3) {
    if (InstrZ >= DivOrderCnt) {
        exit(255);
    }
...
static void AddCondition(char const* BMName, char const* JName, Word NCode) {
    AddInstTable(InstTable, BMName, NCode, DecodeBM);
    AddInstTable(InstTable, JName, NCode, DecodeJ);
}

static void AddBCD(char const* NName) {
    AddInstTable(InstTable, NName, InstrZ++, DecodeBCD);
}

static void AddDir(char const* NName, Word NCode) {
...
static void InitFields(void) {
    ...
    AddCondition("BMGEU", "JGEU", 0);
    AddCondition("BMC", "JC", 0);
    AddCondition("BMGTU", "JGTU", 1);
    AddCondition("BMEQ", "JEQ", 2);
    AddCondition("BMZ", "JZ", 2);
    AddCondition("BMN", "JN", 3);
    AddCondition("BMLTU", "JLTU", 4);
    AddCondition("BMNC", "JNC", 4);
    AddCondition("BMLEU", "JLEU", 5);
    AddCondition("BMNE", "JNE", 6);
    AddCondition("BMNZ", "JNZ", 6);
    AddCondition("BMPZ", "JPZ", 7);
    AddCondition("BMLE", "JLE", 8);
    AddCondition("BMO", "JO", 9);
    AddCondition("BMGE", "JGE", 10);
    AddCondition("BMGT", "JGT", 12);
    AddCondition("BMNO", "JNO", 13);
    AddCondition("BMLT", "JLT", 14);
...
static Boolean DecodeAttrPart_M16C(void) {
    ...
    case 'L':
        ...
        break;
    case 'Q':
        AttrPartOpSize = eSymbolSize64Bit;
        break;
    case 'S':
        AttrPartOpSize = eSymbolSize80Bit;
        break;
    case 'D':
        AttrPartOpSize = eSymbolSizeFloat32Bit;
        break;
    case 'X':
        AttrPartOpSize = eSymbolSizeFloat64Bit;
        break;
    case 'A':
        AttrPartOpSize = eSymbolSizeFloat96Bit;
        break;
    default:
