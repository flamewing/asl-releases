...
\vspace{1cm}\par
{\huge Macro Assembler AS V1.41r7}\\
\rule{9.5cm}{0.3mm}\\
\vspace{2mm}\par
{\huge User's Manual}
...
{\em IBM, PPC403GA, OS/2, and PowerPC} are trademarks of IBM Corporation.

{\em ColdFire} is a registered trademark of Motorola Inc. .
...
Digital Unix, Linux, and OS/2 operating systems running on AMD 5x86/K6 and
...
\tty{EXE} and \tty{OVR} files can rightaway be deleted and replaced by the
OS/2 counterparts.  In contrast to DOS, the setting of the \tty{ASCMD}
variable has to be done from \tty{CONFIG.SYS} (the position of the
statement is however again arbitrary).
...
In contrast to all other versions \marginpar{{\em UNIX}}, the language of
the C version is not compiled into the program.  The fitting set of
messages is instead loaded from a set of message files at runtime.  AS
searches the following directories for these files:
\begin{enumerate}
\item{the current directory}
\item{the directory AS was loaded from (only for DOS)}
\item{the directory specified via the {\tt AS\_MSGPATH} environment variable
      respectively the {\tt PATH} list if the variable does not exist}
\item{in the diorectory set via the {\tt LIBDIR} variable at compile time}
\end{enumerate}
If no file is found or the found message file is incompatible with the
version of AS, program execution is terminated.

The command line parameters can roughly be divided into two categories:
switches and file specifications.  Parameters of these two categories may
be arbitrarily mixed in the command line.  The assembler evaluates at
first all parameters and then assembles the specified files.  From this
follow two things:
\begin{itemize}
...
\item{\tty{g}: This switch instructs AS to create an additional file that
      ...
      AS-specific \tty{MAP} format and the \tty{Atmel} format used by the
      AVR tools. The information stored in the MAP format is comprised of
      a symbol table and a table describing the assignment of source lines
      to machine addresses.  A more detailed description of the MAP format
      can be found in section 5.2.  The file's extension is \tty{MAP} resp.
      \tty{OBJ}, depending on the chosen format.  If no explicit format
      specification is done, the MAP format is chosen.}
...
%%-----------
\begin{quote}
z) 80C166 $\rightarrow$ 80C167,80C165,80C163
\end{quote}
...
%%-----------
\begin{quote}
au) 370C010, 370C020, 370C030, 370C040 and 370C050
\end{quote}
...
{\em\begin{tabbing}
 valid for: \= 680x0, FPU also for 8086..V35, SUPMODE also for TLCS-900, \\
            \> SH7000, 29K, XA, PowerPC, M*Core, and TMS9900
\end{tabbing}}
...
\item{\tty{XDATA}: data in externally connected RAM or 
      X-addressing space of the DSP56xxx;}
...
disassemblers or debuggers.  AS can generate debug files in one of
two formats: On the one hand, the object format used by the AVR tools
from Atmel, and on the other hand an own format.  The first one is   
described in detail in [AVRObj], which is why the following
...
group will always be of the same type.  The Intel, MOS and Tektronix
formats are limited to 16 bit addresses, the 16-bit Intel format reaches
4 bits further. 
