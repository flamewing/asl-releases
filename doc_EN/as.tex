...
\item{Rockwell 6502, 65(S)C02, WDC W65C02S, Rockwell 65C19, and
      Hudson HuC6280}
\item{CMD 65816}
...
REGAVROLD.INC     & register and bit addresses AVR family (outdated)\\
...
STDDEF62.INC      & register addresses and macros ST6 \\
...
{\em valid for: PowerPC, M-Core, 4004/4040, MCS-48/(2)51, 80C16x, AVR, XS1, Z8}
...
{\em valid for: AVR, M*Core, ST9, 80C16x, KCPSM ({\tt NAMEREG} valid only
 for KCPSM(3)), LatticeMico8, MSP430(X)}
...
segment and on the processor type (tables \ref{TabORG1} to \ref{TabORG5}).
...
the {\em execution address} of the code and do not yeld the
...
\hline
\tin{Target} & \tin{CODE} & \tin{DATA} & \tin{IDATA} & \tin{XDATA} & \tin{YDATA} & \tin{BIT-} & \tin{IO} & \tin{REG} & \tin{ROM-} \\
             &            &            &             &             &             & \tin{DATA} &          &           & \tin{DATA} \\
\hline
\hline
...
\hline
\multicolumn{10}{|l|}{$^{*}$ Initial value 80h.} \\
...
\hline
\multicolumn{10}{|l|}{$^{-}$ Initial value 400h.}\\
\hline
\end{tabular}\end{center}
...
\hline
\multicolumn{10}{|l|}{$^{*}$ initial value 800h resp. 0C00h} \\
\hline
\end{tabular}\end{center}
...
\hline
\hline
\input{../doc_DE/taborg5.tex}
\hline
\multicolumn{10}{|l|}{$^{*}$ area for program code is limited to 1 MByte} \\
\hline
\end{tabular}\end{center}
\caption{Address Ranges for \tty{ORG} --- Part 5\label{TabORG5}}
\end{table*}
\normalsize
...
%%-----------
\begin{quote}
u) 8021, 8022, 8039, (MSM)80C39, 8048, (MSM)80C48, 8041, 8042, 80C382
\end{quote}
...
possible to transfer the CMOS-versions with the \tty{IDL} resp.
...
The code address space of 8041, 8042, 8021, and 8022 is not externally
extendable, and so AS limits the code segment of these processors to
one resp. two kilobytes.  The (SAB)80C382 is a variant especially
...
%%-----------
\begin{quote}
ac) AT90S1200, AT90S2313, AT90S4414, AT90S4433, AT90S4434, AT90S8515,
    AT90S8535, ATTINY13, ATTINY26, ATTINY2313, ATTINY2313A, ATTINY4313,
    ...
    ATMEGA3250, ATMEGA328, ATMEGA329, ATMEGA3290, ATMEGA406, ATMEGA64,
    ATMEGA640, ATMEGA644, ATMEGA644RFR2, ATMEGA645, ATMEGA6450, ATMEGA649,
    ATMEGA6490, ATMEGA103, ATMEGA128, ATMEGA1280, ATMEGA1281, ATMEGA1284,
...
%%-----------
\begin{quote}
ae) 80C166 $\rightarrow$ 80C167,80C165,80C163
\end{quote}
...
%%-----------
\begin{quote}
bd) 370C010, 370C020, 370C030, 370C040 and 370C050
\end{quote}
...
\begin{quote}
\begin{tabbing}
bu) \= SYM53C810, SYM53C860, SYM53C815, SYM53C825, \\
    \> SYM53C875, SYM53C895
\end{tabbing}
\end{quote}
...
%%-----------
\begin{quote}
bx) MSM5840, MSM5842, MSM58421, MSM58422, MSM5847
\end{quote}
...
{\em valid for: 680x0, 68xx, M*Core, XA, H8, SH7000, MSP430(X), TMS9900, ST7}
...
\item{the flags that define which part of expanded macros shall be
      printed in the assembly listing (set by \tty{MACEXP}).}
...
related to the Intel 808/8085 - thi is also true for the
...
time.  The limits of this operation mode can be looked up in
detail in section \ref{8080Spec}.
...
{\em valid for: PIC, 320xx, AVR, MELPS-4500, HMCS400, 4004/4040,
...
in the listing when it is used.  This pseudo instruction controls
whether the code expanded form a macro is printed in the listing,
and if yes, which parts of it.  The simples usage is
\begin{verbatim}
        macexp  off
\end{verbatim}
which completely turns off printing of the expanded code.
Consequently, by issuing a
\begin{verbatim}
        macexp  on
\end{verbatim}
the complete listing is turned on again.  This is also the default.

Aside from this global switch, \tty{MACEXP} has means to disable
listing of parts of the macro's contents.  The code contained in
a macro gets divided into three classes:
\begin{enumerate}
\item{Instructions for conditional assembly and instructions that
      are not executed because of conditional assembly can be
      removed from or added to the listing via the arguments
      \tty{IF} resp. \tty{NOIF}.}
\item{Macro definitions (including \tty{REPT}, \tty{WHILE} and
      \tty{IRP(C)}) can be removed from or added to the listing
      via the arguments \tty{MACRO} resp. \tty{NOMACRO}.}
\item{Anything else is the 'rest'...}
\end{enumerate}
The individual switcheswork on the current setting.  For
instance, if printing of everything is initially enabled, a
\begin{verbatim}
        MACEXP  noif,nomacro
\end{verbatim}
has the effect that only the contents of a macro are listed that
are neither part of a macro defintion nor were excluded via
...
macro contains internal flags that rules which parts of its body
...
expanded.  \tty{MACEXP} serves as a default for the macro's internal
flags when it is defined, and it may be overridden by the
\tty{(NO)EXPAND}, \tty{(NO)EXPIF}, and \tty{(NO)EXPMAC} directives.
...
\begin{verbatim}
       Z80SYNTAX ON
\end{verbatim}
makes ii possible to write the vast majority of 8080/8085
instructions in 'Z80 style', i.e. with less mnemonics but with
operands that are easier to understand.  The Z80 syntax is not
allowed for the following insstuctions, because they conflict
...
RP register defines which memory block is used as work registers:
bits 4 to 7 of RP define the 'offset' that is added to every 4-bit
work register address to get a complete 8-bit address.
...
dollar sign (\tty{\$}) or '0x'.  This stands for the very first resp.
last address that has been used in the code file.  So, if you want to
be sure that always the whole program is stored in the hex file, set
the address filter
\begin{verbatim}
 -r $-$
\end{verbatim}
...
\errentry{1504}{Z80 syntax was not allowed}
               {error}
               {This instrction is only allowed if Z80 syntax
