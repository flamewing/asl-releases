...
static void DecodeAdr(tStrComp const* pArg, Word Mask) {
    static char const RegNames[ModIHL + 1][4] = {"A", "L", "H", "HL", "@HL"};
...
static Boolean DualOp(char const* s1, char const* s2) {
    return (((!as_strcasecmp(ArgStr[1].str.p_str, s1))
             && (!as_strcasecmp(ArgStr[2].str.p_str, s2)))
            || ((!as_strcasecmp(ArgStr[2].str.p_str, s1))
                && (!as_strcasecmp(ArgStr[1].str.p_str, s2))));
}
...
static void DecodeLDL(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
            ...
            || (as_strcasecmp(ArgStr[2].str.p_str, "@DC"))) {
        WrError(ErrNum_InvAddrMode);
    } else {
        CodeLen     = 1;
        BAsmCode[0] = 0x33;
...
static void DecodeMOV(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
            ...
            (!as_strcasecmp(ArgStr[1].str.p_str, "A"))
            && (!as_strcasecmp(ArgStr[2].str.p_str, "DMB"))) {
        CodeLen     = 3;
        BAsmCode[0] = 0x03;
        BAsmCode[1] = 0x3a;
        BAsmCode[2] = 0xa9;
        ChkCPU(M_CPU470AC00);
            ...
            (!as_strcasecmp(ArgStr[1].str.p_str, "DMB"))
            && (!as_strcasecmp(ArgStr[2].str.p_str, "A"))) {
        CodeLen     = 3;
        BAsmCode[0] = 0x03;
        BAsmCode[1] = 0x3a;
        BAsmCode[2] = 0x29;
        ChkCPU(M_CPU470AC00);
            ...
            (!as_strcasecmp(ArgStr[1].str.p_str, "A"))
            && (!as_strcasecmp(ArgStr[2].str.p_str, "SPW13"))) {
        CodeLen     = 2;
        BAsmCode[0] = 0x3a;
        BAsmCode[1] = 0x84;
        ChkCPU(M_CPU470AC00);
    } else if (
            (!as_strcasecmp(ArgStr[1].str.p_str, "STK13"))
            && (!as_strcasecmp(ArgStr[2].str.p_str, "A"))) {
        CodeLen     = 2;
        BAsmCode[0] = 0x3a;
        BAsmCode[1] = 0x04;
...
static void DecodeXCH(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if ((as_strcasecmp(pArg1->str.p_str, "A"))
            && (as_strcasecmp(pArg1->str.p_str, "HL"))) {
            pArg1 = &ArgStr[2];
            pArg2 = &ArgStr[1];
        }
        if ((as_strcasecmp(pArg1->str.p_str, "A"))
            ...
            if (AdrType != ModNone) {
                if ((!as_strcasecmp(pArg1->str.p_str, "HL")) && (AdrVal & 3)) {
                    WrError(ErrNum_NotAligned);
                } else {
                    CodeLen = 2;
                    BAsmCode[0]
                            = 0x29 + (0x14 * Ord(!as_strcasecmp(pArg1->str.p_str, "A")));
                    BAsmCode[1] = AdrVal;
                }
            }
        }
...
static void DecodeADDC(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
            ...
            (as_strcasecmp(ArgStr[1].str.p_str, "A"))
            || (as_strcasecmp(ArgStr[2].str.p_str, "@HL"))) {
        WrError(ErrNum_InvAddrMode);
    } else {
        CodeLen     = 1;
        BAsmCode[0] = 0x15;
    ...
    if (ArgCnt == 1) {
        if (!as_strcasecmp(ArgStr[1].str.p_str, "@L")) {
            ...
            if (Code == 2) {
                ...
                (!as_strcasecmp(ArgStr[1].str.p_str, "DMB"))
                || (!as_strcasecmp(ArgStr[1].str.p_str, "DMB0"))) {
            CodeLen     = 2;
            BAsmCode[0] = 0x3b;
            BAsmCode[1] = 0x39 + (Code << 6);
            ChkCPU(as_strcasecmp(ArgStr[1].str.p_str, "DMB0") ? M_CPU470C00
                                                              : M_CPU470AC00);
...
static void DecodeEICLR_DICLR(Word Code) {
    if (!ChkArgCnt(2, 2))
        ;
    else if (as_strcasecmp(ArgStr[1].str.p_str, "IL")) {
        WrError(ErrNum_InvAddrMode);
    } else {
        Boolean OK;
...
static void DecodeBSS(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (EvalResult.OK
            && ChkSamePage(AdrWord, EProgCounter() + 1, 6, EvalResult.Flags)) {
...
static void DecodeBS(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (EvalResult.OK
            && ChkSamePage(AdrWord, EProgCounter() + 2, 12, EvalResult.Flags)) {
            ChkSpace(SegCode, EvalResult.AddrSpaceMask);
            CodeLen     = 2;
...
static void DecodeCALLS(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (OK) {
            ...
            if (!mFirstPassUnknown(Flags) && ((AdrWord & 0xff87) != 6)) {
