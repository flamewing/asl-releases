...
static Word EvalARExpression(tStrComp const* pArg, Boolean* OK) {
    *OK = True;

    if ((as_toupper(pArg->str.p_str[0]) == 'A') && (as_toupper(pArg->str.p_str[1]) == 'R')
...
static Boolean DecodeAdr(tStrComp const* pArg, int MinArgCnt, int aux, Boolean Must1) {
    Word        h;
...
static Word DecodeCond(int argp) {
    ...
    while (argp <= ArgCnt) {
        for (pCondition = Conditions;
             pCondition->Name && as_strcasecmp(pCondition->Name, ArgStr[argp].str.p_str);
             pCondition++)
            ;
...
static Word DecodeShift(tStrComp const* pArg, Boolean* OK) {
    Word         Shift;
    tSymbolFlags Flags;
...
static void DecodeCmdAdr(Word Index) {
    FixedOrder const* pOrder = AdrOrders + Index;

    if (ChkArgCnt(1, 2) && ChkMinCPU(pOrder->MinCPU)
...
static void DecodeCmdJmp(Word Index) {
    JmpOrder const* pOrder = JmpOrders + Index;

    if (ChkMinCPU(pOrder->MinCPU) && ChkArgCnt(1, pOrder->Cond ? ArgCntMax : 3)) {
        Boolean OK;
...
static void DecodeCmdPlu(Word Index) {
    Boolean           OK;
    FixedOrder const* pOrder = PluOrders + Index;
    ...
    } else if (strlen(OpPart.str.p_str) == 4) {
            ...
            if (DecodeAdr(&ArgStr[1], 1, 3, False)) {
                ...
                if (OK) {
                    ...
                    if (Shift == 16) {
                        ...
                        WAsmCode[0] = ((Index << 12) | 0x2000) | ((Shift & 0xf) << 8)
                                      | AdrMode;
                    }
                }
            }
...
static void DecodeBLPD(Word Index) {
    ...
    if (!ChkArgCnt(2, 3))
        ...
        if (ChkMinCPU(CPU32050) && DecodeAdr(&ArgStr[2], 2, 3, False)) {
            CodeLen     = 1;
...
static void DecodeIO(Word Index) {
    if (ChkArgCnt(2, 3) && DecodeAdr(&ArgStr[1], 2, 3, False)) {
        tEvalResult EvalResult;
...
static void DecodeLSST(Word Index) {
    ...
    if (!ChkArgCnt(2, 3))
        ...
        WrError(ErrNum_OnlyImmAddr); /* invalid instruction */
    } else {
...
static void DecodeMPY(Word Index) {
    ...
    if (*ArgStr[1].str.p_str == '#') {
        ...
        if (ChkArgCnt(1, 2) && DecodeAdr(&ArgStr[1], 1, 2, Index)) {
            CodeLen     = 1;
...
static void DecodeNORM(Word Index) {
    ...
    if (ChkArgCnt(1, 2) && DecodeAdr(&ArgStr[1], 1, 2, False)) {
        if (AdrMode < 0x80) {
            WrError(ErrNum_InvAddrMode);
        } else {
            CodeLen     = 1;
...
static void DecodeRPT(Word Index) {
    ...
    if (*ArgStr[1].str.p_str == '#') {
        if (ChkArgCnt(1, 1)) {
            Imm = EvalStrIntExpressionOffs(
                    &ArgStr[1], 1, (MomCPU >= CPU32050) ? UInt16 : UInt8, &OK);
            if (OK) {
                if (Imm > 255) {
                    CodeLen     = 2;
                    WAsmCode[0] = 0xbec4;
...
static void DecodeBSAR(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU32050)) {
...
static void DecodeLSAMM(Word Index) {
    if (ChkArgCnt(1, 2) && ChkMinCPU(CPU32050) && DecodeAdr(&ArgStr[1], 1, 2, True)) {
        CodeLen     = 1;
...
static void DecodeXC(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU32050)) {
        Mode = EvalStrIntExpressionWithFlags(&ArgStr[1], UInt2, &OK, &Flags);
        if (OK) {
            if ((Mode != 1) && (Mode != 2) && !mFirstPassUnknown(Flags)) {
                WrError(ErrNum_UnderRange);
            } else {
                CodeLen     = 1;
...
static void AddFixed(char const* NName, CPUVar MinCPU, Word NCode) {
...
static void AddAdr(char const* NName, CPUVar MinCPU, Word NCode) {
    if (InstrZ >= AdrOrderCnt) {
        exit(255);
    }
...
static void AddJmp(char const* NName, CPUVar MinCPU, Word NCode, Boolean NCond) {
    if (InstrZ >= JmpOrderCnt) {
        exit(255);
    }
...
static void AddPlu(char const* NName, CPUVar MinCPU, Word NCode) {
    if (InstrZ >= PluOrderCnt) {
        exit(255);
    }
...
static void AddAdrMode(char const* NName, CPUVar MinCPU, Word NMode) {
    if (InstrZ >= AdrModeCnt) {
        exit(255);
    }
...
static void AddCond(
        char const* NName, CPUVar MinCPU, Word NCodeAND, Word NCodeOR, Byte NIsZL,
...
static void AddBit(char const* NName, CPUVar MinCPU, Word NCode) {
    if (InstrZ >= BitCnt) {
        exit(255);
    }
...
static void InitFields(void) {
    ...
    InstrZ      = 0;
    AddFixed("ABS", CPU320203, 0xbe00);
    AddFixed("ADCB", CPU32050, 0xbe11);
    AddFixed("ADDB", CPU32050, 0xbe10);
    AddFixed("ANDB", CPU32050, 0xbe12);
    AddFixed("CMPL", CPU320203, 0xbe01);
    AddFixed("CRGT", CPU32050, 0xbe1b);
    AddFixed("CRLT", CPU32050, 0xbe1c);
    AddFixed("EXAR", CPU32050, 0xbe1d);
    AddFixed("LACB", CPU32050, 0xbe1f);
    AddFixed("NEG", CPU320203, 0xbe02);
    AddFixed("ORB", CPU32050, 0xbe13);
    AddFixed("ROL", CPU320203, 0xbe0c);
    AddFixed("ROLB", CPU32050, 0xbe14);
    AddFixed("ROR", CPU320203, 0xbe0d);
    AddFixed("RORB", CPU32050, 0xbe15);
    AddFixed("SACB", CPU32050, 0xbe1e);
    AddFixed("SATH", CPU32050, 0xbe5a);
    AddFixed("SATL", CPU32050, 0xbe5b);
    AddFixed("SBB", CPU32050, 0xbe18);
    AddFixed("SBBB", CPU32050, 0xbe19);
    AddFixed("SFL", CPU320203, 0xbe09);
    AddFixed("SFLB", CPU32050, 0xbe16);
    AddFixed("SFR", CPU320203, 0xbe0a);
    AddFixed("SFRB", CPU32050, 0xbe17);
    AddFixed("XORB", CPU32050, 0xbe1a);
    AddFixed("ZAP", CPU32050, 0xbe59);
    AddFixed("APAC", CPU320203, 0xbe04);
    AddFixed("PAC", CPU320203, 0xbe03);
    AddFixed("SPAC", CPU320203, 0xbe05);
    AddFixed("ZPR", CPU32050, 0xbe58);
    AddFixed("BACC", CPU320203, 0xbe20);
    AddFixed("BACCD", CPU32050, 0xbe21);
    AddFixed("CALA", CPU320203, 0xbe30);
    AddFixed("CALAD", CPU32050, 0xbe3d);
    AddFixed("NMI", CPU320203, 0xbe52);
    AddFixed("RET", CPU320203, 0xef00);
    AddFixed("RETD", CPU32050, 0xff00);
    AddFixed("RETE", CPU32050, 0xbe3a);
    AddFixed("RETI", CPU32050, 0xbe38);
    AddFixed("TRAP", CPU320203, 0xbe51);
    AddFixed("IDLE", CPU320203, 0xbe22);
    AddFixed("NOP", CPU320203, 0x8b00);
    AddFixed("POP", CPU320203, 0xbe32);
    AddFixed("PUSH", CPU320203, 0xbe3c);
    AddFixed("IDLE2", CPU32050, 0xbe23);

    AdrOrders = (FixedOrder*)malloc(sizeof(FixedOrder) * AdrOrderCnt);
    InstrZ    = 0;
    AddAdr("ADDC", CPU320203, 0x6000);
    AddAdr("ADDS", CPU320203, 0x6200);
    AddAdr("ADDT", CPU320203, 0x6300);
    AddAdr("LACT", CPU320203, 0x6b00);
    AddAdr("SUBB", CPU320203, 0x6400);
    AddAdr("SUBC", CPU320203, 0x0a00);
    AddAdr("SUBS", CPU320203, 0x6600);
    AddAdr("SUBT", CPU320203, 0x6700);
    AddAdr("ZALR", CPU320203, 0x6800);
    AddAdr("MAR", CPU320203, 0x8b00);
    AddAdr("LPH", CPU320203, 0x7500);
    AddAdr("LT", CPU320203, 0x7300);
    AddAdr("LTA", CPU320203, 0x7000);
    AddAdr("LTD", CPU320203, 0x7200);
    AddAdr("LTP", CPU320203, 0x7100);
    AddAdr("LTS", CPU320203, 0x7400);
    AddAdr("MADD", CPU32050, 0xab00);
    AddAdr("MADS", CPU32050, 0xaa00);
    AddAdr("MPYA", CPU320203, 0x5000);
    AddAdr("MPYS", CPU320203, 0x5100);
    AddAdr("MPYU", CPU320203, 0x5500);
    AddAdr("SPH", CPU320203, 0x8d00);
    AddAdr("SPL", CPU320203, 0x8c00);
    AddAdr("SQRA", CPU320203, 0x5200);
    AddAdr("SQRS", CPU320203, 0x5300);
    AddAdr("BLDP", CPU32050, 0x5700);
    AddAdr("DMOV", CPU320203, 0x7700);
    AddAdr("TBLR", CPU320203, 0xa600);
    AddAdr("TBLW", CPU320203, 0xa700);
    AddAdr("BITT", CPU320203, 0x6f00);
    AddAdr("POPD", CPU320203, 0x8a00);
    AddAdr("PSHD", CPU320203, 0x7600);

    JmpOrders = (JmpOrder*)malloc(sizeof(JmpOrder) * JmpOrderCnt);
    InstrZ    = 0;
    ...
    PluOrders = (FixedOrder*)malloc(sizeof(FixedOrder) * PluOrderCnt);
    InstrZ    = 0;
    AddPlu("APL", CPU32050, 0x5a00);
    AddPlu("CPL", CPU32050, 0x5b00);
    AddPlu("OPL", CPU32050, 0x5900);
    AddPlu("SPLK", CPU320203, 0xaa00);
    AddPlu("XPL", CPU32050, 0x5800);

    AdrModes = (tAdrMode*)malloc(sizeof(tAdrMode) * AdrModeCnt);
    InstrZ   = 0;
    AddAdrMode("*-", CPU320203, 0x90);
    AddAdrMode("*+", CPU320203, 0xa0);
    AddAdrMode("*BR0-", CPU320203, 0xc0);
    AddAdrMode("*0-", CPU320203, 0xd0);
    AddAdrMode("*AR0-", CPU32050, 0xd0);
    AddAdrMode("*0+", CPU320203, 0xe0);
    AddAdrMode("*AR0+", CPU32050, 0xe0);
    AddAdrMode("*BR0+", CPU320203, 0xf0);
    AddAdrMode("*", CPU320203, 0x80);
    AddAdrMode(NULL, CPU32050, 0);
    ...
    BitTable = (tBitTable*)malloc(sizeof(tBitTable) * BitCnt);
    InstrZ   = 0;
    AddBit("OVM", CPU320203, 0xbe42);
    AddBit("SXM", CPU320203, 0xbe46);
    AddBit("HM", CPU32050, 0xbe48);
    AddBit("TC", CPU320203, 0xbe4a);
    AddBit("C", CPU320203, 0xbe4e);
    AddBit("XF", CPU320203, 0xbe4c);
    AddBit("CNF", CPU320203, 0xbe44);
    AddBit("INTM", CPU320203, 0xbe40);
