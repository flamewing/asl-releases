...
static void DecodeAdr(tStrComp const* pArg, unsigned Mask) {
    Boolean OK;
    Word    Address;
    ...
    if (!as_strcasecmp(pArg->str.p_str, "A")) {
        ...
        AdrMode = ModPS;
            ...
            (strlen(pArg->str.p_str) == 2) && (as_toupper(*pArg->str.p_str) == 'R')
            && (pArg->str.p_str[1] >= '0') && (pArg->str.p_str[1] <= '7')) {
        AdrMode = ModReg;
...
static Boolean DecodeBitAdr(tStrComp const* pArg, Byte* Adr, Byte* Bit) {
    char*    sep;
...
static void DecodeRel(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        Adr = EvalStrIntExpressionWithFlags(&ArgStr[1], UInt16, &OK, &Flags)
              - (EProgCounter() + 2);
        if (OK) {
            if (((Adr < -128) || (Adr > 127)) && !mSymbolQuestionable(Flags)) {
...
static void DecodeMOV(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        DecodeAdr(
                &ArgStr[1],
                MModAcc | MModDir | MModIIX | MModIEP | MModExt | MModReg | MModIA);
        switch (AdrMode) {
        case ModAcc:
            DecodeAdr(
                    &ArgStr[2],
                    MModDir | MModIIX | MModIEP | MModIA | MModReg | MModImm | MModExt);
            switch (AdrMode) {
            case ModDir:
            case ModIIX:
            case ModIEP:
...
static void DecodeMOVW(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                &ArgStr[1], MModAcc | MModDir | MModIIX | MModExt | MModIEP | MModReg16
                                    | MModIA | MModPS);
        switch (AdrMode) {
        case ModAcc:
            DecodeAdr(
                    &ArgStr[2], MModImm | MModDir | MModIEP | MModIIX | MModExt | MModIA
                                        | MModReg16 | MModPS | MModPC);
            switch (AdrMode) {
            case ModImm:
                BAsmCode[0] = 0xe4;
                memcpy(BAsmCode + 1, AdrVals, AdrCnt);
...
static void DecodeBit(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if (!DecodeBitAdr(&ArgStr[1], &Adr, &Bit)) {
        WrError(ErrNum_InvAddrMode);
    } else {
...
static void DecodeBitBr(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if (!DecodeBitAdr(&ArgStr[1], &BAdr, &Bit)) {
        WrError(ErrNum_InvAddrMode);
    } else {
...
static void AddALU(char const* NName, Byte NCode) {
