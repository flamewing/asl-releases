...
typedef struct {
    ...
    tCtReg const* pCtRegs[MAX_CTREGS_GROUPS];
} tCPUProps;
...
static Byte const FSizeCodes[10] = {6, 4, 0, 7, 0, 1, 5, 2, 0, 3};
...
static Boolean CheckColdSize(void) {
    if ((OpSize > eSymbolSize32Bit)
        || ((pCurrCPUProps->Family == eColdfire) && (OpSize < eSymbolSize32Bit))) {
...
static Boolean DecodeRegCore(char const* pArg, Word* pResult) {
...
static Boolean DecodeFPRegCore(char const* pArg, Word* pResult) {
...
static void DissectReg_68K(
    ...
    if (InpSize == NativeFloatSize) {
        ...
        default:
            as_snprintf(
                    pDest, DestSize, "%c%u", Value & 8 ? 'A' : 'D',
                    (unsigned)(Value & 7));
        }
    } else {
...
static Boolean ACheckFamily(
        unsigned FamilyMask, tStrComp const* pAdrComp, tAdrResult* pResult) {
    if (CheckFamilyCore(FamilyMask)) {
        return True;
    }
...
static tRegEvalResult DecodeReg(tStrComp const* pArg, Word* pResult, Boolean MustBeReg) {
    tRegDescr      RegDescr;
    ...
    RegEvalResult = EvalStrRegExpressionAsOperand(
            pArg, &RegDescr, &EvalResult, eSymbolSize32Bit, MustBeReg);
...
static tRegEvalResult DecodeFPReg(
        tStrComp const* pArg, Word* pResult, Boolean MustBeReg) {
    tRegDescr      RegDescr;
    tEvalResult    EvalResult;
    tRegEvalResult RegEvalResult;
    ...
    RegEvalResult = EvalStrRegExpressionAsOperand(
            pArg, &RegDescr, &EvalResult, NativeFloatSize, MustBeReg);
...
static tRegEvalResult DecodeRegOrFPReg(
        tStrComp const* pArg, Word* pResult, tSymbolSize* pSize, Boolean MustBeReg) {
    tRegDescr      RegDescr;
    tEvalResult    EvalResult;
    tRegEvalResult RegEvalResult;
...
static Boolean DecodeRegPair(tStrComp* pArg, Word* Erg1, Word* Erg2) {
    ...
    return (DecodeReg(&Left, Erg1, False) == eIsReg) && (*Erg1 <= 7)
           && (DecodeReg(&Right, Erg2, False) == eIsReg) && (*Erg2 <= 7);
}
...
static Boolean OneField(tStrComp const* pArg, Word* Erg, Boolean Ab1) {
...
static Boolean SplitBitField(tStrComp* pArg, Word* Erg) {
    ...
    if ((!*FieldArg.str.p_str)
        || (FieldArg.str.p_str[strlen(FieldArg.str.p_str) - 1] != '}')) {
        return False;
    }
...
static LongInt NextNonBlankAfter(char const* pSrc, LongInt tPos) {
    tPos++;
    for (; pSrc[tPos]; tPos++) {
...
static LongInt NextNonBlankBefore(char const* pSrc, LongInt tPos) {
    tPos--;
    for (; tPos >= 0; tPos--) {
        if (!isspace((unsigned char)pSrc[tPos])) {
            break;
...
static Boolean ClassComp(AdrComp* C) {
    ...
    reg_len = comp_len;
    c_scale = c_size = '\0';
    int scale_off    = NextNonBlankBefore(C->Comp.str.p_str, reg_len - 1);
    if ((reg_len > 2) && (C->Comp.str.p_str[scale_off] == '*')) {
        c_scale = C->Comp.str.p_str[reg_len - 1];
        reg_len = NextNonBlankBefore(C->Comp.str.p_str, scale_off) + 1;
    }
...
static Boolean IsShortAdr(LongInt Addr) {
    ...
    return (ExtAddr & pCurrCPUProps->AddrSpaceMask)
           == (OrigAddr & pCurrCPUProps->AddrSpaceMask);
}
...
static void DecodeAbs(tStrComp const* pArg, ShortInt Size, tAdrResult* pResult) {
    Boolean      ValOK;
    tSymbolFlags Flags;
    LongInt      HVal;
    Integer      HVal16;
...
static Byte DecodeAdr(tStrComp const* pArg, Word Erl, tAdrResult* pResult) {
    Byte  i;
    int   ArgLen;
    char* p;
    Word  rerg;
    ...
    String         CReg;
    tStrComp       CRegArg;
    unsigned const ExtAddrFamilyMask = (1 << e68KGen3) | (1 << e68KGen2) | (1 << eCPU32);
    IntType        DispIntType;
    tSymbolSize    RegSize;
    ...
    if (*Arg.str.p_str == '#') {
        ...
        case eSymbolSize8Bit:
            pResult->Cnt = 2;
            HVal8        = EvalStrIntExpressionWithFlags(
                    &ImmArg, Int8, &ValOK, &pResult->ImmSymFlags);
            if (ValOK) {
        ...
        case eSymbolSize16Bit:
            pResult->Cnt = 2;
            HVal16       = EvalStrIntExpressionWithFlags(
                    &ImmArg, Int16, &ValOK, &pResult->ImmSymFlags);
            if (ValOK) {
        ...
        case eSymbolSize32Bit:
            pResult->Cnt = 4;
            HVal         = EvalStrIntExpressionWithFlags(
                    &ImmArg, Int32, &ValOK, &pResult->ImmSymFlags);
            if (ValOK) {
                pResult->Vals[0] = HVal >> 16;
                pResult->Vals[1] = HVal & 0xffff;
            }
        ...
        case eSymbolSize64Bit: {
            LargeInt QVal = EvalStrIntExpressionWithFlags(
                    &ImmArg, LargeIntType, &ValOK, &pResult->ImmSymFlags);
            pResult->Cnt = 8;
            if (ValOK) {
#ifdef HAS64
...
#else
                pResult->Vals[0] = pResult->Vals[1]
                        = (QVal & 0x80000000ul) ? 0xffff : 0x0000;
#endif
        ...
        case eSymbolSizeShiftCnt: /* special arg 1..8 */
            pResult->Cnt = 2;
            HVal8        = EvalStrIntExpressionWithFlags(
                    &ImmArg, UInt4, &ValOK, &pResult->ImmSymFlags);
            if (ValOK) {
    ...
    if ((ArgLen >= 4) && (lParenPos != -1) && !haveBrackets
        && ((tPos = NextNonBlankBefore(Arg.str.p_str, lParenPos)) == 0)
        && (Arg.str.p_str[tPos] == '-') && (rParenPos == ArgLen - 1)) {
        tPos = NextNonBlankAfter(Arg.str.p_str, lParenPos);
        wPos = NextNonBlankBefore(Arg.str.p_str, rParenPos);
    ...
    if ((ArgLen >= 4) && (lParenPos == 0) && !haveBrackets
        && ((tPos = NextNonBlankAfter(Arg.str.p_str, rParenPos)) == ArgLen - 1)
        && (Arg.str.p_str[tPos] == '+')) {
        tPos = NextNonBlankAfter(Arg.str.p_str, lParenPos);
        wPos = NextNonBlankBefore(Arg.str.p_str, rParenPos);
        StrCompCopySub(&CRegArg, &Arg, tPos, wPos - tPos + 1);
    ...
    if ((lParenPos != -1) && (rParenPos == ArgLen - 1)) {
        ...
        *p = '\0';
        ...
        do {
            ...
            locStack   = -1;
            for (; ((locStack != -1) || (*pCompSplit != ',')) && (*pCompSplit != '\0');
                 pCompSplit++) {
            ...
            if (*pCompSplit == '\0') {
                ...
                StrCompSplitRef(
                        &AdrComps[CompCnt].Comp, &Remainder, &IndirComps, pCompSplit);
            ...
            if ((CompCnt == 1)
                && ((AdrComps[CompCnt].Art == AReg) || (AdrComps[CompCnt].Art == PC))) {
                /* Index register at "base position": just swap comp 0 & 1, so we get
                 * (An,Xi) or (PC,Xi): */
                ...
                /* Address register at "base position" and we add PC: also swap and
                   convert it to index so we get again (PC,Xi): */
                ...
                /* Otherwise, convert address to general index register.  Result may
                   require 68020++ modes: */
        ...
        /* 1. Variante (An....), d(An....) */
            ...
            if (CompCnt == 1) {
                ...
                /* 1.1.2. Variante d(An) */
                    ...
                    /* only try 32-bit displacement if explicitly requested, or 68020++
                     * and no size given */

                    if (OutDispLen < 0) {
                        DispIntType
                                = CheckFamilyCore(ExtAddrFamilyMask) ? SInt32 : SInt16;
                    } else {
                    ...
                    if (ValOK && (HVal == 0) && ((MModAdrI & Erl) != 0)
                        && (OutDispLen == -1)) {
                        pResult->Mode = 0x10 + AdrComps[0].ANummer;
                        pResult->Num  = ModAdrI;
                        pResult->Cnt  = 0;
            ...
            /* 1.2. Variante d(An,Xi) */
                ...
                if (OutDispLen < 0) {
                    ...
                    DispIntType = (OutDispLen >= 2) ? SInt32
                                                    : (OutDispLen >= 1 ? SInt16 : SInt8);
                }
                HVal = EvalStrIntExpression(&OutDisp, DispIntType, &ValOK);
                if (ValOK) {
                    switch (OutDispLen) {
                ...
                if (ValOK) {
                    ...
                    case 0:
                        ...
                        if ((AdrComps[1].Scale != 0)
                            && (!(pCurrCPUProps->SuppFlags & eFlagIdxScaling))) {
        ...
        /* 2. Variante d(PC....) */
            ...
            if (CompCnt == 1) {
                HVal = EvalStrIntExpressionWithFlags(&OutDisp, Int32, &ValOK, &Flags)
                       - (EProgCounter() + RelPos);
                if (!ValOK) {
                    return ModNone;
                }
        ...
        /* 3. Variante (Xi), d(Xi) */
            ...
            if (*OutDisp.str.p_str == '\0') {
                ...
                HVal = EvalStrIntExpression(
                        &OutDisp, (OutDispLen != 1) ? SInt32 : SInt16, &ValOK);
                if (ValOK) {
                    if (OutDispLen == -1) {
                        OutDispLen = IsDisp16(HVal) ? 1 : 2;
                    }
        ...
        /* 4. Variante indirekt: */
            ...
            do {
                ...
                locStack   = -1;
                for (;
                     ((locStack != -1) || (*pCompSplit != ',')) && (*pCompSplit != '\0');
                     pCompSplit++) {
                    char token = *pCompSplit;
                    switch (token) {
                    case ')':
            ...
            /* Set post-indexed also for no index register for compatibility with older
             * versions. */

            if (AdrComps[2].Art == None) {
                pResult->Vals[0] |= 0x0040 | 0x0004;
            } else {
                pResult->Vals[0] |= (AdrComps[2].INummer << 12)
                                    + (Ord(AdrComps[2].Long) << 11)
                                    + (AdrComps[2].Scale << 9);
            }
            ...
            HVal = EvalStrIntExpression(
                    &OutDisp, (OutDispLen == 1) ? SInt16 : SInt32, &ValOK);
            if (!ValOK) {
                pResult->Num = ModNone;
                pResult->Cnt = 0;
                return ModNone;
...
static Boolean DecodeMACACC(char const* pArg, Word* pResult) {
    /* interprete ACC like ACC0, independent of MAC or EMAC: */

    if (!as_strcasecmp(pArg, "ACC")) {
        *pResult = 0;
    } else if (
            !as_strncasecmp(pArg, "ACC", 3) && (strlen(pArg) == 4) && (pArg[3] >= '0')
            && (pArg[3] <= '3')) {
        *pResult = pArg[3] - '0';
    } else {
        return False;
...
static Boolean DecodeMACReg(char const* pArg, Word* pResult) {
...
static Boolean DecodeRegList(tStrComp const* pArg, Word* Erg) {
    Word     h, h2;
    Byte     z;
    char *   p, *p2;
    String   ArgStr;
...
static Boolean DecodeMACScale(tStrComp const* pArg, Word* pResult) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (*AttrPart.str.p_str && (OpSize != eSymbolSize32Bit)) {
            WrError(ErrNum_InvOpSize);
                ...
                (pCurrCPUProps->Family != eColdfire)
                || CheckISA((1 << eCfISA_APlus) | (1 << eCfISA_B) | (1 << eCfISA_C))) {
        ...
        if (OpSize != eSymbolSize16Bit) {
            ...
            if (DecodeAdr(
                        ...
                        MModData
                                | ((pCurrCPUProps->Family == eColdfire)
                                           ? 0
                                           : MModAdrI | MModPost | MModPre | MModDAdrI
                                                     | MModAIX | MModAbs),
                        &AdrResult)) {
        ...
        if (*AttrPart.str.p_str && (OpSize > eSymbolSize16Bit)) {
            WrError(ErrNum_InvOpSize);
            ...
            (pCurrCPUProps->SuppFlags & eFlagMAC)
            && (DecodeMACReg(ArgStr[1].str.p_str, &MACReg))) {
        Word DestMACReg;
        ...
        if (*AttrPart.str.p_str && (OpSize != eSymbolSize32Bit)) {
            ...
            CodeLen     = 2;
                ...
                (MACReg < 4) && DecodeMACReg(ArgStr[2].str.p_str, &DestMACReg)
                && (DestMACReg < 4) && (pCurrCPUProps->SuppFlags & eFlagEMAC)) {
            ...
            (pCurrCPUProps->SuppFlags & eFlagMAC)
            && (DecodeMACReg(ArgStr[2].str.p_str, &MACReg))) {
        if (*AttrPart.str.p_str && (OpSize != eSymbolSize32Bit)) {
            WrError(ErrNum_InvOpSize);
        } else {
            tAdrResult AdrResult;
        ...
        if (OpSize > eSymbolSize32Bit) {
            ...
            DecodeAdr(
                    &ArgStr[1],
                    ((Variant == I_Variant) ? 0
                                            : MModData | MModAdr | MModAdrI | MModPost
                                                      | MModPre | MModDAdrI | MModAIX
                                                      | MModPC | MModPCIdx | MModAbs)
                            | MModImm,
                    &AdrResult);

            /* Is An as source in byte mode allowed for ColdFire? No corresponding
             * footnote in CFPRM... */

            if ((AdrResult.Num == ModAdr) && (OpSize == eSymbolSize8Bit)
                && (pCurrCPUProps->Family != eColdfire)) {
                WrError(ErrNum_InvOpSize);
                ...
                DecodeAdr(
                        &ArgStr[2],
                        ((Variant == A_Variant) ? 0
                                                : MModData | MModAdrI | MModPost | MModPre
                                                          | MModDAdrI | MModAIX | MModAbs)
                                | MModAdr,
                        &AdrResult);
                if ((AdrResult.Num == ModAdr) && (OpSize == eSymbolSize8Bit)) {
                    WrError(ErrNum_InvOpSize);
                    ...
                    if (pCurrCPUProps->Family == eColdfire) {
                        ...
                        case ModAbs:   /* (xxx).W/L */
                            CombinationOK
                                    = (AdrResult.Num != ModDAdrI)   /* no (d16,An) */
                                      && (AdrResult.Num != ModAIX)  /* no (d8,An,Xi) */
                                      && (AdrResult.Num != ModAbs); /* no (xxx).W/L */
                            break;
                        case ModImm:                         /* #xxx */
                            if (AdrResult.Num == ModDAdrI) { /* (d16,An) OK for 8/16 bit
                                                                starting with ISA B */
                                CombinationOK = (pCurrCPUProps->CfISA >= eCfISA_B)
                    ...
                    if (!CombinationOK) {
                        ...
                        AdrResult.Mode
                                = ((AdrResult.Mode & 7) << 3) | (AdrResult.Mode >> 3);
...
static void DecodeLEA(Word Index) {
    ...
    if (*AttrPart.str.p_str && (OpSize != eSymbolSize32Bit)) {
        ...
        if (DecodeAdr(&ArgStr[2], MModAdr, &AdrResult)) {
            ...
            if (DecodeAdr(
                        &ArgStr[1],
                        MModAdrI | MModDAdrI | MModAIX | MModPC | MModPCIdx | MModAbs,
                        &AdrResult)) {
                WAsmCode[0] |= AdrResult.Mode;
                CodeLen = 2 + AdrResult.Cnt;
                CopyAdrVals(WAsmCode + 1, &AdrResult);
...
static void DecodeShift(Word Index) {
    ...
    if (!ChkArgCnt(1, 2))
        ...
        if (DecodeAdr(
                    &ArgStr[ArgCnt],
                    MModData | MModAdrI | MModPost | MModPre | MModDAdrI | MModAIX
                            | MModAbs,
                    &AdrResult)
            == ModData) {
            if (CheckColdSize()) {
                ...
                if ((AdrResult.Num == ModData)
                    || ((AdrResult.Num == ModImm) && (Lo(AdrResult.Vals[0]) >= 1)
                        && (Lo(AdrResult.Vals[0]) <= 8))) {
                    CodeLen = 2;
                    WAsmCode[0] |= (AdrResult.Num == ModData)
                                           ? 0x20 | (AdrResult.Mode << 9)
                                           : ((AdrResult.Vals[0] & 7) << 9);
                } else {
            ...
            if (pCurrCPUProps->Family == eColdfire) {
                ...
                if (OpSize != eSymbolSize16Bit) {
                    ...
                    CopyAdrVals(WAsmCode + 1, &AdrResult);
                    if (2 == ArgCnt) {
                        HVal8 = EvalStrIntExpressionOffs(
                                &ArgStr[1], !!(*ArgStr[1].str.p_str == '#'), Int8,
                                &ValOK);
                    } else {
                        HVal8 = 1;
                        ValOK = True;
...
static void DecodeADDQSUBQ(Word Index) {
    ...
    if (!DecodeAdr(
                &ArgStr[2],
                MModData | MModAdr | MModAdrI | MModPost | MModPre | MModDAdrI | MModAIX
                        | MModAbs,
                &AdrResult)) {
        return;
    }
    ...
    CopyAdrVals(WAsmCode + 1, &AdrResult);
    ImmVal = EvalStrIntExpressionOffsWithFlags(
            &ArgStr[1], !!(*ArgStr[1].str.p_str == '#'), UInt32, &ValOK, &Flags);
    ...
    if (CheckColdSize()) {
        if (ChkArgCnt(2, 2)) {
            ...
            if (DecodeAdr(&ArgStr[1], MModData | MModPre, &AdrResult)) {
                WAsmCode[0]
                        = 0x9100 | (OpSize << 6) | (AdrResult.Mode & 7) | (Index << 14);
...
static void DecodeCMPM(Word Index) {
    ...
    if (OpSize > eSymbolSize32Bit) {
        WrError(ErrNum_InvOpSize);
    } else if (ChkArgCnt(2, 2) && CheckNoFamily(1 << eColdfire)) {
        tAdrResult AdrResult;
...
static void DecodeADDSUBCMP(Word Index) {
    ...
    if (I_Variant == Variant) {
        ...
        SrcMask = MModData | MModAdr | MModAdrI | MModPost | MModPre | MModDAdrI | MModAIX
                  | MModPC | MModPCIdx | MModAbs | MModImm;
    }

    if (A_Variant == Variant) {
        DestMask = MModAdr;
    } else {
        DestMask = MModData | MModAdr | MModAdrI | MModPost | MModPre | MModDAdrI
                   | MModAIX | MModAbs;
    ...
    if (!OpSizeOK) {
        ...
        if (ChkArgCnt(2, 2)) {
            ...
            case ModData: /* ADD/SUB/CMP <ea>,Dn ? */
                WAsmCode[0] = 0x9000 | (OpSize << 6) | ((Reg = AdrResult.Mode) << 9)
                              | (Op << 13);
            ...
            default: /* CMP/ADD/SUB <ea>, Dn */
                if (DecodeAdr(&ArgStr[1], MModData | MModImm, &AdrResult)
                    == ModImm) /* ADDI/SUBI/CMPI ? */
                {
                    /* we have to set the PC offset before we decode the destination
                       operand.  Luckily, this is only needed afterwards for an immediate
                       source operand, so we know the # of words ahead: */
                    ...
                    if (DecodeAdr(
                                &ArgStr[2],
                                (pCurrCPUProps->Family == eColdfire) ? (Word)MModData
                                                                     : DestMask,
                                &AdrResult)) {
                        WAsmCode[0] |= AdrResult.Mode;
                        CopyAdrVals(WAsmCode + (CodeLen >> 1), &AdrResult);
                ...
                {
                    if (Op == 1) {
                        WrError(ErrNum_InvCmpMode);
                    } else {
                        WAsmCode[0] = 0x9100 | (OpSize << 6) | (AdrResult.Mode << 9)
                                      | (Op << 13);
...
static void DecodeANDOR(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        {
            if (*AttrPart.str.p_str && (OpSize != eSymbolSize8Bit)) {
                WrError(ErrNum_InvOpSize);
            } else if (!(pCurrCPUProps->SuppFlags & eFlagLogCCR)) {
        ...
        } else if (!as_strcasecmp(ArgStr[2].str.p_str, "SR")) /* AND #...,SR */
        {
            ...
            if (AdrResult.Num == ModData) /* AND <EA>,Dn */
            {
                ...
                if (DecodeAdr(
                            ...
                            ((Variant == I_Variant)
                                     ? 0
                                     : MModData | MModAdrI | MModPost | MModPre
                                               | MModDAdrI | MModAIX | MModPC | MModPCIdx
                                               | MModAbs)
                                    | MModImm,
                            &AdrResult)) {
                    if ((AdrResult.Num == ModImm) && (Variant == I_Variant)) {
            ...
            } else if (AdrResult.Num != ModNone) /* AND ...,<EA> */
            {
                    ...
                    == ModImm) /* AND #..,<EA> */
                {
                    ...
                    if (DecodeAdr(
                                &ArgStr[2],
                                MModData | MModAdrI | MModPost | MModPre | MModDAdrI
                                        | MModAIX | MModAbs,
                                &AdrResult)) {
                        WAsmCode[0] |= AdrResult.Mode;
                        CopyAdrVals(WAsmCode + (CodeLen >> 1), &AdrResult);
                ...
                {
                    WAsmCode[0]
                            = 0x8100 | (OpSize << 6) | (AdrResult.Mode << 9) | (Op << 14);
                    if (DecodeAdr(
                                &ArgStr[2],
                                MModAdrI | MModPost | MModPre | MModDAdrI | MModAIX
                                        | MModAbs,
                                &AdrResult)) {
                        CodeLen = 2 + AdrResult.Cnt;
                        CopyAdrVals(WAsmCode + 1, &AdrResult);
...
static void DecodeEOR(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (DecodeAdr(
                    &ArgStr[1], ((Variant == I_Variant) ? 0 : MModData) | MModImm,
                    &AdrResult)
            == ModData) {
...
static void DecodeNBCDTAS(Word Index) {
    ...
    if (*AttrPart.str.p_str && (OpSize != eSymbolSize8Bit)) {
        ...
        if (DecodeAdr(
                    &ArgStr[1],
                    ((pCurrCPUProps->Family == eColdfire) ? 0 : MModData) | MModAdrI
                            | MModPost | MModPre | MModDAdrI | MModAIX | MModAbs,
                    &AdrResult)) {
            CodeLen     = 2 + AdrResult.Cnt;
...
static void DecodeNEGNOT(Word Index) {
    if (ChkArgCnt(1, 1) && CheckColdSize()) {
        tAdrResult AdrResult;
...
static void DecodeEXT(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if ((OpSize == eSymbolSize8Bit) || (OpSize > eSymbolSize32Bit)) {
        WrError(ErrNum_InvOpSize);
    } else {
        tAdrResult AdrResult;
...
static void DecodeWDDATA(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if (DecodeAdr(
                    &ArgStr[1],
                    MModAdrI | MModPost | MModPre | MModDAdrI | MModAIX | MModAbs,
                    &AdrResult)) {
...
static void DecodeWDEBUG(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && CheckFamily(1 << eColdfire) && CheckColdSize()) {
        tAdrResult AdrResult;
...
static void DecodeFixed(Word Index) {
    ...
    if (*AttrPart.str.p_str) {
        WrError(ErrNum_UseLessAttr);
    } else if (ChkArgCnt(0, 0) && CheckFamily(FixedZ->FamilyMask)) {
        CodeLen     = 2;
...
static void DecodeMOVEM(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if ((OpSize < eSymbolSize16Bit) || (OpSize > eSymbolSize32Bit)) {
        WrError(ErrNum_InvOpSize);
    } else if ((pCurrCPUProps->Family == eColdfire) && (OpSize == 1)) {
        WrError(ErrNum_InvOpSize);
    } else {
        tAdrResult AdrResult;
        ...
        if (DecodeRegList(&ArgStr[2], WAsmCode + 1)) {
            if (DecodeAdr(
                        ...
                        MModAdrI | MModDAdrI
                                | ((pCurrCPUProps->Family == eColdfire)
                                           ? 0
                                           : MModPost | MModAIX | MModPC | MModPCIdx
                                                     | MModAbs),
                        &AdrResult)) {
...
static void DecodeMOVEQ(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if ((*AttrPart.str.p_str) && (OpSize != eSymbolSize32Bit)) {
        WrError(ErrNum_InvOpSize);
        ...
        if (DecodeAdr(&ArgStr[2], MModData, &AdrResult)) {
            ...
            if ((Value > 0x7f) && (Value < 0xffffff80ul)) {
                WrStrErrorPos(
                        (Value & 0x80000000ul) ? ErrNum_UnderRange : ErrNum_OverRange,
                        &ArgStr[1]);
            } else {
                CodeLen = 2;
...
static void DecodeMOVE16(Word Index) {
    ...
    if (*AttrPart.str.p_str) {
        WrError(ErrNum_UseLessAttr);
    } else if (ChkArgCnt(2, 2) && CheckFamily(1 << e68KGen3)) {
        tAdrResult AdrResult;
...
static void DecodeCacheAll(Word Index) {
    ...
    if (*AttrPart.str.p_str) {
        ...
        ;
    else if (!CodeCache(ArgStr[1].str.p_str, &w1)) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (!OK) {
            ...
            if (DecodeAdr(
                ...
                CodeLen = 4 + AdrResult.Cnt;
                CheckFamily(
                        (1 << e68KGen3) | (1 << e68KGen2) | (1 << eCPU32)
                        | ((w1 == w2) ? (1 << eColdfire) : 0));
            }
        }
    } else {
...
static void DecodeDIVL(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_InvOpSize);
    } else if (!DecodeRegPair(&ArgStr[2], &w1, &w2)) {
        WrStrErrorPos(ErrNum_InvRegPair, &ArgStr[2]);
    } else {
        tAdrResult AdrResult;
...
static void DecodeASBCD(Word Index) {
    if ((OpSize != eSymbolSize8Bit) && *AttrPart.str.p_str) {
        WrError(ErrNum_InvOpSize);
    } else if (ChkArgCnt(2, 2) && CheckNoFamily(1 << eColdfire)) {
        tAdrResult AdrResult;
        ...
        OpSize = eSymbolSize8Bit;
        if (DecodeAdr(&ArgStr[1], MModData | MModPre, &AdrResult)) {
            WAsmCode[0] = 0x8100 | (AdrResult.Mode & 7) | (Index << 14)
                          | ((AdrResult.Num == ModPre) ? 8 : 0);
            if (DecodeAdr(&ArgStr[2], 1 << (AdrResult.Num - 1), &AdrResult)) {
                CodeLen = 2;
                WAsmCode[0] |= (AdrResult.Mode & 7) << 9;
            }
...
static void DecodeLINK(Word Index) {
    ...
    if (!*AttrPart.str.p_str && (pCurrCPUProps->Family == eColdfire)) {
        OpSize = eSymbolSize16Bit;
    }
    if ((OpSize < 1) || (OpSize > 2)) {
        WrError(ErrNum_InvOpSize);
            ...
            (OpSize == eSymbolSize32Bit)
            && !CheckFamily((1 << eCPU32) | (1 << e68KGen2) | (1 << e68KGen3)))
        ;
    else if (ChkArgCnt(2, 2)) {
        tAdrResult AdrResult;
...
static void DecodeMOVEP(Word Index) {
    ...
    if ((OpSize == eSymbolSize8Bit) || (OpSize > eSymbolSize32Bit)) {
        WrError(ErrNum_InvOpSize);
    } else if (ChkArgCnt(2, 2) && CheckNoFamily(1 << eColdfire)) {
        tAdrResult AdrResult;
...
static void DecodeCALLM(Word Index) {
    ...
    if (*AttrPart.str.p_str) {
        WrError(ErrNum_InvOpSize);
    } else if (!(pCurrCPUProps->SuppFlags & eFlagCALLM_RTM)) {
        WrError(ErrNum_InstructionNotSupported);
    } else if (ChkArgCnt(2, 2)) {
        tAdrResult AdrResult;
...
static void DecodeCAS(Word Index) {
    ...
    if (OpSize > eSymbolSize32Bit) {
        WrError(ErrNum_InvOpSize);
    } else if (ChkArgCnt(3, 3) && CheckFamily((1 << e68KGen3) | (1 << e68KGen2))) {
        tAdrResult AdrResult;
        ...
        if (DecodeAdr(&ArgStr[1], MModData, &AdrResult)) {
            ...
            if (DecodeAdr(&ArgStr[2], MModData, &AdrResult)) {
                ...
                if (DecodeAdr(
                            &ArgStr[3],
                            MModData | MModAdr | MModAdrI | MModPost | MModPre | MModDAdrI
                                    | MModAIX | MModAbs,
                            &AdrResult)) {
...
static void DecodeCAS2(Word Index) {
    ...
    if ((OpSize != eSymbolSize16Bit) && (OpSize != eSymbolSize32Bit)) {
        ...
        ;
    else if (!DecodeRegPair(&ArgStr[1], WAsmCode + 1, WAsmCode + 2)) {
        WrStrErrorPos(ErrNum_InvRegPair, &ArgStr[1]);
    } else if (!DecodeRegPair(&ArgStr[2], &w1, &w2)) {
        WrStrErrorPos(ErrNum_InvRegPair, &ArgStr[2]);
    } else {
        WAsmCode[1] += (w1 << 6);
        WAsmCode[2] += (w2 << 6);
        if (!CodeIndRegPair(&ArgStr[3], &w1, &w2)) {
            WrStrErrorPos(ErrNum_InvRegPair, &ArgStr[3]);
        } else {
            WAsmCode[1] += (w1 << 12);
            WAsmCode[2] += (w2 << 12);
...
static void DecodeEXTB(Word Index) {
    ...
    if ((OpSize != eSymbolSize32Bit) && *AttrPart.str.p_str) {
        WrError(ErrNum_InvOpSize);
    } else if (!CheckFamily((1 << e68KGen3) | (1 << e68KGen2) | (1 << eCPU32)))
        ;
    else if (ChkArgCnt(1, 1)) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (DecodeAdr(&ArgStr[2], MModData, &AdrResult)) {
            ...
            if (!p) {
                ...
                if (DecodeAdr(
                            &ArgStr[1],
                            MModAdrI | MModDAdrI | MModAIX | MModAbs | MModPC | MModPCIdx,
                            &AdrResult)) {
                ...
                *p = '\0';
                if (DecodeAdr(&ArgStr[1], MModData, &AdrResult)) {
                    ...
                    if (DecodeAdr(&ArgStr[3], MModData, &AdrResult)) {
                        ...
                        WAsmCode[1]
                                = 0x0000 | (OpSize << 6) | (Mode << 12) | AdrResult.Mode;
...
static void DecodeBits(Word Index) {
    ...
    if (((AdrResult.Num == ModData) && (OpSize != eSymbolSize32Bit))
        || ((AdrResult.Num != ModData) && (OpSize != eSymbolSize8Bit))) {
        WrError(ErrNum_InvOpSize);
        return;
    }
...
static void DecodeFBits(Word Index) {
    if (!ChkArgCnt(1, 1))
        ...
        WrError(ErrNum_InvOpSize);
    } else if (!SplitBitField(&ArgStr[1], WAsmCode + 1)) {
        WrError(ErrNum_InvBitMask);
    } else {
        tAdrResult AdrResult;
        ...
        if (DecodeAdr(
                    &ArgStr[1],
                    MModData | MModAdrI | MModDAdrI | MModAIX | MModAbs
                            | (Memo("BFTST") ? (MModPC | MModPCIdx) : 0),
                    &AdrResult)) {
...
static void DecodeEBits(Word Index) {
    if (!ChkArgCnt(2, 2))
        ...
        if (DecodeAdr(
            ...
            WAsmCode[0] = 0xe9c0 + AdrResult.Mode + (Index << 9);
            CopyAdrVals(WAsmCode + 2, &AdrResult);
            if (DecodeAdr(&ArgStr[2], MModData, &AdrResult)) {
                WAsmCode[1] |= AdrResult.Mode << 12;
                CodeLen = ThisCodeLen;
...
static void DecodeBFINS(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_InvOpSize);
    } else if (!SplitBitField(&ArgStr[2], WAsmCode + 1)) {
        WrError(ErrNum_InvBitMask);
    } else {
        tAdrResult AdrResult;
...
static void DecodeBcc(Word CondCode) {
    ...
    /* nur ein Operand erlaubt */
            ...
            if (IsDisp8(HVal)) {
                ...
                /* if the jump target is the address right behind the BSR, keep
                   ...
                   8 and 16 bits: */
                        ...
                        (Flags & eSymbolFlag_NextLabelAfterBSR) && (HVal == 2) && IsBSR) {
                    OpSize = eSymbolSize32Bit;
                } else {
        ...
        if (ValOK) {
            ...
            else if (!(pCurrCPUProps->SuppFlags & eFlagBranch32)) {
                WrError(ErrNum_InstructionNotSupported);
            } else {
                CodeLen     = 6;
...
static void DecodeFRegList(tStrComp const* pArg, Byte* pTyp, Byte* pList) {
    ...
    *pTyp = eFMovemTypNone;
    ...
    do {
        ...
        if (p2) {
            StrCompSplitRef(&From, &To, &Arg, p2);
            if ((DecodeFPReg(&From, &RegFrom, False) != eIsReg) || (RegFrom & REG_FPCTRL)
                || (DecodeFPReg(&To, &RegTo, False) != eIsReg) || (RegTo & REG_FPCTRL)) {
                return;
            }
            if (RegFrom <= RegTo) {
                for (z = RegFrom; z <= RegTo; z++) {
                    hw |= (1 << (7 - z));
                }
            } else {
                for (z = RegFrom; z <= 7; z++) {
...
static void DecodeFPUOp(Word Index) {
    ...
    } else if ((pCurrCPUProps->SuppFlags & Op->NeedsSuppFlags) != Op->NeedsSuppFlags) {
        WrError(ErrNum_InstructionNotSupported);
    } else if (ChkArgCnt(2, 2)) {
        tAdrResult AdrResult;
...
static void DecodeFSAVE(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if (DecodeAdr(
                    &ArgStr[1], MModAdrI | MModPre | MModDAdrI | MModAIX | MModAbs,
                    &AdrResult)) {
            CodeLen     = 2 + AdrResult.Cnt;
...
static void DecodeFRESTORE(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if (DecodeAdr(
                    &ArgStr[1], MModAdrI | MModPost | MModDAdrI | MModAIX | MModAbs,
                    &AdrResult)) {
            CodeLen     = 2 + AdrResult.Cnt;
...
static void DecodeFMOVE(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        } else if (AdrResult.Num != ModNone) /* FMOVE.x ????,<ea> ? */
        {
            ...
            CopyAdrVals(WAsmCode + 2, &AdrResult);
            switch (DecodeAdr(
                    &ArgStr[1], (AdrResult.Num == ModAdr) ? MModFPCR : MModFPn | MModFPCR,
                    &AdrResult)) {
            case ModFPn: /* FMOVE.x FPn,<ea> ? */
            {
                WAsmCode[1] = 0x6000 | (((Word)FSizeCodes[OpSize]) << 10)
                              | (AdrResult.Mode << 7);
...
static void DecodeFMOVECR(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        ;
    else if (*AttrPart.str.p_str && (OpSize != eSymbolSizeFloat96Bit)) {
        WrError(ErrNum_InvOpSize);
    } else {
        tAdrResult AdrResult;
        ...
        if (DecodeAdr(&ArgStr[2], MModFPn, &AdrResult) == ModFPn) {
            ...
            if (DecodeAdr(&ArgStr[1], MModImm, &AdrResult) == ModImm) {
                if (AdrResult.Vals[0] > 63) {
                    WrError(ErrNum_RomOffs063);
                } else {
                    CodeLen = 4;
                    WAsmCode[1] |= AdrResult.Vals[0];
                }
            ...
            if (DecodeAdr(pArg3, MModFPn, &AdrResult) == ModFPn) {
                ...
                switch (DecodeAdr(
                        ...
                        ((OpSize <= eSymbolSize32Bit)
                         || (OpSize == eSymbolSizeFloat32Bit))
                                ? MModData | MModAdrI | MModPost | MModPre | MModDAdrI
                                          | MModAIX | MModPC | MModPCIdx | MModAbs
                                          | MModImm | MModFPn
                                : MModAdrI | MModPost | MModPre | MModDAdrI | MModAIX
                                          | MModPC | MModPCIdx | MModAbs | MModImm
                                          | MModFPn,
        ...
        if (Typ != eFMovemTypNone) {
            if (*AttrPart.str.p_str
                ...
                WrError(ErrNum_InvOpSize);
                    ...
                    (Typ != eFMovemTypStatic) && (pCurrCPUProps->Family == eColdfire)) {
                ...
                {
                    Mask |= MModPre;
                    if ((List == REG_FPCR) | (List == REG_FPSR)
                        | (List == REG_FPIAR)) { /* nur ein Register */
...
static void DecodeFDBcc(Word CondCode) {
    if (!FPUAvail) {
        ...
        if ((OpSize != eSymbolSize16Bit) && *AttrPart.str.p_str) {
            WrError(ErrNum_InvOpSize);
        } else if (ChkArgCnt(2, 2)) {
            tAdrResult AdrResult;
            ...
            if (DecodeAdr(&ArgStr[1], MModData, &AdrResult)) {
                ...
                WAsmCode[1] = CondCode;
                HVal = EvalStrIntExpressionWithFlags(&ArgStr[2], Int32, &ValOK, &Flags)
                       - (EProgCounter() + 4);
                if (ValOK) {
                    HVal16      = HVal;
                    WAsmCode[2] = HVal16;
...
static void DecodeFScc(Word CondCode) {
    if (!FPUAvail) {
        ...
        ;
    else if ((OpSize != eSymbolSize8Bit) && *AttrPart.str.p_str) {
        WrError(ErrNum_InvOpSize);
    } else if (ChkArgCnt(1, 1)) {
        tAdrResult AdrResult;
...
static Boolean DecodeFC(tStrComp const* pArg, Word* erg) {
    Boolean OK;
    Word    Val;
...
static void DecodePSAVE(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        WrError(ErrNum_PMMUNotEnabled);
    } else if (!FullPMMU) {
        WrError(ErrNum_FullPMMUNotEnabled);
    } else {
        tAdrResult AdrResult;
...
static void DecodePFLUSHAN(Word Code) {
    ...
    if (*AttrPart.str.p_str) {
        ...
        WrError(ErrNum_PMMUNotEnabled);
    } else if (ChkArgCnt(0, 0) && CheckFamily(1 << e68KGen3)) {
        CodeLen     = 2;
        WAsmCode[0] = 0xf510;
        CheckSup();
    }
...
static void DecodePFLUSH_PFLUSHS(Word Code) {
    ...
    if (*AttrPart.str.p_str) {
        ...
        ;
    else if ((Code) && (!FullPMMU)) {
        WrError(ErrNum_FullPMMUNotEnabled);
    } else if (!DecodeFC(&ArgStr[1], WAsmCode + 1)) {
        WrError(ErrNum_InvFCode);
    } else {
        OpSize = eSymbolSize8Bit;
        if (DecodeAdr(&ArgStr[2], MModImm, &AdrResult)) {
            if (AdrResult.Vals[0] > 15) {
                WrError(ErrNum_InvFMask);
            } else {
                ...
                if (ArgCnt == 3) {
                    WAsmCode[1] |= 0x800;
                    if (!DecodeAdr(
                                &ArgStr[3], MModAdrI | MModDAdrI | MModAIX | MModAbs,
                                &AdrResult)) {
                        CodeLen = 0;
                    } else {
                        WAsmCode[0] |= AdrResult.Mode;
...
static void DecodePFLUSHN(Word Code) {
    ...
    if (*AttrPart.str.p_str) {
        ...
        WrError(ErrNum_PMMUNotEnabled);
    } else if (ChkArgCnt(1, 1) && CheckFamily(1 << e68KGen3)) {
        tAdrResult AdrResult;
...
static void DecodePFLUSHR(Word Code) {
    ...
    if (OpSize != eSymbolSize64Bit) {
        ...
        if (DecodeAdr(
                    &ArgStr[1],
                    MModAdrI | MModPre | MModPost | MModDAdrI | MModAIX | MModPC
                            | MModPCIdx | MModAbs | MModImm,
                    &AdrResult)) {
...
static void DecodePTESTR_PTESTW(Word Code) {
    ...
    if (*AttrPart.str.p_str) {
        ...
        if (!DecodeFC(&ArgStr[1], WAsmCode + 1)) {
            WrError(ErrNum_InvFCode);
        } else {
            if (DecodeAdr(
                        &ArgStr[2], MModAdrI | MModDAdrI | MModAIX | MModAbs,
                        &AdrResult)) {
                WAsmCode[0] = 0xf000 | AdrResult.Mode;
                CodeLen     = 4 + AdrResult.Cnt;
...
static void DecodeColdBit(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && CheckColdSize() && CheckFamily(1 << eColdfire)
...
static void DecodeSTLDSR(Word Code) {
    ...
    if (OpSize != eSymbolSize16Bit) {
        WrError(ErrNum_InvOpSize);
            ...
            ChkArgCnt(1, 1) && CheckFamily(1 << eColdfire)
            && CheckISA((1 << eCfISA_APlus) | (1 << eCfISA_C))) {
        tAdrResult AdrResult;
...
static void DecodeMOV3Q(Word Code) {
    ...
    if (!ChkArgCnt(2, 2) || !CheckFamily(1 << eColdfire)
        || (pCurrCPUProps->CfISA < eCfISA_B) || !CheckColdSize()) {
        return;
    }
...
static void DecodeMVS_MVZ(Word Code) {
    ...
    if (DecodeAdr(
                &ArgStr[1],
                MModData | MModAdr | MModAdrI | MModPost | MModPre | MModDAdrI | MModAIX
                        | MModAbs | MModImm | MModPC | MModPCIdx,
                &AdrResult)) {
...
static void DecodeSATS(Word Code) {
    ...
    if (!ChkArgCnt(1, 1) || !CheckFamily(1 << eColdfire)
        || (pCurrCPUProps->CfISA < eCfISA_B) || !CheckColdSize()) {
        return;
    }
...
static void DecodeMAC_MSAC(Word Code) {
    ...
    if ((RemArgCnt > 3) || (ExplicitLoad && (RemArgCnt < 2))) {
        WrError(ErrNum_WrongArgCnt);
        return;
    }
    ...
    WAsmCode[1] = ((OpSize - 1) << 11) | (Scale << 9) | Code | (Ux << 7) | (Uy << 6)
                  | ((AccNum & 2) << 3);
    ...
    if (RemArgCnt >= 2) {
        ...
        if (!DecodeAdr(
                    &CurrArgStr, MModAdrI | MModPre | MModPost | MModDAdrI, &AdrResult)) {
            return;
        }
...
static void DecodeMOVCLR(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_InvOpSize);
    } else if (!(pCurrCPUProps->SuppFlags & eFlagEMAC)) {
        WrError(ErrNum_InstructionNotSupported);
    } else if (!DecodeMACACC(ArgStr[1].str.p_str, &ACCReg)) {
...
static void DecodeMxxAC(Word Code) {
    ...
    if (!(pCurrCPUProps->SuppFlags & eFlagEMAC) || (pCurrCPUProps->CfISA < eCfISA_B)) {
        WrError(ErrNum_InstructionNotSupported);
        return;
    }
...
static void DecodeCPBCBUSY(Word Code) {
    if (pCurrCPUProps->CfISA == eCfISA_None) {
        WrError(ErrNum_InstructionNotSupported);
    } else if (*AttrPart.str.p_str && (OpSize != eSymbolSize16Bit)) {
        WrError(ErrNum_InvOpSize);
    } else if (ChkArgCnt(1, 1)) {
        Boolean      OK;
        tSymbolFlags Flags;
...
static void DecodeCPLDST(Word Code) {
    if (pCurrCPUProps->CfISA == eCfISA_None) {
        ...
        Boolean         OK;
        Word            Reg;
        tStrComp const *pEAArg = NULL, *pRnArg = NULL, *pETArg = NULL;
        ...
        WAsmCode[1] = EvalStrIntExpressionOffs(
                &ArgStr[ArgCnt], !!(*ArgStr[ArgCnt].str.p_str == '#'), UInt16, &OK);
        if (!OK) {
            return;
        }
...
static void DecodeSTR(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
            ...
            ((l = strlen(ArgStr[1].str.p_str)) < 2) || (*ArgStr[1].str.p_str != '\'')
            || (ArgStr[1].str.p_str[l - 1] != '\'')) {
        WrStrErrorPos(ErrNum_ExpectString, &ArgStr[1]);
    } else {
        PutByte(l - 2);
        for (z = 1; z < l - 1; z++) {
...
static void AddFixed(char const* NName, Word NCode, Boolean NSup, unsigned NMask) {
...
static void AddCond(char const* NName, Byte NCode) {
    char TmpName[30];
...
static void AddFPUOp(char const* NName, Byte NCode, Boolean NDya, tSuppFlags NeedFlags) {
    if (InstrZ >= FPUOpCnt) {
        exit(255);
    }
...
static void AddFPUCond(char const* NName, Byte NCode) {
    char TmpName[30];
...
static void AddPMMUCond(char const* NName) {
    char TmpName[30];
...
static void AddPMMUReg(char const* Name, tSymbolSize Size, Word Code) {
    if (InstrZ >= PMMURegCnt) {
        exit(255);
    }
...
static void InitFields(void) {
    ...
    AddFixed(
            "NOP", 0x4e71, False,
            (1 << e68KGen1a) | (1 << e68KGen1b) | (1 << e68KGen2) | (1 << e68KGen3)
                    | (1 << eCPU32) | (1 << eColdfire));
    AddFixed(
            "RESET", 0x4e70, True,
            (1 << e68KGen1a) | (1 << e68KGen1b) | (1 << e68KGen2) | (1 << e68KGen3)
                    | (1 << eCPU32));
    ...
    InstrZ = 0;
    AddFPUOp("FINT", 0x01, False, eFlagNone);
    AddFPUOp("FSINH", 0x02, False, eFlagExtFPU);
    AddFPUOp("FINTRZ", 0x03, False, eFlagNone);
    AddFPUOp("FSQRT", 0x04, False, eFlagNone);
    AddFPUOp("FSSQRT", 0x41, False, eFlagIntFPU);
    AddFPUOp("FDSQRT", 0x45, False, eFlagIntFPU);
    AddFPUOp("FLOGNP1", 0x06, False, eFlagExtFPU);
    AddFPUOp("FETOXM1", 0x08, False, eFlagExtFPU);
    AddFPUOp("FTANH", 0x09, False, eFlagExtFPU);
    AddFPUOp("FATAN", 0x0a, False, eFlagExtFPU);
    AddFPUOp("FASIN", 0x0c, False, eFlagExtFPU);
    AddFPUOp("FATANH", 0x0d, False, eFlagExtFPU);
    AddFPUOp("FSIN", 0x0e, False, eFlagExtFPU);
    AddFPUOp("FTAN", 0x0f, False, eFlagExtFPU);
    AddFPUOp("FETOX", 0x10, False, eFlagExtFPU);
    AddFPUOp("FTWOTOX", 0x11, False, eFlagExtFPU);
    AddFPUOp("FTENTOX", 0x12, False, eFlagExtFPU);
    AddFPUOp("FLOGN", 0x14, False, eFlagExtFPU);
    AddFPUOp("FLOG10", 0x15, False, eFlagExtFPU);
    AddFPUOp("FLOG2", 0x16, False, eFlagExtFPU);
    AddFPUOp("FABS", 0x18, False, eFlagNone);
    AddFPUOp("FSABS", 0x58, False, eFlagIntFPU);
    AddFPUOp("FDABS", 0x5c, False, eFlagIntFPU);
    AddFPUOp("FCOSH", 0x19, False, eFlagExtFPU);
    AddFPUOp("FNEG", 0x1a, False, eFlagNone);
    AddFPUOp("FACOS", 0x1c, False, eFlagExtFPU);
    AddFPUOp("FCOS", 0x1d, False, eFlagExtFPU);
    AddFPUOp("FGETEXP", 0x1e, False, eFlagExtFPU);
    AddFPUOp("FGETMAN", 0x1f, False, eFlagExtFPU);
    AddFPUOp("FDIV", 0x20, True, eFlagNone);
    AddFPUOp("FSDIV", 0x60, False, eFlagIntFPU);
    AddFPUOp("FDDIV", 0x64, True, eFlagIntFPU);
    AddFPUOp("FMOD", 0x21, True, eFlagExtFPU);
    AddFPUOp("FADD", 0x22, True, eFlagNone);
    AddFPUOp("FSADD", 0x62, True, eFlagIntFPU);
    AddFPUOp("FDADD", 0x66, True, eFlagIntFPU);
    AddFPUOp("FMUL", 0x23, True, eFlagNone);
    AddFPUOp("FSMUL", 0x63, True, eFlagIntFPU);
    AddFPUOp("FDMUL", 0x67, True, eFlagIntFPU);
    AddFPUOp("FSGLDIV", 0x24, True, eFlagExtFPU);
    AddFPUOp("FREM", 0x25, True, eFlagExtFPU);
    AddFPUOp("FSCALE", 0x26, True, eFlagExtFPU);
    AddFPUOp("FSGLMUL", 0x27, True, eFlagExtFPU);
    AddFPUOp("FSUB", 0x28, True, eFlagNone);
    AddFPUOp("FSSUB", 0x68, True, eFlagIntFPU);
    AddFPUOp("FDSUB", 0x6c, True, eFlagIntFPU);
    ...
    InstrZ   = 0;
    AddPMMUReg("TC", eSymbolSize32Bit, 16);
    AddPMMUReg("DRP", eSymbolSize64Bit, 17);
    AddPMMUReg("SRP", eSymbolSize64Bit, 18);
    AddPMMUReg("CRP", eSymbolSize64Bit, 19);
    AddPMMUReg("CAL", eSymbolSize8Bit, 20);
    AddPMMUReg("VAL", eSymbolSize8Bit, 21);
    AddPMMUReg("SCC", eSymbolSize8Bit, 22);
    AddPMMUReg("AC", eSymbolSize16Bit, 23);
    AddPMMUReg("PSR", eSymbolSize16Bit, 24);
    AddPMMUReg("PCSR", eSymbolSize16Bit, 25);
    AddPMMUReg("TT0", eSymbolSize32Bit, 2);
    AddPMMUReg("TT1", eSymbolSize32Bit, 3);
...
static void MakeCode_68K(void) {
    ...
    OpSize    = (AttrPartOpSize != eSymbolSizeUnknown)
                        ? AttrPartOpSize
                        : ((pCurrCPUProps->Family == eColdfire) ? eSymbolSize32Bit
                                                                : eSymbolSize16Bit);
    DontPrint = False;
    RelPos    = 2;
...
static void SwitchTo_68K(void* pUser) {
    ...
    NativeFloatSize = (pCurrCPUProps->Family == eColdfire) ? eSymbolSizeFloat64Bit
                                                           : eSymbolSizeFloat96Bit;
}

static tCtReg const CtRegs_40[] =
{
  { "TC"   , 0x003 },
  { "ITT0" , 0x004 },
  { "ITT1" , 0x005 },
...
static tCtReg const CtRegs_5202[] = {
        {"CACR", 0x002},
        {"ACR0", 0x004},
        {"ACR1", 0x005},
        { "VBR", 0x801},
...
static tCtReg const CtRegs_5202_5204[] = {
        {"RAMBAR", 0xc04},
        {  "MBAR", 0xc0f},
        {    NULL, 0x000},
};

static tCtReg const CtRegs_5202_5208[] = {
        {"RGPIOBAR", 0x009},
        {  "RAMBAR", 0xc05},
        {      NULL, 0x000},
};

static tCtReg const CtRegs_5202_5307[] = {
        {   "ACR2", 0x006},
        {   "ACR3", 0x007},
        {"RAMBAR0", 0xc04},
        {"RAMBAR1", 0xc05},
...
static tCtReg const CtRegs_5202_5329[] = {
        {"RAMBAR", 0xc05},
        {    NULL, 0x000},
};

static tCtReg const CtRegs_5202_5407[] = {
        {   "ACR2", 0x006},
        {   "ACR3", 0x007},
        {"RAMBAR0", 0xc04},
        {"RAMBAR1", 0xc05},
...
static tCtReg const CtRegs_Cf_CPU[] = {
        { "D0_LOAD", 0x080},
        { "D1_LOAD", 0x081},
        { "D2_LOAD", 0x082},
        { "D3_LOAD", 0x083},
...
static tCtReg const CtRegs_Cf_EMAC[] = {
        {   "MACSR", 0x804},
        {    "MASK", 0x805},
        {    "ACC0", 0x806},
        {"ACCEXT01", 0x807},
...
static tCtReg const CtRegs_MCF51[] = {
        {  "VBR", 0x801},
        {"CPUCR", 0x802},
        {   NULL, 0x000},
};
...
static tCPUProps const CPUProps[] = {
        ...
        {   "68008",
         0x000ffffful, e68KGen1a,
         eCfISA_None,                                                    eFlagExtFPU | eFlagLogCCR,
         {NULL}                                                                                                                     },
        {   "68000",
         0x00fffffful, e68KGen1a,
         eCfISA_None,                                                    eFlagExtFPU | eFlagLogCCR,
         {NULL}                                                                                                                     },
        {   "68010",
         0x00fffffful, e68KGen1b,
         eCfISA_None,                                                    eFlagExtFPU | eFlagLogCCR,
         {CtRegs_1040}                                                                                                              },
        {   "68012",
         0x7ffffffful, e68KGen1b,
         eCfISA_None,                                                    eFlagExtFPU | eFlagLogCCR,
         {CtRegs_1040}                                                                                                              },
        { "MCF5202",
         0xfffffffful, eColdfire,
         eCfISA_A,                                                eFlagIntFPU | eFlagIdxScaling,
         {CtRegs_5202}                                                                                                              },
        { "MCF5204",
         0xfffffffful, eColdfire,
         eCfISA_A,                                                eFlagIntFPU | eFlagIdxScaling,
         {CtRegs_5202, CtRegs_5202_5204}                                                                                            },
        { "MCF5206",
         0xfffffffful, eColdfire,
         eCfISA_A,                                                eFlagIntFPU | eFlagIdxScaling,
         {CtRegs_5202, CtRegs_5202_5204}                                                                                            },
        { "MCF5208",
         0xfffffffful, eColdfire,
         eCfISA_APlus,                         eFlagIntFPU | eFlagIdxScaling | eFlagMAC | eFlagEMAC,
         {CtRegs_5202, CtRegs_5202_5208, CtRegs_Cf_CPU, CtRegs_Cf_EMAC}                                                             }, /* V2 */
        {"MCF52274",
         0xfffffffful, eColdfire,
         eCfISA_APlus,                         eFlagIntFPU | eFlagIdxScaling | eFlagMAC | eFlagEMAC,
         {CtRegs_5202, CtRegs_5202_5208, CtRegs_Cf_CPU, CtRegs_Cf_EMAC}                                                             }, /* V2 */
        ...
        { "MCF5307",
         0xfffffffful, eColdfire,
         eCfISA_A,                                     eFlagIntFPU | eFlagIdxScaling | eFlagMAC,
         {CtRegs_5202, CtRegs_5202_5307}                                                                                            }, /* V3 */
        { "MCF5329",
         0xfffffffful, eColdfire,
         eCfISA_APlus,                         eFlagIntFPU | eFlagIdxScaling | eFlagMAC | eFlagEMAC,
         {CtRegs_5202, CtRegs_5202_5329}                                                                                            }, /* V3 */
        { "MCF5373",
         0xfffffffful, eColdfire,
         eCfISA_APlus,                         eFlagIntFPU | eFlagIdxScaling | eFlagMAC | eFlagEMAC,
         {CtRegs_5202, CtRegs_5202_5329}                                                                                            }, /* V3 */
        { "MCF5407",
         0xfffffffful, eColdfire,
         eCfISA_B,                     eFlagBranch32 | eFlagIntFPU | eFlagIdxScaling | eFlagMAC,
         {CtRegs_5202, CtRegs_5202_5407}                                                                                            }, /* V4 */
        { "MCF5470",
         0xfffffffful, eColdfire,
         eCfISA_B,         eFlagBranch32 | eFlagIntFPU | eFlagIdxScaling | eFlagMAC | eFlagEMAC,
         {CtRegs_5202, CtRegs_5202_5407}                                                                                            }, /* V4e */
        { "MCF5471",
         0xfffffffful, eColdfire,
         eCfISA_B,         eFlagBranch32 | eFlagIntFPU | eFlagIdxScaling | eFlagMAC | eFlagEMAC,
         {CtRegs_5202, CtRegs_5202_5407}                                                                                            }, /* V4e */
        ...
        { "MCF51QM",
         0xfffffffful, eColdfire,
         eCfISA_C,                       eFlagBranch32 | eFlagMAC | eFlagIdxScaling | eFlagEMAC,
         {CtRegs_MCF51}                                                                                                             }, /* V1 */
        {   "68332",
         0xfffffffful,    eCPU32,
         eCfISA_None,                                eFlagBranch32 | eFlagLogCCR | eFlagIdxScaling,
         {CtRegs_1040}                                                                                                              },
        {   "68340",
         0xfffffffful,    eCPU32,
         eCfISA_None,                                eFlagBranch32 | eFlagLogCCR | eFlagIdxScaling,
        ...
        {   "68360",
         ...
         {CtRegs_1040}                                                                                                              },
        {   "68020",
         0xfffffffful,  e68KGen2,
         eCfISA_None, eFlagBranch32 | eFlagLogCCR | eFlagIdxScaling | eFlagExtFPU | eFlagCALLM_RTM,
         {CtRegs_1040, CtRegs_2040, CtRegs_2030}                                                                                    },
        {   "68030",
         0xfffffffful,  e68KGen2,
         eCfISA_None,   eFlagBranch32 | eFlagLogCCR | eFlagIdxScaling | eFlagExtFPU | eFlagIntPMMU,
         {CtRegs_1040, CtRegs_2040, CtRegs_2030}                                                                                    },
        /* setting eFlagExtFPU assumes instructions of external FPU are emulated/provided
           by M68040FPSP! */
        {   "68040",
         0xfffffffful,  e68KGen3,
         eCfISA_None,                                                eFlagBranch32 | eFlagLogCCR | eFlagIdxScaling | eFlagIntPMMU | eFlagExtFPU
                                                | eFlagIntFPU,
         {CtRegs_1040, CtRegs_2040, CtRegs_40}                                                                                      },
