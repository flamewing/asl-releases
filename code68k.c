...
static void DecodeAdr(const char *Asc_O, Word Erl)
{
  ...
  if ((lklamm == 1) && (rklamm == 1) && (lastrklamm == strlen(Asc) - 1)) 
  {
    ...
    *p = '\0';
    ...
    strmov(Asc, p + 1);
    OutDispLen = -1;
    if (!SplitSize(OutDisp, &OutDispLen))
      return;
    Asc[strlen(Asc) - 1] = '\0';
    ...
    else if (AdrComps[0].Art == indir) 
    {
      ...
      AdrVals[0] += (AdrComps[2].Art == None)
                  ? 0x40
                  : (AdrComps[2].INummer << 12) + (Ord(AdrComps[2].Long) << 11) + (AdrComps[2].Scale << 9);
  ...
  {
    AdrCnt = 0;
    if (!strcasecmp(Asc + strlen(Asc) - 2, ".W"))
    {
      AdrCnt = 2;
      Asc[strlen(Asc) - 2] = '\0';
    }
    else if (!strcasecmp(Asc + strlen(Asc) - 2, ".L"))
    {
      AdrCnt = 4;
      Asc[strlen(Asc) - 2] = '\0';
    }
...
static void DecodeANDOR(Word Index)
{
  ...
  else if (CheckColdSize())
  {
    ...
    if (!strcasecmp(ArgStr[2], "CCR"))     /* AND #...,CCR */
    {
      ...
      else if ((MomCPU == CPU68008) || (MomCPU == CPUCOLD)) WrError(1500);
      else
      {
...
static void DecodeMUL_DIV(Word Code) 
{
  ...
  if (ArgCnt != 2) WrError(1110);
  else if ((MomCPU == CPUCOLD) && (*OpPart == 'D')) WrError(1500);
  else if (OpSize == 1) 
  {
    DecodeAdr(ArgStr[2], Mdata);
    if (AdrNum != 0)
...
static void DecodeTRAPcc(Word CondCode)
{
  ...
  if (*AttrPart == '\0')
    OpSize = 0;
  ChkArgCnt = (OpSize == 0) ? 0 : 1;
  if (OpSize > 2) WrError(1130);
  else if (ArgCnt != ChkArgCnt) WrError(1110);
  else if ((MomCPU == CPUCOLD) && (CondCode != 1)) WrError(1500);
  else
  {
...
static void DecodePSAVE(Word Code)
{
  ...
  else if (!PMMUAvail) WrError(1500);
...
static void DecodePFLUSH_PFLUSHS(Word Code)
{
  ...
  else if ((Code) && (!FullPMMU)) WrError(1500);
