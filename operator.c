...
static void BitMirrorOp(TempResult* pErg, TempResult* pLVal, TempResult* pRVal) {
    ...
    if ((pRVal->Contents.Int < 1) || (pRVal->Contents.Int > 32)) {
        WrError(ErrNum_OverRange);
    } else {
        LargeInt Result = (pLVal->Contents.Int >> pRVal->Contents.Int)
                          << pRVal->Contents.Int;
    ...
    case TempFloat:
        if (pRVal->Contents.Float == 0.0) {
            ...
            as_tempres_set_float(pErg, pow(pLVal->Contents.Float, pRVal->Contents.Float));
                ...
                (fabs(pRVal->Contents.Float) <= ((double)MaxLongInt))
                && (floor(pRVal->Contents.Float) == pRVal->Contents.Float)) {
...
static void LogXorOp(TempResult* pErg, TempResult* pLVal, TempResult* pRVal) {
    as_tempres_set_int(
            pErg, ((pLVal->Contents.Int != 0) != (pRVal->Contents.Int != 0)) ? 1 : 0);
    PromoteLRValFlags();
}
...
static void EqOp(TempResult* pErg, TempResult* pLVal, TempResult* pRVal) {
    ...
    case TempFloat:
        as_tempres_set_int(
                pErg, (pLVal->Contents.Float == pRVal->Contents.Float) ? 1 : 0);
        break;
    case TempString:
        as_tempres_set_int(
                pErg,
                (as_nonz_dynstr_cmp(&pLVal->Contents.str, &pRVal->Contents.str) == 0)
                        ? 1
                        : 0);
        break;
    default:
...
static void GtOp(TempResult* pErg, TempResult* pLVal, TempResult* pRVal) {
    ...
    case TempString:
        as_tempres_set_int(
                pErg, (as_nonz_dynstr_cmp(&pLVal->Contents.str, &pRVal->Contents.str) > 0)
                              ? 1
                              : 0);
        break;
    default:
...
Operator const* pPotMonadicOperator = NULL;
