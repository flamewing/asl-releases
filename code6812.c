...
static Boolean DecodeReg(char const* pAsc, Byte* pErg) {
    Boolean Result;
    String  Reg;
...
static Boolean DecodeBaseReg(char const* pAsc, Byte* pErg) {
...
static Boolean ValidReg(char const* Asc_o) {
    Byte   Dummy;
    String Asc;
    int    l = strlen(Asc_o);
...
static Boolean DecodeIdxReg(char const* pAsc, Byte* pErg) {
    Boolean Result = DecodeReg(pAsc, pErg);
    ...
    case eRegA:
        if ((DestReg <= eRegCCRL) || ((DestReg >= eRegD) && (DestReg <= eRegSP))) {
            *pExtMask = 0;
                ...
                (DestReg == eRegCCRH) || (DestReg == eRegXH) || (DestReg == eRegYH)
                || (DestReg == eRegSPH)) {
            *pExtMask = 8;
        } else {
            Result = False;
...
static Boolean DistFits(
        Byte Reg, Integer Dist, Integer Offs, LongInt Min, LongInt Max,
        tSymbolFlags Flags) {
    if (Reg == eBaseRegPC) {
        Dist -= Offs;
    }
    return (((Dist >= Min) && (Dist <= Max))
            || ((Reg == eBaseRegPC) && mSymbolQuestionable(Flags)));
}
...
static void DecodeAdr(int Start, int Stop, Word Mask) {
    ...
    if (Stop - Start == 0) {
        ...
        if ((*ArgStr[Start].str.p_str == '[')
            && (ArgStr[Start].str.p_str[strlen(ArgStr[Start].str.p_str) - 1] == ']')) {
        ...
        if (OK) {
            if ((ShortMode != eShortModeNo) && (Hi(AdrWord) == Reg_Direct)
                && ((Mask & MModDir) != 0)) {
                AdrMode    = ModDir;
    ...
    /* two arguments? */
        ...
        if (AutoFlag) {
            if (!DecodeBaseReg(ArgStr[Stop].str.p_str, AdrVals)) {
                WrStrErrorPos(ErrNum_InvReg, &ArgStr[Stop]);
            } else if (AdrVals[0] == eBaseRegPC) {
                WrStrErrorPos(ErrNum_InvReg, &ArgStr[Stop]);
            } else {
                AdrWord = EvalStrIntExpressionWithFlags(
                        &ArgStr[Start], SInt8, &OK, &Flags);
                if (mFirstPassUnknown(Flags)) {
                    AdrWord = 1;
                }

                /* no increment/decrement degenerates to register indirect with zero
                 * displacement! */
            ...
            if (!DecodeBaseReg(ArgStr[Stop].str.p_str, AdrVals)) {
                ...
                CutShort(ArgStr[Start].str.p_str, &ShortMode);
                AdrWord = EvalStrIntExpressionWithFlags(
                        &ArgStr[Start], Int16, &OK, &Flags);
                if (AdrVals[0] == eBaseRegPC) {
                    AdrWord -= EProgCounter() + ExPos;
                }
                if (OK) {
                    if ((ShortMode != eShortModeNo) && (ShortMode != eShortModeYes)
                        && ((Mask & MModIdx) != 0)
                        && (DistFits(AdrVals[0], AdrWord, 1, -16, 15, Flags))) {
                        if (AdrVals[0] == eBaseRegPC) {
                            AdrWord--;
                        }
                        ...
                        AdrMode    = ModIdx;
                            ...
                            && (ShortMode != eShortModeExtreme)
                            && ((Mask & MModIdx1) != 0)
                            && (DistFits(AdrVals[0], AdrWord, 2, -256, 255, Flags))) {
                        if (AdrVals[0] == eBaseRegPC) {
                            AdrWord -= 2;
                        }
...
static void DecodeGen(Word Index) {
    ...
    if (!ChkArgCnt(1, 2))
        ...
        DecodeAdr(
                1, ArgCnt,
                MModAllIdx | (pOrder->MayImm ? MModImm : 0)
                        | (pOrder->MayDir ? MModDir : 0)
                        | (pOrder->MayExt ? MModExt : 0));
        if (AdrMode != ModNone) {
            if (Hi(pOrder->Code) == 0) {
...
static void DecodeBranch(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        Address = EvalStrIntExpressionWithFlags(&ArgStr[1], AddrInt, &OK, &Flags)
                  - EProgCounter() - 2;
        if (OK) {
            if (((Address < -128) || (Address > 127)) && !mSymbolQuestionable(Flags)) {
...
static void DecodeLoop(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (!OK) {
            ...
            Address = EvalStrIntExpressionWithFlags(&ArgStr[2], AddrInt, &OK, &Flags)
                      - (EProgCounter() + 3);
            if (OK) {
...
static void DecodeTransfer(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrError(ErrNum_UseLessAttr);
    } else if (!DecodeReg(ArgStr[2].str.p_str, &Reg2)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else if (eRegPC == Reg2) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else if (!DecodeReg(ArgStr[1].str.p_str, &Reg1)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if (eRegPC == Reg1) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if (!ChkRegPair(Reg1, Reg2, &ExtMask)) {
...
static void DecodeSEX(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else if (ActRegSize != 1) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else if (eRegPC == Reg2) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[2]);
    } else if (!DecodeReg(ArgStr[1].str.p_str, &Reg1)) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if (ActRegSize != 0) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if (eRegPC == Reg1) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if (!ChkRegPair(Reg1, Reg2, &ExtMask)) {
...
static void DecodeMOV(Word Index) {
    ...
    if (!ChkArgCnt(2, 4))
        ...
        case ModImm:
            ...
            DecodeAdr(
                    Arg2Start, ArgCnt,
                    MModExt | MModIdx | MModIdx1 | MModIdx2 | MModDIdx | MModIIdx2);
            switch (AdrMode) {
            case ModExt:
                BAsmCode[1] |= 3;
...
static void DecodeBit(Word Index) {
    ...
    if (!ChkArgCnt(2, 3))
        ...
        HReg = EvalStrIntExpressionOffs(
                &ArgStr[ArgCnt], !!(*ArgStr[ArgCnt].str.p_str == '#'), UInt8, &OK);
        if (OK) {
...
static void DecodePCALL(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU6812X)) {
...
static void DecodeBrBit(Word Index) {
    ...
    if (!ChkArgCnt(3, 4))
        ...
        HReg = EvalStrIntExpressionOffs(
                &ArgStr[ArgCnt - 1], !!(*ArgStr[ArgCnt - 1].str.p_str == '#'), UInt8,
                &OK);
        if (OK) {
            Address = EvalStrIntExpressionWithFlags(&ArgStr[ArgCnt], AddrInt, &OK, &Flags)
                      - EProgCounter();
            if (OK) {
                ExPos = 3; /* Opcode, Maske+Distanz */
                DecodeAdr(
                        1, ArgCnt - 2, MModDir | MModExt | MModIdx | MModIdx1 | MModIdx2);
                if (AdrMode != ModNone) {
...
static void DecodeTRAP(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        BAsmCode[1] = EvalStrIntExpressionOffsWithFlags(
                &ArgStr[1], !!(*ArgStr[1].str.p_str == '#'), UInt8, &OK, &Flags);
        if (mFirstPassUnknown(Flags)) {
            BAsmCode[1] = 0x30;
        }
        if (OK) {
            if ((BAsmCode[1] < 0x30) || ((BAsmCode[1] > 0x39) && (BAsmCode[1] < 0x40))) {
                WrError(ErrNum_OverRange);
            } else {
                BAsmCode[0] = 0x18;
                CodeLen     = 2;
...
static void DecodeBTAS(Word Index) {
    ...
    if (!ChkArgCnt(2, 3))
        ...
        DecodeAdr(
                1, ArgCnt - 1,
                MModDir | MModExt | MModIdx | MModIdx1 | ModIdx2 | ModDIdx);
        if (AdrMode != ModNone) {
...
static void AddBranch(char const* NName, Word NCode) {
    if (InstrZ >= BranchOrderCount) {
        fprintf(stderr, "AddBranch");
        exit(255);
    }
...
static void AddGen(
        char const* NName, Word NCode, Boolean NMayI, Boolean NMayD, Boolean NMayE,
        tSymbolSize NSize, CPUVar NMin) {
    if (InstrZ >= GenOrderCount) {
        fprintf(stderr, "AddGen");
        exit(255);
    }
...
static void AddLoop(char const* NName, Word NCode) {
    if (InstrZ >= LoopOrderCount) {
        fprintf(stderr, "AddLoop");
        exit(255);
    }
...
static void AddLEA(char const* NName, Word NCode) {
    if (InstrZ >= LEAOrderCount) {
        fprintf(stderr, "AddLEA");
        exit(255);
    }
...
static void AddJmp(char const* NName, Word NCode, Boolean NDir) {
    if (InstrZ >= JmpOrderCount) {
        fprintf(stderr, "AddJmp");
        exit(255);
    }
...
static void AddReg(char const* NName, Word NCode, Word NSize, CPUVar NMin) {
    if (InstrZ >= RegCount) {
        fprintf(stderr, "AddReg");
        exit(255);
    }
...
static void InitFields(void) {
    ...
    InstrZ      = 0;
    AddFixed("ABA", 0x1806, CPU6812);
    AddFixed("ABX", 0x1ae5, CPU6812);
    AddFixed("ABY", 0x19ed, CPU6812);
    AddFixed("ASLA", 0x0048, CPU6812);
    AddFixed("ASLB", 0x0058, CPU6812);
    AddFixed("ASLD", 0x0059, CPU6812);
    AddFixed("ASLX", 0x1848, CPU6812X);
    AddFixed("ASLY", 0x1858, CPU6812X);
    AddFixed("ASRA", 0x0047, CPU6812);
    AddFixed("ASRB", 0x0057, CPU6812);
    AddFixed("ASRX", 0x1847, CPU6812X);
    AddFixed("ASRY", 0x1857, CPU6812X);
    AddFixed("BGND", 0x0000, CPU6812);
    AddFixed("CBA", 0x1817, CPU6812);
    AddFixed("CLC", 0x10fe, CPU6812);
    AddFixed("CLI", 0x10ef, CPU6812);
    AddFixed("CLRA", 0x0087, CPU6812);
    AddFixed("CLRB", 0x00c7, CPU6812);
    AddFixed("CLRX", 0x1887, CPU6812X);
    AddFixed("CLRY", 0x18c7, CPU6812X);
    AddFixed("CLV", 0x10fd, CPU6812);
    AddFixed("COMA", 0x0041, CPU6812);
    AddFixed("COMB", 0x0051, CPU6812);
    AddFixed("COMX", 0x1841, CPU6812X);
    AddFixed("COMY", 0x1851, CPU6812X);
    AddFixed("DAA", 0x1807, CPU6812);
    AddFixed("DECA", 0x0043, CPU6812);
    AddFixed("DECB", 0x0053, CPU6812);
    AddFixed("DECX", 0x1843, CPU6812X);
    AddFixed("DECY", 0x1853, CPU6812X);
    AddFixed("DES", 0x1b9f, CPU6812);
    AddFixed("DEX", 0x0009, CPU6812);
    AddFixed("DEY", 0x0003, CPU6812);
    AddFixed("EDIV", 0x0011, CPU6812);
    AddFixed("EDIVS", 0x1814, CPU6812);
    AddFixed("EMUL", 0x0013, CPU6812);
    AddFixed("EMULS", 0x1813, CPU6812);
    AddFixed("FDIV", 0x1811, CPU6812);
    AddFixed("IDIV", 0x1810, CPU6812);
    AddFixed("IDIVS", 0x1815, CPU6812);
    AddFixed("INCA", 0x0042, CPU6812);
    AddFixed("INCB", 0x0052, CPU6812);
    AddFixed("INCX", 0x1842, CPU6812X);
    AddFixed("INCY", 0x1852, CPU6812X);
    AddFixed("INS", 0x1b81, CPU6812);
    AddFixed("INX", 0x0008, CPU6812);
    AddFixed("INY", 0x0002, CPU6812);
    AddFixed("LSLA", 0x0048, CPU6812);
    AddFixed("LSLB", 0x0058, CPU6812);
    AddFixed("LSLX", 0x1848, CPU6812X);
    AddFixed("LSLY", 0x1858, CPU6812X);
    AddFixed("LSLD", 0x0059, CPU6812);
    AddFixed("LSRA", 0x0044, CPU6812);
    AddFixed("LSRB", 0x0054, CPU6812);
    AddFixed("LSRX", 0x1844, CPU6812X);
    AddFixed("LSRY", 0x1854, CPU6812X);
    AddFixed("LSRD", 0x0049, CPU6812);
    AddFixed("MEM", 0x0001, CPU6812);
    AddFixed("MUL", 0x0012, CPU6812);
    AddFixed("NEGA", 0x0040, CPU6812);
    AddFixed("NEGB", 0x0050, CPU6812);
    AddFixed("NEGX", 0x1840, CPU6812X);
    AddFixed("NEGY", 0x1850, CPU6812X);
    AddFixed("NOP", 0x00a7, CPU6812);
    AddFixed("PSHA", 0x0036, CPU6812);
    AddFixed("PSHB", 0x0037, CPU6812);
    AddFixed("PSHC", 0x0039, CPU6812);
    AddFixed("PSHCW", 0x1839, CPU6812X);
    AddFixed("PSHD", 0x003b, CPU6812);
    AddFixed("PSHX", 0x0034, CPU6812);
    AddFixed("PSHY", 0x0035, CPU6812);
    AddFixed("PULA", 0x0032, CPU6812);
    AddFixed("PULB", 0x0033, CPU6812);
    AddFixed("PULC", 0x0038, CPU6812);
    AddFixed("PULCW", 0x1838, CPU6812X);
    AddFixed("PULD", 0x003a, CPU6812);
    AddFixed("PULX", 0x0030, CPU6812);
    AddFixed("PULY", 0x0031, CPU6812);
    AddFixed("REV", 0x183a, CPU6812);
    AddFixed("REVW", 0x183b, CPU6812);
    AddFixed("ROLA", 0x0045, CPU6812);
    AddFixed("ROLB", 0x0055, CPU6812);
    AddFixed("ROLX", 0x1845, CPU6812X);
    AddFixed("ROLY", 0x1855, CPU6812X);
    AddFixed("RORA", 0x0046, CPU6812);
    AddFixed("RORB", 0x0056, CPU6812);
    AddFixed("RORX", 0x1846, CPU6812X);
    AddFixed("RORY", 0x1856, CPU6812X);
    AddFixed("RTC", 0x000a, CPU6812);
    AddFixed("RTI", 0x000b, CPU6812);
    AddFixed("RTS", 0x003d, CPU6812);
    AddFixed("SBA", 0x1816, CPU6812);
    AddFixed("SEC", 0x1401, CPU6812);
    AddFixed("SEI", 0x1410, CPU6812);
    AddFixed("SEV", 0x1402, CPU6812);
    AddFixed("STOP", 0x183e, CPU6812);
    AddFixed("SWI", 0x003f, CPU6812);
    AddFixed("TAB", 0x180e, CPU6812);
    AddFixed("TAP", 0xb702, CPU6812);
    AddFixed("TBA", 0x180f, CPU6812);
    AddFixed("TPA", 0xb720, CPU6812);
    AddFixed("TSTA", 0x0097, CPU6812);
    AddFixed("TSTB", 0x00d7, CPU6812);
    AddFixed("TSTX", 0x1897, CPU6812X);
    AddFixed("TSTY", 0x18d7, CPU6812X);
    AddFixed("TSX", 0xb775, CPU6812);
    AddFixed("TSY", 0xb776, CPU6812);
    AddFixed("TXS", 0xb757, CPU6812);
    AddFixed("TYS", 0xb767, CPU6812);
    AddFixed("WAI", 0x003e, CPU6812);
    AddFixed("WAV", 0x183c, CPU6812);
    AddFixed("XGDX", 0xb7c5, CPU6812);
...
#define ASSUME6812Count (sizeof(ASSUME6812s) / sizeof(*ASSUME6812s))
        static ASSUMERec const ASSUME6812s[] = {
