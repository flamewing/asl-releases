...
static unsigned UpVal(unsigned Val1, unsigned Val3, unsigned Val9, unsigned Val27) {
    return (((Val1 >> 2) & 3) * 1) + (((Val3 >> 2) & 3) * 3) + (((Val9 >> 2) & 3) * 9)
           + (((Val27 >> 2) & 3) * 27);
}
...
static Boolean ParseReg(char const* pArg, unsigned* pResult) {
    char* pEnd;
...
static tRegEvalResult ParseArgReg(int Index, unsigned* pResult, Boolean MustBeReg) {
    ...
    RegEvalResult = EvalStrRegExpressionAsOperand(
            &ArgStr[Index], &RegDescr, &EvalResult, eSymbolSize8Bit, MustBeReg);
...
static void Code_3r(Word Index) {
    ...
    if (ChkArgCnt(3, 3) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op2, True)
...
static void Code_2rus(Word Index) {
    ...
    if (ChkArgCnt(3, 3) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op2, True)) {
        ...
        if (OK) {
            if (Op3 > 11) {
                WrStrErrorPos(ErrNum_OverRange, &ArgStr[3]);
            } else {
...
static void Code_2r(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op2, True)) {
...
static void Code_l3r(Word Index) {
    ...
    if (ChkArgCnt(3, 3) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op2, True)
        && ParseArgReg(3, &Op3, True)) {
        WAsmCode[0] = 0xf800 | ((Op3 & 3) << 0) | ((Op2 & 3) << 2) | ((Op1 & 3) << 4)
                      | (UpVal(Op3, Op2, Op1, 0) << 6);
        WAsmCode[1] = Index;
        CodeLen     = 4;
    }
    ...
    if (ChkArgCnt(3, 3) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op2, True)) {
        ...
        if (OK) {
            if (Op3 > 11) {
                ...
                WAsmCode[0] = 0xf800 | ((Op3 & 3) << 0) | ((Op2 & 3) << 2)
                              | ((Op1 & 3) << 4) | (UpVal(Op3, Op2, Op1, 0) << 6);
                WAsmCode[1] = Index;
                CodeLen     = 4;
            }
...
static void Code_l2r(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op2, True)) {
        ...
        WAsmCode[0] = 0xf800 | ((Index & 1) << 4) | ((Op2 & 3) << 0) | ((Op1 & 3) << 2)
                      | ((Up & 0x1f) << 6) | (Up & 0x20);
        WAsmCode[1] = Index & (~1);
        CodeLen     = 4;
    }
...
static void Code_l4r(Word Index) {
    ...
    if (ChkArgCnt(4, 4) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op2, True)
        && ParseArgReg(3, &Op3, True) && ParseArgReg(4, &Op4, True)) {
        WAsmCode[0] = 0xf800 | ((Op3 & 3) << 0) | ((Op2 & 3) << 2) | ((Op1 & 3) << 4)
                      | (UpVal(Op3, Op2, Op1, 0) << 6);
...
static void Code_l5r(Word Index) {
    ...
    if (ChkArgCnt(5, 5) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op4, True)
        && ParseArgReg(3, &Op2, True) && ParseArgReg(4, &Op3, True)
...
static void Code_l6r(Word Index) {
    ...
    if (ChkArgCnt(6, 6) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op4, True)
        && ParseArgReg(3, &Op2, True) && ParseArgReg(4, &Op3, True)
        && ParseArgReg(5, &Op5, True) && ParseArgReg(6, &Op6, True)) {
        WAsmCode[0] = 0xf800 | ((Op3 & 3) << 0) | ((Op2 & 3) << 2) | ((Op1 & 3) << 4)
                      | (UpVal(Op3, Op2, Op1, 0) << 6);
...
static void Code_branch_core(Word Code, int DistArgIndex) {
    ...
    Delta = EvalStrIntExpressionWithFlags(&ArgStr[DistArgIndex], UInt32, &OK, &Flags)
            - (EProgCounter() + 2);
    if (mFirstPassUnknown(Flags)) {
        Delta &= 0x1fffe;
    }
...
static void Code_lr2r(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ParseArgReg(1, &Op1, True) && ParseArgReg(2, &Op2, True)) {
        ...
        WAsmCode[0] = pOrder->Opcode1 | ((Op1 & 3) << 0) | ((Op2 & 3) << 2)
                      | ((Up & 0x1f) << 6) | (Up & 0x20);
...
static void Add_lr2r(char const* NName, Word NCode1, Word NCode2) {
