...
static Boolean DecodeReg(tStrComp const* pArg, Byte* pResult) {
    /* KU/KL/QU/QL are addressed via dedicated LR instructions! */
    static char const RegNames[][3] = {"J", "HU", "HL", "S", "I", "D", ""};
    int               z;
    Boolean           OK;
...
static Boolean DecodeAux(char const* pArg, Byte* pResult) {
    if (strlen(pArg) != 2) {
        return False;
    }
    *pResult = 0;
...
static Boolean ArgPair(char const* pArg1, char const* pArg2) {
    return !as_strcasecmp(ArgStr[1].str.p_str, pArg1)
           && !as_strcasecmp(ArgStr[2].str.p_str, pArg2);
}
...
static void DecodeFixed(Word Code) {
    if (!ChkArgCnt(0, 0))
        ;
    else if ((Code & Flg_CMOS) && (!(MomCPUFlags & Flg_CMOS))) {
...
static void DecodeImm8(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        BAsmCode[1] = EvalStrIntExpressionWithResult(
                &ArgStr[1], IsIO ? UInt8 : Int8, &EvalResult);
        if (EvalResult.OK) {
            if (IsIO) {
...
static void DecodeImm16(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        tEvalResult EvalResult;
        Word        Arg = EvalStrIntExpressionWithResult(
                &ArgStr[1], IsCode ? CodeIntType : Int16, &EvalResult);
...
static void DecodeBranchCore(tStrComp const* pArg, Word Code) {
    Boolean      OK;
    LongInt      Dist;
    tSymbolFlags Flags;

    Dist = EvalStrIntExpressionWithFlags(pArg, UInt16, &OK, &Flags)
           - (EProgCounter() + 1);
    if (OK) {
...
static void AddImm8(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeImm8);
}

static void AddImm4(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeImm4);
}

static void AddImm3(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeImm3);
}

static void AddImm16(char const* NName, Word NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeImm16);
}

static void AddOneReg(char const* NName, Word NCode) {
...
static void AddGenBranch(char const* NName, Word NCode) {
