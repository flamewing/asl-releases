...
static void Disassemble_68(
    ...
    case eIndexed:
        ...
        pOp            = MakeSymbolic(OpAddr, 1, NULL, NumBuf, sizeof(NumBuf));
        as_snprintf(
                pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\t%s,x", pOpcode->Memo, pOp);
    ...
    case eImmediate:
        ...
        as_snprintf(
                pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\t#%s", pOpcode->Memo, pOp);
        break;
    case eRelative:
        if (!RetrieveData(Address + 1, Data, 1)) {
            return;
        ...
        pOp    = MakeSymbolic(
                OpAddr, 2, strcmp(pOpcode->Memo, "bsr") ? "lab_" : "sub_", NumBuf,
                sizeof(NumBuf));
        as_snprintf(pInfo->SrcLine, sizeof(pInfo->SrcLine), "%s\t%s", pOpcode->Memo, pOp);
        break;
    default:
