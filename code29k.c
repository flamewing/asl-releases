...
static Boolean DecodeRegCore(char const* pArg, LongWord* pResult) {
    int     l = strlen(pArg);
    Boolean OK;
...
static void DissectReg_29K(
    ...
    case eSymbolSize32Bit:
        if (Value & REG_LRMARK) {
            as_snprintf(
                    pDest, DestSize, "%cR%u", "GL"[(Value >> 7) & 1],
                    (unsigned)(Value & 127));
        } else {
...
static void DecodeStd(Word Index) {
    StdOrder const* pOrder = StdOrders + Index;
    ...
    if (ChkArgCnt(2, 3) && DecodeArgReg(1, &Dest)) {
        ...
        if (OK) {
            ...
            if (OK) {
                CodeLen = 4;
                DAsmCode[0]
                        = (pOrder->Code << 24) + Src3 + (Dest << 16) + (Src1 << 8) + Src2;
                if (pOrder->MustSup) {
                    ChkSup();
                }
            }
...
static void DecodeNoImm(Word Index) {
    StdOrder const* pOrder = NoImmOrders + Index;
    Boolean         OK;
...
static void DecodeVec(Word Index) {
    StdOrder const* pOrder = VecOrders + Index;
    Boolean         OK;
    tSymbolFlags    Flags;
    LongWord        Dest, Src1, Src2, Src3;
    ...
    if (ChkArgCnt(3, 3)) {
        ...
        if (OK) {
            if (DecodeArgReg(2, &Src1)) {
                ...
                if (OK) {
                    CodeLen     = 4;
                    DAsmCode[0] = (pOrder->Code << 24) + Src3 + (Dest << 16) + (Src1 << 8)
                                  + Src2;
...
static void DecodeFixed(Word Code) {
    StdOrder const* pOrder = FixedOrders + Code;
...
static void DecodeMem(Word Index) {
    StdOrder const* pOrder = MemOrders + Index;
    Boolean         OK;
...
static void DecodeJmp(Word Index) {
    JmpOrder const* pOrder    = JmpOrders + (Index & 0xff);
            ...
            if (OK) {
                ...
                if (OK) {
                    if ((AdrLong & 3) != 0) {
                        ...
                        DAsmCode[0] = (pOrder->Code << 24) + ((AdrLong & 0x3fc00) << 6)
                                      + (Dest << 8) + ((AdrLong & 0x3fc) >> 2);
                    } else if (!mSymbolQuestionable(Flags) && (AdrLong > 0x3fffff)) {
                        WrError(ErrNum_JmpDistTooBig);
                    } else {
                        CodeLen     = 4;
                        DAsmCode[0] = ((pOrder->Code + 1) << 24)
                                      + ((AdrLong & 0x3fc00) << 6) + (Dest << 8)
                                      + ((AdrLong & 0x3fc) >> 2);
                    }
                }
            }
...
static void DecodeCLASS(Word Code) {
    ...
    if (ChkArgCnt(3, 3) && ChkCPU(CPU29000) && DecodeArgReg(1, &Dest)
        && DecodeArgReg(2, &Src1)) {
...
static void DecodeEMULATE(Word Code) {
    ...
    if (ChkArgCnt(3, 3)) {
        ...
        if (OK) {
            if (DecodeArgReg(2, &Src1) && DecodeArgReg(ArgCnt, &Src2)) {
                CodeLen     = 4;
...
static void DecodeSQRT(Word Code) {
    ...
    if (ChkArgCnt(2, 3) && ChkCPU(CPU29000) && DecodeArgReg(1, &Dest)) {
        if (ArgCnt == 2) {
            OK   = True;
            Src1 = Dest;
        } else {
    ...
    if (ChkArgCnt(2, 2) && DecodeArgReg(1, &Dest)) {
        ...
        if (OK) {
            ...
            if (AdrLong == 0xffff) {
                ...
                if (AdrLong != 0) {
                    CodeLen     = 8;
                    DAsmCode[1] = 0x02000000 + ((AdrLong & 0xff00) << 16) + (Dest << 8)
                                  + (AdrLong & 0xff);
                }
            }
        }
...
static void DecodeCONSTH_CONSTN(Word IsHi) {
    ...
    if (ChkArgCnt(2, 2) && DecodeArgReg(1, &Dest)) {
        ...
        if (ChkRange(AdrLong, 0, 0xffff)) {
            CodeLen     = 4;
            DAsmCode[0] = 0x1000000 + ((AdrLong & 0xff00) << 8) + (Dest << 8)
                          + (AdrLong & 0xff);
            if (IsHi) {
...
static void DecodeCONVERT(Word Code) {
    ...
    if (ChkArgCnt(6, 6) && ChkCPU(CPU29000) && DecodeArgReg(1, &Dest)
        && DecodeArgReg(2, &Src1)) {
        Src2 = 0;
...
static void DecodeEXHWS(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeArgReg(1, &Dest) && DecodeArgReg(2, &Src1)) {
        CodeLen     = 4;
...
static void DecodeINV_IRETINV(Word Code) {
    ...
    if (ChkArgCnt(0, 1)) {
        if (ArgCnt == 0) {
            ...
            Src1 = EvalStrIntExpression(&ArgStr[1], UInt2, &OK);
        }
        if (OK) {
            CodeLen     = 4;
...
static void DecodeMFSR(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeArgReg(1, &Dest) && DecodeArgSpReg(2, &Src1)) {
...
static void DecodeMTSR(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && DecodeArgSpReg(1, &Dest) && DecodeArgReg(2, &Src1)) {
...
static void AddStd(char const* NName, CPUVar NMin, Boolean NSup, LongWord NCode) {
    if (InstrZ >= StdOrderCount) {
        exit(255);
    }
...
static void AddNoImm(char const* NName, CPUVar NMin, Boolean NSup, LongWord NCode) {
    if (InstrZ >= NoImmOrderCount) {
        exit(255);
    }
...
static void AddVec(char const* NName, CPUVar NMin, Boolean NSup, LongWord NCode) {
    if (InstrZ >= VecOrderCount) {
        exit(255);
    }
...
static void AddJmp(
        char const* NName, CPUVar NMin, Boolean NHas, Boolean NInd, LongWord NCode) {
    char IName[30];
...
static void AddFixed(char const* NName, CPUVar NMin, Boolean NSup, LongWord NCode) {
...
static void AddMem(char const* NName, CPUVar NMin, Boolean NSup, LongWord NCode) {
    if (InstrZ >= MemOrderCount) {
        exit(255);
    }
...
static void AddSP(char const* NName, LongWord NCode) {
    if (InstrZ >= SPRegCount) {
        exit(255);
    }
...
static void SwitchTo_29K(void) {
    ...
    static int const ASSUME29KCount = sizeof(ASSUME29Ks) / sizeof(*ASSUME29Ks);
    TurnWords                       = True;
