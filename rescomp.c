...
static void fwritechk(
        char const* pArg, int retcode, void* pBuffer, size_t size, size_t nmemb,
        FILE* pFile) {
    size_t res;
...
static FILE* fopenchk(char const* pName, int retcode, char const* pOpenMode) {
    FILE* pRes;
...
static void GetLine(char* Dest) {
    ...
    if (!as_strncasecmp(Dest, "INCLUDE", 7)) {
        ...
        OneFile->Next     = IncList;
        OneFile->Contents = SrcFile;
        IncList           = OneFile;
        strmov(Dest, Dest + 7);
        KillPrefBlanks(Dest);
        KillPrefBlanks(Dest);
    ...
    if ((feof(SrcFile)) && (IncList)) {
        fclose(SrcFile);
        OneFile = IncList;
        IncList = OneFile->Next;
        SrcFile = OneFile->Contents;
        free(OneFile);
    }
}
...
static void Process_LANGS(char* Line) {
    ...
    KillPrefBlanks(NLine);
    KillPostBlanks(NLine);
    ...
    MsgCats = (PMsgCat)malloc(sizeof(TMsgCat) * (CatCount = z));
    p       = NLine;
    for (z = 0, PCat = MsgCats; z < CatCount; z++, PCat++) {
        ...
        if (*p2 == '\0') {
            ...
            *p2 = '\0';
            strcpy(Part, p);
            for (p = p2 + 1; *p && as_isspace(*p2); p++)
                ; /* TODO: p instead of p2? */
        }
        ...
        do {
            ...
            PCat->CtryCodes
                    = (PCat->CtryCodeCnt == 0)
                              ...
                              : (LongInt*)realloc(
                                        PCat->CtryCodes,
                                        sizeof(LongInt) * (PCat->CtryCodeCnt + 1));
...
int main(int argc, char** argv) {
    ...
    char const *pSrcName = NULL, *pHFileName = NULL, *pMsgFileName = NULL;
    ...
    for (argz = 1; argz < argc; argz++) {
        if (!strcmp(argv[argz], "-h")) {
            ...
            case 0:
                ...
                break;
            case 1:
                pMsgFileName = argv[argz];
                break;
            case 2:
                pHFileName = argv[argz];
                break;
            }
        }
    }
    ...
    if (!pSrcName) {
        fprintf(stderr,
                "usage: %s <input resource file> <[-m] output msg file> [[-h ]header "
                "file]\n",
                *argv);
        exit(1);
    }
    ...
    stamp = MyGetFileTime(argv[1]);
    Id1   = stamp & 0x7fffffff;
    Id2   = 0;
    ...
    while (!feof(SrcFile)) {
        ...
        if ((*Line != ';') && (*Line != '#') && (*Line != '\0')) {
            ...
            Save = *p;
            *p   = '\0';
            strmaxcpy(Cmd, Line, 1024);
            *p = Save;
            strmov(Line, p);
            if (!as_strcasecmp(Cmd, "LANGS")) {
                Process_LANGS(Line);
            } else if (!as_strcasecmp(Cmd, "DEFAULT")) {
                Process_DEFAULT(Line);
            } else if (!as_strcasecmp(Cmd, "MESSAGE")) {
    ...
    if (pMsgFileName) {
        ...
        if (DefCat > 0) {
            TmpCat          = MsgCats[0];
            MsgCats[0]      = MsgCats[DefCat];
            MsgCats[DefCat] = TmpCat;
        }
        ...
        for (z = MsgCats; z < MsgCats + CatCount; z++) {
            RunPos
                    += (z->HeadLength
                        = strlen(z->CtryName) + 1 + 4 + 4 + (4 * z->CtryCodeCnt) + 4);
        }
        for (z = MsgCats; z < MsgCats + CatCount; z++) {
            z->FilePos = RunPos;
            RunPos += z->TotLength + (4 * MsgCounter);
        }
        ...
        for (z = MsgCats; z < MsgCats + CatCount; z++) {
            ...
            for (c = 0; c < z->CtryCodeCnt; c++) {
                Write4(MsgFile, z->CtryCodes + c);
            }
            Write4(MsgFile, &(z->FilePos));
        }
        Save = '\0';
        fwritechk(pMsgFileName, 5, &Save, 1, 1, MsgFile);
        ...
        for (z = MsgCats; z < MsgCats + CatCount; z++) {
            ...
            for (List = z->Messages; List; List = List->Next) {
                fwritechk(
                        pMsgFileName, 5, List->Contents, 1, strlen(List->Contents) + 1,
                        MsgFile);
            }
        }
