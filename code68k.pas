   ...
   IF (lklamm=1) AND (rklamm=1) AND (lastrklamm=Length(Asc)) THEN
	 ...
	 0:BEGIN
	    ...
	    IF ValOK THEN
	     BEGIN
	      ...
	      IF AdrComps[2].Scale<>0 THEN ACheckCPU(CPU68332);
	     END;
	    Goto AdrFnd;
	   END;
	 1:BEGIN
	...
	BEGIN
	 ...
	 0:BEGIN
	    ...
	    IF AdrComps[2].Scale<>0 THEN ACheckCPU(CPU68020);
	    Goto AdrFnd;
	   END;
	 1:BEGIN
	    HVal16:=HVal;
	...
	BEGIN
	 HVal:=EvalIntExpression(AdrComps[1].Name,Int32,ValOK)-(EProgCounter+RelPos);
...
CONST
    CtRegCnt=8;
    CtRegs:ARRAY[0..CtRegCnt-1] OF CtReg=((Name:'SFC'  ;Code:$000;FirstCPU:D_CPU68010),
					  (Name:'DFC'  ;Code:$001;FirstCPU:D_CPU68010),
					  (Name:'CACR' ;Code:$002;FirstCPU:D_CPU68020),
					  (Name:'USP'  ;Code:$800;FirstCPU:D_CPU68010),
					  (Name:'VBR'  ;Code:$801;FirstCPU:D_CPU68010),
					  (Name:'CAAR' ;Code:$802;FirstCPU:D_CPU68020),
					  (Name:'MSP'  ;Code:$803;FirstCPU:D_CPU68020),
					  (Name:'ISP'  ;Code:$804;FirstCPU:D_CPU68020));
VAR
   z:Byte;
BEGIN
   z:=0; NLS_UpString(Asc);
   WHILE (z<CtRegCnt) AND ((CtRegs[z].Name<>Asc) OR (CtRegs[z].FirstCPU+CPU68008>MomCPU)) DO
    Inc(z);
...
BEGIN
   ...
   IF Memo('PEA') THEN
    BEGIN
     ...
     ELSE
      BEGIN
       OpSize:=0;
       DecodeAdr(ArgStr[1],M3+M6+M7+M8+M9+M10);
       IF AdrNum<>0 THEN
	BEGIN
	 CodeLen:=2+AdrCnt;
	...
	END;
     ...
     ELSE IF NLS_StrCaseCmp(ArgStr[1],'SR')=0 THEN
      BEGIN
       ...
       ELSE
	BEGIN
	 DecodeAdr(ArgStr[2],M1+M3+M4+M5+M6+M7+M10);
	 IF AdrNum<>0 THEN
	  BEGIN
	...
	END;
     ...
     ELSE IF NLS_StrCaseCmp(ArgStr[2],'SR')=0 THEN
      BEGIN
       ...
       ELSE
	BEGIN
	 DecodeAdr(ArgStr[1],M1+M3+M4+M5+M6+M7+M8+M9+M10+M11);
	 IF AdrNum<>0 THEN
	  BEGIN
	...
	END;
     ...
     ELSE
      BEGIN
       ...
       ELSE
	BEGIN
	 DecodeAdr(ArgStr[1],M1+M2+M3+M4+M5+M6+M7+M8+M9+M10+M11);
	 IF AdrNum<>0 THEN
	  BEGIN
	   ...
	   WAsmCode[0]:=WAsmCode[0] OR AdrMode;
	   DecodeAdr(ArgStr[2],M1+M2+M3+M4+M5+M6+M7+M10);
	   IF AdrMode<>0 THEN
	    BEGIN
   ...
   IF (Copy(OpPart,1,3)='ADD') OR (Copy(OpPart,1,3)='SUB') OR ((Copy(OpPart,1,3)='CMP') AND (OpPart[4]<>'2')) THEN
    BEGIN
     ...
     ELSE
      BEGIN
       DecodeAdr(ArgStr[2],M1+M2+M3+M4+M5+M6+M7+M10);       { ADDA ? }
       IF AdrNum=2 THEN
	  ...
	  BEGIN
	   ...
	   ELSE
	    BEGIN
	     ...
	     IF OpPart='ADD' THEN WAsmCode[0]:=WAsmCode[0] OR $4000;
	     DecodeAdr(ArgStr[2],M3+M4+M5+M6+M7+M10);
	     IF AdrNum<>0 THEN
	      BEGIN
	       CodeLen:=2+AdrCnt; CopyAdrVals(WAsmCode[1]);
	       WAsmCode[0]:=WAsmCode[0] OR AdrMode;
	...
	BEGIN
	 ...
	 ELSE
	  BEGIN
	   ...
	   OpSize:=0; DecodeAdr(ArgStr[1],M11);
	   IF AdrNum<>0 THEN
	    BEGIN
	...
	BEGIN
	 ...
	 ELSE
	  BEGIN
	   ...
	   OpSize:=1; DecodeAdr(ArgStr[1],M11);
	   IF AdrNum<>0 THEN
	    BEGIN
    ...
    BEGIN
     ...
     ELSE
      BEGIN
       ...
       IF OpPart<>'BTST' THEN DecodeAdr(ArgStr[2],M1+M3+M4+M5+M6+M7+M10)
       ELSE DecodeAdr(ArgStr[2],M1+M3+M4+M5+M6+M7+M8+M9+M10);
       IF AttrPart='' THEN
    ...
    BEGIN
     ...
     ELSE
      BEGIN
       RelPos:=4;
       OpSize:=0;
       IF Memo('BFTST') THEN DecodeAdr(ArgStr[1],M1+M3+M6+M7+M8+M9+M10)
       ELSE DecodeAdr(ArgStr[1],M1+M3+M6+M7+M10);
       IF AdrNum<>0 THEN
	BEGIN
    ...
    BEGIN
     ...
     ELSE
      BEGIN
       RelPos:=4;
       OpSize:=0;
       DecodeAdr(ArgStr[1],M1+M3+M4+M5+M6+M7+M8+M9+M10);
       IF AdrNum<>0 THEN
	BEGIN
   ...
   IF Memo('MOVEM') THEN
	...
	BEGIN
	 DecodeAdr(ArgStr[1],M3+M4+M6+M7+M8+M9+M10);
	 IF AdrNum<>0 THEN
	  BEGIN
	...
	BEGIN
	 DecodeAdr(ArgStr[2],M3+M5+M6+M7+M10);
	 IF AdrNum<>0 THEN
	  BEGIN
   ...
   IF Memo('EXG') THEN
    BEGIN
     ...
     ELSE
      BEGIN
       DecodeAdr(ArgStr[1],M1+M2);
       IF AdrNum=1 THEN
	BEGIN
	 WAsmCode[0]:=$c100 OR (AdrMode SHL 9);
	 DecodeAdr(ArgStr[2],M1+M2);
	 IF AdrNum=1 THEN
	  BEGIN
   ...
   IF Memo('MOVES') THEN
    BEGIN
     ...
     ELSE
      BEGIN
       DecodeAdr(ArgStr[1],M1+M2+M3+M4+M5+M6+M7+M10);
   ...
   IF Memo('CALLM') THEN
	...
	BEGIN
	 ...
	 DecodeAdr(ArgStr[2],M3+M6+M7+M8+M9+M10);
	 IF AdrNum<>0 THEN
	  BEGIN
	...
	BEGIN
	 ...
	 IF AdrNum<>0 THEN
	  BEGIN
	   ...
	   DecodeAdr(ArgStr[3],M1+M2+M3+M4+M5+M6+M7+M10);
	   IF AdrNum<>0 THEN
	    BEGIN
