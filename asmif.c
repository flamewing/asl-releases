...
static LongInt GetIfVal(tStrComp const* pCond) {
    Boolean      IfOK;
    tSymbolFlags Flags;
    LongInt      Tmp;

    Tmp = EvalStrIntExpressionWithFlags(pCond, Int32, &IfOK, &Flags);
    if (mFirstPassUnknown(Flags) || mSymbolQuestionable(Flags) || !IfOK) {
        Tmp = 1;
        if (mFirstPassUnknown(Flags) || mSymbolQuestionable(Flags)) {
...
void CodeIFEXIST(Word Negate) {
    ...
    if (!IfAsm) {
        ...
        strmaxcpy(
                FileName,
                (ArgStr[1].str.p_str[0] == '"') ? ArgStr[1].str.p_str + 1
                                                : ArgStr[1].str.p_str,
                STRINGSIZE);
...
static void CodeELSEIF(void) {
    ...
    if (!FirstIfSave || (FirstIfSave->State != IfState_IFIF)) {
        WrError(ErrNum_MissingIf);
    } else if (ArgCnt == 0) {
...
static void CodeENDIF(void) {
    if (!ChkArgCnt(0, 0))
        ;
            ...
            !FirstIfSave
            || ((FirstIfSave->State != IfState_IFIF)
                && (FirstIfSave->State != IfState_IFELSE))) {
        WrError(ErrNum_MissingIf);
    } else {
        PIfSave NewSave;
...
static void EvalIfExpression(tStrComp const* pCond, TempResult* erg) {
            ...
            if (!FirstIfSave->SaveIfAsm) {
                ...
                do {
                    ...
                    if (eq) {
                        ...
                        case TempFloat:
                            eq = (t.Contents.Float
                                  == FirstIfSave->SaveExpr.Contents.Float);
                            break;
                        case TempString:
                            eq = (as_nonz_dynstr_cmp(
                                          &t.Contents.str,
                                          &FirstIfSave->SaveExpr.Contents.str)
                                  == 0);
                            break;
                        default:
                            eq = False;
...
static void CodeENDCASE(void) {
    if (!ChkArgCnt(0, 0))
        ...
        if ((FirstIfSave->State != IfState_CASESWITCH)
            ...
            IfAsm = FirstIfSave->SaveIfAsm;
            if (!FirstIfSave->CaseFound) {
                WrError(ErrNum_NoCaseHit);
            }
            NewSave     = FirstIfSave;
            FirstIfSave = NewSave->Next;
            as_snprintf(ListLine, STRINGSIZE, "[%u]", (unsigned)NewSave->StartLine);
...
Boolean CodeIFs(void) {
    ...
    case 'I':
        if (Memo("IF")) {
            ...
            CodeIFUSED(False);
        } else if (Memo("IFNUSED")) {
            CodeIFUSED(True);
        } else if (Memo("IFEXIST")) {
            CodeIFEXIST(False);
        } else if (Memo("IFNEXIST")) {
            CodeIFEXIST(True);
        } else if (Memo("IFB")) {
            CodeIFB(False);
        } else if (Memo("IFNB")) {
        ...
        break;
    case 'E':
        if ((Memo("ELSE")) || (Memo("ELSEIF")) || (Memo("ELSEC"))) {
            CodeELSEIF();
        } else if ((Memo("ENDIF")) || (Memo("ENDC"))) {
            CodeENDIF();
        } else if (Memo("ELSECASE")) {
            CodeELSECASE();
        } else if (Memo("ENDCASE")) {
            CodeENDCASE();
        } else {
            Result = False;
        }
        break;
    case 'S':
        if (Memo("SWITCH") && !SwitchIsOccupied) {
            CodeSWITCH();
        } else if (Memo("SELECT") && SwitchIsOccupied) {
            CodeSWITCH();
        } else {
            Result = False;
        }
