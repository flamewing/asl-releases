...
static void WrPatches(void) {
    ...
    if (PatchList || ExportList) {
        ...
        if (fwrite(&T8, 1, 1, PrgFile) != 1) {
            ChkIO(ErrNum_FileWriteError);
        }
        if (!Write4(PrgFile, &Cnt)) {
            ChkIO(ErrNum_FileWriteError);
        }
        if (!Write4(PrgFile, &ExportCnt)) {
            ChkIO(ErrNum_FileWriteError);
        }
        if (!Write4(PrgFile, &StrLen)) {
            ChkIO(ErrNum_FileWriteError);
        }
        ...
        StrLen = 0;
        for (PatchLast = PatchList; PatchLast; PatchLast = PatchLast->Next) {
            if (!Write8(PrgFile, &(PatchLast->Address))) {
                ChkIO(ErrNum_FileWriteError);
            }
            if (!Write4(PrgFile, &StrLen)) {
                ChkIO(ErrNum_FileWriteError);
            }
            if (!Write4(PrgFile, &(PatchLast->RelocType))) {
                ChkIO(ErrNum_FileWriteError);
            }
        ...
        for (ExportLast = ExportList; ExportLast; ExportLast = ExportLast->Next) {
            ...
            if (!Write4(PrgFile, &(ExportLast->Flags))) {
                ChkIO(ErrNum_FileWriteError);
            }
            if (!Write8(PrgFile, &(ExportLast->Value))) {
                ChkIO(ErrNum_FileWriteError);
            }
        ...
        while (PatchList) {
            PatchLast = PatchList;
            if (fwrite(PatchLast->Ref, 1, PatchLast->len, PrgFile) != PatchLast->len) {
                ChkIO(ErrNum_FileWriteError);
            }
            free(PatchLast->Ref);
        ...
        while (ExportList) {
            ...
            if (fwrite(ExportLast->Name, 1, ExportLast->len, PrgFile)
                != ExportLast->len) {
                ChkIO(ErrNum_FileWriteError);
            }
...
static void WrRecHeader(void) {
    ...
    if (fwrite(&b, 1, 1, PrgFile) != 1) {
        ChkIO(ErrNum_FileWriteError);
    }
    if (fwrite(&HeaderID, 1, 1, PrgFile) != 1) {
        ChkIO(ErrNum_FileWriteError);
    }
    b = ActPC;
    if (fwrite(&b, 1, 1, PrgFile) != 1) {
...
void NewRecord(LargeWord NStart) {
    ...
    if (LenSoFar == 0) {
        if (fseek(PrgFile, RecPos, SEEK_SET) != 0) {
    ...
    /* otherwise full record */
        ...
        if (PatchList || ExportList) {
            ...
            if (fwrite(&Header, 1, 1, PrgFile) != 1) {
                ChkIO(ErrNum_FileWriteError);
            }
        }
        ...
        fflush(PrgFile);
        if (fseek(PrgFile, LenPos, SEEK_SET) != 0) {
            ChkIO(ErrNum_FileReadError);
        }
        if (!Write2(PrgFile, &LenSoFar)) {
            ChkIO(ErrNum_FileWriteError);
...
void CloseFile(void) {
    ...
    if (StartAdrPresent) {
        ...
        if (fwrite(&Head, sizeof(Head), 1, PrgFile) != 1) {
            ChkIO(ErrNum_FileWriteError);
        }
        Adr = StartAdr;
        if (!Write4(PrgFile, &Adr)) {
            ChkIO(ErrNum_FileWriteError);
        }
    }
    ...
    if (fwrite(h, 1, strlen(h), PrgFile) != strlen(h)) {
        ChkIO(ErrNum_FileWriteError);
    }
    fclose(PrgFile);
}
