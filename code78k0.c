...
static void DecodeAdr(tStrComp const* pArg, Word Mask) {
    static char const RegNames[8][2] = {"X", "A", "C", "B", "E", "D", "L", "H"};
    ...
    if (as_toupper(*pArg->str.p_str) == 'R') {
        if ((strlen(pArg->str.p_str) == 2) && (pArg->str.p_str[1] >= '0')
            && (pArg->str.p_str[1] <= '7')) {
            AdrMode = ModReg8;
            AdrPart = pArg->str.p_str[1] - '0';
            goto chk;
        } else if (
                (ArgLen == 3) && (as_toupper(pArg->str.p_str[1]) == 'P')
                && (pArg->str.p_str[2] >= '0') && (pArg->str.p_str[2] <= '3')) {
            AdrMode = ModReg16;
    ...
    if ((*pArg->str.p_str == '[') && (pArg->str.p_str[ArgLen - 1] == ']')) {
        ...
        if ((!as_strcasecmp(Arg.str.p_str, "DE"))
            || (!as_strcasecmp(Arg.str.p_str, "RP2"))) {
            AdrMode = ModIReg;
            AdrPart = 0;
        } else if (
                (!as_strncasecmp(Arg.str.p_str, "HL", 2))
                && (!as_strncasecmp(Arg.str.p_str, "RP3", 3))) {
            ...
            if ((!as_strcasecmp(Arg.str.p_str, "+B"))
                || (!as_strcasecmp(Arg.str.p_str, "+R3"))) {
                AdrMode = ModIndex;
                AdrPart = 1;
            } else if (
                    (!as_strcasecmp(Arg.str.p_str, "+C"))
                    || (!as_strcasecmp(Arg.str.p_str, "+R2"))) {
                AdrMode = ModIndex;
                AdrPart = 0;
            } else {
    ...
    if (OK) {
        if ((!LongFlag) && (Mask & MModShort) && (AdrWord >= 0xfe20)
            && (AdrWord <= 0xff1f)) {
            AdrMode    = ModShort;
            AdrCnt     = 1;
            AdrVals[0] = Lo(AdrWord);
        } else if (
                (!LongFlag) && (Mask & MModSFR)
                && (((AdrWord >= 0xff00) && (AdrWord <= 0xffcf))
                    || (AdrWord >= 0xffe0))) {
            AdrMode    = ModSFR;
            AdrCnt     = 1;
            AdrVals[0] = Lo(AdrWord);
...
static Boolean DecodeBitAdr(tStrComp const* pArg, Byte* Erg) {
    char*    p;
    Boolean  OK;
    tStrComp Reg, Bit;
...
static void DecodeMOV(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                &ArgStr[1], MModReg8 | MModShort | MModSFR | MModAbs | MModIReg
                                    | MModIndex | MModDisp);
        switch (AdrMode) {
        case ModReg8:
            HReg = AdrPart;
            DecodeAdr(
                    &ArgStr[2],
                    MModImm | MModReg8
                            | ((HReg == AccReg) ? MModShort | MModSFR | MModAbs | MModIReg
                                                          | MModIndex | MModDisp
                                                : 0));
            switch (AdrMode) {
            case ModReg8:
                if ((HReg == AccReg) == (AdrPart == AccReg)) {
                    WrError(ErrNum_InvAddrMode);
                } else if (HReg == AccReg) {
                    CodeLen     = 1;
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (AdrMode != ModNone) {
            if (AdrPart != AccReg) {
                ...
                DecodeAdr(
                        pArg2, MModReg8 | MModShort | MModSFR | MModAbs | MModIReg
                                       | MModIndex | MModDisp);
                switch (AdrMode) {
                case ModReg8:
                    if (AdrPart == AccReg) {
...
static void DecodeMOVW(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg16:
            ...
            switch (AdrMode) {
            case ModReg16:
                if ((HReg == AccReg16) == (AdrPart == AccReg16)) {
                    WrError(ErrNum_InvAddrMode);
        ...
        if (AdrMode == ModReg16) {
            ...
            if (AdrMode == ModReg16) {
                if ((HReg == AccReg16) == (AdrPart == AccReg16)) {
                    ...
                    BAsmCode[0] = (HReg == AccReg16) ? 0xe0 + (AdrPart << 1)
                                                     : 0xe0 + (HReg << 1);
                    CodeLen     = 1;
                }
            }
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg8:
            ...
            DecodeAdr(
                    &ArgStr[2],
                    MModReg8
                            | ((HReg == AccReg) ? (MModImm | MModShort | MModAbs
...
static void DecodeMOV1(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (as_strcasecmp(pArg1->str.p_str, "CY")) {
            WrError(ErrNum_InvAddrMode);
...
static void DecodeCALLF(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        AdrWord = EvalStrIntExpressionOffs(
                &ArgStr[1], !!(*ArgStr[1].str.p_str == '!'), UInt11, &OK);
        if (OK) {
...
static void DecodeCALLT(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if ((*ArgStr[1].str.p_str != '[') || (ArgStr[1].str.p_str[l - 1] != ']')) {
        WrError(ErrNum_InvAddrMode);
    } else {
        tSymbolFlags Flags;
...
static void DecodeBR(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
            ...
            (!as_strcasecmp(ArgStr[1].str.p_str, "AX"))
            || (!as_strcasecmp(ArgStr[1].str.p_str, "RP0"))) {
        BAsmCode[0] = 0x31;
        BAsmCode[1] = 0x98;
        CodeLen     = 2;
    } else {
        ...
        AdrWord = EvalStrIntExpressionOffsWithFlags(
                &ArgStr[1], Offset, UInt16, &OK, &Flags);
        if (OK) {
            if (HReg == 0) {
...
static void DecodeRel(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        AdrInt = EvalStrIntExpressionOffsWithFlags(
                         &ArgStr[1], ('$' == *ArgStr[1].str.p_str), UInt16, &OK, &Flags)
                 - (EProgCounter() + 2);
        if (OK) {
            if (((AdrInt < -128) || (AdrInt > 127)) && !mSymbolQuestionable(Flags)) {
                WrError(ErrNum_JmpDistTooBig);
...
static void DecodeBRel(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        AdrInt = EvalStrIntExpressionOffsWithFlags(
                         &ArgStr[2], !!(*ArgStr[2].str.p_str == '$'), UInt16, &OK, &Flags)
                 - (EProgCounter() + HReg + 1);
        if (OK) {
            if (((AdrInt < -128) || (AdrInt > 127)) & !mSymbolQuestionable(Flags)) {
                WrError(ErrNum_JmpDistTooBig);
...
static void DecodeDBNZ(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(&ArgStr[1], MModReg8 + MModShort);
        if ((AdrMode == ModReg8) && ((AdrPart & 6) != 2)) {
            WrError(ErrNum_InvAddrMode);
...
static void DecodeSEL(Word Index) {
    ...
    if (ArgCnt != 1) {
        WrError(ErrNum_InvAddrMode);
            ...
            (strlen(ArgStr[1].str.p_str) != 3)
            || (as_strncasecmp(ArgStr[1].str.p_str, "RB", 2) != 0)) {
        WrError(ErrNum_InvAddrMode);
    } else {
