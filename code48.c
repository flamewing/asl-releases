...
static Boolean DecodeRegCore(char const* pAsc, tRegInt* pValue, tSymbolSize* pSize) {
    if ((strlen(pAsc) != 2) || (as_toupper(pAsc[0]) != 'R') || (!isdigit(pAsc[1]))) {
        return False;
    }
...
static tRegEvalResult DecodeReg(tStrComp const* pArg, Byte* pValue, Boolean MustBeReg) {
    tSymbolSize    Size;
    tRegDescr      RegDescr;
...
static Boolean IsPort(char const* pArg, Word PortMask, Byte* pPortNum) {
    if (!as_strcasecmp(pArg, "BUS")) {
        *pPortNum = 8;
    } else if ((strlen(pArg) == 2) && (as_toupper(pArg[0]) == 'P') && isdigit(pArg[1])) {
        *pPortNum = pArg[1] - '0';
    } else {
        return False;
    }
...
static Boolean IsSerialPort(char const* pArg, Word PortMask, Byte* pPortNum) {
    if ((strlen(pArg) == 2) && (as_toupper(pArg[0]) == 'S') && isdigit(pArg[1])) {
        *pPortNum = pArg[1] - '0';
    } else {
        return False;
    }
...
static tAdrMode DecodeAdr(tStrComp const* pArg, unsigned Mask) {
    Boolean OK;
...
static Boolean AChkCPUFlags(LongWord CPUFlags, tStrComp const* pArg) {
    if (pCurrCPUProps->Flags & CPUFlags) {
        return True;
    }
    WrStrErrorPos(ErrNum_InvAddrMode, pArg);
...
static void ChkPx(Byte PortNum, tStrComp const* pArg) {
...
static Boolean IsIReg3(tStrComp const* pArg) {
    tStrComp Arg;
    Byte     RegNum;
...
static void DecodeANL_ORL_XRL(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        if (Code == 0x90) {
            WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]); /* no XRL to ports */
...
static void DecodeCALL_JMP(Word Code) {
    if (!ChkArgCnt(1, 1))
        ;
    else if ((EProgCounter() & 0x7fe) == 0x7fe) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        do {
            if (!strcmp(ClrCplVals[z], ArgStr[1].str.p_str)) {
                if ((*ArgStr[1].str.p_str == 'F')
                    && !(pCurrCPUProps->Flags & eCPUFlagUserFlags)) {
                    WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
                } else {
                    CodeLen     = 1;
...
static void DecodeENT0(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkCPUFlags(eCPUFlagT0CLK)) {
        if (as_strcasecmp(ArgStr[1].str.p_str, "CLK")) {
            WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
        } else {
            CodeLen     = 1;
            BAsmCode[0] = 0x75;
...
static void DecodeIN(Word Code) {
    ...
    } else if (!IsPort(ArgStr[2].str.p_str, 0x07, &PortNum)) {
...
static void DecodeINS(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
    } else if (as_strcasecmp(ArgStr[2].str.p_str, "BUS")) {
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[2]);
...
static void DecodeCond(Word Code) {
    if (ChkArgCnt(1, 1)) {
        ...
        if (EvalResult.OK
            && ChkSamePage(EProgCounter() + 1, AdrWord, 8, EvalResult.Flags)) {
            CodeLen     = 2;
            BAsmCode[0] = Code;
...
static void DecodeANLD_ORLD_MOVD(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkCPUFlags(eCPUFlagIOExpander)) {
        ...
        } else if (!IsPort(pArg1->str.p_str, 0xf0, &PortNum)) {
            WrStrErrorPos(ErrNum_InvAddrMode, pArg1);
        } else {
            PortNum -= 4;

            if ((PortNum == 3) && (pCurrCPUProps->Flags & eCPUFlagSiemens)) {
                WrStrErrorPos(ErrNum_InvAddrMode, pArg2);
            } else {
                CodeLen     = 1;
...
static void DecodeMOVP1(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        ;
    else if (as_strcasecmp(ArgStr[1].str.p_str, "P")) {
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
...
static void DecodeMOVX(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkCPUFlags(eCPUFlagXMem)) {
        tStrComp const *pArg1 = &ArgStr[1], *pArg2 = &ArgStr[2];
        Byte            Code = 0x80;
...
static void DecodeOUT(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if (as_strcasecmp(ArgStr[1].str.p_str, "DBB")) {
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
    } else if (as_strcasecmp(ArgStr[2].str.p_str, "A")) {
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[2]);
...
static void DecodeRET_RETR(Word Code) {
    if (ChkArgCnt(0, 0)) {
        ...
        if ((Code == 0x93)
            && (!(pCurrCPUProps->Flags & eCPUFlagINTLogic)
                || (pCurrCPUProps->Flags & eCPUFlagADConv))) {
            WrStrErrorPos(ErrNum_InstructionNotSupported, &OpPart);
        } else {
            CodeLen     = 1;
            BAsmCode[0] = Code;
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        for (z = 0; z < SelOrderCnt; z++) {
            if (!strcmp(ArgStr[1].str.p_str, SelOrders[z].Name)) {
                ...
                if (!strncmp(SelOrders[z].Name, "MB", 2)
                    && (pCurrCPUProps->CodeSize <= 0x7ff))
                    ;
                else if (
                        !strcmp(SelOrders[z].Name, "MB2")
                        && (pCurrCPUProps->CodeSize <= 0xfff))
                    ;
                else if (
                        !strcmp(SelOrders[z].Name, "MB32")
                        && (pCurrCPUProps->CodeSize <= 0x17ff))
                    ;

                else if (
                        !strncmp(SelOrders[z].Name, "RB", 2)
                        && !(pCurrCPUProps->Flags & eCPUFlagRegBanks))
                    ;

                else if (
                        !strncmp(SelOrders[z].Name, "AN", 2)
                        && !(pCurrCPUProps->Flags & eCPUFlagADConv))
                    ;
...
static void DecodeSTOP(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if (as_strcasecmp(ArgStr[1].str.p_str, "TCNT")) {
        WrStrErrorPos(ErrNum_InvAddrMode, &ArgStr[1]);
    } else {
        CodeLen     = 1;
        BAsmCode[0] = 0x65;
...
static void DecodeIDL_HALT(Word Code) {
    ...
    if (ChkArgCnt(0, 0) && ChkCPUFlags(eCPUFlagCMOS)) {
        CodeLen     = 1;
...
static void AddAcc(char const* Name, Byte Code) {
    AddInstTable(InstTable, Name, Code, DecodeAcc);
}

static void AddCond(char const* Name, Byte Code) {
    AddInstTable(InstTable, Name, Code, DecodeCond);
}

static void AddSel(char const* Name, Byte Code) {
...
static void InitFields(void) {
    ...
    ClrCplVals     = (char const**)malloc(sizeof(char*) * ClrCplCnt);
...
static tCPUProps CPUProps[] = {
        ...
        {    "8022",  0x7ff,
         eCPUFlagPort0 | eCPUFlagPort1 | eCPUFlagPort2 | eCPUFlagT0 | eCPUFlagBUS
         | eCPUFlagADConv | eCPUFlagINTLogic                              },
        {    "8401", 0x1fff,
         eCPUFlagDEC_DJNZ_IREG | eCPUFlagPort0 | eCPUFlagPort1 | eCPUFlagPort2
         | eCPUFlagT0 | eCPUFlagCondBitJmp | eCPUFlagTransferA_PSW
         | eCPUFlagRegBanks | eCPUFlagLogToPort | eCPUFlagDEC_REG
         | eCPUFlagINTLogic | eCPUFlagSerial | eCPUFlag84xx               },
        {    "8421",  0x7ff,
         eCPUFlagDEC_DJNZ_IREG | eCPUFlagPort0 | eCPUFlagPort1 | eCPUFlagPort2
         | eCPUFlagT0 | eCPUFlagCondBitJmp | eCPUFlagTransferA_PSW
         | eCPUFlagRegBanks | eCPUFlagLogToPort | eCPUFlagDEC_REG
        ...
        {    "8461", 0x17ff,
         ...
         | eCPUFlagINTLogic | eCPUFlagSerial | eCPUFlag84xx               },
        {    "8039",  0xfff,
         eCPUFlagXMem | eCPUFlagPort1 | eCPUFlagPort2 | eCPUFlagIOExpander
         | eCPUFlagUserFlags | eCPUFlagT0 | eCPUFlagT0CLK | eCPUFlagCondBitJmp
         | eCPUFlagTransferA_PSW | eCPUFlagRegBanks | eCPUFlagLogToPort
         | eCPUFlagDEC_REG | eCPUFlagMOVP3 | eCPUFlagINTLogic             },
        {    "8048",  0xfff,
         eCPUFlagXMem | eCPUFlagPort1 | eCPUFlagPort2 | eCPUFlagIOExpander
         | eCPUFlagUserFlags | eCPUFlagT0 | eCPUFlagT0CLK | eCPUFlagCondBitJmp
         | eCPUFlagTransferA_PSW | eCPUFlagBUS | eCPUFlagRegBanks
         | eCPUFlagLogToPort | eCPUFlagDEC_REG | eCPUFlagMOVP3
         | eCPUFlagINTLogic                                               },
        {   "80C39",  0xfff,
         eCPUFlagCMOS | eCPUFlagXMem | eCPUFlagPort1 | eCPUFlagPort2 | eCPUFlagIOExpander
         | eCPUFlagUserFlags | eCPUFlagT0 | eCPUFlagT0CLK | eCPUFlagCondBitJmp
         | eCPUFlagTransferA_PSW | eCPUFlagRegBanks | eCPUFlagLogToPort
         | eCPUFlagDEC_REG | eCPUFlagMOVP3 | eCPUFlagINTLogic             },
        {   "80C48",  0xfff,
         ...
         | eCPUFlagINTLogic                                               },
        {    "8041",  0x3ff,
         eCPUFlagUPIPort | eCPUFlagPort1 | eCPUFlagPort2 | eCPUFlagIOExpander
         | eCPUFlagUserFlags | eCPUFlagT0 | eCPUFlagCondBitJmp
         | eCPUFlagTransferA_PSW | eCPUFlagBUS | eCPUFlagRegBanks
         | eCPUFlagLogToPort | eCPUFlagDEC_REG | eCPUFlagMOVP3
         | eCPUFlagINTLogic                                               },
        {    "8042",  0x7ff,
         ...
         | eCPUFlagINTLogic                                               },
        {  "80C382",  0xfff,
         eCPUFlagCMOS | eCPUFlagSiemens | eCPUFlagDEC_DJNZ_IREG | eCPUFlagXMem
         | eCPUFlagPort1 | eCPUFlagIOExpander | eCPUFlagT0 | eCPUFlagCondBitJmp
         | eCPUFlagBUS | eCPUFlagRegBanks | eCPUFlagLogToPort | eCPUFlagDEC_REG
         | eCPUFlagMOVP3 | eCPUFlagINTLogic                               },
        {"MSM80C39",  0xfff,
         eCPUFlagCMOS | eCPUFlagDEC_DJNZ_IREG | eCPUFlagXMem | eCPUFlagPort1
         | eCPUFlagPort2 | eCPUFlagIOExpander | eCPUFlagUserFlags | eCPUFlagT0
         | eCPUFlagT0CLK | eCPUFlagCondBitJmp | eCPUFlagTransferA_PSW
         | eCPUFlagRegBanks | eCPUFlagLogToPort | eCPUFlagDEC_REG | eCPUFlagMOVP3
         | eCPUFlagINTLogic | eCPUFlagOKI                                 },
        {"MSM80C48",  0xfff,
         eCPUFlagCMOS | eCPUFlagDEC_DJNZ_IREG | eCPUFlagXMem | eCPUFlagPort1
         | eCPUFlagPort2 | eCPUFlagIOExpander | eCPUFlagUserFlags | eCPUFlagT0
         | eCPUFlagT0CLK | eCPUFlagCondBitJmp | eCPUFlagTransferA_PSW
         | eCPUFlagBUS | eCPUFlagRegBanks | eCPUFlagLogToPort | eCPUFlagDEC_REG
         | eCPUFlagMOVP3 | eCPUFlagINTLogic | eCPUFlagOKI                 },
        {      NULL,      0,                                             0}
};
