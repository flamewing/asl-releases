...
static ShortInt DecodeIndReg16(char const* pAsc) {
    int  l = strlen(pAsc);
    char Copy[10];
    ...
    memcpy(Copy, pAsc + 1, l - 2);
    Copy[l - 2] = '\0';
    l           = DecodeReg16(Copy);
    return l;
}

static ShortInt DecodeRegBank(char const* pAsc) {
    if ((strlen(pAsc) == 3) && (toupper(pAsc[0]) == 'R') && (toupper(pAsc[1]) == 'B')
        && (pAsc[2] >= '0') && (pAsc[2] <= '7')) {
        return pAsc[2] - '0';
    } else {
        return -1;
...
static ShortInt DecodeAdr(tStrComp const* pArg, Word Mask, tAdrResult* pResult) {
    ...
    pResult->Cnt       = 0;
    pResult->ForceLong = pResult->ForceRel = False;
    ...
    if ((ArgLen >= 2) && (pArg->str.p_str[ArgLen - 1] == ']')) {
        ...
        /* no -> with outer displacement */
            ...
            if (OK) {
                pResult->Mode    = ModMem;
                pResult->Val     = 0x0a;
                pResult->Vals[1] = Lo(WordOp);
                pResult->Vals[2] = Hi(WordOp);
                pResult->Cnt     = 3;
                goto AdrFound;
            }
        }
    ...
    WordOp = EvalStrIntExpressionOffsWithFlags(
            pArg, Offset, UInt16, &OK, &pResult->ValSymFlags);
    if (OK) {
        if ((Mask & MModShort) && (!pResult->ForceLong)
            && ((WordOp >= 0xfe20) && (WordOp <= 0xff1f))) {
            pResult->Mode    = ModShort;
            pResult->Cnt     = 1;
            pResult->Vals[0] = Lo(WordOp);
...
static void AppendDisp(tStrComp const* pArg) {
    Boolean      OK;
    LongInt      Dist;
    tSymbolFlags Flags;
...
static void DecodeMOV(Word Is16) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1],
                MModReg16 | MModSP | MModShort | MModSFR
                        | (Is16 ? 0
                                : (MModReg8 | MModSTBC | MModWDM | MModAbs
                                   | MModShortIndir | MModMem)),
                &DestAdrResult)) {
        case ModReg8: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2],
                    MModImm | MModReg8
                            | ((DestAdrResult.Val == AccReg8())
                                       ? (MModMem | MModAbs | MModShort | MModSFR
                                          | MModShortIndir)
                                       : 0),
                    &SrcAdrResult)) {
            case ModImm:
                if (DestAdrResult.Val >= 8) {
                    WrError(ErrNum_InvAddrMode);
                } else {
            ...
            case ModMem:
                if ((SrcAdrResult.Val == 0x16) && (SrcAdrResult.Vals[0] <= 5)) {
                    ...
                    memcpy(BAsmCode + CodeLen, SrcAdrResult.Vals + 1,
                           SrcAdrResult.Cnt - 1);
        ...
        case ModReg16: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2],
                    MModReg16 | MModImm
                            | ((DestAdrResult.Val == AccReg16())
                                       ? (MModShort | MModSFR | MModSP)
                                       : 0),
                    &SrcAdrResult)) {
            case ModReg16:
                BAsmCode[CodeLen++] = 0x24;
        ...
        case ModShort: {
            ...
            AssumeByte = (OpSize == -1);
            switch (DecodeAdr(
                    &ArgStr[2], MModImm | MModShort | MModReg8 | MModReg16,
                    &SrcAdrResult)) {
            case ModImm:
        ...
        case ModSFR: {
            ...
            AssumeByte = (OpSize == -1);
            switch (DecodeAdr(
                    &ArgStr[2], MModImm | MModReg8 | MModReg16, &SrcAdrResult)) {
            case ModImm:
        ...
        case ModMem: {
            ...
            case ModReg8:
                if (SrcAdrResult.Val != AccReg8()) {
                    ...
                    memcpy(BAsmCode + CodeLen, DestAdrResult.Vals + 1,
                           DestAdrResult.Cnt - 1);
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg8: {
            ...
            switch (DecodeAdr(
                    ...
                    MModReg8
                            | ((DestAdrResult.Val == AccReg8())
                                       ? (MModMem | MModShort | MModShortIndir | MModSFR)
                                       : 0),
                    &SrcAdrResult)) {
            case ModReg8:
                if ((DestAdrResult.Val == AccReg8()) && (SrcAdrResult.Val < 8)) {
                    ...
                    BAsmCode[CodeLen++]
                            = 0x00 | (SrcAdrResult.Val << 4) | DestAdrResult.Val;
                } else if (SrcAdrResult.Val < 8) {
                    BAsmCode[CodeLen++] = 0x25;
                    BAsmCode[CodeLen++]
                            = 0x00 | (DestAdrResult.Val << 4) | SrcAdrResult.Val;
                } else {
        ...
        case ModReg16: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2],
                    MModReg16
                            | ((DestAdrResult.Val == AccReg16()) ? (MModShort | MModSFR)
                                                                 : 0),
                    &SrcAdrResult)) {
            case ModReg16:
                BAsmCode[CodeLen++] = 0x25;
        ...
        case ModShort: {
            ...
            switch (DecodeAdr(
                    &ArgStr[2], MModReg8 | MModReg16 | MModShort, &SrcAdrResult)) {
            case ModReg8:
                if (SrcAdrResult.Val != AccReg8()) {
                    WrError(ErrNum_InvAddrMode);
                } else {
...
static void DecodeALU(Word Props) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1],
                MModShort | MModSFR | (May16 ? MModReg16 : 0)
        ...
        case ModReg8: {
            ...
            switch (DecodeAdr(
                    ...
                    MModReg8
                            | ((DestAdrResult.Val == AccReg8())
                                       ? (MModShort | MModSFR | MModMem | MModImm)
                                       : 0),
                    &SrcAdrResult)) {
            case ModReg8:
                if (SrcAdrResult.Val >= 8) {
...
static void DecodeMULDIV(Word Props) {
    if (ChkArgCnt(1, 1)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1], MModReg16 | ((OpSize == 1) ? 0 : MModReg8), &AdrResult)) {
        case ModReg8:
            if (AdrResult.Val > 7) {
                WrError(ErrNum_InvAddrMode);
            } else {
...
static void DecodeINCDEC(Word Props) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        switch (DecodeAdr(
                &ArgStr[1],
                MModShort | MModReg16 | MModSP | ((OpSize == 1) ? 0 : MModReg8),
                &AdrResult)) {
        case ModReg8:
            if (AdrResult.Val > 7) {
                WrError(ErrNum_InvAddrMode);
...
static void DecodeShift(Word Props) {
    ...
    switch (DecodeAdr(
            &ArgStr[1],
            ((OpSize != 1) ? MModReg8 : 0) | ((Props & 0x4000) ? MModReg16 : 0),
            &AdrResult)) {
    case ModReg8:
        if (AdrResult.Val > 7) {
            WrError(ErrNum_InvAddrMode);
...
static void DecodeBIT(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && DecodeBitAdr(&ArgStr[1], &Result)) {
...
static void DecodeCALLT(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if ((*ArgStr[1].str.p_str != '[') || (ArgStr[1].str.p_str[l - 1] != ']')) {
            WrError(ErrNum_InvAddrMode);
        } else {
            tStrComp     Arg;
            tSymbolFlags Flags;
    ...
    if (ChkArgCnt(1, ArgCntMax)) {
        ...
        for (z = 1; z <= ArgCnt; z++) {
            ...
            /* user stack ptr itself cannot be pushed onto user stack */
                    ...
                    ((Reg = DecodeReg16(ArgStr[z].str.p_str)) < 0)
                    || (IsU && (Reg == 5))) {
...
static void DecodeDBNZ(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg8:
            if ((AdrResult.Val != CReg8()) && (AdrResult.Val != BReg8())) {
...
static void DecodeBRKCS(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ;
    else if ((Bank = DecodeRegBank(ArgStr[1].str.p_str)) < 0) {
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else {
        BAsmCode[CodeLen++] = 0x05;
...
static void DecodeString1(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        case ModMem:
            if ((AdrResult.Val != 0x16) || (AdrResult.Vals[0] & 13)) {
                WrError(ErrNum_InvAddrMode);
            } else {
...
static void DecodeString2(Word Code) {
    ...
    if ((SrcAdrResult.Val != 0x16)
        ...
        || ((DestAdrResult.Vals[0] ^ SrcAdrResult.Vals[0])
            & 2)) /* match [DE+] with [HL+] and [DE-] with [HL-] */
    {
        WrError(ErrNum_InvAddrMode);
        return;
    }
...
static void DecodeSEL(Word Code) {
    ...
    if (!ChkArgCnt(1, 2))
        ...
        WrStrErrorPos(ErrNum_InvReg, &ArgStr[1]);
    } else if ((ArgCnt == 2) && (as_strcasecmp(ArgStr[2].str.p_str, "ALT"))) {
...
static void SwitchTo_78K3(void) {
    static ASSUMERec const ASSUME78K3s[] = {
