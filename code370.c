...
static void DecodeAdrRel(tStrComp const* pArg, Word Mask, Boolean AddrRel) {
    Integer HVal;
    char*   p;
    Boolean OK;
...
static void DissectBit_370(char* pDest, size_t DestSize, LargeWord Symbol) {
    ...
    if (Addr < 2) {
        ...
        as_snprintf(
                pDest, DestSize, "%0.*u%s", ListRadixBase, (unsigned)Addr,
...
static Boolean DecodeBitExpr(int Start, int Stop, LongWord* pResult) {
    ...
    if (Start == Stop) {
        *pResult = EvalStrIntExpression(&ArgStr[Start], UInt19, &OK);
        ...
        if ((!as_strcasecmp(ArgStr[Stop].str.p_str, "A"))
            || (!as_strcasecmp(ArgStr[Stop].str.p_str, "B"))) {
...
static void DecodeMOV(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        DecodeAdr(
                &ArgStr[2], MModAccB + MModReg + MModPort + MModAbs + MModIReg + MModBRel
                                    + MModSPRel + MModRegRel + MModAccA);
        switch (AdrType) {
        case ModAccA:
            DecodeAdr(
                    &ArgStr[1], MModReg + MModAbs + MModIReg + MModBRel + MModRegRel
...
static void DecodeRel8(Word Code) {
    if (ChkArgCnt(1, 1)) {
        Boolean      OK;
        tSymbolFlags Flags;
        Integer AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[1], Int16, &OK, &Flags)
                         - (EProgCounter() + 2);
...
static void DecodeCMP(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModAccA:
            DecodeAdr(
                    &ArgStr[1], MModAbs + MModIReg + MModBRel + MModRegRel + MModSPRel
                                        + MModAccB + MModReg + MModImm);
            switch (AdrType) {
            case ModAbs:
                BAsmCode[0] = 0x8d;
                memcpy(BAsmCode + 1, AdrVals, 2);
...
static void DecodeABReg(Word Code) {
    ...
    if (!ChkArgCnt(1 + IsDJNZ, 1 + IsDJNZ))
        ...
        if ((IsDJNZ) && (CodeLen != 0)) {
            Boolean      OK;
            tSymbolFlags Flags;
            Integer AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[2], Int16, &OK, &Flags)
                             - (EProgCounter() + CodeLen + 1);
...
static void DecodeBit(Word Code) {
    ...
    if (ChkArgCnt(1 + Rela, 2 + Rela) && DecodeBitExpr(1, ArgCnt - Rela, &BitExpr)) {
        Boolean OK;
        Word    Addr;
        Byte    Bit;
...
static void InitBit(char const* NName, Word NCode) {
...
static void InitFields(void) {
    ...
    InitALU2("AND", 3);
    InitALU2("BTJO", 0x0106);
    InitALU2("BTJZ", 0x0107);
    InitALU2("OR", 4);
    InitALU2("XOR", 5);
    ...
    InitABReg("INC", 3);
    InitABReg("INV", 4);
    InitABReg("POP", 0x0209);
    InitABReg("PUSH", 0x0208);
    InitABReg("RL", 14);
    InitABReg("RLC", 15);
    InitABReg("RR", 12);
    InitABReg("RRC", 13);
    InitABReg("SWAP", 7);
    InitABReg("XCHB", 6);
    InitABReg("DJNZ", 0x010a);

    InitBit("CMPBIT", 5);
    InitBit("JBIT0", 0x0107);
    InitBit("JBIT1", 0x0106);
    InitBit("SBIT0", 3);
    InitBit("SBIT1", 4);
}
