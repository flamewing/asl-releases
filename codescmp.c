...
static Boolean DecodeReg(char const* pArg, Byte* pResult) {
    int l = strlen(pArg);
...
static Boolean DecodeAdr(tStrComp const* pArg, Boolean MayInc, Byte PCDisp, Byte* Arg) {
    Word         Target;
    Boolean      OK;
    int          l, SplitPos;
    tSymbolFlags Flags;
...
static void DecodeRegOrder(Word Index) {
    if (!ChkArgCnt(1, 1))
        ;
    else if (!DecodeReg(ArgStr[1].str.p_str, BAsmCode + 0)) {
...
static void AddImm(char const* NName, Byte NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeImm);
}

static void AddReg(char const* NName, Byte NCode) {
    AddInstTable(InstTable, NName, NCode, DecodeRegOrder);
}

static void AddMem(char const* NName, Byte NCode) {
