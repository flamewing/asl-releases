 ...
 */
static void print_doc(char *file_name, size_t file_size, const char *source_dir,
 ...
 */
static void read_flags(char *file_name, size_t file_size,
                       const char *source_dir, char *buffer, size_t size) {
 ...
 */
static void dump_file(const char *data, size_t size) {
...
int main(int argc, char *argv[]) {
  ...
  const char *quiet = verbose == true ? "" : "-q";
  const char *source_dir = convert_path(argv[2]);
  const char *test_name = argv[3];
  const char *include_dir = convert_path(argv[4]);
  const char *asl_path = convert_path(argv[5]);
  ...
  snprintf(io_buffer, sizeof(io_buffer),
           "%s %s -k -l 0 -r 0x-0x %s" PATHSEP "%s", p2bin_path, quiet,
