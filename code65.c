...
static CPUVar CPU6502, CPU65SC02, CPU65C02, CPU65CE02, CPUW65C02S, CPU65C19, CPUM740,
        CPUHUC6280, CPU6502U;
static LongInt SpecPage, RegB, MPR[8];

/*---------------------------------------------------------------------------*/

static unsigned ChkZero(tStrComp const* pArg, Byte* erg) {
    if ((strlen(pArg->str.p_str) > 1)
        && ((*pArg->str.p_str == '<') || (*pArg->str.p_str == '>'))) {
...
static void ChkZeroMode(tAdrResult* pResult, NormOrder const* pOrder, ShortInt ZeroMode) {
...
static Word EvalAddress(tStrComp const* pArg, IntType Type, Boolean* pOK) {
...
static void DecodeAdr(tAdrResult* pResult, NormOrder const* pOrder) {
    Word    AdrWord;
    Boolean ValOK;
    Byte    ZeroMode;
    ...
    if (ArgCnt == 0) {
        ...
        /* 3. Special Page ? */
            ...
            AdrWord = EvalStrIntExpressionOffsWithFlags(
                    &ArgStr[1], 1, UInt16, &ValOK, &Flags);
            if (ValOK) {
                if (mFirstPassUnknown(Flags)) {
                    AdrWord = (SpecPage << 8) | Lo(AdrWord);
                }
                if (Hi(AdrWord) != SpecPage) {
        ...
        /* 4. X-inner-indirekt ? */
                ...
                (strlen(ArgStr[1].str.p_str) >= 5)
                && (!as_strcasecmp(
                        ArgStr[1].str.p_str + strlen(ArgStr[1].str.p_str) - 3, ",X)"))) {
        ...
        if (Indir1 && !as_strcasecmp(ArgStr[2].str.p_str, "Y")
            && (!as_strcasecmp(
                    ArgStr[1].str.p_str + strlen(ArgStr[1].str.p_str) - 4, ",SP)"))) {
            if (*ArgStr[1].str.p_str != '(') {
                WrError(ErrNum_InvAddrMode);
            } else {
                tStrComp DistArg;
                ...
                if (ValOK) {
                    ...
                    if (pResult->ErgMode != -1) {
                        if (IsBasePage(pResult->AdrVals[1]) && (ZeroMode == 0)) {
                            ChkZeroMode(
                                    pResult, pOrder,
                                    (!as_strcasecmp(ArgStr[2].str.p_str, "X")) ? ModZIX
                                                                               : ModZIY);
                        }
                    }
                }
...
static int ImmStart(char const* pArg) {
    return !!(*pArg == '#');
}
...
static void DecodeFixed(Word Index) {
    ...
    if (ChkArgCnt(0, 0) && (ChkExactCPUMask(pOrder->CPUFlag, CPU6502) >= 0)) {
        CodeLen     = 1;
...
static void DecodeSEB_CLB(Word Code) {
    if (ChkArgCnt(2, 2) && ChkExactCPU(CPUM740)) {
        Boolean ValOK;
...
static void DecodeBBC_BBS(Word Code) {
    ...
    if (ChkArgCnt(3, 3) && ChkExactCPU(CPUM740)) {
        ...
        if (ValOK) {
            ...
            if (ValOK) {
                Integer AdrInt
                        = EvalStrIntExpressionWithFlags(&ArgStr[3], Int16, &ValOK, &Flags)
                          - (EProgCounter() + 2 + Ord(b) + Ord(CLI_SEI_Flag));
...
static void DecodeBBR_BBS(Word Code) {
    ...
    if (ChkArgCnt(2, 2)
        && (ChkExactCPUMask(M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_HUC6280, CPU6502)
            >= 0)) {
        Byte     ForceSize;
        ...
        if (ForceSize == 1) {
            ...
            if (ValOK) {
                ...
                BAsmCode[0] = Code;
                AdrInt = EvalStrIntExpressionWithFlags(&ArgStr[2], UInt16, &ValOK, &Flags)
                         - (EProgCounter() + 3);
                if (ValOK) {
                    if (((AdrInt > 127) || (AdrInt < -128))
                        && !mSymbolQuestionable(Flags)) {
...
static void DecodeRBA_SBA(Word Code) {
    if (ChkArgCnt(2, 2) && ChkExactCPU(CPU65C19)) {
        Boolean OK;
        Word    Addr;

        Addr = EvalStrIntExpression(&ArgStr[1], UInt16, &OK);
        if (OK) {
            BAsmCode[3] = EvalStrIntExpressionOffs(
                    &ArgStr[2], ImmStart(ArgStr[2].str.p_str), UInt8, &OK);
            if (OK) {
                BAsmCode[0] = Code;
                BAsmCode[1] = Lo(Addr);
                BAsmCode[2] = Hi(Addr);
...
static void DecodeBAR_BAS(Word Code) {
    if (ChkArgCnt(3, 3) && ChkExactCPU(CPU65C19)) {
        Boolean OK;
        Word    Addr;
        ...
        if (OK) {
            ...
            if (OK) {
                tSymbolFlags Flags;
                Integer      Dist
                        = EvalStrIntExpressionWithFlags(&ArgStr[3], UInt16, &OK, &Flags)
                          - (EProgCounter() + 5);

                if (OK) {
                    if (((Dist > 127) || (Dist < -128)) && !mSymbolQuestionable(Flags)) {
                        WrError(ErrNum_JmpDistTooBig);
                    } else {
                        BAsmCode[0] = Code;
                        BAsmCode[1] = Lo(Addr);
...
static void DecodeSTI(Word Code) {
    ...
    if (ChkArgCnt(2, 2) && ChkExactCPU(CPU65C19)) {
        ...
        if (OK) {
            BAsmCode[2] = EvalStrIntExpressionOffs(
                    &ArgStr[2], ImmStart(ArgStr[2].str.p_str), UInt8, &OK);
            if (OK) {
                BAsmCode[0] = 0xb2;
                CodeLen     = 3;
...
static void DecodeJSB(Word Code) {
    ...
    if (ChkArgCnt(1, 1) && ChkExactCPU(CPU65C19)) {
        Boolean      OK;
        tSymbolFlags Flags;
        Word         Addr;
...
static void DecodeAUG(Word Code) {
    if (ChkArgCnt(0, 0) && ChkExactCPU(CPU65CE02)) {
        BAsmCode[0] = Code;
        BAsmCode[1] = BAsmCode[2] = BAsmCode[3] = 0x00;
        CodeLen                                 = 4;
    }
}

static void DecodeNorm(Word Index) {
    NormOrder const* pOrder = NormOrders + Index;
    tAdrResult       AdrResult;

    DecodeAdr(&AdrResult, pOrder);
    if (AdrResult.ErgMode != -1) {
        if (pOrder->Codes[AdrResult.ErgMode] == -1) {
            if (AdrResult.ErgMode == ModZA) {
                AdrResult.ErgMode = ModA;
            }
            if (AdrResult.ErgMode == ModZIX) {
                AdrResult.ErgMode = ModIX;
            }
            if (AdrResult.ErgMode == ModZIY) {
                AdrResult.ErgMode = ModIY;
            }
            if (AdrResult.ErgMode == ModInd8) {
                AdrResult.ErgMode = ModInd16;
            }
        ...
        if (pOrder->Codes[AdrResult.ErgMode] == -1) {
            ...
            if ((AdrResult.ErgMode == ModInd16) && (MomCPU != CPU65C02)
                && (BAsmCode[1] == 0xff)) {
...
static void DecodeTST(Word Index) {
    ...
    if (MomCPU == CPUHUC6280) {
        ...
        ImmVal = EvalStrIntExpressionOffs(
                &ArgStr[1], ImmStart(ArgStr[1].str.p_str), Int8, &OK);
        if (!OK) {
            return;
        }
...
static void DecodeCond(Word Index) {
    CondOrder const* pOrder = CondOrders + Index;

    if (ChkArgCnt(1, 1) && (ChkExactCPUMask(pOrder->CPUFlag, CPU6502) >= 0)) {
        Integer       AdrInt;
        Boolean       ValOK;
        tSymbolFlags  Flags;
        ...
        case 2:
            if (!MayShort) {
                WrError(ErrNum_InvAddrMode);
                    ...
                    ((AdrInt > 127) || (AdrInt < -128)) && !mSymbolQuestionable(Flags)) {
                WrError(ErrNum_JmpDistTooBig);
            } else {
...
static void DecodeTransfer(Word Code) {
    if (ChkArgCnt(3, 3) && ChkExactCPU(CPUHUC6280)) {
        Boolean OK;
        Word    Address;
        int     z;
...
static void AddFixed(char const* NName, Word NFlag, Byte NCode) {
...
static void AddNorm(
        char const* NName, LongWord ZACode, LongWord ACode, LongWord ZIXCode,
        LongWord IXCode, LongWord ZIYCode, LongWord IYCode, LongWord IndIXCode,
        LongWord IndOXCode, LongWord IndOYCode, LongWord IndOZCode, LongWord Ind16Code,
        LongWord ImmCode, LongWord AccCode, LongWord NoneCode, LongWord Ind8Code,
        LongWord IndSPYCode, LongWord SpecCode) {
    if (InstrZ >= NormOrderCount) {
        exit(255);
    }
    ...
    AddInstTable(
            InstTable, NName, InstrZ++, strcmp(NName, "TST") ? DecodeNorm : DecodeTST);
}

static void AddCond(char const* NName, Word NFlag, Byte NCodeShort, Byte NCodeLong) {
    if (InstrZ >= CondOrderCount) {
        exit(255);
    }
...
static void InitFields(void) {
    Boolean Is740 = (MomCPU == CPUM740), Is65C19 = (MomCPU == CPU65C19);
    int     Bit;
    char    Name[20];
    ...
    AddFixed(
            "RTS",
            M_6502 | M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_MELPS740
                    | M_HUC6280 | M_6502U,
            0x60);
    AddFixed(
            "RTI",
    ...
    AddFixed(
            ...
            0x28);
    AddFixed(
            "PHX", M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_HUC6280, 0xda);
    AddFixed(
            "PLX", M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_HUC6280, 0xfa);
    AddFixed(
            "PHY", M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_HUC6280, 0x5a);
    AddFixed(
            "PLY", M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_HUC6280, 0x7a);
    ...
    AddFixed(
            "SET", M_MELPS740 | M_HUC6280,
            Is740 ? 0x32
                  : 0xf4); /* !!! for HUC6280, is prefix for (x) instead of ACC as dest */
    ...
    AddNorm("NOP",
            ...
            /* ACC   */ -1,
            /* NON   */
            MkMask(M_6502 | M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19
                           | M_MELPS740 | M_HUC6280 | M_6502U,
                   0xea),
            /* (n8)  */ -1,
            /*(n,SP),y*/ -1,
            /* spec  */ -1);
    AddNorm("LDA",
            ...
            MkMask(M_6502 | M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19
                   ...
                   0xb9),
            /* (n,X) */
            MkMask(M_6502 | M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_MELPS740
                           | M_HUC6280 | M_6502U,
                   0xa1),
            ...
            /* NON   */ -1,
            /* (n8)  */
            MkMask(M_65SC02 | M_65C02 | M_W65C02S | M_65C19 | M_MELPS740 | M_HUC6280
                           | M_6502U,
                   Is65C19 ? 0xa1 : 0xb2),
    ...
    AddNorm("STA",
            ...
            /* NON   */ -1,
            /* (n8)  */
            MkMask(M_65SC02 | M_65C02 | M_W65C02S | M_65C19 | M_HUC6280,
                   Is65C19 ? 0x81 : 0x92),
    ...
    AddNorm("STZ",
            /* ZA    */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x64),
            /* A     */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x9c),
            /* ZIX   */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x74),
            /* IX    */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x9e),
            /* ZIY   */ -1,
            /* IY    */ -1,
            /* (n,X) */ -1,
            /* (n),X */ -1,
    ...
    AddNorm("BIT",
            ...
            MkMask(M_6502 | M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19
                   ...
                   0x2c),
            /* ZIX   */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x34),
            /* IX    */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x3c),
            /* ZIY   */ -1,
            /* IY    */ -1,
            /* (n,X) */ -1,
            /* (n),X */ -1,
            ...
            /* (n16) */ -1,
            /* imm   */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x89),
            /* ACC   */ -1,
            /* NON   */ -1,
            /* (n8)  */ -1,
            /*(n,SP),y*/ -1,
    ...
    AddNorm("TSB",
            /* ZA    */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x04),
            /* A     */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x0c),
            /* ZIX   */ -1,
            /* IX    */ -1,
            /* ZIY   */ -1,
            /* IY    */ -1,
    ...
    AddNorm("TRB",
            /* ZA    */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x14),
            /* A     */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_HUC6280, 0x1c),
            /* ZIX   */ -1,
            /* IX    */ -1,
            /* ZIY   */ -1,
            /* IY    */ -1,
    ...
    AddNorm("INC",
            ...
            /* imm   */ -1,
            /* ACC   */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_MELPS740 | M_HUC6280,
                   Is740 ? 0x3a : 0x1a),
            /* NON   */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_MELPS740 | M_HUC6280,
                   Is740 ? 0x3a : 0x1a),
    ...
    AddNorm("JMP",
            ...
            /* IY    */ -1,
            /* (n,X) */
            MkMask(M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_HUC6280, 0x7c),
            /* (n),X */ -1,
            /* (n),Y */ -1,
            /* (n),Z */ -1,
            /* (n16) */
    ...
    AddCond("BVS",
            ...
            0x70, 0x73);
    AddCond("BRA",
            M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_MELPS740 | M_HUC6280,
            0x80, 0x83);
    AddCond("BRU",
            M_65SC02 | M_65C02 | M_65CE02 | M_W65C02S | M_65C19 | M_MELPS740 | M_HUC6280,
            0x80, 0x83);
    AddCond("BSR", M_65CE02 | M_HUC6280, (MomCPU == CPUHUC6280) ? 0x44 : 0x00,
