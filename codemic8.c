...
static Boolean IsWRegCore(char const* pArg, LongWord* pValue) {
    Boolean OK;
...
static tRegEvalResult IsWReg(tStrComp const* pArg, LongWord* pValue, Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static void DecodeShortBranch(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (OK) {
            Dest -= EProgCounter();
            if (((Dest < -512) || (Dest > 511)) && !mSymbolQuestionable(Flags)) {
...
static void DecodeLongBranch(Word Index) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (OK) {
            Dest -= EProgCounter();
            if (((Dest < -2048) || (Dest > 2047)) && !mSymbolQuestionable(Flags)) {
                WrError(ErrNum_JmpDistTooBig);
            } else {
...
static void AddALU(char const* NName, char const* NImmName, LongWord NCode) {
...
static void AddShortBranch(char const* NName, LongWord NCode) {
...
static void AddLongBranch(char const* NName, LongWord NCode) {
...
static void AddMem(char const* NName, char const* NImmName, LongWord NCode, Byte NSpace) {
