...
static Boolean GetReg16(char* Asc, Byte* AdrPart) {
    int               z;
    static char const Reg16Names[4][3] = {"PC", "SP", "P2", "P3"};
...
static void DecodeAdr(int Index, Byte Mask, LongInt PCDelta) {
    ...
    if (Cnt == 1) {
        ...
        if (!OK) {
            WrStrErrorPos(ErrNum_InvReg, &ArgStr[Index + 1]);
        } else if ((Incr) && (AdrPart < 2)) {
            WrStrErrorPos(ErrNum_InvReg, &ArgStr[Index + 1]);
        } else {
            tSymbolFlags Flags;
            ...
            Addr = EvalStrIntExpressionOffsWithFlags(
                    &ArgStr[Index], Incr, Int16, &OK, &Flags);
            if (OK) {
                if (!AdrPart) {
...
static void DecodeSSM(Word Code) {
    ...
    case 0:
        ...
        break;
    case 1:
        if ((!GetReg16(ArgStr[1].str.p_str, BAsmCode + 0)) || (BAsmCode[0] < 2)) {
...
static void DecodeADDSUB(Word Index) {
    if (ChkArgCnt(1, 3)) {
        ...
        switch (AdrMode) {
        case ModAcc:
            /* EA <-> E implicitly give operand size conflict, we don't have to check it
             * here */
...
static void DecodeBranch(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if (AdrMode == ModMem) {
            if ((AdrPart == 1) || (AdrPart > 3)) {
                WrError(ErrNum_InvAddrMode);
            } else if ((Code < 0x60) && (AdrPart != 0)) {
                WrError(ErrNum_InvAddrMode);
            } else {
...
static void AddShift(char const* NName, Byte NCode, Byte NCode16) {
...
static void SwitchTo_807x(void) {
    ...
    MakeCode   = MakeCode_807x;
    IsDef      = IsDef_807x;
    SwitchFrom = SwitchFrom_807x;
    InitFields();
}
