...
static Boolean DecodeAdr(tStrComp const* pArg, Byte Mask, tAdrResult* pResult) {
    static char const RegNames[] = "XAHLDEBC";
    ...
    if (p) {
        ...
        if ((ArgLen == 3) && ((pArg->str.p_str[2] == '\'') || (pArg->str.p_str[2] == '`'))
            && (!Odd(pos))) {
            pResult->Part = pos + 1;
            if (SetOpSize(1)) {
    ...
    if (*pArg->str.p_str == '@') {
        ...
        if (!as_strcasecmp(Arg.str.p_str, "HL")) {
            pResult->Part = 1;
        } else if (!as_strcasecmp(Arg.str.p_str, "HL+")) {
            pResult->Part = 2;
        } else if (!as_strcasecmp(Arg.str.p_str, "HL-")) {
            pResult->Part = 3;
        } else if (!as_strcasecmp(Arg.str.p_str, "DE")) {
            pResult->Part = 4;
        } else if (!as_strcasecmp(Arg.str.p_str, "DL")) {
            pResult->Part = 5;
        } else {
            pResult->Part = 0;
        }
...
static void DissectBit_75K0(char* pDest, size_t DestSize, LargeWord Inp) {
    if (Hi(Inp)) {
        ...
        as_snprintf(
                pDest, DestSize, "%03.*u%s.%c", ListRadixBase,
                (unsigned)(((Inp >> 4) & 0xf00) + Lo(Inp)),
                GetIntConstIntelSuffix(ListRadixBase), '0' + (char)(Hi(Inp) & 3));
    } else {
        switch ((Inp >> 6) & 3) {
        case 0:
            as_snprintf(
                    pDest, DestSize, "@%c+%0.*u%s.%c", HexStartCharacter + ('H' - 'A'),
                    ListRadixBase, (unsigned)(Inp & 0x0f),
                    GetIntConstIntelSuffix(ListRadixBase), '0' + (char)((Inp >> 4) & 3));
            break;
        case 1:
            // Original format string on the next line was "%~03.*u%s.@%c"
            // Trying without the non-standard '~' to see if there is a difference.
            as_snprintf(
                    pDest, DestSize, "%03.*u%s.@%c", ListRadixBase,
                    (unsigned)(0xfc0 + ((Inp & 0x0f) << 2)),
                    GetIntConstIntelSuffix(ListRadixBase),
        ...
        case 2:
            ...
            as_snprintf(
                    pDest, DestSize, "%03.*u%s.%c", ListRadixBase,
                    (unsigned)(0xfb0 + (Inp & 15)), GetIntConstIntelSuffix(ListRadixBase),
                    '0' + (char)((Inp >> 4) & 3));
            break;
        case 3:
            // Original format string on the next line was "%~03.*u%s.%c"
            ...
            as_snprintf(
                    pDest, DestSize, "%03.*u%s.%c", ListRadixBase,
                    (unsigned)(0xff0 + (Inp & 15)), GetIntConstIntelSuffix(ListRadixBase),
                    '0' + (char)((Inp >> 4) & 3));
            break;
        }
    }
...
static Boolean DecodeBitAddr(tStrComp const* pArg, Word* Erg, tEvalResult* pEvalResult) {
    char*    p;
    int      Num;
    Boolean  OK;
    Word     Adr;
    ...
    if (!as_strcasecmp(BitPart.str.p_str, "@L")) {
        ...
        if (OK) {
            if (!as_strncasecmp(AddrPart.str.p_str, "@H", 2)) {
                Adr = EvalStrIntExpressionOffsWithResult(
                        &AddrPart, 2, UInt4, pEvalResult);
                if (pEvalResult->OK) {
...
static Boolean DecodeIntName(char* Asc, Byte* Erg) {
    ...
    if (!strcmp(Asc, "IEBT")) {
        HErg = 0x000;
    } else if (!strcmp(Asc, "IEW")) {
        HErg = 0x102;
    } else if (!strcmp(Asc, "IETPG")) {
        HErg = 0x203;
    } else if (!strcmp(Asc, "IET0")) {
        HErg = 0x104;
    } else if (!strcmp(Asc, "IECSI")) {
        HErg = 0x005;
    } else if (!strcmp(Asc, "IECSIO")) {
        HErg = 0x205;
    } else if (!strcmp(Asc, "IE0")) {
        HErg = 0x006;
    } else if (!strcmp(Asc, "IE2")) {
        HErg = 0x007;
    } else if (!strcmp(Asc, "IE4")) {
        HErg = 0x120;
    } else if (!strcmp(Asc, "IEKS")) {
        HErg = 0x123;
    } else if (!strcmp(Asc, "IET1")) {
        HErg = 0x224;
    } else if (!strcmp(Asc, "IE1")) {
        HErg = 0x126;
    } else if (!strcmp(Asc, "IE3")) {
        HErg = 0x227;
    } else {
        HErg = 0xfff;
    }
...
static void DecodeMOV(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg4: {
            ...
            case ModReg4:
                if (DestResult.Part == 0) {
                    PutCode(0x7899 + (((Word)SrcResult.Part) << 8));
                    CheckCore(eCore004);
                } else if (SrcResult.Part == 0) {
                    PutCode(0x7099 + (((Word)DestResult.Part) << 8));
                    CheckCore(eCore004);
                } else {
                    WrError(ErrNum_InvAddrMode);
                }
            ...
            case ModAbs:
                if (DestResult.Part != 0) {
                    ...
                    BAsmCode[0] = 0xa3;
                    BAsmCode[1] = SrcResult.Part;
                    CodeLen     = 2;
                }
                break;
            case ModImm:
                if (DestResult.Part == 0) {
                    PutCode(0x70 + SrcResult.Part);
                } else {
                    PutCode(0x089a + (((Word)SrcResult.Part) << 12)
                            + (((Word)DestResult.Part) << 8));
                    CheckCore(eCore004);
                }
                break;
            }
        ...
        case ModReg8: {
            ...
            case ModReg8:
                if (DestResult.Part == 0) {
                    PutCode(0x58aa + (((Word)SrcResult.Part) << 8));
                    CheckCore(eCore004);
                } else if (SrcResult.Part == 0) {
                    PutCode(0x50aa + (((Word)DestResult.Part) << 8));
                    CheckCore(eCore004);
                } else {
                    WrError(ErrNum_InvAddrMode);
                }
            ...
            case ModAbs:
                if (DestResult.Part != 0) {
                    ...
                    BAsmCode[1] = SrcResult.Part;
                    CodeLen     = 2;
                    if (!mFirstPassUnknown(SrcResult.PartSymFlags)
                        && Odd(SrcResult.Part)) {
                ...
                break;
            case ModInd:
                if ((DestResult.Part != 0) || (SrcResult.Part != 1)) {
                    WrError(ErrNum_InvAddrMode);
                } else {
                    PutCode(0x18aa);
                    CheckCore(eCore004);
                ...
                break;
            case ModImm:
                if (Odd(DestResult.Part)) {
                    WrError(ErrNum_InvAddrMode);
                } else {
                    BAsmCode[0] = 0x89 + DestResult.Part;
                    BAsmCode[1] = SrcResult.Part;
                    CodeLen     = 2;
                }
                break;
...
static void DecodeXCH(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg4: {
            ...
            case ModReg4:
                if (DestResult.Part == 0) {
                    PutCode(0xd8 + SrcResult.Part);
                } else if (SrcResult.Part == 0) {
                    PutCode(0xd8 + DestResult.Part);
                } else {
                    WrError(ErrNum_InvAddrMode);
                }
                break;
        ...
        case ModReg8: {
            ...
            case ModReg8:
                if (DestResult.Part == 0) {
                    PutCode(0x40aa + (((Word)SrcResult.Part) << 8));
                    CheckCore(eCore004);
                } else if (SrcResult.Part == 0) {
                    PutCode(0x40aa + (((Word)DestResult.Part) << 8));
                    CheckCore(eCore004);
                } else {
                    WrError(ErrNum_InvAddrMode);
                }
        ...
        case ModInd: {
            ...
            case ModReg4:
                if (SrcResult.Part != 0) {
                    ...
                    PutCode(0xe8 + DestResult.Part);
                }
                break;
            case ModReg8:
                if ((SrcResult.Part != 0) || (DestResult.Part != 1)) {
                    WrError(ErrNum_InvAddrMode);
                } else {
                    PutCode(0x11aa);
                    CheckCore(eCore004);
...
static void DecodeMOVT(Word Code) {
    ...
    if (!ChkArgCnt(2, 2))
        ;
    else if (as_strcasecmp(ArgStr[1].str.p_str, "XA")) {
        WrError(ErrNum_InvAddrMode);
...
static void DecodeIN_OUT(Word IsIN) {
    if (ChkArgCnt(2, 2)) {
        tStrComp const *pPortArg = IsIN ? &ArgStr[2] : &ArgStr[1],
                       *pRegArg  = IsIN ? &ArgStr[1] : &ArgStr[2];

        if (as_strncasecmp(pPortArg->str.p_str, "PORT", 4)) {
            WrError(ErrNum_InvAddrMode);
        } else {
            Boolean OK;
...
static void DecodeADDS(Word Code) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg8:
            ...
            } else if (as_strcasecmp(ArgStr[2].str.p_str, "XA")) {
                WrError(ErrNum_InvAddrMode);
            } else {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModReg4:
            if (Result.Part != 0) {
                ...
                switch (Result.Mode) {
                case ModImm:
                    PutCode(0x2099 + (((Word)Result.Part & 15) << 8)
                            + ((Code + 1) << 12));
                    CheckCore(eCore004);
                    break;
                case ModInd:
...
static void DecodeINCS(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        case ModReg4:
            ...
            break;
        case ModReg8:
            if ((Result.Part < 1) || (Odd(Result.Part))) {
                WrError(ErrNum_InvAddrMode);
            } else {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModInd:
            if (DestResult.Part != 1) {
                ...
                DecodeAdr(
                        &ArgStr[2], MModImm | MModReg4 | MModReg8 | MModMinOneIs0,
                        &SrcResult);
                switch (SrcResult.Mode) {
                case ModImm:
...
static void DecodeBR(Word Code) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        AdrInt = EvalStrIntExpressionOffsWithResult(
                &ArgStr[1], Offset, UInt16, &EvalResult);
        if (EvalResult.OK) {
            Dist = AdrInt - EProgCounter();
            if ((BrRel) || ((Dist <= 16) && (Dist >= -15) && (Dist != 0))) {
                if (Dist > 0) {
                    Dist--;
                    if ((Dist > 15) && !mSymbolQuestionable(EvalResult.Flags)) {
                    ...
                    (!BrLong) && (PageNum(AdrInt) == PageNum(EProgCounter()))
                    && ((EProgCounter() & 0xfff) < 0xffe)) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        if (EvalResult.OK) {
            if (!ChkSamePage(AdrInt, EProgCounter(), 12, EvalResult.Flags))
                ;
            else if ((EProgCounter() & 0xfff) >= 0xffe) {
...
static void DecodeCALL(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        ...
        tEvalResult EvalResult;
        Integer     AdrInt = EvalStrIntExpressionOffsWithResult(
                &ArgStr[1], BrLong, UInt16, &EvalResult);
        if (mFirstPassUnknown(EvalResult.Flags)) {
            AdrInt &= 0x7ff;
        }
        if (EvalResult.OK) {
...
static void DecodeCALLF(Word Code) {
    ...
    if (ChkArgCnt(1, 1)) {
        tEvalResult EvalResult;
        Integer     AdrInt = EvalStrIntExpressionOffsWithResult(
                &ArgStr[1], !!(*ArgStr[1].str.p_str == '!'), UInt11, &EvalResult);
        if (EvalResult.OK) {
...
static void AddFixed(char const* NewName, Word NewCode) {
