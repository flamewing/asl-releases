...
static CPUVar CPU87C750, CPU8051, CPU8052, CPU80C320, CPU80501, CPU80502, CPU80504,
        CPU80515, CPU80517, CPU80C390, CPU80251, CPU80251T;

static PRelocEntry AdrRelocInfo, BackupAdrRelocInfo;
static LongWord    AdrOffset, AdrRelocType, BackupAdrOffset, BackupAdrRelocType;
...
static Boolean DecodeRegCore(char const* pAsc, tRegInt* pValue, tSymbolSize* pSize) {
    ...
    if ((alen >= 2) && (as_toupper(*pAsc) == 'R')) {
        ...
        *pSize = eSymbolSize8Bit;
            ...
            (MomCPU >= CPU80251) && (alen >= 3) && (as_toupper(*pAsc) == 'W')
            && (as_toupper(pAsc[1]) == 'R')) {
        Start  = pAsc + 2;
        *pSize = eSymbolSize16Bit;
    } else if (
            (MomCPU >= CPU80251) && (alen >= 3) && (as_toupper(*pAsc) == 'D')
            && (as_toupper(pAsc[1]) == 'R')) {
        Start  = pAsc + 2;
        *pSize = eSymbolSize32Bit;
    } else {
...
static tRegEvalResult DecodeReg(
        tStrComp const* pArg, Byte* pValue, tSymbolSize* pSize, Boolean MustBeReg) {
    tRegDescr      RegDescr;
...
static void TransferBackupAdrRelocs(LargeWord Offset) {
    TransferRelocs2(
            BackupAdrRelocInfo, ProgCounter() + BackupAdrOffset + Offset,
            BackupAdrRelocType);
    AdrRelocInfo = NULL;
}
...
static void DecodeAdr(tStrComp* pArg, Word Mask) {
    ...
    if ((SegType == -2)
        || ((SegType == -1) && (EvalResult.AddrSpaceMask & (1 << SegIO)))) {
        ...
        if (((H32 < 128) || ((H32 < 256) && (MomCPU < CPU80251)))
            && ((Mask & MModDir8) != 0)) {
            if (MomCPU < CPU80251) {
            ...
            AdrVals[0] = H32 & 0xff;
            AdrCnt     = 1;
        } else if ((MomCPU < CPU80251) || (H32 > 0xffff)) {
...
static void DissectBit_251(char* pDest, size_t DestSize, LargeWord Inp) {
    ...
    as_snprintf(
            pDest, DestSize, "%02.*u%s.%u", ListRadixBase, (unsigned)(Inp & 0xff),
            GetIntConstIntelSuffix((unsigned)ListRadixBase), (unsigned)(Inp >> 24));
}
...
static ShortInt DecodeBitAdr(tStrComp* pArg, LongInt* Erg, Boolean MayShorten) {
    ...
    if (MomCPU < CPU80251) {
        ...
        if (!pPos) {
            static LongWord const ValidBits = 0x070000fful;
...
static Boolean IsCarry(char const* pArg) {
...
static void DecodeMOV(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        case ModBit251:
            ...
            (!as_strcasecmp(ArgStr[2].str.p_str, "C"))
            || (!as_strcasecmp(ArgStr[2].str.p_str, "CY"))) {
        ...
        DecodeAdr(
                &ArgStr[1], MModAcc | MModReg | MModIReg8 | MModIReg | MModInd | MModDir8
                                    | MModDir16);
        switch (AdrMode) {
        case ModAcc:
            DecodeAdr(
                    &ArgStr[2], MModReg | MModIReg8 | MModIReg | MModInd | MModDir8
                                        | MModDir16 | MModImm);
            switch (AdrMode) {
            case ModReg:
...
static void DecodeLogic(Word Index) {
    ...
    if (!ChkArgCnt(2, 2))
        ...
        case ModAcc:
            DecodeAdr(
                    &ArgStr[2],
                    MModReg | MModIReg8 | MModIReg | MModDir8 | MModDir16 | MModImm);
            switch (AdrMode) {
            case ModReg:
                if ((AdrPart < 8) && (!SrcMode)) {
                    PutCode(z + 8 + AdrPart);
...
static void DecodeMOVH(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU80251)) {
...
static void DecodeMOVX(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        if ((!as_strcasecmp(ArgStr[2].str.p_str, "A"))
            || ((MomCPU >= CPU80251) && (!as_strcasecmp(ArgStr[2].str.p_str, "R11")))) {
            z = 0x10;
...
#define RelocTypeABranch11 \
    (11 | RelocFlagBig | RelocFlagPage | (5 << 8) | (3 << 12)) | (0 << 16)
#define RelocTypeABranch19 \
    (19 | RelocFlagBig | RelocFlagPage | (5 << 8) | (3 << 12)) | (0 << 16)
...
static void DecodeLBranch(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        tEvalResult EvalResult;
        LongInt     AdrLong = EvalStrIntExpressionWithResult(
                &ArgStr[1], (MomCPU < CPU80C390) ? Int16 : Int24, &EvalResult);
        ...
        if (EvalResult.OK) {
            ...
            if (MomCPU == CPU80C390) {
                ...
                if ((MomCPU >= CPU80251)
                    && !ChkSamePage(EProgCounter() + 3, AdrLong, 16, EvalResult.Flags))
                    ;
                else {
                    PutCode(0x02 + (Index << 4));
                    BAsmCode[CodeLen++] = (AdrLong >> 8) & 0xff;
        ...
        if (OK) {
            ...
            if ((Dist <= 127) && (Dist >= -128)) {
                ...
                BAsmCode[CodeLen++] = Dist & 0xff;
                    ...
                    (!Chk504(EProgCounter()))
                    && ((AdrLong >> 11) == ((((long)EProgCounter()) + 2) >> 11))) {
                PutCode(0x01 + ((Hi(AdrLong) & 7) << 5));
                BAsmCode[CodeLen++] = Lo(AdrLong);
            } else if (MomCPU < CPU8051) {
...
static void DecodeADD(Word Index) {
    ...
    if (ChkArgCnt(2, 2)) {
        ...
        case ModAcc:
            DecodeAdr(
                    &ArgStr[2],
                    MModImm | MModDir8 | MModDir16 | MModIReg8 | MModIReg | MModReg);
            switch (AdrMode) {
            case ModImm:
                PutCode(0x24);
                TransferAdrRelocs(CodeLen);
        ...
        case ModReg:
            if (ChkMinCPUExt(CPU80251, ErrNum_AddrModeNotSupported)) {
                ...
                DecodeAdr(
                        &ArgStr[2],
                        MModImm | MModReg | MModDir8 | MModDir16 | MModIReg8 | MModIReg);
                switch (AdrMode) {
                case ModImm:
                    if ((OpSize == 0) && (HReg == AccReg)) {
                        PutCode(0x24);
...
static void DecodeSUBCMP(Word Index) {
    ...
    if (ChkArgCnt(2, 2) && ChkMinCPU(CPU80251)) {
        ...
        case ModReg:
            HReg = AdrPart;
            DecodeAdr(
                    &ArgStr[2], MModImm | MModReg | MModDir8 | MModDir16 | MModIReg
                                        | (Index ? MModImmEx : 0));
            switch (AdrMode) {
            case ModImm:
                PutCode(0x10e + z);
...
static void DecodeINCDEC(Word Index) {
    ...
    if (!ChkArgCnt(1, 2))
        ;
    else if ((ArgCnt == 2) && (*ArgStr[2].str.p_str != '#')) {
        WrError(ErrNum_InvAddrMode);
    } else {
        if (1 == ArgCnt) {
            HReg  = 1;
                ...
                case ModReg:
                    if ((OpSize == 0) && (AdrPart == AccReg) && (HReg == 0)) {
                        PutCode(0x04 + z);
                            ...
                            (AdrPart < 8) && (OpSize == 0) && (HReg == 0) && (!SrcMode)) {
...
static void DecodeMULDIV(Word Index) {
    ...
    if (!ChkArgCnt(1, 2))
        ...
        if (as_strcasecmp(ArgStr[1].str.p_str, "AB")) {
            WrError(ErrNum_InvAddrMode);
        } else {
            PutCode(0x84 + z);
        }
...
static void DecodeShift(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(CPU80251)) {
        z = Index << 4;
        DecodeAdr(&ArgStr[1], MModReg);
        switch (AdrMode) {
        case ModReg:
...
static void DecodeCond(Word Index) {
    ...
    if (ChkArgCnt(1, 1) && ChkMinCPU(FixedZ->MinCPU)) {
        tEvalResult EvalResult;
        LongInt AdrLong = EvalStrIntExpressionWithResult(&ArgStr[1], UInt24, &EvalResult);
...
static void DecodeFixed(Word Index) {
    ...
    if (ChkArgCnt(0, 0) && ChkMinCPU(FixedZ->MinCPU)) {
        PutCode(FixedZ->Code);
    }
}
...
static void DecodeSFR(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        AdrByte = EvalStrIntExpressionWithFlags(
                &ArgStr[1], (MomCPU >= CPU80251) ? UInt9 : UInt8, &OK, &Flags);
        if (OK && !mFirstPassUnknown(Flags)) {
            ...
            if (Memo("SFRB")) {
                ...
                if (AdrByte > 0x7f) {
                    ...
                    if ((AdrByte & 0xe0) != 0x20) {
                        WrError(ErrNum_NotBitAddressable);
                    }
                    BitStart = (AdrByte - 0x20) << 3;
                }
                if (MakeUseList) {
                    if (AddChunk(SegChunks + SegBData, BitStart, 8, False)) {
                        WrError(ErrNum_Overlap);
                    }
                }
                ...
                // Trying without the non-standard '~' to see if there is a difference.
                as_snprintf(
                        ListLine, STRINGSIZE, "=%02.*u%s-%02.*u%s", ListRadixBase,
                        (unsigned)BitStart,
                        GetIntConstIntelSuffix((unsigned)ListRadixBase), ListRadixBase,
                        (unsigned)BitStart + 7,
                        GetIntConstIntelSuffix((unsigned)ListRadixBase));
            } else {
                // Original format string on the next line was "=%~02.*u%s"
                // Trying without the non-standard '~' to see if there is a difference.
                as_snprintf(
                        ListLine, STRINGSIZE, "=%02.*u%s", ListRadixBase,
                        (unsigned)AdrByte,
                        GetIntConstIntelSuffix((unsigned)ListRadixBase));
            }
            LimitListLine();
...
static void DecodeBIT(Word Index) {
    ...
    if (!ChkArgCnt(1, 1))
        ...
        if (DecodeBitAdr(&ArgStr[1], &AdrLong, False) == ModBit51) {
            ...
            // Trying without the non-standard '~' to see if there is a difference.
            as_snprintf(
                    ListLine, STRINGSIZE, "=%02.*u%s", ListRadixBase, (unsigned)AdrLong,
                    GetIntConstIntelSuffix((unsigned)ListRadixBase));
            LimitListLine();
        }
    }
...
static void AddAcc(char const* NName, Word NCode, CPUVar NCPU) {
...
static void AddCond(char const* NName, Word NCode, CPUVar NCPU) {
    if (InstrZ >= CondOrderCnt) {
        exit(255);
    }
...
static void AddBCond(char const* NName, Word NCode, CPUVar NCPU) {
    if (InstrZ >= BCondOrderCnt) {
        exit(255);
    }
...
static void SwitchTo_51(void) {
    ...
    /* rest of the pack... */
        ...
        ValidSegs = (1 << SegCode) | (1 << SegData) | (1 << SegIData) | (1 << SegXData)
                    | (1 << SegBData);
